...
本章旨在广泛概述 HID 报告描述符是什么，以及一个普通的（非内核）程序员如何处理Linux
下工作不正常HID 设备
...
许多 HID 设备开箱即用（out of the box），即使它们的硬件各不相同。例如，鼠标可以有任
数量的按钮；它们可能有一个滚轮；不同型号之间的移动灵敏度不同，等等。尽管如此，大多
时候一切都能正常工作，而无需为自 1970 年以来开发的每个鼠标型号在内核中编写专门的代码

这是因为现代 HID 设备确实通过**HID 报告描述*（HID report descriptor）——一
固定的字节，精确描述设备与主机之间可以发送哪**HID 报告**（HID reports）以及这
报告中每个单独位的含义——来声明其能力。例如，一HID 报告描述符可以指“在 ID 3
的报告中，第 8 15 位是鼠标X 增量坐标”

HID 报告本身随后仅携带实际数据值，没有任何额外的元信息。请注意，HID 报告可以从设
发出Input Reports"，即输入事件）、发往设备Output Reports"，例如用于改LED）或
用于设备配置Feature reports"）。一个设备可以支持一个或多个 HID 报告

HID 子系统负责解HID 报告描述符，并将 HID 事件转换为正常的输入设备接口（参
Documentation/hid/hid-transport.rst）。设备可能行为异常，原因包括设备提供HID 报告
...
HID 子系统可以处理不同的传输（transport）驱动（USB、I2C、Bluetooth 等）。参
Documentation/hid/hid-transport.rst銆。

## 解析 HID 报告描述


当前 HID 设备的列表可`/sys/bus/hid/devices/` 找到。对于每个设备，例如
...
HID 报告描述符的基本结构HID 规范中定义，HUT “定义了一组常量，可供应用程序解释
识别 HID 报告中数据字段的用途和含义”。每个条目至少由两字节定义，其中第一个字节定
后面跟随的值的类型，并HID 规范中描述；第二个字节携带实际值，并在 HUT 中描述

原则上，HID 报告描述符可以逐字节地、费力地手工解析

关于如何做到这一点的简短介绍概述于 Documentation/hid/hidreport-parsing.rst；只有当
需要修补（patch）HID 报告描述符时才需要理解它

在实践中，你不应手工解析 HID 报告描述符；相反，你应当使用现有的解析器。在所有可用的
解析器中

  - 在线`USB Descriptor and Request Parser
  ...
  - `hid-tools <https://gitlab.freedesktop.org/libevdev/hid-tools>`_锛。
    一套完整的实用工具集，除其它功能外，允许记录和回放原始HID 报告，以及调试和回放
    HID 设备。它正由 Linux HID 子系统维护者积极开发

`hid-tools <https://gitlab.freedesktop.org/libevdev/hid-tools>`_ 解析鼠标HID 报告
描述符得到：
```
...
```
    ...
    I: 3 0001 0001
```
这个报告描述符告诉我们，鼠标输入将使用四个字节传输：第一个字节用于按钮（使用三位，五
用于填充），最后三个字节分别用于鼠标的 X、Y 和滚轮变化

实际上，对于任何事件，鼠标都会发送一个四字节**report**。我们可以通过例如借助来自
`hid-tools <https://gitlab.freedesktop.org/libevdev/hid-tools>`_ `hid-recorder` 工具
来检查发送的值：
```
...
```
这个例子表明，当点击按钮 2 时，会发送字`02 00 00 00`，而紧随其后的事件
...
信息被发往设备。例如，带有力反馈（force feedback）的操纵杆会有某些输出；键盘LED 
需要输出。“Input意味着数据来自设备

“Feature并非供最终用户消费，而是定义设备的配置选项。它们可以从主机查询；当声明
**Volatile**（易变）时，它们应由主机更改


## 集合（Collections）、报ID Evdev 事件


单个设备可以在逻辑上将数据分组到不同的独立集合中，称为 **Collection**（集合）。集合可
嵌套，并且存在不同类型的集合（详HID 规范 6.2.2.6 “Collection, End Collection
Items”）

不同的报告通过不同**Report ID**（报ID）字段来标识，即一个用于标识紧随其后的报告
结构编号。每当需Report ID 时，它都作为任何报告的第一个字节传输。一个只支持单个 HID
报告的设备（如上面的鼠标示例）可以省略报ID
...
```
在解析它之后（试着用建议的工具自己解析！）可以看到，该设备呈现了两`Mouse` 应用集合
（分别由报告 ID 1 2 标识）、一`Keypad` 应用集合（其报告由报ID 5 标识）以及两
`Consumer Controls` 应用集合（分别由报告 ID 6 3 标识）。但请注意，一个设备可以针
同一个应用集合使用不同的报告 ID
...
```
  ...
  0xC0,              // End Collection
```
将是三个字节：第一个字节是报告 ID），接下来的两个字节用于耳机，各为两
（`Report Count (2)`）字节（`Report Size (8)`），每个字节的范围从 0（`Logical Minimum
(0)`）到 255（`Logical Maximum (255)`）

设备发送的所Input 数据都应被转换为相应Evdev 事件，以便协议栈的其余部分能够知
发生了什么，例如第一个按钮的位转换为 `EV_KEY/BTN_LEFT` evdev 事件，相对的 X 移动转换
`EV_REL/REL_X` evdev 事件

## 事件


Linux 中，会为每个 ``Application Collection`` 创建一`/dev/input/event*`。回到鼠标的
...
- 设备提供HID 报告描述符可能是错误的，例如

  - 它不遵循标准，因此内核将无法理解HID 报告描述符；
  - HID 报告描述**与实* 设备发送的内容不匹配（这可以通过读取原始 HID 数据来验证）
- HID 报告描述符可能需要一“quirks”（怪癖，见后文）

因此，可能不会为每个应用集合创建 `/dev/input/event*`，并或者其中的事件可能不符
你的预期
...
内核知道如何修复HID 设备有一些已知的特性——这些被称为 HID quirks，其列表可在
`include/linux/hid.h` 中找到

如果是这种情况，对于手头HID 设备，只需在内核中添加所需quirk 即可。这可以
`drivers/hid/hid-quirks.c` 文件中完成。在查看该文件后，如何做应该相对直观

当前定义quirks 列表（来`include/linux/hid.h`）是

   :doc: HID quirks

USB 设备quirks 可以在加usbhid 模块时指定，参见 `modinfo usbhid`，但正确的修
应当进入 hid-quirks.c **提交到上游（be submitted upstream*。关于如何提交补丁的指南
请参Documentation/process/submitting-patches.rst。其它总线quirks 需要进
hid-quirks.c銆。

### 修补 HID 报告描述


如果你需要修HID 报告描述符，最简单的方法是求助于 eBPF，如 Documentation/hid/hid-bpf.rst
中所述

基本上，你可以更改原HID 报告描述符的任何字节。samples/hid 中的示例应该是一个很好的
起点
```
...
```
当然，这也可以在内核源码中完成，例如参`drivers/hid/hid-aureal.c` 
`drivers/hid/hid-samsung.c` 以获得稍微复杂一些的文件

如果你在查阅 HID 手册和理HID 报告描述符十六进制数字的确切含义方面需要任何帮助，
查阅 Documentation/hid/hidreport-parsing.rst

无论你想出什么解决方案，请记**将修复提交给 HID 维护*，以便它能直接整合进内核，使
那个特定HID 设备能对所有其他人正常工作。关于如何做到这一点的指南，请参见
...
使用 eBPF 还可以修改与设备交换的数据。再次参samples/hid 中的示例

同样地，**请发布你的修*，以便它能整合进内核
...
例如可参`samples/hidraw/hid-example.c` 文件
```
    $ sudo ./hid-example
