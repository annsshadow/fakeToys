...
为了支持设备特定的配置，使用了一个特殊结构 ‘lpxx_device_config’。
...
在 LP5523 的情况下，还需要一个命令 ‘enginex_leds’。它用于在每个引擎号上选择 LED
...
```
一旦 ‘loading’ 被设为 0，注册的回调就会被调用。在回调内部，所选引擎被加载，存储器
被更新。要运行已编程的模式，应启用 ‘run_engine’ 属性。
...
```
（‘run_engine’ 与 ‘firmware_cb’）

运行程序数据的顺序是通用的。

但每个设备都有自己特定的命令寄存器地址。

为此，‘run_engine’ 和 ‘firmware_cb’ 在每个驱动中是可配置的。
...
如果没有固件接口，LP55xx 驱动提供另一种加载 LED 模式的方法。那就是“预定义”模式。
