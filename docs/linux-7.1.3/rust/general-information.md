...
本文档包含在从事内核Rust 支持相关工作时有用的一些信息

### ``no_std``


内核中的 Rust 支持只能链接 `core <https://doc.rust-lang.org/core/>`_而不能链`std <https://doc.rust-lang.org/std/>`_。供内核使用crate 必须
使用 `#![no_std]` 属性来选择这种行为

### 代码文档


内核Rust 代码使用其内置的文档生成`rustdoc` 来编写文档
生成HTML 文档包含集成搜索、链接项（例如类型、函数、常量）、源代码等可以在以下地址阅读
...
虽然 `rustc` 是一个很有帮助的编译器，但还可以通过 Rust lint 工具
...
例如，可以在 Rust 中编写一`Mutex` 抽象，它包装来自 C 侧的
`struct mutex` 并通过绑定调用其函数
并非所有内核内API 和概念都有可用的抽象，但我们有意随着时间推移扩大
覆盖范围。“叶”（Leaf）模块（例如驱动）不应直接使C 绑定。相反，子系应视需要提供尽可能安全的抽象
...
对绑定的不安全访问封装到它们暴露给使用者的、尽可能安全API 中。抽象的
使用者包括用 Rust 编写的驱动或文件系统等
除了安全性方面，抽象还应当是“符合人体工学的”（ergonomic），也就是说它们
C 接口转变为“惯用的”（idiomatic）Rust 代码。基本的例子包括C 的资获取与释放转变为 Rust 的构造函数与析构函数，或C 的整数错误码转变Rust `Result`\ s
...
Rust 代码可以基于内核配置进行条件编译
	...
	#[cfg(not(CONFIG_X))]  // 已禁
对于 Rust `cfg` 不支持的其他谓词（例如带有数值比较的表达式），可定义一个新Kconfig 符号
