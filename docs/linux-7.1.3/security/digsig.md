...
```
keyid 等于对密钥整体内容计SHA1[12-19] 的结果签名头被用作生成签名的输入这种方式确保了密钥或签名头无法被更改它保护时间戳不被修改，可用于回滚保护
## API
...
```
	...
	* @data:	数据
	* @datalen:	数据长度
	* @return:	成功返回 0，否则返-EINVAL
	*
	* 针对数字签名验证数据完整性	* 目前仅支RSA	* 通常将内容的哈希作为该函数的数据使用	*
	*/
...
用于签名与密钥管理的工具 evm-utils 提供了生成签名、将密钥加载到内keyring
的功能。密钥可以是 PEM 格式，也可以转换为内核格式。当密钥被加入内keyring
