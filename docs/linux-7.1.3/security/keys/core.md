...
其它密钥的链接。每个进程有三个标准的密钥环订阅，内核服务可以
在其中搜索相关密钥。
...
每个密钥具有若干属性：
 ...
 - 状态。
  ...
  - 每个密钥都被分配一个 key_serial_t 类型的序列号，该序列号在
     该密钥的生命周期内是唯一的。所有序列号都是正的非零 32 位
     整数。
  ...
  - 每个密钥属于一种已定义的“type”。类型必须先在内核中
     ...
     密钥类型在内核中由 struct key_type 表示。它定义了一组
     可对该类型密钥执行的操作。

     如果某个类型从系统中移除，该类型的所有密钥都将被
     置为无效。

  - 每个密钥都有描述。它应当是一个可打印字符串。密钥
     类型提供一个操作，用于在密钥的描述与
     一个条件字符串之间进行匹配。

  - 每个密钥都有一个属主用户 ID、一个组 ID 和一个权限掩码。它们
  ...
  - 每个密钥可以处于以下几种基本状态之一：
      ...
      - 已实例化（Instantiated）。这是正常状态。密钥已完全成型，并且
	 已附加数据。

      - 负向（Negative）。这是一个相对短暂的状态。密钥充当一个
	 标记，表示之前对用户空间的回调失败，并作为
	 密钥查找的节流器。负向密钥可以被更新为正常
	 状态。

      - 已过期（Expired）。密钥可以设置生命周期。如果超出其生命周期，
	 它们会进入此状态。已过期的密钥可以被更新回
	 正常状态。

      - 已撤销（Revoked）。密钥由用户空间操作进入此状态。它不能被
	 找到或操作（除了将其取消链接）。

      - 已废弃（Dead）。密钥的类型已被注销，因此该密钥现在已无用。

处于后三种状态的密钥会受到垃圾回收。参见
“垃圾回收”一节。
...
除了密钥之外，密钥服务还提供许多特性：
  ...
  - 密钥服务定义了三种特殊的密钥类型：
	 ...
	 与“user”密钥类似，“logon”密钥的载荷也是任意的
	 数据块。它旨在作为存储密钥（secret）的地方，这些密钥
	 可供内核访问，但用户空间程序无法访问。

	 描述可以是任意的，但必须带有一个非零长度的前缀字符串，
	 用于描述密钥的“subclass”。subclass
	 通过“:”与其余描述分隔。“logon”密钥可以
	 由用户空间创建和更新，但其载荷只能
	 从内核空间读取。
  ...
  - 每个进程订阅三个密钥环：线程专属密钥环、
     ...
     在发生任何形式的 clone、fork、vfork 或 execve 时，线程专属密钥环会从子进程中
     丢弃。只有在需要时才会创建新的密钥环。
     需要时才创建。

     在子进程中，进程专属密钥环会在
     clone、fork、vfork 时被一个空的替换，除非提供了 CLONE_THREAD，
     此时它会被共享。execve 也会丢弃进程的进程密钥环，并创建
     一个新的。

     会话专属密钥环在 clone、fork、vfork 和
     execve 过程中保持不变，即使后者执行了 set-UID 或 set-GID 二进制文件。但
     进程可以用一个新的密钥环替换其当前会话密钥环，
     方法是使用 PR_JOIN_SESSION_KEYRING。允许请求一个匿名的
  ...
  - 每个用户有两个配额，用于跟踪其拥有的密钥。一个
     ...
     如果某个以某种方式修改密钥或密钥环的系统调用会导致
     用户超出配额，该操作会被拒绝并返回错误 EDQUOT。
...
密钥具有一个属主用户 ID、一个组访问 ID，以及一个权限掩码。该掩码
为持有者、用户、组和其它访问各提供最多 8 位。只有
每组 8 位中的 6 位被定义。所授予的权限如下：
...
要更改属主、组 ID 或权限掩码，成为密钥的
属主或具有 sysadmin 能力（capability）即可。
...
特定上下文分配给新创建的密钥，这需要使用密钥
     安全类中的“create”权限。

与用户关联的默认密钥环将被标记为用户的默认
上下文，当且仅当登录程序已被改造为在
...
```
还新增了四个 sysctl 文件，用于控制
密钥的配额限制：

  - /proc/sys/kernel/keys/root_maxkeys
     /proc/sys/kernel/keys/root_maxbytes

     这些文件保存了 root 可拥有的密钥最大数量，以及
...
request_key 和 keyctl。后者提供了一系列用于
操作密钥的函数。

直接引用密钥时，用户空间程序应使用密钥的
序列号（一个正的 32 位整数）。不过，也存在一些特殊
...
用于密钥管理的内核服务相当容易处理。它们可以
分为两个领域：密钥和密钥类型。

处理密钥相当直接。首先，内核服务
注册其类型，然后搜索该类型的密钥。它应保留
该密钥，只要它还需要，然后应当释放它。对于
文件系统或设备文件，搜索可能在 open
调用期间执行，并在 close 时释放密钥。如何处理由于
两个不同用户打开同一文件而产生的密钥冲突，留给文件系统作者
去解决。
...
```
特定的密钥类型应在 include/keys/ 下有一个头文件，该文件应
```
...
```
这可与 request_key() 等函数配合使用，以在进程的
密钥环中找到特定的密钥环。随后找到的密钥环可以用
keyring_search() 搜索。注意，无法使用 request_key() 来
...
最简单的载荷就是直接存储在 key->payload 中的数据。在这种情况
下，访问载荷时无需使用 RCU 或加锁。

更复杂的载荷内容必须被分配，并将其指针设置到
key->payload.data[] 数组中。必须选择以下方式之一来
     访问数据：

  1) 不可修改的密钥类型。

     如果密钥类型没有 modify 方法，那么密钥的载荷可以在
...
文件系统可能希望定义 Kerberos 5 票据密钥类型。为此，它
需要填充一个 key_type 结构体并向系统注册。
...
```
<key> 是正在构造的密钥，三个密钥环是发起
搜索的进程所属的密钥环。它们被包含进来
     有两个原因：

   1  某个密钥环中可能有一个认证令牌，
      获取该密钥需要它，例如：一个 Kerberos 票据授予票据（Ticket-Granting Ticket）。

   2  新密钥可能应被缓存到这些环之一中。

该程序应在尝试访问更多密钥之前，
     将其 UID 和 GID 设置为指定的值。然后它可能寻找一个
     用户专属进程来转交该请求（也许是放在另一个密钥中、
     例如由 KDE 桌面管理器放置的路径）。

该程序（或其调用的任何东西）应通过调用
KEYCTL_INSTANTIATE 或 KEYCTL_INSTANTIATE_IOV 来完成密钥的构造，
     这还允许它在返回前将密钥缓存到某个密钥环（可能是会话环）中。
     或者，可以使用 KEYCTL_NEGATE 将密钥标记为负向，
     或使用 KEYCTL_REJECT；这也允许将密钥缓存到
     某个密钥环中。
