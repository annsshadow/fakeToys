...
```
CPU 有可能错误预测条件分支，从而即index >= MAX_ARRAY_ELEMS，也会推测性地加载 array[index]。该值随后会被丢弃，但被推测的加载可能影响微架构状态，而该状态随后可被测量
...
array_index_nospec(index, size) 的调用会返回一个经过净化的索引值，即使CPU 推测条件下，该值也被限制在 [0, size) 范围内
```
