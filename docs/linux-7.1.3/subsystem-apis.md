
## 内核子系统文档

本页是 Linux 内核文档的子系统总索引，按核心、人机接口、网络、存储等类别汇总了各内核子系统的参考文档入口，便于开发者从内核视角快速定位特定子系统的说明。



这些书籍从内核开发者的角度，详细介绍了特定内核子系统是如何工作的。
这里的大部分信息直接取自内核源代码，并视需要添加了补充材料（或者至少
是我们设法添加的内容——很可能 **并非** 全部所需）。

### 核心子系统


- [core-api/index](core-api/index)
- [driver-api/index](driver-api/index)
- [mm/index](mm/index)
- [power/index](power/index)
- [scheduler/index](scheduler/index)
- [timers/index](timers/index)
- [locking/index](locking/index)

### 人机接口


- [input/index](input/index)
- [hid/index](hid/index)
- [sound/index](sound/index)
- [gpu/index](gpu/index)
- [fb/index](fb/index)
- [leds/index](leds/index)

### 网络接口


- [networking/index](networking/index)
- [netlabel/index](netlabel/index)
- [infiniband/index](infiniband/index)
- [mhi/index](mhi/index)

### 存储接口


- [filesystems/index](filesystems/index)
- [block/index](block/index)
- [cdrom/index](cdrom/index)
- [scsi/index](scsi/index)
- [target/index](target/index)
- [nvme/index](nvme/index)

### 其他子系统

**Fixme**：这里还需要更多的分类组织工作。

- [accounting/index](accounting/index)
- [cpu-freq/index](cpu-freq/index)
- [edac/index](edac/index)
- [fpga/index](fpga/index)
- [i2c/index](i2c/index)
- [iio/index](iio/index)
- [pcmcia/index](pcmcia/index)
- [spi/index](spi/index)
- [w1/index](w1/index)
- [watchdog/index](watchdog/index)
- [virt/index](virt/index)
- [hwmon/index](hwmon/index)
- [accel/index](accel/index)
- [security/index](security/index)
- [crypto/index](crypto/index)
- [bpf/index](bpf/index)
- [usb/index](usb/index)
- [PCI/index](PCI/index)
- [misc-devices/index](misc-devices/index)
- [peci/index](peci/index)
- [wmi/index](wmi/index)
- [tee/index](tee/index)
