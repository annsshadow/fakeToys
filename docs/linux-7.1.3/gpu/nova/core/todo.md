...
- `Complexity`（复杂度）：描述所需的对 Rust 和/或相应内核 API 或子系统的熟悉程度。共有四种复杂度：`Beginner`（初级）、`Intermediate`（中级）、`Advanced`（高级）和 `Expert`（专家）。
- `Reference`（引用）：对其他任务的引用。
- `Link`（链接）：外部资源的链接。
- `Contact`（联系人）：可以就任务进一步信息联系的人员。

任务名称后面可能带有 `[ABCD]` 代码。该代码可用于在代码中 grep 与该任务相关的 `TODO` 条目。
...
nova-core 的一个好例子是 `Chipset` 枚举类型，它定义了值 `AD102`。在探测 GPU 时，可以从某个寄存器读出值 `0x192`，表明芯片组为 AD102。因此，枚举值 `AD102` 应当从数字 `0x192` 推导而来。目前，nova-core 为此使用了自定义的**实现（`Chipset` : from_u32）**。
...
- "查找最后置位比特"（Find Last Set Bit，内核 C 部分的 `fls` 函数）运算。

一个 `num` 核心内核模块正在设计中，用于提供这些运算。
...
针对并非由 Rust 页抽象创建、且没有直接所有权的页的 Rust 抽象。

Abdiel Janulgue [^1^] 和 Lina [^2^] 正在进行积极的工作（active ongoing work）。
...
这个问题是 nova-core 使用 Rust 的一大动机之一，因为事实证明 Rust 的过程宏（procedural macro）特性提供了一种相当优雅的方式来解决这一问题：

1. 从 C 头文件为每个版本在独立的命名空间中生成 Rust 结构
2. 构建实现固件接口的抽象结构（位于通用命名空间内）；用版本标识符标注实现的差异
3. 使用过程宏从该抽象生成实际的每个版本的实现
4. 在运行时实例化正确的版本类型（可以确信它们都有相同的接口，因为它由公共 trait 定义）

在 nova-core PoC 驱动的环境中已有该模式的 PoC（概念验证）实现。
...
实现底层的 GSP 消息队列（command、status），用于内核驱动与 GSP 之间的通信。
...
实现用于 client / device 分配的 GSP 消息接口，以及相应的 client 和 device 分配 API。
...
实现对图形引擎的支持，即相应的 GSP 消息接口，并提供用于（golden）上下文创建和提升（promotion）的 API。
...
设计 base API 未覆盖、但 vGPU 管理器所需的 API 部分。
...
这可以从最简单的运行 KUnit 测试，到运行（图形）CTS，再到启动（多个）客户机 VM 来测试 VFIO 用例。

也值得考虑引入一个直接位于 uAPI 之上的新测试套件，以进行更有针对性的测试和调试。可能存在与 Mesa 项目协作/共享代码的选项。
