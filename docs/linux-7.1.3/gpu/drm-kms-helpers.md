...
核心代码与辅助代码之间的这种区分在模式设置（modesetting）代码中尤为明显，因为所有驱动共享同一套用户空ABI。这与渲染侧形成对比，在渲染侧，几乎所有内容（极少数例外）都可以被视为可选的辅助代码

这些辅助代码可以分为几个领域

- 实现模式设置的辅助代码。这里重要的atomic（原子）辅助函数。旧驱动通常仍使用传统的 CRTC 辅助函数。它们共享同一组通用的辅vtable。对于真正简单的驱动（任何本应非常适合已被弃用fbdev 子系统的驱动），还有简单显示管道（simple display pipe）辅助函数
...
严格来说，这不是一DRM 辅助库，而是任何HDMI 输出交互的驱动（v4l alsa 驱动）都可通用。但它很好地契合模式设置辅助库这一整体主题，因此也包含在此处
