...
gitlab.freedesktop.org 上提供了用于自动测试 Mesa 的设施，同样可用于测DRM 子系统。本文档说明了有意进行测试的人如何利用这套共享的基础设施，从而节省相当多的时间和精力
...
IGT_VERSION
    所使用igt-gpu-tools 的修订版本，来自
    https://gitlab.freedesktop.org/drm/igt-gpu-tools

### drivers/gpu/drm/ci/testlist.txt


要在所有驱动上运行IGT 测试（除非在某个驱动\*-skips.txt 文件中有所提及，见下文）
...
1. 如果你还没有的话，在 https://gitlab.freedesktop.org/ 上创建一Linux 代码

2. 在你的内核仓库的配置中（例如
   https://gitlab.freedesktop.org/janedoe/linux/-/settings/ci_cd），
   CI/CD 配置文件.gitlab-ci.yml 改为
   drivers/gpu/drm/ci/gitlab-ci.yml銆。

3. 请求被添加到 drm/ci-ok 组，使你的用户拥有在
   https://gitlab.freedesktop.org/drm/ci-ok 上运CI 所需的权

4. 下次你推送到该代码仓库时，你将看到一CI 流水线被创建（例
   https://gitlab.freedesktop.org/janedoe/linux/-/pipelines锛。

5. 各项作业将会运行，当流水线结束时，除非发现了回归，否则所有作业都应当是绿色的

6. 流水线中的警告表明，在测试期间检测到lockdep
   （参Documentation/locking/lockdep-design.rst）问题
...
如果你的代码修改使得可以运行更多测试（例如通过解决可靠性问题），你可以flakes skips 列表中移除测试，以及（如果存在已知失败）相应的预期结果

如果需要更新所使用IGT 版本（也许你向其中添加了更多测试），请更gitlab-ci.yml 文件顶部IGT_VERSION 变量


## 如何测试你对脚本的修


为了测试drm-ci 仓库中脚本的修改，请
drivers/gpu/drm/ci/gitlab-ci.yml 中的 DRM_CI_PROJECT_PATH DRM_CI_COMMIT_SHA 变量改为与你的项目分支（例如 janedoe/drm-ci）相匹配。该分支需要位https://gitlab.freedesktop.org/


## 如何在测试中引入外部修复


通常，其他代码树中的回归会阻止对当前被测代码树中本地修改的测试。这些修复会在构建作业期间从目标代码树中一个名
