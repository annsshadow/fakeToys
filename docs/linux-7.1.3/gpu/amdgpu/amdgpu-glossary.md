...
这里你可以找到 amdgpu 驱动中使用的一些通用缩写。请注意，我们在
'Documentation/gpu/amdgpu/display/dc-glossary.rst' 为显示核心（Display Core）提供了专门的术语表。



    active_cu_number
      系统中处于活动状态的 CU 数量。活动 CU 的数量可能少于 SE ** SH ** CU，具体取决于板卡配置。
    ...
    GART
      图形地址重映射表（Graphics Address Remapping Table）。这是 GPU 内核驱动所使用的 GPUVM 页表的名称。它将系统资源（内存或 MMIO 空间）重新映射到 GPU 的地址空间，以便 GPU 能够访问它们。GART 这个名字可追溯到 AGP 时代，当时平台提供了一个 GPU 可以用来获取用于 DMA 的连续散页视图的 MMU。此后 MMU 已迁移到 GPU 上，但这个名字保留了下来。
    ...
    GPUVM
      GPU 虚拟内存（GPU Virtual Memory）。这是 GPU 的 MMU。GPU 支持多个任意时刻可以同时在飞的虚拟地址空间。它们允许 GPU 将 VRAM 和系统资源重新映射到 GPU 虚拟地址空间，供 GPU 内核驱动以及使用 GPU 的应用程序使用。它们为使用 GPU 的不同应用提供内存保护。


    GTT
      图形转换表（Graphics Translation Tables）。这是一个通过 TTM 管理的内存池，为 GPU 提供对系统资源（内存或 MMIO 空间）的访问。这些地址可以被映射到“GART” GPUVM 页表供内核驱动使用，或映射到各进程的 GPUVM 页表供应用程序使用。
    ...
    PA
      图元汇编器 / 物理地址（Primitive Assembler / Physical Address）
    ...
    RB
      渲染后端（Render Backends）。有些人称之为 ROPs。


    RLC
      运行列表控制器（RunList Controller）。这个名字是过去时代的遗留，如今已没有太多含义。它是一组用于 GFX 块的通用辅助引擎，除其他职责外，还参与 GFX 电源管理与 SR-IOV。
    ...
    UTC (AMDGPU)
      统一转换缓存（Unified Translation Cache）。UTC 等同于 TLB。你可能会看到该缩写末尾带 L 的变体，即 UTCL 后跟数字；L 表示缓存级别（例如 UTCL1 和 UTCL2）。
