...
debugfs 来支持错误注入。启用后，默认的 NVME_SC_INVALID_OPCODE（不重试）会被
注入到 nvme_try_complete_req 中。用户可以通过 debugfs 修改默认的状态码和不重试
标志。通用命令状态的列表可以在 include/linux/nvme.h 中找到。

下面的例子展示了如何向 nvme 中注入错误。

首先，启用 CONFIG_FAULT_INJECTION_DEBUG_FS 内核配置，重新编译内核。启动该
内核后，执行以下操作。
