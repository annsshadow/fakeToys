...
UAPI 检查器（`scripts/check-uapi.sh`）是一shell 脚本，用于在 git 树中检UAPI 头文件对用户空间向后兼容性

## 选项（Options


本节将描述可以用来运`check-uapi.sh` 的选项
...
```
在这种情况下，脚本报告这个类型变更，是因为它可能会破坏传入负数值的用户空间程序。现在，假设你知道没有任何用户空间程序可能会用到 `imm` 中的负值，因此在那里改成无符号类型应该不会有任何影响。你可以给脚本传 `-i` 标志来忽略这些变
```
...
```
它没抓到任何破坏性变更，因为默认情况下它只比`HEAD` `HEAD^1`。破坏性的变更提交`HEAD~2`。如果我们希望搜索范围回溯得更远，就得用 `-p` 选项传入一个不同的过去引用。在这种情况下，让我们给脚本`-p HEAD~2`，这样它就检`HEAD~2` `HEAD` 之间UAPI 变更
```
...
```
这是对一arm64 专属 UAPI 头文件的改动。在本例中，我正从一台带 x86 编译器的 x86 机器运行脚本，因此默认情况下
```
...
```
x86 编译器，我们无法检`arch/arm64` 中的头文件，所以脚本根本不会尝试
...
```
这里，我们在改动 `types.h` 中的一`typedef`。这并不破坏 `types.h` 中的 UAPI，但树中的其UAPI 可能
```
...
```
移除一UAPI 头文件被视为一种破坏性变更，脚本会将其标记为如此

### 检查历UAPI 兼容性（Checking Historic UAPI Compatibility


你可以用 `-b` `-p` 选项来检git 树中不同的片段。例如，要检查标签之间所有被更改UAPI 头文
```
...
你会注意到脚本检测到了许多不向后兼容UAPI 变更。鉴于内UAPI 本应永远保持稳定，这是一个令人警觉的结果。这把我们带到了下一节：注意事项（caveats）
...
如果一个结构体被用ioctl 的参数，那么内核驱动必须能处理任意大小的 ioctl 命令。除此之外，在从用户复制数据时你需要小心。例如说
```
...
如果一个用户空间程序是用版1 构建的，它会认为 `sizeof(struct foo)` 8。这个尺寸会被编码进发往内核ioctl 值中。如果内核是用版2 构建的，它会认为 `sizeof(struct foo)` 16

内核可以`_IOC_SIZE` 宏来获取用户传入ioctl 码中编码的尺寸，然后
```
...
如果你知道内核代码中已经考虑了这种情况，你可以给脚本`-i`，这样像这样的结构体扩展就会被忽略

### Flex 数组迁移（Flex Array Migration


虽然脚本会处理向已有 flex 数组的扩展，但它仍会标记1 元素的伪 flex 数组到真 flex 数组的初始迁
```
...
虽然许多类型的误报会被脚本过滤掉，仍有可能出现脚本标记了一个并未破UAPI 的变更的情况。也有可能一*确实**破坏用户空间的变更未被此脚本标记。虽然脚本已在大量内核历史上运行过，仍可能存在未被涵盖的边界情况

此脚本的意图是作为维护者或自动化工具的一个快速检查，而非补丁兼容性的最终权威。最好记住：运用你的最佳判断（理想情况下再加上用户空间的一个单元测试）来确保你UAPI 变更是向后兼容的
