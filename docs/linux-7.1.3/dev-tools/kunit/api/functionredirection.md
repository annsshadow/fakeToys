...
编写单元测试时，能够将待测代码与内核其它部分隔离开来是很重要的。这保证了测试的可靠
（不会受外部因素影响）、减少对特定硬件或配置选项的依赖（使测试更容易运行），并保
系统其余部分的稳定性（降低测试特定状态干扰系统其余部分的可能性）

对于某些代码（通常是通用数据结构、辅助函数以及其纯函）来说这很容易，但对
其它代码（如设备驱动、文件系统、核心子系统）来说，代码与内核其它部分高度耦合

这种耦合往往以某种方式源于全局状态：无论是设备的全局列表、文件系统，还是某些硬件状态
测试需要小心地管理、隔离和恢复状态，或者也可以通过（fake）或"模拟"（mock
...
2. 编写一个或多个替换函数
   ...
   使用 kunit_get_current_test() 获取一struct kunit 指针。然后可以将其传给期断言
   宏，或用于查KUnit 资源
...
3. 从你的测试中激活静态桩
   ...
   在测试内部，可以使用 kunit_activate_static_stub() 启用重定向，它接受一struct kunit
...
4. 调用（可能是间接地）真实函数
   ...
   一旦激活了重定向，任何对真实函数的调用都会改为调用替换函数。此类调用可能深埋在另一
   函数的实现中，但必须来自测试kthread
...
5. （可选）禁用该桩
   ...
   当不再需要它时，使用 kunit_deactivate_static_stub() 禁用重定向（从而恢真实"函数
