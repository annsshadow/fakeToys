...
此外还有用于复位告警阈值控制的迟滞（hysteresis）。按照 JC 42.4 规范，迟滞阈值
可配置为 0、1.5、3.0 和 6.0 摄氏度。配置的迟滞值会被舍入到这些限值。芯片仅支持
一个用于配置迟滞的寄存器，该寄存器对所有限值都生效。可通过写入 temp1_crit_hyst
来写入该寄存器。其他迟滞属性为只读。

如果 BIOS 已将传感器配置为自动温度管理，则很可能已锁定相关寄存器，即温度限值
无法被更改。
...
======================= ===========================================
temp1_input		温度（RO）
temp1_min		最低温度（RO 或 RW）
temp1_max		最高温度（RO 或 RW）
temp1_crit		临界高温（RO 或 RW）

temp1_crit_hyst		临界迟滞温度（RO 或 RW）
temp1_max_hyst		最大迟滞温度（RO）
