...
该驱动实现了对 Sensirion SHTC1、SHTW1 和 SHTC3 芯片的支持，这些芯片是温湿度传感器。温度以摄氏度为单位测量，相对湿度以百分比表示。

该设备通过 I2C 协议通信。所有传感器的 I2C 地址均设为 0x70。实例化该设备的方法请参阅 Documentation/i2c/instantiating-devices.rst。

可通过 shtc1_platform_data 配置两个选项：

1. 阻塞模式（在执行测量时拉低 I2C 时钟线）或
