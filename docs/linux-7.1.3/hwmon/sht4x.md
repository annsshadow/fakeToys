...
该器件通过 I2C 协议通信。传感器可使用 I2C 地址 0x44。关于实例化该器件的方法，请参阅 Documentation/i2c/instantiating-devices.rst。
...
update_interval 轮询传感器的最小间隔，单位毫秒。可写。必须至少为 2000。
heater_power	请求的加热器功率，单位毫瓦。
		可用值：20、110、200（默认：200）。
heater_time	请求的加热器工作时间，单位毫秒。
		可用值：100、1000（默认 1000）。
heater_enable	以所选功率、在所选时间内启用加热器，以去除传感器表面的冷凝水。一旦启用便无法手动关闭（完成操作后自动关闭）。
