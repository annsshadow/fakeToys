...
本文介绍 Linux 内核中 ltc2978 系列硬件监控（hwmon）驱动所支持的芯片型号，涵盖 Analog Devices 与 Linear Technology 的 PMBus 电源管理控制器。文档列出各芯片的驱动前缀、扫描地址与数据手册链接，供系统管理员与驱动开发者在电源监控配置与适配时参考。
...
- LT7170 和 LT7171 是 20 A、16 V、单路或双相 Silent Switcher 降压稳压器，
  带有数字电源系统管理。
- LTC2974 和 LTC2975 是四通道数字电源管理器。
- LTC2978 是八通道电源监视器。
- LTC2977 是 LTC2978 的引脚兼容替代品。
- LTC2980 是 16 通道电源系统管理器，由单个芯片上的两个 LTC2977 组成。该芯片被实例化为
  两个独立的芯片，并报告在两个不同的 I2C 总线地址上。
- LTC3880、LTC3882、LTC3886 和 LTC3887 是双输出多相降压 DC/DC 控制器。
- LTC3883 是单相降压 DC/DC 控制器。
- LTM2987 是 16 通道电源系统管理器，在单个芯片上包含两个 LTC2977 以及额外的元件。该芯片
  被实例化为两个独立的芯片，并报告在两个不同的 I2C 总线地址上。
- LTM4675 是双路 9A 或单路 18A μModule 稳压器。
- LTM4676 是双路 13A 或单路 26A μModule 稳压器。
- LTM4686 是双路 10A 或单路 20A μModule 稳压器。


### 使用说明

该驱动不会探测 PMBus 设备。你必须显式地实例化设备。
...
in[N]_lowest		最低输出电压。
			...
			LTC2972、LTC2974、LTC2975 和 LTC2978 仅支持。
...
temp[N]_input		测得的温度。
   ...
   - 在 LTC2972 上，temp[1-2] 报告外部温度，temp3 报告芯片温度。
   - 在 LTC2974 和 LTC2975 上，temp[1-4] 报告外部温度，temp5 报告芯片温度。
   - 在 LTC2977、LTC2979、LTC2980、LTC2978 和 LTM2987 上，
			  仅支持一次温度测量，报告芯片温度。
   - 在 LTC3880、LTC3882、LTC3886、LTC3887、LTC3889、
			  LTM4664、LTM4675、LTM4676、LTM4677、LTM4678、LTM4680、
			  和 LTM4700 上，temp1 和 temp2 报告外部温度，temp3 报告芯片温度。
...
power1_label		"pin"。仅 LTC3883 和 LTC3886 支持。
