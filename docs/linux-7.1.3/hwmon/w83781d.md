...
- reset int
    (default 0)
    该驱动过去会在加载时重置芯片，但现在不再这样做了。使用
    'reset=1' 可恢复旧行为。如果你需要这样做请报告。

force_subclients=bus,caddr,saddr,saddr
  这用于强制设定某个芯片的子客户端（subclient）的 i2c 地址。典型用法是
  `force_subclients=0,0x2d,0x4a,0x4b`，将总线 0 上芯片 0x2d 的子客户端强制设为
  i2c 地址 0x4a 和 0x4b。此参数对某些 Tyan 主板很有用。
...
本驱动实现对 Winbond W83781D、W83782D、W83783S 芯片以及 Asus AS99127F 芯片的支持。
我们将它们统称为 W8378* 芯片。
...
这些芯片之间存在相当大的差异，但它们足够相似，因此将它们放在一个驱动中是很合理的。
Asus 的芯片类似于仅支持 I2C 的 W83782D。
...
W8378* 实现了温度传感器（W83781D 和 W83782D 上有三个，W83783S 上有两个）、三个风扇
转速传感器、电压传感器（W83781D 上有七个，W83782D 上有九个，W83783S 上有六个）、
VID 引脚、带蜂鸣警告的报警，以及一些杂项功能。
...
温度以摄氏度测量。始终有一个主温度传感器，以及一个（W83783S）或两个（W83781D 和
W83782D）其他传感器。当越过超温关闭（Overtemperature Shutdown）限值时，主传感器会
触发一次报警；一旦温度降到迟滞（Hysteresis）值以下，又会再次触发。将迟滞值设为
+127 摄氏度可以得到更有用的行为；在这种情况下，只要实际温度高于超温关闭值，就会
持续发出报警。驱动在初始化时将 temp1 的迟滞值设为 127。
...
迟滞值以下。但在 W83781D 上，只有一个报警同时服务于另外两个传感器！温度保证在
-55 到 +125 度的范围内。主温度传感器的分辨率为 1 度；其他传感器的分辨率为 0.5 度。
...
风扇转速以 RPM（每分钟转数）报告。如果转速降到可编程的限值以下，会触发报警。风扇
读数可被一个可编程的分频器（W83781D 为 1、2、4 或 8；其他芯片为 1、2、4、8、16、
32、64 或 128）除，以获得更大的范围或更高的精度。并非所有 RPM 值都能精确表示，
因此会做一些舍入。当分频器为 2 时，可表示的最低值约为 2600 RPM。
...
电压传感器（也称为 IN 传感器）以伏特报告其值。如果电压越过了可编程的最小或最大限值，
会触发报警。注意，这里的最小值始终指"最接近零"；这对于负电压测量很重要。所有电压
输入可以测量 0 到 4.08 伏之间的电压，分辨率为 0.016 伏。
...
W83782D 和 W83783S 的温度转换机制支持多种温度传感器。你可以在传感器文件中编程所谓的
beta 值。'1' 是 PII/Celeron 二极管，'2' 是 TN3904 晶体管，3435 是默认的 thermistor
值。其他值（尚）不支持。

In addition to the alarms described above, there is a CHAS alarm on the
chips which triggers if your computer case is open.

除了上述报警之外，芯片上还有一个 CHAS 报警，当你的机箱被打开时触发。
...
如果报警触发，它会一直保持触发状态，直到硬件寄存器至少被读取一次。这意味着报警的
...
这些芯片每 1.5 秒才更新一次值；更频繁地读取无害，但会返回"旧"值。
...
（尽管 AS99127F 的修订版 2 看起来更像 w83782d）。在发布数据手册之前，此支持将处于
BETA 状态。有用户报告风扇偶尔停止的问题。
...
已知问题：
 - 二极管/thermistor 设置的问题（是否支持？）
 - 有用户报告在服务器高负载下风扇停止。
 - 修订版 2 似乎有 2 个 PWM 寄存器，但我们不知道
	  如何处理它们。更多细节见下文。
...
这些问题不会被修复，除非我们拿到数据手册。如果你遇到问题，请向 Asus 游说发布数据手册。
遗憾的是其他几个人也这么做了但没有成功。请不要给我们发邮件要求更好的 as99127f 支持。
在没有数据手册的情况下我们已经尽了最大努力。请不要给作者或 sensors 邮件组发邮件索要
数据手册或询问如何说服 Asus。我们无能为力。
...
### 注意事项
  ...
  783s 引脚可编程为 -5V 或 temp1；默认为 -5V，驱动中无控制，因此 temp1 不工作。

  782d and 783s datasheets differ on which is pwm1 and which is pwm2.
  We chose to follow 782d.

  782d 和 783s 的数据手册在 pwm1 和 pwm2 各是哪个上存在分歧。我们选择遵循 782d。
  ...
  782d 和 783s 的引脚可编程为 fan3 输入或 pwm2 输出；默认为 fan3 输入。
  如果启用了 pwm2（通过 echo 255 1 > pwm2），则 fan3 将报告 0。

  782d has pwm1-2 for ISA, pwm1-4 for i2c. (pwm3-4 share pins with
  the ISA pins)

  782d 的 ISA 上有 pwm1-2，i2c 上有 pwm1-4。（pwm3-4 与 ISA 引脚复用）
...
我们没有 Asus 兼容型号（AS99127F 和 ASB100 Bach）的数据手册。以下是 Alex Van Kaam
提供给我们的、关于如何检测这些芯片以及如何读取它们数值的非常有用的信息。他还就另一个
Asus 芯片组 Mozart-2（我们尚不支持）给出了建议。感谢 Alex！
...
这正是 AS99127F 修订版 1 和 2 之间存在差异的地方。
...
- 关于 AS99127F 上 PWM 的额外信息（也可能适用于其他 Asus 芯片），由 Jean Delvare
  于 2004-04-09 提供：
...
AS99127F 修订版 2 似乎在 0x59 和 0x5A 有两个 PWM 寄存器，在 0x5B 有一个温度传感器
类型选择器（这基本上意味着与 Winbond 芯片相比，它们交换了 0x59 和 0x5B 寄存器）。
该芯片的修订版 1 在 0x5B 也有温度传感器类型选择器，但 PWM 寄存器不起作用。
...
我们并不确切知道温度传感器类型的选择是如何工作的。看起来比特 1-0 用于 temp1，比特 3-2
用于 temp2，比特 5-4 用于 temp3，不过也有可能每次只有最高有效位才起作用。迄今为止，
除 0 以外的值总是会破坏读数。
...
PWM 寄存器似乎分为两部分：比特 7 是模式选择器，而其他比特似乎定义一个值或阈值。
...
当比特 7 为 0 时，比特 6-0 似乎持有一个阈值。如果值低于某个给定限值，风扇以低速运行；
如果值高于该限值，风扇以全速运行。我们不知道该限值代表什么。注意此模式似乎存在某种
惯性，速度变化可能需要一些时间才能触发。此外，由于递增遍历所有值再递减遍历会得到
略有不同的限值，怀疑存在迟滞机制。
...
当比特 7 被置位时，比特 3-0 似乎持有一个阈值，而比特 6-4 无关紧要。如果值低于某个
给定限值，风扇以全速运行；如果高于该限值，则以低速运行（所以这在某种意义上与另一种
模式相反）。同样，我们不知道该限值代表什么。
...
总之，你可以向 PWM 寄存器写入 0x00 或 0x8F 让风扇以低速运行，写入 0x7F 或 0x80 让
它们以全速运行。
...
如果你能弄清楚它应当如何工作，请联系我们。在我们了解更多之前，w83781d 驱动根本不处理
AS99127F 芯片上的 PWM。

- Additional info about PWM on the AS99127F rev.1 by Hector Martin:

- 关于 AS99127F 修订版 1 上 PWM 的额外信息，由 Hector Martin 提供：

I've been fiddling around with the (in)famous 0x59 register and
found out the following values do work as a form of coarse pwm:

我一直在摆弄那个（臭名昭著的）0x59 寄存器，发现以下值确实可以作为某种粗略的 pwm 工作：
...
0x80
 - 似乎会在一段时间后（1-2 分钟）关闭风扇……可能是某种基于温度的自动风扇控制？
   嗯（Qfan？这块主板是老 ASUS，并未以 Qfan 名义销售。也许是 BIOS 中某个被放弃的
   Qfan 测试性尝试）
0x81
 - 关闭
0x82
 - 比关闭稍微"动一点"，但我的风扇并未转动。我能听到电机在 pwm 过低时发出的
   高频 PWM 声。
0x83
 - 现在它们确实转动了。估计约为 70% 的速度。
0x84-0x8f
 - 全开
