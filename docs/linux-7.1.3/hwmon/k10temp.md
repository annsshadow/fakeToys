...
- AMD Family 15h 处理器："Bulldozer"（FX 系列）、"Trinity"、"Kaveri"、
...
此驱动允许读取 AMD Family 10h/11h/12h/14h/15h/16h 处理器的内部温度传感器。

所有这些处理器都有一个传感器，但在 Socket F 或 AM2+ 的处理器上，传感器
可能返回不一致的值（erratum 319）。除非你指定 "force=1" 模块参数，否则
驱动将拒绝在这些修订版本上加载。

由于技术原因，驱动只能检测主板插槽类型，而非处理器的实际能力。因此，如果
你在 AM2+ 主板上使用 AM3 处理器，可以安全地使用 "force=1" 参数。

对于早于 Family 17h 的 CPU，有一个温度测量值，在 sysfs 中作为 temp1_input
可用。它以摄氏度为单位测量，分辨率为 1/8 度。请注意，它被定义为一个相对值
```

  Tctl 是处理器温度控制值，由平台用来控制散热系统。Tctl 是一个任意刻度上的
  非物理温度，以度为单位测量。它**不**代表像芯片（die）或机箱（case）温度
  这样的实际物理温度。相反，它指定的是相对于系统必须为处理器指定的最大机箱
  温度和最大热功耗提供最大散热的那一点的处理器温度。

```
Tctl 的最大值在文件 temp1_max 中可用。

如果 BIOS 已启用硬件温度控制，处理器为避免损坏而自行降频（throttle）的阈值
在 temp1_crit 和 temp1_crit_hyst 中可用。

在某些 AMD CPU 上，芯片温度（Tdie）与报告的温度（Tctl）之间存在差异。Tdie
是实际测量的温度，Tctl 用于风扇控制。虽然 Tctl 始终作为 temp1_input 可用，
但对于支持它的那些 CPU，驱动将 Tdie 温度作为 temp2_input 导出。

17h 系列型号报告相对温度，驱动旨在补偿并报告真实温度。

在 Family 17h 和 Family 18h CPU 上，额外的温度传感器可能报告 Core Complex
Die（CCD）温度。最多 8 个这样的温度作为 temp{3..10}_input 报告，标记为
