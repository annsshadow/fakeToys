...
配置设备_不_是内存映射的，必须通过"vexpress_config" API 抽象的自定义接口访问

由于这些设备是不可发现的，必须在传递给内核Device Tree 中描述。它们的 DT 绑定的详细信息可以在 Documentation/devicetree/bindings/hwmon/vexpress.txt 中找到
