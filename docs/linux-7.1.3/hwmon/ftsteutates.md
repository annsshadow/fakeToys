...
BMC Teutates 是 Superior System 监控和热管理的第十一代解决方案。它建立在 BMC Theseus 的基本功能之上，并包含若干新特性和增强。它可以监控最多 4 路电压、16 个温度和 8 个风扇。它还包含一个当前由该驱动实现的集成看门狗。

`pwmX_auto_channels_temp` 属性显示当前是哪个温度传感器在驱动哪个风扇通道。该值可能会在运行时动态变化，取决于风扇控制电路所选的温度传感器。

4 路电压需要板级特定的倍乘系数，因为 BMC 只能测量最高 3.3V 的电压，因此依赖于分压器。详情请参阅你的主板手册。
