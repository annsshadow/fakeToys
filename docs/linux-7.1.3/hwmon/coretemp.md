...
支持的芯片：
  - 所有内置数字温度传感器（DTS）的 Intel Core 系列Atom 处理

    前缀（Prefix）：'coretemp'

    CPUID：family 0x6，具X86_FEATURE_DTHERM 特性的型号，包括：
...
相应的传感器读取每核与每封装的温度。每封装传感器在 Sandy Bridge 及所有更
的处理器上可用。该驱动会在 hwmon 内的单个设备目录下显示封装内所有核的温度

温度以摄氏度度量，测量分辨率1 摄氏度。有效温度范围为 0 TjMax 摄氏度，
因为温度寄存器的实际值实际上是相TjMax 的差值

称为 TjMax 的温度是处理器的最大结温，取决CPU 型号。见表下方。达到该温度
时，保护机制会执行动作以强制为处理器降温。如果温度上升到足以触发 Out-Of-Spec
位（超过 TjMax），可能会发出告警。下表汇总了导出sysfs 文件

所sysfs 条目均以各自core_id 命名（此处用 'X' 表示）

================= ========================================================
tempX_input	  核心温度（单位为毫摄氏度）
tempX_max	  应开启全部散热设备（Core2 上）
tempX_crit	  最高结温（单位为毫摄氏度）
tempX_crit_alarm  Out-of-spec 位置位时置起，且不会自动清除
		  此时无法再保CPU 的正确运行
tempX_label	  包含字符"Core X"，其X 为处理器编号
		  对于封装温度，该值为 "Physical id Y"
		  其中 Y 为封装编号
================= ========================================================

在现CPU（Nehalem 及更新）上，TjMax MSR_IA32_TEMPERATURE_TARGET 寄存
读取。在没有MSR 的旧型号上，TjMax 通过查找表或启发式方法确定。如果这些对
您的 CPU 不起作用，可以将正确TjMax 值作为模块参数（tjmax）传入

附录 A. 已知 TjMax 列表（待定）
部分信息来自 ark.intel.com

=============== =============================================== ================
制程（Process处理器（Processor				TjMax(°C)
