...
本驱动实现了对多款 MAX6697 兼容温度传感器的支持。这些芯片支持一个本地温度传感器
外加四个、六个或七个远程温度传感器。远程温度传感器是二极管连接的散热晶体管，
MAX6698 例外，它除了本地温度传感器外，还支持三个二极管连接的散热晶体管外加三个
热敏电阻。

本驱动提供以下 sysfs 属性。temp1 是本地（芯片）温度，temp[2..n] 是远程温度。实际
支持的每通道属性取决于芯片类型与通道。

================ == ==========================================================
tempX_input      RO 温度
tempX_max        RW 温度最大阈值
tempX_max_alarm  RO 温度最大阈值告警
tempX_crit       RW 温度临界阈值
tempX_crit_alarm RO 温度临界阈值告警
tempX_fault      RO 温度二极管故障（仅远程传感器）
================ == ==========================================================
