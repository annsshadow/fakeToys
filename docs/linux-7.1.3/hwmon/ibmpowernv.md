...
该驱动使用平台设备基础设施。它__init 阶段探测设备树以寻找传感器设备，并将它们注册'hwmon'hwmon' 填充具有属性文件的 'sysfs' 树，每个文件对应一个给定的传感器类型及其属性数据

DT 中的所有节点都出现"/ibm,opal/sensors" 下，DT 中的每个有效节点都映射到 'sysfs' 中的一个属性文件。该节点导出唯一'sensor-id'，驱动使用它向固件发OPAL 调用

### 使用说明


该驱动通过启用配置 CONFIG_SENSORS_IBMPOWERNV 与内核静态构建。它也可以作为模'ibmpowernv' 构建
...
powerX_input		功耗（微瓦
powerX_input_highest	历史最大功
powerX_input_lowest	历史最小功
