...
本驱动实现了对 Maxim MAX20710、MAX20730、MAX20734 与 MAX20743 的支持，
这些是带 PMBus 支持的集成式降压开关稳压器。

该驱动是核心 PMBus 驱动的客户端驱动。有关 PMBus 客户端驱动的详情，
请参阅 Documentation/hwmon/pmbus.rst。
...
=================== ===== =======================================================
curr1_crit          RW/RO 输出电流临界值。支持的限制请参阅数据手册。
                          若芯片处于写保护状态则为只读；否则可读写。
curr1_crit_alarm    RO    输出电流临界告警
curr1_input         RO    输出电流
...
in2_input           RO    输出电压
in2_label           RO    'vout1'
temp1_crit          RW/RO 温度临界值。支持的值为 130 或 150 度 C。
                          若芯片处于写保护状态则为只读；否则可读写。
temp1_crit_alarm    RO    温度临界告警
temp1_input         RO    芯片温度
=================== ===== =======================================================
