...
该驱动支持 Flex PIM4328 及兼容的数字电源接口模块的硬件监视。

该驱动是核心 PMBus 驱动的一个客户端驱动。有关 PMBus 客户端驱动的详细信息，请参阅 Documentation/hwmon/pmbus.rst 与 Documentation.hwmon/pmbus-core。
...
in2_label		"vin.0"
in2_input		输入端 A 上测得的输入电压。

			PIM4328 与 PIM4X06

in3_label		"vin.1"
in3_input		输入端 B 上测得的输入电压。
...
curr1_label		"iin.0"
curr1_input		输入端 A 上测得的输入电流。

			PIM4X06

curr2_label		"iin.1"
curr2_input		输入端 B 上测得的输入电流。
