...
电路
LPC47M15x、LPC47M192 LPC47M292 芯片除了风扇监控与控制外，还包含一个完整的
“硬件监控块”。该硬件监控块不受本驱动支持，对此请使用 smsc47m192 驱动
没有 47M997 的文档可用，但它47M15x 47M192 芯片具有相同的设ID，并且似兼容
风扇转速以 RPM（每分钟转数）报告。如果转速降到可编程限制以下，会触发告警。风读数可以被一个可编程的分频器 8）除，以给予读数更大的范围或精度。并所RPM 值都能精确表示，因此会进行一些舍入。使用分频器 2 时，可表示的最低值约2600 RPM
PWM 值范围为 0 255
如果告警触发，它将一直保持触发状态，直到硬件寄存器至少被读取一次。这意味着告警原因可能已经消失了！注意，在当前实现中，只要读取任何数据，就会读取所有硬件寄存器
（除非距离上次更新不1.5 秒）。这意味着你可能会轻易错过仅触发一次的告警
------------------------------------------------------------------
