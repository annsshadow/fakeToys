...
C) 每用户命名空间（Per-user namespace
   ...
   上述语义提供了一种跨命名空间共享挂载的方式。但命名空间是与进程相关联的。如果命名空间被实现为一流对象，并提供用API 来将命名空间与用ID 关联/解除关联，那么每个用户都可以拥有她自己的命名空间，并根据其需求进行定制。这需要在 PAM 中提供支持
...
本节解释bind、rbind、move、mount、umount 以及克隆命名空间（clone-namespace）操作的详细语义

   本文档中，单'vfsmount' 和名'mount' 被用来指代同一个概念
...
a) 挂载状态（Mount states
   ...
   **对等组（peer group* 定义为一组相互传播事件的 vfsmount
   ...
   (1) 共享挂载（Shared mounts
       ...
       **共享挂载（shared mount* 定义为属于某个对等组vfsmount
...
c) 递归绑定语义（Rbind semantics
   ...
   rbind bind 相同。bind 复制指定的挂载。rbind 复制属于指定挂载的树中的所有挂载。rbind 挂载就是对树中所有挂载应用的 bind 挂载

   如果rbind 的源树中包含一些不可绑定挂载，那么这些不可绑定挂载之下的子树会在新位置被剪除
...
g) 克隆命名空间（Clone Namespace
   ...
   假设 'A' 'B' 分别是父命名空间和子命名空间中对应的挂载

   如果 'A' 是共享的，那'B' 也是共享的，并且 'A' 'B' 相互传播

   如果 'A' 是挂'Z' 的从属挂载，那么 'B' 也是 'Z' 的从属挂载
...
2. 为什么不能只exportfs 来实现共享子树？

   exportfs 是一种实现共享子树部分功能的高开销方式。我无法想象出一种能exportfs 实现从属挂载语义的方法
...
A) 数据结构（Datastructure
   ...
   struct vfsmount 引入了几条新字段

   ->mnt_share
           将所有从这个 vfsmount 发接收传播事件的挂载链接在一起

   ->mnt_slave_list
           链接这个 vfsmount 传播到的所有挂载

   ->mnt_slave
           将这vfsmount 的主（master）vfsmount 传播到的所有从属链接在一起

   ->mnt_master
           指向这个 vfsmount 从中接收传播的主 vfsmount

   ->mnt_flags
           增加了两个标志位，用于指vfsmount 的传播状态。MNT_SHARE 表示vfsmount 是一个共vfsmount。MNT_UNCLONABLE 表示vfsmount 不能被复制

   一个对等组中的所有共vfsmount 通过 ->mnt_share 形成一个循环链表

   所有具有相->mnt_master vfsmount 形成一个循环链表，该链表锚定在 ->mnt_master->mnt_slave_list 中，并通过 ->mnt_slave 串联
   ...
   一个对等组中的所vfsmount 拥有相同->mnt_master。如果它NULL，它们就形成一个连续的（有序的）从属链表段
...
B) 加锁（Locking）：
   ...
   ->mnt_share>mnt_slave>mnt_slave_list>mnt_master namespace_sem 保护（修改时独占，读取时共享）

   通常我们通过 vfsmount_lock 来串行化 ->mnt_flags 的修改。有两个例外：do_add_mount() clone_mnt()。前者修改一个尚未在任何共享数据结构中可见的 vfsmount。后者持namespace_sem，且vfsmount 的唯一引用都位于不持有 namespace_sem 就无法遍历的链表中

C) 算法（Algorithm）：

   实现的核心在rbind/move 操作

   总体算法将该操作分解3 个阶段：（参attach_recursive_mnt() propagate_mnt()
   ...
   1. 准备阶段（Prepare phase）
      ...
      b) 不要将任何树附加到其目标上。但要记录其 ->mnt_parent ->mnt_mountpoint
      c) 将所有新挂载链接起来，形成一个与目标挂载的传播树完全相同的传播树

      如果此阶段成功，应当创建 'n' 个新的传播树，其'n' 是源树中挂载的数量。进入提交阶段

      同时应当创建 'm' 个新的挂载树，其'm' 是目标挂载传播到的挂载数量
