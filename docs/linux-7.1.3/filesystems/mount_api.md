...
```
挂载上下文通过调用 vfs_new_fs_context() vfs_dup_fs_context() 创建，并通过 put_fs_context() 销毁。注意该结构没有被引用计数
VFS、安全和文件系统的挂载选项vfs_parse_mount_option() 逐个设置。由旧的 mount(2) 系统调用作为一页数据提供的选项可以generic_parse_monolithic() 解析
...
```
   ...
   * ::
	...
	int (*parse_param)(struct fs_context *fc,
     ...
     当向文件系统上下文添加参数时调用。param 指向键名，可能还有一个值对象。VFS 相关的选项将已被剔除，并且 fc->sb_flags 已在上下文中更新。安全选项也将已被剔除，并fc->security 已更新
     参数可以fs_parse() fs_lookup_param() 来解析。注意来源（source）是作为名为 "source" 的参数呈现的
   ...
   * ::
	...
	int (*reconfigure)(struct fs_context *fc);
     ...
     调用以使用文件系统上下文中存储的信息来实施超级块的重新配置。它可以将其想要的任何资源从文件系统上下文分离，并转移到超级块。超级块可以fc->root->d_sb 找到
     成功时它应返0。在出错的情况下，它应返回一个负的错误码
...
```
   ...
   * ::
	...
	int security_fs_context_parse_param(struct fs_context *fc,
     ...
     为每个挂载参数（包括来源）调用。参数与 ->parse_param() 方法相同。应返回 0 表示该参数应被传递给文件系统，返1 表示该参数应被丢弃，或返回一个错误以表示该参数应被拒绝
...
```
   ...
   * ::
     ...
     分配一个文件系统上下文，用于为自动挂载点或其他派生的超级块创建一个新的挂载。fs_type 指定管理该上下文的文件系统类型，reference dentry 提供参数。命名空间也reference dentry 的超级块传播
...
```
在所有上述操作中，除put 操作之外，返回的是一个挂载上下文指针或一个负的错误码
...
```
     ...
     向文件系统上下文提供单个挂载参数。这包括来源/设备的指定，它作"source" 参数指定（如果文件系统支持，可以多次指定）
     param 指定参数键名和值。该参数会先被检查，看它是否对应一个标准的挂载标志（这种情况下用于设置一SB_xxx 标志并被消费）或一个安全选项（这种情况下LSM 消费），然后才被传递给文件系统
     参数值是带类型的，可以是以下之一
	====================		=============================
...
```
     ...
     这是核心例程。如test 为非 NULL，它会使test 函数fs_context 中搜索匹配条件的已有超级块。如果没找到匹配项，就创建一个新的超级块，并调用 set 函数来设置它
     在调set 函数之前，fc->s_fs_info 将被转移sb->s_fs_info——并且如set 返回成功（即 0），fc->s_fs_info 将被清空
```
以下辅助函数都包装了 sget_fc()
...
```
	...
	struct fs_parameter_spec {
     ...
     'name' 字段是一个要与参数键精确匹配的字符串（不支持通配符、模式，也不区分大小写）opt' fs_parser() 函数在成功匹配的情况下返回的值
	...
	=======================	=======================	=====================
     ...
     注意，如果值的类型fs_param_is_bool，fs_parse() 会尝试将任何字符串值与 "0"1"no"yes"false"true" 匹配
     每个参数还可以用 'flags' 限定
	=======================	================================================
...
```
解析器应file_system_type 结构中的 parser 指针指向，因为这将提供注册时的验证（如果 CONFIG_VALIDATE_FS_PARSER=y），并将允许通过 fsinfo() 系统调用从用户空间查询该描述

## 参数辅助函数

提供了若干辅助函数来帮助文件系统LSM 处理它所获得的参数
```
   ...
   * ::
     ...
     如果成功，并且如果参数类型指示结果是布尔、整数、枚举、uid gid 类型，该值会被此函数转换，结果存储在 result->{boolean,int_32,uint_32,uint_64,uid,gid} 中
   ...
   * ::
     ...
     成功时返0，并``*_path`` 会被设置；否则返回一个负的错误码
```
