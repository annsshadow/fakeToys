...
希望支持自动挂载的文件系统（例如可在 fs/afs/ 找到的 kAFS，以及
fs/nfs/ 中的 NFS）可以使用该支持。该机制包括允许执行内核内挂载，
...
请参阅 Documentation/filesystems/autofs.rst 的“挂载陷阱（Mount Traps）”一节。
...
只要你在前述自动挂载流程中挂载了将要过期的挂载点，挂载点的自动过期
就很简单。

要进行过期处理，你需要遵循以下步骤：

 (1) 创建至少一个列表，用于挂接将要过期的 vfsmount。

 (2) 在 ->d_automount 方法中创建新的挂载点时，添加
```
...
```
过期标志由对 mntput 的调用清除。这意味着过期只会在挂载点最后一次被访问之后的
第二次过期请求时发生。
...
作为替代，用户空间可以请求任何挂载点的过期（尽管有些会被拒绝——例如当前进程
所认为的 rootfs）。它通过向 umount() 传入 MNT_EXPIRE 标志来实现。该标志被认为
与 MNT_FORCE 和 MNT_DETACH 不兼容。

如果相关挂载点被 umount() 或其父挂载点以外的东西所引用，将返回 EBUSY 错误，
并且该挂载点不会被标记为过期或卸载。
