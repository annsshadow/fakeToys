...
sb_set_blocksize() 和 sb_min_blocksize()。
...
（sb_find_get_block() 取代 2.4 的 get_hash_table()）
...
新增 foo_alloc_inode() 和 foo_destroy_inode()——前者应分配
foo_inode_info 并返回 ->vfs_inode 的地址，后者应释放
...
请注意，现在你需要在调用 iget_locked() 与解锁 inode 之间显式地
初始化私有数据。
...
foo_inode_info 应始终通过 alloc_inode_sb() 而非 kmem_cache_alloc() 或
...
->read_super() 已不复存在。DECLARE_FSTYPE 和 DECLARE_FSTYPE_DEV 同理。

Turn your foo_read_super() into a function that would return 0 in case of
success and negative number in case of error (-EINVAL unless you have more

将你的 foo_read_super() 改成一个函数，成功时返回 0，出错时返回负数
...
（或者视文件系统类型，用 s/bdev/nodev/ 或 s/bdev/single/ 的类似写法）。

Replace DECLARE_FSTYPE... with explicit initializer and have ->get_sb set as
foo_get_sb.

将 DECLARE_FSTYPE... 替换为显式初始化器，并将 ->get_sb 设为
foo_get_sb。
...
加锁变更：->s_vfs_rename_sem 仅由跨目录重命名获取。很可能你不需要
...
现在我们有了 ->lookup() 与目录删除（通过 ->rmdir() 和 ->rename()）
之间的互斥。如果你曾经需要该互斥并通过内部加锁来实现（大多数文件系统
根本不关心）——你可以放宽你的加锁。
...
->lookup()、->truncate()、->create()、->unlink()、->mknod()、->mkdir()、
->rmdir()、->link()、->lseek()、->symlink()、->rename()
和 ->readdir() 现在不再持有 BKL 时被调用。在入口处获取它，在返回时释放
——这将保证与你以往拥有的相同加锁。如果你的方法或其部分不需要 BKL——
那就更好了，现在你可以移动 lock_kernel() 和 unlock_kernel()，使它们
恰好保护需要保护的内容。
...
BKL 也已从 sb 操作周围移走。BKL 本应被移入各个文件系统自己的 sb_op
函数。如果你不需要它，就移除它。
...
对 ->link() 目标不是目录的检查已由调用方完成。可以放心地去掉它……
...
->link() 的调用方对我们所链接到的对象持有 ->i_mutex。你的一些问题可能
就此解决了……
...
FS_LITTER 已不存在——只需从 fs_flags 中移除它。
...
FS_SINGLE 已不存在（实际上，它在 ->get_sb() 引入时就已经消失——只是
没有被文档记录 ;-/）。只需从 fs_flags 中移除它（并参见 ->get_sb() 条目
了解其他操作）。
...
->setattr() 现在不再持有 BKL 时被调用。调用方_始终_持有 ->i_mutex，因此
要注意你可能被 ->setattr() 使用的获取 ->i_mutex 的代码。notify_change()
的调用方现在需要 ->i_mutex。
...
简而言之，它允许定义 decode_fh 和 encode_fh 操作来编码和解码文件句柄
（filehandle），并允许文件系统为 decode_fh 使用一个标准辅助函数，以及
为该辅助函数提供文件系统特定的支持，尤其是 get_parent。
...
iget4() 和 read_inode2 回调已被 iget5_locked() 取代
...
'test' 是一个附加函数，当 inode 号不足以标识实际文件对象时可使用它。
'set' 应是一个非阻塞函数，负责初始化新创建 inode 的那些部分，以使
test 函数能够成功。'data' 作为一个不透明值传递给 test 和 set 两个函数。
...
当 inode 由 iget5_locked() 创建时，它会被返回且带有 I_NEW 标志，并仍被
锁定。然后文件系统需要完成初始化。一旦 inode 初始化完成，必须通过调用
unlock_new_inode() 解锁。
...
iget_locked 函数，它只接受 super_block 和 inode 号作为参数，并为你
完成 test 和 set。
...
注意，如果建立新 inode 的过程失败，则应在该 inode 上调用 iget_failed()
使其失效，并向调用方返回适当的错误。
...
->getattr() 终于被用上了。参见 nfs、minix 等的实例。
...
并让它调用你原来的 ->revlidate()，且（对于曾拥有 ->revalidate() 的
...
安全的：
 ...
 - 文件系统没有跨目录 rename()
 - 我们知道父目录已被锁定（例如，我们正在查看 ->lookup() 参数的
	  ->d_parent）。
 - 我们正从 ->rename() 中被调用。
 - 子项的 ->d_lock 被持有
...
审查你的代码，并在需要时添加加锁。请注意，即便在旧代码树中，任何不
受上述条件保护的地方也是有风险的——你曾依赖 BKL，而那很容易出错。旧代码树
有相当多此类漏洞——对 ->d_parent 的无保护访问会导致从 oops 到静默内存
损坏等各种问题。
...
FS_NOMOUNT 已不存在。如果你使用它——只需在 flags 中设置 SB_NOUSER
（参见 rootfs 作为一类解决方案，以及 bdev/socket/pipe 作为另一类）。
...
使用 bdev_read_only(bdev) 取代 is_read_only(kdev)。后者仍然存在，但仅仅
是因为 drivers/s390/block/dasd.c 中的混乱。一旦它被修复，is_read_only()
就会消失。
...
->permission() 现在不再持有 BKL 时被调用。在入口处获取它，在返回时释放
——这将保证与你以往拥有的相同加锁。如果你的方法或其部分不需要 BKL——
那就更好了，现在你可以移动 lock_kernel() 和 unlock_kernel()，使它们
恰好保护需要保护的内容。
...
有意为之；只要 struct block_device * 被该代码以合理方式传播，修复将
变得轻而易举；在此之前无能为力。
...
在 ->write_begin 出错退出时的块截断，以及 ->direct_IO 已从通用方法
（block_write_begin、cont_write_begin、nobh_write_begin、
blockdev_direct_IO*）移至调用方。参见 ext2_write_failed 及其调用方作为
示例。
...
->truncate 已不存在。整个截断序列需要在 ->setattr 中实现，对于实现了
磁盘上大小变更的文件系统，该方法现在是强制的。从旧版 inode_setattr
和 vmtruncate 的副本开始，并将 vmtruncate + foofs_vmtruncate 序列重新排序为
使用 block_truncate_page 或类似辅助函数清零块、更新大小，最后进行不应失败的
磁盘截断。setattr_prepare（原名 inode_change_ok）现在包含对 ATTR_SIZE 的
...
->clear_inode() 和 ->delete_inode() 已不存在；应改用 ->evict_inode()。
只要 inode 被逐出（无论它是否还有剩余链接），它就会被调用。调用方**不会**
逐出 pagecache 或 inode 关联的元数据缓冲区；该方法必须使用
truncate_inode_pages_final() 来清除它们。调用方确保在 ->evict_inode()
被调用时（或之后）不会对该 inode 运行异步回写。
...
->drop_inode() 现在返回 int；它在最终 iput() 时被调用，持有
inode->i_lock，如果文件系统希望丢弃该 inode 则返回 true。和以前一样，
...
和以前一样，clear_inode() 必须在每次 ->evict_inode() 调用时恰好调用一次
（正如以前对每次 ->delete_inode() 调用那样）。与以前不同的是，如果你使用
inode 关联的元数据缓冲区（即 mark_buffer_dirty_inode()），在 clear_inode()
...
注意：在 ->write_inode() 开头检查 i_nlink，若为零就退出的做法**现在不是**
且**从来都不曾**足够。最终的 unlink() 和 iput() 可能在该 inode 正处于
->write_inode() 过程中时发生；例如，如果你盲目释放磁盘上的 inode，你可能会
在 ->write_inode() 正在向它写入时这么做。
...
.d_delete() 现在只是向 dcache 建议是否缓存未引用的 dentries，并且现在
仅在 dentry 引用计数降为 0 时被调用。即使在引用计数降为 0 的转变时，它也
必须能够容忍被调用 0 次、1 次或多次（例如，常量、幂等）。
...
dcache_lock 已不存在，被细粒度锁取代。关于应分别用哪些锁来取代 dcache_lock
以保护特定内容，详见 fs/dcache.c。大多数情况下，文件系统只需要 ->d_lock，
它保护给定 dentry 的**所有** dcache 状态。
...
命名空间中拥有过路径名），则必须以 RCU 方式释放其 inode。
...
尽管 i_dentry 和 i_rcu 在一个联合中共享存储，我们将在 inode_init_always()
中初始化前者，因此在回调中放着不管即可。过去需要在那里清理它，但现在
不再需要了（自 3.2 起）。
...
vfs 现在尝试以"rcu-walk 模式"进行路径遍历，这避免了 dentries 和 inodes 上的
原子操作及可伸缩性隐患（见 Documentation/filesystems/path-lookup.txt）。上文
的 d_hash 和 d_compare 变更就是为支持这一点所需改动的示例。对于更复杂的文件系统
回调，vfs 会在调用文件系统之前退出 rcu-walk 模式，因此文件系统无需改动。然而，
这代价高昂并会丧失 rcu-walk 模式的优势。我们将开始添加感知 rcu-walk 的文件系统
...
退出 rcu-walk 模式。现在它可能在 rcu-walk 模式（nd->flags & LOOKUP_RCU）下被
调用。如果文件系统无法处理 rcu-walk，应返回 -ECHILD。详见
...
permission 是一个 inode 权限检查，在路径遍历向下过程中对许多或全部目录 inode
...
在 ->fallocate() 中，你必须检查传入的 mode 选项。如果你的文件系统不支持穿孔
（在文件中间释放空间），则当 mode 中设置了 FALLOC_FL_PUNCH_HOLE 时必须返回
-EOPNOTSUPP。目前你只能在同时设置了 FALLOC_FL_KEEP_SIZE 时使用
...
->get_sb() 和 ->mount() 已不存在。请切换到使用新的挂载 API。详见
...
->permission() 和 generic_permission() 已不再有 flags 参数；我们不再传入
IPERM_FLAG_RCU，而是将 MAY_NOT_BLOCK 加入 mask。

generic_permission() has also lost the check_acl argument; ACL checking
has been taken to VFS and filesystems need to provide a non-NULL
->i_op->get_inode_acl to read an ACL from disk.

generic_permission() 也不再具有 check_acl 参数；ACL 检查已被纳入 VFS，
文件系统需要提供一个非 NULL 的 ->i_op->get_inode_acl 来从磁盘读取 ACL。
...
如果你实现了自己的 ->llseek()，你必须处理 SEEK_HOLE 和 SEEK_DATA。你可以通过
返回 -EINVAL 来处理，但最好以某种方式支持它。通用处理函数假定整个文件都是
数据，并且文件末尾有一个虚拟的空洞。因此，如果提供的偏移量小于 i_size 且
指定了 SEEK_DATA，则返回相同的偏移量。如果上述对偏移量成立且你收到 SEEK_HOLE，
则返回文件末尾。如果偏移量为 i_size 或更大，则两种情况下都返回 -ENXIO。
...
如果你有自己实现的 ->fsync()，你必须确保调用 filemap_write_and_wait_range()，
以便所有脏页被正确同步写出。你还必须记住，->fsync() 不再在持有 i_mutex 时
被调用，因此如果你需要 i_mutex 加锁，你必须确保自己获取并在之后释放它。
...
d_alloc_root() 已不存在，连同由滥用它的代码引起的许多 bug 一起。替代者：
d_make_root(inode)。成功时 d_make_root(inode) 分配并返回一个用传入 inode 实例化的
新 dentry。失败时返回 NULL，并且传入的 inode 被丢弃，因此在所有情况下对 inode 的
引用都被消费，错误处理无需对 inode 做任何清理。如果 d_make_root(inode) 被传入一个
NULL inode，它返回 NULL：
...
女巫死了！嗯，至少死了 2/3。->d_revalidate() 和 ->lookup() 不再接受
...
->create() 不再接受 `struct nameidata *`；与前两个不同，它获得一个"它是
O_EXCL 或等价的吗？"布尔参数。注意本地文件系统可以忽略这个参数——它们被保证
对象不存在。真正可能在意的是远程/分布式文件系统……
...
FS_REVAL_DOT 已不存在；如果你曾经使用它，请改在你的 dentry 操作中添加
->d_weak_revalidate()。
...
vfs_follow_link 已被移除。文件系统必须对普通符号链接使用来自 ->follow_link 的
...
iget5_locked()/ilookup5()/ilookup5_nowait() 的 test() 回调过去在同时持有
->i_lock 和 inode_hash_lock 时被调用；前者**不再**被持有，因此请确认你的回调
不依赖于它（树内所有实例都没有依赖）。当然，inode_hash_lock 仍被持有，因此它们
相对于从 inode 哈希中移除、以及相对于 iget5_locked() 的 set() 回调仍然是串行的。
...
切勿直接调用 ->read() 和 ->write()；请使用 __vfs_{read,write} 或包装函数；
不要检查 ->write 或 ->read 是否为 NULL，而是查看 file->f_mode 中的
FMODE_CAN_{WRITE,READ}。
...
请勿对 ->read/->write 使用 new_sync_{read,write}；而是将其保留为 NULL。
...
对于内嵌（"fast"）符号链接，只需将 inode->i_link 设为符号链接正文所在处，并
...
->follow_link() 的调用约定已改变。我们不再返回 cookie 并使用 nd_set_link() 存储
要遍历的正文，而是返回要遍历的正文，并使用显式的 void ** 参数存储 cookie。
...
->put_link() 的调用约定已改变。它获得 inode 而非 dentry，完全不再获得 nameidata，
并且仅在 cookie 非 NULL 时被调用。注意链接正文不再可用，因此如果你需要它，请将其
作为 cookie 存储。
...
开始操作其 pagecache 之前调用 inode_nohighmem(inode)。此类符号链接的 pagecache 中
不应出现高端内存（highmem）页。这包括在符号链接创建过程中可能做的任何预置。
page_symlink() 会遵循映射的 gfp 标志，因此一旦你调用了 inode_nohighmem() 就可以
安全使用它，但如果你是手动分配并插入页面，请确保使用正确的 gfp 标志。
...
->follow_link() 被 ->get_link() 取代；API 相同，除了：
...
->get_link() 现在获得 struct delayed_call `*done`，并且应在过去设置 `*cookie`
的地方改做 set_delayed_call()。

->put_link() is gone - just give the destructor to set_delayed_call()
in ->get_link().

->put_link() 已不存在——只需在 ->get_link() 中将析构函数交给 set_delayed_call()。
...
->getxattr() 和 xattr_handler.get() 的 dentry 和 inode 被分开传入。dentry 可能
尚未附加到 inode，因此实例中**不要**使用其 ->d_inode。理由：!@#!@# 
security_d_instantiate() 需要在我们将 dentry 附加到 inode 之前被调用。
...
符号链接不再是唯一在 inode 逐出时**不**将 i_bdev/i_cdev/i_pipe/i_link 联合清零的
inode。因此，你不能再假定 ->destroy_inode() 时 ->i_nlink 中的非 NULL 值意味着
它是符号链接。现在确实有必要检查 ->i_mode。在树内我们不得不修复曾经走这种
捷径的 shmem_destroy_callback()；请当心，因为该捷径已不再有效。
...
->i_mutex 现在被 ->i_rwsem 取代。inode_lock() 等的工作方式如同以往——它们只是
...
不：
 ...
 - 单独使用 d_instantiate() 和 d_rehash()——请改用 d_add() 或
	  d_splice_alias()。
 - 单独使用 d_rehash()——请改调用 d_add(new_dentry, NULL)。
 - 在极少数情况下，如果（只读）访问文件系统数据结构出于某种原因需要互斥，
	  请自行安排。树内没有任何文件系统需要那样做。
 - 依赖 ->d_parent 和 ->d_name 在 dentry 被交给 d_add() 或
...
我们保证同一目录中相同名称的查找不会并行发生（"相同"指你的 ->d_compare()
...
新增了 ->iterate_shared()。在 struct file 级别上的互斥仍然提供（以及它与同一
...
你可能需要某种机制来串行化对它们的访问。如果你进行了 dcache 预置，你将需要
为此切换到 d_alloc_parallel()；请查找树内示例。
...
不带 O_CREAT 的 ->atomic_open() 调用可能会并行发生。
...
->setxattr() 和 xattr_handler.set() 的 dentry 和 inode 被分开传入。
xattr_handler.set() 会收到该 inode 所属挂载的用户命名空间，以便文件系统
相应地对 i_uid 和 i_gid 进行 id 映射。dentry 可能尚未附加到 inode，因此
实例中**不要**使用其 ->d_inode。理由：!@#!@# security_d_instantiate() 需要在
我们将 dentry 附加到 inode 之前被调用，并且 !@#!@##!@$!$#!@#$!@$!@$ smack 的
...
->d_compare() 不再将父目录作为单独参数。如果你曾用它来寻找相关的 struct
...
->rename() 新增了一个 flags 参数。文件系统未处理的任何标志都应导致返回 EINVAL。
...
->getattr() 现在传入一个 struct path，而不是单独的 vfsmount 和 dentry，并且
它现在有 request_mask 和 query_flags 参数来指定 statx 所请求的字段和同步类型。
不支持任何 statx 特定特性的文件系统可以忽略这些新参数。
...
此外，"调用了 finish_no_open()，由你自己打开"这种情况的返回值变成了 0 而非 1。
由于 finish_no_open() 本身现在返回 0，那部分在 ->atomic_open() 实例中不需要任何
改动。
...
alloc_file_pseudo(inode, vfsmount, name, flags, ops) 用于需要创建 dentry 的情况；
这是大多数旧 alloc_file() 用户的情形。调用约定：成功时返回一个对新 struct file
的引用，调用方对 inode 的引用被其吸收。失败时返回 ERR_PTR()，且不影响调用方的任何
引用，因此调用方需要释放它持有的 inode 引用。
alloc_file_clone(file, flags, ops) 不影响调用方的任何引用。成功时你获得一个与原始
共享挂载/dentry 的新 struct file，失败时——返回 ERR_PTR()。
...
->clone_file_range() 和 ->dedupe_file_range 已被 ->remap_file_range() 取代。
详见 Documentation/filesystems/vfs.rst。
...
无需费心做该检查——当传入的 inode 为 ERR_PTR(...) 时，d_splice_alias() 会做正确的
事。此外，向 d_splice_alias() 传入 NULL inode 也会做正确的事（等价于
...
将 ->destroy_inode() 中 RCU 延迟的部分提取到一个新方法 ->free_inode() 中。如果
->destroy_inode() 变空——那就更好了，直接去掉它。同步工作（例如无法从 RCU 回调中
完成的事情，或任何我们希望获得栈回溯的 WARN_ON()）**可能**可以移到 ->evict_inode()；
不过，这只适用于那些不需要用来平衡 ->alloc_inode() 所完成之事的内容。换言之，如果
...
inode 销毁规则：
 ...
 - 如果 ->free_inode() 非 NULL，则会被 call_rcu() 调度
 - NULL ->destroy_inode 与 NULL ->free_inode 的组合被视为
...
call_rcu()）与超级块销毁**没有**顺序保证；事实上，超级块及所有相关结构
...
DCACHE_RCUACCESS 已不存在；在 dentry 释放时带有 RCU 延迟是默认行为。DCACHE_NORCU
...
[本应在 2016 年加入] 尽管 finish_open() 中有过时的注释，->atomic_open() 实例中
的失败退出**不应** fput() 该文件，无论如何都不应。一切由调用方处理。
...
析构函数是 kern_unmount() 或 kern_unmount_array()。
...
对于基于 bvec 的迭代器，bio_iov_iter_get_pages() 现在不再复制 bvec，而是使用提供的
那一个。任何发起 kiocb-I/O 的人都应确保 bvec 和页引用一直保持到 I/O 完成，即直到
...
mnt_want_write_file() 现在只能与 mnt_drop_write_file() 配对，而此前它也可以与
mnt_drop_write() 配对。
...
区别在于 copy_page_from_iter_atomic() 会推进迭代器，之后你不需要 iov_iter_advance()。
但是，如果你决定只使用所获数据的一部分，你应该做 iov_iter_revert()。
...
file_open_root() 的调用约定改变了；现在它接受 struct path * 而非分别传入挂载和
dentry。对于过去传入 <mnt, mnt->mnt_root> 对（即给定挂载的根）的调用方，提供了一个
...
no_llseek 已不存在；不要将 .llseek 设为它——只需将其保留为 NULL。对"该文件是否有
...
filldir_t（readdir 回调）的调用约定已改变。它现在返回 bool，而非 0 或 -E...。
false 表示"没有了"（如同过去的 -E...），true 表示"继续"（如同旧调用约定中的 0）。
理由：调用方反正从未查看具体的 -E... 值。->iterate_shared() 实例完全无需改动，树内
所有 filldir_t 都已被转换。
...
->tmpfile() 的调用约定已改变。它现在接受一个 struct file 指针而非 struct dentry
指针。d_tmpfile() 也做了类似改动以简化调用方。传入的文件处于未打开状态，成功时
...
page_entry_size，并且可能在未持有 mmap_lock 时被调用。树内所有用户都已被审查，似乎
不依赖持有 mmap_lock，但树外用户应自行核实。如果它们确实需要，可以返回
VM_FAULT_RETRY 以便在持有 mmap_lock 时被调用。
...
由于加锁顺序要求，打开块设备不能在 s_umount 下进行，因此现在在打开块设备时会释放
s_umount，并在调用 fill_super() 之前重新获取。
...
在旧逻辑中，并发挂载者会在文件系统类型的超级块列表中找到该超级块。由于块设备的
第一个打开者会持有 s_umount，它们会等待直到该超级块要么"出生"，要么因初始化失败
而被丢弃。
...
由于新逻辑释放了 s_umount，并发挂载者可能获取 s_umount 并自旋。相反，现在它们使用
...
块设备的持有者过去是 file_system_type，这并不特别有用。如果不匹配超级块中存储的
设备指针，就无法从块设备回溯到所属超级块。该机制只能用于单个设备，因此块层无法
找到任何额外设备的所属超级块。
...
在旧机制中，为竞态中的 mount(2) 和 umount(2) 复用或创建超级块依赖于作为持有者的
...
(1) 任何成功获取现有超级块活动引用的并发挂载者会被要求等待，直到该超级块要么
    准备就绪，要么从文件系统类型的超级块列表中被移除。如果超级块已就绪，调用方
    只需复用它。
...
(2) 如果挂载者在 deactivate_locked_super() 之后、但超级块从文件系统类型的超级块
    列表中被移除之前到来，挂载者会等待直到该超级块关闭，复用块设备并分配一个新的
    超级块。
...
(3) 如果挂载者在 deactivate_locked_super() 之后、且超级块已从文件系统类型的超级块
...
的任何超级块的块设备，而不必担心看到 EBUSY，因为该块设备仍被另一个超级块使用。
...
让超级块成为块设备的拥有者改变了这一点，因为持有者现在是一个唯一的超级块，因此
与之关联的块设备不能被并发挂载者复用。所以，(2) 中的并发挂载者在尝试打开一个持有者为
不同超级块的块设备时，可能突然看到 EBUSY。
...
(2) 如果挂载者在 deactivate_locked_super() 之后、但超级块从文件系统类型的超级块
    列表中被移除之前到来，挂载者会被要求等待，直到超级块和设备在 ->kill_sb() 中关闭，
    并且该超级块从文件系统类型的超级块列表中被移除。挂载者将分配一个新的超级块并获取
    块设备的所有权（块设备的 bd_holder 指针将被设为新分配的超级块）。
...
(3) 这种情况现在被并入 (2)，因为超级块会一直留在文件系统类型的超级块列表中，直到所有
    设备在 ->kill_sb() 中关闭。换言之，如果超级块已不在文件系统类型的超级块列表中，
    那么它已经放弃了所有关联块设备的所有权（bd_holder 指针为 NULL）。

As this is a VFS level change it has no practical consequences for filesystems
other than that all of them must use one of the provided kill_litter_super(),
kill_anon_super(), or kill_block_super() helpers.

由于这是一个 VFS 级别的变更，它对文件系统没有实际影响，只是所有文件系统都必须使用
所提供的 kill_litter_super()、kill_anon_super() 或 kill_block_super() 辅助函数之一。
...
加锁顺序已改变，使得 s_umount 再次排在 open_mutex 之上。所有在 open_mutex 下获取
s_umount 的地方都已被修正。
...
export_operations ->encode_fh() 不再有用于编码 FILEID_INO32_GEN* 文件句柄的默认
...
如果 ->rename() 在跨目录移动时对 .. 的更新需要目录修改的互斥，请**不要**在你的
->rename() 中锁定相关的子目录——现在它由调用方完成 [该项本应在 28eceeda130f
...
在同目录 ->rename() 中，对 .. 的（同义反复式的）更新不受任何锁保护；如果旧父目录
与新父目录相同，就干脆不要做它。我们确实无法在同目录重命名中锁定两个子目录——否则
会死锁。
...
lock_rename() 和 lock_rename_child() 在跨目录情况下可能失败，如果它们的参数没有
共同祖先。此时返回 ERR_PTR(-EXDEV)，且不获取任何锁。树内用户已更新；树外用户需要
自行更新。
...
锚定在父 dentry 中的子项列表现在变成了 hlist。字段名也改了（锚点/子项分别用
->d_children/->d_sib 取代 ->d_subdirs/->d_child），因此任何受影响的地方会被编译器
立即捕获。
...
->d_delete() 实例现在在持有 ->d_lock 且引用计数为 0 的 dentries 上被调用。它们
不允许释放/重新获取 ->d_lock。树内实例都没有做这类事情。请确保你的实例也不要……
...
->d_prune() 实例现在在父目录未持有 ->d_lock 时被调用。dentry 自身的 ->d_lock 仍被
...
->d_iput() 和 ->d_release() 在受害者 dentry 仍位于父目录的子项列表中被调用。它
...
在此变更之前，get_active_super() 只能找到主块设备（即存储在 sb->s_bdev 中的那个）
的超级块。块设备冻结现在适用于给定超级块拥有的任何块设备，而不仅仅是主块设备。
get_active_super() 辅助函数和 bd_fsfreeze_sb 指针已不存在。
...
set_blocksize() 现在接受已打开的 struct file 而非 struct block_device，并且它
**必须**以独占方式打开。
...
->d_revalidate() 获得两个额外的参数——父目录的 inode 和我们的 dentry 预期拥有的
名字。两者都是稳定的（在非 RCU 情况下 dir 被固定，在 RCU 情况下调用期间也会保留，
且名字保证保持不变）。你的实例不一定要使用它们，但使用它们通常有助于避免大量痛苦的
样板代码。注意，虽然 name->name 是稳定且以 NUL 结尾的，但它可能（且经常）使得
name->name[name->len] 等于 '/' 而不是 '\0'——正常情况下它指向正在查找的路径名。
...
kern_path_locked() 和 user_path_locked() 不再返回负 dentry，因此无需检查这一点。
...
lookup_one_qstr_excl() 被改为在更多情况下返回错误，因此这些条件无需显式检查：
 ...
 - 如果未给定 LOOKUP_CREATE，则 dentry 不会是负的，改为返回
   ERR_PTR(-ENOENT)
 - 如果给定了 LOOKUP_EXCL，则 dentry 不会是正的，改为返回
   ERR_PTR(-EEXIST)

LOOKUP_EXCL now means "target must not exist".  It can be combined with
LOOK_CREATE or LOOKUP_RENAME_TARGET.

LOOKUP_EXCL 现在意为"目标必须不存在"。它可以与 LOOK_CREATE 或
...
->mkdir() 现在返回一个 dentry。如果创建的 inode 被发现已在缓存中且拥有一个 dentry
（通常是 IS_ROOT()），则需要将其拼接进给定名字以取代给定的 dentry。现在需要返回那个
dentry。如果使用了原始 dentry，则应返回 NULL。任何错误都应通过 ERR_PTR() 返回。
...
一般来说，使用 d_instantiate_new() 安装新 inode 的文件系统可以安全返回 NULL。可能
...
如果出于某种原因无法返回正的 dentry，诸如 cachefiles、nfsd、smb/server 等内核内
...
lookup_one()、lookup_one_unlocked()、lookup_one_positive_unlocked() 现在接受一个
qstr 而非 name 和 len。每当通过挂载点（它将有一个 mnt_idmap）从文件系统外部访问该
...
函数 try_lookup_one_len()、lookup_one_len()、lookup_one_len_unlocked() 和
lookup_positive_unlocked() 已重命名为 try_lookup_noperm()、lookup_noperm()、
lookup_noperm_unlocked()、lookup_noperm_positive_unlocked()。它们现在接受 qstr 而非
分开的 name 和 length。当需要以 strlen() 作为长度时，可以使用 QSTR()。
...
这些函数不再做任何权限检查——它们过去会检查调用方在父目录上拥有 'X' 权限。它们
...
使用 try_lookup_noperm() 需要包含 linux/namei.h。
...
->d_automount() 的调用约定已改变；我们**不应**对新挂载获取额外引用——它应以引用计数
1 返回。
...
collect_mounts()/drop_collected_mounts()/iterate_mounts() 现在已不存在。取代者
是 collect_paths()/drop_collected_path()，无需特殊迭代器。新接口不再返回克隆的挂载
树，而是返回一个 struct path 数组，collect_mounts() 原本会为每个挂载创建一个。这些
...
如果你的文件系统设置了默认 dentry_operations，请使用 set_default_d_op() 而非手动
设置 sb->s_d_op。
...
请利用 d_splice_alias_ops() 来设置它们。更好的做法是，认真考虑你是否真的需要为不同的
dentries 使用不同的 ->d_op——如果不需要，只需在挂载时使用 set_default_d_op() 并了事。
目前 procfs 是唯一真正需要 ->d_op 在 dentries 之间变化的东西。
...
文件操作 mmap() 回调已被弃用，推荐使用 mmap_prepare()。它会向回调传递一个指向
vm_area_desc 的指针，而非 VMA，因为在此阶段 VMA 尚未有效。

The vm_area_desc provides the minimum required information for a filesystem
to initialise state upon memory mapping of a file-backed region, and output
parameters for the file system to set this state.

vm_area_desc 提供了文件系统在文件后备（file-backed）区域内存映射时初始化状态所需的
...
vfs_parse_fs_string() 的调用约定已改变；它**不再**接受长度参数（使用
value ? strlen(value) : 0）。如果你想要不同的长度，请使用
...
vfs_mkdir() 现在返回一个 dentry——即 ->mkdir() 所返回的那个。如果该 dentry 与传入的
dentry 不同（包括它是 IS_ERR() dentry 指针的情况），原始 dentry 会被 dput()。
...
当 vfs_mkdir() 返回错误，从而既 dput() 原始 dentry 又不提供替代时，它还会解锁父目录。
因此 vfs_mkdir() 的返回值可以传给 end_creating()，而父目录会在恰好必要时被解锁。
...
NULL 时，内核现在会对设置租约的尝试返回 -EINVAL。希望使用内核内部租约实现的文件系统
应将其设为 generic_setlease()。
...
do_mkdirat()、do_mknodat()、do_unlinkat() 和 do_rmdir()）已不存在；它们被非消费性的
对应物取代（filename_renameat2() 等）。调用方已相应调整——现在释放文件名的责任属于
它们。
...
如果你确实需要遍历给定 inode 的 dentries，请使用 for_each_alias(dentry, inode) 而非
hlist_for_each_entry；更好的做法是，看看是否能用任何导出的原语来取代整个循环。无论
哪种形式的手动循环，你仍然需要在循环期间持有该 inode 的 ->i_lock。