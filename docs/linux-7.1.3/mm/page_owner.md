...
当发生分配时，关于分配的信息（如调用栈和页面的 order）会被存储到每个页面的特定存储中。
当我们需要了解所有页面的状态时，可以获取并分析这些信息。

尽管我们已经有用于跟踪页面分配/释放的 tracepoint，但用它来分析谁分配了每个页面相当
复杂。我们需要扩大 trace 缓冲区，以防止在用户空间程序启动前发生重叠。而且，启动的程序会
持续转储 trace 缓冲区以供后续分析，这比仅仅将其保留在内存中更可能改变系统行为，因此不
利于调试。

page owner 也可用于各种其他目的。例如，通过每个页面的 gfp 标志信息可以获得准确的碎片
统计。如果启用了 page owner，这已经实现并激活。欢迎其他用法。

它还可用于显示所有调用栈及其当前分配的基页数量，这让我们无需筛查所有页面并匹配分配和
释放操作，就能快速了解内存的去向。也可以只显示所有调用栈（不含栈回溯）的数字标识符及其
分配的基页数量（读取和解析更快，例如用于监控），之后可以与调用栈匹配（show_handles 和
show_stacks_handles）。

page owner 默认是禁用的。因此，如果你想使用它，需要在启动命令行中添加“page_owner=on”。
如果内核构建了 page owner，但由于未启用启动选项而在运行时被禁用，运行时开销是微乎其微的。
如果在运行时禁用，则不需要内存来存储所有者信息，因此没有运行时内存开销。而且，page owner
只向页面分配器热路径中插入了两个不太可能执行的分支，如果未启用，则分配就像没有 page owner
的内核一样进行。这两个不太可能执行的分支不应影响分配性能，特别是在可用静态键跳转标签
补丁（static keys jump label patching）功能的情况下。以下是该设施导致的内核代码大小变化。

虽然启用 page owner 会使内核大小增加几千字节，但其中大部分代码位于页面分配器及其热路径
之外。在需要时用启用 page owner 构建内核并在需要时打开它，是调试内核内存问题的绝佳选择。

有一个由实现细节引起的注意事项。page owner 将信息存储到 struct page extension 的内存中。
在稀疏内存（sparse memory）系统中，这块内存的初始化时间晚于页面分配器启动的时间，因此在
初始化之前，许多页面可能已经被分配，它们将没有所有者信息。为了修复这一点，这些早期分配的
页面在初始化阶段被检查并标记为已分配。虽然这并不意味着它们有正确的所有者信息，但至少我们
可以更准确地判断页面是否被分配。在一个 2GB 内存的 x86-64 虚拟机上，捕获并标记了 13343 个
