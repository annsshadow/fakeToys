...
KVM 使用一些自定义的 MSR 来处理某些请求。

自定义 MSR 有一个为其保留的区间，范围从 0x4b564d00 到 0x4b564dff。在此区间之外也存在一些 MSR，但它们已被弃用，不建议使用。
...
data:
	一个内存区域的 4 字节对齐物理地址，该区域必须位于客户机 RAM 中，
...
data:
	...
	位于客户机 RAM 中，外加 bit 0 中的使能位。该内存预期用于
```
...
data:
	当 vCPU 上启用了 PV（半虚拟化）中断结束时，bit 0 为 1；禁用时为 0。bit 1 为保留位，必须为 0。当 PV 中断结束被启用（bit 0 置位）时，bit 63-2 保存一个 4 字节对齐的物理地址，指向一个 4 字节内存区域，该区域必须位于客户机 RAM 中且必须被清零。

	该 4 字节内存区域的最低有效位（第一位）将由 hypervisor 写入，通常是在注入中断时。值为 1 表示客户机可以跳过向 APIC 写入 EOI（通过 MSR 或 MMIO 写）；相反，只需通过清除客户机内存中的该位来发出 EOI 信号——该位置稍后会被 hypervisor 轮询。值为 0 表示需要进行 EOI 写操作。

	客户机忽略该优化并直接执行 APIC EOI 写操作始终是安全的。

	Hypervisor 保证只会在当前 VCPU 上下文内修改该最低有效位，这意味着客户机无需使用 lock 前缀或内存排序原语来与 hypervisor 同步。

	然而，hypervisor 可以随时置位或清除该内存位：因此，为了确保 hypervisor 不会在客户机检测是否可以跳过 EOI APIC 写、与清除该位以向 hypervisor 发出 EOI 信号之间的窗口期内打断客户机并清除该内存区域的最低有效位，客户机必须使用单条 CPU 指令（如 test-and-clear 或 compare-and-exchange）同时读取并清除该内存区域的最低有效位。
...
data:
	Bit 0 用于启用（1）或禁用（0）宿主机侧的 HLT 轮询逻辑。

	KVM 客户机可以请求宿主机不要在 HLT 时轮询，例如当它们自身正在进行轮询时。
...
data:
	...
	Bit 0-7：用于投递 'page ready'（页面就绪）APF 事件的 APIC 向量。
	Bit 8-63：保留

	用于异步 'page ready' 通知投递的中断向量。
	该向量必须在异步页错误机制于 MSR_KVM_ASYNC_PF_EN 中启用之前设置好。仅当 CPUID 中存在 KVM_FEATURE_ASYNC_PF_INT 时该 MSR 才可用。
...
data:
	...
	当客户机处理完 'page ready' APF 事件，且 'struct kvm_vcpu_pv_apf_data' 中的 'token' 字段被清除后，应向该 MSR 的 bit 0 写入 '1'，这会促使宿主机重新扫描其队列并检查是否还有更多待处理通知。仅当 CPUID 中存在 KVM_FEATURE_ASYNC_PF_INT 时该 MSR 才可用。

MSR_KVM_MIGRATION_CONTROL:
        0x4b564d08

data:
        仅当 CPUID 中存在 KVM_FEATURE_MIGRATION_CONTROL 时该 MSR 才可用。Bit 0 表示是否允许对客户机进行实时迁移。

        当客户机启动时，若客户机使用了加密内存，bit 0 为 0；若客户机未使用加密内存，bit 0 为 1。如果客户机通过 `KVM_HC_MAP_GPA_RANGE` hypercall 向宿主机通报页加密状态，则它可以将该 MSR 的 bit 0 置位，以允许对客户机进行实时迁移。
