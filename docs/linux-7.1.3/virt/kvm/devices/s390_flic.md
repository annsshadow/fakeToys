...
FLIC 处理浮动（非每 CPU）中断，即 I/O、服务以及某些机器检查（machine check）中断。
...
- 为客机启用/禁用透明的异步页错误（async page faults）
...
组：
  ...
  KVM_DEV_FLIC_GET_ALL_IRQS
    ...
    当缓冲区太小时返回 -ENOMEM，这是指示用户空间用一个更大的缓冲区重试。

    -ENOBUFS 在分配内核空间缓冲区失败时返回。

    -EFAULT 在将数据复制到用户空间失败时返回。所有中断保持挂起，即不会被从当前
    挂起中断列表中删除。attr->addr 包含用户空间缓冲区的地址，所有中断数据将被复制
  ...
  KVM_DEV_FLIC_CLEAR_IO_IRQ
    删除一个（如果存在）I/O 中断，该中断针对由 attr->addr（地址）和 attr->attr（长度）
    所指定缓冲区传入的子系统标识字（subsystem identification word）所标识的子通道
    （subchannel）。

  KVM_DEV_FLIC_APF_ENABLE
    为客机启用异步页错误。因此在大页错误（major page fault）情况下，宿主机被允许异步
    处理它并继续运行客机。

    -EINVAL 在针对 ucontrol VM 的 FLIC 调用时返回。

  KVM_DEV_FLIC_APF_DISABLE_WAIT
    为客机禁用异步页错误，并等待直到已经挂起的异步页错误完成。这对于在迁移中断列表
    之前为每个 init 中断触发一个完成中断是必要的。

    -EINVAL 在针对 ucontrol VM 的 FLIC 调用时返回。

  KVM_DEV_FLIC_ADAPTER_REGISTER
    注册一个 I/O 适配器中断源。接受一个 kvm_s390_io_adapter
```
...
```
注意：在 FLIC 上执行的带有未知组或属性的 KVM_SET_DEVICE_ATTR/KVM_GET_DEVICE_ATTR 设备 ioctl 会给出错误码 EINVAL（而不是 API 文档中规定的 ENXIO）。无法基于使用尝试所产生的错误码来推断某个 FLIC 操作不可用。
