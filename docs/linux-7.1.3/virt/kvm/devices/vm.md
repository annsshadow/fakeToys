...
虚拟机“设备”同样接受 ioctl `KVM_SET_DEVICE_ATTR`、
`KVM_GET_DEVICE_ATTR` 和 `KVM_HAS_DEVICE_ATTR`。该接口使用与其他设备
相同的 `struct kvm_device_attr`，但针对的是虚拟机全局的设置与控制。
...
允许用户空间查询实际限制，并为客户机内存的最大大小设置一个新的限制。
该限制将分别向上取整到 2048 MB、4096 GB、8192 TB，因为此限制由页表
层级数决定。在没有限制的情况下，我们会将限制设为
...
KVM 不以任何形式强制或限制 cpu 模型数据。请把通过
`KVM_S390_VM_CPU_MACHINE` 获取的信息作为合理配置设置的参考。由额外设置的
facility 位触发、但 KVM 未处理的指令拦截，需要在 VM 驱动代码中实现。
...
允许用户空间获取可用的 cpu 特性。若硬件提供且该特性被 kvm 支持，则视为
...
允许用户空间获取或更改某个 VM 所有 VCPU 已启用的 cpu 特性。不可用的特性
无法被启用。
...
允许用户空间获取可用的 cpu 子函数，不做任何由设定 IBC 带来的过滤。这些
子函数通过“查询”或“测试位（test bit）”子函数指示给客户机 VCPU，并被
cpacf 函数、plo 和 ptff 等使用。

只有当 `KVM_S390_VM_CPU_MACHINE` 包含引入相关指令的 STFL(E) 位时，子函数块
才有效。若相关指令通过“查询子函数”指示子函数，则响应块包含在返回的
结构体中；若相关指令通过“测试位”机制指示子函数，则子函数代码以 MSB 0
位编号方式包含在返回的结构体中。
...
允许用户空间获取或更改要指示给某个 VM 所有 VCPU 的 cpu 子函数。仅当内核与
硬件支持就绪时，该属性才可用。

内核使用配置好的子函数块来向客户机指示。仅当关联的 STFL(E) 位未被用户空间
禁用时（即被查询的指令对客户机实际可用），该子函数块才会被使用。

只要尚未写入任何数据，读取就会失败。此种情况下将使用 IBC 来决定可用的
子函数，以保证向后兼容性。
...
允许用户空间设置/获取 POP 中定义的 TOD 时钟寄存器第 0-63 位（u64）。若客户机
CPU 模型支持 TOD 时钟扩展（u8），它还允许用户空间获取/设置该扩展；若客户机
CPU 模型不支持，则将其存为 0 且不允许被设为 != 0 的值。
...
所有内存槽（memslot）上必须启用脏页跟踪，否则返回 `-EINVAL`。当任一
...
KVM 的默认配置允许所有已实现的 SMCCC 调用。因此，用户空间可以稀疏地定义
SMCCC 过滤器，仅需描述那些修改默认行为的范围。

`struct kvm_smccc_filter` 表达的范围为
[`base`, `base + nr_functions`)。该范围不允许回绕，即用户空间不能依赖
`base + nr_functions` 溢出。

SMCCC 过滤器同时适用于客户机发起的 SMC 与 HVC 调用。SMCCC 过滤器会拦截对
SMCCC 调用的内核内模拟，因此其作用早于其他与 SMCCC 调用交互的接口
（例如 hypercall 位图寄存器）。

动作：

 - `KVM_SMCCC_FILTER_HANDLE`：允许该客户机 SMCCC 调用在内核内被处理。强烈
   建议用户空间 **不要** 显式描述允许的 SMCCC 调用范围。

 - `KVM_SMCCC_FILTER_DENY`：在内核内拒绝该客户机 SMCCC 调用并返回给客户机。

 - `KVM_SMCCC_FILTER_FWD_TO_USER`：该客户机 SMCCC 调用被转发到用户空间，
   退出原因为 `KVM_EXIT_HYPERCALL`。

`pad` 字段保留供将来使用，必须为 0。若该字段非零，KVM 可能返回 `-EINVAL`。

KVM 保留了“Arm 架构调用”的函数 ID 范围，并将拒绝为这些范围的任何部分定义
过滤器：
