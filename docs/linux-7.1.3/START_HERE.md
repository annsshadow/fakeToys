...
## 如何使用本指南
- **必读** 标记了建立基础认知不可或缺的文档- **选读** 可按兴趣和需求选择阅读
...
| # | 文档 | 说明 |
|---|------|------|
| 1 | `README` | 按角色（开发研究安全专家/系统管理员）给你的入门指南|
| 2 | `Documentation/index.md` | 文档地图，了解整个文档树的组织结构|
| 3 | `Documentation/admin-guide/README.md` | "内核是什——硬件支持、构建安装、快速概览|
...
| # | 文档 | 说明 |
|---|------|------|
| 6 | `Documentation/kernel-hacking/hacking.md` | **最重要的单篇文档**：CPU 执行上下文（用户/软中硬中空闲）、`current` 指针、调度点、基础|
...
| 11 | `Documentation/RCU/index.md` | RCU 机制：读-复制-更新的核心思想与实践|
...
**阅读建议***
- 每个子系统通常只需要读 `index.md` + 1-2 篇核心设计文档- 不需要按顺序读，根据你的兴趣或工作需求选择
...
| 21 | `Documentation/kbuild/index.md` | 构建系统：顶Makefile、Kconfig 语法、模块构建|
...
- 所有Markdown 文件`tools/docs/rst-to-md.py` Sphinx `.rst` 源文件自动转换生- 如果发现转换质量问题，可以查看原`.rst` 文件或修复转换脚- 文档内容随内核版本更新，本指南基于v7.1.3Baby Opossum Posse"
