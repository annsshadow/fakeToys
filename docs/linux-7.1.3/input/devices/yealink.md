...
如果某个图标也位./lineX 上，则其对应的值会使用该图标的首字母进行更新
...
声音ALSA 驱动 snd_usb_audio 提供支持


设备实际的极限是一16 位通道，采样率与播放率均为 8000 Hz
...
:A: 如果你在 dmesg 中看到类似如下内容：
    ...
    这意味着 hid 驱动抢先占用了该设备。请尝试在任何其usb hid 驱动之前加载 yealink 模块。请参阅你所使用发行版提供的关于模块配置的说明


:Q: 电话现在可以工作了（显示版本并接受按键输入），但我找不到 sysfs 文件
:A: sysfs 文件位于特定usb 端点上。在大多数发行版中，你可以执行："find /sys/ -name get_icons" 来获取线索
