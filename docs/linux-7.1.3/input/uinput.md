...
libevdev evdev 设备的一个封装库，提供了创建 uinput 设备和发送事件的接口。libevdev
比直接访uinput 更不容易出错，新软件应考虑使用它
...
程序需要填充一uinput_user_dev 结构体并将其写入 uinput 文件描述符来配置新的 uinput
设备。新代码不应使用旧接口，而应通过 ioctl 调用uinput 交互，或者使libevdev
