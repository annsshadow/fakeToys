...
API 提供 Kerberos 5 风格的密码学，用于密钥派生、加密和校验和，可供网络文件系统使用，并可用于实GSSAPI 所需的底层加密
```
...
API 使用者感兴趣的字段如下：
  ...
  - `etype` `ctype` 分别指示该编码类型用于加密和校验和的协议编号。它们保`KRB5_ENCTYPE_**` `KRB5_CKSUMTYPE_**` 常量
  - `name` 是该编码的正式名称
  - `key_len` `key_bytes` 是输入密钥长度和派生密钥长度。（我认为它们只DES 时不同，而这里不支持 DES）
  - `Kc_len`、`Ke_len` `Ki_len` 是派生的 Kc、Ke Ki 密钥的大小。Kc 用于校验和模式；Ke Ki 用于加密模式
  - `prf_len` PRF+ 函数计算的结果大小
  - `block_len`、`conf_len` `cksum_len` 分别是加密块长度、混淆数（confounder）长度和校验和长度。三者都用于加密模式，但只有校验和长度用于校验和模式
```
...
为了辅助密钥派生，提供一个函数来计算 Kerberos GSSAPI PRF+
```
...
密钥派生完成后，就可以对数据执行加密操作。调用方在为传输准备消息时，必须在缓冲区中留出用于存放混淆数（如需要）和校验和的空隙。一个枚```
...
```
这两个函数都接受编码类型、传输密钥以及用于派生相应子密钥usage 值。它们创建一个合适的加密对象——用于加密的 AEAD 模板和用于校验和的同步哈希——在其上设置密钥并进行配置。调用方应将这些句柄传递给下面的动作函数
...
```
在这两种情况下，输入和输出缓冲区由同一scatterlist 指示
...
```
在这两种情况下，输入和输出缓冲区由同一scatterlist 指示。可以传入额外的元数据，它会在数据之前被加入哈希
对于 get_mic 函数，输出缓冲区可能比所需更大（返回生成的输出量），并指出数据的位置和大小（必须与编码匹配）
对于验证函数，提供缓冲区中消息的偏移和长度，这些值会被收缩以适应数据。如果校验和不匹配，将返回错误
## krb5enc AEAD 算法


提供了一个名“krb5enc的模AEAD 加密算法，它在加密明文之前先对明文做哈希（与 authenc 相反）。`crypto_krb5_prepare_encryption()` 返回的句柄可能是其中之一，但API 的使用者无需直接与之交互
作为参考，其密钥格式以格式号的 BE32 开头。只提供格式 1，其后跟一Ke 密钥长度BE32，再跟一Ki 密钥长度BE32，然后是 Ke 密钥的字节，再是 Ki 密钥的字节
