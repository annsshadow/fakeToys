...
一般规则是，除非操作系统有其他途径可以发现，否ACPI 命名空间应当描述
操作系统可能用到的一切内[1, 2]
例如，枚PCI 主机桥并没有标准的硬件机制，因此 ACPI 命名空间必须描述
每个主机桥、访问其下方 PCI 配置空间的方法、主机桥转发PCI 的地址空间
窗口（使_CRS），以及传统 INTx 中断的路由（使用 _PRT）
位于主机桥下方的 PCI 设备，通常不需要通过 ACPI 来描述。操作系统可以通过
标准PCI 枚举机制发现它们，使用配置访问来发现并识别设备，以及读取确定它们BAR 大小。不过，如果 ACPI PCI 设备提供了电源管理或热插功能，或者该设备由平台中断控制器连接INTx 中断、并且需_PRT 来描这些连接，那ACPI 也可以描PCI 设备
ACPI 资源描述是通过 ACPI 命名空间中设备的 _CRS 对象完成[^2^]。_CRS 类似
于一种通用化的 PCI BAR：操作系统可以读_CRS 并弄清楚正在被消耗的是何资源，即便它没有该设备的驱动程序 [^3^]。这一点很重要，因为它意味着即使在一
个存在操作系统不认识的新设备的系统上，旧的操作系统也能正确工作。新设备
可能什么都不做，但操作系统至少可以确保没有资源与它们发生冲突
MCFG、HPET、ECDT 等静态表，并**不是**用于预留地址空间的机制。静态表
用于操作系统在能够解ACPI 命名空间之前、在启动早期就需要知道的事情如果定义了一个新表，旧的操作系统即便忽略该表，也需要能正确运行。_CRS 这一点成为可能，因为它是通用的、且能被旧操作系统理解；而静态表则做不到
如果操作系统需要管理一个通过 ACPI 描述的非可发现设备，那么该设备会有一特定_HID/_CID，告诉操作系统应当绑定哪个驱动程序，_CRS 告诉操作系统
和驱动程序设备寄存器在哪里
PCI 主机桥是 PNP0A03 PNP0A08 设备。它们的 _CRS 应当描述它们消耗的所地址空间。这包括它们向下转发PCI 总线的所有窗口，以及主机桥自身那不被转发PCI 的寄存器。主机桥寄存器包括诸如确定桥下方总线范围的主/总线寄存器、描述窗口区间的窗口寄存器等。这些都是设备相关、非架构化的
内容，因PNP0A03/PNP0A08 驱动程序管理它们的唯一途径是通过包含设备相关
细节_PRS/_CRS/_SRS。主机桥寄存器还包括 ECAM 空间，因为它由主机桥所
消耗
ACPI 定义了一Consumer/Producer 位来区分桥寄存器（“Consumer”）与桥窗口
（“Producer”）[4, 5]，但早期BIOS 没有正确使用该位。结果是，当前的 ACPI
规范仅在扩展地址空间描述符中定义Consumer/Producer；在较旧QWord/DWord/
Word 地址空间描述符中应当忽略该位。因此，操作系统不得不假设所有的 QWord/
DWord/Word 描述符都是窗口
在加入扩展地址空间描述符之前，Consumer/Producer 的缺失意味着无法PNP0A03/
PNP0A08 设备自身中描述桥寄存器。变通办法是PNP0C02 兜底设备 [^6^] 中描桥寄存器（包ECAM 空间）。除 ECAM 外，桥寄存器空间本身就是设备相关的，
因此通用PNP0A03/PNP0A08 驱动程序（pci_root.c）无需知道它
新的架构应当能够PNP0A03 设备中使“Consumer扩展地址空间描述符来描述
桥寄存器（包ECAM），尽管[^6^] 的严格解释可能禁止这样做。旧x86 ia64 内核假设所有地址空间描述符（包括 “Consumer扩展地址空间描述符）都是
窗口，因此在那些架构上以这种方式描述桥寄存器并不安全
PNP0C02 “主板”设备基本上是一个兜底设备。除了“不要把这类资源用于其他任何
用途”之外，对它们没有别的编程模型。因PNP0C02 _CRS 应当声明任何满足
以下条件的地址空间1) 未被 ACPI 命名空间中任何其他设备对象的 _CRS 所声明(2) 不应由操作系统分配给别的东西
PCIe 规范要求使用增强配置访问方法（ECAM），除非存在标准的固件接口用配置访问，例ia64 SAL 接口 [^7^]。主机桥消ECAM 内存地址空间，并内存访问转换PCI 配置访问。规范定义了 ECAM 地址空间的布局和功能；只有
地址空间的基地址是设备相关的。ACPI 操作系统从静MCFG 表或 PNP0A03 设备
中的 _CBA 方法得知基地址
MCFG 表必须描述非热插拔主机桥ECAM 空间 [^8^]。由MCFG 是静态表、无通过热插拔更新，因此 PNP0A03 设备中的 _CBA 方法描述热插拔主机桥ECAM 空间
[^9^]。注意，对于 MCFG _CBA，基地址始终对应总线 0，即便桥下方的总线范围
（通过 _CRS 报告）不是从 0 开始
...
[^3^] ACPI 6.2，第 6.2 节：
    ...
    sets those resources by calling the device鈥檚 _SRS control method.
...
[^6^] PCI Firmware 3.2，第 4.1.2 节：
    ...
    the resources in this case should not be claimed in the root PCI bus鈥檚
