...
:作 Frank Li <Frank.Li@nxp.com>

本文档是一份指南，帮助用户使用 pci-epf-vntb 功能驱动ntb_hw_epf 主机驱动
来实NTB 功能。下面给出了在主机侧EP 侧需要遵循的步骤列表。有关使可配置端点的 NTB 的硬件配置与内部机制，请参见
...
```
上面"mkdir func1" 创建了将pci_epf_vntb 驱动探测pci-epf-vntb 功能设备
...
```
PCI 端点框架还会在功能属性目录中自动创建一个子目录。该子目录与功能设备名称相同，并填充有以NTB 特定```
...
```
默认情况下，每个构造（construct）会按需并按顺序分配一BAR。如果平台需特定BAR 设置，可以使用相关的 `XYZ_bar` 条目BAR 分配给每个构造
```
...
Test Client，都可以NTB 功能设备一起使用
有关 NTB 的更多信息，请参[Non-Transparent Bridge <../../driver-api/ntb>](Non-Transparent Bridge <../../driver-api/ntb>)
