...
有关 configfs 的工作方式，请参Documentation/filesystems/configfs.rst
## 2. 用法


为了IIO 中使configfs 支持，我们需要在编译时通过 CONFIG_IIO_CONFIGFS 配置选项将其选中
```
...
```
此时，所有默认的 IIO 组都将被创建，并可在 /config/iio 下访问。后续章节将描述可用IIO 配置对象
## 3. 软件触发

IIO 默认 configfs 组之一“triggers（触发器）”组。它configfs 挂载后自动可访问，可/config/iio/triggers 下找到
...
"hrtimer" 触发器类型在 /config 目录下没有任何可配置属性。它会向触发器目录引sampling_frequency 属性。该属性以 Hz 为单位设置轮询频率，精度mHz