...
本驱动支持 Analog Device 的 ADXL345/375，工作于 SPI/I2C 总线。
...
每个 IIO 设备在 `/sys/bus/iio/devices/iio:deviceX` 下都有一个设备文件夹，其中 X 是该设备的 IIO 索引。这些文件夹下包含一组设备文件，具体取决于相关硬件设备的特性与功能。这些文件是统一泛化的，并在 IIO ABI 文档中有说明。
...
其中 _offset 与 _scale 是设备属性。如果不存在 _offset 属性，则直接假定其值为 0。
...
特定的 IIO 事件由其对应的中断触发。传感器驱动支持「无」或「单个」有效中断（INT）线，可从 INT1 或 INT2 两个可用选项中选择。有效的 INT 线应在设备树中指定。如果未配置 INT 线，传感器默认进入 FIFO 旁路模式，此时事件检测被禁用，仅能获取单独的 X、Y、Z 轴测量值。

下表列出了位于设备特定路径 `/sys/bus/iio/devices/iio:deviceX/events` 下的 ADXL345 相关设备文件。注意，活动（activity）与静止（inactivity）检测默认是直流（DC）耦合的；因此，此处仅显式列出交流（AC）耦合的活动与静止事件。

+---------------------------------------------+---------------------------------------------+
| 事件句柄                                    | 说明                                        |
+---------------------------------------------+---------------------------------------------+
| in_accel_gesture_doubletap_en               | 在所有轴上启用双击检测                      |
+---------------------------------------------+---------------------------------------------+
| in_accel_gesture_doubletap_reset_timeout    | 双击窗口，单位 [us]                         |
+---------------------------------------------+---------------------------------------------+
| in_accel_gesture_doubletap_scale            | 双击手势阈值比例。                          |
+---------------------------------------------+---------------------------------------------+
| in_accel_gesture_doubletap_tap2_min_delay   | 双击延迟，单位 [us]                         |
+---------------------------------------------+---------------------------------------------+
| in_accel_gesture_doubletap_value            | 双击阈值                                    |
+---------------------------------------------+---------------------------------------------+
| in_accel_gesture_singletap_scale            | 单击手势阈值比例。                          |
+---------------------------------------------+---------------------------------------------+
| in_accel_gesture_singletap_timeout          | 单击持续时间，单位 [us]                     |
+---------------------------------------------+---------------------------------------------+
| in_accel_gesture_singletap_value            | 单击阈值                                    |
+---------------------------------------------+---------------------------------------------+
| in_accel_mag_adaptive_falling_period        | 交流耦合静止时间，单位秒                    |
+---------------------------------------------+---------------------------------------------+
| in_accel_mag_adaptive_falling_scale         | 交流耦合静止阈值比例。                      |
+---------------------------------------------+---------------------------------------------+
| in_accel_mag_adaptive_falling_value         | 交流耦合静止阈值                            |
+---------------------------------------------+---------------------------------------------+
| in_accel_mag_adaptive_rising_en             | 在 X 轴上启用交流耦合活动检测               |
+---------------------------------------------+---------------------------------------------+
| in_accel_mag_adaptive_rising_scale          | 交流耦合活动阈值比例。                      |
+---------------------------------------------+---------------------------------------------+
| in_accel_mag_adaptive_rising_value          | 交流耦合活动阈值                            |
+---------------------------------------------+---------------------------------------------+
| in_accel_mag_falling_period                 | 静止时间，单位秒                            |
+---------------------------------------------+---------------------------------------------+
| in_accel_mag_falling_scale                  | 直流耦合静止阈值比例。                      |
+---------------------------------------------+---------------------------------------------+
| in_accel_mag_falling_value                  | 静止阈值                                    |
+---------------------------------------------+---------------------------------------------+
| in_accel_mag_rising_en                      | 在 X 轴上启用活动检测                       |
+---------------------------------------------+---------------------------------------------+
| in_accel_mag_rising_scale                   | 直流耦合活动阈值比例。                      |
+---------------------------------------------+---------------------------------------------+
| in_accel_mag_rising_value                   | 活动阈值                                    |
+---------------------------------------------+---------------------------------------------+
| in_accel_x&y&z_mag_adaptive_falling_en      | 在所有轴上启用交流耦合静止检测              |
+---------------------------------------------+---------------------------------------------+
| in_accel_x&y&z_mag_falling_en               | 在所有轴上启用静止检测                      |
+---------------------------------------------+---------------------------------------------+
| in_accel_x_gesture_singletap_en             | 在 X 轴上启用单击检测                       |
+---------------------------------------------+---------------------------------------------+
| in_accel_y_gesture_singletap_en             | 在 Y 轴上启用单击检测                       |
+---------------------------------------------+---------------------------------------------+
| in_accel_z_gesture_singletap_en             | 在 Z 轴上启用单击检测                       |
+---------------------------------------------+---------------------------------------------+

有关该功能的具体说明，请参阅传感器的数据手册（datasheet）。

手动设置 **ODR** 会使驱动为静止检测时序估算默认值，其中较高的 ODR 值对应较长的默认等待时间，较低的 ODR 值对应较短的等待时间。如果这些默认值不能满足你的应用需求，你可以显式配置静止等待时间。将该值设为 0 会恢复默认行为。

更改 **g 量程** 配置时，驱动会依据旧量程与新量程之比对默认值进行缩放，从而估算出合适的活动与静止阈值。所得阈值永远不会为零，且始终落在 1 到 255 之间，对应数据手册中规定的上限 62.5 mg/LSB（0.612915 m/s^2/LSB）。不过，你也可以通过设置显式值来覆盖这些估算阈值。

当 **activity** 与 **inactivity** 事件被启用时，驱动会通过设置 **link** 与 **auto-sleep** 位自动管理迟滞行为。link 位将活动与静止功能相连，使二者相互跟随。auto-sleep 功能在检测到静止时使传感器进入睡眠模式，将功耗降至 12.5 Hz 以下的速率。

静止时间可在 1 到 255 秒之间配置。除静止检测外，传感器还支持自由落体（free-fall）检测；从 IIO 的角度看，自由落体被视为所有轴上幅值的下降。就传感器而言，自由落体由一个 0.000 到 1.000 秒的静止周期来定义。

驱动的行为如下：

- 若配置的静止周期为 1 秒或以上，驱动使用传感器的静止寄存器。这使得该事件能够与活动检测关联、使用 auto-sleep，并可采用交流（AC）或直流（DC）耦合。

- 若静止周期小于 1 秒，则该事件被视为普通静止或自由落体检测。此时不应用 auto-sleep 与耦合（AC/DC）。

- 若配置 0 秒的静止时间，驱动会选择一个启发式确定的默认周期（大于 1 秒）以优化功耗。这同样使用静止寄存器。

注意：根据数据手册，用于检测活动、静止（或在使用自由落体寄存器时）的最佳 ODR 应落在 12.5 Hz 到 400 Hz 之间。推荐的自由落体阈值为 300 mg 到 600 mg（寄存器值 0x05 到 0x09）。

在直流（DC）耦合模式下，当前加速度幅值直接与 THRESH_ACT 与 THRESH_INACT 寄存器中的值比较，以判定活动或静止。相比之下，交流（AC）耦合的活动检测以检测开始时的加速度值作为参考点，后续采样与该参考进行比较。直流耦合是默认模式——将实时值与固定阈值比较；而交流耦合则依赖相对于所配置阈值的内部滤波器。
...
要配置 **double tap**（双击）检测，还必须设置窗口与延迟参数，单位为微秒（µs）。延迟期从单击信号低于阈值时开始，作为一段等待时间，在此期间双击检测会忽略任何尖峰。延迟期结束后，检测窗口开始。若加速度在该窗口内先升过阈值、再降回阈值以下，则在降至阈值以下时触发双击事件。

双击事件检测在数据手册中有详尽说明。在检测到单击事件后，若信号满足特定条件，可能会跟随一个双击事件。不过，双击检测可能因以下三个原因而失效：

- 若设置了 **suppress bit**，则在点击延迟期内任何超过点击阈值的加速度尖峰都会立即使双击检测失效。换言之，当 suppress 位激活时，延迟期内不允许出现任何尖峰。

- 若双击窗口开始时加速度高于阈值，则双击事件无效。

- 若加速度持续时间超过 duration 寄存器设定的上限，双击检测同样会失效。

对于双击检测，适用的持续时间与单击相同：加速度必须先升过阈值、再在指定持续时间内降回阈值以下。注意，当双击检测处于活动状态时，通常会启用 suppress 位。
...
本驱动支持 IIO 缓冲区。所有设备都支持通过缓冲区获取原始加速度与温度测量值。
...
有关缓冲数据的结构，请参阅 Documentation/iio/iio_devbuf.rst。
