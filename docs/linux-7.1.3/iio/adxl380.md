...
本文件描Linux IIO 子系统中 ADXL380/ADXL382 三轴加速度计的驱动支持，涵盖其基于 SPI/I2C 的总线连接、测量量程以及对应的 sysfs 设备属性，主要面向驱动使用者与硬件适配者


本驱动支Analog Device ADXL380/382，运行在 SPI/I2C 总线上
## 1. 受支持的设备
...
ADXL380/ADXL382 是一款低噪声密度、低功耗、具有可选测量量程的 3 轴加速度计ADXL380 支持 ±4 g、 g ±16 g 量程，ADXL382 支持 ±15 g、0 g ±60 g
量程
## 2. 设备属
...
温度稳定性
每个 IIO 设备，在 `/sys/bus/iio/devices/iio:deviceX` 下都有一个设备文件夹其中 X 是该设备IIO 索引号。根据这些相关硬件设备的特性和功能，在这些文件下会存在一组设备文件。这些文件是统一的，并在 IIO ABI 文档中有说明
...
可以从通道_raw 属性读取通道值。返回的值是设备报告的原始值。要获得通道处理值（processed value），应用以下公式
...
关于缓冲数据的结构，更多信息请参Documentation/iio/iio_devbuf.rst
## 4. IIO 接口工具
