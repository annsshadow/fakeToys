...
本文件介绍 Linux IIO 子系统中 ADXL313 三轴加速度计的驱动实现，说明其 SPI/I2C 连接、测量范围以及对应的 sysfs 设备文件与活动/非活动事件配置，供驱动使用者参考。



本驱动支持 Analog Device 的 ADXL313，通过 SPI/I2C 总线连接。
...
ADXL313 是一款低噪声密度、低功耗的 3 轴加速度计，具有可选的测量范围。ADXL313 支持 ±0.5 g、±1 g、±2 g 与 ±4 g 范围。
...
每个 IIO 设备，在 `/sys/bus/iio/devices/iio:deviceX` 下都有一个设备文件夹，其中 X 是该设备的 IIO 索引。根据所讨论硬件设备的特性与功能，这些文件夹下存放着一组设备文件。这些文件被一致地泛化，并记录在 IIO ABI 文档中。

下表显示了与 adxl313 相关的设备文件，它们位于特定设备文件夹路径 `/sys/bus/iio/devices/iio:deviceX` 下。
...
与 iio 事件相关的设置，位于 `/sys/bus/iio/devices/iio:deviceX/events` 下。
...
+---------------------------------------------------+----------------------------------------------------------+
| in_accel_x\&y\&z_mag_adaptive_falling_en          | 启用或禁用 AC 耦合的非活动事件。                       |
+---------------------------------------------------+----------------------------------------------------------+
| in_accel_x\|y\|z_mag_adaptive_rising_en           | 启用或禁用 AC 耦合的活动事件。                         |
+---------------------------------------------------+----------------------------------------------------------+
...
默认耦合为 DC 耦合事件。在这种情况下阈值将保持原样，而对于 AC 耦合的情况，传感器会应用一个自适应阈值（在 datasheet 中描述）。通常活动，即 `ACTIVITY` 或 `ACTIVITY_AC`，以及非活动，即 `INACTIVITY` 或 `INACTIVITY_AC`，在两者都启用时将与自动休眠关联。这意味着特别是 `ACTIVITY` 也可以与 `INACTIVITY_AC` 关联，反之亦然，没有问题。

注意，这里 `ACTIVITY` 与 `ACTIVITY_AC` 是互斥的。这意味着，最近的一次配置将被设置。例如，如果 `ACTIVITY` 已启用，而 `ACTIVITY_AC` 将被启用，那么传感器驱动将禁用 `ACTIVITY`，但启用 `ACTIVITY_AC`。对于非活动同样成立。在关闭一个事件时，它必须与实际启用的相匹配，即启用 `ACTIVITY_AC` 然后禁用 `ACTIVITY` 会被简单地忽略，因为它已经处于禁用状态。或者，就像对待任何其他未启用的事件一样。

### 通道处理后的值


可以从通道的 _raw 属性读取一个通道值。返回的值是设备所报告的原始值。要获得通道的处理后值，应用以下公式：


        processed value = (_raw + _offset) * _scale

其中 _offset 与 _scale 是设备属性。如果不存在 _offset 属性，则简单地假定其值为 0。
...
设置加速度计通道的校准偏移。注意，校准将根据 LSB 单位的刻度进行四舍五入：
...
注意，当启用 AC 耦合时，事件类型将为 `mag_adaptive`。AC 耦合或 DC 耦合（默认）事件的使用方式类似。

## 4. IIO 接口工具


有关可用 IIO 接口工具的描述，请参阅 Documentation/iio/iio_tools.rst。
