...
数据手册中的"三线"模式最接近标准 SPI 连接，对于这些器件而言，就是将控制器的 CS 信号线连接到器件的 CNV 引脚，其余 SPI 信号线按常规方式连接。该配置在数据手册中被（容易令人误解地）称为"CS 模式，三线 Turbo 模式"连接。
注意：AD4000 系列数据手册中"三线"模式的定义与标准 spi-3wire 模式并不相同。
这是唯一允许访问配置寄存器的连接模式，但要求 SPI 控制器支持 `SPI_MOSI_IDLE_HIGH` 特性。
...
另一种被支持为"三线"模式的接线配置将 SDI 引脚硬连线到数字输入/输出接口电源（VIO）。在此设置下，控制器无需支持 `SPI_MOSI_IDLE_HIGH`，但无法访问寄存器。此连接模式节省一根信号线，且可与任何 SPI 控制器配合工作。

将设备树属性 `adi,sdi-pin` 设置为 `"high"` 以选择此模式。
...
```
此外，也可以将 GPIO 连接到器件的 CNV 引脚。这类似于前一种接线配置，但省去了 CS 信号线的使用。
...
在数据手册的"四线"模式下，控制器的 CS 信号线连接到 ADC 的 SDI 引脚，并将一个 GPIO 连接到 ADC 的 CNV 引脚。此连接模式更适合多个 ADC 可共享同一 CNV 触发信号的场景。

将 `adi,sdi-pin` 设置为 `"cs"` 以选择此模式。
...
AD4000 系列器件提供的量程压缩（span compression）特性，可通过更改电压通道的 `_scale_available` 属性来启用或禁用。请注意，量程压缩配置需要写入 AD4000 配置寄存器，而这只有在 ADC 以三线 Turbo 模式接线、且 SPI 控制器支持 `SPI_MOSI_IDLE_HIGH` 时才可行。若不满足这些条件，则不会提供 `_scale_available` 属性。

除此之外，差分与伪差分电压通道所提供的 sysfs 接口略有不同。

### Pseudo-differential ADCs


伪差分 AD4000 系列器件的典型电压通道属性：
...
为达到最大采样率，本驱动可配合 SPI 卸载引擎（例如通常集成在 `AXI SPI Engine`_ 中的引擎）使用，以提供 SPI 卸载支持。


为匹配 SPI 卸载的传输速度，ADC 必须以三线 Turbo 模式或三线无忙指示模式连接，并将 SPI 控制器的 CS 信号线连接到 CNV 引脚。

当配置为支持 SPI 卸载时，IIO 设备将提供不同的接口。

- 提供 `in_voltage0_sampling_frequency` 或 `in_voltage0-voltage1_sampling_frequency` 文件，用于设置采样率。
- 不提供 IIO 触发设备（无 `trigger` 目录）。
- 不提供 `timestamp` 通道。

此外，由于当器件以"三线"模式接线、且使用 SPI 卸载时每次采样仅进行一次传输，ADC 输出会存在一个采样周期的延迟，因此缓冲区中的第一个数据样本无效，因为它包含的是更早一次转换的结果。
