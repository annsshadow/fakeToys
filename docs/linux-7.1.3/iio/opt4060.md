
## OPT4060 驱动


## 1. 概述


本驱动通过 I2C 支持 Texas Instrument RGBW 高分辨率颜色传感器https://www.ti.com/lit/gpn/opt4060

驱动支持- 红、绿、蓝以及透明（clear）通道的原始值- 照度值- 红、绿、蓝的缩放颜色值- 用于阈值的 IIO 事件- 同时使用自身数据就绪触发以及其他驱动触发IIO 触发缓冲区
## 2. 照度计算


照度使用宽光谱绿色通道计算
lux = GREEN_RAW x 2.15e-3

该值可从以下位置读取：
/sys/bus/iio/devices/iio:deviceX/in_illuminance_input

更多细节请参阅数据手册第 8.4.5.2 节
## 3. 颜色缩放

传感器对不同颜色分量具有不同的灵敏度，驱动会暴露补偿因子
这些值可从以下位置读取：
/sys/bus/iio/devices/iio:deviceX/in_intensity_red_scale
/sys/bus/iio/devices/iio:deviceX/in_intensity_green_scale
/sys/bus/iio/devices/iio:deviceX/in_intensity_blue_scale

用户空间应用程序可将原始值与缩放值相乘，从而对于特定测试光源（通常为白色），各颜色通道的测量强度一致。计算方式如下：

R = RED_RAW x SCALE_RED(2.4)
G = GREEN_RAW x SCALE_GREEN(1.0)
B = BLUE_RAW x SCALE_BLUE(1.3)

数据手册建议使用缩放值来归一化缩放后R、G、B 值。这有助于获得与光强无关的颜色间比值。用户空间应用程序可按以下方式实现：

R_NORMALIZED = R / (R + G + B)
G_NORMALIZED = G / (R + G + B)
B_NORMALIZED = B / (R + G + B)

更多细节请参阅数据手册第 8.4.5.2 节