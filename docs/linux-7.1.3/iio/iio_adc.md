...
## 1. 概述（Overview
IIO 子系统支持许多模数转换器（Analog to Digital Converters，简ADC）。某ADC 具有一特性和特征，需要由 IIO 设备驱动以特定方式来支持。本文档描述了常见的 ADC 特性，并解释了 IIO
子系统是如何支持它们的
## 1. ADC 通道类型（ADC Channel Types
ADC 可以具有不同类型的输入，每种输入以略微不同的方式测量模拟电压。ADC 在通常给定电压参考输入类型和输入极性的一个范围内，对模拟输入电压进行数字化。确定一ADC 通道所允许的输入范是必要的，以便得出把测量值转换为现实单位（电压测量为毫伏、电流测量为毫安等）所需的缩放因和偏移量
复杂的设计可能具有非线性特征，或集成组件（如放大器和参考缓冲区），在推ADC 所允许的输范围时也需要考虑它们。为清晰起见，下面各小节假设输入范围仅取决于所提供的电压参考、输入类和输入极性
ADC 输入有三种通用类型（单端、差分、伪差分）和两种可能的极性（单极性、双极性）。输入类（单端、差分、伪差分）是一个通道特征，与极性（单极性、双极性）方面完全独立。一篇关ADC
...
```
**单端单极*通道的输入电压允许从 GND 摆动VREF（其VREF 是电位高于系统地（system ground的电压参考）。最大输入电压也称为 VFS（Voltage input Full-Scale，满量程输入电压），VREF
...
```
总是允许在设备树中纳ADC 通道节点。不过，如果设备具有一组一致的输入（例如全部为单端），
那么声明这些通道节点是可选的
对于支持单端与差分混合通道的设备，有一个注意事项：`reg` 是一个与输入引脚编号不匹配的任意
...
```
对于**单端双极*通道，模拟电压输入可以从 -VREF +VREF（其-VREF 是电位较低的电压参考，
+VREF 是电位较高的参考）。某ADC 芯片+VREF 推导出较低的参考，另一些则从单独的输入获取通常 +VREF -VREF 是对称的，但也不一定必须如此。当 -VREF 低于系统地（system ground）时这些输入也称为单端真双极性（single-ended true bipolar）。此外，尽管从电气角度看双极性与真双极有明显区别，IIO 并不对它们做显式区分```
...
```
**差分双极*输入的模拟信号同样允许在 -VREF +VREF 之间摆动。名称中双极部分意味着
差值（IN+ - IN-）的结果可以为正或为负。如-VREF 低于系统 GND，这些也称为差分真双极性输入```
...
```
**伪差分单极*输入具有差分单极性通道会有的那些限制，即正输入 IN+ 的模拟电压必须保持在 IN- +VREF 之间。提供给 IN- 的固定电压通常称为共模电压，并且它必须位于 -VREF +VREF 之间，正如对
任何差分通道负输入的信号所预期的那样
IN+ 测得的电压是相对IN- 的，但与差分通道不同，伪差分配置旨在测量单端输入信号。为了让应用
程序能够计算IN+ 相对于系统地（system ground）的电压，IIO 通道可以提供一`_offset` sysfs
属性，在把原始数据转换为电压单位时加到 ADC 输出上。在许多配置中，共模输入电压处于 GND 电平因此由于它恒为零，`_offset` 属性被省略```
...
```
**伪差分双极*输入不受 IN- 电平的限制，但会受到 -VREF GND 的限制（取决于输入端下限，取决于
具体 ADC）。与它们的单极性对应通道类似，伪差分双极性通道应当声明一`_offset` 属性，以便把原ADC 数据转换为电压单位。对于把 IN- 连接GND 的配置，`_offset` 通常被省略```
