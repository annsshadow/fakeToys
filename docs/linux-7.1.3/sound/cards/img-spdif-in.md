...
此控件返回正在接收的 SPDIF 流中所包含的状态位。'IEC958 Capture Mask' 显示可以从该控件读取哪些位。
...
当此控件的值被设为 {0,0,0,0} 时，提供给 hw_params 的速率将决定该模块捕获的单一速率。否则，提供给 hw_params 的速率将被忽略，该模块将尝试捕获此处设置的四种采样率中的每一个。
...
此控件用于修改该模块的锁定/抖动抑制特性。较大的值会扩大锁定范围，但降低抖动抑制能力。
