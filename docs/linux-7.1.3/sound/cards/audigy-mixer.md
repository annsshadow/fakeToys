...
这些控制DSP 指令构建而成，提供了扩展功能。本文仅描述 ALSA 驱动中默认内置的代码。请注意，这些控制用作衰减器（attenuator）：最大值即为不改变信号的中性位置。另请注意，如果多个控制引用了相同的目标（destination），信号会被累加，并可能被削波（clip，即在未做溢出检查的情况下被设为最大或最小值）
...
ADC
	模拟到数字转换器
I2S
	飞利浦半导体公司定义的单向三线串行总线，用于数字音频（该标准用于连接独立的 D/A A/D 转换器）
LFE
	低频效果（用作低音炮信号
AC97
...
该控制用于衰减来自左、右前置 PCM FX-bus 累加器的采样。在 5.1 声道回放中，ALSA 使用累加8 9 来处理左、右前置 PCM 采样。处理后的采样被送往前置扬声器

### name='PCM Surround Playback Volume',索引=0

该控制用于衰减来自左、右环绕（surround）PCM FX-bus 累加器的采样。在 5.1 声道回放中，ALSA 使用累加2 3 来处理左、右环绕 PCM 采样。处理后的采样被送往环绕（后置）扬声器

### name='PCM Side Playback Volume',索引=0

该控制用于衰减来自左、右侧面（side）PCM FX-bus 累加器的采样。在 7.1 声道回放中，ALSA 使用累加14 15 来处理左、右侧面 PCM 采样。处理后的采样被送往侧面扬声器

### name='PCM Center Playback Volume',索引=0

该控制用于衰减来自中央（center）PCM FX-bus 累加器的采样。在 5.1 声道回放中，ALSA 使用累加6 来处理中PCM 采样。处理后的采样被送往中置扬声器

### name='PCM LFE Playback Volume',索引=0

该控制用于衰LFE PCM FX-bus 累加器的采样。在 5.1 声道回放中，ALSA 使用累加7 来处LFE PCM 采样。处理后的采样被送往低音炮

### name='PCM Playback Volume',索引=0

该控制用于衰减来自左、右 PCM FX-bus 累加器的采样。在立体声回放中，ALSA 使用累加0 1 来处理左、右 PCM 采样。处理后的采样被送往前置扬声器

### name='PCM Capture Volume',索引=0

该控制用于衰减来自左、右 PCM FX-bus 累加器的采样。在立体声回放中，ALSA 使用累加0 1 来处理左、右 PCM 采样。处理后的结果被送往标准 capture PCM 设备

### name='Music Playback Volume',索引=0

该控制用于衰减来自左、右 MIDI FX-bus 累加器的采样。ALSA 使用累加4 5 来处理左、右 MIDI 采样。处理后的采样被送往虚拟立体声混音器

### name='Music Capture Volume',索引=0

这些控制用于衰减来自左、右 MIDI FX-bus 累加器的采样。ALSA 使用累加4 5 来处理左、右 MIDI 采样。处理后的结果被送往标准 capture PCM 设备

### name='Mic Playback Volume',索引=0

该控制用于衰减来AC97 编解码器中左、右 Mic 输入的采样。处理后的采样被送往虚拟立体声混音器

### name='Mic Capture Volume',索引=0

该控制用于衰减来AC97 编解码器中左、右 Mic 输入的采样。处理后的结果被送往标准 capture PCM 设备。原始采样同时被送往 Mic capture PCM 设备（设16 / 8 KHz 单声道），且不受音量控制

### name='Audigy CD Playback Volume',索引=0

该控制用于衰减来自左、右 IEC958 TTL 数字输入的采样（通常CDROM 驱动器提供）。处理后的采样被送往虚拟立体声混音器

### name='Audigy CD Capture Volume',索引=0

该控制用于衰减来自左、右 IEC958 TTL 数字输入的采样（通常CDROM 驱动器提供）。处理后的结果被送往标准 capture PCM 设备
...
该控制用于衰减来Philips ADC 的左、右 I2S ADC 输入的采样。处理后的采样被送往虚拟立体声混音器。其中包含来CD、Line In、Aux 等模拟音源的混音

### name='Analog Mix Capture Volume',索引=1

该控制用于衰减来Philips ADC 的左、右 I2S ADC 输入的采样。处理后的结果被送往标准 capture PCM 设备
...
若开启此开关，则用IEC958（S/PDIF）数字输出的采样仅取raw iec958 ALSA PCM 设备（默认情况下它使用累加器 20 21 处理左、右 PCM）
...
通道音量衰减，范0x1fffd。中间值（即无衰减）为默认值。三个值的通道映射如下
...
该控制指定目标——即 FX-bus 累加器。此映射中共24 个值：
...
请不要忘记：将同一通道多次分配到相同的 FX-bus 累加器是非法的（0=0 && 1=0 是一个无效组合）

### name='EMU10K1 PCM Send Volume',索引 0-31

它指定给定目标的衰减量（amount），范围 055。通道映射如下

- 0 -  mono，A 目标 attn，默255（无衰减
- 1 -  mono，B 目标 attn，默255（无衰减
- 2 -  mono，C 目标 attn，默0（静音）
- 3 -  mono，D 目标 attn，默0（静音）
- 4 -  mono，E 目标 attn，默0（静音）
- 5 -  mono，F 目标 attn，默0（静音）
- 6 -  mono，G 目标 attn，默0（静音）
- 7 -  mono，H 目标 attn，默0（静音）
- 8 -  left，A 目标 attn，默255（无衰减
- 9 -  left，B 目标 attn，默0（静音）
- 10 -  left，C 目标 attn，默0（静音）
- 11 -  left，D 目标 attn，默0（静音）
- 12 -  left，E 目标 attn，默0（静音）
- 13 -  left，F 目标 attn，默0（静音）
- 14 -  left，G 目标 attn，默0（静音）
- 15 -  left，H 目标 attn，默0（静音）
- 16 -  right，A 目标 attn，默0（静音）
- 17 -  right，B 目标 attn，默255（无衰减
- 18 -  right，C 目标 attn，默0（静音）
- 19 -  right，D 目标 attn，默0（静音）
- 20 -  right，E 目标 attn，默0（静音）
- 21 -  right，F 目标 attn，默0（静音）
- 22 -  right，G 目标 attn，默0（静音）
- 23 -  right，H 目标 attn，默0（静音）
