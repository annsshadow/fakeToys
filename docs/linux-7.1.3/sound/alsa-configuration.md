## ALSA（Advanced Linux Sound Architecture）驱动配置指南
...
要启用 ALSA 支持，你至少需要在内核中构建主声卡支持（`CONFIG_SOUND`）。由于 ALSA 可以模拟 OSS，因此你无需选择任何 OSS 模块。

如果你希望用 ALSA 运行 OSS 应用程序，请启用“OSS API 模拟”（`CONFIG_SND_OSSEMUL`）以及 OSS 混音器和 PCM 支持。

如果你想支持 SB Live! 等声卡上的波表（WaveTable）功能，则需要启用“音序器支持”（`CONFIG_SND_SEQUENCER`）。

若要让 ALSA 的调试信息更详细，请启用“Verbose printk”和“Debug”选项。若要检查内存泄漏，还要打开“Debug memory”。“Debug detection”会添加用于声卡检测的检查。

请注意，所有 ALSA ISA 驱动都支持 Linux 的 isapnp API（前提是声卡支持 ISA PnP）。你无需使用 isapnptools 来配置声卡。
...
cards_limit
    限制自动加载的声卡索引（1-8）；
    默认值：1；
    若要自动加载多块声卡，请将此选项与 snd-card-X 别名一起指定。
slots
    为给定驱动保留槽位索引；
    此选项接受多个字符串。
...
dsp_map
    分配给第 1 个 OSS 设备的 PCM 设备号；
    默认值：0
adsp_map
    分配给第 2 个 OSS 设备的 PCM 设备号；
    默认值：1
nonblock_open
    打开繁忙的 PCM 设备时不阻塞；
    默认值：1

例如，当 `dsp_map=2` 时，/dev/dsp 将被映射到
第 0 号声卡的第 2 个 PCM。类似地，当 `adsp_map=0` 时，
/dev/adsp 将被映射到第 0 号声卡的第 0 个 PCM。
若要修改第二块或更后面的声卡，可用逗号指定选项，
例如 `dsp_map=0,1`。

`nonblock_open` 选项用于改变 PCM 在打开设备时的行为。
当该选项非零时，打开一个繁忙的 OSS PCM 设备不会被阻塞，
而是立即以 EAGAIN 返回（就像 O_NONBLOCK 标志一样）。
...
midi_map
    分配给第 1 个 OSS 设备的 MIDI 设备号；
    默认值：0
amidi_map
    分配给第 2 个 OSS 设备的 MIDI 设备号；
    默认值：1

### 模块 snd-soc-core


SoC 核心模块。它被所有 ALSA 声卡驱动所使用。
它接受以下具有全局影响的选项。

prealloc_buffer_size_kbytes
    以 kbytes 为单位指定预分配缓冲区大小（默认：512）。
...
id
    ...
    最长 15 个字符；
    默认值：声卡类型；
    在 /proc/asound/ 下会创建以此名称命名的目录，
    其中包含该声卡的相关信息；
    在识别声卡时可以用此 ID 代替索引号
enable
    启用声卡；
    默认值：对于 PCI 和 ISA PnP 声卡为启用

这些选项用于指定实例的顺序，或在多个设备绑定到同一驱动时
控制每个设备的启用与禁用。例如，许多机器有两块 HD-audio
控制器（一块用于 HDMI/DP 音频，另一块用于板载模拟音频）。
在大多数情况下，第二块是主要用途，用户希望将其分配为
最先出现的声卡。可以通过指定 "index=1,0" 模块参数来实现，
这会交换分配槽位。

如今，在带有 PulseAudio 和 PipeWire 等支持动态配置的声音
后端的情况下，这种用法已没什么价值，但在过去它对静态配置
很有帮助。
...
对于实际的 AdLib FM 声卡，端口为 0x388。
注意该声卡没有 PCM 支持和混音器，仅有 FM 合成。

请确保你已准备好 alsa-tools 软件包中的 `sbiload`，并在
加载模块后通过 `sbiload -l` 查明所分配的 ALSA 音序器端口号。
...
```
加载同样由 `sbiload` 提供的 `std.sb` 和 `drums.sb` 音色：
```

      sbiload -p 64:0 std.sb drums.sb

```
如果你使用该驱动来驱动 OPL3，则可以改用 `std.o3` 和 `drums.o3`。
若要让声卡发出声音，可使用 alsa-utils 中的 `aplaymidi`：
```
...
用于 Avance Logic ALS300 和 ALS300+ 的模块。
...
ac97_quirk
    针对异常硬件的 AC'97 规避方案；
    详见下面的 `AC97 Quirk Option`_ 小节。
ac97_codec
    用于指定使用某个 AC'97 编解码器而非探测的规避方案。
    如果这对你有效，请附上你的 `lspci -vn` 输出提交一个 bug。
    （-2 = 强制探测，-1 = 默认行为，0-2 = 使用指定的编解码器。）
spdif_aclink
    通过 AC-link 传输 S/PDIF（默认 = 1）

该模块支持单块声卡和自动探测。

ATI IXP 有两种不同的方法来控制 SPDIF 输出。一种是通过
AC-link，另一种是通过“direct” SPDIF 输出。具体实现取决于
...
注意：该模块的默认 index 值为 -2，即第一个槽位被排除。
...
pcifix
    控制 PCI 规避方案；
    0 = 禁用所有规避方案，
    1 = 将 Aureal 声卡的 PCI 延迟强制设为 0xff，
    2 = 强制 Extend PCI#2 Internal Master，以在 VIA KT133 AGP
    桥上高效处理 Dummy Requests，
    3 = 强制上述两种设置，
    255 = 自动探测所需设置（默认）

该模块支持所有 ADB PCM 通道、ac97 混音器、SPDIF、硬件 EQ、
mpu401、gameport。A3D 和波表支持仍在开发中。
开发和逆向工程工作正在
https://savannah.nongnu.org/projects/openvortex/ 协调进行。
SPDIF 输出是 AC97 编解码器输出的副本，除非你使用
`spdif` pcm 设备，它允许原始数据透传。
硬件 EQ 和 SPDIF 仅存在于 Vortex2 和 Advantage 中。

注意：某些 ALSA 混音器应用程序不能正确处理 SPDIF 采样率控制。
如果你在这方面遇到问题，可以尝试另一个兼容 ALSA 的混音器
（alsamixer 可用）。
...
dma1
    WSS 播放的 DMA #（0,1,3）
dma2
    WSS 采集的 DMA #（0,1），-1 = 禁用（默认）
mpu_port
    MPU-401 UART 的端口号 #（0x300,0x330），-1 = 禁用（默认）
mpu_irq
    MPU-401 UART 的 IRQ #（3,5,7,9），-1 = 禁用（默认）
fm_port
    OPL3 的端口号 #（0x388），-1 = 禁用（默认）

该模块支持多块声卡。它不支持自动探测：`port`、`wss_port`、
`irq` 和 `dma1` 必须指定。其他值为可选。

`port` 需要匹配声卡上 BASE ADDRESS 跳线（0x220 或 0x240）
或声卡 EEPROM 中存储的值（适用于带 EEPROM 且将“CONFIG MODE”
跳线设为“EEPROM SETTING”的声卡）。其他值可以从上面列举的
选项中自由选择。

如果 `dma2` 被指定且与 `dma1` 不同，声卡将以全双工模式工作。
当 `dma1=3` 时，只有 `dma2=0` 有效，并且由于只有通道 0 和 1
可用于采集，这也是启用采集的唯一方式。

通用设置为 ``port=0x220 wss_port=0x530 irq=10 dma1=1 dma2=0
mpu_port=0x330 mpu_irq=9 fm_port=0x388``。

无论你选择哪个 IRQ 和 DMA 通道，请务必在 BIOS 中为传统 ISA
保留它们。
...
mpu_port
    MPU-401 UART 的端口号 #（0x300,0x330），-1 = 禁用（默认）
mpu_irq
    MPU-401 UART 的 IRQ #（5,7,9,10），-1 = 禁用（默认）
fm_port
    OPL3 的端口号 #（0x388），-1 = 禁用（默认）
...
fm_port
    ...
    0 = 禁用
soft_ac3
    软件转换原始 SPDIF 数据包（仅 model 033）（默认 = 1）
joystick_port
    游戏杆端口地址（0 = 禁用，1 = 自动探测）
...
fm_port
    CS4236 芯片的 FM 端口号 #（PnP 设置 - 0x388），-1 = 禁用
irq
    CS4236 芯片的 IRQ #（5,7,9,11,12,15）
mpu_irq
    MPU-401 UART 的 IRQ #（9,11,12,15）
dma1
    CS4236 芯片的第一个 DMA #（0,1,3）
dma2
    CS4236 芯片的第二个 DMA #（0,1,3），-1 = 禁用
...
此模块也被别名为 snd-cs4232，因为它同时提供了旧的
snd-cs4232 功能。
...
该模块支持多块声卡和自动探测。
通常外部放大器和 CLKRUN 控制会根据 PCI 子系统厂商/设备 ID 自动
...
pcm_devs
    分配给每块声卡的 PCM 设备数（默认 = 1，最多 4）
pcm_substreams
    分配给每个 PCM 的 PCM 子流数（默认 = 8，最多 128）
hrtimer
    使用 hrtimer（=1，默认）或系统定时器（=0）
fake_buffer
    虚假缓冲区分配（默认 = 1）

当创建多个 PCM 设备时，snd-dummy 对每个 PCM 设备给出不同的行为：
- 0 = 带 mmap 支持的交错模式
- 1 = 带 mmap 支持的非交错模式
- 2 = 不带 mmap 的交错模式
- 3 = 不带 mmap 的非交错模式
...
读/写或将单个虚拟页 mmap 到所有缓冲区页，以节省资源。
如果你的应用程序需要读/写缓冲区数据保持一致，请传入
fake_buffer=0 选项。
...
joystick_port
    游戏杆的端口号 #（0x200,0x208,0x210,0x218），0 = 禁用
    （默认），1 = 自动探测
...
fm_port
    OPL3 的端口号 #（可选；默认与 MPU 端口共用）

在 `isapnp=0` 时，可使用以下附加选项：
...
例程则不含 MPU-401 端口）。当 `dma2` 与 `dma1` 相等时，驱动以
半双工方式工作。
...
tea575x_tuner
    ...
    3 = MediaForte 64-PCR
    高 16 位为视频（收音机）设备号 + 1；
    例如：0x10002（MediaForte 256-PCPR，设备 1）
...
position_fix
    ...
    4 = COMBO：播放使用 LPIB，采集流自动
    5 = SKL+：应用近期 Intel 芯片上可用的延迟计算
    6 = FIFO：用固定的 FIFO 大小修正位置，用于近期 AMD 芯片
probe_mask
    用于探测编解码器的位掩码（默认 = -1，即所有槽位）；
...
patch
    指定在初始化编解码器之前用于修改 HD-audio 设置的早期
    “patch”文件。
    此选项仅在设置了 `CONFIG_SND_HDA_PATCH_LOADER=y` 时可用。
...
pm_blacklist
    启用 / 禁用电源管理拒绝列表（默认 = 查询 PM
    拒绝列表，0 = 跳过 PM 拒绝列表，1 = 强制关闭运行时 PM）
align_buffer_size
    强制将缓冲区/周期大小四舍五入到 128 字节的倍数。
    这在内存访问方面更高效，但 HDA 规范并不要求，
    且会阻止用户指定精确的周期/缓冲区大小。（默认 = 开启）
snoop
    启用/禁用窥探（默认 = 开启）

该模块支持多块声卡和自动探测。

有关 HD-audio 驱动的更多细节，请参见 hd-audio/notes.rst。

每个编解码器都可能有针对不同配置的型号表。
如果你的机器未列在其中，则会设置默认（通常是最精简的）
配置。在这种情况下你可以传入 `model=<name>` 选项来指定
某个型号。根据编解码器芯片的不同有不同型号。可用型号列表
见 hd-audio/models.rst。

型号名 `generic` 被视为一种特殊情况。当给定此型号时，驱动
使用通用的编解码器解析器，而不使用“codec-patch”。这有时
对测试和调试很有用。

model 选项也可用于别名到另一个 PCI 或编解码器 SSID。
当以 `model=XXXX:YYYY` 的形式传入时，其中 XXXX 和 YYYY
分别是十六进制的子系统厂商和子系统设备 ID，驱动将把该
SSID 作为异常表的参考。

如果默认配置不工作，而上述某一项与你的设备匹配，请将它
连同 alsa-info.sh 的输出（使用 `--no-upload` 选项）一起
报告给 kernel bugzilla 或 alsa-devel 邮件列表
（见 `Links and Addresses`_ 小节）。

`power_save` 和 `power_save_controller` 选项用于省电模式。
详见 powersave.rst。

注意 2：如果你的输出有咔嗒声，请尝试模块选项
`position_fix=1` 或 `2`。`position_fix=1` 将使用未做 FIFO
大小修正的 SD_LPIB 寄存器值作为当前 DMA 指针。`position_fix=2`
将使驱动使用位置缓冲区而不是读取 SD_LPIB 寄存器。
（通常 SD_LPIB 寄存器比位置缓冲区更精确。）

`position_fix=3` 专用于 VIA 设备。采集流的位置从 LPIB 和
POSBUF 两个值中检查。`position_fix=4` 是组合模式，播放使用
LPIB，采集使用 POSBUF。

注意：如果在加载时出现大量 `azx_get_response timeout` 消息，
很可能是中断的问题（例如 ACPI irq 路由）。尝试用
`pci=noacpi` 之类的选项启动。此外，你可以尝试 `single_cmd=1`
模块选项。这会将 HDA 控制器与编解码器之间的通信方式切换为
单一立即命令，而不是 CORB/RIRB。基本上，单一命令模式仅用于
BIOS，你也不会收到未 solicited 事件。但至少，它独立于 irq
工作。请记住这是最后手段，应尽可能避免……

关于 `azx_get_response timeout` 问题的更多说明：
在某些硬件上，你可能需要添加合适的 probe_mask 选项来避免
上述 `azx_get_response timeout` 问题。当访问不存在或不工作
的编解码器槽位（很可能是调制器槽位）导致 HD-audio 总线上的
通信停滞时就会发生这种情况。你可以通过启用 `CONFIG_SND_DEBUG_VERBOSE`
来查看探测了哪些编解码器槽位，或者直接从编解码器 proc 文件的
...
注意：当设置了 `CONFIG_FW_LOADER` 时，固件数据可以通过 hotplug
自动加载。否则，你需要通过 alsa-tools 软件包中包含的 hdsploader
工具加载固件。固件数据位于 alsa-firmware 软件包中。

注意：snd-page-alloc 模块承担了以前 snd-hammerfall-mem 模块的
工作。它会在发现任何 HDSP 声卡时预先分配缓冲区。为了确保
缓冲区分配成功，请在启动序列的早期阶段加载 snd-page-alloc
模块。详见 `Early Buffer Allocation`_ 小节。
...
cs8427_timeout
    CS8427 芯片（S/PDIF 收发器）的复位超时，以 msec
    为单位，默认值为 500（0.5 秒）

该模块支持多块声卡和自动探测。
注意：消费部分并非在所有基于 Envy24 的声卡上都使用
（例如在 MidiMan Delta 系列中）。

注意：支持的板卡通过读取 EEPROM 或 PCI SSID（若 EEPROM
不可用）来检测。如果驱动配置不正确，或你想尝试另一种
...
buggy_semaphore
    ...
    ASUS 笔记本）（默认关闭）
spdif_aclink
    使用通过 AC-link 的 S/PDIF，而不是来自控制器芯片的
    直接连接（0 = 关闭，1 = 开启，-1 = 默认）
...
本驱动不支持游戏杆/MIDI 端口。如果你的主板有这些设备，请
分别使用 ns558 或 snd-mpu401 模块。
...
pcm_voices
    为合成器保留的 PCM 语音数（默认 2）
effect
    1 = 启用 InterWave 效果（默认 0）；需要 8 个语音
isapnp
    ISA PnP 检测 - 0 = 禁用，1 = 启用（默认）
...
irq
    InterWave 芯片的 IRQ #（3,5,9,11,12,15）
dma1
    InterWave 芯片的 DMA #（0,1,3,5,6,7）
dma2
    InterWave 芯片的 DMA #（0,1,3,5,6,7,-1=禁用）
...
用于 UltraSound 32-Pro（Compaq 使用的 STB 声卡）以及基于 AMD
...
注意：放大器的绑定取决于硬件。如果所有通道都已解除静音却
仍然没有声音，请尝试通过 amp_gpio 选项指定其他 gpio 连接。
例如，某些松下笔记本可能需要 `amp_gpio=0x0d` 选项。
...
该模块支持多块声卡。
注意：一块 miXart8 板卡会被表示为 4 个 alsa 声卡。
详见 Documentation/sound/cards/mixart.rst。

当驱动编译为模块且支持 hotplug 固件时，固件数据会通过
hotplug 自动加载。请在 alsa-firmware 软件包中安装所需的
固件文件。当没有可用的 hotplug 固件加载器时，你需要通过
alsa-tools 软件包中的 mixartloader 工具加载固件。
...
该驱动需要固件文件 `turtlebeach/pndspini.bin` 和
`turtlebeach/pndsperm.bin` 位于正确的固件目录中。

### 模块 snd-mtpav


用于 MOTU MidiTimePiece AV 多端口 MIDI（并口）的模块。

port
    MTPAV 的 I/O 端口号 #（0x378,0x278，默认=0x378）
irq
    MTPAV 的 IRQ #（7,5，默认=7）
hwports
    受支持的硬件端口数，默认=8。

模块仅支持 1 块声卡。该模块没有 enable 选项。
...
注意：NM256 芯片可以在内部与非 AC97 编解码器连接。本驱动
仅支持 AC97 编解码器，无法与带有其他（很可能是 CS423x 或
OPL3SAx）芯片的机器工作，即使该设备在 lspci 中能被探测到。
在这种情况下，请尝试其他驱动，例如 snd-cs4232 或 snd-opl3sa2。
其中一些支持 ISA-PnP，一些不支持。在没有 ISA PnP 的情况下，
你需要指定 `isapnp=0` 以及正确的硬件参数。

注意：某些笔记本需要针对 AC97 RESET 的规避方案。对于已知的
硬件如 Dell Latitude LS 和 Sony PCG-F305，此规避方案会自动
...
注意：这个驱动真的很糟糕。它移植自 OSS 驱动，而后者是黑魔法
般逆向工程的产物。如果驱动在 X-server 之后加载（如上所述），
编解码器的探测会失败。你可能能够强制加载该模块，但可能导致
挂起。因此，如果遇到这类问题，请确保在 X 之前加载此模块。
...
irq
    OPL3-SA 芯片的 IRQ #（5,7,9,10）
dma1
    Yamaha OPL3-SA 芯片的第一个 DMA #（0,1,3）
dma2
    Yamaha OPL3-SA 芯片的第二个 DMA #（0,1,3），-1 = 禁用
...
用于基于 OPTi 82c92x 和 Analog Devices AD1848 芯片的声卡的模块。
该模块也适用于 OAK Mozart 声卡。
...
fm_port
    OPL3 设备的端口号 #（0x388）
irq
    WSS 芯片的 IRQ #（5,7,9,10,11）
mpu_irq
    MPU-401 UART 的 IRQ #（5,7,9,10）
dma1
    WSS 芯片的第一个 DMA #（0,1,3）
...
用于 PowerMac、iMac 和 iBook 板载声芯片的模块。
...
用于 Intel PXA2xx 芯片的 AC97 驱动的模块。
...
该模块支持多块声卡。
该驱动需要内核提供固件加载器支持。
你需要将固件文件 `riptide.hex` 安装到标准固件路径
（例如 /lib/firmware）。

### 模块 snd-rme32


用于 RME Digi32、Digi32 Pro 和 Digi32/8（Sek'd Prodif32、
...
注意：snd-page-alloc 模块承担了以前 snd-hammerfall-mem 模块
的工作。它会在发现任何 RME9652 声卡时预先分配缓冲区。为了确保
缓冲区分配成功，请在启动序列的早期阶段加载 snd-page-alloc
模块。详见 `Early Buffer Allocation`_ 小节。
...
用于 Compaq iPAQ H3600 声卡上 Philips UDA1341TS 的模块。

模块仅支持一块声卡。
模块没有 enable 和 index 选项。
...
用于 8 位 SoundBlaster 声卡：SoundBlaster 1.0、SoundBlaster 2.0、
...
用于 16 位 SoundBlaster 声卡：SoundBlaster 16（PnP）、
...
mpu_port
    MPU-401 UART 的端口号 #（0x300,0x330），-1 = 禁用
awe_port
    EMU8000 合成器的基端口号 #（0x620,0x640,0x660）（仅 snd-sbawe
    模块）
irq
...
注意：若要在 16 位半双工模式下使用 Vibra16X 声卡，必须
通过 dma16 = -1 模块参数禁用 16 位 DMA。此外，所有 Sound Blaster 16
类型声卡都可以通过禁用其 16 位 DMA 通道，改用 8 位 DMA 通道
以 16 位半双工模式工作。
...
用于 Gallant SC-6000 声卡及后续型号：SC-6600 和 SC-7000 的模块。
...
该声卡也称为 Audio Excel DSP 16 或 Zoltrix AV302。
...
wait_usecs
    ...
    如果你想尝试并找到更好的值，请随意。记住，要点是得到一个
    让我们能尽可能多地忙等待 WaveFront 命令的数字，而不会大到
    霸占整个 CPU。
    具体来说，使用这个数字，在约 134,000 次状态等待中，只有
    约 250 次导致睡眠。
sleep_interval
    等待回复时睡眠多长时间（默认：100）
sleep_tries
...
ospath
    ...
    ISC2115 OS 固件的路径名。在最近的版本中，它通过固件加载器
    框架处理，因此必须安装在正确的路径中，通常是 /lib/firmware。
reset_time
    等待复位生效多长时间（默认：2）
ramcheck_time
    等待 RAM 测试多少秒（默认：20）
...
注意：早期的版本中固件文件 `wavefront.os` 位于 /etc。现在它通过
固件加载器加载，必须位于正确的固件路径中，例如 /lib/firmware。
如果在升级内核后遇到有关固件下载的错误，请适当地复制（或建立
符号链接）该文件。
...
speed
    速度，单位为波特（9600,19200,38400,57600,115200）
    38400 = 默认
base
    波特率除数基准（57600,115200,230400,460800）
    115200 = 默认
outs
    一个串行端口中的 MIDI 端口数（1-4）
    1 = 默认
adaptor
    适配器类型。
...
ignore_ctl_error
    ...
    `ignore_ctl_error=1` 可能在你访问混音器元素（如 URB error -22）
    时遇到错误有所帮助。这发生在某些有缺陷的 USB 设备或控制器上。
...
quirk_alias
    异常别名列表，传入类似 `0123abcd:5678beef` 的字符串，将设备
...
use_vmalloc
    ...
    对于像 ARM 或 MIPS 这样具有非一致性内存的架构，mmap 访问使用
    vmalloc 分配的缓冲区可能产生不一致的结果。如果在此类架构上使用
    mmap，请关闭此选项，这样会分配并使用 DMA 一致性缓冲区。
delayed_register
    该选项用于具有在多个 USB 接口中定义的多个流的设备。驱动可能
    会多次（每个接口一次）进行注册，这可能导致设备枚举不完整。
    该选项接收一个字符串数组，你可以传入类似 `0123abcd:4` 的
    ID:INTERFACE 来执行对该给定设备的延迟注册。在此例中，当探测到
    USB 设备 0123:abcd 时，驱动会等待 USB 接口 4 被探测后才注册。
    对此类设备，驱动会打印类似 “Found post-registration device
    assignment: 1234abcd:04” 的消息，以便用户注意到这一需要。
skip_validation
    跳过单元描述符校验（默认：否）。
    该选项用于忽略单元描述符的校验错误（以单元描述符的十六进制转储
    形式），而不是产生驱动探测错误，以便我们检查其细节。
quirk_flags
    该选项提供了用于应用异常标志的精细且灵活的控制。它允许为每个
    设备指定异常标志，并且可以通过 sysfs 动态修改。
    旧的用法接受一个整数数组，其中每个整数按照探测顺序对设备应用
    异常标志。例如，`quirk_flags=0x01,0x02` 对第一个设备应用
    get_sample_rate，对第二个设备应用 share_media_device。
    新的用法接受格式为 `VID1:PID1:FLAGS1;VID2:PID2:FLAGS2;...` 的
    字符串，其中 `VIDx` 和 `PIDx` 指定设备，`FLAGSx` 指定要应用的
    标志。`VIDx` 和 `PIDx` 是 4 位十六进制数，可以指定为 `*` 以匹配
    任意值。`FLAGSx` 可以是一组以 `|` 分隔的、按名称给出的标志，或
    表示位标志的十六进制数。可用的标志名称如下。可以在标志名前加
    叹号以对该标志取反。
    例如，`1234:abcd:mixer_playback_min_mute|!ignore_ctl_error;**:**:0x01;`
    对设备 1234:abcd 应用 `mixer_playback_min_mute` 标志并清除
    `ignore_ctl_error` 标志，并对所有设备应用 `skip_sample_rate` 标志。

        - 第 0 位：`get_sample_rate`
          跳过读取设备的采样率
        - 第 1 位：`share_media_device`
          创建 Media Controller API 条目
        - 第 2 位：`align_transfer`
          允许在传输时对音频子时隙进行对齐
        - 第 3 位：`tx_length`
          在传输中添加长度说明符
        - 第 4 位：`playback_first`
          在隐式反馈模式下首先启动播放流
        - 第 5 位：`skip_clock_selector`
          跳过时钟选择器设置
        - 第 6 位：`ignore_clock_source`
          忽略时钟源搜索的错误
        - 第 7 位：`itf_usb_dsd_dac`
          表示基于 ITF-USB DSD 的 DAC
        - 第 8 位：`ctl_msg_delay`
          在每个控制消息处理时添加 20ms 延迟
        - 第 9 位：`ctl_msg_delay_1m`
          在每个控制消息处理时添加 1-2ms 延迟
        - 第 10 位：`ctl_msg_delay_5m`
          在每个控制消息处理时添加 5-6ms 延迟
        - 第 11 位：`iface_delay`
          在每个接口设置时添加 50ms 延迟
        - 第 12 位：`validate_rates`
          在探测时执行采样率校验
        - 第 13 位：`disable_autosuspend`
          禁用运行时 PM 自动挂起
        - 第 14 位：`ignore_ctl_error`
          忽略混音器访问的错误
        - 第 15 位：`dsd_raw`
          支持通用的 DSD 原始 U32_BE 格式
        - 第 16 位：`set_iface_first`
          像 UAC1 一样首先设置接口
        - 第 17 位：`generic_implicit_fb`
          应用通用的隐式反馈同步模式
        - 第 18 位：`skip_implicit_fb`
          不应用隐式反馈同步模式
        - 第 19 位：`iface_skip_close`
          在设置采样率期间不关闭接口
        - 第 20 位：`force_iface_reset`
          在每次停止和重启流时强制复位接口
        - 第 21 位：`fixed_rate`
          当给定端点只有一个可用速率时，不设置 PCM 速率（频率）
        - 第 22 位：`mic_res_16`
          为 Mic Capture Volume 设置固定分辨率 16
        - 第 23 位：`mic_res_384`
          为 Mic Capture Volume 设置固定分辨率 384
        - 第 24 位：`mixer_playback_min_mute`
          将最小音量控制值设为静音，适用于最低播放值表示静音状态
          而非最小可听音量的设备
        - 第 25 位：`mixer_capture_min_mute`
          类似于第 24 位，但用于采集流
        - 第 26 位：`skip_iface_setup`
          跳过探测时的接口设置（usb_set_interface、init_pitch、
          init_sample_rate）；与流打开时的 snd_usb_endpoint_prepare()
          重复
        - 第 27 位：`mixer_playback_linear_vol`
          为播放音量控制值线性映射到电压（而非 dB）水平的设备设置
          线性音量映射。简而言之：`x(raw) = (raw - raw_min) / (raw_max - raw_min)`；
          `V(x) = k ** x`；`dB(x) = 20 ** log10(x)`。覆盖第 24 位
        - 第 28 位：`mixer_capture_linear_vol`
          类似于第 27 位，但用于采集流。覆盖第 25 位

该模块支持多设备、自动探测和热插拔。

注意：`nrpacks` 参数可以通过 sysfs 动态修改。不要将该值设得超过 20。
通过 sysfs 修改不进行健全性检查。

注意：`ignore_ctl_error=1` 只是提供了一种快速绕过问题的方法。如果你
有需要这些异常的有缺陷设备，请向上游报告。

注意：`quirk_alias` 选项仅用于测试/开发。如果你希望获得适当的支持，
请联系上游，在驱动代码中静态添加匹配的异常。`quirk_flags` 同理。如果
...
注意：你需要通过 alsa-tools 和 alsa-firmware 软件包中包含的
`usx2yloader` 工具加载固件。

### 模块 snd-via82xx


用于基于 VIA 82C686A/686B、8233、8233A、8233C、8235、8237
（南桥）的 AC'97 主板的模块。
...
mpu_port
    ...
    [仅 VIA686A/686B]
joystick
    启用游戏杆（默认关闭）[仅 VIA686A/686B]
ac97_clock
    AC'97 编解码器时钟基准（默认 48000Hz）
dxs_support
    支持 DXS 通道，0 = 自动（默认），1 = 启用，2 = 禁用，
    3 = 仅 48k，4 = 无 VRA，5 = 启用任意采样率且不同通道使用
    不同的采样率 [仅 VIA8233/C、8235、8237]
ac97_quirk
    针对异常硬件的 AC'97 规避方案；
    见下面的 `AC97 Quirk Option`_ 小节。
...
在这种情况下，请尝试将 BIOS 上的 SMP（或 MPS）版本设为 1.1 而不是
默认值 1.4。这样中断号将被分配在 15 以下。你也可以升级你的 BIOS。

注意：VIA8233/5/7（非 VIA8233A）可以将 DXS（direct sound）通道支持为
第一个 PCM。在这些通道上，最多可同时播放 4 个流，且控制器可以对每个
通道以独立的速率执行采样率转换。
默认情况下（`dxs_support = 0`），除已知设备外，选择固定的 48k 速率，
因为在某些主板上，由于 BIOS 缺陷，除 48k 外输出往往有杂音。
请先尝试一次 `dxs_support=5`，如果它在其他采样率（例如 mp3 播放的
44.1kHz）下工作，请将 PCI 子系统厂商/设备 ID（即 `lspci -nv` 的输出）
告诉我们。
如果 `dxs_support=5` 不工作，尝试 `dxs_support=4`；如果还不工作，尝试
dxs_support=1。（dxs_support=1 通常用于旧主板。正确实现的板卡应该
能在 4 或 5 下工作。）如果仍然不工作，而默认设置可以，则 `dxs_support=3`
是正确选择。如果默认设置根本不工作，尝试 `dxs_support=2` 来禁用 DXS 通道。
在任何情况下，请将结果和子系统厂商/设备 ID 告诉我们。见下面的
`Links and Addresses`_。

注意：对于 VIA823x 上的 MPU401，请另外使用 snd-mpu401 驱动。mpu_port
选项仅用于 VIA686 芯片。
...
用于虚拟 rawmidi 设备的模块。
该模块创建与相应 ALSA 音序器端口通信的虚拟 rawmidi 设备。
...
用于 Digigram VX-Pocket VX222、V222 v2 和 Mic 声卡的模块。
...
当驱动编译为模块且支持 hotplug 固件时，固件数据会通过 hotplug 自动
加载。请在 alsa-firmware 软件包中安装所需的固件文件。当没有可用的
hotplug 固件加载器时，你需要通过 alsa-tools 软件包中的 vxloader 工具
...
```
（对于 2.2/2.4 内核，改为将 `post-install /usr/bin/vxloader` 添加到
/etc/modules.conf。）
IBL 大小定义了 PCM 的中断周期。更小的大小带来更低的延迟，但也会导致
更多的 CPU 消耗。该大小通常对齐到 126。默认（=0）时选择最小的大小。
可能的 IBL 值可以在 /proc/asound/cardX/vx-status proc 文件中找到。
...
用于 Digigram VX-Pocket VX2 和 440 PCMCIA 声卡的模块。

ibl
    采集 IBL 大小。（默认 = 0，最小大小）

该模块支持多块声卡。该模块仅在设置了 PCMCIA 支持的内核中才被编译。

在较旧的 2.6.x 内核上，要通过卡管理器激活驱动，你需要设置
/etc/pcmcia/vxpocket.conf。参见 sound/pcmcia/vx/vxpocket.c。2.6.13 或
更新的内核不再需要配置文件。
...
mpu_port
    0x300,0x330,0x332,0x334，默认 0（禁用），
    1（仅 YMF744/754 自动探测）
fm_port
    0x388,0x398,0x3a0,0x3a8，默认 0（禁用）
    1（仅 YMF744/754 自动探测）
joystick_port
    0x201,0x202,0x204,0x205，默认 0（禁用），
    1（自动探测）
rear_switch
    启用共享的后置/线路输入开关（bool）
...
ac97_quirk 选项用于为板载 AC'97 控制器（如 snd-intel8x0）驱动上的
特定设备启用/覆盖规避方案。某些硬件把 Master 和 Headphone 或 Surround
...
none
    禁用异常规避
hp_only
    将 Master 和 Headphone 控制绑定为单一控制
swap_hp
    交换耳机和主控制
swap_surround
    交换主和环绕控制
ad_sharing
    对于 AD1985，开启 OMS 位并使用耳机
alc_jack
    对于 ALC65x，开启插孔检测模式
inv_eapd
    反转的 EAPD 实现
mute_led
    绑定 EAPD 位以开启/关闭静音 LED

为了向后兼容，相应的整数值 -1、0 等也被接受。

例如，如果 `Master` 音量控制对你的设备无效，而只有 `Headphone` 有效，
请传入 ac97_quirk=hp_only 模块选项。


## 配置非 ISAPNP 声卡


当内核配置了 ISA-PnP 支持时，支持 isapnp 声卡的模块会有 `isapnp`
模块选项。如果设置了此选项，将**只**探测 ISA-PnP 设备。要探测非
ISA-PnP 声卡，你必须传入 `isapnp=0` 选项以及正确的 I/O 和 irq 配置。
...
ALSA 驱动可以通过定义模块别名按需自动加载。对于 ALSA 原生设备，会请求
字符串 `snd-card-%1`，其中 `%i` 是从 0 到 7 的声卡号。

要为 OSS 服务自动加载 ALSA 驱动，请定义字符串 `sound-slot-%i`，其中
...
```
可自动加载的声卡数量取决于 snd 模块的 `cards_limit` 模块选项。默认
设为 1。要启用多块声卡的自动加载，请在该选项中指定声卡数量。

当有多块声卡可用时，最好也通过模块选项为每块声卡指定 index 号，以便
声卡的顺序保持一致。
...
另一种（较新的）固定槽位分配的方法是使用 snd 模块的 `slots` 选项。
对于上面的例子，按如下方式指定：
```
...
```
这样，第一个槽位（#0）保留给 snd-interwave 驱动，第二个（#1）保留给
snd-ens1371。如果使用 slots 选项，可以省略每个驱动中的 index 选项
（不过只要不冲突，也可以同时保留）。

slots 选项对于避免可能的热插拔及其导致的槽位冲突特别有用。例如，再次
考虑上面的例子，前两个槽位已被保留。如果有任何其他驱动（例如
snd-usb-audio）在 snd-interwave 或 snd-ens1371 之前加载，它将被分配到
第三个或更后的槽位。

当模块名以 '!' 给出时，该槽位将保留给除该名称外的任何模块。例如，
`slots=!snd-pcsp` 将把第一个槽位保留给除 snd-pcsp 外的任何模块。
...
表示设备号。ALSA 设备带有 `c` 或 `p` 后缀，分别表示方向：采集和播放。
...
<app_name> <fragments> <fragment_size> [<options>]
	...
	no-silence
	    不预先填充静音以避免咔嗒声
	buggy-ptr
	    在 GETOPTR ioctl 中返回空白块而不是已填充的块
...
某些驱动（例如 hdsp）需要大的连续缓冲区，而由于内存碎片，有时在
驱动模块实际加载时再寻找这样的空间已经太迟。你可以通过提前加载
snd-page-alloc 模块并向其 proc 文件写入命令来预分配 PCM 缓冲区，
例如在早期启动阶段（如 `/etc/init.d/*.local` 脚本）进行。
...
VENDOR 和 DEVICE 是 PCI 厂商和设备 ID。它们取整数（十六进制需要
0x 前缀）。MASK 是 PCI DMA 掩码。如果不限制则传 0。SIZE 是要分配的
每个缓冲区的大小。你可以为 KB 和 MB 使用 k 和 m 后缀。最大数量为 16MB。
