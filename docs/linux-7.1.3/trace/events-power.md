...
'处理 事件 class gathers CPU-related 事件: cpuidle
cpufreq.
```
...
```
说明: '-1' '4294967295' 状means exit current 状
i.e. trace_cpu_idle(4, smp_processor_id()) means 系统
enters idle 状4, trace_cpu_idle(PWR_EVENT_EXIT, smp_processor_id())
means 系统 exits previous idle 状

事件 '状4294967295' trace very important 用户
space tools 使用 检end current 状
...
second 参数 previous QoS 
third 参数 current QoS 更新.
...
third 参数 added/updated/removed.
