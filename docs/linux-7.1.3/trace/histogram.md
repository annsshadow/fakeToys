...
## 1. 简
  ...
  有关跟踪事件与事件触发器的更多信息，请参Documentation/trace/events.rst
...
## 2. 直方图触发器命令
  ...
  该哈希表以一个（或多个）跟踪事件格式字段（或栈回溯）作为键，并以
  一个（或多个）跟踪事件格式字段或事件计数（hitcount）派生出的一
  累计总和作为值
```
...
- enable_hist/disable_hist
  ...
  enable_hist disable_hist 触发器可用于让某个事件有条件地启动和停止另一
  事件已附着hist 触发器。可以将任意数量enable_hist disable_hist 触发
...
### 2.2. 扩展错误信息
  ...
  在调hist 触发器命令时遇到某些错误条件，可通过 tracing/error_log 文件
...
### 2.3. ‘hist触发器示
  ...
  第一组示例使kmalloc 事件创建聚合。可用于 hist 触发器的字段列示
```
...
并利用这些数据创建直方图。事件间直方图的 data 反过来又可以作为进一步组合直方图
来源，从而形成一条相关的直方图链，这对某些应用很重要

可以用这种方式使用的事件间量中，最重要的例子是延迟（latency），它其实就是两
事件之间时间戳的差值。尽管延迟是最重要的事件间量，但请注意，由于该支持在整
...
通常，一hist 触发器规格说明包含一个（可能为复合的）键，以及一或多个数值，
这些数值是与该键关联、持续更新的总和。在这种情况下，直方图规格说明由单个键与
值的规格组成，它们引用与单一事件类型关联的跟踪事件字段

事件间直方图触发器扩展允许引用来自多个事件的字段，并将其组合为一个多事件直方
规格说明。为了支持这一总体目标，向 hist 触发器支持中新增了若干使能特性：
  ...
  - 由事件间量构成的直方图，在逻辑上并不是任一事件的直方图（因此让任一事件
    'hist' 文件来承载直方图输出并不合理）。为了体现该直方图与一组事件的组合相关
    这一概念，新增了支持以允许创建“合成（synthetic）”事件，即从其他事件派生出来
    事件。这些合成事件与任何其他事件一样是完备的事件，可以按此方式使用，例如用
    创建前面提到的“组合”直方图
  ...
  - 跟踪事件本身并不带有与之关联的“时间戳”，但在底层ftrace 环形缓冲区中
    与每个事件一起保存了一个隐式的时间戳。该时间戳现在以一个名'common_timestamp'
    的合成字段的形式暴露出来，可以像其他任何事件字段一样在直方图中使用；它并不
    跟踪格式中的真实字段，而是一个合成出来的值，尽管如此仍可以像真实字段一样使用
    默认情况下其单位为纳秒；common_timestamp 字段后附'.usecs' 可将单位改为微秒

关于事件间时间戳的注意事项：如果在直方图中使用了 common_timestamp，跟踪缓冲区
自动切换为使用绝对时间戳和“global”跟踪时钟，以避免与其他在跨 CPU 时不连续的时
之间出现虚假的时间戳差异。也可以通过改用其他跟踪时钟来覆盖这一行为，即使用
"clock=XXX" hist 触发器属性，其中 XXX tracing/trace_clock 伪文件中列出
任一时钟
...
唯一的例外是：任何在表达式中使用的变量本质上都是“读一次（read-once）”的——一
它被后续事件中的某个表达式使用，就会被重置为“未设置（unset）”状态，这意味着除非
再次设置，否则不能再次使用。这不仅确保事件不会在计算中使用未初始化的变量，也确
该变量只被使用一次，而不会用于任何不相关的后续匹配

保存变量的基本语法是：简单地把一个不对应任何关键字的唯一变量名，连同 '=' 号作
前缀，加在任意事件字段上

键或值都可以用这种方式保存和检索。这会为带有该键的直方图条目创建一个名'ts0'
的变
```
...
变量引用是通过在变量名前加 '$' 符号构成的。因此，例如上面ts0 变量在表达式
会被引用'$ts0'

由于使用'vals='，上面这common_timestamp 变量的值也会像普通直方图值一样被
求和（尽管对于时间戳而言这没什么意义）
```
...
```
如果一个变量不是键变量，也没有'vals=' 为前缀，那么相关联的事件字段会
保存到变量中，但不会被求
```
...
```
可以同时赋值多个变量。下面这行会ts0 b 都创建为变量，二
```
...
```
任意数量的、未绑定'vals=' 前缀的变量，也可以通过用冒号分隔来赋值
下面同样
```
...
```
在上面第一行中，事件的时间戳被保存到变ts0 中。在下一行中，从第二个事
的时间戳减去 ts0，得到延迟值，并将其赋给另一个变'wakeup_lat'。下面的
直方图触发器进而利wakeup_lat 变量来计算组合延
```
...
用户自定义的事件。它们的目的是提供一种机制，以与现有且已为人所熟悉的普通事
用法一致的方式，展示跨越多个事件的数据

要定义一个合成事件，用户需编写一个简单的规格说明，其中包含新事件的名称以
一个或多个变量及其类型（可以是任何合法的字段类型），用分号分隔，写
tracing/synthetic_events 文件

可用类型请参synth_field_size()

如果字段名包[n]，则该字段被视为静态数组

如果字段名包[]（无下标），则该字段被视为动态数组，它只会占用事件中
容纳该数组所需同样大小的空间
...
例如，下面创建一个名'wakeup_latency' 的新事件，包3 个字段：lat、pid
prio。这些字段中的每一个都只是
```
...
```
读取 tracing/synthetic_events 文件会列出当前所有已定义
```
...
```
此时，事件子系统中还没有真正实例化出一'wakeup_latency' 事件——要做到
这一点，需要实例化一个“hist 触发器动作”并将其绑定到其他事件上定义的真
字段与变量（关于如何使用 hist 触发器的 'onmatch' 动作来完成，请参见下面的
2.7 节）。一旦完成，就会创建'wakeup_latency' 合成事件实例

新事件创建在 tracing/events/synthetic/ 目录
```
...
```
要保存栈回溯，可创建一个带"unsigned long[]" 类型字段（甚至只需 "long[]"
的合成事件。例如，要查看某个任务在
```
...
当直方图条目被添加或更新时，由直方图触发器的“处理器”来决定相应的动作是否真
被调用
...
要为给定事件指定一handler.action 对，只需hist 触发器规格说明中、用冒号
将该 handler.action 对括起来即可

理论上，任何处理器都可以与任何动作组合，但在实践中，并非每一handler.action
组合当前都受支持；如果某handler.action 组合不受支持，hist 触发器将-EINVAL
失败

如果未显式指定，默认的“handler.action”与以往一样，仅仅是更新与某条目关联的一
值。不过，某些应用可能希望此时执行额外的动作，例如生成另一个事件，或者比较并
保存最大值

受支持的处理器与动作列于下方，并在后续段落中结合一些常见且有用handler.action
组合的描述进行更详细的说明

可用的处理器有：

  - onmatch(matching.event)    - 在任意添加或更新时调用动
  - onmax(var)                 - var 超过当前最大值时调用动作
  - onchange(var)              - var 发生变化时调用动
...
以下是常用的 handler.action 组合
  ...
  - onmatch(matching.event).trace(<synthetic_event_name>,param list)
    ...
    list)' 直方图触发器动作在事件匹配且直方图条目将被添加或更新时被调用。它
    'param list' 中给出的值生成指定的合成事件。其结果是生成一个合成事件，
...
修改 cyclictest，使其在休眠前写trace_marker 文件
```
...
```
上面创建了一个名"latency" 的合成事件，以及两个针对 trace_marker 的直方图
当向 trace_marker 文件写入 "start" 时触发其中一个，写入 "end" 时触发另一个
如果 pid 匹配，则它会以计算得到的延迟作为参数来调"latency" 合成事件。最后，
latency 合成事件添加一个直方图，以记录计算得到的延迟以pid
```
...
```
注意，写操作发生在休眠前后，因此理想情况下它们都应为 250 微秒。如果你好奇为什
会有若干次小250 微秒，那是因cyclictest 的工作方式：如果某次迭代来迟了，
下一次就会把定时器设置为在小250 微秒后唤醒。也就是说，如果某次迭代晚了
50 微秒，则下一次唤醒会200 微秒后
...
事件，将 trace_marker 写入时的 common_pid sched_waking 正在唤醒pid 进行匹配
```
...
反映cyclictest 被唤醒到它进入用户空间所经历的时间
