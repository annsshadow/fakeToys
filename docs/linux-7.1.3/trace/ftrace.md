...
- 编写针对: 2.6.28-rc2
- 更新针对: 3.10
- 更新针对: 4.13 - 版权所有 2017 VMware 公司 Steven Rostedt
- 转换为 rst 格式 - Changbin Du <changbin.du@intel.com>

### 简介


ftrace 是一个内部跟踪器，旨在帮助开发者与系统设计者了解内核内部
...
架构移植者等相关细节请参阅 Documentation/trace/ftrace-design.rst。
...
当 tracefs 被配置进内核（选择任意 ftrace 选项都会如此）时，会创建
...
```
  ...
  在 4.1 之前，所有 ftrace 跟踪控制文件都位于 debugfs 文件系统中，
  通常位于 /sys/kernel/debug/tracing。为了向后兼容，当挂载 debugfs
  ...
  任何被选中的 ftrace 选项也会创建 tracefs 文件系统。文档的其余部分
  将假设你处于 ftrace 目录中（cd /sys/kernel/tracing），并且只会
  关注该目录内的文件，而不会用冗长的 "/sys/kernel/tracing" 路径名
  来分散对内容的注意力。

就这样！（假设你已经将 ftrace 配置进了内核）

挂载 tracefs 之后，你将可以访问 ftrace 的控制和输出文件。以下是其中
一些关键文件的列表：
 ...
 注意：所有时间值均以微秒为单位。
  ...
  current_tracer:
	...
	该文件用于设置或显示当前已配置的跟踪器。更改当前跟踪器会同时
	清除环形缓冲区的内容以及 "snapshot"（快照）缓冲区。

  available_tracers:

	该文件保存已编译进内核的各种不同类型的跟踪器。这里列出的
	跟踪器可以通过将它们的名字 echo 到 current_tracer 来配置。

  tracing_on:

	该文件设置或显示是否启用了向跟踪环形缓冲区的写入。向该文件
	echo 0 可禁用跟踪器，echo 1 可启用它。注意，这只会禁用向环形
	缓冲区的写入，跟踪开销仍有可能在继续发生。

	内核函数 tracing_off() 可用于在内核内部禁用向环形缓冲区的写入，
	这会把该文件设置为 "0"。用户空间可以通过向该文件 echo "1" 来
	重新启用跟踪。

	注意，函数和事件的 "traceoff" 触发器也会将该文件清零并停止跟踪。
	它同样可以由用户空间使用该文件来重新启用。

  trace:

	该文件以人类可读的格式（如下所述）保存跟踪的输出。以 O_TRUNC
	标志打开该文件进行写入会清除环形缓冲区的内容。注意，该文件
	不是一个消费型文件。如果跟踪已关闭（没有跟踪器在运行，或
  ...
  trace_pipe:
	...
	它。"trace" 文件是静态的，如果跟踪器没有添加更多数据，每次读取它
	都会显示相同的信息。

  trace_options:

	该文件让用户可以控制上述某个输出文件中所显示的数据量。也有一些
	选项用于修改跟踪器或事件的工作方式（栈回溯、时间戳等）。

  options:

	这是一个目录，其中包含每个可用跟踪选项（同样存在于 trace_options
	中）对应的一个文件。也可以向对应选项名的文件写入 "1" 或 "0" 来
	设置或清除该选项。

  tracing_max_latency:

	部分跟踪器会记录最大延迟。例如，中断被禁用的最长时间。最大时间
	保存在该文件中。最大跟踪记录也会被保存，并由 "trace" 显示。只有当
	延迟大于该文件中的值时，才会记录一条新的最大跟踪（以微秒为单位）。

	向该文件 echo 一个时间值后，除非延迟大于该文件中的时间，否则不会
	记录任何延迟。

  tracing_thresh:

	当延迟大于该文件中的数值时，部分延迟跟踪器会记录一条跟踪。仅当
	该文件中的数值大于 0 时才生效。（以微秒为单位）

  buffer_percent:

	这是环形缓冲区在被唤醒之前需要填充多少的水位线。也就是说，如果一个
	应用程序在一个 per_cpu 的 trace_pipe_raw 文件上调用阻塞读取系统调用，
...
```
  ...
  buffer_size_kb:
	...
	该文件设置或显示每个 CPU 缓冲区所持有的千字节数。默认情况下，每个
	CPU 的跟踪缓冲区大小相同。显示的数值是 CPU 缓冲区的大小，而不是所有
	缓冲区的总大小。跟踪缓冲区以页（内核用于分配的内存块，通常为 4 KB）
	为单位分配。可能会额外分配少许几页以容纳缓冲区的管理元数据。如果最后
  ...
  buffer_subbuf_size_kb:
	...
	等于架构的页大小（x86 上为 4K）。子缓冲区开头还包含元数据，这同样
	限制了事件的大小。这意味着当子缓冲区为一个页大小时，没有任何事件能
	大于页大小减去子缓冲区元数据。
  ...
  free_buffer:
	...
	它被信号杀死）被收缩/"释放"，则可以使用该文件来实现此目的。在该文件
	关闭时，环形缓冲区会被重置为最小大小。让正在跟踪的进程同时打开该文件，
	当该进程退出时，其对应此文件的文件描述符会被关闭，在此过程中环形缓冲区
	会被"释放"。

	如果设置了 disable_on_free 选项，它也可能停止跟踪。

  tracing_cpumask:

	这是一个掩码，让用户只能在特定 CPU 上进行跟踪。格式为表示 CPU 的
	十六进制字符串。

  set_ftrace_filter:

	当配置了动态 ftrace 时（见下面的 "dynamic ftrace" 一节），代码会被
	动态修改（代码文本重写）以禁用对函数性能分析器（mcount）的调用。这使得
	配置跟踪带来的性能开销几乎可以忽略不计。这还有一个副作用，即能够启用
	或禁用对特定函数的跟踪。向该文件 echo 函数名，将把跟踪限制为仅这些函数。
	这会影响 "function" 和 "function_graph" 跟踪器，因此也影响函数性能分析
	...
	跟踪中的函数，因此可以改为向该文件写入一个索引。写入一个数字（以 "1"
	开头）将改为选择 "available_filter_functions" 文件中对应行位置的相同
	函数。

  set_ftrace_notrace:

	该文件的作用与 set_ftrace_filter 相反。添加到此处的任何函数都不会被
	跟踪。如果一个函数同时存在于 set_ftrace_filter 和 set_ftrace_notrace
	中，则该函数在_不_会被跟踪。

  set_ftrace_pid:

	让函数跟踪器只跟踪 PID 列在该文件中的线程。

	如果设置了 "function-fork" 选项，那么当 PID 列在该文件中的任务 fork 时，
	子进程的 PID 会自动添加到该文件中，子进程也将被函数跟踪器跟踪。该选项
	还会导致退出的任务的 PID 从该文件中被移除。
  ...
  set_ftrace_notrace_pid:
	...
	同样会导致退出的任务的 PID 从该文件中被移除。

        如果一个 PID 同时存在于该文件和 "set_ftrace_pid" 中，则该文件优先，
	该线程不会被跟踪。

  set_event_pid:

	让事件只跟踪 PID 列在该文件中的任务。注意，sched_switch 和 sched_wake_up
	也会跟踪列在该文件中的事件。

	要让列在该文件中的任务的子进程 PID 在 fork 时被添加进来，请启用
	"event-fork" 选项。该选项还会导致任务的 PID 在任务退出时从该文件中被移除。

  set_event_notrace_pid:

	让事件不跟踪 PID 列在该文件中的任务。注意，sched_switch 和 sched_wakeup
	会跟踪未列在该文件中的线程，即使某个线程的 PID 在该文件中，如果
	sched_switch 或 sched_wakeup 事件同时也跟踪某个应当被跟踪的线程。
  ...
  set_graph_function:
	...
	（更多细节见 "dynamic ftrace" 一节。）注意，set_ftrace_filter 和
	set_ftrace_notrace 仍然会影响哪些函数被跟踪。

  set_graph_notrace:

	类似于 set_graph_function，但在命中该函数时禁用函数图跟踪，直到它退出
	该函数为止。这样可以忽略对某个特定函数所调用函数的跟踪。

  available_filter_functions:

	该文件列出 ftrace 已处理并且可以跟踪的函数。这些就是你可以传递给
	"set_ftrace_filter"、"set_ftrace_notrace"、"set_graph_function" 或
	"set_graph_notrace" 的函数名。
	（更多细节见下面的 "dynamic ftrace" 一节。）

  available_filter_functions_addrs:

	类似于 available_filter_functions，但为每个函数显示地址。显示的地址是
	补丁站点地址，可能与 /proc/kallsyms 中的地址不同。

  syscall_user_buf_size:

	部分系统调用跟踪事件会记录某个参数所指向的用户空间地址中的数据。每个
	事件的数据量是受限的。该文件保存将被记录进环形缓冲区以保存这些数据的最大
	字节数。当前最大值为 165。
  ...
  enabled_functions:
	...
	该文件更多用于调试 ftrace，但在查看是否有任何函数挂接了回调时也很有用。
	不仅跟踪基础设施会用到 ftrace 的函数跟踪功能，其他子系统也可能用到。该文件
	显示所有挂接了回调的函数，以及已挂接的回调数量。注意，一个回调也可能调用
	多个函数，这些不会被计入此计数。

	如果注册的回调是以 "save regs" 属性（因此开销更大）被跟踪的函数，则在与
	返回寄存器的函数同一行会显示一个 'R'。

	如果注册的回调是以 "ip modify" 属性（因此 regs->ip 可以被修改）被跟踪的
	函数，则在同一行会显示一个 'I'。

	如果挂接了一个非 ftrace 的蹦床（BPF），则会显示一个 'D'。注意，普通的
	ftrace 蹦床也可以挂接，但一个给定的函数一次只能挂接一个"直接"蹦床。

	某些架构无法调用直接蹦床，而是将 ftrace ops 函数放置在函数入口点之上。
	在这种情况下会显示一个 'O'。

	如果一个函数过去曾挂接了 "ip modify" 或直接调用，则会显示一个 'M'。该标志
	永远不会被清除。它用于了解某个函数是否曾被 ftrace 基础设施修改过，可用于
	调试。

	如果架构支持，它还会显示该函数正在直接调用的回调。如果计数大于 1，则很可能
	是 ftrace_ops_list_func()。
  ...
  touched_functions:
	...
	要查看任何曾被 "ip modify" 或直接蹦床修改过的函数，可以执行以下命令：
  ...
  printk_formats:
	...
	该文件供读取原始格式文件的工具使用。如果环形缓冲区中的一个事件引用了一个
	字符串，则只把指向该字符串的指针记录进缓冲区，而不是字符串本身。这导致工具
	无法知道那个字符串是什么。该文件显示字符串及其地址，使工具能够将指针映射
	到对应的字符串。

  saved_cmdlines:

	除非事件特别保存了任务的 comm，否则在跟踪事件中只记录任务的 pid。ftrace
	建立一个 pid 到 comm 的映射缓存，以尝试为事件显示 comm。如果某个 comm 的 pid
	未列出，则输出中会显示 "<...>"。

	如果 "record-cmd" 选项被设置为 "0"，则在记录期间不会保存任务的 comm。默认
	情况下它是启用的。

  saved_cmdlines_size:

	默认情况下保存 128 个 comm（见上面的 "saved_cmdlines"）。要增加或减少被缓存
	的 comm 数量，向该文件 echo 要缓存的 comm 数量。

  saved_tgids:

	如果设置了 "record-tgid" 选项，则每次调度上下文切换时，任务的线程组 ID 会被
	保存到一个映射 PID 到其 TGID 的表中。默认情况下，"record-tgid" 选项是禁用的。

  snapshot:

	该文件显示"快照"缓冲区，并允许用户对当前正在运行的跟踪拍摄快照。更多细节
	请参阅下面的 "Snapshot"（快照）一节。

  stack_max_size:

	当启用栈跟踪器时，该文件会显示它所遇到的最大栈大小。请参阅下面的 "Stack Trace"
	（栈跟踪）一节。

  stack_trace:

	该文件显示启用栈跟踪器时所遇到的最大栈的栈回溯。请参阅下面的 "Stack Trace"
	（栈跟踪）一节。
...
```
	...
	global:
		该时钟与所有 CPU 同步，但可能比本地时钟稍慢。

	counter:
		这根本不是时钟，而是一个字面上的原子计数器。它逐个递增，但与所有
		CPU 同步。当你需要确切知道不同 CPU 上事件相互之间的发生顺序时，这
		很有用。

	uptime:
		该时钟使用 jiffies 计数器，时间戳相对于系统启动后的时间。

	perf:
		该时钟使 ftrace 使用与 perf 相同的时钟。最终 perf 将能够读取 ftrace
		缓冲区，这将有助于交织合并数据。
	...
	ppc-tb:
		该时钟使用 powerpc 的 timebase 寄存器值。它与所有 CPU 同步，并且如果
		已知 tb_offset，还可用于关联管理程序/客户机之间的事件。
	...
	mono_raw:
		该时钟是原始单调时钟（CLOCK_MONOTONIC_RAW），它是单调的，但不受任何
		速率调整影响，并以与硬件时钟源相同的速率走时。

	boot:
		该时钟是启动时钟（CLOCK_BOOTTIME），基于快速单调时钟，但也会计入在
		挂起状态中所花费的时间。由于时钟访问是为在挂起路径中的跟踪使用而设计，
		如果在快速 mono 时钟更新之前、在挂起时间被计入之后访问该时钟，可能会
		产生一些副作用。在这种情况下，时钟更新看起来会比正常情况下稍早发生。
		此外在 32 位系统上，64 位启动偏移量可能会看到部分更新。这些效应很罕见，
		后处理应当能够处理它们。更多信息请参阅 ktime_get_boot_fast_ns() 函数
		中的注释。

	tai:
		该时钟是 tai 时钟（CLOCK_TAI），派生自墙上时钟时间。但是，该时钟不会
		经历由 NTP 插入闰秒所导致的间断和回跳。由于时钟访问是为跟踪使用而设计，
		可能会产生副作用。如果内部 TAI 偏移量被更新（例如由设置系统时间或使用
		带偏移量的 adjtimex() 引起），时钟访问可能会产生错误的读数。这些效应
...
```
  ...
  trace_marker:
	...
	该文件对于将用户空间与内核中发生的事件同步非常有用。向该文件写入字符串会
	被写入 ftrace 缓冲区。
...
```
	...
	注意：写入 trace_marker 文件也可以触发写入到
	/sys/kernel/tracing/events/ftrace/print/trigger 的触发器。请参阅
	Documentation/trace/events.rst 中的 "Event triggers"（事件触发器）以及
	Documentation/trace/histogram.rst（第 3 节）中的示例。

  trace_marker_raw:

	该文件类似于上面的 trace_marker，但用于向其写入二进制数据，可以用工具从
  ...
  events:
	...
	它显示存在哪些事件跟踪点，以及它们如何按系统分组。在不同层级有 "enable"
  ...
  timestamp_mode:
	...
	某些跟踪器可能会改变将跟踪事件记录进事件缓冲区时所使用的时间戳模式。具有
...
```
	  ...
	  delta: 默认时间戳模式 - 时间戳是相对于每个缓冲区时间戳的增量。

	  absolute: 时间戳是完整时间戳，而不是相对于其他某个值的增量。因此它会占用
	                更多空间，效率也较低。

  hwlat_detector:

	硬件延迟探测器的目录。请参阅下面的 "Hardware Latency Detector"（硬件延迟
	探测器）一节。
  ...
  per_cpu/cpu0/buffer_size_kb:
	...
	ftrace 缓冲区是按 per_cpu 定义的。也就是说，每个 CPU 都有一个独立的缓冲区，
	以便写入可以原子方式进行，并避免缓存抖动。这些缓冲区可能有不同的大小。该文件
	类似于 buffer_size_kb 文件，但它只显示或设置特定 CPU（此处为 cpu0）的缓冲区大小。

  per_cpu/cpu0/trace:

	该文件类似于 "trace" 文件，但它只显示特定于该 CPU 的数据。如果向其写入，它
	只清除特定 CPU 的缓冲区。
  ...
  per_cpu/cpu0/trace_pipe_raw
	...
	与 trace_pipe 一样，这是一个消费型读取器，多次读取总是会产生不同的数据。

  per_cpu/cpu0/snapshot:

	该文件类似于主 "snapshot" 文件，但只会对当前 CPU 拍摄快照（如果支持）。它只
	显示给定 CPU 的快照内容，如果向其写入，则只清除该 CPU 的缓冲区。

  per_cpu/cpu0/snapshot_raw:

	类似于 trace_pipe_raw，但会从给定 CPU 的快照缓冲区读取二进制格式。
...
以下是当前可以配置的跟踪器列表。
  ...
  "function_graph"
	...
	函数的入口和出口都进行跟踪。它还提供了绘制类似于 C 代码源码的函数调用
	图的能力。
  ...
  "hwlat"
	...
	硬件延迟跟踪器，用于检测硬件是否产生任何延迟。请参阅下面的 "Hardware Latency
	Detector"（硬件延迟探测器）一节。

  "irqsoff"

	跟踪禁用中断的区域，并保存具有最长最大延迟的跟踪。请参阅 tracing_max_latency。
	当记录到新的最大值时，它会替换旧的跟踪。最好配合 latency-format 选项启用时
	查看此跟踪，选择该跟踪器时会自动启用该选项。

  "preemptoff"

	类似于 irqsoff，但跟踪并记录抢占被禁用的时间长度。

  "preemptirqsoff"

	类似于 irqsoff 和 preemptoff，但跟踪并记录 irq 和/或抢占被禁用的最长时间。
  ...
  "wakeup_rt"
	...
	跟踪并记录仅仅 RT 任务（如同当前的 "wakeup" 那样）被唤醒所花费的最大延迟。这
	对关注 RT 任务唤醒时间的人很有用。

  "wakeup_dl"

	跟踪并记录 SCHED_DEADLINE 任务被唤醒（如同 "wakeup" 和 "wakeup_rt" 那样）所
	花费的最大延迟。
  ...
  "branch"
	...
	该跟踪器可以在跟踪内核中的 likely/unlikely 调用时配置。它会跟踪命中一个
	likely 或 unlikely 分支的时机，以及它对该分支预测的是否正确。

  "nop"

	这是"什么都不跟踪"的跟踪器。要移除所有跟踪器，只需向 current_tracer echo
	"nop" 即可。

### 错误情形


  对于大多数 ftrace 命令，失败模式是显而易见的，并且使用标准返回码进行反馈。

  对于其他更复杂的命令，可通过 tracing/error_log 文件获取扩展错误信息。对于支持
  它的命令，在出错后读取 tracing/error_log 文件会显示有关出错原因的更详细信息
  （如果有信息可用）。tracing/error_log 文件是一个循环错误日志，显示少量（当前为
  8 条）最近的（8 条）失败命令的 ftrace 错误。
...
以下是在仅使用 tracefs 接口（不使用任何用户态工具）控制跟踪器时的典型示例。
...
会打印一个头部，其中包含由跟踪表示的跟踪器名。在本例中跟踪器是 "function"。然后它
显示缓冲区中的事件数量以及已写入的条目总数。两者的差值就是由于缓冲区填满而丢失的
条目数（250280 - 140080 = 110200 个事件丢失）。

头部解释了事件的内容。任务名 "bash"、任务 PID "1977"、运行所在的 CPU "000"、延迟格式
（如下解释）、<secs>.<usecs> 格式的时间戳、被跟踪的函数名 "sys_close" 以及调用该函数的
父函数 "system_call_fastpath"。时间戳是函数被进入的时间。

### 延迟跟踪格式


当启用了 latency-format 选项，或者设置了某个延迟跟踪器时，trace 文件会提供更多信息
以便查看
...
```
这表明当前跟踪器是 "irqsoff"，正在跟踪中断被禁用的时间。它给出跟踪版本（从不改变）
以及其上执行的内核版本（3.8）。然后它显示最大延迟（以微秒为单位，259 us）。显示的
跟踪条目数以及总数（两者都是四：#4/4）。VP、KP、SP 和 HP 始终为零，保留供以后使用。
#P 是在线 CPU 的数量（#P:4）。

任务是延迟发生时正在运行的进程。（ps pid：6143）。
...
头部之后的几行是跟踪本身。头部解释了哪个是哪个。
  ...
  irqs-off: 'd' 表示中断被禁用，否则为 '.'。

  need-resched:
 - 'B' 表示 TIF_NEED_RESCHED、PREEMPT_NEED_RESCHED 和 TIF_RESCHED_LAZY 都已设置，
 - 'N' 表示 TIF_NEED_RESCHED 和 PREEMPT_NEED_RESCHED 都已设置，
 - 'n' 仅 TIF_NEED_RESCHED 被设置，
 - 'p' 仅 PREEMPT_NEED_RESCHED 被设置，
 - 'L' 表示 PREEMPT_NEED_RESCHED 和 TIF_RESCHED_LAZY 都已设置，
 - 'b' 表示 TIF_NEED_RESCHED 和 TIF_RESCHED_LAZY 都已设置，
 - 'l' 仅 TIF_RESCHED_LAZY 被设置
 - '.' 否则。
 ...
 - '.' - 普通上下文。
  ...
  preempt-depth: preempt_disabled 的层级
...
```
  ...
  sym-offset
	不仅显示函数名，还显示函数内的偏移量。例如，你看到的不再是 "ktime_get"，
...
```
  ...
  raw
	该选项显示原始数字。此选项最适合与能够更好地翻译原始数字的用户态应用程序
	配合使用，而不是由内核来完成。
  ...
  fields
	按其类型所述打印字段。这比使用 hex、bin 或 raw 更好，因为它能更好地解析
	事件的内容。
  ...
  trace_printk_dest
	设置后让 trace_printk() 及类似的内部跟踪函数写入此实例。注意，只有一个跟踪
	实例可以设置此标志。设置此标志会清除之前设置了该标志的实例的 trace_printk_dest
  ...
  copy_trace_marker
	如果有应用程序硬编码写入顶层 trace_marker 文件（/sys/kernel/tracing/trace_marker
	或 trace_marker_raw），而工具希望将其转到某个实例，则可以使用此选项。创建一个
	实例并设置此选项，之后所有对顶层 trace_marker 文件的写入也都会被重定向到该
	实例。

	注意，默认情况下顶层实例设置了此选项。如果它被禁用，那么对 trace_marker 或
	trace_marker_raw 文件的写入将不会被写入顶层文件。如果没有任何实例设置此选项，
	则写入将以 ENODEV 错误码失败。

  annotate
	当 CPU 缓冲区已满，且某个 CPU 缓冲区最近有大量事件（因而时间窗口较短），而
	另一个 CPU 可能只有少量事件（从而可以保留较旧的事件）时，情况有时会令人困惑。
	当报告跟踪时，它先显示最旧的事件，并且可能看起来好像只有运行时间最长的那个
	CPU（拥有最旧事件的那个）在运行。当设置了 annotate 选项时，它会显示一个新的
	CPU 缓冲区何时开始：
...
```
  ...
  sym-userobj
	...
	时这尤其有用，否则在应用不再运行后，你无法将地址解析为对象/文件/行。
...
```
  ...
  printk-msg-only
	设置后，trace_printk() 将只显示格式而不显示其参数（如果使用了 trace_bprintk()
	或 trace_bputs() 来保存 trace_printk()）。

  context-info
	只显示事件数据。隐藏 comm、PID、时间戳、CPU 和其他有用数据。

  latency-format
	该选项会改变跟踪输出。启用时，跟踪会显示有关延迟的附加信息，如 "Latency trace
	format"（延迟跟踪格式）中所述。

  pause-on-trace
	设置后，为读取而打开 trace 文件会暂停向环形缓冲区的写入（如同 tracing_on 被
	设置为 0）。这模拟了 trace 文件最初的行为。当文件关闭时，跟踪会重新启用。
  ...
  bitmask-list
        启用后，位掩码使用 printk 的 "%*pbl" 格式说明符显示为可读的范围列表（例如
	0,2-5,7）。禁用时（默认），位掩码以传统的十六进制位图表示形式显示。列表格式
	对于跟踪 CPU 掩码和其他大型位掩码特别有用，其中单个位的位置比其十六进制编码
	更有意义。

  record-cmd
	当启用任何事件或跟踪器时，会在 sched_switch 跟踪点中启用一个钩子，以填充映射了
	pid 和 comm 的 comm 缓存。但这可能会带来一些开销，如果你只关心 pid 而不关心任务
	名，禁用此选项可以降低跟踪的影响。请参阅 "saved_cmdlines"。

  record-tgid
	当启用任何事件或跟踪器时，会在 sched_switch 跟踪点中启用一个钩子，以填充映射了
	线程组 ID（TGID）到 pid 的缓存。请参阅 "saved_tgids"。

  overwrite
	该选项控制当跟踪缓冲区已满时发生的情况。如果为 "1"（默认），最旧的事件会被
	丢弃并覆盖。如果为 "0"，则丢弃最新的事件。
	（见 per_cpu/cpu0/stats 中的 overrun 和 dropped）

  disable_on_free
	当 free_buffer 关闭时，跟踪会停止（tracing_on 被设置为 0）。
...
```
  ...
  markers
	设置后，trace_marker 可写（仅 root）。禁用时，对 trace_marker 的写入将以 EINVAL
	错误。

  event-fork
	设置后，列在 set_event_pid 中的 PID 的任务在 fork 时，其子进程的 PID 会被添加到
	set_event_pid。同样，当列在 set_event_pid 中的 PID 的任务退出时，其 PID 会从该
	文件中移除。
  ...
  function-fork
	设置后，列在 set_ftrace_pid 中的 PID 的任务在 fork 时，其子进程的 PID 会被添加到
	set_ftrace_pid。同样，当列在 set_ftrace_pid 中的 PID 的任务退出时，其 PID 会
	从该文件中移除。
  ...
  branch
	用跟踪器启用分支跟踪。这会启用分支跟踪器以及当前设置的跟踪器。用 "nop" 跟踪器
...
函数跟踪器的选项：
  ...
  func_stack_trace
	设置后，在每次记录的函数之后都会记录一条栈回溯。注意！在启用此选项之前，先用
...
函数图跟踪器的选项：
  ...
  funcgraph-overrun
	设置后，在每次被跟踪的函数之后会显示图栈的"溢出"。溢出是指调用栈深度大于为每个
	任务保留的深度。每个任务都有一个固定大小的函数数组用于调用图跟踪。如果调用
	深度超过该数组，该函数就不会被跟踪。溢出就是由于超出该数组而错过的函数数量。

  funcgraph-cpu
	设置后，显示发生跟踪的 CPU 的 CPU 编号。
  ...
  funcgraph-tail
	设置后，返回事件会包含它所代表的函数。默认情况下关闭，只为函数返回显示一个
	闭合花括号 "}"。

  funcgraph-retval
	设置后，每个被跟踪函数的返回值会打印在等号 "=" 之后。默认情况下关闭。

  funcgraph-retval-hex
	设置后，返回值将始终以十六进制格式打印。如果未设置该选项且返回值是错误码，则
...
当中断被禁用时，CPU 无法对任何其他外部事件（除了 NMI 和 SMI）做出反应。这会阻止
...
要重置最大值，向 tracing_max_latency echo 0。以下是
...
```
这里我们看到延迟为 16 微秒（非常好）。run_timer_softirq 中的 _raw_spin_lock_irq 禁用了
中断。显示的 16 与显示的时间戳 25us 之间的差异，是因为记录最大延迟的时间与记录具有
该延迟的函数的时间之间，时钟被增加了。

注意上面的示例未设置 function-trace。如果我们设置
...
```
这里我们跟踪了一个 71 微秒的延迟。但我们也看到了在此期间被调用的所有函数。注意，通过
启用函数跟踪，我们带来了额外的开销。这个开销可能会延长延迟时间。但尽管如此，此跟踪
提供了一些非常有帮助的调试信息。

如果我们偏好函数图输出而非函数输出，可以设置
...
当抢占被禁用时，我们可能能接收中断，但任务无法被抢占，更高优先级的任务必须等待抢占
被重新启用后才能抢占较低优先级的任务。

preemptoff 跟踪器跟踪禁用抢占的位置。与 irqsoff 跟踪器类似，它记录抢占被禁用的最大
延迟。preemptoff 跟踪器的控制方式与 irqsoff 跟踪器非常相似。
...
```
这有一些更多的变化。当进入中断时（注意 'h'），抢占被禁用，并在退出时启用。但我们也
看到在进入抢占禁用段和离开它时中断已被禁用（'d'）。我们不知道在此期间或此后不久中断
是否被重新启用。
...
```
上面是设置了 function-trace 的 preemptoff 跟踪示例。这里我们看到中断并非在整个期间都
被禁用。irq_enter 代码让我们知道我们进入了一个中断 'h'。在此之前，被跟踪的函数仍然
显示它不在中断中，但我们从函数本身可以看出情况并非如此。

### preemptirqsoff


了解中断被禁用或抢占被禁用时间最长的位置很有帮助。但有时我们想知道抢占和/或中断
何时被禁用。
...
```
irqsoff 跟踪器会记录 call_function_with_irqs_off() 和
call_function_with_irqs_and_preemption_off() 的总长度。

preemptoff 跟踪器会记录 call_function_with_irqs_and_preemption_off() 和
call_function_with_preemption_off() 的总长度。

但二者都不会跟踪中断和/或抢占被禁用的时间。这个总时间是我们无法调度的时长。要记录
此时长，请使用 preemptirqsoff 跟踪器。
...
```
trace_hardirqs_off_thunk 在 x86 上从中断被禁用时由汇编代码调用。没有函数跟踪，我们
...
```
这是一次有趣的跟踪。它从 kworker 运行并调度出去、ls 接管开始。但一旦 ls 释放了 rq 锁
并启用了中断（但未启用抢占），一个中断就被触发了。当中断结束时，它开始运行软中断。
但在软中断运行期间，另一个中断被触发了。当中断在软中断内部运行时，标记为 'H'。
...
```
跟踪器只跟踪系统中最高优先级的任务，以避免跟踪正常情况。这里我们看到 nice 优先级为
...
在实时环境中，了解被唤醒的最高优先级任务从被唤醒到它执行所花费的唤醒时间非常
重要。这也被称为"调度延迟"。我强调一点，这是关于 RT 任务的。了解非 RT 任务的调度
延迟也很重要，但对于非 RT 任务，平均调度延迟更合适。像 LatencyTop 这样的工具更适合
此类测量。

实时环境关注最坏情况延迟。也就是某件事发生所需的最长时间，而不是平均时间。我们可以
有一个非常快的调度器，它可能只是偶尔才出现一次大延迟，但这对实时任务来说并不合适。
wakeup_rt 跟踪器就是为记录 RT 任务的最坏情况唤醒而设计的。非 RT 任务不会被记录，因为
该跟踪器只记录一个最坏情况，跟踪不可预测的非 RT 任务会覆盖掉 RT 任务的最坏情况延迟
（只需运行一段时间的普通 wakeup 跟踪器就能看到这种效果）。

由于该跟踪器只处理 RT 任务，我们将以与之前跟踪器略有不同的方式运行它。不再执行 'ls'，
而是在 'chrt' 下运行 'sleep 1'，这会更改任务的优先级。
...
跟踪点位于实际"切换"之前，我们在被记录的任务即将调度进来时停止跟踪。如果我们在一个
调度器末尾添加一个新的标记，这可能会改变。

注意记录的任务 'sleep' 的 PID 为 2389，它的 rt_prio 为 5。该优先级是用户空间优先级，
而不是内核内部优先级。policy 为 1 表示 SCHED_FIFO，2 表示 SCHED_RR。

注意，跟踪数据显示的是内部优先级（99 - rtprio）。
...
```
0:120:R 表示 idle 以 nice 优先级 0（120 - 120）运行，并处于运行态 'R'。sleep 任务以
2389: 94:R 被调度进来。也就是说优先级是内核 rtprio（99 - 5 = 94），它也处于运行态。

用 chrt -r 5 并设置 function-trace 做同样的事。
...
由于函数跟踪会带来大得多的延迟，但如果不看到延迟期间发生了什么，就很难知道是什么
...
注意，该跟踪器会影响系统性能，因为它会周期性地让一个 CPU 在中断禁用的情况下持续忙等。
...
上面的输出在头部上大致相同。所有事件都会有中断禁用标志 'd'。在 FUNCTION 标题下方是：
 ...
 #1
	这是记录的、大于 tracing_threshold（见下文）的事件计数。

 inner/outer(us):   11/11

      这显示两个数字："内部延迟"和"外部延迟"。测试在一个循环中运行，检查两次时间戳。
      在两个时间戳之间检测到的延迟就是"内部延迟"，而在前一个时间戳和循环中下一个
      时间戳之间检测到的延迟就是"外部延迟"。
 ...
 nmi-total:7 nmi-count:1
      ...
      在支持它的架构上，如果测试期间来了 NMI，则 NMI 中花费的时间会报告在 "nmi-total"
      中（以微秒为单位）。

      所有具有 NMI 的架构都会在测试期间来了 NMI 时显示 "nmi-count"。

hwlat 文件：

  tracing_threshold
	该文件会自动设置为 "10"，表示 10 微秒。这是需要被检测到才会记录跟踪的延迟阈值。

	注意，当 hwlat 跟踪器结束（向 "current_tracer" 写入另一个跟踪器）时，tracing_threshold
	的原始值会被放回该文件。
  ...
  hwlat_detector/window
	测试运行的窗口的时长。也就是说，测试会在每个 "window" 微秒内运行 "width"
	微秒。

  tracing_cpumask
	测试启动时，会创建一个内核线程来运行测试。该线程会在每个周期（一个 "window"）
	之间于 tracing_cpumask 中列出的 CPU 之间交替。要将测试限制在特定 CPU 上，请
	将该文件中的掩码设置为测试应当运行的那些 CPU。

### function


该跟踪器就是函数跟踪器。可以通过调试文件系统启用函数跟踪器。确保 ftrace_enabled 已
设置；否则该跟踪器就是一个 nop。请参阅下面的 "ftrace_enabled" 一节。
...
```
注意：函数跟踪器使用环形缓冲区来存储上述条目。最新的数据可能会覆盖最旧的数据。有时
使用 echo 来停止跟踪并不够，因为跟踪可能已经覆盖了你想记录的数据。因此，有时最好直接
从程序中禁用跟踪。这让你能够在命中你感兴趣的部分时停止跟踪。要从 C 程序直接禁用跟踪，
...
该跟踪器与函数跟踪器类似，区别在于它在函数的入口和出口处都进行探测。这是通过在每个
...
- 你想找出某个奇怪的内核行为的原因，并且需要详细查看任何区域（或特定区域）内部发生了
  什么。
...
```
有几个列可以动态启用/禁用。你可以根据需要使用任意选项组合。

- 函数执行所在的 CPU 编号默认启用。有时最好只跟踪一个 CPU（见 tracing_cpumask 文件），
  或者你可能在 CPU 跟踪切换时看到乱序的函数调用。
...
```
- 绝对时间字段是系统时钟自启动以来给出的绝对时间戳。在函数的每次进入/退出时给出此时间的
  快照。
...
对于开头在跟踪缓冲区中的函数，也可以启用闭合花括号之后显示函数名，以便用 grep 更
容易地搜索函数持续时间。默认禁用。
...
```
上面的示例显示函数 cpu_cgroup_can_attach 首先返回了错误码 -22，然后我们可以阅读该
函数的代码来找到根本原因。
...
```
目前，使用 funcgraph-retval 选项有一些限制，这些限制将在未来被消除：

- 即使函数的返回类型是 void，仍然会打印一个返回值，你可以直接忽略它。

- 即使返回值存储在多个寄存器中，也只有第一个寄存器中的值会被记录和打印。举例来说，
  在 x86 架构中，eax 和 edx 用于存储一个 64 位的返回值，低 32 位保存在 eax 中，高
  32 位保存在 edx 中。但是，只有保存在 eax 中的值会被记录和打印。

- 在某些过程调用标准中，例如 arm64 的 AAPCS64，当类型小于一个 GPR 时，由调用者负责
  执行窄化操作，高位可能包含 UNKNOWN 值。因此，对于此类情况检查代码是明智的。例如，
  当在 64 位 GPR 中使用 u8 时，位 [63:8] 可能包含任意值，尤其是在较大类型被截断时
...
```
  ...
  如果你向该函数传入 0x123456789abcdef 并想将其窄化，它可能被记录为 0x123456789abcdef
  而不是 0xef。
  ...
  **案例二**：
  ...
  当传入 0x2_0000_0000 时，返回值可能被记录为 0x2_0000_0000 而不是 0。

```
你可以使用 trace_printk() 在特定函数上添加一些注释。例如，如果你想在 __might_sleep()
函数内部添加注释，只需包含
...
```
你可能会在该跟踪器的以下 "dynamic ftrace"（动态 ftrace）一节中发现其他有用功能，例如
只跟踪特定函数或任务。

### 动态 ftrace


如果设置了 CONFIG_DYNAMIC_FTRACE，在函数跟踪被禁用时，系统运行的开销几乎为零。其
工作原理是，mcount 函数调用（位于每个内核函数的开头，由 gcc 的 -pg 开关生成）一开始
指向一个简单的返回。（启用 FTRACE 会在内核编译中包含 -pg 开关。）

在编译时，每个 C 文件目标都会经过 recordmcount 程序（位于 scripts 目录）。该程序会
解析 C 目标中的 ELF 头，以找到 .text 段中所有调用 mcount 的位置。从 gcc 4.6 版本开始，
x86 增加了 -mfentry，它调用 "__fentry__" 而不是 "mcount"。它在栈帧创建之前调用。

注意，并非所有段都会被跟踪。它们可能会被 notrace 阻止，或以其他方式被阻止，并且所有
内联函数都不会被跟踪。查看 "available_filter_functions" 文件以了解哪些函数可以被跟踪。

会创建一个名为 "__mcount_loc" 的段，其中包含对 .text 段中所有 mcount/fentry 调用站点
的引用。recordmcount 程序将这个段重新链接回原始目标。内核的最终链接阶段会将所有
这些引用添加到一个单独的表中。

在启动时，在 SMP 初始化之前，动态 ftrace 代码会扫描此表并将所有位置更新为 nop。它
还会记录这些位置，并将它们添加到 available_filter_functions 列表中。模块在加载时、
执行前被处理。当卸载模块时，它也会将其函数从 ftrace 函数列表中移除。这在模块卸载代码
中是自动完成的，模块作者无需为此担心。

启用跟踪时，修改函数跟踪点的过程依赖于架构。旧方法是使用 kstop_machine 来防止与正在
执行被修改代码的 CPU 发生竞争（这可能导致 CPU 做出不希望的事情，特别是如果修改后的代码
跨越了缓存（或页）边界），并将 nop 打补丁回调用。但这一次，它们调用的不再是 mcount
（那只是一个函数桩）。它们现在调用进入 ftrace 基础设施。

修改函数跟踪点的新方法是：在要修改的位置放置一个断点，同步所有 CPU，修改断点未覆盖的
指令其余部分。再次同步所有 CPU，然后用完成的版本（指向 ftrace 调用站点）移除断点。

某些架构甚至不需要折腾同步，可以直接将新代码覆盖在旧代码之上，而不会出现其他 CPU
同时执行它的问题。

记录被跟踪函数的一个特殊副作用是，我们现在可以有选择地选择要跟踪哪些函数，以及希望
mcount 调用保持为 nop 的哪些函数。
...
也许这还不够。过滤器还允许 glob(7) 匹配。
  ...
  `**<match>**`
	匹配包含 <match> 的函数
  `<match1>*<match2>`
	匹配以 <match1> 开头并以 <match2> 结尾的函数

      最好使用引号将通配符括起来，否则 shell 可能会将参数展开为本地目录中的文件名。
...
```
这是因为 '>' 和 '>>' 的行为与在 bash 中完全一样。要重写过滤器，使用 '>'；要追加到
过滤器，使用 '>>'。
...
也可以使用一个索引来启用函数。这在一次性设置数千个特定函数时很有用。通过传入一个数字
...
虽然上面解释的内容同时涉及函数跟踪器和函数图跟踪器，但函数图跟踪器中只有一些特殊
功能可用。
...
注意，proc sysctl ftrace_enable 是函数跟踪器的一个总开/关开关。默认情况下它是启用的
...
- mod:
  该命令按模块启用函数过滤。参数定义模块。例如，如果只想要 ext3 模块中的 write*
  函数，运行：
...
```
- traceon/traceoff:
  这些命令在命中指定函数时打开和关闭跟踪。参数决定跟踪系统被打开和关闭的次数。如果
  未指定，则没有限制。例如，要在出现 schedule bug 时禁用跟踪
  ...
  无论是否追加到 set_ftrace_filter，这些命令都是累积的。要移除一个命令，在其前加 '!'
  并去掉参数：

   echo '!__schedule_bug:traceoff:0' > set_ftrace_filter

  上面移除了带有计数器的 __schedule_bug 的 traceoff 命令。要移除不带计数器的命令：
...
- enable_event/disable_event:
  ...
  时，跟踪点会被激活，但以"软"模式禁用。也就是说，跟踪点会被调用，但只是不会被跟踪。
...
- cpudump:
  命中该函数时，它会将当前 CPU 的 ftrace 环形缓冲区内容转储到控制台。与 "dump" 命令
...
trace_pipe 输出与 trace 文件相同的内容，但对跟踪的影响不同。每次从 trace_pipe 读取
...
```
注意，读取 trace_pipe 文件会阻塞，直到有更多输入加入。这与 trace 文件相反。如果有
任何进程打开了 trace 文件进行读取，它实际上会禁用跟踪并阻止添加新条目。trace_pipe 文件
没有此限制。
...
缓冲区的大小。列出的数字是每个 CPU 可以记录的条目数。要知道完整大小，将可能的 CPU 数量
乘以条目数。
...
```
要修改缓冲区，只需 echo 一个数字（以 1024 字节为单位）。
...
```
写入顶层的 buffer_size_kb 会将所有缓冲区重置为相同大小。

### 快照


CONFIG_TRACER_SNAPSHOT 为所有非延迟跟踪器提供一个通用的快照功能。（记录最大延迟的
延迟跟踪器，例如 "irqsoff" 或 "wakeup"，不能使用此功能，因为它们已经在内部使用了
快照机制。）

快照在某一时刻保留当前的跟踪缓冲区，而不停止跟踪。ftrace 将当前缓冲区与一个备用缓冲
...
"tracing" 目录中以下与 tracefs 相关的文件与此功能有关：
  ...
  snapshot:
	...
	该文件用于拍摄快照并读取快照的输出。向该文件 echo 1 以分配一个备用缓冲区并
	拍摄快照（交换），然后以与 "trace" 相同的格式（在上面 "文件系统" 一节中描述）
...
在 tracefs 的 tracing 目录中，有一个名为 "instances" 的目录。可以使用 mkdir 在该目录
...
```
如你所见，新目录看起来与 tracing 目录本身相似。实际上它非常相似，只是缓冲区与事件与主
目录或创建的任何其他实例无关。

新目录中的文件与 tracing 目录中同名的文件工作方式相同，只是使用的缓冲区是一个独立的新
缓冲区。这些文件影响该缓冲区，但不会影响主缓冲区（trace_options 除外）。当前，
trace_options 对所有实例和顶层缓冲区的影响相同，但这在未来版本中可能会改变。也就是说，
选项可能会变成特定于它们所在的实例。

注意，那里没有任何函数跟踪器文件，也没有 current_tracer 和 available_tracers。这是
因为缓冲区目前只能为它们启用事件。
...
执行检查，那将非常有用。由于 ftrace 提供了函数跟踪器，使得在每次函数调用时检查栈大小
变得方便。这通过栈跟踪器启用。

CONFIG_STACK_TRACER 启用 ftrace 的栈跟踪功能。要启用它，向 /proc/sys/kernel/stack_tracer_enabled 写入 '1'。
...
```
你也可以在内核命令行上启用它，以跟踪内核在启动期间的栈大小，方法是向内核命令行参数
添加 "stacktrace"。
...
```
注意，如果 gcc 使用了 -mfentry，函数会在设置栈帧之前被跟踪。这意味着当使用 -mfentry 时，
叶子级函数不会被栈跟踪器测试。

目前，-mfentry 仅由 x86 上 gcc 4.6.0 及以上版本使用。
