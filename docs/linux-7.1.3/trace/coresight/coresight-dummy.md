...
Qualcomm 平台上的 CoreSight TPDM。对于这些设备，需要一个虚拟驱动程将它们注册为 Coresight 设备。该模块也可用于定义可能没有任何编程接口组件，从而可以在驱动程序中创建路径。它为虚拟设备上的操作提Coresight
API，例如启用和禁用它们。它还提供用于调试的 Coresight 虚拟 sink/source
路径
### 配置详情


有两种类型的节点：虚sink 和虚source。这些节点位`/sys/bus/coresight/devices`
```
