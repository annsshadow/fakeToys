...
一remote 必须提供一组回调函`struct trace_remote_callbacks`，其描述见下文。这些回调允Tracefs 启用和禁用追踪与事件、加载和卸载追踪缓冲区（一组环形缓冲区），以及与头页交换一个读取器页，从而实现消费式读取
一旦注册，remote 的一个实例就会出现在 Tracefs 目录 **remotes/** 下。然后可以使用常规的 Tracefs 文件 **trace_pipe** **trace** 来读取缓冲区
## 声明一remote 事件

提供了一些宏来简remote 事件的声明，其方式与内核内事件类似。声明必须提ID、事件参数的描述以及事件的打印方式：
...
然后必须C 文件中使用以下内容声明这些事件：

	#define REMOTE_EVENT_INCLUDE_FILE foo_events.h
	#include <trace/define_remote_events.h>

这会提供一`struct remote_event remote_event_foo`，可以把它传`trace_remote_register`
