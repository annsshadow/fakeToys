...
形式上，一个确定性自动机（记G）被定义为一个五元组
        **G** = { **X**, **E**, **f**, x\ `0`, X\ `m` }

其中
- **X** 是状态的集合- **E** 是事件的有限集合- x\ `0` 是初始状态；
- X\ `m`*X** 的子集）是标记（或最终）状态的集合- **f** : **X** x **E** -> **X** 是转移函数。它定义了在状**X** 中发生来**E** 的事件时的状态转移。在确定性自动机的特殊情况下，在 **X** 中的某个状态下发生 **E** 中的事件，会确定性地得到 **X** 中的下一个状态
例如，一个称'wip'（wakeup in preemptive，抢占式唤醒）的给定自动机可以定义为
...
这种形式化定义的好处之一是它可以用多种格式呈现。例如，使用*操作系统**从业者非常直观的、由顶点（节点）和边组成*图形表示**，且没有任何信息损失
```
...
```
这种 DOT 格式可以使用 dot 工具转换为位图或矢量图像，或使用 graph-easy 转换ASCII art。对```
...
dot2c 是一个工具，可以解析包含如上例所示自动机.dot 文件，并自动将其转换[^3^] 中介绍的 C 表示
例如，将前面'wip' 模型放入名为 'wip.dot' 的文件中，以下命令将.dot 文件转换C
```

  $ dot2c wip.dot > wip.h

```
'wip.h' 的内容就'Deterministic Automaton in C' 一节中的代码示例
### Remarks


自动机形式化允许以多种格式对离散事件系统（DES）建模，以适应不同的应用户
