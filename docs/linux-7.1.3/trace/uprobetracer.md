...
kprobe-event 跟踪器类似，这不需要通过 current_tracer 激活。取而代之的是，通过
...
你也可以使用 /sys/kernel/tracing/dynamic_events 代替 uprobe_events。该接口还将其他动态事件提供统一访问
### uprobe_tracer 语法
...
fetch-args 支持多种类型。Uprobe 跟踪器将按给定类型访问内存。前缀 's' 'u' 分别
表示这些类型是有符号和无符号的x' 前缀表示它是无符号的。被跟踪的参数以十进s' 'u'）或十六进制x'）显示。在没有类型转换的情况下，根据架构使'x32'
'x64'（例x86-32 使用 x32，x86-64 使用 x64）字符串类型是一种特殊类型，它从用户空间获取一null 结尾"的字符串位域是另一种特殊类型，它接3 个参数：位宽、位
```
...
你可以通过 /sys/kernel/tracing/uprobe_profile 检查每个事件的探测命中总数。第一是文件名，第二列是事件名，第三列是探测命中次数
### 使用示例
...
```
0x46420 是对/bin/zsh zfree 的偏移量，该对象被加载到
```
...
```
输出显示，uprobe pid 24842 触发，ip 0x446420，ax 寄存器内容为 79。uretprobe
被触发时 ip 0x446540，对应的函数入口0x446420