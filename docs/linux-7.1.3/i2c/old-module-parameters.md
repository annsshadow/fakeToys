## 旧内核中从用户空间控I2C 设备驱动绑定
   ...
   注意：本节仅在你处理在内2.6 中发现的一些旧代码时相关。如果你使用的是较新的内核，可以安全地跳过本节
...
```
当然，重要的是在加载驱动之前实例`dummy` 设备。dummy 设备将由 i2c-core 自身处理，从而阻止其他驱动稍后绑定到它。如果问题地址处有一个真实设备，并且你希望另一个驱动绑定到它，那么只需传入相关设备的名称而不`dummy`