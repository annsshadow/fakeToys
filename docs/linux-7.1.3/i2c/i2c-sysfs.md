...
内核MUX 通道抽象为逻辑 I2C 总线编号。然而，I2C 总线物理编号MUX
拓扑映射到逻辑 I2C 总线编号之间存在知识鸿沟。本文档旨在填补这一鸿沟，使
读者（例如硬件工程师和新的软件开发人员）能够通过了解物理 I2C
拓扑并在 Linux shell 中浏I2C sysfs，掌握内核中逻辑 I2C 总线的概念。这一知识
对于使用 `i2c-tools` 进行开发和调试十分有用且必不可少
### 目标读

需要使Linux shell 与运Linux 的系统上I2C 子系统进行交互的人员
### 前提条件


1. 了解 Linux shell 文件系统命令和操作的一般知识
2. 了解 I2C、I2C MUX I2C 拓扑的一般知识
## I2C Sysfs 的位

通常，Linux Sysfs 文件系统挂载`/sys` 目录下，因此可以`/sys/bus/i2c/devices` 下找I2C Sysfs，你可以直接 `cd` 到该目录该目录下有一系列符号链接。以 `i2c-` 开头的链接I2C 总线，可能是物理的，
也可能是逻辑的。其他以数字开头并以数字结尾的链接I2C 设备，其中第一个数字是
I2C 总线编号，第二个数字I2C 地址
```
...
```
`i2c-2` 是编号为 2 I2C 总线，`2-0049` 是总线 2 上地址0x49、已绑定内核驱动I2C 设备
## 术语
...
运行 Linux 内核的硬件系统可能拥有多个物I2C 总线控制器。这些控制器是硬件且
物理的，系统可能在内存空间中定义多个寄存器来操作这些控制器。Linux 内核在源目录 `drivers/i2c/busses` 下有 I2C 总线驱动，用于将内核 I2C API 转换为针对不系统的寄存器操作。此术语并不局限于 Linux 内核
### I2C 总线物理编号


对于每个物理 I2C 总线控制器，系统厂商可能会为每个控制器分配一个物理编号例如，具有最低寄存器地址的第一I2C 总线控制器可能被称为 `I2C-0`
### 逻辑 I2C 总线


你在 Linux I2C Sysfs 中看到的每个 I2C 总线编号都是一个被分配了编号的逻辑 I2C
总线。这类似于软件代码通常编写在虚拟内存空间之上，而非物理内存空间
每个逻辑 I2C 总线可能是某个物I2C 总线控制器的抽象，也可能是某I2C MUX
之后的通道的抽象。如果它MUX 通道的抽象，那么每当我们通过此类逻辑总线访问
I2C 设备时，内核会作为抽象的一部分为你I2C MUX 切换到相应的通道
### 物理 I2C 总线


如果逻辑 I2C 总线是某个物I2C 总线控制器的直接抽象，我们称之为物理 I2C 总线
### 注意事项


对于只了解电路板物理 I2C 设计的人来说，这部分可能令人困惑。实际上，在设备源文件（DTS）的 `aliases` 段下，可以将 I2C 总线物理编号重命名为逻辑 I2C 总线
级别上的不同编号。相DTS 文件示例参见
`arch/arm/boot/dts/nuvoton-npcm730-gsj.dts`銆。
最佳实践：**（针对内核软件开发人员）** 最好让 I2C 总线物理编号与其对应的逻辑 I2C
总线编号保持一致，而不是重命名或映射它们，这样可以减少给其他用户的困惑。这物理 I2C 总线可以作为 I2C MUX 扇出的良好起点。在后续示例中，我们将假设物I2C
总线的编号与I2C 总线物理编号相同
## 遍历逻辑 I2C 总线


以下内容将使用一个更复杂I2C 拓扑作为示例。下面是I2C 拓扑的简要图示。如你第一眼没看懂，不必担心，继续阅读本文档，读完后再回看即可
```
...
区分物理 I2C 总线和逻辑 I2C 总线的一个简单方法，是使`ls -l` `readlink`
命令读取 I2C 总线目录下名`device` 的符号链接
另一个可检查的符号链接`mux_device`。该链接只存在于从另一I2C 总线扇出逻辑 I2C 总线目录中。读取此链接还能告诉你是哪个 I2C MUX 设备创建了这个逻辑 I2C
总线
如果符号链接指向`.i2c` 结尾的目录，那么它应该是一个物I2C 总线，直接抽象了
某个物理 I2C 总线控制器。例如：
```
...
```
在此例中，`i2c-7` 是一条物I2C 总线，因此其目录下没`mux_device` 符号链接如果内核软件开发人员遵循不重命名物I2C 总线的惯例，这也意味着它对应系统中编号7 的物I2C 总线控制器
另一方面，如果符号链接指向另一I2C 总线，则当前目录所表示I2C 总线必定是一逻辑总线。该链接指向I2C 总线是父总线，可能是物理 I2C 总线，也可能是逻辑 I2C
总线。在这种情况下，当前目录所表示I2C 总线抽象的是父总线下的某个 I2C MUX 通道
```
...
```
`i2c-73` 是由 `i2c-7` 下某I2C MUX 扇出的逻辑总线，该 MUX I2C 地址0x71。每当我们访问总线 73 上的某个 I2C 设备时，内核总会作为抽象的一部分，把地址0x71 I2C MUX 为你切换到相应通道
### 查找逻辑 I2C 总线编号


本节将描述如何基于物理硬I2C 拓扑的知识，找出表示特定 I2C MUX 通道的逻辑 I2C
总线编号
在此示例中，我们有一个系统，其物I2C 总线 7 DTS 中未被重命名。该总线上有一地址0x71 4 通道 MUX。在 0x71 这个 MUX 的通道 1 之后，还有一个地址0x72 8 通道 MUX。让我们浏览 Sysfs，找0x72 MUX 的通道 3 的逻辑 I2C 总线编号
```
...
```
我们发现 `i2c-7` 0x71 MUX 的通道 1 被分配了逻辑 I2C 总线编号 73```
...
```
无论哪种方式，最终都会进`i2c-73` 的目录。类似地，我们现在可以找0x72 MUX
及其对应的逻辑 I2C 总线编号```
...
```
在这里，我们得知 0x72 MUX 的通道 3 的逻辑 I2C 总线编号81。之后我们可以用这个
编号切换到它自己I2C Sysfs 目录，或发出 `i2c-tools` 命令
提示：一旦你理解了带 MUX I2C 拓扑，如果你的系统上可用，`I2C Tools
...
如果DTS 中未指定，当 I2C MUX 驱动被应用且 MUX 设备成功 probe 时，内核将基当前最大的逻辑总线编号，递增地为 MUX 通道分配逻辑总线编号。例如，如果系统`i2c-15` 是最高的逻辑总线编号，且一4 通道 MUX 被成功应用，那么 MUX 通道 0 获得 `i2c-16`，一直到 MUX 通道 3 获得 `i2c-19`
内核软件开发人员能够在 DTS 中将扇出MUX 通道固定到静态的逻辑 I2C 总线编号。本文档
不会深入讲解如何DTS 中实现这一点，但我们可以在以下示例中看到：
`arch/arm/boot/dts/aspeed-bmc-facebook-wedge400.dts`

在上述示例中，物I2C 总线 2 上有一个地址0x70 8 通道 I2C MUX。该 MUX 通道 2 DTS 中被定义`imux18`，并通过 `aliases` 段中`i2c18 = &imux18;` 固定到逻辑 I2C 总线编号 18
更进一步，可以设计一套便于人类记忆或通过算术计算得出的逻辑 I2C 总线编号方案。例如，
我们可以将总线 3 MUX 的扇出通道固定为从 30 开始。于30 将是总线 3 MUX 通道 0
的逻辑总线编号，37 将是总线 3 MUX 通道 7 的逻辑总线编号
## I2C 设备


在之前的章节中，我们主要介绍的是 I2C 总线。本节让我们看看从链接名`${bus}-${addr}`
格式I2C 设备目录中可以了解到什么。名称中`${bus}` 部分是逻辑 I2C 总线的十进制
编号，`${addr}` 部分是每个设I2C 地址的十六进制编号
### I2C 设备目录内容


在每I2C 设备目录内部，有一个名`name` 的文件。该文件说明内核驱动用来
匹配该设备的设备名称是什么：
```
...
```
有一个名`driver` 的符号链接，用于说明使用了哪Linux 内核驱动```
...
```
但如`driver` 链接一开始就不存在，则可能意味着内核驱动由于
以下原因未能成功 probe 该设备：
```
...
```
根据 I2C 设备的不同以及用probe 该设备的内核驱动的不同，设备目录中的内容也可不同
### I2C MUX 设备
...
I2C 传感器设备也很常见。如果它们被某个内核 hwmon（硬件监控）驱动成功绑定，你I2C 设备目录中看到一个名`hwmon` 的目录。继续深入，你会找到 Hwmon
sysfs 接口```
...
### I2C Sysfs 中实例化 I2C 设备


请参instantiating-devices.rst 中的“方4：从用户空间实例化”一节