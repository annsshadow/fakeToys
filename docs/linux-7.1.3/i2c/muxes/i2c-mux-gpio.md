...
```
I2C 总线SCL/SDA 根据 GPIO 引脚 1..N 的设置被多路复用到总线1..M
### 用法


i2c-mux-gpio 使用 platform 总线，因此你需要提供一struct platform_deviceplatform_data 指向一struct i2c_mux_gpio_platform_data，其中包含主总线I2C adapter 编号、要创建的总线段数量，以及用于控制它的 GPIO 引脚。详情请参见
include/linux/platform_data/i2c-mux-gpio.h銆。
例如，为一个提4 个总线段的 MUX，可以这样写
```
...
ID，platform 核心会为你的设备分配一个动ID。如果你在注册时不知道绝对的
GPIO 引脚编号，这甚至是唯一的选择