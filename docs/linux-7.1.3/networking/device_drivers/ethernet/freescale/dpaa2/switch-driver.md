...
DPAA2 交换机驱动在 Datapath Switch（DPSW）对象上探测（probe），该对象可在以DPAA2 SoC 及其变体上实例化：LS2088A LX2160A
...
dpaa2-switch 驱动fsl-mc 总线上发现的 DPSW 设备上探测。这些设备既可以通过启动时配置文件——DataPath Layout（DPL）——静态创建，也可以在运行时使DPAA2 对象 API（已集成restool 用户空间工具中）创建
...
```
除了实际 DPSW 对象的配置外，dpaa2-switch 驱动还需要以DPAA2 对象
 - 1 DPMCP - 任何MC 固件的交互都需要一Management Command Portal 对象
 - 1 DPBP - 一Buffer Pool 用于为控制接口上 Rx 路径准备的缓冲区播种
 - 需要访问至少一DPIO 对象（Software Portal）才能对控制接口队列执行任何入队/出队操作。DPIO 对象将被共享，无需私有的
## 交换特

驱动支持在硬件中配置 L2 转发规则，用于端口桥接以及独立交换机接口的独立使用
硬件VLAN 感知方面不可配置，因此任DPAA2
```
...
```
当设`stp_state 1` 时，支持通过 STP 进行拓扑和环路检```

        $ ip link add dev br0 type bridge vlan_filtering 1 stp_state 1

```
支持 L2 FDB 操作（添删除/转储）
可以通过网桥命令在每个交换机端口上独立配HW FDB 学习。当禁用 HW 学习时，会运行一个快速老化（fast age）过程，任何先前学习到的地址都会被移除```
...
DPAA2 交换机能够利ACL 表卸载基于流的包重定向。通过在多个端口间共享单个 ACL 表来支持共享过滤块
支持以下流关键字
...
示例 1：把 eth4 上接收到的、SA 00:01:02:03:04:05 的帧发送到
```
...
DPAA2 交换机仅支持每端口镜像和VLAN 镜像。也支持在共享块中添加镜像过滤器
当使用带802.1q 协议tc-flower 分类器时，只接受 ‘’vlan_id‘关键字。基于任何其他字段的镜像
```
...
```
如果在端口上请求了某VLAN 的镜像过滤器，则VLAN 必须已安装在相关交换机端口上，可以使‘’bridge‘```
...
```
此外，应注意镜像流量将受到与其他任何流量相同的出口限制。这意味着当镜像数据包到达镜像端口时，如果包中发现VLAN 未安装在该端口上，它将被丢弃
