...
Pensando Linux 以太网驱动Copyright(c) 2019 Pensando Systems, Inc
...
要确定系统上是否安装了一个或多个 Pensando PCI 以太网设备，可使```
...
```
如果列出了如上所示的设备，那`ionic.ko` 驱动应该能找到并配置它们以供使用。内核日志中应当有相关条```
...
```
有关 devlink dev info 数据的更多信息，请参`Documentation/networking/devlink/ionic.rst`
## 启用驱动
...
目前提供最基础SR-IOV 支持，可通过设置 sysfs 'sriov_numvfs' 值来启用（如果你的特定固件配置支持）
### XDP


XDP 的支持包含基本功能，外加巨型帧、Redirect `ndo_xmit`。目前不支持零拷贝套接字或硬件卸载
## 通过辅助设备RDMA 支持


当固件声明支持时，ionic 驱动通过 Linux 辅助设备框架支持 RDMA（Remote Direct Memory Access，远程直接内存访问）功能。RDMA 能力在设备初始化期间被检测到，如果受支持，以太网驱动将创建一个辅助设备，允许 RDMA 驱动绑定并提InfiniBand/RoCE 功能
## 统计信息
...
命令 `netstat -i`、`ip -s link show` `ifconfig` 显示
```
