...
如果列出了上述设备，则 `pds_core.ko` 驱动应能找到并配置它们以供使用。内核日志中应有如下条目
...
`pds_core` 驱动报告以下版本
   ...
   - - `fw.mainfwa`
     - stored
     - 存储在 mainfwa 槽位中的固件版本
   - - `fw.mainfwb`
     - stored
     - 存储在 mainfwb 槽位中的固件版本
   - - `asic.id`
     - fixed
     - 该设备的 ASIC 类型
   - - `asic.rev`
...
`pds_core` 驱动实现了以下通用参数，用于控制作为 auxiliary_bus 设备提供的功能。
...
`flash` 命令可以更新 DSC 固件。下载的固件将保存到固件 bank 1 或 bank 2 中的任意一个（即当前未使用的那个），
