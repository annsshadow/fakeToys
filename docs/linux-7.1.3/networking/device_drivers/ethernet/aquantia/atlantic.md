...
### Jumbo Frames（巨型帧）
  ...
  该驱动对所有适配器支持 Jumbo Frames。通过将 MTU 更改为大于默认值 1500
  的值来启用 Jumbo Frames 支持。MTU 的最大值为 16000。使用 `ip` 命令：
```
...
### ethtool
  ...
  驱动利用 ethtool 接口进行驱动配置和诊断，以及显示统计信息。此功能需要
  最新版本的 ethtool。
...
```
 ...
 .. note::
    ...
    AQrate 速率（2.5/5 Gb/s）仅会在 linux 内核 > 4.10 时显示。
    但您仍可使用这些速率::
...
```
 ...
 禁用 ntuple 过滤器时，所有用户编程的过滤器都会从驱动缓存和硬件中被刷新。
 重新启用 ntuple 后必须重新添加所有需要的过滤器。

 由于规则的固定顺序，过滤器的位置也是固定的：

 - 位置 0 - 15 用于 VLAN ID 过滤器
 - 位置 16 - 31 用于 L2 EtherType 过滤器
 - 位置 32 - 39 用于 L3/L4 5 元组过滤器（位置 32、36 用于 IPv6）

 L3/L4 5 元组（协议、源和目的 IP 地址、源和目的 TCP/UDP/SCTP 端口）与 8 个
 过滤器进行比较。对于 IPv4，最多可匹配 8 个源地址和目的地址。对于 IPv6，
 最多支持 2 对地址。源端口和目的端口仅对 TCP/UDP/SCTP 数据包进行比较。

 要添加一条将数据包导向队列 5 的过滤器，使用
 ``<-N|-U|--config-nfc|--config-ntuple>`` 开关::
 ...
 设在 32 - 39 之间。
 对于 ``flow-type ip4|udp4|tcp4|sctp4|ip6|udp6|tcp6|sctp6``，您可以为 IPv4
 流量设置 8 条规则，或为 IPv6 流量设置 2 条规则。IPv6 流量的 loc 编号为 32
 和 36。
 目前您不能同时使用 IPv4 和 IPv6 过滤器。
 ...
 VLAN 过滤器（VLAN id）与 16 个过滤器进行比较。
 VLAN id 必须伴随掩码 0xF000。这是为将 VLAN 过滤器与带有 UserPriority 的
 L2 EtherType 过滤器区分开，因为 User Priority 和 VLAN ID 都通过同一个
 'vlan' 参数传入。
 ...
 L2 EtherType 过滤器允许按 EtherType 字段，或同时按 802.1Q 的 EtherType 和
 User Priority（PCP）字段过滤数据包。
 UserPriority（vlan）参数必须伴随掩码 0x1FFF。这是为将 VLAN 过滤器与带有
 UserPriority 的 L2 Ethertype 过滤器区分开，因为 User Priority 和 VLAN ID
 都通过同一个 'vlan' 参数传入。

 要添加一条将优先级 3 的 IP4 数据包导向队列 3 的过滤器::
 ...
 Rx 过滤器是一个将过滤表加载的接口，除非使用 “action” 指定替代队列，否则
 它将所有流汇入队列 0。在这种情况下，任何匹配过滤器条件的流都会被导向相应的
 队列。RX 过滤器在所有 2.6.30 及以后版本的内核上受支持。

```
### UDP 的 RSS

 目前，NIC 不支持对分片 IP 数据包的 RSS，这会导致对分片 UDP 流量的 RSS
 工作不正确。要禁用 UDP 的 RSS，可以使用 RX Flow L3/L4 规则。
...
### UDP GSO 硬件卸载
 ...
 UDP GSO 通过将 UDP 头部分配卸载到硬件，来提升 UDP 发送速率。为此需要特殊的
 用户空间 socket 选项，
```

    udpgso_bench_tx -u -4 -D 10.0.1.1 -s 6300 -S 100

 将导致从单个 6300 字节的用户缓冲区发出 100 字节大小的 UDP 数据包。
...
```
 ...
 DMASystemLoopback:   DMA 主机回环。
 PKTSystemLoopback:   数据包缓冲区主机回环。
 DMANetworkLoopback:  DMA 块上的网络侧回环。
 PHYInternalLoopback: Phy 上的内部回环。
 PHYExternalLoopback: Phy 上的外部回环（使用回环以太网线缆）。
...
======   ==============================================================
0        禁用中断节流。
1        启用中断节流并使用指定的 tx 和 rx 速率。
0xFFFF   自动节流模式。驱动将根据链路速率选择最佳的 RX 和 TX
	 中断节流设置。
======   ==============================================================
...
以微秒计的 TX 侧节流。适配器会将最大中断延迟设置为此值。最小中断延迟为
此值的一半。
...
以微秒计的 RX 侧节流。适配器会将最大中断延迟设置为此值。最小中断延迟为
此值的一半。
...
增大页阶可以改善页复用（在启用 iommu 的系统上尤为明显）。
...
RX 填充阈值。RX 路径在观察到指定数量的空闲描述符之前不会填充已释放的描述符。
较大的值可能有助于更好地复用页，但也可能导致丢包。
...
此特性允许适配器将接收处理分布到多个 CPU 核心上，以防止单个 CPU 核心过载。
...
如果在受支持的内核上使用受支持的适配器，发现已发布源代码存在问题，请将
与该问题相关的具体信息通过电子邮件发送至 aqn_support@marvell.com
