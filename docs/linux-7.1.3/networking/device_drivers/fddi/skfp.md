...
本 README 介绍如何在你的网络适配器上使用 Linux 驱动 'skfp'。

第 2 章：列出本驱动支持的所有网络适配器。
...
从 v2.01 起，该驱动已集成到 linux 内核源码中。因此，安装方式与内核支持的任何其他适配器相同。
...
Reason:
	  ...
	  如果该条目存在，则 FDDI 适配器已被系统找到，应当可以使用。
	  ...
	  一些 COMPAQ 机器在 Linux 下存在 PCI 相关问题。这在 'PCI howto' 文档（包含在某些发行版中，或可从 www 获取，例如 'www.linux.org'）中有描述，目前没有解决办法。

Problem:
	  你想把你的电脑用作多个 IP 子网（使用多个适配器）之间的路由器，但你无法访问其他子网中的计算机。
...
## 5. Function of the Adapter LEDs
	...
	FDDI 网络适配器上 LED 的功能在 SMT 版本 v2.82 中做了变更。在这个新的 SMT 版本中，黄色 LED 用作环运行指示。黄色 LED 点亮表示环已断开。适配器上的绿色 LED 现在用作链路指示，绿色 LED 点亮表示该端口有物理连接。

	在 v2.82 之前的 SMT 版本中，黄色 LED 熄灭表示环正常，而绿色 LED 显示适配器的连接状态。环断开时绿色 LED 熄灭而黄色 LED 点亮。

	所有实现都表明，如果所有 LED 都熄灭，则表示驱动未加载。
