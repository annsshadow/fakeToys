...
要配置一DSA 交换机，需要执行若干命令。本文档将一些常见配置场景作为示例进行讲解：
...
**网关（gateway*
  除一个上游端口外的每个交换机端口都是可配置以太网桥的一部分  上游端口作为一个可配置的独立以太网端口
所有配置都使用来自 iproute2 的工具完成，iproute2 可在
https://www.kernel.org/pub/linux/utils/net/iproute2/ 获取
通过 DSA，交换机的每个端口都像普通的 Linux 以太网接口一样被处理。CPU 端口是连接到以太MAC
芯片的交换机端口。相应的 Linux 以太网接口称conduit 接口（导管接口）。所有其他相应的 Linux
接口称为用户接口
用户接口依赖conduit 接口处于 up 状态才能发送或接收流量。在内核 v5.12 之前，conduit 接口状态必须由用户显式管理。从内核 v5.12 开始，行为如下
- 当一DSA 用户接口被拉起（up）时，conduit 接口会被自动拉起- conduit 接口被关闭（down）时，所DSA 用户接口会被自动关闭
本文档中使用以下以太网接口：
...
配置的情况下，对 incoming outgoing 流量进行标记
**单端*
  .. code-block:: sh
...
add`` `bridge fdb del`` 命令作用于其中一个或两个表里的条目）
直到内核 v4.14，DSA 仅支持使用桥接旁路操作（这些操作不更新软FDB，只更新硬件 FDB）来由用空间管理桥接 FDB 条目，使`self` 标志（该标志是可选的，可以省略）
  .. code-block:: sh
...
由于一bug，DSA 提供的桥接旁FDB 实现没有区分 `static` `local` FDB 条目（`static` 旨在
被转发，`local` 旨在被本地终结，即发往主机端口）。相反，所有带`self` 标志（隐式或显式FDB 条目都被 DSA 当作 `static` 处理，即使它们实际上`local`
  .. code-block:: sh
...
最后一条命令是使用桥接旁路操作DSA 交换机添加静态桥FDB 条目的不正确方式，它碰巧能够工作其他驱动会将同一命令添加FDB 条目当作 `local` 处理，因此不会转发它，这DSA 不同
在内v4.14 v5.14 之间，DSA 并行支持两种向交换机添加桥接 FDB 条目的模式：上文讨论的桥旁路，以及一种使`master` 标志的新模式，该模式也会FDB 条目安装进软件桥
  .. code-block:: sh

    bridge fdb add dev swp0 00:01:02:03:04:05 master static

自内v5.14 起，DSA 获得了与桥接软件 FDB 更强的集成，并且对桥接旁FDB 实现（使`self`
标志）的支持已被移除。这导致了以下变化：
...
给某个特定的 CPU 端口处理。这样，用户空间可以通过根据可用CPU 端口来分散亲和性，实现用户端口
之间静态负载均衡的自定义策略
其次，可以在每个数据包的基础上而不是静态地将用户端口分配给 CPU 端口，从而在 CPU 端口之间执行
负载均衡。这可以通过DSA conduit 置于一LAG 接口（bonding team）下来实现。DSA 监控操作，并在构LAG 从设备的、面向物DSA conduit CPU 端口上创建该软件 LAG 的镜像
为了利用多个 CPU 端口，交换机的固件（设备树）描述必须使用 `ethernet` 引用/phandle 标记所CPU 端口与其 DSA conduit 之间的链接。在启动时，只会使用一个单一CPU 端口DSA conduit —即固件描述中数值上第一个带`ethernet` 属性的端口。由用户来配置系统以使交换机使用其他 conduit
DSA 使用 `rtnl_link_ops` 机制（带"dsa" `kind`）来允许更改用户端口DSA conduit。`IFLA_DSA_CONDUIT`
u32 netlink 属性包含处理每个用户设备的 conduit 设备ifindex。DSA conduit 必须是一个基于固节点信息的有效候选，或者一个只包含有效候选作为从设备LAG 接口
...
注意，在 CPU 端口位于 LAG 之下的情况下，使`IFLA_DSA_CONDUIT` netlink 属性并非严格需要，相反DSA 会对其当conduit（`eth0`）的 `IFLA_MASTER` 属性变更做出反应，并将所有用户端口迁移到 `eth0`
的新上层 `bond0`。类似地，当使用 `RTM_DELLINK` 销`bond0` 时，DSA 会将其分配的用户端口迁移基于固件描述符合条件的第一个物DSA conduit（它实际上会回退到启动配置）
因此，在具有超过 2 个物CPU 端口的设置中，可以将静态的用户CPU 端口分配DSA conduit 之间LAG 混合使用。不可能将用户端口静态分配给具有任何上层接口（这包括 LAG 设备——此conduit 必须
始终是该 LAG）的 DSA conduit
允许在运行时更改用户端口DSA conduit（以CPU 端口）亲和性，以允许根据流量进行动态重新分配
物理 DSA conduit 可以随时加入和离开用作 DSA conduit LAG 接口；但是，除非LAG 接口至少有一物理 DSA conduit 作为从设备，否则 DSA 会拒绝将其作DSA conduit 的有效候选