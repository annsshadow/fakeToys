...
你为设备获取的任何硬件层地址都应经过校验。例如，对于以太网，可使用 linux/etherdevice.h:is_valid_ether_addr() 进行校验。
...
在调用 ndo_stop 例程之后，硬件不得再接收或发送任何数据。所有在途的数据包都必须被中止。如有必要，轮询或等待任何复位命令完成。
...
ndo_start_xmit 方法在任何正常情况下都不得返回 NETDEV_TX_BUSY。除非你的设备无法提前获知其发送功能何时会变得繁忙，否则这被视为一个严重错误。

相反，它必须正确地维护队列。例如，对于实现了分散/聚集（scatter-gather）的驱动来说，这意味着：
...
不要忘记，一旦你的 ndo_start_xmit 方法返回 NETDEV_TX_OK，释放该 SKB 就是你的驱动的责任，并且必须在有限的时间内完成。

例如，这意味着如果你的 TX 缓解（mitigation）方案在没有任何新的 TX 数据包发送时，不允许让 TX 数据包永远“滞留”在 TX 环中而未被回收。此错误可能导致正在等待发送缓冲区空间释放的套接字发生死锁。
