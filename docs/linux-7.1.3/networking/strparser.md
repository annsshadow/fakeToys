...
在接收回调模式下，strparser TCP 套接字的 data_ready 回调中被调用。消息在套接字上收到时即被解析并投递
在通用模式下，一系列 skb 从外部来源喂strparser。消息在该序列被处理时被解析并投递。此模式允许 strparser 应用于任意的数据流
## 接口


API 包括一个上下文结构体、一组回调、实用函数，以及用于接收回调模式data_ready 函数。这些回调包括一parse_msg 函数（在解析时被调用，例KCM 中的 BPF 解析）和一rcv_msg 函数（在一条完整消息完成时调用）
## 函数
...
每个流解析器实例都维护着各种计数器。这些计数器位于 strp_stats 结构体中。strp_aggr_stats 是一个便于为多个流解析器实例累计统计信息的结构体。save_strp_stats aggregate_strp_stats 是用于保存和聚合统计信息的辅助函数
## 消息组装限制


流解析器提供了限制消息组装所消耗资源的机制
当开始组装一条新消息时会设置一个定时器。在接收回调模式下，消息超时取自关联 TCP 套接字的 rcvtime。在通用模式下，超时作为 strp_process 的参数传入。如果定时器在组装完成之前触发，则流解析器被中止，并且在接收回调模式下会TCP 套接字上设置 ETIMEDOUT 错误
在接收回调模式下，消息长度限制为关联 TCP 套接字的接收缓冲区大小。如parse_msg 返回的长度大于套接字缓冲区大小，则流解析器被中止，并TCP 套接字上设置 EMSGSIZE 错误。注意，这使得带有流解析器的套接字的最大接skbuff 大小TCP 套接字的 2*sk_rcvbuf
