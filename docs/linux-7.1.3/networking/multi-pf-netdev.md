...
多 PF NIC 技术使多路服务器中的多个 CPU 能够通过各自的专用 PCIe 接口直接连接到
网络，既可以通过在两张卡之间分割 PCIe 通道的连接线束，也可以通过为单张卡分叉
PCIe 插槽来实现。这消除了跨插槽内部总线传输的网络流量，显著降低了开销和延迟，
同时减少了 CPU 利用率并提高了网络吞吐量。

## 概述


该特性支持在多 PF 环境中将同一端口的多个 PF 合并到一个 netdev 实例之下。它实现于
netdev 层。底层实例（如 pci 功能、sysfs 条目和 devlink）保持独立。
通过不同的设备（属于不同的 NUMA 插槽）传输流量，可以节省跨 NUMA 流量，并允许
运行于同一 netdev 上、来自不同 NUMA 的应用程序仍然感受到与设备的邻近性，从而获得
改善的性能。

## mlx5 实现


mlx5 中的多 PF 或 Socket-direct 是通过将属于同一 NIC 且启用了 socket-direct 属性的
PF 分组在一起来实现的，一旦所有 PF 都被探测到，我们就创建一个单一的 netdev 来
代表它们全部；对称地，当任何一个 PF 被移除时，我们销毁该 netdev。

netdev 的网络通道被分配到所有设备之间，正确的配置会在处理某个应用/CPU 时使用
正确的邻近 NUMA 节点。

我们选择一个 PF 作为主 PF（领导者），它承担特殊角色。其它设备（从属）在芯片层面
与网络断开（设为静默模式）。在静默模式下，从属 PF 之间没有南 <-> 北流量直接流过。
它需要在领导者 PF（东 <-> 西流量）的协助下才能工作。所有 Rx/Tx 流量都通过主 PF
向从属 PF 导向或从中流出。

目前，我们将支持限制为仅 PF，且最多两个 PF（插槽）。
...
```
我们倾向于轮询的原因是，它较少受到通道数量变化的影响。通道索引与 PF 之间的映射
是固定的，无论用户配置了多少通道。由于通道统计在通道关闭期间是持久的，每次都
改变映射会使累积统计不能很好地代表通道的历史。

这是通过在每个通道中使用正确的核心设备实例（mdev），而不是全部使用 "priv->mdev"
下的同一实例来实现的。
...
从属 PF 被设为"静默"模式，意味着它们与网络断开。

在 Rx 中，流导向表仅属于主 PF，由其负责通过跨 vhca 流导向能力将 incoming 流量
分发到其它 PF。仍然维护一个单一的默认 RSS 表，它能够指向不同 PF 的接收队列。

在 Tx 中，主 PF 创建一个新的 Tx 流表，由从属 PF 别名引用，以便它们可以通过它
出去到网络。

此外，我们设置默认的 XPS 配置，它基于 CPU 选择属于与该 CPU 同一节点的 PF 的 SQ。
...
多 PF 的本质是不同通道与不同 PF 配合工作，这与状态维护在其中一个 PF 中的有状态
功能相冲突。例如，在 TLS 设备卸载功能中，会为每个连接创建特殊的上下文对象并维护
在 PF 中。在不同 RQ/SQ 之间切换会破坏该功能。因此，我们暂时禁用了这种组合。
