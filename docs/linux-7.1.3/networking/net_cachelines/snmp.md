...
## netns_ipv4 枚举快速路径使用情况分解

本文档拆解网络命名空间 netns_ipv4 中 SNMP 计数（LINUX_MIB_*）在发送与接收快速路径的缓存行访问特性，标明各计数器被哪些内核函数读写以及其 write_mostly/read_mostly 缓存亲和性，供网络性能分析与缓存行布局优化参考。
