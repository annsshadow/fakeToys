...
本页以表格形式列出网络命名空间（netns）中 IPv4 相关 sysctl 参数在结构体 netns_ipv4 内的字段布局与缓存行（cacheline）分布，便于理解sysctl 在内存中的热路径归属。字段名与标识符保留英文原文，说明文字已译为中文


## netns_ipv4 结构体快速路径使用情况分
...
u8                              sysctl_tcp_l3mdev_accept                                         读取_mostly         __inet6_lookup_established/inet_请求_bound_dev_
...
u8                              sysctl_tcp_无_ssthresh_metrics_save                                                  TCP_最后_ACK/tcp_(更新/初始_metrics
