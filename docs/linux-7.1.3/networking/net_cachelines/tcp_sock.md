...
## tcp_sock 结构体快速路径使用情况分

本文档分`tcp_sock` 结构体各字段在网络协议栈快速路径（发送与接收）上的访问模式，列出字段类型、名称、读写属性及所涉及TCP 核心函数，供内核网络开发者理解缓存行布局与性能热点
...
__2                        pred_标志              读取_写入          读取_mostly         tcp_select_window(tx);tcp_rcv_established(rx)
u64                           bytes_received                              读取_写入          tcp_rcv_nxt_更新(rx)
u32                           segs_                                    读取_写入          tcp_v6_rcv(rx)
u32                           数据_segs_                               读取_写入          tcp_v6_rcv(rx)
...
结构列出_head              tsq_node
结构列出_head              tsorted_sent_队列      读取_写入                              tcp_更新_skb_之后_send
u32                           snd_wl1                                     读取_mostly         tcp_可_更新_window
u32                           snd_wnd                 读取_mostly         读取_mostly         tcp_wnd_end,tcp_tso_应当_defer(tx);tcp_fast_path_rx)
...
结构minmax                 rtt_min                                     读取_mostly         tcp_min_rtt/tcp_rate_gen,tcp_min_rtttcp_更新_rtt_min
...
结构tcp_选项_received   rx_opt                  读取_mostly         读取_写入          tcp_established_选项(tx);tcp_fast_path_tcp_ack_更新_window,tcp_是_sack,tcp_数据_队列,tcp_rcv_established,tcp_ack_更新_rtt(rx)
...
结构hrtimer                pacing_timer
结构hrtimer                compressed_ack_timer
结构sk_buff*               retransmit_skb_hint     读取_mostly                             tcp_clean_rtx_队列
结构rb_root                out_的_order_队列                          读取_mostly         tcp_数据_队列,tcp_fast_path_check
结构sk_buff*               ooo_最后_skb
结构tcp_sack_块[^1^]      duplicate_sack
结构tcp_sack_块[^4^]      selective_acks
结构tcp_sack_块[^4^]      recv_sack_缓存
结构sk_buff*               highest_sack            读取_写入                              tcp_事件_新_数据_sent
u32                           prior_ssthresh
u32                           high_seq
u32                           retrans_stamp
u32                           undo_marker
...
结构                       rcv_rtt_est                                 读取_写入          tcp_rcv_space_adjust,tcp_rcv_established
结构                       rcvq_space                                  读取_写入          tcp_rcv_space_adjust
结构                       mtu_probe
u32                           plb_rehash
u32                           mtu_info
bool                          是_mptcp
...
bool                          syn_smc
结构tcp_sock_af_ops*       af_特定
结构tcp_md5sig_info*       md5sig_info
结构tcp_fastopen_请求*  fastopen_req
结构请求_sock*          fastopen_rsk
结构saved_syn*             saved_syn
============================= ======================= =================== =================== ==================================================================================================================================================================================================================
