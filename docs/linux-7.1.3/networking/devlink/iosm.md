...
`iosm` 驱动实现了以下驱动特定的参数
   ...
   - - `erase_full_flash`
     - u8
     - runtime
     - erase_full_flash 参数用于检查在固件刷写期间设备是否需要完全擦除       如果设置，将向设备发送完整的 nand 擦除命令。默认情况下       仅启用条件擦除支持
## 闪存更新（Flash Update

`iosm` 驱动实现了使`devlink-flash` 接口进行闪存更新的支持
它支持使用包Bootloader 镜像和其他调制解调器软件镜像的组合闪存镜来更新设备闪存
...
请求的闪存刷写类型。支持的固件镜像类型
    ...
    - - 名称
      - 描述
    - - `PSI RAM`
      - Primary Signed Image（主签名镜像    - - `EBL`
      - External Bootloader（外部引导加载程序）
    - - `FLS`
      - Modem Software Image（调制解调器软件镜像
PSI RAM EBL RAM 镜像，当设备处于 BOOT ROM 阶段时被注入到设备。一旦成功，
实际的调制解调器固件镜像将被刷写到设备。调制解调器软件镜像包含多个文件每个文件有一个安bin 文件以及至少一Loadmap/Region 文件。为了刷写这文件，需要向调制解调器设备发送适当的命令以及刷写所需的数据。诸如区域计数和
每个区域的地址这样的数据必须使devlink param 命令传递给驱动
如果设备需要在固件刷写前被完全擦除，用户应用程序需要使devlink param 命令
...
3) 在调制解调器进入 PSI 阶段后注EBL
$ devlink dev flash pci/0000:02:00.0 file <EBL_File_name>

4) 一EBL 注入成功，就会进行实际的固件刷写。以下是用于每个固件镜像的命   序列
a) 刷写安全 bin 文件
...
如果固件遇到异常，驱动将获取一个快照。以下区域用于访问设备内部数据
    ...
    - - 名称
      - 描述
    - - `report.json`
      - 作为该区域一部分记录的异常详情摘要    - - `coredump.fcd`
      - 该区域包含与设备中发生的异常相关的详情（RAM 转储）    - - `cdd.log`
      - 该区域包含与调制解调CDD 驱动相关的日志    - - `eeprom.bin`
      - 该区域包eeprom 日志    - - `bootcore_trace.bin`
      - 该区域包含当前实例的 bootloader 日志    - - `bootcore_prev_trace.bin`
