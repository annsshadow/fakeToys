...
本文档描Alienware Dell G 系列机型上实现的 AWCC WMI 接口（alienware-wmi 驱动），介绍其通过 Platform Profile API 暴露的散热与超频控制方法，以及由社区逆向工程得到AWCCMethodFunction 工作机制


## 简


WMI 设备 WMAX 已在许多 Alienware Dell G 系列机型上实现。在这些机型中，已识别出两种实现。第一种用于较老的系统，处HDMI、亮度、RGB、放大器与深度睡眠控制。第二种用于较新的系统，主要处理散热控制与超频

我们怀疑后者被 Alienware Command Center（AWCC）用来管理厂商预定义的散热配置（thermal profile）。alienware-wmi 驱动通过 Platform Profile API 暴露 Thermal_Information Thermal_Control 方法，以模拟 AWCC 的行为

这个较新的接口名AWCCMethodFunction，是Dell 没有提供任何官方文档的情况下通过逆向工程得到的。我们将尽力描述其已被发现的内在工作机制
...
WMI 接口描述可以使用 `bmfdec <https://github.com/pali/bmfdec>`_ 工具从嵌入的二进MOF（bmof）数据中解码
...
所有输入参数的类型均为 **uint32**，并且它们在各方法之间的结构非常相似。通常，第一个字节对应于方法执行的特*操作**，随后的字节对应于传给该**操作***参数**。例如，如果某个操作码为 0x01 且需要一ID 0xA0，则你传给该方法的参数为 0xA001
...
+====================+====================================+====================+
| 0x01               | 获取与某个风ID 相关的温度传  | - Byte 1: Fan ID   |
|                    | 器数                            |                    |
+--------------------+------------------------------------+--------------------+
...
|                    | - Byte 1: 温度传感器数          |                    |
|                    | - Byte 2: 未知                     |                    |
|                    | - Byte 3: 散热配置（profile）数 |                    |
+--------------------+------------------------------------+--------------------+
| 0x03               | 在给定索引处列出一ID 或资源  | - Byte 1: Index    |
|                    | 风扇 ID、温ID、未ID 与散  |                    |
|                    | 配置 ID 按该确切顺序列出        |                    |
|                    |                                    |                    |
|                    | 操作 0x02 用于了解哪些索引映射  |                    |
...
|                    |                                    |   ID               |
+--------------------+------------------------------------+--------------------+
| 0x05               | 获取给定风扇的当RPM          | - Byte 1: Fan ID   |
|                    |                                    |                    |
+--------------------+------------------------------------+--------------------+
...
| 0x07               | 未知                            | - Unknown          |
+--------------------+------------------------------------+--------------------+
| 0x08               | 获取给定风扇 ID 的最RPM      | - Byte 1: Fan ID   |
|                    |                                    |                    |
+--------------------+------------------------------------+--------------------+
| 0x09               | 获取给定风扇 ID 的最RPM      | - Byte 1: Fan ID   |
|                    |                                    |                    |
+--------------------+------------------------------------+--------------------+
...
+--------------------+------------------------------------+--------------------+
| 0x0C               | 获取给定风扇 ID 的当`boost` 值。| - Byte 1: Fan ID   |
|                    |                                    |                    |
+--------------------+------------------------------------+--------------------+
...
|                    |                                    |   profile ID       |
+--------------------+------------------------------------+--------------------+
| 0x02               | 为给定风ID 设置 `boost` 值   | - Byte 1: Fan ID   |
|                    |                                    | - Byte 2: Boost    |
+--------------------+------------------------------------+--------------------+
...
如果某型号支User Selectable Thermal Tables（USTT，用户可选散热表）配置，它将不支Legacy 配置，反之亦然

每个型号都支CUSTOMx00）散热配置。在 G 系列笔记本中，GMODE 取代 PERFORMANCE
...
该方法仅存在Dell G 系列笔记本中，其实现意味着 GMODE 散热配置可用，即Thermal_Information 的操0x03 并未列出它

Dell G 系列笔记本上G 键也会改Game Shift 状态，因此二者直接相关
...
带有 AWCC 接口Alienware Dell G 系列设备通常有一个嵌入的 STM32 RGB 灯光控制器，具备 USB/HID 能力。其厂商 ID `187c`，而产ID 可能因型号而异

MCU 的两GPIO 引脚的控制被作为 WMI 方法暴露出来，用于调试目的

+--------------+--------------------------------------------------------------+
| Pin          | Description                                                  |
+==============+===============================+==============================+
| 0            | 设备固件更新（DFU）模式引脚| **HIGH**：下MCU 启动时启DFU 模式|
|              |                               +------------------------------+
|              |                               | **LOW**：下MCU 启动时禁DFU 模式 |
+--------------+-------------------------------+------------------------------+
...
关于MCU 的更多信息请参见致谢部分
   ...
   某些 GPIO 控制方法打破了通常的参数结构，在第一个字节上使用**引脚*而非操作码
