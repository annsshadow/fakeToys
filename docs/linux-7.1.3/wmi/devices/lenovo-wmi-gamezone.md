...
档和超频等高级特性。它与多个事件 GUID 和数据块 GUID 配对，为各种方法提供上下文。
...
Gamezone Data（Gamezone 数据）WMI 接口为属于 Lenovo“游戏系列”的设备提供 platform-profile（平台配置档）和风扇曲线设置。它使用一个通知链（notifier chain）来在
当前平台配置档发生变化时，将该变化告知其他 Lenovo WMI 接口驱动。用户可以通过硬件上
电源或配置档 LED 的颜色（取决于机型）来确定当前设置的配置档。

支持以下平台配置档：
 - low-power（低功耗），蓝色 LED
 - balanced（均衡），白色 LED
 - performance（性能），红色 LED
 - max-power（最大功耗），紫色 LED
 - custom（自定义），紫色 LED

#### 极限模式（Extreme Mode）


一些较新的 Lenovo“游戏系列”笔记本电脑在其 BIOS 中启用了“Extreme Mode（极限模式）”配置档。当可用时，该模式将由 max-power（最大功耗）平台配置档表示。

对于这些设备的一个子集，BIOS 中的“Extreme Mode”配置档不完整，设置它会导致未定义行为。提供了一个 BIOS bug 怪癖（quirk）表，以确保这些设备无法从驱动中设置“Extreme Mode”。

#### 自定义配置档（Custom Profile）


自定义配置档代表 Lenovo 设备上的一种硬件模式，它允许用户修改 Package Power Tracking（PPT，封装功耗追踪）和风扇曲线设置。当要修改由 Other Mode WMI 接口暴露的某个属性时，必须先将 Gamezone 驱动手动切换到“custom（自定义）”配置档，否则设置将不生效。如果从受支持配置档列表中设置了另一个配置档，BIOS 在切换到该配置档时会覆盖任何用户 PPT 设置。
...
Gamezone Thermal Mode Event（Gamezone 散热模式事件）接口会在平台配置档发生变化时通知系统，这种变化可能是由硬件事件（笔记本电脑的 Fn+Q，或 Go 系列的 Legion + Y）触发，也可能是由 Gamezone WMI 接口触发。该事件在 Lenovo WMI Events 驱动（lenovo-wmi-events）中实现。


## WMI 接口描述


WMI 接口描述可以使用 `bmfdec <https://github.com/pali/bmfdec>`_ 工具从内嵌的二进制 MOF（bmof）数据中解码出来：
