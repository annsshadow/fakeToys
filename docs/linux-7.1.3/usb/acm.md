...
隐含担保。有关更多细节，请参GNU 通用公共许可证
您应该已经随本程序收到了一GNU 通用公共许可证；如果没有，请写信给自由软基金会，地址：Inc., 59 Temple Place, Suite 330, Boston, MA 02111-1307 USA
...
我知道有一ISDN TA 可以acm 驱动配合使用
 - 3Com USR ISDN Pro TA

一些手机也通过 USB 连接。我知道以下手机可以工作
 - SonyEricsson K800i

遗憾的是，许多调制解调器和大多数 ISDN TA 使用专有接口，因此无法与该驱动配工作。购买前请确认是否符ACM 规范
```
	usbcore.ko
...
之后，调制解调器应当可被访问。您应当能够使用 minicom、ppp mgetty 来操作它们
#### 2. 验证是否工作


第一步应当检/sys/kernel/debug/usb/devices，其内容应当类似如下
...
这三行（以及 Cls= 'comm' 'data' 类）的存在很重要，它表示这是一ACM 设备Driver=acm 表示 acm 驱动正被用于该设备。如果您只看Cls=ff(vend.)，那么您
```
  D:  Ver= 1.00 Cls=02(comm.) Sub=00 Prot=00 MxPS= 8 #Cfgs=  2
  I:  If#= 0 Alt= 0 #EPs= 1 Cls=02(comm.) Sub=02 Prot=01 Driver=acm
  I:  If#= 1 Alt= 0 #EPs= 2 Cls=0a(data ) Sub=00 Prot=00 Driver=acm
...
如果一切看起来正常，启minicom 并把它设置为ttyACM 设备通信，然后试着输入
