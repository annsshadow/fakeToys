...
如果你正在使Linux 作为嵌入式操作系统编写打印机固件，可以使用本驱动。本驱动与在你的 Linux 主机系统上使用打印机无关

你需要一USB 设备控制器，以及能接受基Linux USB Gadget API gadget / “设备类”驱动的 Linux 驱动。在加载 USB 设备控制器驱动之后，再加载打印机 gadget 驱动。这将在你的 USB 设备端口所连接USB 主机上呈现一个打印机接口

本驱动面向运行在用户模式的打印机固件而设计。用户模式的打印机固件通过设备文件与内核模式的打印gadget 驱动读写数据。当 USB 主机发送设备请求以获取打印机状态时，打印机会返回一个打印机状态字节。用户空间固件可以使用设备文/dev/g_printer 读取或写入该状态字节。阻塞与非阻塞的写调用均受支持
...
iPNPstring
	本打印机使用PNP ID 字符串。你需要通过命令行或
	硬编码方式设置你的打印机产品所使用PNP ID 字符串

qlen
	每个端点使用8k 缓冲区数量。默认值为 10，你应针
