...
通用 NAND 驱动支持几乎所有基于 NAND 和 AG-AND 的芯片，并将它们连接到 Linux 内核的
存储技术设备（MTD）子系统。

本文档面向希望实现适用于 NAND 设备的板级驱动或文件系统驱动的开发者。
...
函数与结构体文档是自动生成的。每个函数和结构体成员都有一段简短描述，以 [XXX]
标识符标记。后续章节将解释这些标识符的含义。

### 函数标识符 [XXX]


函数在简短注释中以 [XXX] 标识符标记。这些标识符说明了函数的用途与作用范围。使用
如下标识符：

- [MTD Interface]

   这些函数提供到 MTD 内核 API 的接口。它们不可替换，提供完全独立于硬件的功能。

- [NAND Interface]

   这些函数被导出，提供到 NAND 内核 API 的接口。
...
- [DEFAULT]
   ...
   默认函数提供适用于大多数实现的硬件相关功能。这些函数如有必要可由板级驱动
   替换。这些函数通过 NAND 芯片描述结构中的指针调用。板级驱动可在调用 nand_scan()
...
结构体成员在注释中以 [XXX] 标识符标记。这些标识符说明了成员的用途与作用范围。使用
如下标识符：

- [INTERN]

   这些成员仅供 NAND 驱动内部使用，不得修改。这些值大多由 nand_scan() 期间评估的
   芯片几何信息计算得出。

- [REPLACEABLE]

   可替换成员保存可由板级驱动提供的硬件相关函数。板级驱动可在调用 nand_scan() 之前
...
对于大多数板子，只需提供基本函数并填写 nand_chip 描述结构中一些真正与板级相关的
成员即可。

### 基本定义

至少你需要提供一个 nand_chip 结构以及用于保存 ioremap 后的芯片地址的存储区。你
可以用 kmalloc 分配 nand_chip 结构，也可以静态分配。NAND 芯片结构内嵌了一个 mtd
结构，该结构会被注册到 MTD 子系统。你可以使用 nand_to_mtd() 辅助函数从 nand_chip
指针中提取出 mtd 结构的指针。
...
硬件控制函数提供对 NAND 芯片控制引脚的访问。访问可以通过 GPIO 引脚或地址线完成。如果
使用地址线，请确保满足时序要求。
...
如果硬件接口的 NAND 芯片 ready busy 引脚连接到了 GPIO 或其他可访问的 I/O 引脚，则使用
此函数来读回该引脚的状态。该函数没有参数，当设备忙时（R/B 引脚为低）应返回 0，当设备
就绪时（R/B 引脚为高）应返回 1。如果硬件接口不能访问 ready busy 引脚，则不得定义该
函数，并且函数指针 this->legacy.dev_ready 应设为 NULL。

### 初始化函数


初始化函数分配内存并设置所有板级相关的参数和函数指针。当一切设置就绪后调用
nand_scan()。该函数尝试检测并识别芯片。如果找到芯片，则所有内部数据字段都会被相应地
...
只有当驱动被编译为模块时才需要退出函数。它释放芯片驱动占用的所有资源，并在 MTD 层
注销分区。
```
...
（去）选择所请求的芯片。函数指针必须在调用 nand_scan() 之前设置好。nand_scan() 的
...
nand 驱动支持三种不同类型的硬件 ECC。

- NAND_ECC_HW3_256

   每 256 字节提供 3 字节 ECC 的硬件 ECC 生成器。

- NAND_ECC_HW3_512

   每 512 字节提供 3 字节 ECC 的硬件 ECC 生成器。

- NAND_ECC_HW6_512

   每 512 字节提供 6 字节 ECC 的硬件 ECC 生成器。

- NAND_ECC_HW8_512

   每 512 字节提供 8 字节 ECC 的硬件 ECC 生成器。
...
- enable_hwecc
   ...
   该函数在对芯片读/写之前调用。在此函数中复位或初始化硬件生成器。调用时会传入一个
   参数，用于区分读操作和写操作。
...
- correct_data
   ...
   发生 ECC 错误时调用此函数进行错误检测与纠正。如果错误可以纠正，则分别返回 1 或 2。
   如果错误不可纠正则返回 -1。如果你的硬件生成器与 nand_ecc 软件生成器的默认算法匹配，
   则使用 nand_ecc 提供的纠正函数，而不要实现重复的代码。

#### 使用校验子计算的硬件 ECC


许多硬件 ECC 实现提供 Reed-Solomon 码，并在读取时计算错误校验子（syndrome）。该校验子
在调用通用 Reed-Solomon 库中的纠错代码之前，必须转换为标准的 Reed-Solomon 校验子。

为了使校验子生成器工作，ECC 字节必须紧接在数据字节之后放置。这与软件 ECC 使用的常规
布局相反。数据区与带外（out of band）区的分离不再可能。nand 驱动代码会处理这种布局，
oob 区中剩余的空闲字节由自动放置（autoplacement）代码管理。这种情况下请提供匹配的 oob
布局。实现参考见 rts_from4.c 和 diskonchip.c。在这些情况下，我们还必须在 FLASH 上使用
坏块表，因为 ECC 布局与坏块标记位置相互冲突。详见坏块表支持一节。

### 坏块表支持


大多数 NAND 芯片在 spare 区的固定位置标记坏块。这些块在任何情况下都不得被擦除，否则
坏块信息会丢失。可以在每次访问块时通过读取块中第一页的 spare 区来检查坏块标记。这很
耗时，因此使用坏块表。
...
nand_scan() 调用 nand_default_bbt() 函数。nand_default_bbt() 根据 nand_scan() 获取的
芯片信息选择适当的默认坏块表描述符。

标准策略是扫描设备以查找坏块，并构建一个基于 RAM 的坏块表，这比始终检查闪存芯片本身
上的坏块信息访问更快。

#### 基于 FLASH 的表


可能期望或必须将坏块表保存在 FLASH 中。对于 AG-AND 芯片这是强制性的，因为它们没有
...
激活基于 FLASH 的坏块表支持最简单的方法，是在调用 nand_scan() 之前，于 nand 芯片结构
的 bbt_option 字段中设置 NAND_BBT_USE_FLASH 选项。对于 AG-AND 芯片，这是默认完成的。这
会激活 NAND 驱动默认的基于 FLASH 的坏块表功能。默认的坏块表选项为：
...
用户自定义表通过填写 nand_bbt_descr 结构，并在调用 nand_scan() 之前将该指针存入 nand_chip
结构的 bbt_td 成员来创建。如果还需要镜像表，则必须创建第二个结构，并将指向该结构的指针
存入 nand_chip 结构内的 bbt_md。如果 bbt_md 成员设为 NULL，则只使用主表，不会扫描镜像表。

nand_bbt_descr 结构中最重要的字段是 options 字段。options 定义了表的大部分属性。使用
...
- 表位置是绝对的
   ...
   使用选项常量 NAND_BBT_ABSPAGE，并在 pages 字段中定义坏块表起始的绝对页号。如果你选择了
   每芯片坏块表并且拥有多芯片阵列，则必须为芯片阵列中的每个芯片给定起始页。注意：不会
   扫描表标识模式，因此 pattern、veroffs、offs、len 字段可以不初始化。

- 表位置自动检测

   表可以位于芯片（设备）的第一个或最后一个好块中。设置 NAND_BBT_LASTBLOCK 将坏块表放置在
   芯片（设备）末尾。坏块表通过存储在持有坏块表的块的第一页 spare 区中的模式来标记和识别。
   在 pattern 字段中存入指向该模式的指针。此外，模式的长度必须存入 len，spare 区中的偏移
   必须在 nand_bbt_descr 结构的 offs 成员中给定。对于镜像坏块表，必须使用不同的模式。
...
- 表版本控制
   ...
   丢失坏块表信息的风险降低到仅丢失关于那一个应标记为坏的磨损块的信息。版本号存储在设备 spare
   区中连续的 4 个字节。版本号的位置由坏块表描述符的 veroffs 成员定义。
...
- 保留块数
   ...
   对于自动放置，必须保留一些块用于坏块表存储。保留块数在坏块表描述结构的 maxblocks 成员中
   定义。为镜像表保留 4 个块应该是一个合理的数字。这也限制了为坏块表标识模式而扫描的块数。
...
默认的放置函数是自动放置。nand 驱动内建了针对各种芯片类型的默认放置方案。如果由于硬件 ECC
...
调用函数提供一个指向 nand_oobinfo 结构的指针，该结构定义 ecc 放置。对于写操作，调用者必须
提供 spare 区缓冲区以及数据缓冲区。spare 区缓冲区大小为（页数）*（spare 区大小）。对于读
操作，缓冲区大小为（页数）*（（spare 区大小）+（每页 ecc 步数）* sizeof (int)）。驱动存储
ecc 检查的结果
```
...
如果 spare 区缓冲区为 NULL，则仅根据 nand_oobinfo 结构中给定的方案进行 ECC 放置。

#### 自动放置


自动放置使用内建默认值将 ecc 字节放置在 spare 区中。如果需要将文件系统数据存储/读取到 spare
区中，则调用函数必须提供一个缓冲区。每页的缓冲区大小由 nand_oobinfo 结构中的 oobfree 数组
决定。

如果 spare 区缓冲区为 NULL，则仅根据默认内建方案进行 ECC 放置。
...
偏移     内容               注释
======== ================== ===================================================
0x00     ECC byte 0         错误纠正码字节 0
0x01     ECC byte 1         错误纠正码字节 1
0x02     ECC byte 2         错误纠正码字节 2
0x03     Autoplace 0
0x04     Autoplace 1
0x05     Bad block marker   如果该字节中任一位为零，则该块为坏块。这只适用于块中
			    的第一页。在其余页中该字节被保留
0x06     Autoplace 2
0x07     Autoplace 3
======== ================== ===================================================
...
偏移        内容               注释
=========== ================== ================================================
0x00        Bad block marker   如果该字节中任一位为零，则该块为坏块。这只适用于块中
			       的第一页。在其余页中该字节被保留
0x01        Reserved           保留
0x02-0x27   Autoplace 0 - 37
0x28        ECC byte 0         本页中第一份 256 字节数据的错误纠正码字节 0
0x29        ECC byte 1         本页中第一份 256 字节数据的错误纠正码字节 1
0x2A        ECC byte 2         本页中第一份 256 字节数据的错误纠正码字节 2
...
文件系统必须了解 NAND 的特性和限制。NAND Flash 的一个主要限制是，你不能想写一页就写一页
地频繁写入。在再次擦除之前，对一页的连续写入被限制为 1-3 次，具体取决于厂商规格。这同样
适用于 spare 区。

因此，支持 NAND 的文件系统必须以页大小块写入，或者持有一个写缓冲区，以收集较小的写入
直到它们累加到页大小。可用的支持 NAND 的文件系统有：JFFS2、YAFFS。

用于存储文件系统数据的 spare 区使用，由前面某章描述的 spare 区放置功能控制。

## 工具


MTD 项目提供了一组有用的工具来处理 NAND Flash。

- flasherase, flasheraseall：擦除并格式化 FLASH 分区

- nandwrite：将文件系统镜像写入 NAND FLASH

- nanddump：转储 NAND FLASH 分区的内容

这些工具了解 NAND 的限制。请使用这些工具，而不是抱怨由不支持 NAND 的访问方法导致的错误。
...
这些常量定义在 rawnand.h 中。它们通过按位或组合在一起
```
    ...
    /* AND 芯片，具有 4 个 bank 以及令人困惑的页/块
     * 分配。更多信息参见 Renesas 数据手册 */
...
```
    ...
    /* 硬件 ecc 生成器在读取时提供校验子而非 ecc 值
     * 这只有在 ecc 字节紧接数据字节之后时才能工作。适用于 DOC 和 AG-AND Renesas 硬件 Reed Solomon 生成器 */
...
本章包含导出的 NAND 内核 API 函数的自动生成文档。每个函数都有一段以 [XXX] 标识符标记的
简短描述。解释请参见“文档说明”一章。
