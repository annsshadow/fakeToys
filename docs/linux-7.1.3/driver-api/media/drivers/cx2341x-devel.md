...
cx2341x 通过 PCI BAR0（基地址寄存0）把其整64M 内存空间暴露PCI 主机。这里的地址是相对于 BAR0 中所持地址的偏移
...
这些寄存器显示用于编码的各个缓冲区区域相关内存位置的偏移，需先左<<1
...
这些寄存器显示用于解码的各个缓冲区区域相关内存位置的偏移，需先左<<1
...
- 0x9008：GPIO 获取输入状
- 0x900c：GPIO 设置输出状
- 0x9020：GPIO 方向（Bit7（GPIO 0..7）—：输入，1：输出）
- 0x9050：SPU 控制
- 0x9054：复位硬件模
- 0x9058：VPU 控制
- 0xA018：Bit6：中断挂起？
...
中断状态寄存器 0x0040 与中断掩0x0048 中各比特的定义。如果在掩码中某比特被清零，则我们希望我们的 ISR 执行
...
固件邮箱可以通过在编码器和解码器内存中搜索一16 字节的签名来定位。该签名会位256 字节边界上
...
固件实现20 个邮箱，每个 20 32 位字。前 10 个保留给 API 调用。后 10 个由固件用于事件通知
...
命令是一32 位枚举值。API 细节可在本章找到

返回值是一32 位枚举值。目前仅定义了两个值：

- 0=success
- -1=command undefined.

共有 16 个参结果 32 位字段。驱动用调用所需的全部参数值填充这些字段。驱动再用调用返回的結果值覆盖这些字段

超时值保护卡免受挂起的驱动线程影响。如果驱动没有在指定的超时内处理完该调用，固件将复位该邮箱

要进行一API 调用，驱动遍历每个邮箱，寻找第一个可用的（bit 0 已被清零）。驱动置位该比特，填入命令枚举值、超时值以及任何所需参数。驱动随后置位参数就绪比特（bit 1）。固件扫描邮箱以寻找待处理命令，处理它们，设置结果码，用该调用的返回值填充结果值数组，并置位调用完成比特（bit 2）。一bit 2 被置位，驱动应取回结果并清除所有标志。如果驱动没有在超时寄存器设定的时间内完成此任务，固件将复位该邮箱

事件通知由固件发送给主机。主机通过一API 调用告诉固件它感兴趣的事件。该调用告诉固件使用哪个通知邮箱。固件通过一个中断向主机发信号。仅使用 16 个结果字段，标志、命令、返回值和超时字不被使用
...
开始捕获视频、音频和/VBI 数据。所有编码参数必须在API 调用之前初始化。持续捕获，直到捕获了预定义数量的帧
...
要停止的流类型，API 0x81 param[^0^]

##### 参数[2]


子类型，API 0x81 param[^1^]
...
例如：IBBPBBPBBPBB --> GOP 大小2，B 帧数+1 = 3
...
设置动态降噪（Dynamic Noise Reduction）工作模
...
这些动态降噪中值滤波器的值仅当相应滤波器被设手动"（见 API 0x9B）时才有意义
...
设置流输出端口。当数据通过 PCI 总线（DMA）复制时通常0，当数据流向另一颗芯片（pvrusb cx88-blackbird）时1
...
获取编码器缓冲区中的序列结束码。当一次捕获开始时仍会产生若干中断，其中最后一个中断的 Result[^0^] 将被置为 1，Result[^1^] 将包含缓冲区的大小
...
Result[^0^] 1，则此处包含最后一个缓冲区的大小，否则未定义
...
每个中断的帧数（最8）。仅raw 模式下有效

##### 参数[2]


raw VBI 总帧数。仅raw 模式下有效
...
仅在 raw 模式下观察到的每个中断帧数。范1 Param[^1^]

##### 结果[1]


raw 模式下观察到的帧数。范1 Param[^2^]
...
DMA 传输块大小（字节或帧）。当单位为字节时，支持的块大小为 2^7^8 2^9 字节
...
结合中断掩码 bit 27 返回前一DMA 传输的信息。使用邮10
...
结合中断掩码 bit 27 bit 18 返回前一DMA 传输的信息。使用邮9
...
与“Vertical Crop Line”相关的一些操

##### 参数[0]


若为 saa7114 raw VBI 捕获60 Hz，则设为 10001
否则0
...
杂项操作。不100% 确定它的作用。它更像是一ioctl 调用。第一个参数是命令号，第二个是值
...
API 的每次调用都会将播放步进到下面定义的、当前播放方向上的下一个单元
...
DMA 传输块大小（字节）。发DMA 传输命令时可指定不同大小
...
API 调用可用于检测流结束（end of stream）条件
...
立即冻结播放。在此模式下，当内部缓冲区满时，不再接收更多数据，数据请IRQ 也会被屏蔽
...
事件
 - 0=音频模式monojoint) stereo dual channel 之间变化
 - 3=解码器已启动
...
0=extension & user data 提取, 1=从私有包提取
...
模式=MPEG from host, 1=YUV from encoder, 2=YUV from host
...
解码器预缓冲，启用时，对<8mbps 的流缓冲最128KB，对>8mbps 的流缓冲最640KB
...
在实践中，我见过平均传输量大约是 80K，但超过 128K 的传输并不少见，尤其是在启动时28K 这个数字很重要，因为它是内核通常能分配的最大块。即便如此，128K 的块也很难获得，因此强烈建议驱动编写者选择更小的块大小，并学习 scatter-gather 技术
...
本节总体上描述处DMA 传输时事件的顺序。详细信息在本节之后给出

- 卡触Encoder 中断
- 驱动从邮#10 读取传输类型、偏移和大小
- 驱动用足够多的空dma 缓冲区构scatter-gather 数组以覆盖该大小
- 驱动通过 ScheduleDMAtoHost API 调用调度 DMA 传输
- 卡触DMA Complete 中断
- 驱动检DMA 状态寄存器以发现任何错误
- 驱动对刚传输的缓冲区进行后处理

注意！Encoder DMA Complete 中断有可能同时被触发。（上一次的结束、下一次的开始，等等。）

#### 邮箱 #10


Flags、Command、Return Value Timeout 字段被忽略
...
我推测，既然 StartCapture API 有一个可用的捕获类型 "RAW"，那type 字段将会有对YUV PCM 数据的其他值

#### Scatter-Gather 数组


scatter-gather 数组是一块连续分配的内存，它告诉卡每个待传输数据块的源和目的。卡的“地址”由邮箱 #10 提供的偏移推导而来。主机地址是目DMA 缓冲区的物理内存位置

每个 S-G 数组元素是一个由三个 32 位字组成的结构体。第一个字是源地址，第二个是目的地址。两者各占满 32 位。第三个字的18 位是传输字节计数。第三个字的最高位是“last”标志。last 标志告诉卡触DMA_DONE 中断。根据我痛苦的个人经验，如果你忘了设置这个比特，卡仍会“工作”，但流极有可能被损坏

传输计数必须256 的倍数。因此，驱动需要跟踪目标缓冲区中有多少数据是有效的，并相应地处理
