...
对于兼容 SMIA++ 的设备，相应的文件名
	ccs/smiapp-sensor-vv-mmmm-rr.fw（传感器）以	ccs/smiapp-module-vv-mmmm-rrrr.fw（模块）
...
工具。CCS tools 包含人类可读CCS 静态数YAML 格式的定义，并包含一个将其转换为二进制的
程序
### 寄存器定义生成器


ccs-regs.asc 文件包含 MIPI CCS 寄存器定义，用于生成更便C 语言程序使用C 源代码定义文件由于生成的文件之间存在许多依赖关系，请不要手动修改它们，因为那容易出错且徒劳无功，而应修改
生成它们的脚本
#### 用法


按照惯例，脚本以如下方式调用来更CCS 驱动定义
...
CCS PLL 计算器用于在给定传感器能力、板配置以及用户指定配置的情况下计算 PLL 配置。由于涵盖所这些配置的配置空间非常庞大，PLL 计算器并非完全简单。但对于驱动而言它相对易于使用
PLL 计算器实现的 PLL 模型对应MIPI CCS 1.1
