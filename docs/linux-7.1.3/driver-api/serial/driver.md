...
底层串口硬件驱动负责向核心串口驱动提供端口信息（uart_port 定义）与一组控制方法（uart_ops 定义）。底层驱动还负责处理该端口的中断，并提供任何控制台支持
...
还有一个辅助函数（uart_console_write()）执行逐字符写入，将换行符转换CRLF 序列。建议驱动编写者使用此函数，而不是实现自己的版本


### 锁定


底层硬件驱动有责任使port->lock 执行必要的锁定。有一些例外（在下面的 struct uart_ops 列表中有描述）
...
计划有朝一日从 uart_port 中移'unused' 条目，并允许底层驱动向核心注册它们各自的 uart_port。这将允许驱动把 uart_port 用作一个指向结构体的指针，该结构体既包uart_port 条目，也包含它们自己的扩展，
```
