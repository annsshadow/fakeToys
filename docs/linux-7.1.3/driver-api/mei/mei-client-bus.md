...
接收数据很有用。然而，对于 ME 的某些功能而言，复用现有的软件栈并通过现有内核子系统来暴露它们更有意义
为了无缝接入内核设备驱动模型，我们在 MEI 驱动之上添加了一个内核虚拟总线
抽象。这使得可以为各MEI 特性实Linux 内核驱动，作为各自子系统中独立的
实体。甚至可以通过向现有代码添加一MEI CL 总线层，来潜在地复用已有设备驱动

## MEI CL 总线 API


为某MEI 客户端实现驱动与任何其它基于总线的设备驱动非常相似。驱动通过
`include/linux/mei_cl_bus.c` 中定义的 `struct mei_cl_driver` 结构将自注册MEI CL 总线驱动
...
通常在模块初始化时调用
一旦驱动注册并绑定到设备，驱动通常会尝试在该总线上做一I/O，而这应当通过
`mei_cl_send` `mei_cl_recv` 函数完成。更详细的信息见 API 一节
...
作为一个理论示例，假设 ME 带有一"contact" NFC IP。该设备的驱动初始化退出例程如下所示：
...
probe 例程中，驱动首先使能 MEI 设备，然后注册一rx 处理程序，这尽可接近于注册一个线程化 IRQ 处理程序。该处理程序的实现通常会调`mei_cldev_recv`，然后处理接收到的数据
