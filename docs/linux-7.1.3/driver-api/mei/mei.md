...
Intel Management Engine（Intel ME，英特尔管理引擎）是驻留在某些 Intel 芯片组内部的一个隔离且受保护的计算资源（协处理器）。Intel ME 为计算机/IT 管理与安全特性提供支持。实际的功能集合取决于 Intel 芯片组的 SKU。

Intel Management Engine Interface（Intel MEI，此前称为 HECI）是主机与 Intel ME 之间的接口。该接口作为 PCI 设备暴露给主机，实际上可能会暴露出多个 PCI 设备。Intel MEI 驱动负责主机应用程序与 Intel ME 特性之间的通信通道。

每个 Intel ME 特性（或 Intel ME 客户端）都由一个唯一的 GUID 寻址，并且每个客户端都有自己的协议。该协议是基于消息的，带有一个头部和负载，负载最大字节数由客户端在连接时通告。
...
应用程序在 /dev/meiX 处于打开状态时与某个 Intel ME 特性保持通信。与特定特性的绑定通过调用 `MEI_CONNECT_CLIENT_IOCTL` 完成，该调用传入期望的 GUID。可以同时打开的某个 Intel ME 特性的实例数量取决于该 Intel ME 特性，但大多数特性只允许单个实例。
...
连接到固件特性/客户端。
	...
	Input field:
		...
		in_client_uuid -	需要连接到的 FW 特性的 GUID。
         Outputs:
		out_client_properties - 客户端属性：MTU 与协议版本。

         Error returns:

                ENOTTY  没有这样的客户端（即错误的 GUID）或连接不被允许。
		EINVAL	错误的 IOCTL 编号
...
:Note:
        max_msg_length（MTU）在客户端属性中描述了可以发送或接收的最大数据。（例如，如果 MTU=2K，则可以发送最多 2k 字节的请求，并接收最多 2k 字节的响应）。
