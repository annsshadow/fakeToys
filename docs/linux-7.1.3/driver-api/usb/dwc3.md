...
**Synopsys DesignWare Core SuperSpeed USB 3.0 鎺у埗鍣?*
（以下简**DWC3**）是一个符USB SuperSpeed 规范
控制器，可通过以下 4 种方式之一进行配置

 1. 仅外设（Peripheral-only）配
 2. 仅主机（Host-only）配
 3. 双角色（Dual-Role）配
 4. 集线器（Hub）配

Linux 目前支持该控制器的多个版本
SoC 中的版本极有可能已经受支持。在撰写本文时，
已知经过测试的版本范围从 2.02a 3.10a
作为经验法则，高2.02a 的版本应该都能稳定工作
...
你的 IP 团队*Synopsys DesignWare Core SuperSpeed USB 3.0
Controller Databook*。以下是撰写本文时驱动所支持
特性列表：
 ...
 2. 灵活的端点配
 3. 同时支持 IN OUT 传输
 4. 散列表（Scatter-list）支
 5. 每个端点最256 TRB [#trb]_
...
gadget 驱动进行了验证。我们已验证 **ConfigFS** [#configfs]_ 
传统gadget 驱动

## 驱动设计


DWC3 驱动位于 **drivers/usb/dwc3/** 目录。所有文
都与此驱动相关并位于同一目录中。这使得
新手能够轻松阅读代码并理解其行为
...
该驱动最主要的部分涉Gadget API

## 已知限制


与任何其他硬件一样，DWC3 也有其自身的一组限制。为
避免不断被问及此类问题，我们决定在此
...
根据 Synopsys Databook，所OUT 传输 TRB [#trb]_ 必须
将其 **size** 字段设置为一个能被端**wMaxPacketSize**
整除的值。这意味着，例如，为了
接收 Mass Storage **CBW** [#cbw]_，req->length 必须设置
一个能**wMaxPacketSize** 整除的值（SuperSpeed 下为 1024
HighSpeed 下为 512 等），或DWC3 驱动必须添加一个指
废弃缓冲区的链式 TRB 以处理剩余长度。否则，OUT
传输*无法**启动

请注意，截至撰写本文时，这不会成为问题，因为 DWC3
完全能够为剩余长度追加一个链TRB，并
gadget 驱动完全隐藏这一细节。但仍有必要
提及，因为这似乎是有DWC3 以及
**传输无法工作**的最大疑问来源
...
目前，我们对每个端点256 TRB [#trb]_ 的硬性限制，
最后一TRB 是一个指
第一个的 Link TRB [#link_trb]_。该限制是任意设定的，但其好处是
总和恰好4096 字节，即 1 个页（Page）

DWC3 驱动会尽力处理超255 个请求的情况，并
在大多数情况下应能正常工作。但这并不是
...
确保
 ...
 1. 你正在运`Linus' tree`_ 的最新标
 2. 你能够在不对 DWC3 做任何树外（out-of-tree）修改的情况
	   复现该错
 3. 你已确认该问题并非主机（host）端的故
...
DWC3 完全依赖 Trace Events 进行调试。相关信
都在其中暴露出来，另有部分额外信息暴露在 DebugFS
[#debugfs]_ 中

为了捕获 DWC3 Trace Events，你应在
USB 线缆插入主机**之前**运行以下命令
...
请务必将 `trace.txt` `regdump.txt` 压缩为一tar 包，
并通过电子邮件发送给 `me`_，同时抄送（Cc）`linux-usb`_。如果你想更
为了确保我能帮助你，请按以下格式撰写邮件主题

	**[BUG REPORT] usb: dwc3: Bug while doing XYZ**

在邮件正文中，请务必详细说明你在做什么、使用的是哪gadget
驱动、如何复现问题、使用的是哪SoC、以及主机上运行的是哪个操作系统（及其版本）
...
总之，除Trace Events 对排DWC3 的问题确实很有帮助之外，这里没什么可多说的。此外，能够
查阅 Synopsys Databook 在这种情况下*非常**有价值

USB 抓包工具（Sniffer）有时会有帮助，但并非完全必需，很多信息无需查看线路即可理解

如果你需要任何帮助，随时可以发送电子邮件给 `me`_ 并抄`linux-usb`_
...
`Loopback`、`Reset`、`Resume` `UNKNOWN link state` 之一
...
DWC3 的寄存器转储（register dump）。请注意，可以对该文
执行 grep 以查找所需信息
...
`test_packet`、`test_force_enable`），或者在没有测试正在执行
打印字符`no test`

要启动这些测试模式中的任意一个，可以将相同的字符
...
`control`、`bulk`、`interrupt` `isochronous` 之一
如果端点尚未被启用，则打`--`

#### ``trb_ring``


读取时，`trb_ring` 将打印出环上所TRB 的详细信息
它还会告诉你我们的入队（enqueue）和出队（dequeue）指
在环中的位置
...
DWC3 还提供了多个 trace events，帮助我们在运行
收集关于驱动行为的信息

要使用这些事件，你必须在
内核配置中启`CONFIG_FTRACE`

关于如何启用 DWC3 事件的详细信息，请参
“报告缺陷”一节
...
在查找缺陷时，查看每一MMIO 访问有时很有用
因此，DWC3 提供了两Trace Events（一个用于读操作，一个用于写操作）：
```
...
`struct usb_request` 的整个生命周期都可以trace 缓冲区中被跟踪
我们为分配（allocation）、释放（free）等操作提供trace 事件
```
...
我们可以记录并解码每个通用命令（Generic Command）及其完
```
