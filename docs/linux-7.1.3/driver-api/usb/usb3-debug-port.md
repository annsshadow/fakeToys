...
:作者: Lu Baolu <baolu.lu@linux.intel.com>
:日期: 2017 年 3 月

## 概述（GENERAL）


本文介绍如何在 x86 系统上使用 USB3 调试端口。

在使用任何基于 USB3 的调试功能之前
...
xHCI 调试能力（DbC）是 xHCI 主机控制器提供的一个可选但独立的
功能。xHCI 规范在 7.6 节中描述了 DbC。

当 DbC 被初始化并启用后，它会通过调试端口（通常是第一个 USB3
超高速端口）呈现一个调试设备。该调试设备完全符合
USB 框架，并在调试目标（被调试的系统）与调试主机（debug host）之间
提供相当于一条非常高性能的、全双工串行链路。

## 早期打印（EARLY PRINTK）


DbC 被设计用来记录 early printk 消息。该特性的一个用途是内核调试。
例如，当你的机器在常规控制台代码初始化之前就非常早地崩溃时。
其他用途包括更简单、无锁（lockless）的日志，而非完整的
printk 控制台驱动与 klogd。
...
如果你的系统中有多个 xHCI 控制器，你可以
在该内核参数后附加一个主机控制器索引。该索引从 0 开始。

当前设计不支持 DbC 运行时挂起/恢复。因此，你最好为
以下参数禁用运行时电源管理
...
在启动调试目标之前，你应该将调试端口连接到调试主机上的
一个 USB 端口（根端口或任何外部集线器的端口）。用于连接这两个端口的
线缆应当是一条 USB 3.0 超高速 A-to-A 调试线缆。

在调试目标早期启动过程中，DbC 会被检测到并初始化。
初始化完成后，调试主机应当能够枚举调试目标中的调试设备。
随后调试主机会将调试设备与 usb_debug 驱动模块绑定，
并创建 /dev/ttyUSB 设备。
...
你可以使用任何通信程序（例如 minicom）来读取并查看这些消息。
下面这个简单的 bash 脚本可以帮助你检查设置是否正确。
...
DbC 支持已被添加到 xHCI 驱动中。你可以在运行时获得
由 DbC 提供的调试设备。

为了使用此功能，你需要确保内核已配置为支持
USB_XHCI_DBGCAP。xHCI 设备节点下的一个 sysfs 属性
用于启用或禁用 DbC。默认情况下，
...
使用一条 USB 3.0 超高速 A-to-A 调试线缆将调试目标连接到
调试主机。你会看到 /dev/ttyDBC0 被创建，
