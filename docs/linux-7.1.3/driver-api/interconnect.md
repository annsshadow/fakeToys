...
该框架旨在提供一个标准的内核接口，用于控制 SoC 上互连（interconnect）的
设置。这些设置可以是多个互连设备或功能块之间的吞吐量、延迟和优先级。可以
动态控制这些设置，以节省功耗或提供最大性能。

互连总线是一种具有可配置参数的硬件，可根据从各个驱动收到的请求在数据路径上
进行设置。互连总线的一个例子是芯片组中各个组件或功能块之间的互连。一个 SoC
上可以存在多条互连，并且可以是多层的。

下面是一张真实 SoC 互连总线拓扑的简化示意图。
```
...
互连提供者（Interconnect provider）是互连硬件的软件定义。上图中的互连提供者
是 M NoC、S NoC、C NoC、P NoC 和 Mem NoC。

互连节点（Interconnect node）是互连硬件端口的软件定义。每个互连提供者由多个
互连节点组成，这些节点连接到其他 SoC 组件，包括其他互连提供者。图中 CPU 连接
到内存的点称为互连节点，它属于 Mem NoC 互连提供者。

互连端点（Interconnect endpoints）是路径的第一个或最后一个元素。每个端点都是
一个节点，但并非每个节点都是端点。
...
互连使用者（Interconnect consumers）是利用提供者所暴露的数据路径的实体。使用者
...
显示系统中所有互连节点及其聚合的带宽请求。在每个节点下方，缩进显示各设备发出的
带宽请求。

`/sys/kernel/debug/interconnect/interconnect_graph`：

以 graphviz dot 格式显示互连图。它显示系统中所有的互连节点和链接，并将来自
同一提供者的节点归为子图。该格式具有人类可读性，也可以管道（pipe）输出
```
...
```
`test-client` 目录提供了向任意路径发出带宽（BW）请求的接口。请注意，出于安全
原因，该特性默认是禁用的，且没有用于启用它的 Kconfig。启用它需要修改代码
```
