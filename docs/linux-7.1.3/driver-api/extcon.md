...
标准化的接口来查询和监控这些状态
Extcon 在具有多种连接选项的现代设备（例如智能手机、平板电脑和笔记本电脑）尤其有用。它有助于管理各种类型的连接器，包括
1. USB 连接器（例如 USB-C、micro-USB2. 充电端口（例如快充、无线充电）
3. 音频插孔（例3.5mm 耳机插孔4. 视频输出（例HDMI、DisplayPort5. 扩展
真实世界的例子：

1. 智能手机 USB-C 端口   智能手机上单USB-C 端口可以提供多种功能。Extcon 可以管理该端口的不同状态，
   例如   - USB 数据连接
   - 充电（各种类型，如快充、USB Power Delivery   - 音频输出（USB-C 耳机   - 视频输出（USB-C HDMI 适配器）

2. 笔记本电脑扩展坞   当笔记本电脑连接到扩展坞时，会同时进行多个连接。Extcon 可以处理以下状态的
   变化   - 电力传输
   - 外部显示   - USB 集线器连   - 以太网连
3. 无线充电板：
   Extcon 可以管理无线充电连接的状态，使系统能够在设备放上或移开充电板时做出
   恰当的响应
4. 智能电视 HDMI 端口   在智能电视中，Extcon 可以管理多个 HDMI 端口，检测设备何时连接或断开，并可能
   识别设备的类型（例如游戏机、机顶盒、蓝光播放器）
Extcon 框架通过提供标准化的方式来报告与查询连接器状态、处理互斥连接以及管连接器属性，简化了这些复杂场景的驱动开发。这使得现代设备中对外部连接的处理更稳健和灵活
## 关键组件
...
```
关键字段
- `name`：Extcon 设备的名- `supported_cable`：支持的线缆类型数组
- `mutually_exclusive`：定义互斥线缆类型的数组
  该字段对于强制实施硬件约束至关重要。它是一32 位无符号整数数组，其中每  元素代表一组互斥的线缆类型。该数组应以 0 结尾
  例如
```
  ...
  在此示例中，线缆 0 1 不能同时连接，线2 4 也是互斥的。这对于诸如
...
- `name`：Extcon 设备的名- `state`：所有受支持线缆的当前状- `cable.N/name`：第 N 个受支持线缆的名- `cable.N/state`：第 N 个受支持线缆的状
### 使用示例
