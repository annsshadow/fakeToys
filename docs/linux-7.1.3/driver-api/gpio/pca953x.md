...
这些芯片拥有 4 个寄存器组：input、output、invert direction。每寄存器组包含 (lines/8) 个寄存器，每GPIO 端口对应一个
寄存器组的偏移量始终2 的幂
- 4  -> 寄存器组偏移1
- 8  -> 寄存器组偏移1
- 16 -> 寄存器组偏移2
...
这些 Maxim 芯片具有总线超时功能，可timeout_en 寄存器中启用。仅存在两款芯片中。默认超时禁用
========== ===== ========= ===== ====== ====== ========= ==========
...
除了 4 个基本寄存器外，这些芯片还带有一个中断掩码寄存器。中断掩码默所有中断均被禁用。要在这些芯片上使用中断，驱动必须设int_mask 寄存器
========== ===== ========= ===== ====== ====== ========= ========
...
### 带有额外 int_mask out_conf 寄存器的芯片


此芯片带有一个中断掩码寄存器，以及一个输出端口配置寄存器，可push-pull（推挽）open-drain（开漏）模式之间选择。每个比特控制两条线这两个寄存器也存在于 PCAL 芯片中，尽管 out_conf 的工作方式有所不同
========== ===== ========= ===== ====== ====== ========= ======== ========
...
pca9698 还带有一"master output"（主输出）寄存器，用于同时将所有端口的
输出设置为相同的值，以及一个芯片特定的 mode 寄存器，用于各种额外的芯设置
========== ============= ====
...
它们有两组输出寄存器（output0 output1）。如果在 blink 寄存器中启用blink 模式，内部定时器会在这些寄存器中所设的值之间交替切换有效输出master_intensity 寄存器与 intensity 寄存器一起，为每一对输出定PWM
亮度值
如果驱动处理 input、output0 direction 寄存器，这些芯片可以用作简单的
GPIO 扩展器
========== ===== ========= ===== ======= ========= ======= ================ ===== =========
...
这些芯片具有若干额外特性：
    ...
    1. 输出驱动强度设置（out_strength    2. 输入锁存（in_latch    3. 上拉/下拉（pull_in、pull_sel    4. 推挽/开漏输出（out_conf    5. 中断掩码与中断状态（int_mask、int_status
========== ============ ======== ======= ======== ======== ========== ========
...
目前驱动支持输入锁存、上下拉，并使用 int_mask int_status 处理
中断
...
这些芯片拥有完整PCAL 寄存器集合，外加以下功能
    1. 中断事件选择：level（电平）、rising（上升沿）、falling（下降沿）、any edge（任意边沿）
    2. 按线清除中断状    3. 读取输入而不清除中断状    4. 每个输出线的独立输出配置（推开漏）
    5. 输入去抖
...
如上表所示，pcal6534 不遵循通常的寄存器组间距规则。相反，它的寄存器组紧密排列的
### 寄存器布局完全不同PCA957X 芯片


这些芯片拥有基本4 个寄存器，但位于不寻常的地址
此外，它们还具有
    1. 上拉/下拉（pull_sel    2. 一个全局上拉使能，默认禁用（config    3. 中断掩码、中断状态（int_mask、int_status
========== ===== ========= ===== ====== ====== ======== ========= ====== ======== ==========
...
基本4 个寄存器，外加高级特性：
    ...
    1. 中断掩码，默认中断禁    2. 中断状    3. 中断事件选择，电平、上升沿、下降沿、任意边沿（int_mask、rising_mask、falling_mask    4. 上拉（无下拉    5. 三    6. 去抖
...
基本4 GPIO 功能在所有芯片类别中都存在，只有一个例外，`带有 LED 闪烁与亮度控制的芯片`_ 缺少 invert 寄存器
这些寄存器使用了 3 种不同的布局
    1. 寄存器组 0，寄存器组偏移为 2^n
        - 所有其他芯
    2. 寄存器组 0，寄存器组紧密排        - pcal6534

    3. 寄存器组 0，寄存器组偏移为 2^n
        - pca9574
        - pca9575
...
在所有芯片中这些以相同的方式工作：mask status 每个线对应一个比特，
mask 中为 1 表示中断已启用
布局
    1. 基址偏移 0x40，寄存器5 和寄存器6，寄存器组偏移为 2^n
        - pcal6408
        - pcal6416
        - pcal9535
        - pcal9554b
    ...
    2. 基址偏移 0x30，寄存器5 6，寄存器组紧密排        - pcal6534

    3. 寄存器组 6 7，寄存器组偏移为 2^n
        - pca9574
        - pca9575

    4. 寄存器组 5 7，寄存器组偏移为 2^n
        - xra1202
...
包含 2 个比特，可以为每个线选择 4 种事件之一
    0：电平，1：上升沿：下降沿：任意边
布局
    1. 基址偏移 0x40，寄存器7，寄存器组偏移为 2^n

        - pcal6524

    2. 基址偏移 0x30，寄存器7 + 偏移 0x01，寄存器组紧密排       （out_conf 1 字节，而非 (lines/8) 字节，因此有 0x01 偏移
        - pcal6534

`XRA1202`_ 芯片对同样的功能采用了不同的机制：它们具有一rising mask
（上升沿掩码）和一falling mask（下降沿掩码），每个线对应一个比特
布局
    1. 寄存器组 5，寄存器组偏移为 2^n
...
`基本 PCAL 芯片`_ `带有扩展中断与输出配置功能的 PCAL 芯片`_ 具有
此功能。当锁存启用时，在读取输入端口之前中断不会被清除。当锁存禁用时，
即使未读取输入寄存器，只要输入引脚恢复到产生中断之前的电平值，中断也会
被清除。默认锁存禁用
目前驱动为每条启用了中断的线启用锁存
一个中断状态寄存器记录了哪些引脚触发了中断。然而，状态寄存器和输入端寄存器必须分别读取；没有原子机制可以同时读取二者，因此可能出现竞态。请
参阅 `中断源检测`_ 一章，了解其影响以及驱动如何仍然利用锁存特性
    1. 基址偏移 0x40，寄存器2，寄存器组偏移为 2^n
        - pcal6408
        - pcal6416
        - pcal9535
        - pcal9554b
    ...
    2. 基址偏移 0x30，寄存器2，寄存器组紧密排        - pcal6534

### 上拉与下

`基本 PCAL 芯片`_ `带有扩展中断与输出配置功能的 PCAL 芯片`_ 使用相同
的机制：它们pull_en 寄存器启用上拉或下拉功能，它们的 pull_sel 寄存选择方向。它们都使用每个线一个比特
    0：下拉，1：上
布局
    1. 基址偏移 0x40，寄存器3（en）和 4（sel），寄存器组偏移2^n
        - pcal6408
        - pcal6416
        - pcal9535
        - pcal9554b
    ...
    2. 基址偏移 0x30，寄存器3（en）和 4（sel），寄存器组紧密排列
        - pcal6534

`寄存器布局完全不同PCA957X 芯片`_ 具有一pull_sel 寄存器（每个线一比特），以及config 寄存器中的一个全局 pull_en 比特
布局
    1. 寄存器组 2（config），寄存器组 3（sel），寄存器组偏移2^n
        - pca9574
        - pca9575

`XRA1202`_ 芯片只能上拉。它们具有一pullup_en 寄存器
布局
...
`带有额外 int_mask out_conf 寄存器的芯片`_ 具有此功能，但仅针对选定IO 端口。寄存器每个 2 条线对应 1 个比特。在 pca9698 中，只有 port0 port1 具有此功能
    0：开漏，1：推
布局
    1. 基址偏移 5*bankoffset
        - pca9698

`基本 PCAL 芯片`_ 在一个单独的 out_conf 寄存器中每个端口对应 1 个比特只能配置整个端口
    0：推挽，1：开
布局
    1. 基址偏移 0x4F
        - pcal6408
...
`带有扩展中断与输出配置功能的 PCAL 芯片`_ 可以为每个线单独设置此项。它具有`基本 PCAL 芯片`_ 相同的按端口 out_conf 寄存器，但还拥有一indiv_out_conf 寄存器（每个线一个比特），它会反转按端口设置的效果
    0：推挽，1：开
布局
    1. 基址偏移 0x40 + 7*bankoffset（out_conf），
       基址偏移 0x60，寄存器4（indiv_out_conf），寄存器组偏移2^n

        - pcal6524

    2. 基址偏移 0x30 + 7*banksize（out_conf），
       基址偏移 0x54，寄存器4（indiv_out_conf），寄存器组紧密排列
...
==== ==============
    ...
    1. 基址偏移 0x40，寄存器0 1，寄存器组偏移为 2^n
        - pcal6408
        - pcal6416
        - pcal9535
        - pcal9554b
    ...
    2. 基址偏移 0x30，寄存器0 1，寄存器组紧密排        - pcal6534
...
输入状态，并且只为正确的边沿或电平发出 IRQ。该系统与所启用中断的数无关。即使事件发生在 GPIO 扩展器的中断与实I2C 读取之间，也不会丢失
事件。当然，如果I2C 读取之前相关信号电平已变回驱动先前保存的值，边沿可能会被漏掉。PCAL 变体正是出于这一原因提供了输入锁存功能
### PCAL 输入锁存


PCAL 变体具有输入锁存，驱动为所有启用了中断的线启用此功能。只有在读取
输入端口时，中断才会被清除。这些变体提供了一个中断状态寄存器，记录哪引脚触发了中断，但状态寄存器和输入寄存器无法以原子方式读取。如果在读取
状态寄存器之后、但在采样输入端口寄存器之前，另一条线上发生了另一次中断，
该事件不会反映在先前的状态快照中，因此仅依赖中断状态寄存器是不够的
因此，PCAL 变体也必须使用现有的电平变化逻辑
对于短脉冲，当读取输入寄存器时会捕获第一个边沿，但如果在该读取之前信已恢复到先前的电平，则不会观察到第二个边沿。结果是，连续的脉冲在读取时
...
```
为了应对这种情况，中断状态寄存器指示的事件与通过现有电平变化逻辑检测到事件会被合并。结果是
- 第二个边沿不可见的短脉冲，通过中断状态寄存器被检测到，并- 发生在状态读取与输入读取之间的中断，仍然被通用的电平变化逻辑捕获
请注意，这仍然是最佳努力：状态寄存器和输入寄存器是分别读取的，其他线上的
短脉冲可能在这两次读取之间发生。此类脉冲仍可能被锁存为中断，而在读取不会留下可观察到的电平变化，并且可能无法归因于特定的边沿。这并不会比通用
