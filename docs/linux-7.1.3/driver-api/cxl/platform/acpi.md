...
ACPI 是“高级配置与电源接口”（Advanced Configuration and Power Interface），是一项
定义平台与操作系统如何管理电源以及配置计算机硬件的标准。出于本操作理论的目的，当
提到“ACPI”时，我们通常指的是“ACPI 表”——平台（BIOS/EFI）向操作系统传递静态配置
信息的方式。

以下 ACPI 表包含关于 CXL 设备的**静态**配置与性能数据：
...
SRAT 表也可能包含通用的端口/发起者（initiator）内容，旨在描述通用端口，但不包含
通往端点路径其余部分的信息。

Linux 使用这些表来为静态配置（由 BIOS/EFI）的 CXL 设备配置内核资源，例如：

- NUMA 节点
- 内存分层（Memory Tiers）
- NUMA 抽象距离（Abstract Distances）
- SystemRAM 内存区域
...
所有这些问题在用户看来都像是驱动未能支持 CXL——而实际上它们都是平台未能正确配置
ACPI 表所导致的失败。
