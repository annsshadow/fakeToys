...
该表Linux DSMAS 结合使用，以确定 CXL 设备的性能属性。DSLBIS 包含基于
...
该表Linux 用于计算从设备到根端口的 CXL 路径的性能坐标，其中交换机是路径的一部分
```
