...
在此示例系统中，我们有一个单插槽和一个 CXL 主机桥接。主机桥接上连接了两个带有
4GB 的 CXL 内存扩展器。

注意事项：

- 桥接内交错（Intra-Bridge interleave）此处不作描述。
- 这两个扩展器由单个 CEDT/CFMWS 描述。
- 该 CEDT/SRAT 为两个设备描述了一个节点。
- 两个设备的 HMAT 只有一个 proximity domain。
