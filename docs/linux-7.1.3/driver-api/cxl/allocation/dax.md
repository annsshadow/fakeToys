...
作为 DAX 设备暴露的 CXL 容量可通过 mmap 直接访问。用户可能希望使用此接口
机制编写自己的用户空间 CXL 分配器，或管理跨多个主机的共享或持久内存区域。

如果容量跨主机共享或持久化，则必须采用适当的刷新机制，除非该区域支持
Snoop Back-Invalidate。

注意，映射（大小和基址）必须与 DAX 设备的基址对齐方式对齐，通常为 2MB——
但也可能配置得更大。
