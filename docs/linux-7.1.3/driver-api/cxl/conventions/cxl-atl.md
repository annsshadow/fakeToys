...
主机物理地址（HPA）窗口。CXL 主机桥的每个 HPA 范围由一CFMWS 表项表示。一
HPA 范围可以包含当前分配CXL.mem 设备的地址，或者操作系统也可以将一个地址
窗口中的范围分配给某个设备

主机管理型设备内存（Host-managed Device Memory）是映射到系统一致性地址空间
且主机可以使用标准回写（write-back）语义访问的设备附加内存。被管理的地址范围
配置在设备的 CXL HDM Decoder 寄存器中。设备中HDM Decoder 负责通过剥离特定
地址位，HPA 转换DPA

CXL 设备CXL 桥使用相同的 HPA 空间。它在属于同一主机域的所有组件之间是通用
的。在主机与设备之间的 CXL.mem 路径上，地址区域的视图必须保持一致

这一点在 **CXL 3.2 规范**（表 1-1.3.1.2.4.20.13.1.18.1.3）中
有所描述[#cxl-spec-3.2]_

取决于平台的互联架构，挂接到主机的组件可能不共享相同的主机物理地址空间。那
平台需要地址转换，以在主机与所挂接的组件（例如 CXL 设备）之间转HPA。转
机制是主机特定的，且依赖于具体实现

例如，x86 AMD 平台使用数据 fabric（Data Fabric）来管理对物理内存的访问。设
拥有自己的内存空间，并可被配置为使用与系统物理地址（SPA）不同的“归一化地址
（Normalized addresses）”。因此就需要地址转换。详情参
...
包括针对 CXL 端点。AMD Zen5 系统实现ACPI PRM CXL 地址转换固件调用。ACPI PRM
处理程序有一个特定的 GUID，用于唯一标识支持归一化地址的平台。这**ACPI v6.5
移植指南**（Address Translation - CXL DPA to System Physical Address）中有记载
[#amd-ppr-58088]_

在归一化地址模式下，HDM 解码器的地址范围必须以不同的方式配置和处理。端HDM
解码器配置中使用的硬件地址不是 SPA，需要从其地址范围转换CXL 主机桥的地址
范围。这对于CFMWS 中查找端点相关联CXL 主机桥与 HPA 窗口尤为重要。此外，
交错（interleave）解码由数据 fabric 完成，端点在HPA 转换DPA 时并不执
解码。相反，端点的交错被关闭-way）。最后，地址转换在检查端点的硬件地址
...
相应CXL 主机桥（HDM）解码器与根解码器（CFMWS）匹配上面所示的计算出的端点
映射
...
- 允许 CXL 设备处于主机地址空间之外HPA 空间中

- 允许平台在主机与设备之间 CXL.mem 路径上跨越内存域时使用特定于实现的地址
  转换

- 定义一种将设备地址转换SPA PRM 处理程序方法
...
如果不做此变更，操作系统可能无法确定端点的内存区域与根解码器，以及其对应
...
**8.2.4.20 CXL HDM 解码器能力结*

“一个设备可以使用与其主机域其他组件不通用HPA 空间。平台负责在跨越 HPA 空间
时进行地址转换。操作系统必须确定交错配置，并在需要时执行HDM 解码HPA 范围
的地址转换。转换机制是主机特定的，且依赖于具体实现

平台通过提供一个平台运行时机制（PRM）处理程序来表明对独HPA 空间的支持以
对地址转换的需要。操作系统应使用该处理程序执行从 DPA 空间HPA 空间所需的转换
该处理程序在 9.18.4 *PRM Handler for CXL DPA 到系统物理地址转换* 中定义。

添加以下章节与小节（含表格）

**9.18.4 用于 CXL DPA 到系统物理地址转换PRM 处理程序**

“一个平台可被配置为使用‘归一化地址’。主机物理地址（HPA）空间是组件特定的，
并且不同于系统物理地址（SPA）。端点拥有自己的物理地址空间。呈现给设备的所
请求已经使用设备物理地址（DPA）。CXL 端点解码器关闭交错（1-way 交错），并且
设备不执HPA 解码来确DPA

平台提供一个用CXL DPA 到系统物理地址转换PRM 处理程序。该 PRM 处理程序
指定 CXL 端点的设备物理地址（DPA）转换为系统物理地址（SPA）。在主机的地址空间中，
SPA HPA 是等价的，操作系统应使用该处理程序来确定与设备地址对应HPA，例
在配置了归一化地址的平台上配置 HDM 解码器时。处理程序的 GUID 与参数缓冲区格式
9.18.4.1 节中规定。如果操作系统识别出PRM 处理程序，则说明平台支持归一
地址，且操作系统必须在需要时执行 DPA 地址转换。

**9.18.4.1 PRM 处理程序调用**

“操作系统使用直接调用机制来调用 CXL DPA 到系统物理地址转换PRM 处理程序
...
调用者分配并准备一个参数缓冲区，然后传PRM 处理程序 GUID 与指向参数缓冲区
指针来调用该处理程序。参数缓冲区在表 9-32 中描述。

**9-32. 用于 CXL DPA 到系统物理地址转换PRM 参数缓冲*
（用CXL DPA 到系统物理地址转换PRM 参数缓冲区）

+-------------+-----------+------------------------------------------------------------------------+
| Byte Offset | Length in | Description                                                            |
| （字节偏移）|   Bytes   | （描述）                                                               |
+=============+===========+========================================================================+
| 00h         | 8         | **CXL Device Physical Address (DPA)**：CXL DPA（例如来            |
...
**9-33. 用于 CXL DPA 到系统物理地址转换PRM 输出缓冲*
（用CXL DPA 到系统物理地址转换PRM 输出缓冲区）

+-------------+-----------+------------------------------------------------------------------------+
| Byte Offset | Length in | Description                                                            |
| （字节偏移）|   Bytes   | （描述）                                                               |
+=============+===========+========================================================================+
| 00h         | 8         | **System Physical Address (SPA)**：从 CXL DPA 转换而来SPA       |
|             |           |                                                                        |
+-------------+-----------+------------------------------------------------------------------------+
