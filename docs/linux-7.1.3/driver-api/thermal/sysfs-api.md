...
通用thermal sysfs 提供了一组接口，thermal zone 设备（传感器）和 thermal cooling 设备（风扇、处理器……）向热管理方案注册并成为其中的一部分
本使用指南侧重于使新thermal zone cooling 设备能够参与热管理该方案是与平台无关的，任何类型的 thermal zone 设备cooling 设备都应当能够利用这一基础设施
thermal sysfs 驱动的主要任务是thermal zone 属性以cooling 设备属性暴露给用户空间一个智能的热管理应用程序可以根据来thermal zone 属性（当前温度和触发点温度）的输入来做出决策，并对相应的设备进行节流（throttle）
- `[0-*]`		表示任何0 开始的正整- `[1-*]`		表示任何1 开始的正整
...
Thermal sysfs 属性将表示/sys/class/thermal 之下如果 hwmon 被编译进内核或作为模块构建，hwmon sysfs 接口扩展也可/sys/class/hwmon 下使用
```
...
```
接下来的两个动态属性是成对创建/移除的。它们表thermal zone 与其关联cooling 设备之间的关系
```
  /sys/class/thermal/thermal_zone[0-*]:
...
```
除了 thermal zone 设备 sysfs 接口cooling 设备 sysfs 接口外，通用thermal 驱动还会为每_type_ thermal zone 设备创建一hwmon sysfs 接口。例如，通用thermal 驱动会注册一hwmon 类设备，并为所有已注册ACPI thermal zone 构建关联hwmon sysfs 接口
请阅Documentation/ABI/testing/sysfs-class-thermal 以了thermal zone cooling 设备属性的详细信息
```
...
```
请阅Documentation/hwmon/sysfs-interface.rst 以获取更多信息
## 3. 一个简单的实现（A simple implementation

ACPI thermal zone 可能支持多个触发点，critical、hot、passive、active。如果一ACPI thermal zone 同时支持 critical、passive、active[^0^] active[^1^]，它可以将自己注册为一个带 4 个触发点thermal_zone_device（thermal_zone1）。它有一个处理器和一台风扇，两者都注册thermal_cooling_device。两者在冷却thermal zone 方面被认为具有相同的有效性
如果处理器列_PSL 方法中，且风扇列_AL0 方法```
 /sys/class/thermal:
  |thermal_zone1:
    |---type:			acpitz
    |---temp:			37000
...
```
## 4. Export Symbol APIs（导出符API

### 4.1. get_tz_trend


此函数返thermal zone 的趋势，即该 thermal zone 温度的变化率。理想情况下，thermal 传感器驱动应当实现该回调。如果没有，thermal 框架会通过比较先前和当前的温度值来计算趋势
### 4.2. thermal_cdev_update


此函数充当仲裁者来设置 cooling 设备的状态。如果可能，它会cooling 设备设置为最深的冷却状态
## 5. 关键事件（Critical Events

当发critical 触发温度越界事件时，thermal 框架将触发硬件保护性的断电（关机）或重启，具体取决于配置
首先，内核会尝试有序地断电或重启，但会接受一个延迟，超过该延迟后它将分别继续进行强制断电或重启。如果这失败，将作为最后手段调`emergency_restart()`
应当仔细分析该延迟，以便为有序断电或重启留出充足的时间
如果该延迟被设置0，则将不支持紧急动作。因此，要触发紧急动作，必须经过仔细分析的非零正整数值