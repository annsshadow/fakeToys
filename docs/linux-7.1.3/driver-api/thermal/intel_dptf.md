...
:作 Srinivas Pandruvada <srinivas.pandruvada@linux.intel.com>

### 简


Intel(R) Dynamic Platform and Thermal Framework（DPTF，动态平台与热管理框架）是一个用于电源与热管理的平台级硬软件解决方案

作为一个容纳多种电热管理技术的容器，DPTF 为不同策略影响系统硬件状态提供了一种协调的方式

由于它是一个平台级框架，因此包含多个组件。该技术的部分内容实现在固件中，并使用 ACPI PCI 设备来暴露各种监控与控制功能。Linux 拥有一组内核对用户空间暴露硬件接口的驱动。这使得诸如 “Linux Thermal Daemon之类的用户空间热管理方案能够读取平台特定的热与电源表，从而在将系统保持在热限制范围内的同时提供充足的性能

### DPTF ACPI 驱动接口


`/sys/bus/platform/devices/<N>/uuids`，其<N>
...
`/sys/bus/platform/devices/<N>/`，其<N>
=INT3400|INTC1040|INTC1041|INTC10A0

`imok` (WO)
	用户空间守护进程写入 1 以响应固件的发送保活（keep alive）通知事件。当固件调用 imok ACPI 方法要求用户空间响应时，用户空间会收
...
`/dev/acpi_thermal_rel`
	...
	该设备提IOCTL 接口，通过 ACPI 方法 _TRT _ART 读取标准 ACPI 热关系表。这IOCTL 定义
	...
	ACPI_THERMAL_GET_TRT: 读取二进TRT 表，读取长度通过 ioctl() 的参数提供

	ACPI_THERMAL_GET_ART: 读取二进ART 表，读取长度通过 ioctl() 的参数提供
...
DPTF 散热驱动以标准的热管sysfs cooling_device 形式呈现
...
有关 powercap ABI 请参Documentation/power/powercap/powercap.rst

`power_limit_0_max_uw` (RO)
	Intel RAPL powercap sysfs constraint_0_power_limit_uw 的最大

`power_limit_0_step_uw` (RO)
	Intel RAPL 约束 0 功率限制的功率增减量

`power_limit_0_min_uw` (RO)
	Intel RAPL powercap sysfs constraint_0_power_limit_uw 的最小

`power_limit_0_tmin_us` (RO)
	Intel RAPL powercap sysfs constraint_0_time_window_us 的最小

`power_limit_0_tmax_us` (RO)
	Intel RAPL powercap sysfs constraint_0_time_window_us 的最大

`power_limit_1_max_uw` (RO)
	Intel RAPL powercap sysfs constraint_1_power_limit_uw 的最大

`power_limit_1_step_uw` (RO)
	Intel RAPL 约束 1 功率限制的功率增减量

`power_limit_1_min_uw` (RO)
	Intel RAPL powercap sysfs constraint_1_power_limit_uw 的最小

`power_limit_1_tmin_us` (RO)
	Intel RAPL powercap sysfs constraint_1_time_window_us 的最小

`power_limit_1_tmax_us` (RO)
	Intel RAPL powercap sysfs constraint_1_time_window_us 的最大

`power_floor_status` (RO)
	当置1 时，表示当前配置下已达到系统的功率下限。需要重新配置才能进一步降低功率

`power_floor_enable` (RW)
	当置1 时，启用功率下限状态的读取与通知。当 power_floor_status 属性值发生变化时会触发通知

`/sys/bus/pci/devices/0000\:00\:04.0/`

`tcc_offset_degree_celsius` (RW)
	硬件将限CPU 的临界温度之上的 TCC 偏移量
...
`enable` (RW)
	1 表示启用 表示禁用。显示平台温度控制功能当前的启用状态。用户空间可以启禁用硬件控制

`temperature_target` (RW)
	更新硬件用于温度控制的新温度目标，单位为毫摄氏度

`thermal_tolerance` (RW)
	该属性取值范围为 0 7，其0 表示最激进的控制以避免任何温度超调，7 表示更平缓的方式，即便以温度超调为代价也偏向性能
	注意：该级别可能并非线性缩放。例如，3 并不一定意味着相比0 50% 的性能提升
...
RFIM 接口允许调整 FIVR（全集成电压调节器）、DDR（双倍数据速率）与 DLVR（数字线性电压调节器）的频率，以避免WiFi 5G 的射频干扰

开关电压调节器（VR）会在基频及其谐波处产生辐射 EMI RFI。某些谐波可能会干扰集成到笔记本等主机系统中非常敏感的无线接收器，如 Wi-Fi 与蜂窝网络。缓解方法之一是请求将 SoC 集成VR（IVR）开关频率调整一个小的百分比，并将开关噪声的谐波干扰从无线信道移开。OEM ODM 可以使用该驱动在不会影响 IVR 性能的范围内控制 SoC IVR 的运行
...
DDR IO 接口DRAM 设备及其电源平面可能在数据速率处产EMI。与 IVR 控制机制类似，Intel 提供了一种机制，在满足若干条件时改变 DDR 数据速率：由DDR 存在强烈RFI 干扰；CPU 电源管理在改DDR 数据速率方面没有其他限制；PC ODM BIOS 中为此启用了该特性（实时 DDR RFI 缓解，称DDR-RFIM）以用于 Wi-Fi
...
`vco_ref_code_lo` (RW)
	VCO 参考码是一11 位字段，控制 FIVR 的开关频率。这3 位的低字节（LSB）字段

`vco_ref_code_hi` (RW)
	VCO 参考码是一11 位字段，控制 FIVR 的开关频率。这8 位的高字节（MSB）字段
...
`rfi_restriction_run_busy` (RW)
	请求限制特定DDR 数据速率，并将此值置1。操作完成后自动复位0
...
`rfi_restriction_data_rate_Delta` (RW)
	用于 RFI 保护的受DDR 数据速率：下

`rfi_restriction_data_rate_Base` (RW)
	用于 RFI 保护的受DDR 数据速率：上
...
`dlvr_freq_select` (RW)
	设置 DLVR PLL 时钟频率。一旦设置并通过 dlvr_rfim_enable 启用，dlvr_freq_mhz 将显示当DLVR PLL 频率
...
请参Documentation/ABI/testing/sysfs-platform-dptf

### DPTF 风扇控制


请参Documentation/admin-guide/acpi/fan_performance_states.rst
...
用户空间可以轮询属“workload_type_index获取当前提示，也可以在该属性值更新时收到通知

file:`/sys/bus/pci/devices/0000:00:04.0/workload_hint/`
0、总线 0、设4、功0 在所Intel 客户端处理器上都保留给处理器热设备。因此，上述路径不会随处理器代的更迭而改变
...
`notification_delay_ms` (RW)
	固件通知 OS 之前的最小延迟，单位为毫秒。这用于控制通知的速率。该延迟介于固件改变工作负载类型预测与将改变通知 OS 之间。默认延迟为 1024 ms。延迟为 0 是无效的。延迟会被向上取整到最接近2 的幂，以简化固件对延迟值的编程。读notification_delay_ms 属性会显示所使用的有效值

`workload_type_index` (RO)
	预测的工作负载类型索引。用户空间可以通过现有sysfs 属性变更通知机制获得变更通知
	...
	2 持续（Sustained）：在较长一段时间内功耗相对较高，几乎没有空闲时段，最终会耗尽 RAPL Power Limit 1 2

	3 突发（Bursty）：消耗相对恒定的平均功率，但相对空闲的时段会被突发活动打断。突发相对较短，其间相对空闲的时段通常能防RAPL Power Limit 1 被耗尽

	4 未知（Unknown）：无法分类

	Panther Lake 开始的处理器提供了额外的提示。硬件在较长一段时间内分析工作负载驻留情况，以确定该工作负载分类倾向于空电池续航状态还是持性能状态。基于此长期分析，它分类如下

	功耗分类（Power Classification）：如果工作负载表现出更多的空闲或电池续航驻留，则归类为 “power”（功耗）

	性能分类（Performance Classification）：如果工作负载表现出更多的持续或性能驻留，则归类“performance”（性能）

	这种方式使应用可以忽略短期的工作负载波动，转而响应长期的功耗与性能趋势

	该分类的驻留阈值是 CPU 代特定的。分类通过 workload_type_index 的第 4 位报告：

	4 = 1：功耗分类（Power classification

	4 = 0：性能分类（Performance classification
