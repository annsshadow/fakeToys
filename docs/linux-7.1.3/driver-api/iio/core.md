...
工业 I/O（Industrial I/O，IIO）核心既为编写多种不同类型嵌入式传感器的驱动提供了一
...
- iio_device_alloc() - 从驱动分配一`iio_dev`
- iio_device_free() - 从驱动释放一`iio_dev`
- iio_device_register() - IIO 子系统注册一个设
- iio_device_unregister() - IIO 子系统注销一个设

一IIO 设备通常对应于单个硬件传感器，并提供处理该设备的驱动所需的全部信息
让我们先了解一下嵌入在 IIO 设备中的功能，然后再展示设备驱动如何使用一IIO 设备

用户空间应用程序可以通过两种方式IIO 驱动交互

1. `/sys/bus/iio/devices/iio:device{X}/`，这代表一个硬件传感器，并将同一芯片的数据通道分组在一起
2. `/dev/iio:device{X}`，用于缓冲数据传输和事件信息获取的字符设备节点接口

一个典型的 IIO 驱动会将自己注册[I2C <../i2c>](I2C <../i2c>) 
[SPI <../spi>](SPI <../spi>) 驱动，并创建 probe remove 两个例程

probe 时：

1. 调用 iio_device_alloc()，为 IIO 设备分配内存
2. 用驱动特定的信息（例如设备名、设备通道）初始化 IIO 设备字段
3. 调用 iio_device_register()，将设备注册IIO 核心。在此调用之后，设备即可接受来自用户空间应用程序的请求

remove 时，我们以相反的顺序释放 probe 中分配的资源
...
属性是用于暴露芯片信息并允许应用程序设置各种配置参数的 sysfs 文件。对于索引为 X 
设备，属性可/sys/bus/iio/devices/iio:deviceX/ 目录下找到。常见属性包括：

- `name`，对物理芯片的描述
- `dev`，显示与 `/dev/iio:deviceX` 节点关联major:minor 对
- `sampling_frequency_available`，设备可用的离散采样频率值集合
- IIO 设备的可用标准属性在 Linux 内核源码
  :file:Documentation/ABI/testing/sysfs-bus-iio 文件中有描述

## IIO 设备通道


struct iio_chan_spec - 单个通道的规格说

一IIO 设备通道是对一个数据通道的表示。一IIO 设备可以有一个或多个通道。例如：

- 温度计传感器有一个表示温度测量的通道
- 一个光传感器有两个通道，分别表示可见光与红外光谱的测量值
- 加速度计最多可3 个通道，分别表X、Y Z 轴上的加速度

一IIO 通道struct iio_chan_spec 描述
上面示例中温度传感器的温度计驱动
...
```
向用户空间暴露的通道 sysfs 属性以位掩码的形式指定。根据其共享信息的不同，属性可
设置在以下掩码之一中：
...
- `iio_chan_spec` **.modified** 字段设为 1。修饰符通过同一 `iio_chan_spec`
  结构**.channel2** 字段指定，用于表示通道的某个物理上唯一的特征，例如其方向或
  光谱响应。例如，一个光传感器可以有两个通道，一个用于红外光，一个用于红外与可见光
- `iio_chan_spec` **.indexed** 字段设为 1。在这种情况下，该通道只是另一
  带有**.channel** 字段指定的索引的实例
