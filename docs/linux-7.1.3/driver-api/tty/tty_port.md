...
本页面介绍 TTY 子系统中 struct tty_port 辅助机制及其对外 API，面向 TTY 驱动开发者，涵盖端口的初始化、打开/关闭/挂断处理、引用计数与调制解调器信号控制等辅助函数。


建议 TTY 驱动尽可能使用 struct tty_port 辅助函数。如果驱动实现了
:c`tty_port.ops.activate()` 与 :c`tty_port.ops.shutdown()`，它们可以在相应的
:c`tty_struct.ops` 钩子中使用 tty_port_open()、tty_port_close() 与
tty_port_hangup()。

引用与细节包含在底部的 `TTY Port Reference`_ 与 `TTY Port Operations Reference`_
小节中。
