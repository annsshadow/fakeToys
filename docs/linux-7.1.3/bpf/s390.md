...
IBM Z 是大型机，其源自 1964 年的 IBM System/360。它们由 Linux 内核"s390" 之名提供
支持。本文档描述如何s390 QEMU 客户机中测试 BPF
...
除了运行 BPF 测试套件所需的常Kconfig 选项之外，还
```
...
```
因为这样可以提供一种非常简便的方式s390 虚拟机共享文件

编译内核、模块和测试套件，以及准gdb 脚本
```
...
可以使用 QEMU GDB stub 调试 s390 内核，它通过QEMU 传入 `-s` 来启用

最好关KASLR，这gdb 才能知道去哪里找
```
