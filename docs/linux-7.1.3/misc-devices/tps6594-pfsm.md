...
TPS6594 PMIC（电源管理 IC，Power Management IC）集成了一个管理运行模式的状态机。根据当前的运行模式，某些电压域保持上电，而其他域可以关闭。

PFSM 驱动可用于触发已配置状态之间的转换。它还提供对设备寄存器的读/写访问。
...
运行模式之一。
PMIC 完全正常工作，并向所有 PDN 负载供电。
MCU 与主处理器两个部分的电压域均上电。

`PMIC_SET_MCU_ONLY_STATE`
运行模式之一。
仅有分配给 MCU Safety Island 的电源资源开启。

`PMIC_SET_RETENTION_STATE`
运行模式之一。
根据所设置的触发器，部分 DDR/GPIO 电压域可保持上电，而所有其他域关闭，以最小化系统总功耗。
