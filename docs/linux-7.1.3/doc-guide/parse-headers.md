...
将输入的 C 头文件或源文`FILE_IN` 转换为一ReStructured Text，通过
..parsed-literal 块包含，并为描述API 的文档文件创建交叉引用。它接受一可选的 `FILE_RULES` 文件，用于描述哪些元素将被忽略，或指向非默认引用
类型/名称
输出写入 `FILE_OUT`
它能够识`define`、`struct`、`typedef`、`enum` 以及枚举 `symbol`，并为它全部创建交叉引用
...
######## 选项
  ...
  `-h`, `--help`
      显示帮助信息并退  `-d`, `--debug`
      提高调试级别。可以多次使  `-t`, `--toc`
      不在字面块中输出，而是RST 文件中输出一个目录表（TOC
######## 描述


`FILE_IN` 创建内核头文件的增强版本，为其每C 数据结构类型添加交叉链接并使reStructuredText 标记进行格式化，可以是原样，也可以是一个目录表
它接受一个可选的 `FILE_RULES`，用于描述哪些元素将被忽略或指向非默认引用，
并可选择性地定义要使用的 C 命名空间
其目标是允许拥有更全面的文档，其uAPI 头文件将为代码创建交叉引用链接
输出写入 `FILE_OUT`
`FILE_RULES` 可能包含三种类型的语句：**ignore***replace** **namespace**
默认情况下，它会为所有符号和宏定义创建规则，但也允许解析一个例外文件。此文件包含一组使用以下语法的规则
1. 忽略规则
    ignore **type** **symbol**
...
2. 替换规则
    ...
    **old_symbol** 替换**new_reference**    **new_reference** 可以是：
