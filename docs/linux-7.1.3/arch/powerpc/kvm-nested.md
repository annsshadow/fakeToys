...
本文档解释了一个客户机操作系统如何在管理程序实现了相关超级调用（hypercall）的情况下，充当管理程序并通过使用超级调用来运行嵌套客户机。术语 L0、L1 与 L2 用于指代不同的软件实体。L0 是处于管理程序模式的实体，通常被称为“宿主机（host）”或“管理程序（hypervisor）”。L1 是一个直接在 L0 之下运行、由 L0 发起并控制的客户机虚拟机。L2 是一个由充当管理程序的 L1 发起并控制的客户机虚拟机。

## 现有 API


Linux/KVM 自 2018 年起就支持作为 L0 或 L1 进行嵌套（Nesting）
...
```
此 API 主要借助单一的超级调用 h_enter_nested() 工作。该调用由 L1 发出，用以告知 L0 以给定状态启动一个 L2 vCPU。随后 L0 启动这个 L2 并运行，直到达到一个 L2 退出条件。一旦 L2 退出，L2 的状态就由 L0 交还给 L1。每当 L2 运行时，完整的 L2 vCPU 状态总是在 L1 之间来回传递。L0 不在 L2 vCPU 上保留任何状态（除了在 L0 中 L1 -> L2 进入与 L2 -> L1 退出的短暂序列期间）。

L0 保留的唯一状态是分区表（partition table）。L1 使用 h_set_partition_table() 超级调用注册它的分区表。L0 持有的关于 L2 的所有其它状态都是被缓存的状态（例如影子页表）。

L1 可以在不事先通知 L0 的情况下运行任何 L2 或 vCPU。它只需使用 h_enter_nested() 启动 vCPU 即可。L2 与 vCPU 的创建在每次调用 h_enter_nested() 时隐式完成。

在本文档中，我们称这个现有 API 为 v1 API。

## 新的 PAPR API


新的 PAPR API 与 v1 API 的不同之处在于：创建 L2 及其关联的 vCPU 是显式的。在本文档中，我们称之为 v2 API。

h_enter_nested() 被 H_GUEST_VCPU_RUN() 取代。在这之前，L1 必须使用 h_guest_create() 显式地创建 L2，并使用 h_guest_create_vCPU() 创建任何关联的 vCPU。获取与设置 vCPU 状态也可以使用 h_guest_{g|s}et 超级调用完成。

L1 创建一个 L2、运行它并删除它的基本执行流程是：

- L1 与 L0 通过 H_GUEST_{G,S}ET_CAPABILITIES() 协商能力（通常在 L1 启动时）。

- L1 请求 L0 用 H_GUEST_CREATE() 创建一个 L2，并收到一个令牌（token）

- L1 请求 L0 用 H_GUEST_CREATE_VCPU() 创建一个 L2 vCPU

- L1 与 L0 使用 H_GUEST_{G,S}ET() 超级调用沟通 vCPU 状态

- L1 请求 L0 通过运行 H_GUEST_VCPU_RUN() 超级调用来运行该 vCPU

- L1 用 H_GUEST_DELETE() 删除 L2
...
提供本文档是为了让人对 API 有一个整体的理解。它并不旨在提供实现一个 L1 或 L0 所需的全部细节。更多细节可参考最新版本的 PAPR。

所有这些 HCALL 都由 L1 向 L0 发出。

### H_GUEST_GET_CAPABILITIES()


此调用用于获取 L0 嵌套管理程序的能力。这包括诸如 CPU 版本之类的能力（例如
```
...
此调用用于将 L1 管理程序的能力告知 L0。这里传入的标志集合与 H_GUEST_GET_CAPABILITIES() 相同。

通常，先调用 GET，然后再用从 GET 返回的标志子集调用 SET。这一过程允许 L0 与
```
...
此调用用于创建一个 L2。会返回所创建 L2 的唯一 ID（类似于一个 LPID），可在后续 HCALL 中使用它来
```
...
此调用用于创建一个与 L2 关联的 vCPU。应当传入 L2 的 id（从 H_GUEST_CREATE() 返回）。同时传入的还有一个（对此 L2 而言）唯一的 vCPUid。这个 vCPUid 由
```
...
此调用用于获取与 L2 关联的状态（客户机级或 vCPU 特定）。该信息通过客户机状态缓冲区（GSB）传递，它是一种标准格式，如本文档后面所解释，必要细节如下：

这可以获取 L2 级或 vCPU 特定的信息。L2 级的例子有时基偏移或进程作用域页表信息。vCPU 特定的例子有 GPR 或 VSR。flags 参数中的一个位指明此调用是 L2 级还是 vCPU 特定的，并且 GSB 中的 ID 必须与之匹配。

L1 提供一个指向 GSB 的指针作为此调用的参数。同时提供的还有与要设置的状态关联的 L2 与 vCPU ID。

L1 只在 GSB 中写入 ID 与大小。L0 写入
```
...
此调用用于设置 L2 级或 vCPU 特定的 L2 状态。该信息通过客户机状态缓冲区（GSB）传递，必要细节如下：

这可以设置 L2 级或 vCPU 特定的信息。L2 级的例子有时基偏移或进程作用域页表信息。vCPU 特定的例子有 GPR 或 VSR。flags 参数中的一个位指明此调用是 L2 级还是 vCPU 特定的，并且 GSB 中的 ID 必须与之匹配。

L1 提供一个指向 GSB 的指针作为此调用的参数。同时提供的还有与要设置的状态关联的 L2 与 vCPU ID。

L1 在 GSB 中写入所有值，而 L0 只读取 GSB 中的
```
...
此调用用于运行一个 L2 vCPU。L2 与 vCPU ID 作为参数传入。该 vCPU 以之前使用 H_GUEST_SET_STATE() 设置的状态运行。当 L2 退出时，L1 将从这个超级调用处恢复执行。

这个超级调用还有关联的输入与输出 GSB。与 H_GUEST_{S,G}ET_STATE() 不同，这些 GSB 指针不是作为超级调用的参数传入的（这样做是出于性能考虑）。这些 GSB 的位置必须使用 H_GUEST_SET_STATE() 调用、以 ID 0x0c00 与 0x0c01（见下表）预先注册。

输入 GSB 可能只包含要设置的 vCPU 特定元素。这个 GSB 也可以包含零个元素（即 GSB 前 4 字节为 0），如果无需设置任何东西的话。

从超级调用退出时，输出缓冲区被填入由 L0 决定的元素。退出的原因包含在 GPR4 中（即 NIP 被放入 GPR4）。返回的元素取决于退出类型。例如，如果退出原因是 L2 执行了一个超级调用（GPR4 = 0xc00），那么 GPR3-12 会被提供在输出 GSB 中，因为这是服务该超级调用可能需要的状态。如果需要额外的状态，L1 可以调用 H_GUEST_GET_STATE()。

要在 L2 中合成中断，当调用 H_GUEST_RUN_VCPU() 时，L1 可以设置一个标志（作为超级调用参数），L0 就会在 L2 中合成该中断。或者，L1 也可以使用 H_GUEST_SET_STATE() 自行合成中断，或
```
...
此调用用于删除一个 L2。所有关联的 vCPU 也会被删除。不提供单独的 vCPU 删除调用。
...
客户机状态缓冲区（GSB）是 L1 与 L0 之间通过 H_GUEST_{G,S}ET() 与 H_GUEST_VCPU_RUN() 调用沟通 L2 状态的主要方法。

状态可以与整个 L2 关联（例如时基偏移），也可以与特定的 L2 vCPU 关联（例如 GPR 状态）。只有 L2 VCPU 状态可能由 H_GUEST_VCPU_RUN() 设置。

GSB 中的所有数据都是大端（big endian）的（与 PAPR 中的标准一致）。

客户机状态缓冲区有一个头部，给出元素的数量，随后是 GSB 元素本身。
...
GSB 元素中的 ID 指定了要设置什么。这包括架构状态（如 GPR、VSR、SPR），以及一些关于分区的元数据，如时基偏移与分区作用域页表信息。
...
在 v1 API 中，某些状态不在 ptregs/hvstate 中。这包括向量寄存器与某些 SPR。为了让 L1 为 L2 设置此状态，L1 在 h_enter_nested() 调用之前载入这些硬件寄存器，而 L0 确保它们最终成为 L2 状态（通过不去触碰它们）。
...
在 v1 API 中，所有状态都在每次 h_enter_nested() 超级调用时从 L1 发往 L0，反之亦然。如果 L0 当前没有运行任何 L2，L0 就没有关于它们的状态信息。唯一的例外是通过 h_set_partition_table() 注册的分区表的位置。

v2 API 改变了这一点，使得 L0 即使在它的 vCPU 不再运行时也保留 L2 状态。这意味着 L1 只需要在需要修改 L2 状态、或者它的值已过期时，才与 L0 沟通 L2 状态。这提供了一个性能优化的机会。

当一个 vCPU 从 H_GUEST_RUN_VCPU() 调用退出时，L1 在内部将所有 L2 状态标记为无效。这意味着如果 L1 想要知道 L2 状态（比如通过 kvm_get_one_reg() 调用），它需要调用 H_GUEST_GET_STATE() 来获取该状态。一旦读取，它在 L1 中被标记为有效，直到 L2 再次运行。

此外，当 L1 修改 L2 vCPU 状态时，它不需要在 L2 vCPU 再次运行之前把它写入 L0。因此当 L1 更新状态（比如通过 kvm_set_one_reg() 调用）时，它写入一个内部的 L1 副本，并且只在 L2 通过 H_GUEST_VCPU_RUN() 输入缓冲区再次运行时，才把这个副本刷新到 L0。

L1 这种惰性更新状态的做法避免了不必要的 H_GUEST_{G|S}ET_STATE() 调用。
