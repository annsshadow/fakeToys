...
该架构定义了一个称I/O 虚拟化报告结构（IVRS）的 ACPI 兼容数据结构，用于向
系统软件传达I/O 虚拟化相关的信息。IVRS 描述了平台中包含IOMMU 的配置与
能力，以及每IOMMU 虚拟化的设备的信息
IVRS 提供以下关于以下方面的信息：

- 平台中存在的 IOMMU，包括它们的能力与正确配- 与每IOMMU 相关的系I/O 拓扑
- 无法以其他方式枚举的外设
- SMI/SMM、平台固件与平台硬件使用的内存区域。这些通常是需要由系统软件配置  排除范围
### 如何生成 I/O 虚拟地址（IOVA）？


行为良好的驱动在发送需要执DMA 的命令到设备之前调用 dma_map_*() 调用。一DMA 完成且不再需要映射，驱动执行 dma_unmap_*() 调用以取消映射该区域
### Intel 特定说明
...
中断范围不被地址转换xfee00000 - 0xfeefffff）。对等（peer to peer）事务也
同样如此。因此我们保留来PCI MMIO 范围的地址，使它们不被分配IOVA 地址
### AMD 特定说明
...
```
DMAR ACPI 处理并初始化时，打印 DMAR 位置以及任何已处理的 RMRR
```
