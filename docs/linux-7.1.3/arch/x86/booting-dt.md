### DeviceTree 启动（DeviceTree Booting）
  ...
  内核在 code32_start 处有一个单一的 32 位入口点，
  解压器（实模式入口点在切换到保护模式后会进入同一个 32 位
  入口点）。该入口点支持一种调用约定，记录在
  ...
  此 device-tree 用作“启动页（boot page）”的扩展。因此它
  不会解析 / 考虑启动页已涵盖的数据，包括内存大小、保留范围、
  命令行参数或 initrd 地址。它仅保存无法通过其他方式获取的
