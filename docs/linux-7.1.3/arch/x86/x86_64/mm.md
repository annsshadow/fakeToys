...
## 采用 4 级页表的完整虚拟内存映射
 ...
 - "-23 TB" 这样的负地址是绝对地址（以字节计），从 64 位地址空间的顶部向下计数。同时以绝对地址和距顶部的距离这两种方式来看布局，会更容易理解
   例如 0xffffe90000000000 == -23 TB，它64 位地址空间的顶部（ffffffffffffffff）低 23 TB
   注意，当我们越靠近地址空间顶部时，计数单位会从 TB 变为 GB，再MB/KB
 - "16M TB" 乍一看可能有些奇怪，但它"16 EB" 更直观地表示大小——很少会有人一眼就认出 16 EB 16 艾字节（exabytes）。它也很好地展示64 位地址空间到底有多么巨大
```
...
## 采用 5 级页表的完整虚拟内存映射
 ...
 - 对于 56 位地址，用户空间内存扩大了 512 倍，0.125 PB 增加64 PB。所有的内核映射都下移到-64 PB 的起始偏移处，并且许多区域也进行了扩展，以支持大得多的物理内存
```
...
```
该架构定义了一64 位虚拟地址。实现可以支持更少。目前支持的48 位和 57 位虚拟地址。从63 位到最高有效实现位之间的位进行符号扩展。如果你将它们解释为无符号数，这就在用户空间和内核地址之间造成了一个空洞
直接映射覆盖了系统中直到最高内存地址的所有内存（这意味着在某些情况下它也可能包含 PCI 内存空洞）
我们EFI 运行时服务映射在 'efi_pgd' PGD 中，位于一64GB 大小的虚拟内存窗口中（这个大小是任意的，如果将来需要可以调大）。这些映射不属于任何其它内核 PGD 的一部分，并且仅EFI 运行时调用期间可用
注意，如果启用了 CONFIG_RANDOMIZE_MEMORY，则所有物理内存的直接映射、vmalloc/ioremap 空间以及虚拟内存映射都会被随机化。它们的顺序得以保留，但它们的基址会在启动早期被偏移
在修改这里的任何内容时，要非常小KASLR。KASLR 地址范围不得与除 KASAN 影子区以外的任何东西重叠，这是正确的，因KASAN 会禁KASLR
对于 4 级和 5 级两种布局，最后一2MB 空洞处的 KSTACK_ERASE_POISON 值为：ffffffffff4111
