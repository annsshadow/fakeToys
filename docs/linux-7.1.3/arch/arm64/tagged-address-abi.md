...
- `mmap()` 系统调用，且满足以下任一条件
  - 标志设置`MAP_ANONYMOUS` 位，  - 文件描述符引用一个普通文件（包括`memfd_create()` 返回的文件）`/dev/zero`
...
1. 不被内核访问但用于地址空间管理的用户地址（例`mprotect()`、`madvise()`）。在此上下文中允许使用有效的带标签指针，但有以下例外
   - `brk()`、`mmap()` 以及 `mremap()` `new_address` 参数，因为它们有可能与现有的用户地址产生别名
     ﻿注意：此行为在 v5.6 中发生了变化，因此某些较早的内核可能会错误地接受针对 `brk()`、`mmap()` `mremap()` 系统调用的有效带标签指针
   - 在从 `userfaultfd()` 获得的文件描述符上使用的 `UFFDIO_*` `ioctl()` `range.start`、`start` `dst` 参数，因为随后通过读取该文件描述符获得的故障地址将是去标签的，否则可能会让不知道标签的程序感到困惑
     ﻿注意：此行为在 v5.14 中发生了变化，因此某些较早的内核可能会错误地接受针对该系统调用的有效带标签指针
2. 被内核访问的用户地址（例`write()`）。此 ABI 放宽默认是禁用的，应用程序线程需要通过 `prctl()` 显式启用，如下所示：

   - `PR_SET_TAGGED_ADDR_CTRL`：为调用线程启用或禁AArch64 带标签地址 ABI
     `(unsigned int) arg2` 参数是一个描述所用控制模式的位掩码：

     - `PR_TAGGED_ADDR_ENABLE`：启AArch64 带标签地址 ABI。默认状态为禁用
     ﻿参`arg3`、`arg4` `arg5` 必须0
   - `PR_GET_TAGGED_ADDR_CTRL`：获取调用线程的 AArch64 带标签地址 ABI 状态
     ﻿参`arg2`、`arg3`、`arg4` `arg5` 必须0
   上述 ABI 属性是线程作用域的，在 clone() fork() 时继承，exec() 时清除
   如果 AArch64 带标签地址 ABI `sysctl abi.tagged_addr_disabled=1` 全局禁用，那么调`prctl(PR_SET_TAGGED_ADDR_CTRL, PR_TAGGED_ADDR_ENABLE, 0, 0, 0)` 将返`-EINVAL`。默认的 `sysctl abi.tagged_addr_disabled` 配置0
