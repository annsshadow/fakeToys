...
本文档简要概述了 Linux 为支持使ARM 可伸缩矩阵扩展（SME）而提供给用户空间的接口
...
- SME 的存在通过辅助向量 AT_HWCAP2 条目中的 HWCAP2_SME 报告给用户空间。该标志  存在意味着 SME 指令和寄存器以及本文档所描述Linux 特定系统接口的存在。SME   /proc/cpuinfo 中以 "sme" 形式报告
...
- SME2 的存在通过辅助向量 AT_HWCAP2 条目中的 HWCAP2_SME2 报告给用户空间。该标志  存在意味着 SME2 指令ZT0 以及本文档所描述Linux 特定系统接口的存在。SME2   /proc/cpuinfo 中以 "sme2" 形式报告
- Support for the execution of SME instructions in userspace can also be
  detected by reading the CPU ID register ID_AA64PFR1_EL1 using an MRS
  instruction, and checking that the value of the SME field is nonzero. [^3^]

- 用户空间也可通过 MRS 指令读取 CPU ID 寄存ID_AA64PFR1_EL1，并检SME 字段  值是否非零，来检测对用户空间中执SME 指令的支持。[^3^]
...
- 存在若干可选的 SME 特性，它们的存在通过 AT_HWCAP2 报告如下
  ...
  这些扩展也通过 CPU ID 寄存ID_AA64SMFR0_EL1 报告，用户空间可使用 MRS 指令读取
  该寄存器。详elf_hwcaps.txt cpu-feature-registers.txt
...
- 调试器应仅限于通过 NT_ARM_SVE、NT_ARM_SSVE、NT_ARM_ZA NT_ARM_ZT regset 与目  交互。检测对这些 regset 支持情况的推荐方式是先连接到一个目标进程，然后尝试一
...
- 每当 ZA 寄存器值在用户空间与内核之间通过内存交换时，寄存器值以一系列水平向量  形式编码在内存中，从 0 VL/8-1，采用与 SVE 向量相同的字节序无关格式存储
...
- 在线程创建时，PSTATE.ZA TPIDR2_EL0 会被保留，除非指定了 CLONE_VM，在那种情况  PSTATE.ZA 被设0，TPIDR2_EL0 被设0
### 2.  Vector lengths
...
SME 定义了第二个向量长度，类似于 SVE 向量长度，它控制流式模式 SVE 向量ZA 矩阵
数组的大小。ZA 矩阵是方形的，每边的长度等于一个流式模SVE 向量的字节数
...
- 在系统调用时 PSTATE.ZA 被保留，如果 PSTATE.ZA==1，则 ZA 矩阵ZTn（若存在）的
  内容被保留
- On syscall PSTATE.SM will be cleared and the SVE registers will be handled
  as per the standard SVE ABI.

- 在系统调用时 PSTATE.SM 会被清除，SVE 寄存器将按标SVE ABI 处理
- None of the SVE registers, ZA or ZTn are used to pass arguments to
  or receive results from any syscall.

- SVE 寄存器、ZA ZTn 都不用于向任何系统调用传递参数或从任何系统调用接收结果
- On process creation (eg, clone()) the newly created process will have
  PSTATE.SM cleared.

- 在进程创建时（例clone()），新创建的进程PSTATE.SM 会被清除
...
- 线程的所有其SME 状态，包括当前配置的向量长度、PR_SME_VL_INHERIT 标志的状态，
...
- 新增了一个信号帧记录 TPIDR2_MAGIC，其格式struct tpidr2_context，以便从信号处理
  函数访问 TPIDR2_EL0
- A new signal frame record za_context encodes the ZA register contents on
  signal delivery. [^1^]

- 一个新的信号帧记录 za_context 在信号递交时编ZA 寄存器内容。[^1^]
...
- ZA 矩阵是否包含在记录中，取决于 PSTATE.ZA 的值。当且仅  za_context.head.size >= ZA_SIG_CONTEXT_SIZE(sve_vq_from_vl(za_context.vl))
...
- ZTn 的信号记录具有魔ZT_MAGICx5a544e01），由一个标准信号帧头，后跟一  struct zt_context（指定系统支持的 ZTn 寄存器数量），然后是每个寄存64 字节数据
  zt_context.nregs 个块组成
...
- 如果信号帧中存在 za_context 且包含矩阵数据，PSTATE.ZA 被设1，并ZA 被填  指定的数据
- The vector length cannot be changed via signal return.  If za_context.vl in
  the signal frame does not match the current vector length, the signal return
  attempt is treated as illegal, resulting in a forced SIGSEGV.

- 向量长度不能通过信号返回来改变。如果信号帧中的 za_context.vl 与当前向量长度不匹配  则信号返回尝试被视为非法，导致强制产SIGSEGV
...
- 如果不支ZTn PSTATE.ZA==0，则拥有 ZTn 的信号帧记录是非法的，会导致强制产生
  SIGSEGV銆。
...
prctl(PR_SME_SET_VL, unsigned long arg)
    ...
    PR_SME_VL_INHERIT
        ...
        execve() 期间继承当前向量长度。否则，向量长度execve() 时被重置        系统默认值。（参见9 节。）
    ...
    PR_SME_SET_VL_ONEXEC
        ...
        若没PR_SME_SET_VL_ONEXEC，所请求的变更会立即生效
    ...
    - 调用线程的向量长度，或将在下一execve() 时由该线程应用的延迟向量长度（取决于
      arg 中是否含PR_SME_SET_VL_ONEXEC），被设为系统支持的小于或等vl 的最大值      如果 vl == SVE_VL_MAX，所设的值为系统支持的最大值
    ...
    - 返回值描述了结果配置，按 PR_SME_GET_VL 的方式编码。如arg 中不      PR_SME_SET_VL_ONEXEC，此值中报告的向量长度是本线程新的当前向量长度；否则      报告的向量长度是将在调用线程的下一execve() 时应用的延迟向量长度
    ...
    - 改变向量长度会使 ZA、ZTn、P0..P15、FFR 以及 Z0..Z31 中除 Z0 比特 [127:0] ..
      Z31 比特 [127:0] 之外的所有比特变为未指定状态，包括流式和非流式 SVE 状态      以等于线程当前向量长度的 vl 调用 PR_SME_SET_VL，或PR_SME_SET_VL_ONEXEC
    ...
    - 改变向量长度会导PSTATE.ZA 被清除。以等于线程当前向量长度vl 调用
      PR_SME_SET_VL，或PR_SME_SET_VL_ONEXEC 标志调用 PR_SME_SET_VL，在此意义上
      不构成对向量长度的更改
...
prctl(PR_SME_GET_VL)
    ...
    无法判断是否存在未决的延迟向量长度变更（通常只会在典型的 fork() vfork()     相应execve() 之间出现）
    To extract the vector length from the result, bitwise and it with
    PR_SME_VL_LEN_MASK.

    要从结果中提取向量长度，对其按位PR_SME_VL_LEN_MASK
...
- 定义了一个新regset NT_ARM_SSVE，用于通过 PTRACE_GETREGSET PTRACE_SETREGSET
  访问流式模式 SVE 状态，这在 sve.rst 中有记载
- A new regset NT_ARM_ZA is defined for ZA state for access to ZA state via
  PTRACE_GETREGSET and PTRACE_SETREGSET.

- 定义了一个新regset NT_ARM_ZA，用于通过 PTRACE_GETREGSET PTRACE_SETREGSET
  访问 ZA 状态
...
regset 数据struct user_za_header 开头，其中包含
    size
        ...
        regset 的完整大小，以字节为单位        这取决于 vl，将来也可能取决于其他因素
        If a call to PTRACE_GETREGSET requests less data than the value of
        size, the caller can allocate a larger buffer and retry in order to
        read the complete regset.

        如果PTRACE_GETREGSET 的调用请求的数据少于 size 的值，调用者可以分        更大的缓冲区并重试，以读取完整的 regset
    max_size
...
- 改变向量长度或标志的效果等同PR_SME_SET_VL 中记载的效果
  The caller must make a further GETREGSET call if it needs to know what VL is
  actually set by SETREGSET, unless is it known in advance that the requested
  VL is supported.

  如果调用者需要知SETREGSET 实际设置VL，则必须进行进一步的 GETREGSET 调用  除非事先已知所请求VL 受支持
...
- 无论哪种情况，对SETREGSET，可以省略负载，此时向量长度和标志会被改变且
  PSTATE.ZA 被设0（以及这些变更带来的任何后果）。如果提供了负载，则 PSTATE.ZA
  将被设为 1
...
- 对于 SETREGSET，如果所请求VL 不受支持，其效果与省略负载相同，只是会报告一  EIO 错误。不会尝试将负载数据转换为实际设置的向量长度所对应的正确布局。由调用  负责为实际的 VL 转换负载布局并重试
...
- 定义了一个新regset NT_ARM_ZT，用于通过 PTRACE_GETREGSET PTRACE_SETREGSET
  访问 ZTn 状态
...
- PSTATE.ZA==0 时，NT_ARM_ZT 的读取会ZTn 的所有比特报告为 0
- Writes to NT_ARM_ZT will set PSTATE.ZA to 1.

- NT_ARM_ZT 的写入会PSTATE.ZA 设为 1
...
- 如果提供了任何寄存器数据并同时带SME_PT_VL_ONEXEC，则寄存器数据将使用当前向量
  长度来解释，而非exec 配置使用的向量长度
...
- 将为dump 进程的每一个线程，在每coredump 中添NT_ARM_SSVE 备注。其内容等同  在生coredump 时，如果对每个线程执行了相应类型PTRACE_GETREGSET 所会读到的数据
- A NT_ARM_ZA note will be added to each coredump for each thread of the
  dumped process.  The contents will be equivalent to the data that would have
  been read if a PTRACE_GETREGSET of NT_ARM_ZA were executed for each thread
  when the coredump was generated.

- 将为dump 进程的每一个线程，在每coredump 中添NT_ARM_ZA 备注。其内容等同  在生coredump 时，如果对每个线程执行了 NT_ARM_ZA PTRACE_GETREGSET 所会读到的
  数据
...
- 将为dump 进程的每一个线程，在每coredump 中添NT_ARM_ZT 备注。其内容等同  在生coredump 时，如果对每个线程执行了 NT_ARM_ZT PTRACE_GETREGSET 所会读到的
  数据
- The NT_ARM_TLS note will be extended to two registers, the second register
  will contain TPIDR2_EL0 on systems that support SME and will be read as
  zero with writes ignored otherwise.

- NT_ARM_TLS 备注将扩展到两个寄存器，第二个寄存器在支SME 的系统上将包  TPIDR2_EL0，否则将被读作零且写入被忽略
...
/proc/sys/abi/sme_default_vector_length
    ...
    在启动时，默认向量长度最初被设为 32 或最大受支持的向量长度，取其中较小且受支持者    这决定了 init 进程（PID 1）的初始向量长度
...
- 在每execve() 调用时，新进程的新向量长度被设为系统默认向量长度，除
    - PR_SME_VL_INHERIT (or equivalently SME_PT_VL_INHERIT) is set for the
      calling thread, or

    - 为调用线程设置了 PR_SME_VL_INHERIT（或等价SME_PT_VL_INHERIT），
...
- 修改系统默认向量长度不会影响任何不进execve() 调用的已有进程或线程的向量长度
...
- 一种新模式，即流式模式（streaming mode），在该模式下可用正FPSIMD SVE 特性的
...
- 一个新的向量长度，用于控制处于流式模式ZA Z 寄存器的大小，与不在流式模式  用于 SVE 的向量长度相互独立。对于某个给定系统，当前选择的向量长度，或两种模式所
  支持的向量长度集合，都不要求有任何关系。流式模式的向量长度被称SVL
...
- 一个新ZA 矩阵寄存器。这是一SVLxSVL 比特的方形矩阵。对 ZA 的大多数操作要求
  启用流式模式，但 ZA 可以在不启用流式模式的情况下被启用，以便加载、保存和保留数据
  For best system performance it is strongly encouraged for software to enable
  ZA only when it is actively being used.

  为了获得最佳系统性能，强烈建议软件仅在主动使ZA 时才启用它
...
- PSTATE 中两个新1 比特字段，可通过 SMSTART SMSTOP 指令，或通过SVCR 系统
  ...
  - PSTATE.ZA，如果为 1，则 ZA 矩阵可访问且含有有效数据；如果为 0，则 ZA 不可访问    PSTATE.ZA 0 变为 1 时，ZA 中的所有比特被清除
