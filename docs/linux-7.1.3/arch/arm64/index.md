
## ARM64 架构（ARM64 Architecture）

本页收录 ARM64 架构相关的文档，涵盖启动、CPU 特性、内存、电源管理与调试等主题。



- [acpi_object_usage](acpi_object_usage)
- [amu](amu)
- [ARM ACPI](arm-acpi)
- [arm-cca](arm-cca)
- [asymmetric-32bit](asymmetric-32bit)
- [启动](booting)
- [CPU 特性寄存器](cpu-feature-registers)
- [CPU 热插拔](cpu-hotplug)
- [elf_hwcaps](elf_hwcaps)
- [gcs](gcs)
- [hugetlbpage](hugetlbpage)
- [kdump](kdump)
- [legacy_instructions](legacy_instructions)
- [内存](memory)
- [内存标记扩展](memory-tagging-extension)
- [mops](mops)
- [mpam](mpam)
- [perf](perf)
- [指针认证](pointer-authentication)
- [页表转储](ptdump)
- [芯片勘误](silicon-errata)
- [sme](sme)
- [sve](sve)
- [带标签地址 ABI](tagged-address-abi)
- [带标签指针](tagged-pointers)
- [特性](features)

