...
1999 骞?5 鏈?29 鏃。
   ...
   3. 增强896 驱动的优
         ...
         3.2 SYM53C896 的新特性（64 PCI 双通道 LVD SCSI 控制器）
   4. 内存映射 I/O 与普I/O
   5. 标记命令队列
   6. 奇偶校验
   7. 性能剖析信息
   ...
   13. 与体系结构相关的特
   14. 已知问题
         14.1 使用 Iomega Jaz 设备的标记命
         14.2 添加另一控制器时设备名发生变
         14.3 WIDE SCSI 控制器上仅使8 位设
         14.4 内存写并使无效期间可能出现的数据损坏
   15. SCSI 问题排查
         15.1 问题追踪
         15.2 理解硬件错误报告
   16. 同步传输协商
         16.1 53C875 53C860 Ultra-SCSI 控制器的同步时序
         16.2 快SCSI-2 53C8XX 控制器的同步时序
   17. 串行 NVRAM 支持（作Richard Waltham
         17.1 特
...
最初的 Linux ncr53c8xx 驱动FreeBSD ncr 驱动的一个移植版本，
以下人员1995 11 月完成：

 - Gerard Roudier              <groudier@free.fr>

最初的驱动由以下人员为 386bsd FreeBSD 编写
...
- ncr53c8xx 通用驱动，支持整SYM53C8XX 系列，包括最早的 810 rev. 1
  最新的 896（双通道 LVD SCSI 控制器）以及新的 895A（单通道 LVD SCSI 控制器）
- sym53c8xx 增强型驱动（又称 896 驱动），它放弃了对最老芯片的支持
  以获得新特性的优势，例如自 810A 起可用的 LOAD/STORE 指令，以
  896 895A 上可用的硬件相位失配处理

关于 NCR 8xx 系列的技术信息，可在 Michael Will 编写PCI-HOWTO
以及 Drew Eckhardt 编写SCSI-HOWTO 中找到

关于新芯片的信息可在 LSILOGIC Web 服务器上获取

          - http://www.lsilogic.com/

SCSI 标准文档可在 SYMBIOS ftp 服务器上获取
...
这些工具并非 ALPHA 版本，而是相当干净并且工作良好的。拥
'scsiinfo' 软件包是必不可少的

这份简短的文档描述了通用驱动与增强型驱动的特性、配置参数，以及
通过 proc SCSI 文件系统的读/写操作可用的控制命令

该驱动已linux/i386、Linux/Alpha Linux/PPC 上测试通过
...
"宽协 对支持它的芯片提供。下面的表格展示NCR 8xx 系列部分芯片
的特性，以及哪些驱动支持它们
...
810A25A759596 895A 支持名为 LOAD STORE 的新 SCSI
SCRIPTS 指令，它们能够比 53c7xx 53c8xx 系列所支持MOVE MEMORY
指令更快地在 IO 寄存器与内存之间移动最1 DWORD
LOAD/STORE 指令支持绝对寻址DSA 相对寻址模式。SCSI SCRIPTS 已完
使用 LOAD/STORE 重写了，取代MOVE MEMORY 指令

### 3.2 SYM53C896 的新特性（64 PCI 双通道 LVD SCSI 控制器）


896 895A 允许SCRIPTS 处理相位失配上下文（避免了相位失配中断，
该中断会暂停 SCSI 处理器，直到 C 代码保存了传输的上下文）
如果不使LOAD/STORE 指令来实现这一点将十分痛苦，我甚至都不想尝试

896 芯片支持 64 PCI 事务与寻址，895A 支持 32 PCI 事务
64 位寻址。这些芯片的 SCRIPTS 处理器并非真正的 64 位，而是使用
段寄存器来处理第 32-63 位。另一个有趣的特性是，寻址片上 RAMk）的
LOAD/STORE 指令保持在芯片内部

由于使用LOAD/STORE SCRIPTS 指令，该驱动不再支持以下芯片
...
内存映射 I/O 比普I/O 具有更低的延迟。自 linux-1.3.x 起，使用内存映射
I/O 而非普I/O。内存映I/O 在大多数硬件配置上似乎工作良好，
一些设计不佳的主板可能会破坏这一特性
...
向一个设备一次排队多1 条命令，可以让它基于实际的磁头位置及其机
特性进行优化。这一特性还可以降低平均命令延迟。为了真正获得该特性的
优势，设备必须拥有合理的缓存大小（对128 KB 或更小的低端硬盘
不要指望什么奇迹）
...
如果你的控制器带NVRAM，你可以通过用户设置工具按目标配置这一特性
Tekram 设置程序允许将排队命令的最大数量调整到 32。Symbios 设置程序
只允许启用或禁用该特性

排队到某个设备的同时标记命令的最大数量目前默认设8。这个值适用
大多SCSI 硬盘。对于大SCSI 硬盘= 2GB，缓>= 512KB，平
寻道时间 <= 10 ms），使用更大的值可能会获得更好的性能

sym53c8xx 驱动每个设备最多支255 条命令，通用 ncr53c8xx 驱动最多支
64 条，但使用超32 条通常并不划算，除非你使用的是非常大的硬盘
磁盘阵列。值得注意的是，大多数近期的硬盘似乎不接受超过 64 条并发命令
因此，使用超64 条排队命令大概只是在浪费资源

如果你的控制器没NVRAM，或者它SDMS BIOS/SETUP 管理，你可以配置
标记队列特性以及设备队
```
...
- 控制0 上的目标 2  所有逻辑单元  --> 15
- 控制0 上的目标 3  所有逻辑单元  --> 15
- 控制0 上的目标 4  所有逻辑单元  -->  7
- 控制1 上的目标 1  逻辑单元 0     --> 32
- 所有其他目逻辑单元             -->  4

在某些特殊条件下，某SCSI 硬盘固件可能会针对一SCSI 命令返回
QUEUE FULL（队列已满）状态。驱动使用以下启发式方法处理这种行为

- 每次返回 QUEUE FULL 状态时，标记队列深度会被降低到当前已断开连接
  命令的实际数量

- 每成功完1000 SCSI 命令，若当前上限允许，可排队的命令最大数
  会递增

由于接收和处QUEUE FULL 状态会浪费资源，默认情况下驱动会通过
指示实际使用的命令数量及其状态，以及它对设备队列深度变化的决定，
将此问题通知用户
驱动处理 QUEUE FULL 所使用的启发式方法确保了性能所受影响不会太糟。你
可以通过以下方式将消息关掉：将详细输出级别设0，如下所示：

第一种方法：
	    使用 'ncr53c8xx=verb:0' 选项启动你的系统

第二种方法：
	    在启动后，对与你的控制器对应proc 文件系统条目应用
            "setverbose 0" 控制命令

## 6. 奇偶校验


该驱动支SCSI 奇偶校验PCI 总线主设备奇偶校验。为了确保数据传
安全，必须启用这些特性。然而，某些有缺陷的设备或主板会在奇偶校验上
遇到问题。你可以通过在启动命令行中输入适当的选项来禁PCI 奇偶校验
SCSI 奇偶校验。（参见 10：启动设置命令）
...
```
常规信息很好理解。设ID 与修ID SCSI 芯片的标识如下：
...
性能剖析信息SCSI 命令完成时更新。当主机适配器被挂载时会分配
清零一个数据结构。因此，如果驱动是模块，则每次加载驱动时性能剖析
计数器都会被清零clearprof" 命令允许你随时清除这些计数器
...
由于系统时钟1/100 秒节拍，"ms_post" 时间可能是错误的

在上面的示例中，我们得到18038 运行的中断，以及通常由于
分散列表某一段内部的断开连接所导致1673 次脚本中断


## 8. 控制命令


控制命令可以通过proc SCSI 文件系统执行写操作发送给驱动。通用
命令语法如下
...
```
对以下命令使"all" 作为 "<target>" 参数将应用于 SCSI 链上的所
目标（控制器本身除外）
...
然而，如果只有一个设备在某个 SCSI 特性上有缺陷，你可以在 Linux 启动
禁用驱动对该特性的支持，并在启动后仅为能安全支持该特性的设备启用它

CONFIG_SCSI_NCR53C8XX_IOMAPPED       (默认回答：n)
    如果你怀疑你的主板不允许内存映射 I/O，回"y"

    可能会稍微降低一点性能。Linux/PPC 需要此选项，无论你在此选择什
    都会使用它。Linux/PPC 使用此选项不会损失性能，因为所IO 都是
    内存映射的

CONFIG_SCSI_NCR53C8XX_DEFAULT_TAGS    (默认回答)
    默认标记命令队列深度

CONFIG_SCSI_NCR53C8XX_MAX_TAGS         (默认回答)
    此选项允许你指定可排队到某个设备的最大标记命令数量。最大支持值为 32

CONFIG_SCSI_NCR53C8XX_SYNC            (默认回答)
    此选项允许你指定驱动在启动时用于同步数据传输协商的频率（单MHz）
    此频率之后可"setsync" 控制命令更改 表示"异步数据传输"

CONFIG_SCSI_NCR53C8XX_FORCE_SYNC_NEGO (默认回答：n)
    对所SCSI-2 设备强制进行同步协商

    某些 SCSI-2 设备不在查询响应的第 7 字节中报告此特性，但却
    正确支持它（例如 TAMARACK 扫描仪）

CONFIG_SCSI_NCR53C8XX_NO_DISCONNECT   (默认以及唯一合理的回答：n)
    如果你怀疑你的某个设备不能正确支持断开连接，可以回"y"。这样，
    所SCSI 设备即使在执行长时间SCSI 操作时也永远不会断开总线

CONFIG_SCSI_NCR53C8XX_SYMBIOS_COMPAT
    正品 SYMBIOS 板卡使用 GPIO0 作为输出以驱动控制器 LED，并使用 GPIO3
    位作为单差分接口的标示标志
    如果你的系统中的所有板卡都是正SYMBIOS 板卡，或者使用来SYMBIOS
    BIOS 和驱动，你会希望启用此选项

    如果你的系统至少有一个基53C8XX SCSI 板卡带有厂商特定BIOS
    则绝不能启用此选项。例如，Tekram DC-390/U、DC-390/W DC-390/F
    SCSI 控制器使用厂商特定的 BIOS，并且已知不使用 SYMBIOS 兼容GPIO
    接线。因此，如果你的系统安装了这样的板卡，则绝不能启用此选项

CONFIG_SCSI_NCR53C8XX_NVRAM_DETECT
    启用Symbios 以及部分 Symbios 兼容卡（还有 Tekram DC390W/U/F 卡）
    上的串行 NVRAM 数据的读取支持。对于拥有多Symbios 兼容控制器
    其中至少一个带有串NVRAM 的系统，或同时混Symbios Tekram
    卡的系统很有用。可让主机适配器的扫描顺序设置为默认顺序或"反向探测"
    顺序之外的其他顺序
    还能区分 Symbios 卡与 Tekram 卡，从而可以在混有 Symbios Tekram
    卡的系统上设CONFIG_SCSI_NCR53C8XX_SYMBIOS_COMPAT，使 Symbios 
    能够使用包括差分、LED 引脚在内的全Symbios 特性，而不会给 Tekram
    卡带来问题
...
启动设置命令既可以在启动时传递给驱动，也可以作为字符串变量使
'insmod' 传递

ncr53c8xx（sym53c8xx）驱动的启动设置命令以驱动名 "ncr53c8xx="（sym53c8xx
开头。然后内核语法解析器期望一个可选的、以逗号分隔的整数列表，后跟一
...
由于在使'insmod' 定义字符串变量时似乎不允许使用逗号，驱动也接受
<空格> 作为选项分隔符。以下命令将使用与上面相同的选项安装驱动模块
...
在包含多53C8xx 适配器的系统中，insmod 会在每个适配器上安装指定
...
```
将在IO 端口地址 0x1400 处的适配器之外的所有适配器上安装 sym53c8xx
...
##### 10.2.4  特殊特
   ...
   仅适用810A25A6075 895 控制器。对其他控制器无效
   ...
   驱动的默认设置为 'specf:3'。因此，若要启用"内存写并使无
   （Memory Write And Invalidate），必须在启动设置命令中指定选项 'specf:y'

##### 10.2.5  Ultra SCSI 支持


   仅适用860759595a96010 1010_66 控制器。对其他控制器无效
...
##### 10.2.6  默认标记命令数量
  ...
  #tags 会被截断最大排队命令数"配置参数的值。此选项还允许为
...
burst:#x   burst enabled (1<<#x burst transfers max)
	   ...
	   #x 是一个整数值，为突发传输最大值的2 为底的对数

	   NCR53C875 NCR53C825A 最多支128 次突发传输（#x = 7）
...
##### 10.2.13 最大总线宽度
  ...
  某些 SCSI 板卡使用 875（ultra wide）但只提供窄型连接器。如果你用一
  50 针转 68 针的电缆转接器连接了一个宽型设备，任何被接受的宽协商都
...
##### 10.2.21 为主机建议默SCSI ID
    ...
    如果 NVRAM 中提供了主机 SCSI ID，驱动将忽略任何作为启动选项建议
    值。否则，如果提供了不同于 255 的建议值，则会使用它。否则，它会
    尝试推断此前在硬件中设置的值，并在硬件值为零时使用7

##### 10.2.22 启用 IMMEDIATE ARBITRATION（立即仲裁）


        （仅sym53c8xx 驱动支持。详10.7
...
"ncr53c8xx=verb:2" 来获取驱动的"静设置，或者在你的启动设置命令
...
这些选项仅适用于新SYMBIOS 芯片 810A25A6075 895，并
仅支Pentium 486 级处理器。近期的 SYMBIOS 53C8XX SCSI 处理器能
使用 PCI 读多个（read multiple）与 PCI 写并使无效（write and invalidate
命令。这些特性要求芯片的 PCI 配置空间中正确设置了缓存行大小寄存器
另一方面，芯片只有在 PCI 命令寄存器中相应位被置为 1 时才会使PCI 
并使无效命令

并非所PCI BIOS 都会设置 53C8XX 芯片 PCI 配置空间中的 PCI 缓存
寄存器与 PCI 写并使无效位。优化的 PCI 访问在某PCI/内存控制器上可能
出问题，或在某些 PCI 板卡上产生问题

此修复在我的旧系统上运行完美
（主Triton HX / 53C875 / 53C810A
...
启用此选项时，驱动会尝试检测所有使用串NVRAM 的板卡。该存储器用
保存用户设置的参数

驱动能够NVRAM 获取的参数取决于所使用的数据格式，如下所示：
...
为了加快系统启动，对于每个配置为不带"启动时扫（scan at boot time
选项的设备，驱动会在收到的该设备的第一TEST UNIT READY 命令上强
返回一个错误

某些 SDMS BIOS 修订版似乎无法与非常快的硬盘一起干净地启动。在这种情况下，
你无法用优化后的参数值来配置 NVRAM

'nvram' 启动选项可以以十六进制形式输入，以忽NVRAM 中配置的某些选项
如下所示：
...
该选项），sym53c8xx 驱动不会挂载NVRAM 中设OFF 的控制器

ncr53c8xx 始终尝试挂载所有控制器。选项 0x80 没有被加ncr53c8xx 驱动
...
信号线的逻辑状态。驱动只是读SCSI 信号线，并检查除 RESET 外所有信号线
读取结果均为 FALSE。由SCSI 设备SCSI RESET 被断言后最800 纳秒
会释放总线，任何信号为 TRUE 都可能表明存SCSI 总线问题。遗憾的是，
以下常见SCSI 总线问题无法被检测到
...
此选项仅由 SYM53C8XX 驱动支持（不NCR53C8XX 支持）

SYMBIOS 53C8XX 芯片能够在检测到预期的断开连接（BUS FREE 相位）后
立即SCSI 总线进行仲裁。要使该过程启动，当芯片连接SCSI 总线时，
SCNTL1 IO 寄存器的1 必须被置位

当为当前连接启用了此特性后，如果只有优先级较低的设备在竞争 SCSI 总线
芯片有十足的把握赢得仲裁。顺便说一句，当芯片使SCSI ID 7 时，它必
赢得下一SCSI 总线仲裁

由于无法知道有哪些设备正试图对总线进行仲裁，使用此特性可能极不公平
因此，不建议你启用它，或者最多仅为芯片在上一次仲裁中失败的情
（启动选项 'iarb:1'）启用此特性

此特性具有以下优点：

a) 允许 ID 7 的发起者在需要时赢得仲裁
b) 将至4 微秒的仲裁时间与处理当前连接结束并开始下一个任务的
   SCRIPTS 执行重叠

嗯……但是（a）可能只是阻止其他设备重新选择发起者，并延迟数据传输或
状完成；而（b）如SCRIPTS 执行持续时间超过 4 微秒，可能只
浪费 SCSI 总线带宽

使用 IARB 需要在编译时定SCSI_NCR_IARB_SUPPORT 选项，并在启动时
'iarb' 启动选项设为非零值。它对实际工作并不是那么有用，但可用
SCSI 设备施加压力，或用于某些能从中获益的应用。顺便说一句，如果
你在IO 负载下使IARB 时遇到诸意外的断开连接"错误的重新选择"
...
SCSI_NCR_SETUP_SPECIAL_FEATURES	(default: defined)
	如果定义，驱动将根据芯片与修ID 启用一些特殊特性

        对于 810A6025A75 895 这些 SCSI 芯片，此选项启用
	SCSI 传输处理期间减轻 PCI 总线与内存访问负载的特性：突发
	取指（burst op-code fetch）、读多个（read multiple）、读
	（read line）、预取（prefetch）、缓存行（cache line）、写并使无效
	（write and invalidate）、突128（仅 875）、大 DMA FIFO
	（仅 875）、偏16（仅 875）
...
```
新版驱动会单独提供，以便在将其纳Linux 内核发行版之前测试变更与
新特性。以URL 提供了最新可用补丁的信息
...
队列能力。然而在自旋启动（spinning up）期间，它会拒绝标记命令。这
行为符合 SCSI-2 规范 6.8.2 节。驱动在这种情况下的当前行为并不令人满意
因此，不要为能够自旋降速（spin down）的设备启用标记命令队列。另一
可能出现的问题是超时。避免超时的唯一方法似乎是编
linux/drivers/scsi/sd.c 并增大当前的超时值

### 14.2 添加另一控制器时设备名发生变


当你向一个已经拥有一块或多块该系列控制器的系统添加一块新
NCR53C8XX 芯片控制器时，驱动向内核注册它们的顺序可能会导致因设备名
变化而产生的问题。当至少有一块控制器使用 NvRAM 时，SDMS BIOS 4 版允
你定BIOS 扫描 SCSI 板卡的顺序。如果设置了 NvRAM 检测选项，驱动会
根据 BIOS 信息挂载控制器
...
- fstab 做适当的修改
- 使用 Eric Youngdale 'scsidev' 工具

### 14.3 WIDE SCSI 控制器上仅使8 位设


当只8 位窄型（NARROW）设备连接到 16 位宽型（WIDE）SCSI 控制器时
你必须确SCSI 总线宽型部分的信号线被上拉。这可以通过启用 SCSI
控制器卡的宽型终结器（WIDE TERMINATOR）部分来实现

TYAN 1365 文档 1.2 版关于此类设置的描述不正确。（10 页，3.3）

### 14.4 内存写并使无效期间可能出现的数据损坏


此问题在 SYMBIOS DEL 397、部件号 69-039241、条4 中有描述

在某些复杂情况下，修订号 <= 3 53C875 芯片可能会从一个未与缓存行
对齐4 DWORD 边界开PCI 写并使无效命令。这只有在缓存行大小
8 DWORD 或更大时才可能发生。Pentium 系统使用 8 DWORD 的缓存行大小
因此受此芯片缺陷影响，i486 系统使用 4 DWORD 的缓存行大小，不
影响

当这种情况发生时，芯片可能在只填充了传输所涉及的最后一个缓存行
部分内容后就完成了写并使无效命令，从而让该缓存行的其余部分发生数
损坏

不使用写并使无效显然可以规避此芯片缺陷，因此现在它是驱动的默认设置
然而，对于像我这样想启用此特性的人，我加入了 SYMBIOS 建议的部分应
方法。该应对方法在进DATA IN 相位时重置寻址逻辑，从而防止该缺陷
相位的第一SCSI MOVE 时被触发。根据以下分析，该应对方法应当足够：

驱动内部唯一大于 8 DWORD 且由 SCRIPTS 处理器移动的数据结构
包含 SCSI 传输上下文的"CCB （CCB header）。该数据结构8 DWORD
边界（Pentium 缓存行大小）对齐，因此在 Pentium 系统上不受此芯片缺陷
影响

但是，当使用未与缓存行对齐的 4 DWORD 缓冲区执SCSI 读命令时
可能满足该缺陷的条件。在 Linux 下使用分聚集列表时不会发生这种情况，
因为它们只引用对齐良好的系统缓冲区。因此，Linux 下，仅当未使
分散/聚集列表，且在相位失配后重新进入 SCSI DATA IN 相位时，才可能需
应对方法
...
大多SCSI 问题源于不符合规范的 SCSI 总线或有缺陷的设备。如果你不幸
遇到SCSI 问题，可以检查以下事项：

- SCSI 总线电缆
- SCSI 链两端处的终结器
- Linux syslog 消息（其中一些可能会对你有帮助）
...
现在，如果你SCSI 总线正常，你的系统很有机会在此安全配置下工作
但性能不会是最优的

如果仍然失败，则你可以将你的问题描述发送到相应的邮件列表或新闻组
给我发一份副本，以确保我能收到。显然，驱动代码中可能存bug
...
```
将重置目3 的标志（no_disc），从而允许它断开 SCSI 总线
...
字段 B：DSTAT IO 寄存器（DMA 状态）
  ...
  如果报告DSTAT 值包MDPE (0x40) BF (0x20) 的组合，则原
  很可能是 PCI 总线问题

字段 C：SIST IO 寄存器（SCSI 中断状态）
  ========   ==================================================================
  Bit 0x08   SGE  SCSI GROSS ERROR
             表示芯片SCSI 总线上检测到了严重的错误条件，导SCSI 协议
             无法正常工作
  Bit 0x04   UDC  Unexpected Disconnection
             表示设备在芯片未预期的情况下释放SCSI 总线。设备可能如
             表现，以SCSI 发起者指示发生了无法SCSI 协议报告
             错误条件
  Bit 0x02   RST  SCSI BUS Reset
             通常 SCSI 目标不会复位 SCSI 总线，尽管总线上的任何设备
             可以在任何时候复位它
  Bit 0x01   PAR  Parity
             检测到SCSI 奇偶错误
  ========   ==================================================================

  在有故障SCSI 总线上，芯片可能检测到 SGE (0x08)、UDC (0x04) 
  PAR (0x01) 中的任何错误条件。如果你SCSI 系统有时遇到此类错误条件
  尤其SCSI GROSS ERROR，则 SCSI 总线问题很可能是这些错误的根源

对于字段 D、E、F、G H，你可以查看 sym53c8xx_defs.h 文件，其中包
IO 寄存器位的一些简要注释
...
理解字段 I、J、K 与转储需要对 SCSI 标准、芯片核心功能以及驱动内
...
第一张表对应于使80 MHz 时钟5 个时钟分频器Ultra 芯片 53875
53C860。第二张表是通过SCSI 时钟设为 40 MHz 并使4 个时钟分频器
计算的，因此适用于快SCSI-2 模式下的所NCR53C8XX 芯片

周期以纳秒为单位，速度以兆传输/秒为单位 兆传秒在 8 SCSI 
表示 1 MB/s，在 Wide16 SCSI 下表2 MB/s

16.1 53C8953C875 53C860 SCSI 控制器的同步时序
...
启用串行 NVRAM 支持后，可以检Symbios 以及部分 Symbios 兼容主机
适配器（还有 Tekram 板卡）上包含的串NVRAM。串NVRAM Symbios
Tekram 用来保存主机适配器及其所连驱动器的设置参数

Symbios NVRAM 还保存了拥有多个主机适配器的系统中主机适配器的启动
顺序数据。这样可以改变检测主机适配器时扫描各卡以寻找驱动器的顺序

目前，使反向探测"只能在一定程度上做到这一点，而且它只会改
不同类型卡的检测顺序NVRAM 启动顺序"设置既能做到这一点，也能改变
同类卡的扫描顺序，这反向探测"做不到的

使用 Symbios 芯片Tekram 板卡（DC390W/F/U）带NVRAM，会被检测出来，
并用于区Symbios 兼容Tekram 主机适配器。如果设置了
CONFIG_SCSI_53C8XX_SYMBIOS_COMPAT 配置参数，这用于禁用Tekram 板卡
错误设置Symbios 兼容"差分"（diff）设置，从而让 Symbios 卡与 Tekram
卡可以一起使用，Symbios 卡使用其全部特性，包括"差分"支持。（Symbios
兼容卡使LED 引脚"（led pin）支持可以保持启用。它Tekram 主机
适配器没有实际用处，但也不会造成问题。）
...
53c810a 53c875 NVRAM 的默认设置相
（可移动介质Symbios BIOS 4.09 版起添加
...
在驱动版2.5 及更高版本中可用。目前大端支持仅Linux/PPC
（PowerPC）上测试过

### 18.2 运行于大端模式的 NCR 芯片


SYMBIOS 文档中可以看到，某些芯片支持一种特殊的大端模式，理论上
包括3C8153C825A3C8753C875N3C895。此工作模式不是
软件选择的，而是需要将名为 BigLit 的引脚上拉。使用此模式，当驱动运行
于大CPU 上时，应当可以避免大部分字节重排。驱动版2.5 在理论上
也已为此特性做好准备
