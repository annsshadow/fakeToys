...
AdvanSys（Advanced System Products, Inc.）生产以下基RISC、支持总线主控（Bus-Mastering）、Fast0 MHz）与 Ultra0 MHz）窄带（8 位传输）SCSI 主机适配器，适用ISA、EISA、VL PCI 总线；以及基RISC、支持总线主控、Ultra0 MHz）宽带（16 位传输）SCSI 主机适配器，适用PCI 总线

下文CDB 数量表示可存储在 RISC 芯片缓存与板LRAM 中的 SCSI CDB（命令描述块，Command Descriptor Block）请求个数。一CDB 即一SCSI 命令。驱动的探测例程会显示每个被检测到的适配器可用的 CDB 数量。通过BIOS 中更改适配器的"Host Queue Size"（主机队列大小）设置，可以降低驱动所使用CDB 数量
...
2. ADVANSYS_DEBUG - 启用驱动调试（默认：禁用
   ...
   如果驱动在引导时加载，且系统中包含了 LILO 驱动选项，则可以通过指定5 个（ASC_NUM_IOPORT_PROBE + 1）I/O 端口来更改调试级别。伪 I/O 端口的前三位十六进制数字必须设为 `deb`，第四位十六进制数字指定调试级别 - F。以下命令行将在 0x330 处查找适配
```
...
3. ADVANSYS_STATS - 启用统计（默认：启用
   ...
   统计信息以每个适配器为单位进行维护。会维护驱动入口点调用次数与传输大小计数。统计信息仅适用于版本大于或等于 v1.3.0、且配置CONFIG_PROC_FSproc）文件系统的内核
...
## Driver LILO Option
   ...
   如果init/main.c 进行了上AdvanSys 驱动添加Linux"（B.4.）一节所述的修改，驱动将识别 `advansys` LILO 命令行选项以及 /etc/lilo.conf 选项。该选项可用于禁I/O 端口扫描，或将扫描限制为 1 - 4 I/O 端口。无论该选项如何设置，EISA PCI 板卡仍会被搜索并检测到。该选项仅影响对 ISA VL 板卡的搜索
...
```
   对于可加载模块，在加载时设置 `asc_iopflag` 变量`asc_ioport` 数组亦可达到相同效果
```

      insmod advansys.o asc_iopflag=1 asc_ioport=0x110,0x330

```
   如果定义ADVANSYS_DEBUG，可以添加一个第 5 个（ASC_NUM_IOPORT_PROBE + 1）I/O 端口来指定驱动调试级别。更多信息请参阅上文"驱动编译时选项与调一节
