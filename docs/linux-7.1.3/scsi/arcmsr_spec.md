...
本文档规定 Areca 基于 IOP331 的 RAID 控制器固件接口规范，涵盖消息机制、Doorbell/RS-232 仿真、PostQ 队列及对应的命令码与数据结构，供 RAID 驱动开发者实现主机与控制器通信时参考。


本文档描述 Areca RAID 控制器（基于 IOP331）的固件接口规范，涵盖消息机制、RS-232 仿真、PostQ 队列，以及用于 RAID 管理的 RS-232 命令码与数据结构。
...
offset 0xa00   用于入站消息码 message_rwbuffer
	       （驱动发送给 IOP331）
offset 0xa00   用于出站消息码 message_rwbuffer
	       （IOP331 发送给驱动）
============   ==========================================
...
============   =====================
1st uint32_t   数据长度（1--124）
Byte 4--127    最多 124 字节数据
============   =====================
...
GUI_SET_VENDOR
	...
	byte 0x14--0x3B   厂商字符串（必须为 40 字节）
	================  =============================================
...
GUI_SET_ETHERNET_ADDR
	...
	byte 3            password length (should be 0x08)
	byte 4-0x13       should be "ArEcAvAr"
	byte 0x14--0x19   以太网 MAC 地址（必须为 6 字节）
	================  =============================================
...
GUI_SET_LOGO
	...
	byte 3            页号（0/1/2/3）（0xff --> 清除 OEM 徽标）
...
GUI_GET_HW_MONITOR
	...
	byte 13/14 	  Voltage#0 值
	byte 15/16 	  Voltage#1 原始值
	byte 17/18 	  Voltage#1
	byte 19/20 	  Voltage#2 原始值
...
GUI_QUICK_CREATE
	...
	byte 8 		  stripe size
	byte 9 		  spare
	byte 10/11/12/13  设备掩码（用于创建 raid/volume 的设备）
	================  ==============================================
...
GUI_GET_INFO_P
	...
	byte 3            驱动器编号（从 0 到 max-channels - 1）
	================  =============================================
...
GUI_COM_PORT_SETTING
	...
	byte 7 		  校验位（0:无, 1:关, 2:偶校验）
	byte 8 		  flow control
			  (0:无, 1:xon/xoff, 2:硬件 => 必须使用无)
	================  =================================================
...
GUI_CREATE_PASS_THROUGH
	...
	byte 7 		  标记队列（1   启用）
	byte 8 		  缓存模式（1   启用）
	byte 9 		  最大速度（0/1/2/3/4，
			  scsi 下为 async/20/40/80/160）
			  （ide 下为 0/1/2/3/4，33/66/100/133/150  ）
	================  =============================================
...
GUI_IDENTIFY_DEVICE
	...
	byte 3            Flash 方式
			  （0:选择 flash, 1:未选择 flash）
	byte 4/5/6/7      待 flash 的 IDE 设备掩码
			  .. 注意:: 无可用响应数据
	================  =============================================
...
GUI_EXPAND_RAIDSET
	...
	byte 8/9/10       (8:0 无变化, 1 变化, 0xff:终止,
			  9:新 raid 级别,
			  10:新条带大小
			  0/1/2/3/4/5->4/8/16/32/64/128K )
	byte 11/12/13     对 raidset 中的每个 volume 重复
	================  =============================================
