...
对于基于 Intel IOP 的适配器，控制IOP 通过 PCI BAR0 访问
     ============== ==================================
     BAR0 offset    Register
     ============== ==================================
            0x10    Inbound Message Register 0
...
对于基于 Marvell（非 Frey）IOP 的适配器，IOP 通过 PCI BAR0 BAR1 访问
     ============== ==================================
     BAR0 offset    Register
     ============== ==================================
...
对于基于 Marvell Frey IOP 的适配器，IOP 通过 PCI BAR0 BAR1 访问
     ============== ==================================
     BAR0 offset    Register
     ============== ==================================
...
### Marvell Frey I/O 请求工作

所有排队的请求都通过入站/出站队列端口处理请求包可以在 IOP 或主机内存中分配
要向控制器发送请求：

    - 通过读取入站队列端口获取一个空闲请求包，或
      在主DMA 一致性内存中分配一个空闲请求
      从入站队列端口返回的值是一个相对于 IOP BAR0 的偏移量
      在主机内存中分配的请求必须按 32 字节边界对齐
    - 填充该包
    - 通过将包写入入站队列将其投递给 IOP。对于在 IOP 内存中分配的请求      将偏移量写入入站队列端口。对于在主机内存中分配的请求，将 (0x80000000|(bus_addr>>5))
      写入入站队列端口
    - IOP 处理该请求。当请求完成时，它将被放入出站队列。将产生一个出站中断
      对于IOP 内存中分配的请求，请求偏移量被投递到出站队列
      对于在主机内存中分配的请求，(0x80000000|(bus_addr>>5))
      被投递到出站队列。如果请求中设置IOP_REQUEST_FLAG_OUTPUT_CONTEXT 标志      则改为投递低 32 位上下文值
    - 主机读取出站队列并完成请求
      对于IOP 内存中分配的请求，主机驱动通过将其写入出站队列来释放该请求
非排队请求（reset/flush 等）可以通过入站消息寄存0 发送。带有相同值的出站消息表示
入站消息的完成
...
要向控制器发送请求：
    ...
    - 将入站列表写指针投递给 IOP
    - IOP 处理该请求。当请求完成时，带有或运算了 IOPMU_QUEUE_MASK_HOST_BITS 标志的请求将被放入一      空闲出站列表单元，并且出站列表单元的索引将被放入复制指针影子（copy pointer shadow）寄存器。将产生一个出站中断
    - 主机读取出站列表复制指针影子寄存器，并与之前保存的读指针 N 比较。如果它们不同，主机      读取(N+1) 个出站列表单元
...
Copyright |copy| 2006-2012 HighPoint Technologies, Inc. All Rights Reserved.
  ...
  本文件以“希望它有用”的方式分发  WITHOUT ANY WARRANTY（不提供任何担保）；甚至不暗示对
  MERCHANTABILITY（适销性）FITNESS FOR A PARTICULAR PURPOSE（特定用途适用性）的担保。详  GNU General Public License
