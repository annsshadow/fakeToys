...
Microchip Corp. 的新一SCSI 驱动，也是首个实PQI 队列模型SCSI 驱动
smartpqi 驱动将取Adaptec Series 9 控制器的 aacraid 驱动。使Adaptec Series 9 控制器且运行较旧
内核.9 之前）的客户必须配置 smartpqi 驱动，否则其卷将不会被添加到操作系统
要获Microchip smartpqi 控制器的支持，请在配置内核时启用 smartpqi 驱动
...
### smartpqi 主机属
  ...
  host rescan 属性是一个只写属性。向该属性写入将触发驱动扫描新增、更改或移除  设备，并通知 SCSI 中间层所检测到的任何变化
...
```
### smartpqi sas 设备属
  HBA 设备会被添加SAS 传输层。这些属性由 SAS 传输层自动添加
...
## smartpqi 专用ioctls
  ...
  CCISS_DEREGDISK, CCISS_REGNEWDISK, CCISS_REGNEWD
	上述三个 ioctl 都执行完全相同的操作，即让驱动重新扫描新设备。这与写	smartpqi 专用的主“rescan属性作用完全相同
  CCISS_GETPCIINFO
	返回 PCI 域、总线、设备和功能以及 “board ID”（PCI 子系ID）
  CCISS_GETDRIVVER
```
  ...
  CCISS_PASSTHRU
	允许“BMIC“CISS命令透传Smart Storage Array	这些命令SSA Array Configuration Utility、SNMP 存储代理等广泛使用
```
