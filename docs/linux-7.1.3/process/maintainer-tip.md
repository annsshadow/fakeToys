...
tip 树包含以下子系统
   - **x86 架构**
     ...
     x86 架构的开发在 tip 树中进行，但 x86 KVM XEN 的特定部分除外，它们     相应的子系统维护，并直接从那里合并到主线（mainline）。在 x86 特定KVM      XEN 补丁Cc x86 维护者仍然是好习惯
     除了整体x86 维护者外，一x86 子系统还有它们自己的维护者。即MAINTAINER
     文件没有特别点名，在修改 arch/x86 下文件的补丁上也Cc 整体 x86 维护者
     注意，`x86@kernel.org` 并不是一个邮件列表。它仅仅是一个邮件别名，将邮件分     x86 顶层维护者团队。请始终 Cc Linux 内核邮件列表（LKML     `linux-kernel@vger.kernel.org`，否则你的邮件只会进入维护者的私人收件箱
   - **调度器（Scheduler*

     调度器开发在 -tip 树中进行，位sched/core 分支——偶尔会有用于进行中补丁集的
     子主题树
   - **锁（Locking）与原子操作（atomics*

     锁的开发（包括原子操作以及与锁相关的其它同步原语）-tip 树中进行，位     locking/core 分支——偶尔会有用于进行中补丁集的子主题树
   - **通用中断子系统与中断芯片驱动**
     - 中断核心开发发生在 irq/core 分支

     - 中断芯片驱动开发也发生irq/core 分支，但补丁通常先应用到单独的维护者树       然后再聚合到 irq/core

   - **时间、定时器、时间保持（timekeeping）、NOHZ 以及相关芯片驱动**
     - 时间保持、clocksource 核心、NTP alarmtimer 的开发发生在 timers/core 分支       但补丁通常先应用到单独的维护者树，然后再聚合timers/core

     - clocksource/event 驱动开发发生在 timers/core 分支，但补丁大多先应用到单独       维护者树，然后再聚合timers/core

   - **性能计数器（Performance counters）核心、架构支持以及工具（tooling*
     - perf 核心和架构支持开发发生在 perf/core 分支

     - perf 工具开发发生在 perf tools 维护者树，并聚合tip 树
   - **CPU 热插拔（CPU hotplug）核*

   - **RAS 核心**

     大部x86 特定RAS 补丁被收集在 tip ras/core 分支
   - **EFI 核心**

     EFI 开发在 efi git 树中进行。收集的补丁被聚合到 tip efi/core 分支
   - **RCU**

     RCU 开发发生在 linux-rcu 树。产生的改动被聚合到 tip core/rcu 分支
   - **各种核心代码组件**
       - debugobjects
...
树中的改动之间潜在的冲突由维护者处理
##### 补丁主题（subject
tip 树偏好的补丁主题前缀格式是“subsys/component:”，例如“x86/apic:”、“x86/mm/fault:”“sched/fair:”、“genirq/core:”。请不要使用文件名或完整文件路径作为前缀。“git log
path/to/file在多数情况下应该能给你一个合理的提示
...
当变更日志中提到一个函数时（无论是在正文还是主题行中），请使用“function_name()格式。省略)”是错误的，例如
```
...
为了统一查看提交标签，tip 维护者使用以下标签排序方案：
 ...
 - Fixes: 12+字符-SHA1（“sub/sys: 原始主题行”）
   ...
   即使对于不需要回移植（backport）到稳定（stable）内核的改动，也应添Fixes 标签   即当处理一个最近引入的、只影响 tip 或主线当前头部的问题时。这些标签有助于识别
   原始提交，其价值远高于在变更日志正文中醒目地提及引入问题的提交，因为它们可   被自动提取
```
 ...
 - Signed-off-by: ``Author <author@mail>``
   ...
   在最后一Co-developed-by/SOB 对之后的第一Signed-off-by（SOB）是作SOB   即被 git 标记为作者的人
 - Signed-off-by: ``Patch handler <handler@mail>``

   作SOB 之后SOB 来自处理和传送该补丁、但未参与开发的人。SOB 链应反映补丁
   ...
   以便保留作者身份From:' 行之后必须跟一个空行。如果缺少该 'From:' 行，那么补丁
   会被归于发送（传送、处理）它的人From:' 行在补丁被应用时会被自动移除，不   出现在最终的 git 变更日志中。它仅影响最Git 提交的作者信息
...
请不要使用组合标签，例如 `Reported-and-tested-by`，因为它们只会使标签的自动提变得复杂
##### 文档链接

在变更日志中提供文档链接对日后调试和分析是极大的帮助。遗憾的是，URL 往往很快失效，因为公司频繁地重构其网站。非“易变（volatile）”的例外包括 Intel SDM AMD APM
因此，对于“易变”文档，请在 kernel bugzilla https://bugzilla.kernel.org 创建一条目，并将这些文档的副本附到bugzilla 条目上。最后，在变更日志中提供bugzilla
条目URL
##### 补丁重发或提
参见 resend_reminders
##### 合并窗口（Merge window
...
重新开放
大型补丁系列（series）应该在合并窗口开*至少**一周之前以可合并状态提交。对缺陷修复以及**有时**针对新硬件的小型独立驱动或侵入性极小的硬件支持补丁，可以有
例外
在合并窗口期间，维护者转而专注于跟踪上游改动、修复合并窗口产生的问题、收集缺修复，并让自己喘口气。请尊重这一点
所谓的*紧急（urgent分支会在每个发布版本stabilization 阶段被合并到主线
##### Git

tip 维护者接受来自维护者的 git pull 请求，这些维护者提供要tip 树中聚合的子系统
改动
针对新补丁提交的 pull 请求通常不被接受，也不能取代向邮件列表的正确补丁提交。主原因是审阅工作流程是基于邮件的
如果你提交一个较大的补丁系列，提供一个私有仓库中git 分支会很有帮助，使感兴趣
的人可以轻松拉取该系列进行测试。通常的做法是在补丁系列的封面信（cover letter）中
提供 git URL
##### 测试

代码在提交给 tip 维护者之前应当经过测试。除了微小的改动之外，任何改动都应该构建启动，并在启用了全面（且重量级）的内核调试选项的情况下进行测试
这些调试选项可以kernel/configs/x86_debug.config 中找到，并可通过运行以下命令
添加到已有的内核配置中：
...
对于旨在描述硬件或作为访问硬件的函数参数的变量，请使用适当u8、u16、u32、u64
类型。这些类型清晰地定义了位宽，并避免了截断、扩展以32/64 位混淆
在如果使用“unsigned long”会32 位内核产生歧义的代码中也推荐使用 u64。虽在这种情况下也可以使用“unsigned long long”，u64 更短，并且也清楚地表明该操作
要求64 位宽，与目标 CPU 无关
请使用“unsigned int”而不是“unsigned”
##### 常量

请不要在代码或初始化器中使用字面（十六进十进制）数字。要么使用具有描述性名的适当 define，要么考虑使用 enum
##### 结构体声明与初始化器
...
相反，请考虑在结构体声明之前的注释中使用 kernel-doc 格式，这样做更易读，并且有一个额外的好处，即把信息纳入内核文档中，例如：
...
注意，虽C99 语法允许省略最后的逗号，但我们建议在最后一行使用逗号，因为这使得
...
这也包括被立即放入全局可见驱动模板的静态文件作用域函数——对于这些符号，带上一好的前缀也很有用，以便回溯（backtrace）时可读
对于局部静态函数和变量，可以省略命名空间前缀。真正局部的函数，只被其它局部函调用，可以有更短的描述性名称——我们主要关心的是可搜索性和回溯可读性
请注意，“xxx_vendor\_”和“vendor_xxx_”前缀对于厂商特定文件中的静态函数并无帮助毕竟，代码是厂商特定的这一点已经很清楚了。此外，厂商名只应用于真正厂商特定的功能
一如既往，运用常识，以一致性和可读性为目标
### 提交通知

tip 树由一个机器人监控新提交。该机器人为每次新提交向一个专用邮件列（`linux-tip-commits@vger.kernel.org`）发送邮件，Cc 在其中一个提交标签中提及的所有人。它使用标签列表末尾 Link 标签中的邮件 Message-ID 来设In-Reply-To
