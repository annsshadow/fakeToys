...
第一步，你必须弄清楚想要调试的是哪类问题。根据答案的不同，你的方法论与
工具选择可能会有所不同。
...
在这种情况下，你的调试能力取决于所提供发行版内核的内建调试支持。
[/process/debugging/userspace_debugging_guide](/process/debugging/userspace_debugging_guide)
...
相同、不正确的输出），还是不一致地表现出来。如果它不一致地表现出来，可能
有某个时序因素在起作用。如果在代码中插入延迟确实改变了行为，那么很可能
时序是一个因素。

当时序确实改变了代码执行的输出时，使用简单的 printk() 进行调试可能无效，
一个类似的替代方案是使用 trace_printk()，它会将调试消息记录到 trace 文件
而非内核日志。
