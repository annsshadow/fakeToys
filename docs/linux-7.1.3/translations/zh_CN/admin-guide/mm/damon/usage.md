...
- **sysfs接口。**
  这 <sysfs_interface> 是为那些希望更高级的使用DAMON的特权用户空间程序员准备的。
  ...
  空间的监测。
- **内核空间编程接口。**
  [这 </mm/damon/api>](这 </mm/damon/api>) 这是为内核空间程序员准备的。使用它，用户可以通过为你编写内
  核空间的DAMON应用程序，最灵活有效地利用DAMON的每一个功能。你甚至可以为各种地址空间扩展DAMON。
  详细情况请参考接口 [文件 </mm/damon/api>](文件 </mm/damon/api>)。
...
stats section <sysfs_schemes_stats>. 将 `update_schemes_tried_regions` 写到
...
关于间隔和监测区域范围的更多细节，请参考设计文件 ([/mm/damon/design](/mm/damon/design))。
...
内存区域的详细信息，在下一个 聚集区间 <sysfs_monitoring_attrs>，相应的方案的 `动作`
