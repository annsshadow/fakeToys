...
该设备也可以通过 NAPI 进行事件轮询，而无需先接收中断信号（忙轮询<poll_zh_CN>）。

NAPI 处理通常发生在软中断上下文中，但有一个选项，可以使用 单独的内核线程<threaded_zh_CN>
来进行 NAPI 处理。
...
（更多信息请参见 drv_sched_zh_CN）。成功的 napi_schedule() 调用将获得 NAPI 实例
的所有权。
...
正如在 drv_ctrl_zh_CN 部分所提到的，napi_disable() 以及后续对轮询方法的调用，
