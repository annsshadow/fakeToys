...
consultate it_sphinx-pre-install.
...
``last row <last row>`` / last row <it last row>)
...
verrà rappresentato come **blocco testuale** (it_hello_dot_render).
...
riferimenti (it_hello_svg_render).
