...
macro `in_softirq()` (`include/linux/preempt.h`).
    ...
    se bottom half lock <it_local_bh_disable> è bloccato.
...
`printk()` fornisce messaggi alla console, dmesg, e al demone syslog.
...
```
`printk()` utilizza un buffer interno di 1K e non s'accorge di
...
`put_user()` e `get_user()` sono usate per ricevere ed
...
esse copiano una quantità arbitraria di dati da e verso lo spazio utente.
    ...
    Al contrario di`put_user()` e `get_user()`, queste
...
potete usare `vmalloc()`. Essa allocherà memoria virtuale all'interno
...
caricabili dinamicamente). L'utilizzo delle macro `module_init()`
e `module_exit()` semplifica la scrittura di codice che può funzionare
sia come modulo, sia come parte del kernel, senza l'ausilio di #ifdef.

La macro `module_init()` definisce quale funzione dev'essere
...
macro `module_init()` diventa equivalente a `__initcall()`,
...
codice del modulo, dovreste chiamare `try_module_get()` su quel modulo:
...
chiamare `module_put()` quando avete finito.
...
a questo scopo: `wait_event_interruptible()` (`include/linux/wait.h`).
...
La versione `wait_event()` ignora i segnali.
...
`atomic_t`. `atomic_read()` e `atomic_set()` leggono ed
impostano il contatore, `atomic_add()`, `atomic_sub()`,
`atomic_inc()`, `atomic_dec()`, e
`atomic_dec_and_test()` (ritorna vero se raggiunge zero dopo essere
stata decrementata).
...
`set_bit()`, `clear_bit()` e `change_bit()`
impostano, cancellano, ed invertono il bit indicato.
`test_and_set_bit()`, `test_and_clear_bit()` e
`test_and_change_bit()` fanno la stessa cosa, ad eccezione che
...
Utilizzate `ERR_PTR()` (`include/linux/err.h`) per codificare
un numero d'errore negativo in un puntatore, e `IS_ERR()` e
`PTR_ERR()` per recuperarlo di nuovo: così si evita d'avere un
...
per avermi fornito un ottimo riassunto di `disable_irq()`,
