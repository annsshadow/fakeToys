...
- **sysfs接口。**
  這 <sysfs_interface> 是爲那些希望更高級的使用DAMON的特權用戶空間程序員準備的。
  ...
  空間的監測。
- **內核空間編程接口。**
  [這 </mm/damon/api>](這 </mm/damon/api>) 這是爲內核空間程序員準備的。使用它，用戶可以通過爲你編寫內
  核空間的DAMON應用程序，最靈活有效地利用DAMON的每一個功能。你甚至可以爲各種地址空間擴展DAMON。
  詳細情況請參考接口 [文件 </mm/damon/api>](文件 </mm/damon/api>)。
...
stats section <sysfs_schemes_stats>. 將 `update_schemes_tried_regions` 寫到
...
關於間隔和監測區域範圍的更多細節，請參考設計文件 ([/mm/damon/design](/mm/damon/design))。
...
內存區域的詳細信息，在下一個 聚集區間 <sysfs_monitoring_attrs>，相應的方案的 `動作`
