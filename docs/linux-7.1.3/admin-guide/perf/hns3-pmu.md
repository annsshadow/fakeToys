...
```
PMU 驱动在 sysfs 中提供可用事件、过滤模式、格式、标识符（identifier）和 cpumask 的
描述。

"events" 目录描述了 perf list 中显示的所有受支持事件的事件码。

"filtermode" 目录描述了每个事件所支持的过滤模式。

"format" 目录描述了 perf_event_attr 结构的 config（事件）和 config1（过滤选项）
字段的所有格式。

"identifier" 文件显示 PMU 硬件设备的版本。

"bdf_min" 和 "bdf_max" 文件显示每个 pmu 设备所支持的 bdf 范围。
...
config 的 0~15 位是真正的硬件事件码。如果两个事件的 config 的 0~15 位取值相同，
就表示它们是一对事件。而 config 的第 16 位表示获取硬件事件计数器 0 还是计数器 1。
...
1. global 模式
PMU 收集 IO DIE 的所有 HNS3 PCIe 功能的性能统计信息。将 "global" 过滤选项设为 1
即可启用此模式。
```
...
```
2. port 模式
PMU 收集整个一个物理端口的性能统计信息。端口 id 与 mac id 相同。在此模式下，"tc"
过滤选项必须设为 0xF，这里 tc 代表流量类别（traffic class）。
```
...
```
3. port-tc 模式
PMU 收集物理端口某一个 tc 的性能统计信息。端口 id 与 mac id 相同。在此模式下，"tc"
过滤选项必须设为 0 ~ 7。
```
...
```
4. func 模式
PMU 收集一个 PF/VF 的性能统计信息。功能 id 是 PF/VF 的 BDF，其中
```
...
```
5. func-queue 模式
PMU 收集一个 PF/VF 的某一个队列的性能统计信息。功能 id 是 PF/VF 的 BDF，"queue"
过滤选项必须设为该功能确切的队列 id。
```
...
```
6. func-intr 模式
PMU 收集一个 PF/VF 的某一次中断的性能统计信息。功能 id 是 PF/VF 的 BDF，"intr"
过滤选项必须设为该功能确切的中断 id。
```
