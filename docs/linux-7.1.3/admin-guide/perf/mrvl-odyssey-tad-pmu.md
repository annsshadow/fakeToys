...
每个 TAD 提供八个 64 位计数器用于监控缓存行为。驱动始终为所有 TAD 配置相同的计数器。用户最终会有效地在每个 TAD 中保留八个计数器之一，以便跨所有 TAD 进行观察。
事件的发生次数会被聚合，并在工作负载运行结束时呈现给用户。驱动没有提供让用户对 TAD 进行分区以便不同 TAD 用于不同应用程序的方法。
