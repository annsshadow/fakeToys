...
实现了一组 4 x 32 位的计数器。这组计数器由在计数器控制寄存器中编程的 CSV
模式控制，从而会产生大量的 PERF 信号。

每个计数器的值通过 config 寄存器进行选择。每个计数器对应一个寄存器。计数器 0
比较特殊，它总是计数“时间”，并在到期时对自身以及其它计数器加锁，并触发一个
中断。如果任何其它计数器溢出，它会继续计数，且不会触发中断。

"format" 目录描述了 perf_event_attr 结构的 config（事件 ID）以及 config1/2
（AXI 过滤器设置）字段的格式，参见 /sys/bus/event_source/devices/imx8_ddr0/format/。
"events" 目录描述了可与 perf 工具配合使用的、硬件支持的事件类型，参见
/sys/bus/event_source/devices/imx8_ddr0/events/。"caps" 目录描述了 DDR PMU
...
与过滤设置相匹配的读或写操作。过滤设置因不同的 DRAM 控制器实现而异，这由驱动
中的 quirks 来区分。你也可以从用户空间转储信息，"caps" 目录会显示 AXI 过滤器
的类型（filter、enhanced_filter 和 super_filter）。值 0 表示不支持，值 1 表示
支持。

- With DDR_CAP_AXI_ID_FILTER quirk(filter: 1, enhanced_filter: 0, super_filter: 0).
  该 quirk 通过以下两部分配置来定义过滤器：
  ...
  AXI_ID 和 AXI_MASKING 被映射到性能计数器中的 DPCR1 寄存器。当非屏蔽位与
  相应的 AXI_ID 位匹配时，计数器为
```
...
- With DDR_CAP_AXI_ID_FILTER_ENHANCED quirk(filter: 1, enhanced_filter: 1, super_filter: 0).
  这是对 DDR_CAP_AXI_ID_FILTER quirk 的扩展，它允许在与另一组数据计数器并发的
  情况下，计数来自 DDR 读写事务的字节数（而非突发次数）。

- With DDR_CAP_AXI_ID_PORT_CHANNEL_FILTER quirk(filter: 0, enhanced_filter: 0, super_filter: 1).
  先前的 AXI 过滤器存在限制，由于过滤器在计数器间共享，它无法同时过滤不同的 ID。
  该 quirk 是 AXI ID 过滤器的扩展。一处改进是计数器 1-3 拥有各自的过滤器，意味着
  它支持并发过滤不同的 ID。另一处改进是计数器 1-3 支持 AXI PORT 和 CHANNEL 选择，
  支持选择地址通道或数据通道。
  ...
  DDR 子系统中的 PMU 仅存在单一的 port0，因此 axi_port 被保留，应为 0。
