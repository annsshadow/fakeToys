...
本文介绍如何检查某个 Linux 内核问题是否出现在开发者当前维护的代码中——并进一步说明，若该问题属于回归（例如早期版本中并未出现），应如何定位导致该问题的变更。

正文主要面向在普通硬件上运行主流 Linux 发行版内核、并希望向上游 Linux 开发者报告内核缺陷的用户。尽管如此，这些说明同样适用于已经熟悉自行构建内核的用户：它们有助于避免即使是经验丰富的开发者偶尔也会犯下的错误。
...
*[如果你是初次构建内核或对其进行二分定位，请忽略本节，直接前往下方的 'step-by-step guide <introguide_bissbs>'。本节使用与下文相同的命令，只是描述更为简略；不过这些步骤依然易于遵循，并且参考章节中的相关条目还提到了许多替代方案、陷阱以及其他注意事项，在你当前的情况下这些可能都至关重要。]*

**如果你想检查某个缺陷是否存在于开发者当前维护的代码中**，只需执行 **准备工作（preparations）** 与 **第 1 段（segment 1）**；在此过程中，把你日常使用的、最新的 Linux 内核视为“可用（working）”内核。下面的示例假设该内核为 6.0，因此将使用它的源码来准备 .config 文件。

**如果你遇到的是一个回归**，请至少执行到 **第 2 段（segment 2）** 结束。随后你可以提交一份初步报告——也可以继续 **第 3 段（segment 3）**，其中说明了如何执行一份完整的回归报告所需的二分定位。下面的示例假设 6.0.13 为“可用（working）”内核、6.1.5 为第一个“损坏（broken）”内核，因此将把 6.0 视为“良好（good）”版本并用于准备 .config 文件。
...
```
- **第 1 段（Segment 1）**：基于最新的 mainline 代码库构建一个内核。

  这除了可以检查问题是否已经被修复之外，还能告诉后续需要知会哪些开发者；在回归的情况下，这一步还能排除问题是因 .config 变更而起的可能。
...
```
- **第 2 段（Segment 2）**：确保“良好（good）”内核同时也是“可用（working）”内核。
...
- **辅助任务（Complementary tasks）**：在流程进行期间及之后进行清理。
  ...
  a) 为了避免在二分定位过程中耗尽磁盘空间，你可能需要删除一些之前构建的内核。你很可能希望将第 1 段和第 2 段期间构建的内核保留一段时间，但在实际二分定位过程中测试过的内核，你多半不再需要它们。
...
本指南介绍如何搭建你自己的 Linux 内核，以调查你打算报告的缺陷或回归。你想在多大程度上遵循这些说明，取决于你遇到的问题：

执行到 **第 1 段（segment 1）** 结束，以 **验证你的内核问题是否出现在 Linux 内核开发者维护的代码中**。如果是，你就可以准备报告该缺陷了——除非它在更早的内核版本中并未发生，那样你就至少应当继续 **第 2 段（segment 2）** 以 **检查该问题是否符合回归（regression）的定义**，回归会得到优先处理。根据结果，你就可以准备报告缺陷或提交一份初步的回归报告；与其提交后者，你也可以直接继续 **第 3 段（segment 3）** 来 **执行二分定位**，以获得一份开发者有义务处理的完整回归报告。

 Preparations: 搭建一切以构建你自己的内核 <introprep_bissbs>.

 Segment 1: 用最新的代码库尝试复现问题 <introlatestcheck_bissbs>.

 Segment 2: 检查你构建的内核是否工作正常 <introworkingcheck_bissbs>.

 Segment 3: 执行二分定位并验证结果 <introbisect_bissbs>.

 Complementary tasks: 在遵循本指南期间及之后的清理工作 <introclosure_bissbs>.

 Optional tasks: 测试 revert、补丁或更新的版本 <introoptional_bissbs>.

每个段落中的步骤说明了流程的重要方面，而一份详尽的参考章节为几乎所有步骤都提供了更多细节。参考章节有时还会列出替代方案、陷阱，以及在该特定步骤可能出现的问题——以及如何让事情重新回到正轨。

关于如何报告 Linux 内核问题或回归的更多细节，请参阅 Documentation/admin-guide/reporting-issues.rst，它与本文档配合使用。其中特别解释了为什么即使你面对的是来自“stable/longterm”系列（例如 6.0.13）的内核问题，也需要用最新的“mainline”内核（例如 6.0、6.1-rc1 或 6.1-rc6 等版本）来验证缺陷。

对于遇到回归的用户，该文档还解释了为什么在第 2 段之后提交一份初步报告是明智的，因为该回归及其 culprit 可能已经被知晓。关于究竟什么才算回归的更多细节，请参阅 Documentation/admin-guide/reporting-regressions.rst。

如果你在遵循本指南时遇到任何问题，或者有好点子来改进它，请告知内核开发者 <submit_improvements_vbbr>。
...
- 移除所有依赖外部开发的内核驱动、或会在启动时自动构建它们的软件。这包括但不限于 DKMS、openZFS、VirtualBox，以及 Nvidia 的图形驱动（包括其 GPL 许可的内核模块）。

  [details <vanilla_bisref>]


- 在带有“Secure Boot”或类似机制的平台上，准备好一切，确保系统允许你自行编译的内核启动。在普通 x86 系统上，最快捷简便的方法是在 BIOS 设置工具中禁用此类机制；或者，通过由 `mokutil --disable-validation` 发起的流程来解除其限制。
...
- 确定贯穿本指南被视为“良好（good）”和“损坏（bad）”的内核版本：
  ...
  - 你遵循本指南是想验证某个缺陷是否出现在主要开发者所关注的代码中？那么把你当前日常使用的最新内核版本视为“良好（good）”（例如 6.0、6.0.13 或 6.1-rc2）。

  - 你遇到了回归，例如在切换到较新的内核版本后，某些功能损坏或表现变差？这种情况下取决于问题出现时的版本范围：

    - 在从某个 stable/longterm 版本（例如 6.0.13）更新到更新的 mainline 系列（如 6.1-rc7 或 6.1），或基于它的 stable/longterm 版本（如 6.1.5）时发生了回归？那么把你可用内核所基于的 mainline 版本视为“良好（good）”版本（例如 6.0），并将第一个损坏的版本视为“损坏（bad）”版本（例如 6.1-rc7、6.1 或 6.1.5）。注意，此时仅仅是假设 6.0 没问题；这一假设将在第 2 段中检验。

    - 在从一个 mainline 版本（例如 6.0）切换到更新的版本（如 6.1-rc1）或基于它的 stable/longterm 版本（如 6.1.5）时发生了回归？那么将最后一个可用版本（例如 6.0）视为“良好（good）”，将第一个损坏版本（例如 6.1-rc1 或 6.1.5）视为“损坏（bad）”。

    - 在 stable/longterm 系列内部更新时（例如从 6.0.13 到 6.0.15）发生了回归？那么将这些版本视为“良好（good）”和“损坏（bad）”（例如 6.0.13 和 6.0.15），因为你需要在该系列内部进行二分定位。
...
- 确保有足够的可用空间来构建 Linux。主目录下 15 GB 通常已经足够。如果你可用空间更少，请务必留意后续关于获取 Linux 源码和处理调试符号的步骤：两者都介绍了能减少空间占用的办法，应当能让你在约 4 GB 可用空间的情况下完成这些任务。

  [details <diskspace_bisref>]


- 安装构建 Linux 内核所需的所有软件。通常你会需要：'bc'、'binutils'（'ld' 等）、'bison'、'flex'、'gcc'、'git'、'openssl'、'pahole'、'perl'，以及 'libelf' 和 'openssl' 的开发头文件。参考章节展示了如何在多种流行的 Linux 发行版上快速安装它们。
...
- 获取 mainline Linux 源代码；然后进入存放这些代码的目录，因为本指南后续所有命令都应从该目录执行。
  ...
  *注意，下面介绍的是通过完整的 mainline 克隆来获取源代码，截至 2024 年初其下载量约为 2.75 GB。* 参考小节介绍了两种替代方案 <sources_bisref> *：一种下载量不到 500 MB，另一种更适合网络不稳定的连接。*
...
- 你之前确定的“良好（good）”或“损坏（bad）”版本中，是否有一个是 stable 或 longterm 发行版（如 6.1.5）？那么，请下载它所属系列的源代码
```
...
- 开始准备内核构建配置（即 '.config' 文件）。

  在此之前，请确认你仍在运行早前步骤要求你启动的“可用（working）”内核；如果不确定，可用 `uname -r` 查看当前的 kernelrelease 标识符。

  之后，检出早前确定为“良好（good）”的版本对应的源代码。下面的示例命令假定该版本为 6.0；请注意，本命令及后续所有 Git 命令中的版本号都需要加上前缀
```
...
- 禁用那些对你的配置而言明显多余的任意内核模块。这一步是可选的，但对于二分定位尤其明智，因为它能极大地加快构建过程——除非上一步取得的 .config 文件已经针对你和你的硬件需求做了定制，那样的话你应跳过此步。
  ...
  为准备精简，请连接你偶尔使用的外部硬件（USB 密钥、令牌等），快速启动一个 VM，并启用 VPN。如果你在开始遵循本指南后重启过，请确保已经尝试使用那个导致
```
...
- 检查你是否想要或需要调整其他一些内核配置选项：
  ...
  - 你使用的是 Debian 吗？那么，你可能希望通过执行参考小节中介绍的额外调整来避免已知问题。

    [details <configmods_distros_bisref>].

  - 如果你想影响配置的其他方面，现在就用你喜欢的工具去做。注意，要使用 'menuconfig' 或 'nconfig' 这样的 make 目标，你需要安装 ncurses 的开发文件；对于 'xconfig'，你同样需要 Qt5 或 Qt6 的头文件。

    [details <configmods_individual_bisref>].

- 在最新调整之后重新处理 .config，并将其保存在安全的
```
...
以下步骤用于确认问题是否出现在开发者当前维护的代码中。如果你遇到的是回归问题，它还能确认问题不是由某些 .config 变更引起的，否则报告该问题就是在浪费时间。 [details <introlatestcheck_bisref>]


- 检出最新的 Linux 代码库。

  - 你的“良好（good）”和“损坏（bad）”版本是否来自同一个 stable 或 longterm 系列？那么请查看 `kernel.org 首页 <https://kernel.org/>`_：如果它列出了该系列中一个不带“[EOL]”标签的发行版，就检出该系列
```
...
- 你刚构建的是 stable 或 longterm 内核吗？并且你能否用它复现该回归？那么你也应当测试最新的 mainline 代码库，因为结果决定了该缺陷必须提交给哪些开发者。
```
...
你遵循本指南是为了验证某个问题是否存在于 Linux 内核开发者当前维护的代码中吗？那么到此你就完成了。如果你之后想删除刚构建的内核，请参阅“补充任务：遵循本指南期间及之后的清理工作 <introclosure_bissbs>”。

如果你遇到的是回归问题，请继续并至少执行下一段。


### 第 2 段：检查你构建的内核是否工作正常
...
在完成了所有准备工作和预防性构建之后，你现在可以开始二分定位了。这会让你构建相当多的内核——通常约为 15 个，如果你是在更新到较新系列（如从 6.0.13 到 6.1.5）时遇到的回归。但不用担心，由于早前创建的精简构建配置，这个过程比许多人想象的要快得多：在普通 x86 机器上，平均来说编译每个内核通常只需约 10 到 15 分钟。


- 开始二分定位，并告知 Git 早前确定的版本
```
...
- 在以下操作之前，先将 Git 的二分日志与当前的 .config 文件保存在安全的地方
```
...
- 尝试在最新 mainline 之上回退罪魁祸首，看是否能修复你的回归。
...
- 一旦完成二分定位，不要立即删除你搭建的任何东西，因为你可能还需要用到其中一些。哪些可以安全删除，取决于二分定位的结果：
  ...
  - 你最初能否用最新代码库复现该回归，并且在二分定位后通过回退最新代码库之上的罪魁祸首修复了问题？那么你可能想把那两个内核保留一段时间，但安全地删除所有发行标识符中包含“-local”的其他内核。
...
如果有上述任何情况，请通过发送简短说明或补丁给 Thorsten Leemhuis <linux@leemhuis.info>，并最好抄送公开的 Linux 文档邮件列表 <linux-doc@vger.kernel.org>，让开发者知晓。这样的反馈对进一步改进本文至关重要，也符合所有人的利益，因为它能让更多人掌握此处描述的任务。
...
#### 应对 Secure Boot 之类的机制


  *在启用了“Secure Boot”或类似机制的平台（如 commodity x86）上，请确保系统稍后允许你自行编译的内核启动，并做好一切准备。* [... <secureboot_bissbs>]

许多现代系统只允许某些特定的操作系统启动；这正是它们默认拒绝启动自行编译的内核的原因。

理想情况下，你应通过证书让你的平台信任你自行构建的内核，从而解决这个问题。具体做法此处不展开，因为那需要多个步骤，会使本文偏离主题；'Documentation/admin-guide/module-signing.rst' 以及各种网页 already 更详细地说明了所需的一切。

临时禁用 Secure Boot 之类的机制，是让你自己的 Linux 启动的另一种办法。在普通 x86 系统上，可以在 BIOS 设置中完成；所需步骤因机器而异，因此此处无法详述。

在主流 x86 Linux 发行版上，还有第三种通用的办法：为你的 Linux 环境禁用所有 Secure Boot 限制。你可以通过运行 `mokutil --disable-validation` 来启动这一过程；它会提示你创建一个一次性密码，记下来是安全的。现在重启；在 BIOS 完成所有自检后，引导加载程序 Shim 会显示一个蓝色方框，提示“Press any key to perform MOK management”。在倒计时结束前按下任意键，即可打开一个菜单。选择“Change Secure Boot state”。Shim 的“MokManager”会要求你输入之前设定的一次性密码中的三个随机字符。输入后，确认你确实想要禁用该验证。之后，允许 MokManager 重启机器。
...
If you have space constraints, be sure to hay attention to the :ref:`关于调试符号的步骤 <debugsymbols_bissbs>` and its :ref:` accompanying 参考小节 <debugsymbols_bisref>`, as disabling then will reduce the consumed disk space by quite a few gigabytes.
...
确定待检查的提交范围通常很直接，除非回归发生在从一个 stable 系列的发行版切换到较新系列的发行版时（如从 6.0.13 到 6.1.5）。这种情况下 Git 需要一些引导，因为没有一条直系的继承线。

这是因为随着 6.0 的发布，mainline 推进到了 6.1，而 stable 系列 6.0.y 则分叉到了一旁。因此从理论上讲，你在 6.1.5 上遇到的问题可能只在 6.0.13 中正常，因为它是由进入某个 6.0.y 发行版的提交修复的，但从未进入 mainline 或 6.1.y 系列。所幸，由于 stable/longterm 维护者维护代码的方式，这种情况通常不会发生。因此，将 6.0 假设为“良好（good）”内核是相当安全的。不过这个假设无论如何都会被检验，因为该内核将在本指南的第 2 段中被构建和测试；如果你尝试在 6.0.13 与 6.1.15 之间进行二分，Git 也会强制你这样做。
...
内核相当独立，但除了编译器之类的工具外，有时你还需要几个库才能构建它。如何安装所需的一切，取决于你的 Linux 发行版以及你即将构建的内核的配置。
...
这些命令会安装一些通常（但并非总是）需要的软件包。例如，你可能想跳过安装 ncurses 的开发头文件，因为只有在以后想用 'menuconfig' 或 'nconfig' 这些 make 目标来调整内核构建配置时才需要它们；同样，如果你不打算用 'xconfig' 调整 .config，也可以省略 Qt6 的头文件。

此外，对于本指南未涵盖的任务——例如从内核的 tools/ 目录构建工具时——你可能还需要额外的库及其开发头文件。
...
分步指南介绍了如何通过 Linus 的 mainline 仓库的完整 Git 克隆来下载 Linux 源代码。关于这一点没什么更多可说的——但还有两种替代的获取方式，可能对你更合适：

- 如果你的网络连接不稳定，可以考虑使用 'Git bundle'<sources_bundle_bisref>。

- 如果下载完整的仓库耗时太久或需要过多存储空间，可以考虑 :ref:`使用 'shallow clone'（浅克隆）<sources_shallow_bisref>`。


###### 使用 bundle 下载 Linux mainline 源代码
...
```
现在将克隆的历史加深到你“良好（good）”版本对应 mainline 发行版的上上个版本。如果后者是 6.0 或 6.0.13，那么 5.19 是上一个版本、5.18 是上上个版本——因此将历史加深到
```

    git fetch --shallow-exclude=v5.18 mainline

```
之后，按照分步指南中的说明，将 stable Git 仓库添加为远程，并添加所有需要的 stable 分支。

注意，浅克隆有几个特殊之处：

- 对于二分定位，历史需要比看起来必要的程度再加深几个 mainline 版本，如前所述。这是因为否则 Git 将无法回退或描述某个范围内的多数提交（如 6.1..6.2），因为它们在内部基于更早的内核发行版（如 6.0-rc2 或 5.19-rc3）。

- 本文档在大多数地方使用带有 `--shallow-exclude=` 的 `git fetch` 来指定你关心的最早版本（准确地说：是它的 git 标签）。你也可以改用 `--shallow-since=` 参数，指定一个绝对日期（如 `'2023-07-15'`）或相对日期（如 `'12 months'`）来定义想要下载的历史深度。在对 mainline 进行二分时，请确保将历史至少加深到你“良好（good）”内核所基于的 mainline 发行版发布前 7 个月。

- 警告：在加深克隆时，你可能会遇到类似“fatal: error in object: unshallow cafecaca0c0dacafecaca0c0dacafecaca0c0da”的错误。这种情况下请运行 `git repack -d` 并重试。
...
*注意，这是本指南中创建或修改构建产物的多个步骤中的第一步。本指南中的命令为了简单起见，将这些产物直接存放在源代码树中。如果你更愿意把构建产物单独存放，可以创建一个类似“~/linux-builddir/”的目录，并在本指南所有 make 调用中加入参数 `O=~/linux-builddir/`。你还需要让其他命令也指向该目录——其中包括 ``./scripts/config [...]`` 命令，它们需要 `--file ~/linux-builddir/.config`` 才能找到正确的构建配置。*
...
有时，把一个为某个内核（如 6.1）准备的配置文件用于更旧的 mainline 发行版时，会发生奇怪的事情——尤其是当后者旧得多时（如 5.15）。这也是指南中上一步让你启动一切正常的内核的原因之一。因此，如果你手动添加 .config 文件，务必确保它来自工作正常的内核，而不是来自出现回归的那个。

如果你想为另一台机器构建内核，请找到它的内核构建配置；通常 `ls /boot/config-$(uname -r)` 会打印出它的名称。将该文件复制到构建机器上，并保存为 ~/linux/.config；之后运行 `make olddefconfig` 进行调整。
...
正如分步指南中已经简要说明的：使用 localmodconfig 时，很容易出现你自行构建的内核缺少某些模块的情况，这些模块对应的任务你在使用该 make 目标前至少执行过一次。这是因为某些任务依赖的内核模块只有在你首次执行该任务时才会自动加载。所以，如果你自启动内核以来从未执行过该任务，这些模块就不会被加载——在 localmodconfig 看来它们显得多余，于是便被禁用，从而减少需要编译的代码量。

你可以通过执行那些通常会自动加载额外内核模块的典型任务来尽量避免这一问题：启动一个 VM、建立 VPN 连接、回环挂载 CD/DVD 的 ISO、挂载网络共享（CIFS、NFS 等），并连接所有外部设备（2FA 密钥、头戴设备、网络摄像头等）以及你平时不使用的文件系统的存储设备（btrfs、ext4、FAT、NTFS、XFS 等）。但很难想到所有可能需要的全部东西——即便内核开发者在这一步也常常忘掉这或那。

不要被这种风险困扰，尤其是仅为测试目的而编译内核时：所有通常关键的东西都会在那里。而且如果你忘了某些重要内容，以后可以手动开启缺失的功能，并快速重新运行命令，编译并安装一个具备你所需一切的内核。

但如果你打算定期构建并使用自行编译的内核，可以通过记录你的系统在几周内加载了哪些模块来降低风险。你可以用 `modprobed-db <https://github.com/graysky2/modprobed-db>`_ 将这一过程自动化。之后使用 `LSMOD=<path>` 来
```

  yes '' | make LSMOD='${HOME}'/.config/modprobed.db localmodconfig

```
如果你复制了一份合适的 .config 作为基础（见上一步），该参数也允许你为另一台机器构建精简内核。只需在那台系统上运行 `lsmod > lsmod_foo-machine`，并将生成的文件复制到你的构建主机的主目录。然后运行以下命令，而不是
```
...
这能让你将自己发行版的内核与本过程中创建的内核区分开来，因为后者的文件或目录名称中会包含“-local”；它还有助于在启动菜单中挑选正确的条目，并避免混淆你构建的内核——因为在二分定位期间，它们的版本号看起来会有些混乱。
...
当你的内核在后续运行中抛出“panic”、“Oops”、“warning”或“BUG”时，拥有调试符号可能很重要，因为那样你就能找到问题中代码确切发生的位置。但收集和嵌入所需的调试信息需要时间，并且会消耗相当多的空间：在 2022 年底，用 localmodconfig 精简的典型 x86 内核，启用调试符号时构建产物约为 5 GB，而禁用时不到 1 GB。生成的内核镜像与模块也会更大，从而增加 /boot/ 的存储需求和加载时间。
...
此时你可以使用 `make menuconfig` 或 `make nconfig` 这样的命令，通过基于文本的用户界面来启用或禁用某些功能；若要使用图形化配置工具，则运行 `make xconfig`。两者都需要其所依赖工具包（分别是 ncurses 以及 Qt5 或 Qt6）的开发库；如果缺少所需内容，会出现错误消息提示你。
...
#### 将 .config 文件妥善收好
  ...
  **在最新更改之后重新处理 .config，并将其保存在安全的地方。**
  [... <saveconfig_bissbs>]
...
对某些读者来说，此时检查最新代码库可能显得没有必要，尤其是如果你已经用发行版提供的内核做过，或者遇到的是 stable/longterm 系列内部的回归。但在以下这些理由下，我们强烈建议这样做：

- 你会在真正开始二分定位之前，就遇到由你的环境引起的任何问题。这将让你很容易区分“这很可能是我环境里的某个问题”与“这次变更需要在二分过程中跳过，因为该阶段的源代码含有一个不相关的问题，导致构建或启动失败”。

- 这些步骤能排除你的问题是否由“可用（working）”内核与“损坏（broken）”内核之间构建配置的某些变更引起。例如，当你的发行版在新内核中启用了某个额外的安全特性，而旧内核中该特性被禁用或尚不支持时，就可能出现这种情况。该安全特性可能会妨碍你做的某些事情——这种情况下，从 Linux 内核上游开发者的角度看，你的问题并不构成回归，正如 Documentation/admin-guide/reporting-regressions.rst 中更详细解释的那样。因此，如果你去二分它，就是在浪费时间。

- 如果你回归的成因在最新 mainline 代码库中已经被修复，那么你的二分定位就白做了。这一点对于你在 stable/longterm 发行版中遇到的回归同样成立，因为它们往往是由被回移植（backport）的 mainline 变更中的问题引起的——这种情况下，问题必须先在 mainline 中修复。也许它已经在那里被修复，并且修复正在被回移植的过程中。

- 此外，对于 stable/longterm 系列内部的回归，至关重要的是弄清该问题是否特定于该系列，还是在 mainline 内核中也会出现，因为报告需要发送给不同的人：

  - 特定于某个 stable/longterm 系列的回归由 stable 团队负责；mainline 的 Linux 开发者可能会在意，也可能不会。

  - 在 mainline 中也出现的回归，则是由常规的 Linux 开发者与维护者负责处理；stable 团队不关心，也不需要参与报告，只需要在修复就绪时被告知去回移植它。
...
在这个阶段很多事情都可能出错，但下面的说明能帮你自助解决。另一个小节介绍了如何直接将内核打包成 deb、rpm 或 tar 文件。

###### 处理构建错误


当构建错误发生时，它可能是由你机器环境的某些方面引起的，这种情况通常能快速修复；但有时问题出在代码中，只能由开发者修复。仔细查看失败信息，再结合在网上做一些调研，通常能告诉你属于哪一种情况。要进行这样的调查，请重新启动构建
```

  make V=1

```
`V=1` 会启用详细输出，这可能是查看真实错误所必需的。为了让错误更容易被发现，这条命令还省略了早前用于让系统所有 CPU 核心都参与该任务的 ``-j $(nproc --all)``——但这种并行性在出错时也会带来一些混乱。

几秒钟后，构建过程应该会再次遇到该错误。现在试着找出描述该问题最关键的那一行。然后在网上搜索该行中最重要、最不通用的一段（比如 4 到 8 个单词）；避免或去掉任何看起来与特定系统相关的内容，比如你的用户名或像 `/home/username/linux/` 这样的本地路径名。先用你常用的搜索引擎搜这个字符串，然后再通过 `lore.kernel.org/all/ <https://lore.kernel.org/all/>`_ 搜索 Linux 内核邮件列表。
...
分步指南使用默认的 make 目标（在 x86 上为 'bzImage' 和 'modules'）来构建内核的镜像与模块，随后由指南中的后续步骤安装。你也可以改用以下目标之一，直接构建所有内容并直接打包：

- `make -j $(nproc --all) bindeb-pkg` 以生成 deb 包

- `make -j $(nproc --all) binrpm-pkg` 以生成 rpm 包

- `make -j $(nproc --all) tarbz2-pkg` 以生成 bz2 压缩的 tar 包

这里只是为此目的提供的部分 make 目标，其他目标请见 `make help`。你也可以在运行 `make -j $(nproc --all)` 之后再使用这些目标，因为它们会拾取已经构建好的所有内容。

如果你使用这些目标来生成 deb 或 rpm 包，请忽略分步指南中关于安装和卸载内核的说明；改为使用对应格式的包管理工具（如 dpkg 和 rpm），或构建在它们之上的包管理工具（apt、aptitude、dnf/yum、zypper 等）来安装和卸载这些包。请注意，用这两个 make 目标生成的包旨在适用于使用这些格式的各种发行版，因此有时它们的行为会与你发行版的内核包有所不同。
...
在分步指南中执行命令之后你需要做什么，取决于你的发行版上是否存在 `/sbin/installkernel` 可执行文件，以及它的实现方式。

如果找到了 installkernel，内核的构建系统会把内核镜像的实际安装工作委托给这个可执行文件，它会执行以下部分或全部任务：

- 在几乎所有 Linux 发行版上，installkernel 都会把你的内核镜像存入 /boot/，通常名为“/boot/vmlinuz-<kernelrelease_id>”；通常它还会在旁边放一个“System.map-<kernelrelease_id>”。

- 在大多数发行版上，installkernel 随后会生成一个“initramfs”（有时也叫“initrd”），通常存储为“/boot/initramfs-<kernelrelease_id>.img”或“/boot/initrd-<kernelrelease_id>”。普通发行版依赖这个文件来启动，因此务必先执行 make 目标“modules_install”，否则你发行版的 initramfs 生成器将无法找到打包进镜像所需的模块。

- 在某些发行版上，installkernel 还会为你的内核在引导加载程序的配置中添加一个条目。

如果你的发行版缺少 installkernel 脚本，或只处理了其中一部分任务，你就必须自己完成部分或全部任务。详情请查阅发行版的文档。如果拿不准，可以安装
```
...
二分定位过程中构建的内核会在 /boot/ 和 /lib/modules/ 下占用相当多的空间，尤其是当你启用了调试符号时。这使得在二分过程中很容易把卷填满——以至于连早先还能正常工作的内核都可能无法启动。为避免这种情况，你需要知道每个已安装内核通常需要多大空间。

注意，多数情况下本指南中使用的模式“/boot/**$(make -s kernelrelease)**”会匹配启动内核所需的所有文件——但路径和命名方案都不是强制性的。因此在某些发行版上，你需要到不同的位置去查找。
...
当发生某些可能导致后续看似完全无关的错误的事情时，Linux 会将自己标记为 tainted（被污染）。这就是为什么开发者可能会忽略或草率回应来自被污染内核的报告——当然，除非内核正是在所报告缺陷发生的那一刻设置了该标志。

因此，你应参照 Documentation/admin-guide/tainted-kernels.rst 中的说明，去查明内核为何被污染；这样做也符合你自己的利益，否则你的测试可能有问题。

[back to step-by-step guide <tainted_bissbs>]


#### 检查基于最新 mainline 代码库构建的内核
...
- 如果你是用 stable/longterm 内核遇到的该回归，那么它可能是特定于该系列的问题；本指南的下一步会对此进行检查。

[back to step-by-step guide <recheckbroken_bissbs>]


#### 检查基于最新 stable/longterm 代码库构建的内核


  *你是否正面临某个 stable/longterm 发行版内部的回归，却未能用你刚用最新 mainline 源代码构建的内核复现它？那么请检查该特定系列的最新代码库是否已经修复了这个问题。* [... <recheckstablebroken_bissbs>]
...
- 检查 taint 状态以及 `dmesg` 的输出，也许是某个不相关的问题出错了。

- 也许 localmodconfig 做了些奇怪的事，禁用了测试该功能所需的模块？那你可能需要基于最后一个工作正常的内核的 .config 重新创建一个配置文件，并跳过精简；在 .config 中手动禁用某些功能同样可能奏效，并能减少构建时间。

- 也许这根本不是内核回归，而是由某些偶然因素、损坏的 initramfs（也叫 initrd）、新的固件文件，或更新后的用户态软件引起的？

- 也许那是你发行版内核中添加的某个功能，而当时的 vanilla Linux 从未支持过？

注意，如果你发现并修复了 .config 文件的问题，你会想用它从最新代码库再构建一个内核，因为你早前对 mainline 以及某个受影响 stable/longterm 系列的最新版本所做的测试，很可能都是有问题的。
...
#### 开始二分定位
  ...
  *开始二分定位，并告知 Git 早前确定的“良好（good）”与“损坏（bad）”版本。* [... <bisectstart_bissbs>]

这将启动二分定位过程；最后一条命令会让 Git 检出位于“良好（good）”与“损坏（bad）”变更之间大约中点处的某个提交供你测试。
...
```
- 二分过程中可能会出现那些看起来有点奇怪的版本标识符，这是因为 Linux 内核的各个子系统会在其前一个版本（如 6.1）完成之前，就为新的 mainline 发行版（如 6.2）准备变更。因此它们会基于稍早一些的点（如 6.1-rc1 甚至 6.0）进行开发——然后在 6.1 发布后，未经变基或压缩就合并进 6.2。这就导致了二分过程中会出现那些看起来有点奇怪的版本标识符。
...
确保你告诉 Git 的内容准确无误：只要错一次，就会让后续的二分定位完全偏离正轨，因此那之后的所有测试都将白费。
...
#### 收好二分日志
  ...
  **将 Git 的二分日志与当前的 .config 文件保存在安全的地方。**
  [... <bisectlog_bisref>]
...
把 .config 文件收好，是因为在你报告回归之后，开发者很有可能会向你要它。
...
分步指南已经涵盖了所有相关内容，只有一件略显少见的情况除外：你是否用某个 stable/longterm 系列对一个同样出现在 mainline 中的回归做了二分定位，但 Git 无法在 mainline 中 revert 该提交？那么尝试在受影响的 stable/longterm 系列中 revert 该 culprit——如果成功，就改为测试该内核版本。
...
这两个位置之一是 /lib/modules/ 下的一个目录，其中保存了每个已安装内核的模块。该目录以内核的 release 标识符命名；因此，要删除你构建的某个内核的所有模块，只需删除它在 /lib/modules/ 中的模块目录即可。

另一个位置是 /boot/，安装内核时通常会在其中放置两到五个文件。它们的文件名中通常都包含 release 名称，但具体文件数量和确切名称在一定程度上取决于你发行版的 installkernel 可执行文件及其 initramfs 生成器。在某些发行版上，分步指南中提到的 `kernel-install remove...` 命令会替你删除所有这些文件，同时还会从你的 bootloader 配置中移除该内核的菜单项。在其他发行版上，这两项任务需要你自己完成。以下命令应当能以交互方式删除某个具有该 release 名称的内核的三个主要文件：

```
  rm -i /boot/{System.map,vmlinuz,initr}-6.0-rc1-local-gcafec0cacaca0

```
之后，检查 /boot/ 中是否还有其他文件名包含 '6.0-rc1-local-gcafec0cacaca0' 的文件，并考虑也将其删除。现在从你的 bootloader 配置中移除该内核的启动项；具体步骤在不同的 Linux 发行版之间差异很大。

注意，手动删除内核的文件或目录时要小心像 '*' 这样的通配符：你可能本想删除 6.0 或 6.0.1，却不小心删除了 6.0.13 内核的文件。
...
当你确实存储空间紧张时，按分步指南所述删除内核可能释放不了你期望的那么多空间。这种情况下，现在也可以考虑一并运行 `rm -rf ~/linux/*`。这会删除构建产物和 Linux 源码，但会保留 Git 仓库（~/linux/.git/）——因此一条简单的 `git reset --hard` 就能把源码恢复回来。
...
你之所以想把 ~/kernel-config-working 文件保留几周，也是出于这些额外的测试。
...
本节中使用的所有命令都应当相当直白，因此除了有一点之外没有太多可补充的：按照说明设置内核 tag 时，确保它不要比示例中用的那个长太多，因为如果 kernelrelease 标识符超过 63 个字符就会出问题。
...
- 在执行 ':ref:`Boot into the working kernel and briefly use the apparently broken feature <bootworking_bissbs>`' 之后，使用 `lsmod > ~/test-machine-lsmod` 将已加载模块的列表保存到一个文件。然后找到正在运行的内核的构建配置（关于在哪里可以找到它，请参阅 ':ref:`Start defining the build configuration for your kernel <oldconfig_bisref>`'），并将其保存为 '~/test-machine-config-working'。将这两个文件传输到你的构建主机的家目录。

- 在构建主机上继续遵循本指南（例如从 ':ref:`Ensure to have enough free space for building [...] <diskspace_bisref>`' 开始）。

- 当你到达 ':ref:`Start preparing a kernel build configuration[...] <oldconfig_bissbs>`' 时：在第一次运行 `make olddefconfig` 之前，执行以下命令，将你的配置基于来自
...
```
- 继续遵循本指南，但忽略那些说明每次都要如何编译、安装并重启进入某个内核的指示。改为构建
