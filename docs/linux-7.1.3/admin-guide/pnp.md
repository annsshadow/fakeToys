...
Linux 即插即用的用户界面为那些不支Linux 即插即用的传统驱动与用户态驱
提供了一种激PnP 设备的手段。该用户界面集成sysfs 中

除了标准sysfs 文件外，还会在每个设备的目录下创建以下文件：
- id —显示所支持EISA ID 列表
- options —显示可能的资源配
...
负责PnP 驱动PnP 协议之间交换信息，因此会自动将命令转发给相应的协议
这使得编PnP 驱动变得容易得多
...
pnp_register_driver
  ...
  其中包含驱动模型的集
  成功时返0，失败时返回负的错误号；若你想了解有多少个设备绑定到该驱动，可统计对 .add() 方法的调用次
...
它旨在取PNPBIOS。Linux 即插即用目前尚未支持它，但计划在不久的将来实现


Linux PnP 协议的要求：
1. 协议必须使用 EISA ID
2. 协议必须PnP 层报告设备当前的配置
...
```
2. 可选地定义 probe remove 函数。如果驱动已经拥有可靠的资源检测方法（例如 parport_pc 驱动），
   不定义这些函数是合理的
