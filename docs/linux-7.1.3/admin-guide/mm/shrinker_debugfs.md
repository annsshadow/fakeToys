...
Shrinker debugfs 接口提供了对内核内存 shrinker（内存回收器）子系统的
可见性，并允许获取单个 shrinker 的信息以及与之交互。

系统中注册的每个 shrinker 都会在 **<debugfs>/shrinker/** 下创建一个目录。
目录名由 shrinker 的名称和一个唯一 id 组成：例如 **kfree_rcu-0** 或
**sb-xfs:vda1-36**。

每个 shrinker 目录包含 **count** 和 **scan** 文件，用于为每个 memcg 和
numa 节点（如适用）触发 **count_objects()** 和 **scan_objects()** 回调。
