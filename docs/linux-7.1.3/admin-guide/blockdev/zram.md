...
zram 模块会创建名/dev/zram<id>id> = 0, 1, ...）的基于内存的块设备
写入这些磁盘的页会被压缩并直接存储在内存中。这些磁盘具有非常快I/O
性能，并且压缩能够带来可观的内存节省。部分使用场景包/tmp 存储、用
swap 磁盘var 下的各种缓存，以及可能更多的用途)
...
a) 使用 zram zram_control sysfs 属
b) 使用 util-linux 提供zramctl 工具（util-linux@vger.kernel.org）

本文档仅描述“手动”配zram 的步骤，zram zram_control sysfs 属性

若想进一步了zramctl，请查阅 util-linux 的文档、zramctl 手册页或
`zramctl --help`。请注意，zram 的维护者并不开维护 util-linux 
...
为简洁起见，下面的大多数示例省略了错误检查部分。但是，处理错误是你
全部责任

zram sysfs 属性在出错时总是返回负值。可能的返回码列表如下：

========  =============================================================
-EBUSY	  试图修改设备初始化后无法再更改的属性。请先重置设备
-ENOMEM	  zram 无法分配足够的内存来满足你的需求
-EINVAL	  提供了无效的输入
-EAGAIN	  稍后重试该操作（例如当试图同时执recompress writeback 时）
========  =============================================================

如果你使'echo'，返回值由 'echo' 工具设置，因
...
num_devices 参数是可选的，用于告zram 应预先创建多少设备。默认值：1
...
压缩算法可能支持针对特定数据集进行调整的特定参数。ZRAM 提供了一
`algorithm_params` 设备属性，用于按算法进行参数配置

例如，若干压缩算法支`level` 参数。此外，某些压缩算法支持预训练字典，
...
某些算法甚至可以取负值）；对于另一些算法，`level` 是加速级别（值越
压缩比越低）

## 4) 设置磁盘大小


通过sysfs 节点 'disksize' 写入值来设置磁盘大小。该值可以是字节数，
也可以使用内存后缀

```
	# 50MB 的磁盘大小初始化 /dev/zram0
...
通过sysfs 节点 'mem_limit' 写入值来设置内存上限。该值可以是字节数，
...
要添加一个新/dev/zramX 设备，请hot_add 属性执行读操作。该操作
返回新设备的设备 id（意味着你可以使/dev/zram<id>），或者返回一个错误码
...
每个设备的统计信息作/sys/block/zram<id>/ 下的各种节点导出
...
initstate         	RO	显示设备的初始化状
reset             	WO	触发设备重置
mem_used_max      	WO	重置 `mem_used_max` 计数器（见后文）
mem_limit         	WO	指定 ZRAM 可用于存储压缩数据的最大内存量
writeback_limit   	WO	指定 zram 可以写出到后端设备的最大写 IO
				量，4KB 为单
writeback_limit_enable  RW	显示并设writeback_limit 功能
writeback_batch_size	RW	显示并设置最大的在writeback 操作数量
compressed_writeback	RW	显示并设置压writeback 功能
comp_algorithm    	RW	显示并更改压缩算
algorithm_params	WO	设置压缩算法参数
compact           	WO	触发内存规整
debug_stat        	RO	该文件用zram 调试目的
backing_dev	  	RW	zram 设置用于写出的后端存
...
stat 文件表示未被块层统计、因而在 zram<id>/stat 文件中不可用的设I/O
统计信息。它由单行文本组成，包含以下以空白分隔的统计项：

 =============    =============================================================
 failed_reads     读取失败的次
 failed_writes    写入失败的次
 invalid_io       非页大小对齐I/O 请求数量
 notify_free      取决于设备使用场景，可能统计

                  a) 由于 swap 槽位释放通知而释放的页数

                  b) 由于 bio 发送的 REQ_OP_DISCARD 请求而释放的页数。前者在
                     释放 swap 槽位时发送给 swap 块设备，这意味着该磁盘正
                     用作 swap 磁盘

                  b) 后者由discard 选项挂载的文件系统在丢弃某些数据块时
                     发送
 =============    =============================================================

文件 /sys/block/zram<id>/mm_stat

mm_stat 文件表示设备mm 统计信息。它由单行文本组成，包含以下以空
分隔的统计项
 ...
 mem_used_total   为该磁盘分配的内存量。这包含为该磁盘分配的分配器碎片
                  元数据开销。因此，可以使用 compr_data_size 和该项统计计
                  分配器的空间效率
                  单位：字
...
bd_stat 文件表示设备的后端设备统计信息。它由单行文本组成，包含以下
空白分隔的统计项
...
zram 内置支持 idle 页跟踪（即已分配但未被使用的页）。该功能对例zram
writeback 等非常有用，
...
这会将所有已分配zram 页标记为 idle。只有当该页（块）被访问（例如被
覆盖或释放）时，idle 标记才会被移除。此外，当启CONFIG_ZRAM_TRACK_ENTRY_ACTIME
...
在本例中，所有超86400 秒（一天）未被访问的页将被标记idle

### writeback


通过 CONFIG_ZRAM_WRITEBACK，zram 可以idle/不可压缩页写入后端存储，
不是保留在内存中
...
如果管理员想zram 设备中的某个特定页写入后端设备，

```
	echo "page_index=1251" > /sys/block/zramX/writeback
```

Linux 6.16 中，该接口经历了一些重构。首先，该接口现在对其所有参
支持 `key=value` 格式（`type=huge_idle` 等）。其次，引入了对 `page_indexes`
的支持，用于指定要写回的页的 `LOW-HIGH` 范围（或多个范围）。这减少了系
...
我们现在还允许每次调用传入多page_index 参数，以及混合使
...
如果闪存设备上存在大量写 IO，则可能存在闪存磨损问题，因此管理员需
设计写入限制，以保证整个产品生命周期内的存储健康

为解决这个问题，zram 支持 "writeback_limit" 功能writeback_limit_enable"
的默认值为 0，因此不限制任何 writeback。也就是说，如果管理员想要应
writeback 预算，他们应
...
如果管理员想要限制每400M writeback，可以这样做
...
writeback_limit 计数会在你重zram 时（例如系统重启、echo 1 > /sys/block/zramX/reset
复位，因此记录重zram 之前发生了多少次 writeback，以便在下次设置时分
额外writeback 预算，是用户的工作

默认情况下，zram 以解压缩（原始）形式存储写回的页，这意味着 writeback
操作在写入后端设备之前需要对该页进行解压缩。该行为可以通过启用
`compressed_writeback` 功能来改变，该功能会zram 将压缩后的页写入后端
设备，从而避免解压缩开销。要启用它，
...
请注意，该功能应`zramX` 设备初始化之前配置

根据后端设备的存储类型，writeback 操作可能受益于更多的在途写请求（批
写入）。最大的在writeback 操作数量可以通过 `writeback_batch_size` 属
配置。要更改默认值（32），
...
如果管理员想测量某个时间段内writeback 计数，可以通过
...
页进行重新压缩。其基本思想是，替代压缩算法可以以（潜在的）更慢的压
解压缩速度为代价，提供更好的压缩比。例如，替代压缩算法可以更有效地压缩
huge 页（那些默认算法未能压缩的页）。另一个应用是 idle 页重新压缩——那
冷数据并驻留在内存中的页可以使用更有效的算法重新压缩，从而减zsmalloc
的内存占用

通过 `CONFIG_ZRAM_MULTI_COMP`，zram 最多支4 种压缩算法：1 个主算法
最3 个次级算法。zram 主压缩器在) 选择压缩算法”中已说明，次级算法
...
lz4hc 被配置为优先2 的压缩算法。替代压缩算法的优先级是在配置算法时
提供的：
...
算法名称来确定优先级，但并不推荐这样做，因为当同一算法以不同优先级配置
...
通过 CONFIG_ZRAM_MEMORY_TRACKING，用户可以了zram 块的信息。它对于通过
*pagemap 捕获进程的冷页或不可压缩页可能很有用

如果启用该功能，你可以通过如下方式查看块状
...
上述示例的第一行表示第 300 个块75.033841 秒时被访问，且该块的状态为
