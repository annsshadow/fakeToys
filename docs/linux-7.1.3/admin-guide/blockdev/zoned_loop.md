...
重置一个顺序写区时，其后端文件大小会被截断为零。相反，对于区的 finish 操作，后端文件会被截断到区大小。由此，创建zloop 分区块设备的最大容量可以配置为大于后端文件系统上可用的存储空间。当然，对于这种配置，写入的数据量超过后端文件系统上可用存储空间时会导致写错误
分区循环块设备驱动实现了一个完整的区状态转换状态机。也就是说，区可以是空、隐式打开、显式打开、关闭或已满。当前实现不支持对最大打开区数和活动区数施加任何限制
创建和删zloop 设备不需要任何用户态工具

### 2) 创建分区设备


一旦加载了 zloop 模块（或zloop 被编译进内核），就可以使用字符设备文/dev/zloop-control 来添加一zloop 设备。这是通过直接/dev/zloop-control 写入一"add" 命令来完成的
...
====================   =========================================================
id                    设备号（/dev/zloopX 中的 X）                      默认值：自动分配capacity_mb           设备总容量，单位MiB。该值总是向上取整                      区大小最接近的更高倍数                      默认值：16384 MiB6 GiB）zone_size_mb          设备区大小，单位MiB。默认值：256 MiBzone_capacity_mb      设备区容量（必须始终等于或小于区大小）。默认值：区大小conv_zones            从扇0 开始的常规区总数                      默认值：8
max_open_zones        所需的打开顺序写区的最大数量（0 表示无限制）                      默认值：0
base_dir              用于创建包含该设备区文件的目录的基础目录路径                      默认/var/local/zloop                      包含区文件的设备目录总是以设ID 命名。例                      /dev/zloop0 的默认区文件目录/var/local/zloop/0nr_queues             分区块设备的 I/O 队列数量。该值总是受在                      CPU 数量的上限约束                      默认值：1
queue_depth           每个 I/O 队列的最I/O 队列深度                      默认值：64
buffered_io           执行缓冲 I/O 而非直接 I/O（默认值：false）zone_append           启用或禁zloop 设备的原zone append 支持                      默认值：1（启用）                      若禁用了原生 zone append 支持，块层将使用常规                      操作来模拟该操作ordered_zone_append   启用 zloop zone append 重排序的缓解                      默认值：禁用                      这对于测试文件系统文件数据映射（extent）很有用                      因为启用后，可以显著减少文件数据映射所需                      数据 extent 数量discard_write_cache   设备被移除时，通过将每个区文件截断到上一次刷                      操作期间记录的大小，丢弃所有未通过刷新操作
                      显式持久化的数据。这模拟了未提交数据丢失                      掉电事件====================   =========================================================
...
要彻底删除一个分区设备，在执remove 操作后，必须删除包含该设备各区的后端文件的设备基础目录

### 4) 示例


以下命令序列创建了一2GB 的分区设备，其区大小64
...
对于所创建的设备（/dev/zloop0），其区后端文件全部创建
...
被移除的设备可以使用与首次创建设备时相同"add" 命令再次添加。要彻底删除一个分区设备，其后端文
