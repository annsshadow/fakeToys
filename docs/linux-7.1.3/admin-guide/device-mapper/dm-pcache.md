...
本文档描**dm-pcache**，这是一Device-Mapper 目标，它让一个可按字节寻址**DAX**（持久内存，“pmem”）区域充当位于较慢块设备之前的、高性能且崩溃持久化的缓存。相关代码位`drivers/md/dm-pcache/`

## 特性速览


- **回写（write-back* 缓存（目前唯一支持的模式）
- pmem 设备上分配的 **16 MiB 段（segment*
- **数据 CRC32** 校验（可选，按缓存设置）
- 崩溃安全：每个元数据结构都做了双份复制（`PCACHE_META_INDEX_MAX == 2`），并使CRC 加序列号进行保护
- **多树索引**（按逻辑地址分片的索引树），以获得较高的 PMem 并行
- **DAX 路径** I/O —没有额外BIO 往
- **日志结构回写（log-structured write-back*，保持后端崩溃一致
...
- 每次写入会从段内每个 CPU **data_head** 分配空间
- 一**cache-key（缓存键* 记录了源设备上的一段逻辑范围，以及它pmem 中的位置（段 + 偏移 + 代（generation））
- 128 个键组成一**key-set（kset*；kset pmem 中顺序写入，并且自身是崩溃安全的（CRC）
- 这一**(key_tail, dirty_tail)** 界定了干净/脏以及存死亡 kset 的边界

### 回写


脏键被排入一棵树中；一个后台工作线程将数据复制backing_dev，并推进 **dirty_tail**。来自上层的 FLUSH/FUA bio 会强制立即提交元数据

### 垃圾回收


`segs_used >= seg_total * gc_percent / 100` 时，GC 启动。它**key_tail** 开始遍历，释放其中每个键都已失效的段，并推**key_tail**

### CRC 校验


`data_crc 已启用`，dm-pcache 会在每次插入时为每个缓存数据范围计算 CRC32，并将其存储在介质上的键中。读取时会在复制到调用者之前验CRC
...
- **缓存已满** —如果找不到空闲段，写入返`-EBUSY`；dm-pcache 会在内部重试（请求延迟）
- **系统崩溃** —在挂载时，驱动会**key_tail** 重放 kset 以重建内存中的树；每个段的代（generation）可防止出现悬空（use-after-free）键
