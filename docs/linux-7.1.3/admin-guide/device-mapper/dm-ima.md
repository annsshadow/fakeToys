...
对于给定系统，各种外部服基础设施工具（包括证明服务）会与之交互——既在初始化设置期间，也在系统其余运行时段。它们共享敏感数据，或在该系统上执行关键工作负载。在将业务关键型数据/工作负载完全托付给该系统之前，外部服务可能希望先验证相关内核子系统的当前运行时状态
设备映射器（Device mapper）通过在块设备上使crypt、verity、integrity 等多种目标类型，为块设备提供各种重要功能，从而在特定系统上发挥着关键作用。这些目标类型各自的功能都可以通过各种属性进行配置。用于配置这些目标类型的属性选择，会显著影响块设备乃至整个系统的安全配置。例如，加密算法的类型和密钥长度决定了给定块设备的加密强度
因此，在将业务关键型数据/工作负载完全托付给该系统之前，外部服务验证各种块设备的当前状态及其各种目标属性至关重要
IMA 内核子系统为设备映射器提供了必要的功能，用于度量（measure）各种块设备的状态与配置—
- 由设备映射器自身在内核内部完成，
- 以一种抗篡改（tamper resistant）的方式- 并在状配置发生变化时重新度量
## 设置 IMA 策略
为了IMA 度量给定系统上的数据，需要更新该系统上的 IMA 策略以包含如下一行内容，并且需要重启系统才能使度量生效
```
...
```
 ...
 PCR := Platform Configuration Register，即平台配置寄存器，值会被登记在其中       这仅在使用了 TPM 芯片时适用
 TEMPLATE_DATA_DIGEST := IMA 记录的模板数据摘要 TEMPLATE_NAME := 登记完整性值（integrity value）的模板名称（如 ima-buf）
 TEMPLATE_DATA := <ALG> ":" <EVENT_DIGEST> <EVENT_NAME> <EVENT_DATA>
                  它包含要以给定模板数据格式进行度量的特定事件数据
 ALG := 用于计算事件摘要的算 EVENT_DIGEST := 事件数据的摘 EVENT_NAME := 事件的描述（'dm_table_load'） EVENT_DATA := 要被度量的事件数据
```
|
| **NOTE #1:**
| IMA 子系统度量的 DM 目标数据，也可以通过设置 DM_IMA_MEASUREMENT_FLAG DM_TABLE_STATUS_CMD，从用户空间查询|
|
| **NOTE #2:**
| 内核配置CONFIG_IMA_DISABLE_HTABLE 允许度量重复记录| 为支持在 IMA 日志中记录重复的 IMA 事件，内核需要配CONFIG_IMA_DISABLE_HTABLE=y
## 支持的设备状态：
...
```
 ...
 dm_version_str := "dm_version=" <N> "." <N> "." <N>
                  与设备映射器驱动版本相同 device_metadata := <device_name> "," <device_uuid> "," <device_major> "," <device_minor> ","
 ...
 dm-device-name := 设备名称。如果其中包'\',';' 等特殊字符，
                   会在其前面加'\' 前缀 dm-device-uuid := 设备UUID。如果其中包'\',';' 等特殊字符，
                   会在其前面加'\' 前缀
 table_load_data := <target_data>
                    表示正在加载DM 设备非活跃表槽中的、来自表中各目标的数                    （以 name=value 对的形式） target_data := <target_data_row> | <target_data><target_data_row>
 ...
 target_index := "target_index=" <N>
                 表示表中的第 n 个目标（范围<num_device_targets> 指定0 N-1）                 如果 N 个目标的数据无法全部放入给定缓冲区，则能够放入缓冲区的那部分数据
                 （例如从目标 0 x）会在一次给定的 IMA 事件中被度量                 其余从目x+1 N-1 的数据会在后续的 IMA 事件中度量，
                 其格式与 'dm_table_load' 相同                 <dm_version_str> ";" <device_metadata> ";" <table_load_data>
 target_begin := "target_begin=" <N>
 target_len := "target_len=" <N>
 target_name := 目标的名称linear'crypt'integrity' 等                支持 IMA 度量的目标在下面的“支持的目标”小节中有文档说明 target_version := "target_version=" <N> "." <N> "." <N>
 target_attributes := 包含以逗号分隔的、目标特定属name=value 对的数据
 例如，如果使用以下表项创建一linear 设备  # dmsetup create linear1
  0 2 linear /dev/loop0 512
  2 2 linear /dev/loop0 512
  4 2 linear /dev/loop0 512
  6 2 linear /dev/loop0 512

 那么 IMA ASCII 度量日志中将包含如下条目 （为便于阅读，已ASCII 转换为文本）
...
```
 ...
 dm_version_str := 如上面“表加载”小节所述 device_metadata := 如上面“表加载”小节所述 active_table_hash := "active_table_hash=" <table_hash_alg> ":" <table_hash>
                      表示正在度量的、该设备活跃表的 IMA 数据的哈希 table_hash_alg := 用于计算哈希的算法 table_hash := (<dm_version_str> ";" <device_metadata> ";" <table_load_data> ";")
               的哈希，如上面的 'dm_table_load' 所述               注意：如table_load 数据跨越了某个设备的多个 IMA 'dm_table_load'
               事件，则哈希是结合所有这些事件的数据
               鍗?(<dm_version_str> ";" <device_metadata> ";" <table_load_data> ";")
               计算得到的 current_device_capacity := "current_device_capacity=" <N>

 例如，如果使用以下命令恢复一linear 设备 #dmsetup resume linear1
...
```
 ...
 dm_version_str := 如上面“表加载”小节所述 device_active_metadata := 反映当前已加载活跃表的设备元数据                           其格式与上面“表加载”小节中描述'device_metadata' 相同 device_inactive_metadata := 反映非活跃表的设备元数据                             其格式与上面“表加载”小节中描述'device_metadata' 相同 active_table_hash := 当前已加载活跃表的哈希                      其格式与上面“设备恢复”小节中描述'active_table_hash' 相同 inactive_table_hash :=  非活跃表的哈希                         其格式与上面“设备恢复”小节中描述'active_table_hash' 相同 remove_all := "remove_all=" <yes_no>
 yes_no := "y" | "n"
 current_device_capacity := "current_device_capacity=" <N>

 例如，如果使用以下命令移除一linear 设备  #dmsetup remove l1
...
当非活跃表从设备中清除时，设备信息以及该非活跃表数据sha256 哈希会被度量
...
```
 ...
 dm_version_str := 如上面“表加载”小节所述 device_inactive_metadata := 在加载时捕获的、正被清除的非活跃表的设备元数据                             其格式与上面“表加载”小节中描述'device_metadata' 相同 inactive_table_hash := 正被从设备清除的非活跃表的哈希                        其格式与上面“设备恢复”小节中描述'active_table_hash' 相同 current_device_capacity := "current_device_capacity=" <N>

 例如，如果一linear 设备的非活跃表被清除  #dmsetup clear l1
...
当设备的 NAME UUID 被更改时，设备信息以及新NAME UUID 会被度量
...
```
 ...
 dm_version_str := 如上面“表加载”小节所述 device_active_metadata := 反映当前已加载活跃表的设备元数据                           其格式与上面“表加载”小节中描述'device_metadata' 相同 new_device_name := "new_name=" <dm-device-name>
 dm-device-name := 与上面“表加载”小节中描述<dm-device-name> 相同
 new_device_uuid := "new_uuid=" <dm-device-uuid>
 dm-device-uuid := 与上面“表加载”小节中描述<dm-device-uuid> 相同
 current_device_capacity := "current_device_capacity=" <N>

 1：如果使用以下命令更改一linear 设备的名称，
...
```
 ...
 例如 当加载一'cache' 目标时，IMA ASCII 度量日志会有一条类似于下面的条目，
 展示'dm_table_load' 事件EVENT_DATA 中度量了哪些 'cache' 属性 （为便于阅读，已ASCII 转换为文本）
...
作为 EVENT_DATA 一部分在上述“表加载”小节中描述'target_attributes'，对'crypt' 目标具有以下数据格式
```
 ...
 例如 当加载一'crypt' 目标时，IMA ASCII 度量日志会有一条类似于下面的条目，
 展示'dm_table_load' 事件EVENT_DATA 中度量了哪些 'crypt' 属性 （为便于阅读，已ASCII 转换为文本）
...
作为 EVENT_DATA 一部分在上述“表加载”小节中描述'target_attributes'，对'integrity' 目标具有以下数据格式
```
 ...
 例如 当加载一'integrity' 目标时，IMA ASCII 度量日志会有一条类似于下面的条目，
 展示'dm_table_load' 事件EVENT_DATA 中度量了哪些 'integrity' 属性 （为便于阅读，已ASCII 转换为文本）
...
作为 EVENT_DATA 一部分在上述“表加载”小节中描述'target_attributes'，对'linear' 目标具有以下数据格式
```
 ...
 例如 当加载一'linear' 目标时，IMA ASCII 度量日志会有一条类似于下面的条目，
 展示'dm_table_load' 事件EVENT_DATA 中度量了哪些 'linear' 属性 （为便于阅读，已ASCII 转换为文本）
...
作为 EVENT_DATA 一部分在上述“表加载”小节中描述'target_attributes'，对'mirror' 目标具有以下数据格式
```
 ...
 mirror_device_data := <mirror_device_row> | <mirror_device_data><mirror_device_row>
                       mirror_device_row 会重<NR> 次——对<nr_mirrors> 中描述的 <NR> mirror_device_row := <mirror_device_name> "," <mirror_device_status>
 mirror_device_name := "mirror_device_" <X> "=" <mirror_device_name_str>
                       其中 <X> 的范围从 0 (<NR> -1)——对<nr_mirrors> 中描述的 <NR> mirror_device_status := "mirror_device_" <X> "_status=" <mirror_device_status_char>
                         其中 <X> 的范围从 0 (<NR> -1)——对<nr_mirrors> 中描述的 <NR> mirror_device_status_char := "A" | "F" | "D" | "S" | "R" | "U"
 handle_errors := "handle_errors=" <yes_no>
 keep_log := "keep_log=" <yes_no>
 log_type_status := "log_type_status=" <log_type_status_str>
 yes_no := "y" | "n"

 例如 当加载一'mirror' 目标时，IMA ASCII 度量日志会有一条类似于下面的条目，
 展示'dm_table_load' 事件EVENT_DATA 中度量了哪些 'mirror' 属性 （为便于阅读，已ASCII 转换为文本）
...
作为 EVENT_DATA 一部分在上述“表加载”小节中描述'target_attributes'，对'multipath' 目标具有以下数据格式
```
 ...
 priority_groups_row := "pg_state_" <X> "=" <pg_state_str> "," "nr_pgpaths_" <X>  "=" <NPGP> ","
                        ...
                        其中 <X> 的范围从 0 (<NPG> -1)——对<nr_priority_groups> 中描述的 <NPG> pg_state_str := "E" | "A" | "D"
 ...
 priority_group_paths_row := "path_name_" <X> "_" <Y> "=" <string> "," "is_active_" <X> "_" <Y> "=" <is_active_str>
                             ...
                             其中 <X> 的范围从 0 (<NPG> -1)——对<nr_priority_groups> 中描述的 <NPG>                             <Y> 的范围从 0 (<NPGP> -1)——对<priority_groups_row> 中描述的 <NPGP> is_active_str := "A" | "F"

 例如 当加载一'multipath' 目标时，IMA ASCII 度量日志会有一条类似于下面的条目，
 展示'dm_table_load' 事件EVENT_DATA 中度量了哪些 'multipath' 属性 （为便于阅读，已ASCII 转换为文本）
...
作为 EVENT_DATA 一部分在上述“表加载”小节中描述'target_attributes'，对'raid' 目标具有以下数据格式
```
 ...
 raid_device_status := <raid_device_status_row> | <raid_device_status_row><raid_device_status>
                       <raid_device_status_row> 会重<NRD> 次——对<raid_disks> 中描述的 <NRD> raid_device_status_row := "raid_device_" <X> "_status=" <raid_device_status_str>
                           其中 <X> 的范围从 0 (<NRD> -1)——对<raid_disks> 中描述的 <NRD> raid_device_status_str := "A" | "D" | "a" | "-"
 journal_dev_mode := "journal_dev_mode=" <journal_dev_mode_str>
 journal_dev_mode_str := "writethrough" | "writeback" | "invalid"

 例如 当加载一'raid' 目标时，IMA ASCII 度量日志会有一条类似于下面的条目，
 展示'dm_table_load' 事件EVENT_DATA 中度量了哪些 'raid' 属性 （为便于阅读，已ASCII 转换为文本）
...
作为 EVENT_DATA 一部分在上述“表加载”小节中描述'target_attributes'，对'snapshot' 目标具有以下数据格式
```
 ...
 例如 当加载一'snapshot' 目标时，IMA ASCII 度量日志会有一条类似于下面的条目，
 展示'dm_table_load' 事件EVENT_DATA 中度量了哪些 'snapshot' 属性 （为便于阅读，已ASCII 转换为文本）
...
作为 EVENT_DATA 一部分在上述“表加载”小节中描述'target_attributes'，对'striped' 目标具有以下数据格式
```
 ...
 stripe_device_name := "stripe_" <X> "_device_name=" <stripe_device_name_str>
                       其中 <X> 的范围从 0 (<NS> -1)——对<stripes> 中描述的 <NS> stripe_physical_start := "stripe_" <X> "_physical_start=" <N>
                          其中 <X> 的范围从 0 (<NS> -1)——对<stripes> 中描述的 <NS> stripe_status := "stripe_" <X> "_status=" <stripe_status_str>
                  其中 <X> 的范围从 0 (<NS> -1)——对<stripes> 中描述的 <NS> stripe_status_str := "D" | "A"

 例如 当加载一'striped' 目标时，IMA ASCII 度量日志会有一条类似于下面的条目，
 展示'dm_table_load' 事件EVENT_DATA 中度量了哪些 'striped' 属性 （为便于阅读，已ASCII 转换为文本）
...
作为 EVENT_DATA 一部分在上述“表加载”小节中描述'target_attributes'，对'verity' 目标具有以下数据格式
```
 ...
 例如 当加载一'verity' 目标时，IMA ASCII 度量日志会有一条类似于下面的条目，
 展示'dm_table_load' 事件EVENT_DATA 中度量了哪些 'verity' 属性 （为便于阅读，已ASCII 转换为文本）
