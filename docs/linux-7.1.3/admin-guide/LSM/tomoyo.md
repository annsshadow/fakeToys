...
TOMOYO 是 Linux 内核的一个基于名称的 MAC 扩展（LSM 模块）。
...
尽管这些教程使用的是非 LSM 版本的 TOMOYO，它们仍有助于你了解 TOMOYO 是什么。

## 如何启用 TOMOYO？


使用 `CONFIG_SECURITY_TOMOYO=y` 构建内核，并在内核命令行上传递
`security=tomoyo`。
...
我们为研讨会和专题讨论会准备的材料位于
https://sourceforge.net/projects/tomoyo/files/docs/ 。以下列表从三个方面选取。
