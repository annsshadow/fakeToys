...
如果启用`audit`，那么被沙箱化的程序所发起的被拒绝访问请求会默认记录日志
这一默认行为可以通过 sys_landlock_restrict_self() 的标志来更改（参
Documentation/userspace-api/landlock.rst）。Landlock 日志也可以通过审计规则
...
```
    ...
    **net.*** - 网络访问权限（ABI 4+）：
        - net.bind_tcp - TCP 端口绑定被拒
        - net.connect_tcp - TCP 连接被拒

    **scope.*** - IPC 范围限制（ABI 6+）：
        - scope.abstract_unix_socket - 抽象 UNIX 套接字连接被拒绝
        - scope.signal - 信号发送被拒绝

    当缺少多项访问权限时，多blockers 可能出现在同一条事件中（以逗号分隔）
    例如，在一个既缺少 ``make_reg`` 又缺``refer`` 权限的目录中创建普通文件，
    会显``blockers=fs.make_reg,fs.refer``

    对象标识字段（文件系统对应的path、dev、ino；信号对应的opid、ocomm
    取决于被阻止的访问类型，并提供关于拒绝所涉及资源的上下文

```
AUDIT_LANDLOCK_DOMAIN
    该记录类型描述一Landlock 域的状态。`status` 字段可以`allocated` 
    `deallocated`銆。

    `allocated` 状态属于同一个审计事件的一部分，并跟在某个域首次记录的
    `AUDIT_LANDLOCK_ACCESS` 记录之后。它标识在调sys_landlock_restrict_self()
...
```
    ...
    ``deallocated`` 状态是一个独立的事件，它标识一Landlock 域的释放。在
    事件之后，可以保证在系统生命周期内相关域 ID 绝不会被复用。``domain`` 字段
    指示被释放域ID，``denials`` 字段指示被拒绝访问请求的总数，其中部分请
...
```
该命令生成两个事件，每个事件都带有一个跟随时间戳的唯一序列
（`msg=audit(1729738800.268:30)`）。第一个事件（序列`30`）包4 条记录
第一条记录（`type=LANDLOCK_ACCESS`）显示一个被`1a6fdc66f` 拒绝的访问
...
该域由进`286` 执行 root 用户启动`/root/sandboxer` 程序所创建
...
与某Landlock 域相关的资源（`status=deallocated`）。这有助于了解后续日
...
如果你被Landlock 相关的审计日志刷屏，这要么是一次攻击尝试，要么是安
策略中的 bug。我们可以通过两种互补的方式来设置一些过滤器以限制噪音：

- 如果我们能修复被沙箱化的程序，可以使sys_landlock_restrict_self() 的标志；
