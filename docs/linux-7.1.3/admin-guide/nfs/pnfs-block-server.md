...
要使pNFS 块布局配合 Linux NFS 服务器，被导出的文件系统需要支pNFS 块布局
（目前仅 XFS），并且该文件系统必须位MDS 之外客户端也能访问的共享存储（通常iSCSI）。截至目前，文件系统需要直接位于导出的卷上，尚不支持在 MDS 客户端上对卷进行条带化或拼接
在服务器端，如果文件系统支持，pNFS 块卷支持会自动启用。在客户端，需确保内核
启用CONFIG_PNFS_BLOCK 选项，nfs-utils 中的 blkmapd 守护进程正在运行，并文件系统使用 NFSv4.1 协议版本挂载（mount -o vers=4.1）
如果 nfsd 服务器需要对一个无响应的客户端进行隔离（fence），它会调用
/sbin/nfsd-recall-failed，第一个参数设为客户端IP 地址，第二个参数设为待隔文件系统的设备节点（不带 /dev 前缀）。下面是一个示例文件，展示了：
...
如果 nfsd 服务器需要对一个无响应的客户端进行隔离，而隔离操作失败，服务器会系统日志中记录一条格式如下的警告信息
    FENCE failed client[IP_address] clid[#n] device[dev_name]

    其中
    - IP_address：指受影响客户端IP 地址    - #n：表示唯一的客户端标识符    - dev_name：指与此次隔离尝试相关的块设备名称
