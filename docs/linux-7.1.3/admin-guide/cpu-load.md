...
Linux 通过 `/proc/stat` `/proc/uptime` 导出各种信息，用户态工具（top(1)）利用这些信息来计算
...
此处系统认为，在默认采样周期内，系统10.01% 的时间在用户空间执行工作.92% 在内核中，整体空闲时间为 81.63%
...
在上述情形下，根`/proc/stat` 的判断，系统负载0%（因为定时器中断总是发生在系统执idle 处理程序时），但实际上负载更接近 99%
人们可以设想更多此类内核行为导致偏差的情
```
