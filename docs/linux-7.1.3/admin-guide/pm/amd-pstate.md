...
`amd-pstate` 是 AMD CPU 性能缩放驱动，它在现代 AMD APU 和 CPU 系列中
为 Linux 内核引入了一种新的 CPU 频率控制机制。新机制基于协作处理器性能控制
（Collaborative Processor Performance Control，CPPC），它提供比传统 ACPI
硬件 P-States 更细粒度的频率管理。当前 AMD CPU/APU 平台使用 ACPI P-states
驱动，仅能在 3 个 P-state 之间切换来管理 CPU 频率和时钟。CPPC 取代了 ACPI
P-states 控制，并为 Linux 内核提供了一个灵活、低延迟的接口，用于直接与硬件
通信性能提示。

`amd-pstate` 利用 Linux 内核的调控器（governor），例如 `schedutil`、
`ondemand` 等，来管理由 CPPC 硬件功能提供的性能提示，后者在内部遵循硬件
规范（详情参见 AMD64 架构程序员手册第 2 卷：系统编程 [^1^]_）。目前，
`amd-pstate` 已在一些 Zen2 和 Zen3 处理器上，根据内核调控器支持基本的频率
控制功能，在我们在硬件和 SBIOS 上验证之后，未来会实现更多 AMD 特有的功能。

## AMD CPPC 概述


协作处理器性能控制（Collaborative Processor Performance Control，CPPC）接口
枚举一个连续的、抽象的、无单位（unit-less）的性能值，其刻度并不绑定到特定的
性能状态 / 频率。这是一项 ACPI 标准 [^2^]_，软件可以据此将应用性能目标和提示
作为相对目标指定给基础设施的限制。AMD 处理器提供低延迟的寄存器模型（MSR），
而不是用 AML 代码解释器来进行性能调整。`amd-pstate` 会用回调初始化一个
...
这是单个处理器在理想条件下可能达到的绝对最大性能。该性能水平可能无法长时间
持续，并且可能仅当其他平台组件处于特定状态时才可实现；例如，它可能要求其他
...
的情况下，这是处理器预期能够持续维持的性能水平。所有核 / 处理器都应能够同时
维持其标称性能状态。
...
阈值时，较低的性能水平通常应比较高性能水平更节能。该寄存器有效地向 `amd-pstate`
传达了最高效的性能水平。
...
`amd-pstate` 在 CPPC 性能刻度中以一个相对数字指定期望目标。这可以表示为标称
性能（基础设施最大值）的百分比。在标称持续性能水平以下，期望性能表示受硬件
约束的处理器平均性能水平。在标称性能水平以上，处理器必须至少提供所请求的标称
性能，并在当前运行条件允许时进一步提高。

Energy Performance Preference (EPP) (RW)
.........................................

该属性向硬件提供一个提示，表示软件希望偏向性能（0x0）还是能效（0xff）。


## 关键调控器支持


`amd-pstate` 可以与 `sysfs` 中 `scaling_available_governors` 策略属性列出的
所有（通用）缩放调控器一起使用。然后，它负责配置与 CPU 对应的策略对象，并向
`CPUFreq` 核心（以及附加到策略对象的缩放调控器）提供硬件支持的最大和最小运行
频率的准确信息。用户可以查看来自 `CPUFreq` 核心的 `scaling_cur_freq` 信息。

`amd-pstate` 主要支持 `schedutil` 和 `ondemand` 用于动态频率控制。它是将
处理器配置通过 `amd-pstate` 微调到带 CPU CFS 调度器的 `schedutil`。`amd-pstate`
注册 adjust_perf 回调，以实现类似于 CPPC 的性能更新行为。它由 `sugov_start`
初始化，然后填充 CPU 的 update_util_data 指针，将 `sugov_update_single_perf`
...
如果检测到的处理器中 ACPI SBIOS 里不存在 `_CPC` 条目，`amd-pstate` 的初始化
将失败。它使用 `acpi_cpc_valid` 来检查 `_CPC` 是否存在。所有基于 Zen 的处理器
...
`Shared Memory Support <perf_cap_>`_。它可以使用 `X86_FEATURE_CPPC` 特性标志
来指示不同的类型。（详情参见 AMD Family 19h Model 51h Revision A1 处理器编程
参考手册（PPR）[^3^]_。）`amd-pstate` 会为不同的硬件实现注册不同的 `static_call`
实例。

目前，一些 Zen2 和 Zen3 处理器支持 `amd-pstate`。未来，它将在越来越多的 AMD
处理器上得到支持。

### 完整 MSR 支持（Full MSR Support）


一些新的 Zen3 处理器（如 Cezanne）在 `X86_FEATURE_CPPC` CPU 特性标志被设置时，
直接提供 MSR 寄存器。`amd-pstate` 可以处理 MSR 寄存器，在 `CPUFreq` 中实现快速
切换（fast switch）功能，从而降低中断上下文中频率控制的延迟。带有 `pstate_xxx`
前缀的函数表示对 MSR 寄存器的操作。

### 共享内存支持（Shared Memory Support）


如果未设置 `X86_FEATURE_CPPC` CPU 特性标志，则处理器支持共享内存方案。在这种
情况下，`amd-pstate` 使用 `cppc_acpi` 辅助方法来实现定义在 `static_call` 上的
回调函数。带有 `cppc_xxx` 前缀的函数表示对共享内存方案的 ACPI CPPC 辅助方法的
操作。


AMD P-States 和 ACPI 硬件 P-States 始终可以在同一个处理器上得到支持。但 AMD
P-States 具有更高的优先级，如果它通过 `MSR_AMD_CPPC_ENABLE` 或 `cppc_set_enable`
...
`amd-pstate` 在 `sysfs` 中暴露了几个全局属性（文件）来在系统级别控制其功能。
它们位于
...
驱动允许设置的最大 CPPC 性能和 CPU 频率，以最大支持的 CPPC 性能水平
（在 `AMD CPPC Performance Capability <perf_cap_>`_ 中的最高性能）的百分比表示。
在某些 ASIC 中，最高的 CPPC 性能并不在 `_CPC` 表中，因此我们需要把它暴露到
sysfs。如果 boost 未激活但仍受支持，该最大频率将大于 `cpuinfo` 中的那个。
该属性是只读的。

`amd_pstate_lowest_nonlinear_freq`

驱动允许设置的最低非线性 CPPC CPU 频率，以最大支持的 CPPC 性能水平的百分比表示。
（请参见 `AMD CPPC Performance Capability <perf_cap_>`_ 中的最低非线性性能。）
该属性是只读的。

`amd_pstate_hw_prefcore`

平台是否支持首选核（preferred core）特性并且已启用。该属性是只读的。这个文件
只在支持首选核特性的平台上可见。

`amd_pstate_prefcore_ranking`

该核的性能排名。这个数字没有任何单位，但在读取时数值越大越被优先。它会根据
平台条件在运行时变化。该属性是只读的。这个文件只在支持首选核特性的平台上可见。

`amd_pstate_floor_freq`

与每个 CPU 关联的地板频率（floor frequency）。用户空间可以向该文件写入
`cpuinfo_min_freq` 和 `scaling_max_freq` 之间的任意值。当系统处于功耗或温度
约束下时，平台固件会尝试先将 CPU 频率限制到 `amd_pstate_floor_freq` 中指定的
值，然后再进一步限制。这允许用户空间为不同的 CPU 指定不同的地板频率。为了获得
最佳结果，同一核的线程应具有相同的地板频率值。这个文件只在支持 CPPC 性能优先级
...
sort -n | uniq`` 获得的唯一值的数量，对于 `amd_pstate_floor_freq` 中描述的行为
生效而言，最多应为该数字。零值表示平台支持无限多的地板性能级别。这个文件只在
支持 CPPC 性能优先级特性的平台上可见。

**注意**：当 `amd_pstate_floor_count` 非零时，如果在功率或温度约束下对 CPU 进行
限制的频点，在系统中所有 CPU 的 `amd_pstate_floor_freq` 唯一值数量超过
`amd_pstate_floor_count` 时，是未定义的。

`energy_performance_available_preferences`

可用于本系统 `energy_performance_preference` 的所有受支持的 EPP 偏好的列表。
这些配置文件代表了提供给底层固件的不同提示，关于用户所期望的能效与性能权衡。
`default` 表示 epp 值由平台固件设置。`custom` 表示也可以写入 0-255 的整数值。
该属性是只读的。
...
该属性中提供了粗粒度的命名配置文件
`energy_performance_available_preferences`。
用户也可以写入 0 到 255 之间的单个整数值。
当启用了动态 EPP 时，即使平台固件已启用 EPP 特性，对 energy_performance_preference
的写入也会被阻止。较低的 epp 值会将偏向转向改进性能，而较高的 epp 值会将偏向
转向节能。确切的影响会因平台而异。
...
`boost`
`boost` sysfs 属性提供对 CPU 核性能 boost 的控制，允许用户管理 CPU 的最大
频率限制。该属性可用于在单个 CPU 上启用或禁用 boost 特性。

当 boost 特性启用时，CPU 可以动态地将频率提升到基础频率之上，为要求苛刻的
工作负载提供增强的性能。另一方面，禁用 boost 特性会将 CPU 限制在基础频率下
运行，在某些场景下为了优先考虑能效或管理温度，这可能是可取的。

要操作 `boost` 属性，用户可以使用 sysfs 路径
`/sys/devices/system/cpu/cpuX/cpufreq/boost`（其中 `X` 表示 CPU 编号），向
相应的 CPU 写入值 `0` 来禁用 boost，或写入 `1` 来启用 boost。

其他性能和频率值可以从
`/sys/devices/system/cpu/cpuX/acpi_cppc/` 读回，参见 cppc_sysfs。
...
驱动还会向平台配置文件处理程序（platform profile handler）注册，以接收用户
期望的电源状态通知并做出反应。当设置为禁用时，驱动不会根据电源来源改变能效
性能配置文件，也不会对用户的期望电源状态做出反应。

当 `dynamic_epp` 启用时，尝试手动写入 `energy_performance_preference` sysfs
文件将会失败。

## ``amd-pstate`` 与 ``acpi-cpufreq`` 对比


在 `acpi-cpufreq` 支持的大多数 AMD 平台上，平台固件提供的 ACPI 表用于 CPU
性能缩放，但在 AMD 处理器上仅提供 3 个 P-state。然而，在现代 AMD APU 和 CPU
系列上，硬件根据 ACPI 协议提供协作处理器性能控制，并针对 AMD 平台进行了定制。
也就是说，是细粒度且连续的频率范围，而不是传统的硬件 P-states。`amd-pstate`
是支持未来大多数 AMD 平台上新的 AMD P-States 机制的内核模块。AMD P-States
机制是 AMD 处理器上性能和能效更高的频率管理方法。


## ``amd-pstate`` 驱动运行模式


`amd_pstate` CPPC 有 3 种运行模式：自主（active）模式、非自主（passive）模式
...
这是由 `amd_pstate_epp` 驱动实现的底层固件控制模式，通过在命令行向内核传递
`amd_pstate=active` 来启用。在此模式下，`amd_pstate_epp` 驱动向硬件提供一个
提示，表示软件想要偏向性能（0x0）还是能效（0xff）到 CPPC 固件。然后 CPPC 电源
算法将根据电源供应和温度、核电压以及其他一些硬件条件计算运行时工作负载并调整
实时核频率。
...
`amd_pstate` 驱动软件在 CPPC 性能刻度中以一个相对数字指定期望的 QoS 目标。
这可以表示为标称性能（基础设施最大值）的百分比。在标称持续性能水平以下，
期望性能表示受性能降低容差（Performance Reduction Tolerance）寄存器约束的
...
驱动请求最小和最大性能水平，平台在该范围内自主选择一个适合当前工作负载的
性能水平。
...
最大频率。因此，AMD 重新定义了部件最大频率的概念。这意味着一部分核可以达到
最大频率。为了给给定场景找到最佳的进程调度策略，OS 需要通过 CPPC 接口的最高
性能能力寄存器获知平台告知的核排序。

`amd-pstate` 首选核使调度器优先调度在能够以更低电压达到更高频率的核上。首选核
排名可以根据工作负载、平台条件、温度和老化而动态变化。

优先级度量将由 `amd-pstate` 驱动初始化。`amd-pstate` 驱动还将确定平台是否支持
`amd-pstate` 首选核。

`amd-pstate` 驱动将在系统启动时提供一个初始的核排序。平台使用 CPPC 接口将核
排名传达给操作系统和调度器，以确保 OS 优先选择具有最高性能的核来调度进程。当
`amd-pstate` 驱动收到最高性能变化的消息时，它将更新核排名并设置 cpu 的优先级。
...
当 AMD pstate 处于自动模式时，动态 EPP 将控制内核是否自主更改 EPP 模式。默认
...
`amd-pstate` 在 `sysfs` 中暴露了几个全局属性（文件）来在系统级别控制其功能。
它们位于 `/sys/devices/system/cpu/amd_pstate/` 目录，并影响所有 CPU。

`status`
	驱动的运行模式："active"、"passive"、"guided" 或 "disable"。

	"active"
		驱动处于可用状态，并处于 `active mode`

	"passive"
		驱动处于可用状态，并处于 `passive mode`

	"guided"
		驱动处于可用状态，并处于 `guided mode`

	"disable"
		驱动已注销，当前不可用。

        可以写入该属性以更改驱动的运行模式或注销它。写入的字符串必须是其可能值
        之一，如果成功，向该 sysfs 文件写入这些值之一将导致驱动切换到该字符串
        所代表的运行模式——或在 "disable" 情况下被注销。

`prefcore`
	驱动的首选核状态："enabled" 或 "disabled"。
...
`amd-pstate` 由 `cpupower` 工具支持，该工具可用于转储频率信息。目前正在开发中，
以支持越来越多的
...
事件，通常由 `CPUFreq` 使用；另一个是 `amd_pstate_perf` 跟踪事件，特定于
`amd-pstate`。可以使用以下 shell 命令序列来启用它们并查看其输出（如果内核是
...
```
`cpu_frequency` 跟踪事件会由 `schedutil` 缩放调控器（对于它所附加的策略）或
由 `CPUFreq` 核心（对于使用其他缩放调控器的策略）触发。


### 跟踪工具（Tracer Tool）


`amd_pstate_tracer.py` 可以记录和解析 `amd-pstate` 跟踪日志，然后生成性能图。
该工具可用于调试和调优 `amd-pstate` 驱动的性能。该跟踪工具需要导入 intel pstate
跟踪器。

跟踪工具位于 `linux/tools/power/x86/amd_pstate_tracer`。它有两种使用方式。如果
跟踪文件可用，则直接解析该文件
...
```
测试结果可以在 `results/test_name` 中找到。以下是示例
...
`amd-pstate-ut` 是一个用于测试 `amd-pstate` 驱动的测试模块。
...
1. 测试用例描述
    ...
    2). Tbench 测试
        ...
        在指定调控器下运行 tbench 基准测试时，测试并监控 cpu 的变化。
        这些变化包括期望性能、频率、负载、性能、能耗等。
        指定的调控器是 ondemand 或 schedutil。
        Tbench 也可以在 `acpi-cpufreq` 内核驱动上进行测试以作比较。

    3). Gitsource 测试

        在指定调控器下运行 gitsource 基准测试时，测试并监控 cpu 的变化。
...
#. 如何执行测试
   ...
   自测试 [^4^]_）。
    ...
    1). 构建
        ...
        - 将 `CONFIG_X86_AMD_PSTATE_UT` 配置选项设置为 M。
        - 构建工程
```
