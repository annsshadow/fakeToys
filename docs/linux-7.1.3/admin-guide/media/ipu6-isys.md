...
Intel IPU6 可以在某Intel SoC 中找到，但并非所SKU 都有
- Tiger Lake
- Jasper Lake
- Alder Lake
- Raptor Lake
...
输入系统主要作为 MIPI CSI-2 接收器工作，接收并处理来自传感器的图像数据，并将输出到内存
共有 2 个驱动模块——intel-ipu6 intel-ipu6-isys。intel-ipu6 是一IPU6 通用
驱动，负PCI 配置、固件加载与解析、固件认证、DMA 映射以及 IPU-MMU（内部内映射单元）配置。intel_ipu6_isys 实现V4L2、Media Controller V4L2 子设接口。IPU6 ISYS 驱动支持通过 V4L2 子设备传感器驱动连接IPU6 ISYS 的摄像头传感器
	 有关 IPU6 硬件的信息
## 输入系统驱动
...
以下IPU6 ISYS Dell XPS 9315 笔记本上进行 raw 捕获的示例。在该机器上，ov01a10
传感器连接到 IPU ISYS CSI-2 端口 2，可以以 1280x800 分辨率生sBGGR10 图像
使用媒体控制API，我们可以通过 media-ctl [#f1]_ yavta [#f2]_ 配置 ov01a10
传感器，将帧传输IPU6 ISYS
...
配置好媒体流水线之后，可以使yavta 工具设置所需的传感器特定设置（例如曝光和增益
设置）
例如
...
以下是另一个示例，Lenovo X1 Yoga 笔记本上从摄像头传感ov2740 进行 IPU6 ISYS
RAW 和元数据捕获
