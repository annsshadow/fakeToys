...
- 你是个勇敢者，想尝试新东西- 如果你想报告一bug- 如果你正在开发新补丁

你应该使用主媒体开发树`master` 分支
    https://git.linuxtv.org/media.git/

在这种情况下，你可以`LinuxTv wiki 页面 <https://linuxtv.org/wiki>`_ 找到一些有用的信息
...
`HDMI CEC RC integration` 选项启用 HDMI CEC Linux 的集成，允许像接收由直接连接到机器的遥控器产生的数据一样，通过 HDMI CEC 接收数据
`HDMI CEC drivers` 选项允许选择通过 HDMI 接口接收或发CEC 码的平臺USB 驱动\ [^3^]_
...
```
       ...
       使用媒体 HDMI CEC 支持时
       这些特定GPU 的驱动通过 `Device Drivers` 下的 `Graphics support`
       菜单选择
       当某GPU 驱动支持 HDMI CEC 时，它会自动在媒体子系统启用 CEC 核心支持
### 媒体依赖


应当注意，从一个干净的配置开始启用上述选项通常还不够。媒体子系统依赖于若干其Linux 核心支持才能工作
...
```
根据你想启用的具体驱动，可能还需要其他核心功能（PCI USB 支持）
### 启用遥控器支
...
`Support for eBPF programs attached to lirc devices` 选项允许使用特殊程序（称eBPF），让应用程序能够向 Linux 内核添加额外的遥控器解码功能
`Remote controller decoders` 选项允许选择将被 Linux 内核识别的协议。除非你想禁用某个特定的解码器，否则建议保持所有子选项启用
`Remote Controller devices` 允许你选择支持你的设备所需的驱动
同样的配置也可以通过 `script/config` 脚本设置。例如，为了支持 ITE 遥控```
...
当某个驱动需HDMI CEC 支持时，它会自动设置。因此，你只需要启用对需要它的显卡的支持，或者通过某个现有HDMI 驱动来启用
特定HDMI 的驱动位`HDMI CEC drivers`
```
...
```
除非你确切知道自己在做什么，或者你想为 SoC 平台构建一个驱动，否则强烈建议保持 `Autoselect ancillary drivers` 选项开启，因为它会自动选择所需I²C 辅助驱动
现在有如下所述的两种方式来选择媒体设备驱动
##### ``Filter media drivers`` 菜单


此菜单旨在简PC 和笔记本电脑硬件的设置。它的工作方式是让用户指定需要哪类媒体驱动，
```
...
一旦选择了此菜单上的选项，构建系统就会自动选择所需的核心驱动，以支持所选功能
   ...
   如果你有一张混合卡，可能需要在菜单中同时启`Analog TV`
   鍜?`Digital TV`銆。
...
如果禁用 `Filter media drivers` 菜单，所有依赖已满足、可用于你的系统的驱动都应该显示`Media drivers` 菜单中
但请注意，你应当首先确保 `Media Core Support` 菜单具备你的驱动所需的所有核心功能，否则相应的设备驱动不会显示
### 示例


为了[this table <cx231xx-cardlist>](this table <cx231xx-cardlist>) 中列出的某一块板卡启用模块化支持，并配合模块化的媒体核心模块```
...
有一`Linux Kernel backports 项目
<https://backports.wiki.kernel.org/index.php/Main_Page>`_，其中包含旨在针对稳定内核编译的较新驱动
负责维护媒体子系统的 LinuxTV 开发者也维护了一backport 树，其中只包含每天从最新内核更新的媒体驱动。该树位于：
...
如果你发现它有任何问题，欢迎Linux 媒体子系统的邮件列表提交补丁：media@vger.kernel.org。如果你media-build 提交新补丁，请在邮件主题中添`[PATCH media-build]`
```
...
```
    ...
    2) 如果你过去曾为与你当前使用的不同内核版本构建过它，你可能需要执行一`make distclean`    3) 默认情况下，它会使用你正在运行的内核中为媒体定义的相同配置选项
为了选择不同的驱动或不同的配置选项```
