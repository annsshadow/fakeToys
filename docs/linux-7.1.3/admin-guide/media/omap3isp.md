...
本文档描述了位于 drivers/media/platform/ti/omap3isp 的 Texas Instruments OMAP 3 图像信号
处理器（ISP）驱动。原始驱动由 Texas Instruments 编写，但此后在 Nokia 被重写（两次）。

该驱动已成功用于以下版本的 OMAP 3：

- 3430
- 3530
- 3630

该驱动实现了 V4L2、Media controller 和 v4l2_subdev 接口。支持内核中使用 v4l2_subdev 接口的
传感器、镜头和闪光灯驱动。
...
OMAP 3 ISP 被拆分为 V4L2 subdev，ISP 内部的每个模块都有一个 subdev 来表示它。每个 subdev
都向用户空间提供一个 V4L2 subdev 接口。
...
ISP 中的所有模块，除 CSI-2 以及可能还有 CCP2 接收器外，都坚持接收完整的帧。因此传感器
绝不能向 ISP 发送不完整的帧。

至少在 3430 上，autoidle 与某些 ISP 模块存在问题。autoidle 仅在 3630 上、且 omap3isp 模块
参数 autoidle 非零时启用。
