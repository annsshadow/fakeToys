...
用于无状uAPI 开发的虚拟无状态解码器设备
该工具的目标是为使用 V4L2 无状API 解码媒体的用户空间应用程序的开发与测试
提供帮助。即使没有可用硬件，或者编解码器的内核 uAPI 尚未合入主线，用户空实现也可以使visl 来运行解码循环。这有助于在早期阶段发现缺陷
该驱动还可以跟踪提交给它V4L2 控件的内容。它还可以通过 debugfs 接口转储 vb2
缓冲区的内容。这在许多方面类似于其他流行编解API 所提供的跟踪基础设施，并可以通过以另一个（可用的）实现作为参考来帮助开发用户空间应用程序
...
- visl_debug：激活调试信息，通过 dprintk 打印各种调试消息。同时控制是否显  逐帧调试信息。默认关闭。注意，启用此功能会通过串口导致性能下降
- visl_transtime_ms：模拟的处理时间（毫秒）。降低解码速度有助于调试
- visl_dprintk_frame_start、visl_dprintk_frame_nframes：指定启dprintk   帧范围。这仅按帧控dprintk 跟踪。注意，通过串口打印大量数据可能较慢
- keep_bitstream_buffers：控制解码会话结束后是否保留码流（即 OUTPUT）缓冲区  默认false 以减少杂乱。当使用 GDB 实时调试客户端程序时  keep_bitstream_buffers == false 表现良好
- bitstream_trace_frame_start、bitstream_trace_nframes：与 visl_dprintk_frame_start  visl_dprintk_nframes 类似，但改为控制通过 debugfs 转储缓冲区数据
- tpg_verbose：在每个输出帧上写入额外信息以便于调API。当设为 true 时，给定
  输入的输出帧不再稳定，因为会向其中添加指针或队列状态等信息
### 该驱动的默认用例是什么？


该驱动可用于比较不同的用户空间实现。这假设先运行一个可用的客户端对visl然后利用 ftrace OUTPUT 缓冲区数据来调试一个正在开发中的实现
尽管实际上并未进行视频解码，但输出帧仍可作为给定输入的参考，除非 tpg_verbose
被设true
根据 tpg_verbose 参数的值，关于参考帧、其时间戳、OUTPUT CAPTURE 队列的状态等
...
### 通过 debugfs 转储 OUTPUT 缓冲区数

如果启用**VISL_DEBUGFS** Kconfig，visl 会根bitstream_trace_frame_start bitstream_trace_nframes 的值，OUTPUT 缓冲区数据填**/sys/kernel/debug/visl/bitstream**。这可以发现错误，因为存在缺陷的客户端可无法正确填充缓冲区
为每个已处理OUTPUT 缓冲区创建一个单独的文件。其名称包含一个表示缓冲区序号
的整数，即：
