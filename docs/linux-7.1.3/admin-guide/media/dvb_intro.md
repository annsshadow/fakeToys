...
数字电视（Digital TV）与模拟电视（Analogue TV）之间一个重大区别——不谨慎的人（比如我自己）应当考虑到——是：尽DVB-T 卡的组成结构与模拟电视卡大体相似，但它们的运作方式却大不相同
...
模拟电视卡产生的数字数据流由卡上的电路生成，通常以未压缩形式呈现。对于以 768x576 分辨率4 位彩色像素、每25 帧编码的 PAL 电视信号，会产生相当大量的数据，必须先由 PC 处理才能在视频监视器屏幕上显示。一PC 用的模拟电视卡带有板MPEG2 编码器，可使原始数字数据流以编码且压缩的形式呈现PC——类似于数字电视中使用的形式
一块简单的入门级数字电视卡（DVB-T、C S）的用途很简单：

- 调谐接收器以接收广播信号 从广播信号中提取编码后的数字数据流- 使编码后的数字数据流（MPEG2）可供数据总线使用
...
Linux DVB 的设备驱API 会通过 devfs 文件系统创建以下设备节点
...
`/dev/dvb/adapter0/ca0` 用于解码加密频道。其他设备节点仅存在于使av7110 驱动的设备上，该驱动（及其所用额API）现已被废弃
#### 接收数字电视频道
...
1. 发射机频率（Mount Dandenong，维多利亚州，澳大利亚）
