...
重要提示：该驱动不支Technisat USB 2 设备
首先用发行版自带的内核启动你Linux 主机

	lspci -vvv （对 PCI 设备）或 lsusb -vvv （对 USB 设备）会显示例如	02:0b.0 Network controller: Techsan Electronics Co Ltd B2C2 FlexCopII DVB chip /
	Technisat SkyStar2 DVB card (rev 02)

	dmesg | grep frontend 可能显示例如	DVB: registering frontend 0 (Conexant CX24123/CX24109)...

### 内核编译

如果 Flexcop / Technisat 是你主机中唯一DVB / TV / Radio 设备，请去掉不必要的模块并选中以下项：

`Multimedia support` => `Customise analog and hybrid tuner modules to build`

在此目录中取消勾选其中所有已激活的驱动（除用于第三ATSC `Simple tuner support` —见情9）
然后请激活：
...
- 主模块部分：
  ...
  #) => `Technisat/B2C2 Air/Sky/Cable2PC PCI` （PCI 卡）  #) => `Technisat/B2C2 Air/Sky/Cable2PC USB` （USB 1.1 适配器）
     以及用于故障排除  #) => `Enable debug for the B2C2 FlexCop drivers`
