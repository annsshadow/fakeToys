...
Mali-C55 ISP 接收来自传感器的数据，数据可以是原始 Bayer 格式或 RGB/YUV 格式，
通过并行接口或内存总线进入，经处理后由内部 DMA 引擎输出。存在两条可能的
输出流水线（不过具体实现可能只装配了一条），分别称为“全分辨率（Full resolution）”
和“缩放（Downscale）”，但此命名是历史沿用，两条流水线都具备裁剪/缩放能力。
全分辨率流水线还能输出 RAW 数据，绕过 ISP 的大部分处理；缩放流水线则不能输出
RAW 数据。集成的测试图案发生器可在没有连接相机传感器时驱动 ISP 并产生图像数据。
驱动模块名为 mali_c55，通过 CONFIG_VIDEO_MALI_C55 配置选项启用。

该驱动实现了 V4L2、Media Controller 与 V4L2 Subdevice 接口，并期望连接到 ISP
的相机传感器具备 V4L2 子设备接口。
...
下面给出一个 ISP 拓扑的示例（实现于带 IMX415 相机传感器与通用 CSI-2 接收器的
系统）：
...
- `mali_c55 resizer fr`：全分辨率流水线的 resizer
- `mali_c55 resizer ds`：缩放流水线的 resizer

该驱动拥有 3 个 V4L2 视频设备：

- `mali-c55 fr`：全分辨率流水线的采集设备
- `mali-c55 ds`：缩放流水线的采集设备
- `mali-c55 3a stats`：3A 统计信息采集设备
...
**mali-c55 isp**
`mali-c55 isp` 子设备有一个单一的 sink 衬垫（pad），所有数据源都应连接到它。
通过启用相应的媒体链路并禁用其他所有链路来选择活跃的数据源。ISP 有两个 source
衬垫，反映了其内部路由数据的不同路径。ISP 内部的抽头点（tap point）允许用户分流
数据，以避开部分或全部硬件处理步骤。下图仅用于说明旁路机制如何工作，并非对
那些处理步骤的真实反映；有关高层功能框图，请参阅 ARM 的 Mali-C55 开发者页面：
```
...
```
    ...
    - - 1
      - source
      - RGB/YUV 数据，连接到 FR 与 DS V4L2 子设备

    - - 2
      - source
      - RAW bayer 数据，连接到 FR V4L2 子设备

ISP 的输入与输出分辨率均限制在 640x480 到 8192x8192 之间，这一点体现在 ISP 与
resizer 子设备的 .set_fmt() 操作中。

**mali-c55 resizer fr**
`mali-c55 resizer fr` 子设备有两个 *sink* 衬垫，以反映硬件中不同的插入点（RAW 或
去马赛克后的数据）：
...
如果去马赛克路由处于活跃状态，则 FR 流水线只能以 RGB/YUV 格式输出。如果 raw 路由
处于活跃状态，则输出反映输入（可以是 Bayer 或 RGB/YUV 数据）。

## Using the driver to capture video（使用驱动采集视频）


利用媒体控制器 API，我们可以将输入源与 ISP 配置为以多种格式采集图像。在以下
...
第一步是通过启用正确的媒体链路来设定我们期望的输入源。使用上面的示例拓扑，
我们可以如下选择 TPG：
...
驱动会等待所有视频设备都调用了 VIDIOC_STREAMON ioctl 之后，才告知传感器开始
流式传输。为此，我们需要启用到想要使用的视频设备的链路。在下面的示例中，我们
启用了到两个图像采集视频设备的链路：
...
要从源端采集 1920x1080 的 bayer 数据，并将其推过 ISP 的完整处理流水线，我们需要在
源、ISP 与 resizer 子设备上适当地配置数据格式，并将 FR resizer 的路由设为选择
已处理的数据。resizer source 衬垫上的媒体总线格式将是 RGB121212_1X36 或
YUV10_1X30，取决于你想要采集 RGB 还是 YUV。ISP 的去马赛克块原生输出 RGB 数据，
将 source 衬垫格式设为 YUV10_1X30 会启用色彩空间转换块。

在本示例中，我们以 RGB565 输出为目标，因此选择 RGB121212_1X36 作为 resizer source
衬垫的格式：
...
resizer sink 衬垫的 crop 与 compose 矩形，并在视频设备上设置格式：
...
全分辨率与缩放两条流水线都能将图像缩小至多 8 倍，前提是遵守最小 640x480 的
输出分辨率。为获得最佳图像效果，各方向的缩放比应相同。要配置缩放，我们使用
resizer sink 衬垫上的 compose 矩形：
...
如果我们需要输出 YUV 数据而非 RGB，则需要启用色彩空间转换块，方法是在 resizer
的 source 衬垫上设置 MEDIA_BUS_FMT_YUV10_1X30。随后我们可以配置一个采集格式，
例如 NV12（此处为其多平面变体）：
...
任何图像质量块，但仍可以按常规方式裁剪/缩放数据。因此，输入 ISP 的 RGB 数据
仍然经过 ISP 子设备的衬垫 1 进入 resizer。

为实现这一点，ISP sink 衬垫的格式被设为 MEDIA_BUS_FMT_RGB202020_1X60——这反映了
数据要与 ISP 协同工作所必须具备的格式。将相机传感器的输出转换为该格式是外部硬件
的职责。

在本示例中，我们让测试图案发生器为我们提供 RGB 数据而非 bayer 数据：
...
要配置 raw 旁路，需要先配置 FR resizer 子设备的路由表，然后在适当位置设置格式：
...
## Capturing ISP Statistics（采集 ISP 统计信息）


ISP 能够产生统计信息，供运行在用户空间的图像处理算法使用。这些统计信息可以在
ISP 流式传输期间，通过向 `mali-c55 3a stats` V4L2 设备排队缓冲区来采集。仅支持
...
缓冲区的布局由 `mali_c55_stats_buffer` 描述，但大体上统计信息是为支持三种图像
处理算法而生成的：AEXP（自动曝光）、AWB（自动白平衡）与 AF（自动对焦）。这些
统计信息可以取自 Mali-C55 ISP 流水线中的不同位置，即所谓的“抽头点（tap points）”。
下面这个高层框图旨在说明这些统计信息在何处生成：
```
...
可对 AEXP 与 AWB 统计信息的抽头点进行配置。


## Programming ISP Parameters（编程 ISP 参数）


ISP 可以从用户空间以各种参数进行编程，以便在视频流开始前及进行中应用到硬件。
这使用户空间能够动态改变诸如黑电平、白平衡与镜头阴影增益等数值。
