...
如果你已经编译并安装bttv，只需启动内核就应当足以让它尝试探测。不过，取决于型号，内核可能需要关于硬件的额外信息，因为设备可能无法直接向内核提供此类信息

如果 bttv 没能工作，很可能是它无法自动探测到你的卡，需要一insmod 选项。bttv 最重要insmod 选项"card=n"，用于选择正确的卡类型。如果你得到了视频但没有声音，你很可能指定了错误（或没有指定）的卡类型。受支持卡的列表Documentation/admin-guide/media/bttv-cardlist.rst

如果 bttv 加载非常慢（在使用没有调谐器的廉价卡时有时会发生），尝试把以下内容加入你的模块配置文件（通常它是 `/etc/modules.conf` `/etc/modules-load.d/` 下的某个文件，但具体位置取决于你的发行版）：

```
	options i2c-algo-bit bit_test=1

```
某些卡可能需要一个额外的固件文件才能工作。例如，对于 WinTV/PVR，你需要其驱动 CD 中的一个名`hcwamc.rbf` 的固件文件。它在一个名`pvr45xxx.exe` 的自解压 zip 文件中。只需把它放在 `/etc/firmware` 目录下，就足以让它在驱动探测模式（例如内核启动时或手动通过 `modprobe` 命令加载驱动时）被自动加载

如果你的卡没有列Documentation/admin-guide/media/bttv-cardlist.rst，或者你在让音频工作时遇到困难，请阅still_doesnt_work

### 自动探测


bttv 使用 PCI 子系ID 来自动探测卡类型。lspci 在第二行列出子系ID，看起来像这样：
...
```
只有基于 bt878 的卡可以有子系统 ID（这并不意味着每张卡真的都有）。bt848 卡不能有子系ID，因此无法被自动探测。在 Documentation/admin-guide/media/bttv-cardlist.rst 有一份带有这ID 的列表（如果你感兴趣或想发补丁更新）


### 仍然不工作？


我家里没有摆着 30 多块不同采集板的实验室，也没PAL/NTSC/SECAM 测试信号发生器，所以我常常无法复现你的问题。这使得调试对我来说非常困难
...
对于声音问题：世界各地用于电视声音的系统有很多不同种类。并且也有不同的芯片来解码音频信号。关于声音问题的报告立体声不工作"）除非你包含一些关于你的硬件以及你所在国家使用的电视声音制式的细节（或至少是你所居住的国家），否则基本没有用处
...
它可能是 bttv 驱动bug，也可能是有问题的硬件，也可能是别的原因…

仅仅给我发一"bttv freezes"（bttv 冻结）的邮件是帮不上什么忙的。这README 中有一些提示，可以帮你定位问题
...
对于硬冻结，你大概不会在日志文件中找到任何东西。捕获任何内核消息的唯一方法是接上一个串口控制台，并让某个终端应用程序记录消息me 使用 screen。关于设置串口控制台的细节，请参Documentation/admin-guide/serial-console.rst

阅读 Documentation/admin-guide/bug-hunting.rst，了解如何从内核在保护故障（所"kernel oops"）时打印的寄存器+栈转储中获取任何有用的信息

如果你遇到某种死锁，可以尝试sysrq-t 转储每个进程的调用栈跟踪（参Documentation/admin-guide/sysrq.rst）。这样就能弄清楚处于 "D" 状态的进程 **确切** 卡在哪里

我见过这样的报告：对某些人来bttv 0.7.x 崩溃，0.8.x 工作得非常稳定。因此大概是 bttv 0.7.x 中某处还残留着一个小buglet。我不知道确切在哪里，它对我和许多其他人都稳定工作。但如果你在 0.7.x 版本上遇到问题，可以尝试一0.8.x…


#### 硬件 bug


某些硬件无法处理 PCI-PCI 传输（即采集=> vga）。有时问题就因为 PCI 总线上的高负载而出现在 bttv 上。bt848/878 芯片对已知的兼容性问题有几个变通方法，参见 README.quirks

有些人报告说提高 PCI latency（延迟）也有帮助，虽然我不确定这到底是真正修复了问题，还是只是让它不太可能发生。bttv btaudio 都有一insmod 选项来设置设备的 PCI 延迟

某些主板在正确处理多个设备同时进DMA 时有问题。bttv + ide 有时会导致这种情况，如果是这样，你大概只会在视频和硬盘访问同时进行时看到冻结。更IDE 驱动以获取针对硬bug 的最新最全的变通方法，可能会修复这些问题
...
IRQ 共享在某些情况下已知会引起问题。理论上和在许多配置中它工作得很好。不过，值得一试去重新摆放 PCI 卡，bttv 另一IRQ，或者让它和别的硬件共享 IRQ。与 VGA 卡共IRQ 有时似乎会带来麻烦。我也见bttv ACPI 桥（以及启用apci 的内核）共享 IRQ 时的奇怪现象

### Bttv 兼容性（quirks


下面bt878 数据手册关于 bt878 芯片 PCI bug 兼容模式的说明

triton1 insmod 选项设置控制寄存器中EN_TBFX 位。vsfx insmod 选项EN_VSFX 位做同样的事情。如果你有稳定性问题，可以尝试其中一个选项是否能让你的机器稳定工作

drivers/pci/quirks.c 了解这些问题，这样这些位就会为已知的bug 芯片组自动启用（查看内核消息，bttv 会告诉你）

#### 普PCI 模式


PCI REQ 信号是对输入的功能请求的逻辑或（logical-or）。内部的 GNT[0:1] 信号GNT 异步选通，并由音频请求信号解复用。因此仲裁器在加电时默认为视频功能，并在没有总线访问请求时停在那里。这是可取的，因为视频会更频繁地请求总线。不过，音频将拥有最高的总线访问优先级。因此，即使音频在视频请求之后、但PCI 外部仲裁器授予对 Bt879 的访问之前发出请求，音频也将首先获得总线访问权。一旦某个功能上了总线，另一个功能就无法抢占它。把整个视频 PCI FIFO 倒空PCI 总线上的时间，相对于音频 PCI FIFO 所能容忍的总线访问延迟来说非常短
...
当启EN_TBFX 时，仲裁器确保满足这两个兼容规则。在 PCI 仲裁器断言 GNT 之前，这个内部仲裁器仍然可以将两个请求逻辑或起来。然而，一GNT 被发出，这个仲裁器必须锁定它的决定，现在只把被授予的请求路由REQ 引脚。仲裁器决定锁定不管 FRAME 的状态如何都会发生，因为它不知道 FRAME 何时会被断言（典型情况是——每个发起方会在 GNT 之后的周期断言 FRAME）。当 FRAME 被断言时，移除其请求是发起方的责任。允许这个请求流经到 REQ 而不允许另一个请求保REQ 被断言，是仲裁器的责任。决定锁定可以在事务结束时解除：例如，当总线空闲时（FRAME IRDY）。然后仲裁器决定可以继续异步进行，直GNT 再次被断言


#### 与不符合 PCI 2.1 的核心逻辑接口


一小部分核心逻辑设备可能GNT 被撤销的同一周期启动一个总线事务。这不符PCI 2.1。为确保与使用这PCI 控制器的 PC 兼容，必须启EN_VSFX 位（参见104 页设备控制寄存器）。在这种模式下，仲裁器不会把 GNT 传递给内部功能，除REQ 被断言。这防止了总线事务GNT 被撤销的同一周期启动。这也有一个副作用，即无法利用总线停放（bus parking），从而降低了仲裁性能。Bt879 驱动必须查询这些不兼容的设备，并且仅在需要时设置 EN_VSFX 位
...
```
如果某个配置项同时从 tvcards 数组insmod 选项指定，则 insmod 选项优先
...
所有带Bt848/Bt848a/Bt849/Bt878/Bt879 以及普Composite/S-VHS 输入的卡都受支持。通过软件中的 VBI 采样解码，所有卡都支持图文电视（Teletext）和 Intercast（仅 PAL）
...
MV-Delta
- Bt848A
- 4 Composite 输入 S-VHS 输入（与4 composite 共享
- EEPROM

http://www.matrix-vision.de/

这张卡没有调谐器，但支持 Bt848A 的全4 composite（其1 个与 S-VHS 输入共享）。如果你只有卫星电视、但有多个调谐器通过 composite 连到卡上，这是一张非常不错的卡

非常感谢 Matrix-Vision 免费给了我们 2 张卡，使Bt848a/Bt849 单晶振操作支持成为可能！！！
...
- Bt848
  有些（全部？？）2 个晶振，用于 PAL/SECAM NTSC
- PAL、SECAM NTSC 电视调谐器（Philips TEMIC
- MSP34xx 声音解码器在附加板上
  解码器受支持，但据我所知还不能
  （GPIO 端口中需要其他声MUX 设置？？？有人修复了这个问题吗？？？
- 1 个调谐器 composite 1 S-VHS 输入
- 调谐器类型自动探
...
非常感谢这张免费卡，1997 年的首个 NTSC 支持成为可能
...
- Philips SAA5246 SAA5284（或无）图文电视解码器芯
  带缓RAM（例Winbond W24257AS-35: 32Kx8 CMOS 静RAM
  SAA5246（I2C 0x22）受支持

- 256 字节 EEPROM: Microchip 24LC02B Philips 8582E2Y
  带有配置信息
  I2C 地址 0xa04LC02B 也响0xa2-0xaf

- 1 个调谐器 composite 和（取决于型号）1 S-VHS 输入

- 14052B: 用于选择声音源的复用器（mux

- 声音解码 TDA9800、MSP34xx（立体声卡）


#### Askey CPH 系列


TelSignal 开发，由许多厂OEM（Typhoon、Anubis、Dynalink
...
识别:
  ...
  #) PCB 上有一个印
     ...
     - LR50 Rev.Q = Flyvideo 98（带 eeprom PCI 子系ID
     - LR50 Rev.W = Flyvideo 98（无 eeprom
     - LR51 Rev.E = Flyvideo 98 EZ（仅采集
     - LR90       = Flyvideo 2000（Bt878
     - LR90 Flyvideo 2000S（Bt878）带立体TV（包装含 LR91 子板
     - LR91       = LR90 的立体声子卡
     - LR97       = Flyvideo DVBS
     - LR99 Rev.E = 用于 OEM 集成的薄型卡（仅内部音频！）bt878
     - LR136	 = Flyvideo 2100/3100（薄 SAA7130/SAA7134
     - LR137      = Flyvideo DV2000/DV3000（SAA7130/SAA7134 + IEEE1394
     - LR138 Rev.C= Flyvideo 2000（SAA7130
     - LR138 Flyvideo 3000（SAA7134）带立体TV

 - 这些存在FM 和带 Remote 的变体，有时用后缀 "FM" "R" 表示
  ...
  #) 你有一台笔记本（miniPCI 卡）:
      ...
      - Lifeview.com.tw 说明002 2 月）:
        "FlyVideo2000 FlyVideo2000s 产品名已重命名为 FlyVideo98
        它们Bt8x8 卡被列为已停产
      - Flyvideo 2000S 在某些国家（欧洲？）可能作为 Flyvideo 3000 出售
        新的 Flyvideo 2000/3000 是基SAA7130/SAA7134 的

"Flyvideo II" 曾是 848 卡的名称，如今（在德国）这个名字被重新用LR50 Rev.W

Lifeview 网站曾在某些时候提Flyvideo III，但这样的卡尚未见过（也许它LR90 [立体声] 的德文名）。这些卡也被许多 OEM 出售

FlyVideo A2（Elta 8680 LR90 Rev.F（带 Remote，无 FM，立体声 TV tda9821）{德国}
...
```
  ...
  - 50868 "TV/FM Tuner Pal I"0682 的变体）
  - 50999 "TV/FM Tuner Secam"0682 的变体）
...
Mentor TV card55-878TV-U1" Pixelview 878TV(Rev.3F)（带 FM Remote
...
- 更多
  ...
  - PV-BT878P+rev.9B（Play TV Pro, 可选带 FM NICAM
  - PV-BT878P+rev.2F
  - PV-BT878P Rev.1D (bt878, 仅采
...
##### Leadtek WinView 601 TV/FM 的支


本节的作 Jon Tombs <jon@gte.esi.us.es>

这张卡基本上和所有其他卡一样（Bt484A, Philips 调谐器），主要区别是它们把可编程衰减器接到了 3 GPIO 线上，以提供一些音量控制。它们还在板上装了一个红外遥控解码器，等有时间我会加上对它的支持（它很简单，每次按键产生一个中断，键码放在 GPIO 端口中）

我还没有任何应用程序来测试收音机支持。调谐器频率设置应当能用，但音频复用器可能是错的。如果它不工作，给我发邮件

- 不感Leadtek，他们拒绝回答任何关于其硬件的问题。这个驱动是通过目视检查卡写出来的。如果你使用这个驱动，给他们发一封辱骂邮件，告诉他们除非他们支持 Linux，否则你不会再购买他们的硬件
...
要使用这个驱动，我使用以下选项，调谐器pll 设置可能在你的国家不同。你可以通过 modprobe 参数强制设置

```
    modprobe bttv  tuner=1 pll=28 radio=1 card=17

```
设置调谐器类1（Philips PAL_I），28 MHz 晶振PLL，启FM 收音机，并选择 bttv ID 17（Leadtek WinView 601）
...
DE 对这些型号有各种驱动（截2002 09 月）:
  ...
  - TVCapture48）带 Philips 调谐FI12X6
  - TVCapture48）非 Philips 调谐
  - TVCapture98（Bt878
  - TVPhone98（Bt878
  - AVerTV TVCapture98 VCR（Bt 878
  - AVerTVStudio TVPhone98 VCR（Bt878
  - AVerTV GO Series（无 SVideo 输入
  - AVerTV98（BT-878 芯片
  - AVerTV98 Fernbedienung（遥控）（BT-878 芯片
...
- Terra TValueRadio,             PCB 上印"LR102 Rev.C"
- Terra TV/Radio+ Version 1.0,   PCB 上印"80-CP2830100-0" TTTV3,
  PCB 鑳岄潰鏈?"CPH010-E83", SAA6588T, TDA9873H
- Terra TValue Version BT878,    PCB 上印"80-CP2830110-0 TTTV4",
  鑳岄潰鏈?"CPH011-D83"
- Terra TValue Version 1.0       "ceb105.PCB"（与 Terra TV+ Version 1.0 完全相同
- Terra TValue New Revision	  "LR102 Rec.C"
- Terra Active Radio Upgrade（tea5757h, saa6588t

- LR74 ceb105 的一个较PCB 修订版（两者都含用Active Radio Upgrade 的连接器

- Cinergy 400（saa7134 PCB 上印"E877 11(S)", "PM820092D"
- Cinergy 600（saa7134
...
Hollywood plus（em8300, em9010, adv7175 (PCB "M340-10") MPEG DVD 解码
...
- TT-DVB-Sat
 ...
 - Galaxis DVB Sat
   ...
   - 如今这张卡称TT-PCline Premium Family
...
- ITC VideoMaker（saa7146, StreamMachine sm2110, tvtunerPV-SM2210P+ (REV:1C)"
...
- KWORLD KW-TV713XRF（saa7134
 ...
 MPEG TV Station（与上述相同的卡，加WinDVR 软件 MPEG 解码器）
...
JTT-02（JTT TVTV watchmate pro"（bt848
...
TV Excel = "PV-BT878P+ 8E" "878TV Rev.3\_" 的澳大利亚名
...
- Primetime TV（ISA
  ...
  - 被新加坡科技（Singapore Technologies）收
...
Maxron MaxTV/FM Radio（KW-TV878-FNT Kworld JW-TV878-FBK
...
虽然通过有技巧的谷歌搜索找到 www.techmakers.com
...
SL DVB-S PCI = Technotrend Budget PCI（su1278 bsru 版本

#### Origouk）www.origo2000.com
...
- 鏉跨壒瀹。
  ...
  - Hauppauge Win/TV pci（version 405
    ...
    - Philips SAA5246AGP/E: 图文电视解码器芯 I2C 0x22-0x23

    - TDA9800: 声音解码

    - Winbond W24257AS-35: 32Kx8 CMOS 静RAM（图文电视缓冲内存）
...
- Gerd Hoffmann，提供了 MSP3400 支持和模块化I2C、调谐器……支持


- MATRIX Vision，免费给了我2 张卡，使单晶振操作的支持成为可能
