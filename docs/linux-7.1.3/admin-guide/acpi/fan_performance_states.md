...
当代表风扇的 ACPI 设备（例如 PNP0C0B 或 INT3404）下存在可选的 _FPS 对象时，
ACPI 风扇驱动会在该 ACPI 设备的 sysfs 目录中创建额外的 “state*” 属性。这些
属性列出了风扇性能状态的属性。

有关 _FPS 的更多信息，请参考 ACPI 规范：

http://uefi.org/specifications

例如，INT3404 ACPI 设备 sysfs 目录的内容
...
```
其中每个 “state*” 文件代表风扇的一个性能状态，并包含一个以冒号分隔的 5 个
整数（字段）的列表
...
```
- `control_percent`：用于通过 _FSL 对象（0-100）将风扇速度设置为特定级别的
  百分比值。
...
```
当给定字段未被填充，或其由平台固件提供的值无效时，会显示 “not-defined” 字符串
而非该值。

## ACPI 风扇细粒度控制


当 _FIF 对象指定支持细粒度控制时，可以通过 _FSL 对象将风扇速度从 0 设置到
100%（带有推荐的最小“步长”）。用户可以使用热 sysfs 冷却设备调整风扇速度。

这里用户可参考风扇性能状态中的参考速度（speed_rpm），并通过更改冷却设备的
cur_state 来设置它。如果支持细粒度控制，用户还可以调整到性能状态中未定义的
其他速度。

细粒度控制的支持通过 sysfs 属性 “fine_grain_control” 呈现。如果存在细粒度
控制，该属性显示 “1”，否则显示 “0”。

该 sysfs 属性与性能状态位于同一目录中。
...
该速度通过属性 “fan_speed_rpm” 在 sysfs 中呈现，与性能状态位于同一目录。
