...
cgroup 子系"blkio" 实现了块 IO 控制器。在存储层级中，无论是叶
节点还是中间节点，似乎都需要各种类型的 IO 控制策略（例如按比例带宽
最大带宽）。其计划是对 blkio 控制器使用相同的基于 cgroup 的管理接口，
并根据用户选项在后台切IO 策略

一IO 控制策略是节流（throttling）策略，可用于在设备上指IO 速率
...
```
root 组在指定设备上指定带宽速率。其格式
```

        echo "8:16  1048576" > /sys/fs/cgroup/blkio/blkio.throttle.read_bps_device

```
这将对设备主/次设备号8:16 上发生的读操作施1MB/秒的限制
...
```
启用"sane_behavior" 的节流能正确处理层级关系。对于节流，所有限
都作用于整个子树，而所有统计信息仅针对cgroup 内任务直接生成的 IO
的本地数据

未启cgroup "sane_behavior" 的节流，实际上会把所有组视为同一
层级，就好像看起来像下面
```
...
### 按比例权重策略文
  ...
  blkio.bfq.weight
	  ...
	  除非被每设备规则覆盖（见下面`blkio.bfq.weight_device`）

	  当前允许的权重范围是 1 1000。更多细节，参见
...
```
  ...
  blkio.sectors
	  该组在磁盘上传输（读或写）的扇区数。前两个字段指定设备
	  主设备号和次设备号，第三个字段指定该组在设备上传输的
	  扇区数

  blkio.io_service_bytes
	  该组在磁盘上传输的字节数。这些字节数进一步按操作类型
	  （读或写、同步或异步）划分。前两个字段指定设备的主设备
	  和次设备号，第三个字段指定操作类型，第四个字段指
	  字节数

  blkio.io_serviced
	  该组向磁盘发出的 IO 数（bio）。这IO 数进一步按操作类型
	  （读或写、同步或异步）划分。前两个字段指定设备的主设备
	  和次设备号，第三个字段指定操作类型，第四个字段指IO 数

  blkio.io_service_time
	  cgroup 所发出 IO 从请求分发到请求完成的总耗时。采用纳
	  单位以便对闪存设备也有意义。对于队列深度为 1 的设备，该时
	  代表实际服务时间。当 queue_depth > 1 时，这不再成立，因为请求
	  可能乱序得到服务。这可能导致某个给定 IO 的服务时间包含了
	  多个 IO 的服务时间（当乱序服务时），从而可能造成
  ...
  blkio.io_wait_time
	  cgroup IO 在调度器队列中等待服务的总时间。由于它
	  所IO 累积io_wait_time，因此可能大于总经过时间。它不是
	  衡量 cgroup 等待总时间的指标，而是衡量其各IO 等待时间
	  指标。对于队列深> 1 的设备，该指标不包含 IO 被分发到
	  设备后直到实际得到服务所等待的时间（由于设备对请求的重排序，
	  这里可能存在时间差）。采用纳秒单位以便对闪存设备也有意义
	  该时间进一步按操作类型（读或写、同步或异步）划分。前两个
	  字段指定设备的主设备号和次设备号，第三个字段指定操作类型
	  第四个字段指io_wait_time（单位为 ns）

  blkio.io_merged
	  合并到属于该 cgroup 的请求中bios/请求总数。进一步按
	  操作类型（读或写、同步或异步）划分
  ...
  blkio.avg_queue_size
	  ...
	  cgroup 在其整个存在期间的平均队列大小。每当该 cgroup 
	  某个队列获得一个时间片时采集队列大小样本

  blkio.group_wait_time
	  仅在 CONFIG_BFQ_CGROUP_DEBUG=y 时启用的调试辅助
	  这是 cgroup 自变为繁忙（即从 0 个请求排队变1 个请求排队）
	  起，到为其某个队列获得时间片所等待的时间。这io_wait_time
	  不同，后者是cgroup 中每IO 在调度器队列中等待的累积
	  总时间。采用纳秒单位。如果在 cgroup 处于等待（等待时间片
	  状态时读取该统计，则只会报告截至上次获得时间片为止累积
  ...
  blkio.empty_time
	  ...
	  如果cgroup 处于空状态时读取该统计，则只会报告截至上次有
	  待处理请求为止累积的 empty_time，不包含当前的增量

  blkio.idle_time
	  仅在 CONFIG_BFQ_CGROUP_DEBUG=y 时启用的调试辅助
	  这是 IO 调度器为了期待来自其他队cgroup 的更好请求
	  为给cgroup 空闲等待的时间。采用纳秒单位。如果在 cgroup
	  处于空闲状态时读取该统计，则只会报告截至上个空闲周期为
	  累积idle_time，不包含当前的增量

  blkio.dequeue
	  仅在 CONFIG_BFQ_CGROUP_DEBUG=y 时启用的调试辅助。它提供
	  关于一个组从设备的服务树中出队次数的统计。前两个字段指定
	  设备的主设备号和次设备号，第三个字段指定一个组从特定设
	  出队的次数

  blkio.*_recursive
	  各种统计的递归版本。这些文件显示的信息与其非递归对应
...
### 各策略之间通用的文
  ...
  blkio.reset_stats
	  向该文件写入一int 将重置该 cgroup 的所有统计信息
