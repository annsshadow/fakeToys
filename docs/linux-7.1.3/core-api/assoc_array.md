...
9. 只要迭代器持RCU 读锁，就可以在数组被修改的同时遍历它。但请注意，在这种情况下，某些对象可能会被看到多次。如果这是个问题，迭代器应当加锁以阻止修改。不过，除非对象被删除，否则不会被遗漏
10. 数组中的对象可以通过其索引键进行查找
11. 只要执行查找的线程持RCU 读锁，就可以在数组被修改的同时查找对象
...
公共 API 可以`<linux/assoc_array.h>` 中找到。该关联
...
这也用于在脚本应用之后跟踪失效（dead）块和失效对象，以便它们稍后被释放。释放是在经过一RCU 宽限期之后进行的——从而允许访问函数在 RCU 读锁下继续执行
```
...
索引键以机器字为单位读取。每个字被细分为每层一个半字节 位），因此在 32 CPU 上这适用8 层，64 CPU 上适用16 层。除非分散确实很差，否则不太可能需要用到某个特定索引键的多于一个字
...
- 一NULL 指针，表示该槽位为空- 一个指向对象（叶子）的指针- 一个指向下一层节点的指针- 一个指向快捷方式的指针
...
```
在上述示例中，有 7 个节点（A-G），每个16 个槽位（0-f）。假设树中没有其他元数据节点，键空间划分如下
    ===========     ====
    KEY PREFIX      NODE
    ===========     ====
    137*            D
...
树的根节点有可能是一个快捷方式——例如，假设树中包含至少 17 个键前缀均为 `1111` 的节点。插入算法将插入一个快捷方式，一次性跳`1111` 键空间，直达这些节点实际产生差异的第四层

### 拆分与合并节

每个节点的最大容量为 16 个叶子和元数据指针。如果插入算法发现它正试图向一个节点中插入17 个对象，那么该节点将被拆分，使得至少两个在该层具有共同键段（key segment）的叶子最终进入一个以该共同键段槽位为根的新节点
...
每个节点和快捷方式都包含一个指向其父节点的反向指针，以及在父节点中指向它的槽位号。非递归迭代利用这些指针自底向上（rootwards）地遍历树，转到父节点、槽N + 1，以确保无需栈即可取得进展
然而，这些反向指针使得同时修改与迭代变得棘手

### 同时修改与迭

有若干种情况需要考虑
1. 简单插替换。这只需在屏障（barrier）之后，用一个指向新叶子的指针替NULL 或旧的匹配叶子指针。除此之外元数据块不会改变。旧叶子RCU 宽限期之后才会被释放
2. 简单删除。这只涉及清除一个旧的匹配叶子。除此之外元数据块不会改变。旧叶子RCU 宽限期之后才会被释放
...
5. 插入替换我们正在处理其依赖分支的节点。这在我们沿反向指针回溯之前不会影响我们。与）类似
6. 删除合并我们下方的分支。这不会影响我们，因为反向指针会使我们在看到新节点之前就回到新节点的父节点。整个被合并的子树原样被丢弃——并且仍将以同一个槽位为根，因此我们在回到槽+ 1 时不应再次处理它
...
失效的块和叶子在经过一RCU 宽限期之后才会被释放，因此只要任何执行遍历或迭代的人持有 RCU 读锁，旧的支撑结构就不应当从它们下面消失