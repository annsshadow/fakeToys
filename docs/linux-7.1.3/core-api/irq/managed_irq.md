...
CPU 关联。如果该 CPU 离线，中断会被迁移到另一个在线 CPU。

具有大量中断向量的设备会占用可用的向量空间。例如，在至少有 128 个 CPU 的系统上，一个
具有 128 个 I/O 队列的 NVMe 设备通常每个队列请求一个中断。因此两个这样的设备请求 256
个中断。在 x86 上，中断向量空间 notoriously 很低，每个 CPU 仅提供 256 个向量，且内核保留
了其中一部分，进一步减少了可用于设备中断的数量。在实践中这不是问题，因为中断被分布到
许多 CPU 上，因此每个 CPU 只接收少量向量。

然而，在系统挂起期间，所有次级 CPU 都离线，所有中断都被迁移到唯一在线的 CPU。这可能
耗尽该 CPU 上可用的中断向量，并导致挂起操作失败。

亲和性受管中断解决了这一限制。每个中断被赋一个 CPU 亲和性掩码，指定该中断可以被定向到的
CPU 集合。当掩码中的一个 CPU 离线时，中断被移动到掩码中的下一个 CPU。如果掩码中的最后一个
CPU 离线，该中断被关闭。使用亲和性受管中断的驱动必须确保在中断被禁用之前相关的队列已静止，
以免产生进一步的中断。当亲和性掩码中的一个 CPU 重新上线时，该中断被重新启用。

### 实现


设备必须提供每实例中断，例如 NVMe 这类存储设备的每 I/O 队列中断。驱动使用 struct
irq_affinity 分配具有所需亲和性设置的中断向量。对于 MSI-X 设备，这是通过带有
PCI_IRQ_AFFINITY 标志的 pci_alloc_irq_vectors_affinity() 完成的。

基于提供的亲和性信息，IRQ 核心尝试把中断均匀地散布到整个系统。亲和性掩码在这一分配步骤中
计算，但最终的 IRQ 分配是在调用 request_irq() 时执行的。

### 隔离的 CPU


受管中断的亲和性完全在内核中处理，无法通过 /proc 接口从用户空间修改。isolcpus 启动选项的
managed_irq 子参数指定一个 CPU 掩码，受管中断应当尽量避免。这种隔离是尽力而为的，仅当自动
分配的中断掩码也包含被避开掩码之外的在线 CPU 时才适用。如果请求的掩码只包含隔离的 CPU，
则该设置不起作用。

列在避开掩码中的 CPU 仍然是中断亲和性掩码的一部分。这意味着如果所有非隔离的 CPU 离线而
隔离的 CPU 仍然在线，该中断会被分配给其中一个隔离的 CPU。

以下示例假设一个具有 8 个 CPU 的系统。

- 一个 QEMU 实例以 "-device virtio-scsi-pci" 启动。该 MSI-X 设备暴露 11 个中断：3 个
  "管理"中断和 8 个 "队列"中断。驱动请求这 8 个队列中断，每个都刚好亲和于一个 CPU。
  如果该 CPU 离线，该中断被关闭。
...
```
- 一个 QEMU 实例以 "-device virtio-scsi-pci,num_queues=2" 启动，并且内核命令行包含：
  "irqaffinity=0,1 isolcpus=domain,2-7 isolcpus=managed_irq,1-3,5-7"。该 MSI-X 设备暴露
  5 个中断：3 个管理中断和 2 个队列中断。管理中断遵循 irqaffinity= 设置。
```
