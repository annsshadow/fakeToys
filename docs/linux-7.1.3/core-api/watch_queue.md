...
该机制表现为一个以特殊模式打开的管道。管道内部的环形缓冲区用于保存由内核生成的消息。这些消息随后由 read() 读出。此类管道上禁用了 splice 及类似操作，因为它们在某些情况下可能希望撤销对环形缓冲区的添加——而这最终可能会与通知消息交错在一起。

管道的所有者必须告诉内核它希望通过该管道监视哪些来源。只有已连接到某个管道的来源才会向其中插入消息。请注意，一个来源可以绑定到多个管道，并同时向所有这些管道插入消息。

也可以在管道上放置过滤器，以便在不感兴趣时忽略某些来源类型和子事件。

如果环形缓冲区中没有可用的槽位，或者没有可用的预分配消息缓冲区，则消息将被丢弃。在这两种情况下，read() 会在缓冲区中当前最后一条消息被读取之后，向输出缓冲区插入一条 WATCH_META_LOSS_NOTIFICATION 消息。
...
「info」表示多项内容，包括：
  ...
  - 消息的长度（以字节为单位，含头部）（用 WATCH_INFO_LENGTH 掩码，并右移 WATCH_INFO_LENGTH__SHIFT）。这表示记录的大小，介于 8 到 127 字节之间。

  - 监视 ID（用 WATCH_INFO_ID 掩码，并右移 WATCH_INFO_ID__SHIFT）。这表示监视的调用者 ID，介于 0 到 255 之间。多个监视可以共享一个队列，这提供了一种区分它们的方法。
...
```
    ...
    初始化一个监视列表。如果 ``release_watch`` 不为 NULL，则它表示在
    watch_list 对象被销毁时应调用的一个函数，用于释放监视列表对被监视对象
    持有的任何引用。

  * ``void remove_watch_list(struct watch_list *wlist);``

    移除订阅到某个 watch_list 的所有监视并释放它们，然后销毁
    watch_list 对象本身。
...
```
`info_id` 值应是一个从用户空间获得的 8 位数，并左移 WATCH_INFO_ID__SHIFT。当通知被写入关联的监视队列缓冲区时，它会通过 OR 运算并入 **struct watch_notification** 的 info 字段中的 WATCH_INFO_ID。

`private` 字段是与 watch_list 关联的驱动数据，由 **``watch_list** : release_watch()`` 方法清理。
...
```
  ...
  * ``int remove_watch_from_object(struct watch_list *wlist, NULL, 0, true);``
    ...
    移除监视列表中的所有监视。预计这将在销毁之前被调用，并且到此为止
    该监视列表对新的监视应当已不可访问。会向每个已订阅监视的监视队列
    发送一个通知（``WATCH_META_REMOVAL_NOTIFICATION``），指示该监视
    已被移除。
...
```
通知应预先格式化，并传入指向头部（`n`）的指针。通知可能大于此大小，以缓冲区槽位为单位的尺寸记录在 `n->info & WATCH_INFO_LENGTH` 中。
...
```
其中 "nr_filters" 是 filters[] 中过滤器的数量，"__reserved"
```
...
其中：
  ...
  - `type` 是要过滤的事件类型，应为类似
    "WATCH_TYPE_KEY_NOTIFY" 的值

  - `info_filter` 和 `info_mask` 用作对 info 字段的过滤器，其
```
