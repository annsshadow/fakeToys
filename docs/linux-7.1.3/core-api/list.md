...
的目的并非泛泛地解释链表，而是向新的内核开发者展示如何使Linux 内核的链表实现
请注意，尽管链表确实无处不在，但在简单的数组已经无法满足需求的场合，链表很少是
最佳的数据结构选择。特别是，由于其数据局部性（data locality）较差，在需要考虑性能情形下，链表是个糟糕的选择。强烈建议去熟悉内核中的其他通用数据结构，尤其是在并访问方面
## Linux 双向链表的实

Linux 的链表实现可以通过包含头文`<linux/list.h>` 来使用
...
双向链表中的节点，是通过在你想放入链表的数据结构中添加一struct list_head 成员声明的：
...
带有指向前一个和后一个链表节点的指针，以及负载（payload）数据。Linux 选择这种做法是因为它允许编写通用的链表修改代码，而无需关心链表中包含的是什么数据结构。由struct list_head 成员不是指针，而是数据结构本身的组成部分，链表实现可以使用
container_of() 模式来访问负载数据，而不管其类型是什么，同时对所言之类型究竟是什一无所知
### 声明并初始化一个链

然后，双向链表可以被声明为又一struct list_head，并在初始赋值时LIST_HEAD_INIT()
...
```
该图显示了单独的 "clowns" 节点指向自身。在本图以及之后所有图中，为了清晰起见，只出了正向的边
```
...
```
该图显示"clowns" 节点指向一个标记为 "Grock" 的新节点。Grock 节点指回"clowns"
节点
```
...
```
该图显示"clowns" 节点指向一个标记为 "Dimitri" 的新节点，而该节点又指向标记为
"Grock" 的节点Grock" 节点仍指"clowns" 节点
如果我们想让 Dimitri 插到链表末尾，就应使list_add_tail()。我们的代码将如下所示：
...
```
该图显示"clowns" 节点指向标记"Grock" 的节点，而后者又指向标记"Dimitri" 的新
节点Dimitri" 节点指回 "clowns" 节点
### 遍历链表


要遍历链表，我们可以list_for_each() 循环经过链表中的所有节点
...
list_entry() 宏在内部使用了前面提到的 container_of() 来取`node` 作为其成员的那个
数据结构实例
注意这里额外list_entry() 调用有点笨拙。它之所以存在，是因为我们是在遍`node`
成员，但我们真正想要遍历的是负载，即包含每个节点struct list_head 的那`struct clown`。为此，提供了第二个宏：list_for_each_entry()
...
clown 数据结构中对应于链表struct list_head 的那个成员名，这样它仍然能够遍历链表
### 从链表中移除节点


list_del() 函数可用于从链表中移除条目。它不仅从链表中移除给定的条目，还会毒化
（poison）该条目`prev` `next` 指针，使得移除后对该条目的无意使用不会被忽视
...
```
该图显示"clowns" 节点指向标记"Grock" 的节点，而后者又指回 "clowns" 节点。在一是一个孤立的、标记为 "Dimitri" 的节点，没有任何箭头指向任何地方
...
```
该图显示"clowns" 节点指向标记"Grock" 的节点，而后者又指回 "clowns" 节点。在一是一个孤立的、标记为 "Dimitri" 的节点，它指向自身
### 在遍历的同时移除节点


如果我们使用 list_for_each() list_for_each_entry()，在遍历链表的同时删除条目会
引发问题，因为删除当前条目会修改它的 `next` 指针，这意味着遍历无法正确地前进到
下一个链表条目
不过有一个解决方案：list_for_each_safe() list_for_each_entry_safe()。它们额接受一个指struct list_head 的指针作为参数，用作遍历期间下一个条目的临时存储，从解决该问题
如何使用它的示例
...
有两个辅助函数可用于切割链表。二者都从链`head` 中取出元素，并替换链`list`
的内容
第一个这样的函数list_cut_position()。它`head` 中直到并包括 `entry` 的所有链条目移除，转而从 `list` 中放置它们
```
...
```
通过下面的代码，"clowns" 直到并包"Pic" 的每个小丑都被从 "clowns" 链表头移动到一单独struct list_head，该结构在局部栈变量 `retirement` 处初始化
...
```
第二个函list_cut_before() 基本一样，只是它在 `entry` 节点之前切割，即它移`head`
中直到但不包`entry` 的所有链表条目，转而从 `list` 中放置它们。此例假定与前面例子
相同的初始起始链表：
...
list_move() list_move_tail() 函数可用于将一个条目从一个链表移动到另一个链表，
分别移动到起始或末尾
在下面的例子中，我们假设从两个链表开始（“clowns```
...
```
State 2 中，在我们将 Dimitri 移动sidewalk 的末尾之后，情形变为
```
...
```
只要源链表头与目标链表头属于同一个链表，我们还可以高效地将链表的一段批量移动到链表尾端。我们在前一个例子的基础上，State 2 之后添加一list_bulk_move_tail()，将 Pic
Pio 移动sidewalk 链表的尾端
...
```
请注意，list_bulk_move_tail() 不会检查所给出的三`struct list_head *` 参数是否真的
...
前端的条目被送到后端
对于旋转，Linux 为我们提供了两个函数：list_rotate_left() list_rotate_to_front()前者可以被想象成自行车链条，取所给出`struct list_head *` 之后的那个条目并将其移动尾部，由于链表的环形本质，这实质上意味着整个链表旋转了一个位置
后list_rotate_to_front() 将同一概念推进一步：它不是让链表前进一个条目，而是前进
**直到**指定的条目成为新的前端
```
...
```
接下来，list_rotate_to_front() 调用之后，我们到达如下状```
...
```
对比两图可以明显看出Pic" "Dimitri" 节点交换了位置
### 将两个链表拼接在一

假设我们有两个链表，在下面的例子中，一个由我们称为 "knie" 的链表头表示，另一个由我们
称为 "stey" 的链表头表示。在一次假想的马戏团收购中，这两组小丑应当被拼接在一起。我```
...
```
将这两个链表拼接在一起的函数list_splice()。我们的示例代码如下
...
这里list_splice() 调用`stey` 中的所有条目添加到 `dimitri` `node` 链表头所的链表中，位`dimitri` `node` 之后。一```
...
```
遍历 `stey` 链表不再产生正确的行为。在 `stey` 上调list_for_each() 会导致无限循环，
因为它永远不会回`stey` 链表头
这是因为 list_splice() 没有重新初始化它取走条目的那个链表头，使其指针指向了现在已是
不同的链表的位置
如果我们想避免这种情况，可以使用 list_splice_init()。它做与 list_splice() 相同的事只是会在移植之后重新初始化那个供体链表头
### 并发考量


在大多数情况下，对链表的并发访问与修改需要用锁来保护。另一种（也是更可取的）方式是在“读多写少”的使用场景下，对链表使RCU 原语，即读取链表很常见、但修改链表较少的情形更多细节参见 Documentation/RCU/listRCU.rst
### 延伸阅读
