...
用户空间（客户端）通过打开 /dev/tee[0-9]* 或 /dev/teepriv[0-9]* 来连接到驱动。

- TEE_IOC_SHM_ALLOC 分配共享内存并返回一个文件描述符，用户空间可以对其进行 mmap。当用户空间不再需要该文件描述符时，应当将其关闭。当不再需要共享内存时，应当使用 munmap() 解除映射，以便重用内存。
...
客户端与 TEE 之间的大部分通信对驱动是不透明的。驱动的主要工作是接收来自客户端的请求，将其转发给 TEE，并把结果发回。在请求者的情况下，通信方向相反，TEE 向请求者发送请求，请求者随后将结果发回。
