...
本页记录ntsync 驱动的用户空API
ntsync 是一个用于支持用户空NT 模拟器模NT 同步原语的辅助驱动。它之所存在，是因为使用现有工具在用户空间进行实现，无法在提供准确语义的同时匹配
Windows 的性能。它完全由软件实现，不驱动任何硬件设备
该接口仅作为兼容性工具使用，不应被用于通用同步。应当改用通用、灵活的接口例如 futex(2) poll(2)
## 同步原语


ntsync 驱动暴露三种类型的同步原语：信号量（semaphore）、互斥体（mutex）和
事件（event）
信号量持有一个易失的 32 位计数器，以及一个表示最大值的静32 位整数。当计数非零时，认为它处于有信号（signaled）状态（即可以无竞争地获取，或将唤醒一等待的线程）。当某个等待被满足时，计数器减一。初始计数与最大计数都在信号量
被创建时确定
互斥体持有一个易失的 32 位递归计数，以及一个表示其拥有者的易失 32 位标识符当它的拥有者为零（表示未被拥有）时，认为互斥体处于有信号状态。当某个等待满足时，递归计数加一，并且拥有者被设置为给定的标识符
...
除了零所代表的“未拥有”语义之外，ntsync 驱动对拥有者标识符的实际值完全不解释。其预期用途是存储一个线程标识符；不过，ntsync 驱动实际上并不会校验调用
线程提供的标识符是否一致或唯一
事件类似于最大计数为一的信号量。它持有一个表示其是否有信号的易失布尔状态事件有两种类型：自动复位（auto-reset）与手动复位（manual-reset）。自动复事件在被等待满足时会被解除信号状态（designaled）；手动复位事件则不会被解除事件类型在事件被创建时指定
除非另有说明，针对同一对象的所有操作都是原子的，并且相对于该对象上的其他操完全有序
对象由文件表示。当指向某个对象的所有文件描述符都被关闭时，该对象即被删除
## 字符设备


ntsync 驱动创建一个单一的字符设/dev/ntsync。在该设备上打开的每个文件描述符
都代表一个唯一的实例，用于支撑一个独立的 NT 虚拟机。由一ntsync 实例所创建
的对象，只能与由同一实例创建的其他对象一起使用
## ioctl 参

对该设备的所有操作都通过 ioctl 完成。共有四```
