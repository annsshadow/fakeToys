...
`req_fd`
    GPIO 字符设备的文件描述符，如
    `request.fd<gpio_v2_line_request>` 中 gpio-v2-get-line-ioctl.rst 所返回的那样。

`config`
    要应用到
    请求线路上的新 `configuration<gpio_v2_line_config>`。
...
请求线路时适用的相同 gpio-v2-get-line-config-rules 和
gpio-v2-get-line-config-support 在更新线路配置时同样适用，另加
