...
获取所有已请求线的值
返回的值是逻辑值，指示该线是激活还是非激活。`GPIOHANDLE_REQUEST_ACTIVE_LOW` 标志控制物理值（低）与逻辑值（激非激活）之间的映射。如果未设置 `GPIOHANDLE_REQUEST_ACTIVE_LOW`，则高为激活、低为非激活。如果设置了 `GPIOHANDLE_REQUEST_ACTIVE_LOW`，则低为激活、高为非激活
输入线和输出线的值均可被读取
对于输出线，返回的值取决于驱动和配置，可能是输出缓冲区（最后设置的请求值）或输入缓冲区（线的实际电平），并且根据硬件和配置的不同，二者可能存在差异
ioctl 也可用于读取线事件的线值，`event_fd` 替换`handle_fd`。由于这种情况下只请求了一条线，因`values` 中只返回一个值
## 杩斿洖鍊。
成功时返0，且 `values` 被填充为读取到的值
出错时返-1，并适当设置 `errno` 变量。常见的错误码在 error-codes.rst 中描述