...
######## GPIO_LINEEVENT_DATA_READ
    ...
    该 ioctl 属于 chardev_v1.rst，已被
    gpio-v2-line-event-read.rst 废弃。
...
`event_fd`
    GPIO 字符设备的文件描述符，由 gpio-get-lineevent-ioctl.rst 在
    `request.fd<gpioevent_request>` 中返回。
...
必须为输入线路启用边沿检测，使用 `GPIOEVENT_REQUEST_RISING_EDGE` 或 `GPIOEVENT_REQUEST_FALLING_EDGE`，或两者都使用。随后，只要在该输入线路上检测到边沿中断，就会生成边沿事件。
...
`event.timestamp<gpioevent_data>` 的时钟源为 `CLOCK_MONOTONIC`，但在 Linux 5.7 之前的版本中为 `CLOCK_REALTIME`。`gpioevent_data` 中并未指明使用的是哪个时钟源，必须根据内核版本或对时间戳本身进行合理性检查来确定。
...
可通过使用 `poll()` 或等价方式检查 `event_fd` 是否可读，来测试是否存在事件。
