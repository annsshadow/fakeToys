...
`req_fd`
    GPIO 字符设备的文件描述符，由 gpio-v2-get-line-ioctl.rst     `request.fd<gpio_v2_line_request>` 中返回
`buf`
    用于容纳 `events<gpio_v2_line_event>` 的缓冲区
`count`
    `buf` 中可用的字节数，必须至少    一`gpio_v2_line_event` 的大小
## 描述


从请求中读取线路的边沿检测事件
必须为输入线路启用边沿检测，使用 `GPIO_V2_LINE_FLAG_EDGE_RISING` `GPIO_V2_LINE_FLAG_EDGE_FALLING`，或两者都使用。随后，只要在该输入线路上检测到边沿中断，就会生成边沿事件
...
`request.event_buffer_size<gpio_v2_line_request>` 影响默认大小为所请求线路数量16 倍
...
可通过使用 `poll()` 或等价方式检`req_fd` 是否可读，来测试是否存在事件
## 杩斿洖鍊。
