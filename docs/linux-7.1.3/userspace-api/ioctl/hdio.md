...
本文试图描述 HD/IDE 层所支持的 ioctl(2) 调用。这些调用（截至 Linux 5.11）
大都在 drivers/ata/libata-scsi.c 中实现。

ioctl 的值列于 <linux/hdreg.h>。截至本文撰写时，它们如下：
...
（向用户空间传递参数指针的 ioctl：）
	...
	HDIO_GET_32BIT		get current io_32bit setting（获取当前 io_32bit 设置）
	HDIO_GET_IDENTITY	get IDE identification info（获取 IDE 标识信息）
	HDIO_DRIVE_TASKFILE	execute raw taskfile（执行原始 taskfile）
	HDIO_DRIVE_TASK		execute task and special drive command（执行任务与特殊驱动器命令）
...
（传递非指针值的 ioctl：）
	...
	=======================	=======================================
	HDIO_SET_32BIT		change io_32bit flags（改变 io_32bit 标志）
	=======================	=======================================


下文中的信息是通过阅读内核源代码确定的。随着时间的推移，其中一些内容
很可能会得到修正。
...
General（一般说明）：
	...
	除非另有说明，所有 ioctl 调用在成功时返回 0，失败时返回 -1 并将
	errno 设为适当的值。

	除非另有说明，所有 ioctl 调用在试图向/从用户地址空间拷贝数据失败
...
HDIO_DRIVE_TASKFILE
	...
	注意：
		如果你手边没有 ANSI ATA 规范副本，大概应当忽略此 ioctl。

 - 通过写入驱动器的“taskfile”寄存器，直接执行一条 ATA 磁盘命令。需要
   ADMIN 与 RAWIO 访问权限。
