...
本文档试图描CDROM 层支持的 ioctl(2) 调用。这些调用大体上（截Linux 2.6
实现drivers/cdrom/cdrom.c drivers/block/scsi_ioctl.c

ioctl 值列<linux/cdrom.h> 中。截至本文撰写时，它们如下所示：
	...
	CDROMREADTOCHDR		  读取 TOC (struct cdrom_tochdr)
	CDROMREADTOCENTRY	  读取 TOC 条目 (struct cdrom_tocentry)
	CDROMSTOP		  停止 cdrom 驱动
	CDROMSTART		  启动 cdrom 驱动
	CDROMEJECT		  弹出 cdrom 介质
	CDROMVOLCTRL		  控制输出音量（struct cdrom_volctrl
	CDROMSUBCHNL		  读取子通道数据 (struct cdrom_subchnl)
	CDROMREADMODE2		  读取 CDROM 模式 2 数据336 字节
				  (struct cdrom_read)
	CDROMREADMODE1		  读取 CDROM 模式 1 数据048 字节
				  (struct cdrom_read)
...
General（通用说明）：
	...
	除非另有说明，所ioctl 调用在成功时返回 0，出错时返回 -1 并将
	errno 设置为适当的值。（某些 ioctl 返回非负的数据值。）

	除非另有说明，所ioctl 调用在尝试向用户地址空间复制数据或从中复
	数据失败时返-1，并errno 设置EFAULT

	各个驱动可能返回此处未列出的错误码

	除非另有说明，所有数据结构和常量都定义在 <linux/cdrom.h> 
...
```
CDROMREADMODE1
	读取 CDROM 模式 1 数据048 字节

	(struct cdrom_read)

	notes:
		CDROMREADRAW 相同，只是块大小
		CD_FRAMESIZE048）字



CDROMREADMODE2
	读取 CDROM 模式 2 数据336 字节

	(struct cdrom_read)

	notes:
		CDROMREADRAW 相同，只是块大小
		CD_FRAMESIZE_RAW0336）字
...
CDROMREADALL
	...
	CDROMREADCOOKED 相同，但读取 2646 字节
...
CDROM_CLEAR_OPTIONS
	...
	CDROM_SET_OPTIONS 相同，只是所选选项被关闭
