...
如果你正在为内核新增 ioctl，应当使<linux/ioctl.h> 中定义的 _IO
宏：
...
空间的未使用区块2 256 ioctl 命令通常足够。你可以通过修改
此文件并经由 :doc:`常规的补丁提交流
</process/submitting-patches>` 提交补丁来登记该区块

第二个参数是一个序列号，用于区分不同的 ioctl。第三个参数（不适用
_IO）是进出内核的数据类型（例如 'int' 'struct foo'）

   不要使用 sizeof(arg) 作为第三个参数，因为这会导致你的 ioctl 误以
   它传入了一size_t 类型的参数
...
(1) 保持 ioctl 全局唯一有助于错误检查：
    如果某个程序在错误的设备上调ioctl，它会得到一个错误，
    而不是某些意外行为

(2) 'strace' 的构建过程会自动找到用这些宏定义ioctl 编号

(3) 当编号唯一时，'strace' 能够将编号解码回有用的名称

(4) 当使用此约定来定ioctl 编号时，寻找 ioctl 的人可以更轻松地
    grep 查找它们

(5) 遵循约定时，驱动代码可以使用通用代码在用户空间与内核空间之间
    复制参数

本表列出了从用户空间可见ioctl，不包括来自 drivers/staging/ 的
====  =====  ========================================================= ================================================================
...
0x04  D2-DC  linux/umsdos_fs.h                                         2.6.11 起已废弃，但不要复用这些
0x06  all    linux/lp.h
...
'*'   00-1F  uapi/linux/user_events.h                                  用户事件子系
                                                                       <mailto:linux-trace-kernel@vger.kernel.org>
'1'   00-1F  linux/timepps.h                                           来自 Ulrich Windl PPS 工具
...
'['   00-3F  linux/usb/tmc.h                                           USB 测试与测量设
                                                                       <mailto:gregkh@linuxfoundation.org>
'a'   all    linux/atm*.h, linux/sonet.h                               Linux 上的 ATM
                                                                       <http://lrcwww.epfl.ch/>
'b'   00-FF                                                            冲突bit3 vme 主机
...
'u'   20-3F  linux/uvcvideo.h                                          USB 视频类主机驱
