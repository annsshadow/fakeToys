...
`/sys/class/rc/` 类子目录属于遥控器（Remote Controller）核心，并提供一个用于配置红遥控器接收器sysfs 接口


## /sys/class/rc/rcN/


为每一个遥控器接收器设备创建一`/sys/class/rc/rcN` 目录，其N 是接收器的编号
...
```
已启用的协议显示[] 方括号中
写入 proto会将该协议添加到已启用协议列表中
写入 proto会从已启用协议列表中移除该协议
写入 “proto将仅启用 “proto”
写入 “none将禁用所有协议
...
`/sys/class/rc/rcN/filter_mask` 结合使用，以设置 filter mask 中所设位的期望值。如硬件支持，则不匹配该过滤器的扫描码将被忽略。否则写入将以错误失败
如果当前协议被更改，该值可能被重置0
...
```
注意，此处列出了协议变体，因`nec`、`sony`、`rc-5`、`rc-6` 各自不同的位长度编码会被列出
（如果可用）
注意，所有协议变体都会被列出
已启用的唤醒协议显示[] 方括号中
一次只能选择一个协议
写入 “proto将使“proto作为唤醒事件
写入 “none将禁用唤醒
如果使用了无效的协议组合或未知的协议名，或硬件不支持唤醒，写入将`EINVAL` 失败
...
如果硬件支持wakeup_filter_mask 不为 0，则匹配该过滤器的扫描码将唤醒系统，例如从挂起到
