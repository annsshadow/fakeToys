

## LIRC 函数参考（LIRC Function Reference

本页LIRC（红外遥控）字符设备函数参考的索引，汇总了 lirc 设备ioctl 与读写操作的文档链接（如 lirc-read/write、lirc-get/set-features 等），便于开发者查阅红外设备接口用法


本页面为该分类的文档索引，汇总相关的条目链接；链接目标与标识符保持原文以便于检索

本页列出 LIRC 字符设备ioctl 与读写操作的参考文档



- [lirc-read](lirc-read)
- [lirc-write](lirc-write)
- [lirc-get-features](lirc-get-features)
- [lirc-get-send-mode](lirc-get-send-mode)
- [lirc-get-rec-mode](lirc-get-rec-mode)
- [lirc-get-rec-resolution](lirc-get-rec-resolution)
- [lirc-set-send-duty-cycle](lirc-set-send-duty-cycle)
- [lirc-get-timeout](lirc-get-timeout)
- [lirc-set-rec-timeout](lirc-set-rec-timeout)
- [lirc-set-rec-carrier](lirc-set-rec-carrier)
- [lirc-set-rec-carrier-range](lirc-set-rec-carrier-range)
- [lirc-set-send-carrier](lirc-set-send-carrier)
- [lirc-set-transmitter-mask](lirc-set-transmitter-mask)
- [lirc-set-measure-carrier-mode](lirc-set-measure-carrier-mode)
- [lirc-set-wideband-receiver](lirc-set-wideband-receiver)

