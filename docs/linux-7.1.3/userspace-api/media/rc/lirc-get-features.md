...
获取底层硬件设备的特性。驱动会宣告它所支持的某些特性，调用方再据此发起相应ioctl
...
未使用。保留仅为避免破uAPI。在发送时使用 `LIRC_MODE_PULSE` <lirc-模式-pulse>
...
未使用。保留仅为避免破uAPI。在接收时使`LIRC_MODE_MODE2` <lirc-模式-mode2>
...
成功时返0，出错时返回 -1，并相应地设`errno` 变量。通用的错误码在《Generic 错误 Codesgen-错误> 章节中描述
