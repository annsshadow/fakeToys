...
该系统调用返回经过过滤的数据，可能是段（section）或打包的基本流（PES）数据。过滤后的数据从驱动的内部循环缓冲区传输到 `buf`。要传输的最大数据量由 count 隐含指定。


   如果创建了带有 `DMX_CHECK_CRC <dmx_sct_filter_params>` 标志集的段过滤器，则 CRC 检查失败的数据将被静默忽略。
