...
在此处重置专属于frontend 的数据缓存（cache）。这不会影响硬件
...
数字电视转发器（transponder频道（channel）的频率


  #. 对于卫星传输系统，频率单位为 kHz

  #. 对于有线电视（cable）和地面（terrestrial）传输系统，频率单位Hz

  #. 在大多数传输系统中，频率为转发器/频道的中心频率。ISDB-T 例外，其主载波相
     中心1/7 的偏移

  #. 对于 ISDB-T，频道通常带有143kHz 的偏移进行传输。例如，一个有效的频率可以
     474,143 kHz。步进（stepping）与频道带宽相关，通常6MHz

  #. ISDB-Tsb 中，频道仅由一段或三段组成，频率步进分别为 429kHz*429
...
======================= =======================================================
   ...
   请注意，上述某些调制类型当前可能尚未在内核（Kernel）中定义。原因很简单：尚没
   驱动需要这样的定义
...
======================= =======================================================
  ...
  #. 在卫星和有线电视传输系统中，带宽取决于符号率（symbol rate）。内核会静默忽略任何
     ...
     该带宽估算会考虑DTV-SYMBOL-RATE 设置的符号率，以及滚降（rolloff）因
     （对DVB-C DVB-S 为固定值）
...
电压通常用于不具DiSEqC 能力LNB，以切换极化方式（水垂直）。使DiSEqC
...
如果 `DTV_ISDBT_SOUND_BROADCASTING` '0'，该位字段表示频道是否处于部分接
（partial reception）模式

如果'1'，则 `DTV_ISDBT_LAYERA_*` 的值被分配给中心段（center segment），
`DTV_ISDBT_LAYERA_SEGMENT_COUNT` 必须'1'

如果 `DTV_ISDBT_SOUND_BROADCASTING` 还为 '1'，则 `DTV_ISDBT_PARTIAL_RECEPTION`
表示ISDB-Tsb 频道是由一段一层还是三段两层组成
...
该字段表示其DTV_ISDBT_*-参数所指的是一ISDB-T 频道还是一ISDB-Tsb 频道
（另`DTV_ISDBT_PARTIAL_RECEPTION`）
...
该字段仅`DTV_ISDBT_SOUND_BROADCASTING` '1' 时适用

（作者注：这可能并非`SUBCHANNEL-ID` 全部细节的准确描述，但它是我对编程设
所需技术背景的理解

一ISDB-Tsb 频道 段或 3 段）可以单独广播，也可以成组（set）地与其他相连的
ISDB-Tsb 频道一起广播。在这组频道中，每个频道都可以独立接收。相连的 ISDB-Tsb
段的数量可以变化，例如取决于可用的频谱带宽

示例：假设广播了 8 个相连的 ISDB-Tsb 段。广播方有多种方式将这些频道送上空中
假设一个普通的 13 ISDB-T 频谱，他可以将这 8 段从位置 1-8 对齐5-13，或介于
两者之间的任何位置

段（segment）的下层是子信道（sub-channel）：每个段由若干具有预定ID 的子信道
组成。子信道用于帮助解调器（demodulator）与频道同步

一ISDB-T 频道总是以所有子信道为中心对齐。如上面示例所述，ISDB-Tsb 中就不再
如此简单了

`DTV_ISDBT_SB_SUBCHANNEL_ID` 参数用于给出待解调的段的子信ID
...
`DTV_ISDBT_SB_SEGMENT_IDX` 给出待解调段的索引，用于多个 ISDB-Tsb 频道以相连方
传输的情况
...
`DTV_ISDBT_SB_SEGMENT_COUNT` 给出相连ISDB-Tsb 频道的总数
...
同时解码。因此一ISDB-T 解调器拥3 Viterbi 3 Reed-Solomon 解码器

ISDB-T 3 个分层，每一层可以使用可用段的一部分。所有层的总段数在 ISDB-T 
必须13
...
`DTV_ISDBT_LAYER_ENABLED` 的所有位设为 '1' 会强制解调所有层（如适用）。这
默认行为

如果频道处于部分接收模式（`DTV_ISDBT_PARTIAL_RECEPTION` = 1），中心段可以独立于
其他 12 段被解码。在该模式下，层 A `SEGMENT_COUNT` 必须1

ISDB-Tsb 中仅使用A，根`DTV_ISDBT_PARTIAL_RECEPTION`，它可以1 3
`SEGMENT_COUNT` 必须相应地填写

仅使用前 3 位的值。其他位将被静默忽略

`DTV_ISDBT_LAYER_ENABLED` 0：启用层 A

`DTV_ISDBT_LAYER_ENABLED` 1：启用层 B

`DTV_ISDBT_LAYER_ENABLED` 2：启用层 C

`DTV_ISDBT_LAYER_ENABLED` 3-31：未使用
...
给定 ISDB 层所使用的前向纠错（Forward Error Correction）机制，`fe_code_rate`
定义
...
给定 ISDB 层所使用的调制（modulation），`fe_modulation` 定义

可能的值为：`QAM_AUTO`、`QPSK`、`QAM_16`、`QAM_64`、`DQPSK`


   #. 如果C `DQPSK`，则B 必须`DQPSK`

   #. 如果B `DQPSK` `DTV_ISDBT_PARTIAL_RECEPTION` = 0，则层必须为
      `DQPSK`銆。
...
注意：`DTV_ISDBT_SOUND_BROADCASTING`、`DTV_ISDBT_PARTIAL_RECEPTION` 
...
DTV_ISDBT_SOUND_BROADCASTING 处于激活状态时，8 也是有效的

注意：实际的时域交织（time interleaving）长度取决于模式（fft 大小）。此处的
指的是可TMCC 结构中找到的内容，如下表所示
...
一parade 是至8 MH 组（group）的集合，承载一个或两个 ensemble
...
指定 parade 中每MH 子帧（subframe）的 MH 组（group）数量
...
MH 组的总数，包含属于一MH 子帧中所MH parade 的全MH 组
...
可接受的值由 `fe_guard_interval` 定义
   ...
   #. 如果 `DTV_GUARD_INTERVAL` 设置`GUARD_INTERVAL_AUTO`，硬件将尝试找到正确
      保护间隔（guard interval）（若支持），并使用 TMCC 填充缺失的参数
   #. 间隔 `GUARD_INTERVAL_1_64` 仅用DVB-C2
   #. 间隔 `GUARD_INTERVAL_1_128` 同时用于 DVB-C2 DVB_T2
   #. 间隔 `GUARD_INTERVAL_19_128` `GUARD_INTERVAL_19_256` 仅用DVB-T2
   #. 间隔 `GUARD_INTERVAL_PN420`、`GUARD_INTERVAL_PN595` 
      `GUARD_INTERVAL_PN945` 当前仅用DMTB。在该标准下，仅这些间隔
...
仅用于基OFDM 的标准，例如 DVB-T/T2、ISDB-T、DTMB

指定该标准所使用FFT 大小（对应于载波的近似数量）

可接受的值由 `fe_transmit_mode` 定义


   #. ISDB-T 支持三种载波/符号大小KKK。在该标准中称为**模式（mode*
      并从 1 3 编号
   ...
   #. 如果 `DTV_TRANSMISSION_MODE` 设置`TRANSMISSION_MODE_AUTO`，硬件将尝试找到
      正确FFT 大小（若支持），并使TMCC 填充缺失的参数

   #. DVB-T 规定 2K 8K 为有效大小

   #. DVB-T2 规定 1KKKK6K 32K
...
用于 DVB-C2、DVB-S2、DVB-T2 ISDB-S

DVB-C2、DVB-S2、DVB-T2 ISDB-S 支持在单一传输流（transport stream）上传输多个
流（stream）。当硬件支持时，该属性使数字电视驱动能够处理子流过滤（substream
filtering）。默认情况下，子流过滤是禁用的

对于 DVB-C2、DVB-S2 DVB-T2，有效的子流 id 范围0 255

对于 ISDB，有效的子流 id 范围1 65535

要禁用它，应使用特殊NO_STREAM_ID_FILTER
...
打算使用多标准前端的应用程序必须枚举与其关联的传输系统，而不是尝试使
FE_GET_INFO。对于遗留前端，结果FE_GET_INFO 相同，但格式更具结构化
...
硬件可能提供可控LNA，可通过该参数手动设置。通常 LNA 只在地面设备中存在（如果
有的话）
...
18 位字段（存在时）承载 DVB-S2 物理层加扰序列（scrambling sequence）的索引
EN 302 307 5.5.4 节所定义。没有显式的信令方法将加扰序列索引发送给接收端
如果可用的话，可以使S2 卫星传输系统描述符来读取加扰序列索引（EN 300 468
41）

默认使用 gold 加扰序列索引 0

有效的加扰序列索引范围为 0 262142
