...
DVB 音频设备控制着 DVB 硬件MPEG2 音频解码器。可以通过
`/dev/dvb/adapter/audio` 访问它。数据类型与 ioctl 定义可以通过应用程序中包`linux/dvb/audio.h` 来使用
请注意，大多DVB 卡没有自己的 MPEG 解码器，因此会省略音频和视频
设备
这些 ioctl 也曾V4L2 用来控制 V4L2 中实现的 MPEG 解码器。将这类
...
#### 常量
    ...
    - ..
       ...
       - `1` 选择解复用器（由前端DVR 设备提供数据）作为视频流的来源
    - ..
...
音频流来源通过 `AUDIO_SELECT_SOURCE`_ 调用设置，可取值如下，取决我们是回放内部（demux）还是外部（用户写入）来源
...
此值可`AUDIO_GET_STATUS`_ 调用返回，表示音频播放的状态
...
此结构体`AUDIO_SET_MIXER`_ 调用用来设置音频音量
...
#### 常量
    ...
    - ..
       ...
       - 硬件接受 MPEG-1 Audio Layer 2          也称MUSICAM
    - ..

       - `AUDIO_CAP_MP3`

       - 硬件接受 MPEG-1 Audio Layer III          通常称为 .mp3
    - ..
...
ioctl 调用暂停正在播放的音频流。解码与播放都被暂停。之后可以使`AUDIO_CONTINUE`_ 命令重新开始音频流的解码与播放过程
#### 杩斿洖鍊。
...
ioctl 重新启动先前`AUDIO_PAUSE`_ 命令暂停的解码与播放过程
#### 杩斿洖鍊。
...
ioctl 调用告知音频设备输入数据应使用哪个来源。可能的来源demux
memory。若选择 `AUDIO_SOURCE_MEMORY`，则数据通过 write 命令送入音频
设备。若选择 `AUDIO_SOURCE_DEMUX`，数据则直接从板载解复用设备传输解码器。注意：到目前为止这仅支持具有一个解复用器和一个解码器DVB 设备
#### 杩斿洖鍊。
...
ioctl 仅适用DVB 设备。要控制 V4L2 解码器，请改V4L2
VIDIOC_DECODER_CMD，并带上 `V4L2_DEC_CMD_START_MUTE_AUDIO` 标志
ioctl 调用请求音频设备对当前正在播放的流进行静音
#### 杩斿洖鍊。
...
ioctl 调用请求音频设备旁路音频解码器，并直接转发流而不进行解码当无法被 DVB 系统处理的流需要解码时，应使用此模式。如果硬件支持，
Dolby DigitalTM 流会DVB 子系统自动转发
#### 杩斿洖鍊。
...
#### 参数
    ...
    - ..
       ...
       - 返回受支持的声音格式的位数组          比特位在 `audio encodings`_ 中定义
#### 描述


             See: legacy_dvb_decoder_notes

ioctl 调用请求音频设备告知我们音频硬件的解码能力
#### 杩斿洖鍊。
...
ioctl 调用请求音频设备清空音频解码器设备的所有软件与硬件缓冲区
#### 杩斿洖鍊。
...
如果未设置音频流类型，则对于 MPEG 声音，id 必须[0xC0,0xDF] 范围内；
对于 AC3，在 [0x80,0x87] 范围内；对于 LPCM，在 [0xA0,0xA7] 范围内更多说明请参ITU-T H.222.0 | ISO/IEC 13818-1
如果流类型已通过 `AUDIO_SET_STREAMTYPE`_ 设置，则 id 只表示音频流子流 id，且只识别前 5 个比特（& 0x1F）
#### 杩斿洖鍊。
...
ioctl 告诉驱动程序预期接收哪种音频流。当流提供多种音频子流（LPCM AC3）时，这很有用
...
对于通过 V4L2 控制MPEG 解码器，ioctl 已被 V4L2
`V4L2_CID_MPEG_AUDIO_DEC_MULTILINGUAL_PLAYBACK` 控件取代
ioctl 调用在可能的情况下请求音频设备为双语流选择所请求的声道
#### 杩斿洖鍊。
...
到非阻塞模式（反之亦然）。这是一个标准的系统调用，在 Linux fcntl
手册页中有说明。只有一个用户能O_RDWR 模式打开音频设备。所有其以该模式打开设备的尝试都会失败，并返回错误码。如果以 O_RDONLY 模式
打开音频设备，则唯一可以使用ioctl 调用`AUDIO_GET_STATUS`_所有其他调用都会返回错误码
#### 杩斿洖鍊。
...
此系统调用只能在 ioctl 调用 `AUDIO_SELECT_SOURCE`_ 中选择`AUDIO_SOURCE_MEMORY` 时使用。所提供的数据应PES 格式。如果未指定
`O_NONBLOCK`，该函数将阻塞，直到缓冲区空间可用。要传输的数据量count 隐含给出
#### 杩斿洖鍊。
