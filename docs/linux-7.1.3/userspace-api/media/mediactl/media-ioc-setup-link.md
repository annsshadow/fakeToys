...
`argp`
    指向结构`media_link_desc` 的指针

## Description


为了更改链路属性，应用程序需先用链路的标识信息（pad 和目pad）以及请求的新链路标志填充一
`media_link_desc` 结构体，然后
使用该结构的指针调用 MEDIA_IOC_SETUP_LINK ioctl

唯一可配置的属性是用于启用/禁用链路`ENABLED` 链路标志。被
`IMMUTABLE` 链路标志标记的链路无法被启用或禁用

链路配置不会对其他链路产生副作用。如果在目的 pad 上已启用的链路阻止该链路被启用，驱动会返
`EBUSY` 错误码

只有`DYNAMIC` 链路标志标记的链路才能在媒体数据流传输过程中被启禁用。试图启用或禁用正在传输数据的非动态链路将返回
`EBUSY` 错误码
