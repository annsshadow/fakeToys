...
这些 ioctl 用于获取和设置客户端（使用 subdevice ioctl 的应用程序）能力。客户端
能力存储在打开的 subdev 设备节点的文件句柄中，客户端必须分别为每个打开的
subdev 设置能力。
...
`VIDIOC_SUBDEV_S_CLIENT_CAP` 会修改 struct `v4l2_subdev_client_capability` 以
反映已被接受的能力。内核不接受某能力的一种常见情况是，内核比用户空间使用的
头文件更旧，因此该能力对内核而言是未知的。
    ...
    - - `V4L2_SUBDEV_CLIENT_CAP_STREAMS`
      - 客户端了解流（stream）。设置此标志可启用各种 ioctl 中 'stream' 字段
        （指流编号）的使用。若未设置（默认如此），'stream' 字段将被内核强制
        为 0。
    - - `V4L2_SUBDEV_CLIENT_CAP_INTERVAL_USES_WHICH`
      - 客户端了解 `v4l2_subdev_frame_interval` 的 `which` 字段。若未设置
        （默认如此），`which` 字段将被内核强制为 `V4L2_SUBDEV_FORMAT_ACTIVE`。
