...
本文档给V4L2 视频画面抓取示例程序 v4l2grab.c 的完整源代码，演示如何通过 V4L2 API 打开视频设备、协商图像格式、申请并映射缓冲区，最终抓取一帧图像保存，可作为用户空V4L2 采集程序的参考实现
...
本程序是自由软件；您可以在自由软件基金会发布GNU 通用公共许可证第 2 
或（根据您的选择）任何更高版本的条款下重新分发和/或修改它

本程序旨在为您提供有用之处，但绝不提供任何担保；不提供对适销性或特定用
适用性的暗示担保。详GNU 通用公共许可证
*/
