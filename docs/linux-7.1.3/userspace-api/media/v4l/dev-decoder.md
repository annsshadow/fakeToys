...
H.264/HEVC 流、原始 VP8/9 流），并将其解码为按显示顺序排列的原始视频帧。解码器
在处理这些缓冲区时，不应需要来自客户端的任何额外信息。

强烈不建议在驱动中对该数据流进行软件解析、处理等操作以支持本接口。如果确实需要
此类操作，强烈建议使用无状态（Stateless）视频解码器接口（开发中）。

## 本文档使用的约定与记号

1. 若本文档未另有说明，则通用的 V4L2 API 规则适用。

2. 词语 "must"、"may"、"should" 等的含义以 `RFC
   2119 <https://tools.ietf.org/html/rfc2119>`_ 为准。

3. 所有未标注 "optional" 的步骤都是必需的。

4. 除非另有说明，`VIDIOC_G_EXT_CTRLS` 和 `VIDIOC_S_EXT_CTRLS` 可与
   `VIDIOC_G_CTRL` 和 `VIDIOC_S_CTRL` 互换使用。

5. 单平面（single-planar）API（见 planar-apis）及适用的结构体，与多平面
   （multi-planar）API 可在满足解码器能力并遵循通用 V4L2 指南的前提下互换使用，
   除非另有说明。

6. i = [a..b]：从 a 到 b（含端点）的整数序列，即 i = [0..2] 表示 i = 0, 1, 2。

7. 给定一个 `OUTPUT` 缓冲区 A，则 A' 表示 `CAPTURE` 队列上包含由处理缓冲区 A
   所得数据的缓冲区。

## 术语表

CAPTURE
   目标缓冲区队列；对于解码器，是包含已解码帧的缓冲区队列；对于编码器，是包含
   已编码数据流的缓冲区队列；对应 `V4L2_BUF_TYPE_VIDEO_CAPTURE` 或
   `V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE`；数据由硬件捕获到 `CAPTURE` 缓冲区中。

client
   与实现了本接口的解码器或编码器通信的应用程序。

coded format
   已编码/压缩的视频数据流格式（例如 H.264、VP8 等）；另见：raw format。

coded height
   给定编码分辨率下的高度。

coded resolution
   以像素计的流分辨率，对齐到编解码器和硬件的要求；通常为可见分辨率向上取整到
...
coding tree unit
   HEVC 编解码器的处理单元（对应于 H.264、VP8、VP9 中的宏块单元），可使用最大
   64×64 像素的块结构。擅长将图像细分为可变大小的结构。

decode order
   帧被解码的顺序；如果编码格式包含帧重排序特性，则可能与显示顺序不同；对于解码器，
   `OUTPUT` 缓冲区必须由客户端按解码顺序入队；对于编码器 `CAPTURE` 缓冲区必须
   由编码器按解码顺序返回。
...
DPB
   Decoded Picture Buffer（已解码图像缓冲区）；H.264/HEVC 中的一个术语，指用于存储
...
macroblock
   基于线性块变换的图像和视频压缩格式中的处理单元（例如 H.264、VP8、VP9）；与具体
   编解码器相关，但大多数流行编解码器的尺寸为 16x16 采样（像素）。HEVC 编解码器
   使用一种更灵活的处理单元，称为 coding tree unit（CTU）。

OUTPUT
   源缓冲区队列；对于解码器，是包含已编码数据流的缓冲区队列；对于编码器，是包含
   原始帧的缓冲区队列；对应 `V4L2_BUF_TYPE_VIDEO_OUTPUT` 或
...
raw format
   包含原始像素数据的未压缩格式（例如 YUV、RGB 格式）。

resume point
   数据流中可以开始/继续解码、且不存在任何先前状态/数据的点，例如：关键帧
...
source resolution
   馈送给编码器的源帧的像素分辨率，并受限于进一步裁剪到可见分辨率的边界；仅与
   编码器相关。
...
visible resolution
   可见图像的流分辨率（像素），用于显示目的；必须小于或等于编码分辨率；即
   显示分辨率。
...
1. 要枚举解码器支持的编码格式集合，客户端可以在 `OUTPUT` 上调用
   `VIDIOC_ENUM_FMT`。

   - 无论 `CAPTURE` 上设置的是什么格式，都会返回受支持格式的全部集合。
   - 检查 `v4l2_fmtdesc` 的 flags 字段，以了解解码器相对于每种编码格式的
     能力。尤其是解码器是否具有完备的数据流解析器，以及是否支持动态分辨率
     变化。

2. 要枚举受支持的原始（raw）格式集合，客户端可以在 `CAPTURE` 上调用
   `VIDIOC_ENUM_FMT`。

   - 只会返回当前在 `OUTPUT` 上处于活动状态的格式所支持的格式。

   - 为了枚举某给定编码格式所支持的原始格式，客户端必须先在 `OUTPUT` 上
     设置该编码格式，然后再在 `CAPTURE` 上枚举格式。

3. 客户端可以使用 `VIDIOC_ENUM_FRAMESIZES` 来检测给定格式支持的
   分辨率，方法是把期望的像素格式传入 `v4l2_frmsizeenum` 的 `pixel_format`
   字段。
   ...
   - `VIDIOC_ENUM_FRAMESIZES` 针对原始像素格式返回的值，将包含解码器
     针对给定原始像素格式以及当前在 `OUTPUT` 上设置的编码格式支持的所有
     可能帧缓冲区分辨率。

4. 对于当前在 `OUTPUT` 上设置的编码格式，如果适用，其支持的档次（profile）
   和级别（level）可以通过各自的控件，经由 `VIDIOC_QUERYCTRL` 查询。

## 初始化

1. 通过 `VIDIOC_S_FMT` 在 `OUTPUT` 上设置编码格式。

   - **必填字段：**

     `type`
         `OUTPUT` 适用的 `V4L2_BUF_TYPE_*` 枚举值。
   ...
   - `CAPTURE` 格式会根据 `VIDIOC_S_FMT` 返回的宽度和高度，立即更新为
     合适的帧缓冲区分辨率。但是，对于包含流分辨率信息的编码格式，在解码器完成
     从数据流中解析该信息后，无论其是否与客户端设置的值匹配，它都会用新值更新
...
2. 通过 `VIDIOC_REQBUFS` 在 `OUTPUT` 上分配源（bytestream）缓冲区。
...
3. 通过 `VIDIOC_STREAMON` 在 `OUTPUT` 队列上启动数据流（streaming）。

4. **此步骤仅适用于在流中包含分辨率信息的编码格式。** 继续通过 `VIDIOC_QBUF`
   和 `VIDIOC_DQBUF` 在 `OUTPUT` 队列上入队/出队数据流缓冲区。缓冲区将被
   按顺序处理并返还给客户端，直到找到配置 `CAPTURE` 队列所需的元数据为止。这由
   解码器发送 `changes` 设为 `V4L2_EVENT_SRC_CH_RESOLUTION` 的
   `V4L2_EVENT_SOURCE_CHANGE` 事件来指示。

    - 如果第一个缓冲区包含的数据不足以触发该事件，这不算错误。只要还需要更多
      数据，就会继续处理缓冲区。

    - 如果触发该事件的缓冲区中的数据是解码第一帧所必需的，那么在初始化序列
      完成且该帧被解码之前，该缓冲区不会被返还给客户端。

    - 如果客户端没有自行设置数据流的编码分辨率，那么在 `CAPTURE` 队列上调用
      `VIDIOC_G_FMT`、`VIDIOC_S_FMT`、`VIDIOC_TRY_FMT` 或
      `VIDIOC_REQBUFS`，在发出 `changes` 设为
      `V4L2_EVENT_SRC_CH_RESOLUTION` 的 `V4L2_EVENT_SOURCE_CHANGE` 事件
      之前，都不会返回数据流的真实值。
...
1. 在 `CAPTURE` 队列上调用 `VIDIOC_G_FMT`，以获取从数据流中解析/解码出的
    目标缓冲区的格式。

    - **必填字段：**

      `type`
          `CAPTURE` 适用的 `V4L2_BUF_TYPE_*` 枚举值。
...
2. **可选。** 通过 `VIDIOC_G_SELECTION` 获取可见分辨率。
    ...
    - **返回字段：**
      ...
      `r.left`、`r.top`、`r.width`、`r.height`
          可见矩形；它必须落在 `CAPTURE` 上 `VIDIOC_G_FMT` 返回的帧缓冲区
          分辨率之内。
    ...
    - 在 `CAPTURE` 上支持以下选择目标：
      ...
      `V4L2_SEL_TGT_CROP`
          编码分辨率内将要输出到 `CAPTURE` 的矩形；默认等于
          `V4L2_SEL_TGT_CROP_DEFAULT`；在不具备额外 compose/缩放能力的硬件上
          为只读。
      ...
      `V4L2_SEL_TGT_COMPOSE`
          ...
          `V4L2_SEL_TGT_COMPOSE_DEFAULT`；在不具备额外 compose/缩放能力的硬件上
          为只读。

      `V4L2_SEL_TGT_COMPOSE_PADDED`
          `CAPTURE` 缓冲区中被硬件覆盖的矩形；若硬件不写入填充像素，则等于
          `V4L2_SEL_TGT_COMPOSE`。
...
3. **可选。** 通过 `CAPTURE` 队列上的 `VIDIOC_ENUM_FMT` 枚举
    `CAPTURE` 格式。一旦流信息被解析并已知，客户端可以使用此 ioctl 来发现
...
4. **可选。** 通过 `CAPTURE` 队列上的 `VIDIOC_S_FMT` 设置
    `CAPTURE` 格式。客户端可以选择不同于解码器在 `VIDIOC_G_FMT` 中
    选择/建议的格式。
    ...
    - **必填字段：**
      ...
      `width`、`height`
          已解码流的帧缓冲区分辨率；通常与 `VIDIOC_G_FMT` 返回的值相同，
          但如果硬件支持合成（composition）和/或缩放，则可能不同。

   - 如前一节所述，设置 `CAPTURE` 格式会根据新分辨率将 compose 选择矩形
     重置为它们的默认值。

5. **可选。** 如果期望且解码器具备 compose 和/或缩放能力，通过 `CAPTURE`
   ...
   - **必填字段：**
     ...
     `r.left`、`r.top`、`r.width`、`r.height`
         `CAPTURE` 缓冲区中写入裁剪后帧的矩形；默认等于
         `V4L2_SEL_TGT_COMPOSE_DEFAULT`；在不具备额外 compose/缩放能力的
         硬件上为只读。
...
6. 如果满足以下所有条件，客户端可以立即恢复解码：
    ...
    - 当前已分配的缓冲区数量大于或等于前面步骤中获取的最小缓冲区数量。为满足
      此要求，客户端可以使用 `VIDIOC_CREATE_BUFS` 来新增缓冲区。
    ...
    - 如果 `CAPTURE` 队列未流式传输，则在 `CAPTURE` 队列上调用
      `VIDIOC_STREAMON`。

    但是，如果客户端出于降低内存占用或其他任何原因打算更改缓冲区集合，则可以
    通过执行以下步骤实现。

7. **如果** `CAPTURE` **队列正在流式传输，** 继续在 `CAPTURE` 队列上
    入队和出队缓冲区，直到出一个带有 `V4L2_BUF_FLAG_LAST` 标志的缓冲区。

8. **如果** `CAPTURE` **队列正在流式传输，** 在 `CAPTURE` 队列上调用
...
11. 在 `CAPTURE` 队列上调用 `VIDIOC_STREAMON` 以开始解码帧。

## 解码

`Capture Setup` 序列成功完成后即进入此状态。在此状态下，客户端通过
`VIDIOC_QBUF` 和 `VIDIOC_DQBUF` 按照标准语义向两个队列入队和出队缓冲区。

源 `OUTPUT` 缓冲区的内容取决于当前活动的编码像素格式，并可能受编解码器
特定的扩展控件影响，如每种格式的文档所述。

两个队列独立运行，遵循 V4L2 缓冲区队列和内存到内存（memory-to-memory）设备的
标准行为。此外，由于所选编码格式的特性（例如帧重排序），从 `CAPTURE` 队列
出队的已解码帧的顺序，可能与向 `OUTPUT` 队列入队编码帧的顺序不同。

客户端不得假定 `CAPTURE` 与 `OUTPUT` 缓冲区之间，以及缓冲区可被出队的
任何特定时序之间存在任何直接关系。具体而言：

- 入队到 `OUTPUT` 的缓冲区可能不在 `CAPTURE` 上产生任何缓冲区（例如，如果
  它不包含已编码数据，或者其中仅存在元数据语法结构），

- 入队到 `OUTPUT` 的缓冲区可能在 `CAPTURE` 上产生多于一个缓冲区（如果已编码
  数据包含多个帧，或者返回一个已解码帧使得解码器能够返回一个在解码顺序上位于其
  之前、但在显示顺序上位于其之后的帧），

- 入队到 `OUTPUT` 的缓冲区可能在解码过程的更晚阶段、和/或在处理了更多
  `OUTPUT` 缓冲区之后，才在 `CAPTURE` 上产生缓冲区，或者乱序返回（例如，
  如果使用了显示重排序），

- 即使没有向 `OUTPUT` 额外入队缓冲区，`CAPTURE` 队列上也可能会出现可用
  缓冲区（例如在 drain 或 `EOS` 期间），这是因为过去入队到 `OUTPUT` 的
  某些缓冲区，其解码结果由于解码过程的特性要等到更晚的时刻才可用。

   为了能够将已解码的 `CAPTURE` 缓冲区与产生它们的 `OUTPUT` 缓冲区对应起来，
   客户端可以在入队 `OUTPUT` 缓冲区时设置 `v4l2_buffer` 结构体的 `timestamp`
   字段。由解码该 `OUTPUT` 缓冲区所产生的 `CAPTURE` 缓冲区，在出队时其
   `timestamp` 字段将被设为相同的值。

   除了一个 `OUTPUT` 缓冲区产生一个 `CAPTURE` 缓冲区这种简单情况外，还定义了
   以下情况：

   - 一个 `OUTPUT` 缓冲区产生多个 `CAPTURE` 缓冲区：同一个 `OUTPUT` 时间戳
     将被复制到多个 `CAPTURE` 缓冲区。

   - 多个 `OUTPUT` 缓冲区产生一个 `CAPTURE` 缓冲区：将复制最先入队的
     `OUTPUT` 缓冲区的时间戳。

   - 解码顺序与显示顺序不同（即 `CAPTURE` 缓冲区相对于 `OUTPUT` 缓冲区是
     乱序的）：`CAPTURE` 时间戳将不会保留 `OUTPUT` 时间戳的顺序。


   被流用作参考帧的 `CAPTURE` 缓冲区，其底层内存在出队后仍可能被硬件读取。
   因此，客户端应避免在 `CAPTURE` 队列流式传输期间写入这块内存。否则可能导致
   已解码帧损坏。

   类似地，当使用的内存类型不是 `V4L2_MEMORY_MMAP` 时，客户端应确保在
   `CAPTURE` 队列流式传输期间，每个 `CAPTURE` 缓冲区始终使用相同的底层内存
   入队。原因是 V4L2 缓冲区索引可被驱动用来识别帧。因此，如果参考帧的底层内存
   以不同的缓冲区 ID 提交，驱动可能会误识别它，并在其仍被使用时将新帧解码到其中，
   从而导致后续帧损坏。

在解码过程中，解码器可能会启动下列特殊序列之一。这些序列会导致解码器返回所有
在序列开始之前处理的 `OUTPUT` 缓冲区所产生的 `CAPTURE` 缓冲区。最后一个
缓冲区将带有 `V4L2_BUF_FLAG_LAST` 标志。为了确定需要遵循哪个序列，客户端必须
检查是否存在待处理事件，并且：

- 如果待处理的是 `changes` 设为 `V4L2_EVENT_SRC_CH_RESOLUTION` 的
  `V4L2_EVENT_SOURCE_CHANGE` 事件，则需要遵循 `Dynamic Resolution
  Change` 序列，

- 如果待处理的是 `V4L2_EVENT_EOS` 事件，则需要遵循 `End of Stream` 序列。
...
- 包含失败解码操作结果的 CAPTURE 缓冲区将被返回，并带有 V4L2_BUF_FLAG_ERROR 标志，

- 如果解码器能够精确报告触发错误的 OUTPUT 缓冲区，则该缓冲区将被返回，并带有
  V4L2_BUF_FLAG_ERROR 标志。

如果发生不允许继续解码的致命失败，则对该解码器文件句柄的任何进一步操作都会返回
-EIO 错误码。客户端可以关闭该文件句柄并打开一个新的，或者通过在两个队列上停止
流式传输、释放所有缓冲区并再次执行 `Initialization` 序列来重新初始化实例。

## 定位（Seek）

Seek 由 `OUTPUT` 队列控制，因为它是已编码数据的来源。seek 不需要对 `CAPTURE`
...
1. 通过 `VIDIOC_STREAMOFF` 停止 `OUTPUT` 队列以开始 seek 序列。
   ...
   - 解码器将丢弃所有待处理的 `OUTPUT` 缓冲区，它们必须被视为已返还给客户端
     （遵循标准语义）。
...
3. 开始将包含 seek 之后编码数据的缓冲区入队到 `OUTPUT` 队列，直到找到合适的
   恢复点（resume point）。
...
4. 找到恢复点后，解码器将开始返回包含已解码帧的 `CAPTURE` 缓冲区。
   ...
   seek 可能导致 `Dynamic Resolution Change` 序列被启动，原因是 seek 目标的
   解码参数与 seek 之前已解码的流部分不同。必须按解码器的正常操作来处理该序列。


   未规定 `CAPTURE` 队列何时开始产生包含 seek 之后入队的 `OUTPUT` 缓冲区
   解码数据的缓冲区，因为它与 `OUTPUT` 队列独立运行。

   解码器可能会返回若干剩余的 `CAPTURE` 缓冲区，其中包含由在 seek 序列执行前
   入队的 `OUTPUT` 缓冲区产生的已解码帧。

   `VIDIOC_STREAMOFF` 操作会丢弃所有剩余的已入队 `OUTPUT` 缓冲区，这意味着
   并非所有在 seek 序列之前入队的 `OUTPUT` 缓冲区都一定有对应的 `CAPTURE`
   缓冲区产生。例如，给定 `OUTPUT` 队列上的操作序列：

     QBUF(A), QBUF(B), STREAMOFF(), STREAMON(), QBUF(G), QBUF(H),

   在 `CAPTURE` 队列上出现以下任何结果都是允许的：

     {A', B', G', H'}, {A', G', H'}, {G', H'}.

   要确定 seek 之后包含第一个已解码帧的 CAPTURE 缓冲区，客户端可以观察时间戳以
   匹配 CAPTURE 和 OUTPUT 缓冲区，或者使用 V4L2_DEC_CMD_STOP 和
...
在流中包含分辨率元数据的数据流，可能在解码过程中需要切换到不同的分辨率。
   ...
   并非所有解码器都能检测分辨率变化。那些能够检测的解码器会在调用
   `VIDIOC_ENUM_FMT` 时，为编码格式设置 `V4L2_FMT_FLAG_DYN_RESOLUTION`
   标志。
...
1. 在流中遇到分辨率变化后，解码器发送 `changes` 设为
    `V4L2_EVENT_SRC_CH_RESOLUTION` 的 `V4L2_EVENT_SOURCE_CHANGE` 事件。
...
2. 继续 `Capture Setup` 序列。
   ...
   在分辨率变化序列期间，`OUTPUT` 队列必须保持流式传输。在 `OUTPUT` 队列上
   调用 `VIDIOC_STREAMOFF` 会中止该序列并启动一次 seek。

   原则上，`OUTPUT` 队列与 `CAPTURE` 队列独立运行，在整个分辨率变化序列期间
   也是如此。
...
为了确保所有已入队的 `OUTPUT` 缓冲区都已被处理，且相关的 `CAPTURE` 缓冲区
都已交给客户端，客户端必须遵循下述 drain 序列。drain 序列结束后，客户端已收到
在序列启动之前入队的所有 `OUTPUT` 缓冲区对应的所有已解码帧。

1. 通过发出 `VIDIOC_DECODER_CMD` 开始 drain。
...
2. 客户端在发出 `VIDIOC_DECODER_CMD` 之前入队的任何 `OUTPUT` 缓冲区，将
   像正常情况一样被处理和解码。客户端必须继续独立处理两个队列，类似于正常的
   解码操作。这包括：

   - 在处理这些缓冲区所触发的所有操作（例如 `Dynamic Resolution Change`
     序列）之后，再继续 drain 序列，

   - 入队和出队 `CAPTURE` 缓冲区，直到出一个带有 `V4L2_BUF_FLAG_LAST`
     标志的缓冲区，
...
3. 一旦在 `V4L2_DEC_CMD_STOP` 调用之前入队的所有 `OUTPUT` 缓冲区都已出队，
   且最后一个 `CAPTURE` 缓冲区也已出队，解码器即停止，它将接受但不会处理任何
   新入队的 `OUTPUT` 缓冲区，直到客户端发出以下任一操作：

   - `V4L2_DEC_CMD_START` - 解码器不会被重置，并将带着 drain 之前的所有状态
     正常恢复操作，

   - 在 `CAPTURE` 队列上的一对 `VIDIOC_STREAMOFF` 和 `VIDIOC_STREAMON` -
     解码器将正常恢复操作，但队列中任何仍存在的 `CAPTURE` 缓冲区将被返回给
     客户端，

   - 在 `OUTPUT` 队列上的一对 `VIDIOC_STREAMOFF` 和 `VIDIOC_STREAMON` - 任何
     待处理的源缓冲区将被返回给客户端，并且会触发 `Seek` 序列。


   一旦 drain 序列启动，客户端就需要按上述步骤将其驱动至完成，除非它通过在
   `OUTPUT` 或 `CAPTURE` 队列上发出 `VIDIOC_STREAMOFF` 中止该过程。在 drain
   序列进行期间，客户端不允许再次发出 `V4L2_DEC_CMD_START` 或
...
1. 在 `OUTPUT` 队列上设置格式，可能会改变 `CAPTURE` 队列上受支持/通告的
   格式集合。特别地，这也意味着 `CAPTURE` 格式可能会被重置，客户端不得依赖
   先前设置的格式被保留。

2. 在 `CAPTURE` 队列上枚举格式，总是只返回当前 `OUTPUT` 格式所支持的格式。

3. 在 `CAPTURE` 队列上设置格式，不会改变 `OUTPUT` 队列上可用格式列表。尝试
   设置一个对当前所选 `OUTPUT` 格式不支持的 `CAPTURE` 格式，会导致解码器将
   所请求的 `CAPTURE` 格式调整为受支持的格式。

4. 在 `OUTPUT` 队列上枚举格式，总是返回受支持编码格式的完整集合，与当前
   `CAPTURE` 格式无关。

5. 只要 `OUTPUT` 或 `CAPTURE` 队列上分配了缓冲区，客户端就不得更改 `OUTPUT`
   队列上的格式。对于任何此类格式更改尝试，驱动都会返回 -EBUSY 错误码。

总而言之，设置格式和分配必须始终从 `OUTPUT` 队列开始，并且 `OUTPUT` 队列是
