...
本页记录 V4L2（Video4Linux2）用户空间 API 的演进与变更历史，按时间顺序整理了自 1998 年 V4L2 取代 V4L 以来各版本在接口、ioctl 与数据格式上的关键改动，供驱动开发者与应用程序作者追溯 API 的演变。
...
在 V4L API 加入内核后不久，人们便批评它过于
缺乏灵活性。1998 年 8 月，Bill Dirks 提出了一系列改进建议，
并开始着手编写文档、示例驱动程序以及应用程序。
在其他志愿者的帮助下，这些工作最终演变成了 V4L2 API，
它不仅仅是 V4L API 的扩展，而是其替代品。然而又过了
四年以及两次稳定的内核发布，这个新 API 才最终以
目前的形式被内核接受并合入。
...
1998-09-18：`VIDIOC_NONCAP` ioctl 被一个本身
无意义的 `O_TRUNC` `open()` 标志所取代，同时定义了
别名 `O_NONCAP` 和 `O_NOIO`。如果应用程序仅打算访问
控制项（与需要独占访问的采集应用程序相对），则可以设置
该标志。`VIDEO_STD_XXX`
标识符现在是序数而非标志位，而
`video_std_construct()` 辅助函数接受 id 和
传输（transmission）参数。

1998-09-28: Revamped video standard. Made video controls individually
enumerable.

1998-09-28：重塑了视频标准。使视频控制项可被单独
枚举。
...
颜色副载波（color subcarrier）字段被重命名。VIDIOC_QUERYSTD ioctl 被
重命名为 VIDIOC_ENUMSTD，
VIDIOC_G_INPUT <VIDIOC_G_INPUT> 被重命名为
...
1998-11-12：某些 ioctl 的读/写方向定义有误。
...
`V4L2_PIX_FMT_RGB32` 改为 `V4L2_PIX_FMT_BGR32`。音频
控制项现在可通过
VIDIOC_G_CTRL <VIDIOC_G_CTRL> 和
VIDIOC_S_CTRL <VIDIOC_G_CTRL> ioctl 以
`V4L2_CID_AUDIO` 开头的名称访问。`V4L2_MAJOR` 宏定义
从 `videodev.h` 中移除，因为它仅在 `videodev` 内核
模块中被使用过一次。`YUV422` 和 `YUV411` 平面图像格式被加入。

1998-11-28: A few ioctl symbols changed. Interfaces for codecs and video
output devices were added.

1998-11-28：少数 ioctl 符号发生变化。新增了用于编解码器（codec）和视频
输出设备的接口。
...
1999-01-27：现在只有一个 QBUF ioctl，VIDIOC_QWBUF 和 VIDIOC_QRBUF
已废弃。VIDIOC_QBUF 以 v4l2_buffer 作为参数。新增了
数字缩放（裁剪）控制项。

## V4L2 Version 0.18 1999-03-16

## V4L2 0.18 版 1999-03-16
...
在 videodev.c 中新增了 v4l 到 V4L2 的 ioctl 兼容层。驱动
开发者请注意，这改变了 ioctl 处理函数的实现方式。请参阅
《驱动开发者指南》。新增了更多控制 id 代码。

## V4L2 Version 0.19 1999-06-05

## V4L2 0.19 版 1999-06-05
...
1999-03-18：在将 v4l2_queryctrl 对象传递给驱动之前，需要先
填好其 category 和 catname 字段。这需要对
...
1999-03-31：改善了对 v4l 内存采集 ioctl 的兼容性。需要
...
V4L2_CTRL_FLAG_GRAYED（后改名为 V4L2_CTRL_FLAG_GRABBED）标志。

1999-05-20: Definition of VIDIOC_G_CTRL was wrong causing a
malfunction of this ioctl.

1999-05-20：VIDIOC_G_CTRL 的定义有误，导致该 ioctl 工作异常。
...
0.20 版引入了许多与 0.19 及更早版本*不向后兼容*的改动。这些改动的
目的是简化 API，同时使其更具可扩展性，并遵循
通用的 Linux 驱动 API 惯例。

1. Some typos in `V4L2_FMT_FLAG` symbols were fixed. struct v4l2_clip
   was changed for compatibility with v4l. (1999-08-30)

1. 修正了 `V4L2_FMT_FLAG` 符号中的一些拼写错误。为兼容 v4l，
   修改了 struct v4l2_clip。（1999-08-30）

2. `V4L2_TUNER_SUB_LANG1` was added. (1999-09-05)

2. 新增了 `V4L2_TUNER_SUB_LANG1`。（1999-09-05）
...
3. 所有使用整数参数的 ioctl() 命令现在都改为接受一个指向整数的
   指针。在合理的情况下，ioctl 会通过参数所指向的整数返回
   实际的新值，这是 V4L2 API 中的通用惯例。受影响的 ioctl 包括：
   VIDIOC_PREVIEW、VIDIOC_STREAMON、VIDIOC_STREAMOFF、VIDIOC_S_FREQ、
   VIDIOC_S_INPUT、VIDIOC_S_OUTPUT、VIDIOC_S_EFFECT。例如
...
4. 所有不同的获取/设置格式命令被合并为单一的
   VIDIOC_G_FMT <VIDIOC_G_FMT> 和
   VIDIOC_S_FMT <VIDIOC_G_FMT> ioctl，其参数为一个联合（union）
   以及一个用于选择联合成员的 type 字段。其目的是通过消除若干
   ioctl 来简化 API，并在不新增 ioctl 的前提下支持新的以及
   驱动私有的数据流。
   ...
   这一改动使以下 ioctl 过时：`VIDIOC_S_INFMT`、
   `VIDIOC_G_INFMT`、`VIDIOC_S_OUTFMT`、`VIDIOC_G_OUTFMT`、
   `VIDIOC_S_VBIFMT` 和 `VIDIOC_G_VBIFMT`。图像格式
   struct v4l2_format 被重命名为 struct v4l2_pix_format，而
...
5. 与上面的改动类似，`VIDIOC_G_PARM` 和
   `VIDIOC_S_PARM` ioctl 与 `VIDIOC_G_OUTPARM` 和
   ...
   这一改动使 `VIDIOC_G_OUTPARM` 和 `VIDIOC_S_OUTPARM` ioctl 过时。
...
6. 简化了控制枚举，引入了两个新的控制标志并去掉了一个。`catname` 字段
   ...
   驱动现在可以使用 `V4L2_CTRL_FLAG_DISABLED` 和
   `V4L2_CTRL_FLAG_GRABBED` 分别标记不受支持和临时不可用的
   控制项。`group` 名称表示可能比 `category` 更细的
   分类。换句话说，一个 category 中可能有多个 group。同一 group 内的
   控制项通常会被绘制在一个分组框（group box）中。不同 category 中的
...
7. struct v4l2_buffer 的 `timestamp` 被
   改为 64 位整数，以纳秒为单位保存帧的采样或输出时间。此外，
   时间戳将采用绝对系统时间，而不是从流开始时的零算起。时间戳的
   数据类型名为 stamp_t，定义为有符号 64 位整数。输出设备在该
   timestamp 字段所表示的时间到来之前不应发出缓冲区。我希望
   效仿 SGI 的做法，采用类似其 UST（Unadjusted System Time，
   未校正系统时间）的多媒体时间戳系统。参见
   http://web.archive.org/web/\*/http://reality.sgi.com
   /cpirazzi_engr/lg/time/intro.html。UST 使用 64 位有符号整数
   （而非 struct timeval）作为时间戳，单位为纳秒。UST 时钟在
   ...
   v4l2_masterclock_gettime() 函数（仅驱动使用）改为
   返回一个 64 位整数。
...
8. 在 struct v4l2_buffer 中新增了 `sequence` 字段。`sequence`
...
2000-07-31：为了与原始的 `videodev.h` 文件兼容，`videodev.h`
现在包含了 `linux/poll.h` 头文件。

2000-11-20: `V4L2_TYPE_VBI_OUTPUT` and `V4L2_PIX_FMT_Y41P` were
added.

2000-11-20：新增了 `V4L2_TYPE_VBI_OUTPUT` 和 `V4L2_PIX_FMT_Y41P`。
...
2001-01-18：为避免命名空间冲突，`videodev.h` 头文件中定义的
...
2001-01-25：修复了 Linux 2.4.0 中的 `videodev.h` 文件与
`videodevX` 补丁中所包含的 `videodev.h` 文件之间可能存在的
驱动级兼容性问题。在 Linux 2.4.0 上使用较早版本
`videodevX` 的用户应重新编译其 V4L 和 V4L2
驱动。
...
与打了 devfs 补丁的 Linux 2.2.x 中的 `videodev.h` 文件之间
可能存在的內核级不兼容问题。
...
2001-03-02：某些 V4L ioctl 以只读参数定义，却会双向传递数据，
它们通过向后兼容层时无法正确工作。[解决方案？]

2001-04-13: Big endian 16-bit RGB formats were added.

2001-04-13：新增了大端（big endian）16 位 RGB 格式。
...
VIDIOC_G_FREQUENCY <VIDIOC_G_FREQUENCY> 和
VIDIOC_S_FREQUENCY <VIDIOC_G_FREQUENCY> ioctls。
（旧的 `VIDIOC_G_FREQ` 和 `VIDIOC_S_FREQ` ioctl 没有考虑
多个调谐器的情况。）
...
2000-09-18：新增了 `V4L2_BUF_TYPE_VBI`。这可能会*破坏
兼容性*，因为如果 struct `v4l2_fmt` 的 `type` 字段不包含
`V4L2_BUF_TYPE_VBI`，VIDIOC_G_FMT <VIDIOC_G_FMT> 和
VIDIOC_S_FMT <VIDIOC_G_FMT> ioctls 现在可能会失败。在
struct v4l2_vbi_format 的文档中，`offset` 字段处含糊的
短语 "rising edge" 被改为 "leading edge"。

## V4L2 Version 0.20 2000-11-23

## V4L2 0.20 版 2000-11-23
...
1. 在 V4L2 API 规范中新增了用于阐明行编号方案的图示。`start`\ [^0^]
   和 `start`\ [^1^] 字段不再从零开始计数行号。理由：a) 之前的定义
   不清。b) `start`\ [] 的值是序数。c) 没有必要发明新的行
   编号方案。现在我们采用 ITU-R 定义的行号，仅此而已。
   兼容性：需将 start 值加一。依赖先前语义的应用程序可能无法
   正常工作。
...
2. 限制 "count[^0^] > 0 且 count[^1^] > 0" 已放宽至
   "(count[^0^] + count[^1^]) > 0"。理由：驱动可能以扫描行为粒度
   分配资源，而某些数据服务仅在第一个场（field）上传输。关于两个
   `count` 值通常相等的注释具有误导性且无意义，已被移除。这一改动
   **破坏了与早期版本的兼容性**：驱动可能返回 `EINVAL`，
   应用程序可能无法正常工作。
...
3. 驱动再次被允许返回负的（未知的）start 值，正如早先所建议的。
   不清楚当初为何去掉了这一特性。这一改动可能**破坏与依赖
   正的 start 值的应用程序的兼容性**。澄清了 `EBUSY` 和 `EINVAL`
   错误码与 VIDIOC_S_FMT <VIDIOC_G_FMT> ioctl 的使用方式。`EBUSY`
   错误码终于得到了文档说明，而此前仅在 `videodev.h` 头文件中
   提及的 `reserved2` 字段也得到了说明。
...
4. 新增了新的缓冲区类型 `V4L2_TYPE_VBI_INPUT` 和
   `V4L2_TYPE_VBI_OUTPUT`。前者是旧的 `V4L2_TYPE_VBI` 的别名，
   后者则是 `videodev.h` 文件中缺失的。

## V4L2 Version 0.20 2002-07-25

## V4L2 0.20 版 2002-07-25
...
在 2002 年 10 月至 11 月前后，在 Linux 2.5 宣布特性冻结之前，
该 API 借鉴 V4L2 0.20 的经验进行了修订。这个未命名的版本最终
被合并进了 Linux 2.5.46。

1. As specified in related, drivers must make related device
    functions available under all minor device numbers.

1. 如相关章节所规定，驱动必须在所有次设备号下提供相关的设备
   功能。
...
2. 无论设备类型如何，`open()` 函数都要求使用
    `O_RDWR` 访问模式。所有与应用程序交换数据的 V4L2 驱动必须
    支持 `O_NONBLOCK` 标志。曾经作为 V4L2 符号、将无意义的
    `O_TRUNC` 别名为表示无数据交换访问（面板应用程序）的
    `O_NOIO` 标志被去掉了。驱动必须保持在"面板模式"
    直到应用程序尝试发起数据交换，详见 open。
...
3. struct v4l2_capability 发生了
    ...
    新增了用于标识驱动程序的字段、新的 RDS 设备功能
    `V4L2_CAP_RDS_CAPTURE`；`V4L2_CAP_AUDIO` 标志
    指示设备是否具有音频连接器；另一个 I/O 能力 V4L2_CAP_ASYNCIO 也
    可以被标记。作为对这些改动的响应，`type` 字段变成了一个位集合，
    并被合并进 `flags` 字段。`V4L2_FLAG_TUNER` 被重命名为
    `V4L2_CAP_TUNER`，`V4L2_CAP_VIDEO_OVERLAY` 取代了
    `V4L2_FLAG_PREVIEW`，`V4L2_CAP_VBI_CAPTURE` 和
    `V4L2_CAP_VBI_OUTPUT` 取代了 `V4L2_FLAG_DATA_SERVICE`。
    `V4L2_FLAG_READ` 和 `V4L2_FLAG_WRITE` 被合并为
    `V4L2_CAP_READWRITE`。
    ...
    冗余的 `inputs`、`outputs` 和 `audios` 字段被
    移除。这些属性可按 video 和 audio 章节所述的方式确定。
    ...
    那些不太稳定、因而几乎无用的字段
    `maxwidth`、`maxheight`、`minwidth`、`minheight`、
    `maxframerate` 被移除。这些信息可按 format 和 standard 章节
    所述的方式获取。
    ...
    `V4L2_FLAG_SELECT` 被移除。我们认为 select() 函数
    非常重要，要求所有与应用程序交换数据的 V4L2 驱动都支持它。
    冗余的 `V4L2_FLAG_MONOCHROME` 标志被移除，该信息可按
    format 章节所述的方式获取。
...
4. 在 struct v4l2_input 中，`assoc_audio`
    字段以及 `capability` 字段及其唯一的标志
    `V4L2_INPUT_CAP_AUDIO` 被新的 `audioset` 字段取代。
    ...
    `capability` 和 `assoc_audio` 字段。取而代之新增了
    `audioset`、`modulator` 和 `std`。
...
5. 为与其他结构保持一致，struct v4l2_audio 的 `audio` 字段被
    重命名为 `index`。新增了一个能力标志 `V4L2_AUDCAP_STEREO`
    以指示相关音频输入是否支持立体声。`V4L2_AUDCAP_EFFECTS` 及
    相应的 `V4L2_AUDMODE` 标志被移除。这可轻易地使用控制项
    实现。（不过 AVL 的情况类似，但它仍然存在。）

    Again for consistency the struct v4l2_audioout field `audio` was renamed
    to `index`.

    同样为了保持一致，struct v4l2_audioout 的 `audio` 字段被重命名为
    `index`。
...
6. struct v4l2_tuner 的 `input` 字段被
    一个 `index` 字段取代，从而支持具有多个调谐器的设备。视频输入
    与调谐器之间的关联现在被反转，输入指向其调谐器。`std` 子结构
    变为一个简单的集合（详见下文）并移入 struct v4l2_input。
    新增了一个 `type` 字段。

    Accordingly in struct v4l2_modulator the
    `output` was replaced by an `index` field.

    相应地，在 struct v4l2_modulator 中，`output` 被一个
    `index` 字段取代。
    ...
    在 struct v4l2_frequency 中，`port`
    字段被一个包含相应调谐器或调制器索引号的 `tuner` 字段取代。
    新增了调谐器 `type` 字段，并且 `reserved` 字段被扩大以
    便将来扩展（尤其是卫星调谐器）。
...
7. 完全透明的视频标准这一设想被放弃了。经验表明，应用程序必须
    能够超越"向用户展示菜单"来与视频标准打交道。现在，应用程序不再
    用 ioctl 枚举所支持的标准，而是可以通过 `videodev2.h` 头文件中
    定义的 v4l2_std_id <v4l2-std-id> 和符号来引用标准。详情
    见 standard。VIDIOC_G_STD <VIDIOC_G_STD> 和
    VIDIOC_S_STD <VIDIOC_G_STD> 现在以指向该类型的指针作为参数。
    新增了 VIDIOC_QUERYSTD，用于在硬件支持的情况下
    自动检测所接收的标准。在 struct v4l2_standard 中新增了
    `index` 字段用于
    VIDIOC_ENUMSTD。新增了一个名为 `id` 的
    v4l2_std_id <v4l2-std-id> 字段作为机器可读标识符，同时
    取代了 `transmission` 字段。具有误导性的 `framerate` 字段
    被重命名为 `frameperiod`。现已过时的 `colorstandard` 信息
    （最初用于区分标准的不同变体）被移除。
    ...
    以指向 struct v4l2_standard 的指针作为参数。某特定视频
    输入或输出所支持的标准这一信息分别移入了名为 `std` 的
    struct v4l2_input 和 struct v4l2_output 字段。
...
8. struct v4l2_queryctrl <v4l2-queryctrl> 的
    `category` 和 `group` 字段未被广泛采用和/或未如预期
    实现，因此被移除。
...
9. 新增了 VIDIOC_TRY_FMT <VIDIOC_G_FMT> ioctl，用于
    像 VIDIOC_S_FMT <VIDIOC_G_FMT> 那样协商数据格式，但不
    ...
    在 struct v4l2_format 中，`fmt` 联合被
    扩展为包含 struct v4l2_window。现在所有图像格式协商都可以通过
    `VIDIOC_G_FMT`、`VIDIOC_S_FMT` 和 `VIDIOC_TRY_FMT` ioctl
    完成。`VIDIOC_G_WIN` 和 `VIDIOC_S_WIN` 这两个用于准备视频
...
10. 在 struct v4l2_fmtdesc 中，新增了一个名为 `type` 的
    enum v4l2_buf_type 字段，与 struct v4l2_format 中相同。
    `VIDIOC_ENUM_FBUFFMT` ioctl 不再需要，已被移除。这些调用可被
    使用 `V4L2_BUF_TYPE_VIDEO_OVERLAY` 类型的 VIDIOC_ENUM_FMT 取代。
...
11. 在 struct v4l2_pix_format 中，`depth`
    字段被移除，因为假定那些通过四字符码（four-character-code）识别
    格式的应用程序已经知道颜色深度，而其他应用程序并不关心它。同样的
    理由导致了 `V4L2_FMT_FLAG_COMPRESSED` 标志的移除。
    `V4L2_FMT_FLAG_SWCONVECOMPRESSED` 标志被移除，因为驱动不应在
    内核空间转换图像。应改为提供一个用户态的转换函数库。
    `V4L2_FMT_FLAG_BYTESPERLINE` 标志是冗余的。应用程序可以将
    `bytesperline` 字段设为零以获得合理的默认值。由于其余标志也
    被替换了，因此 `flags` 字段本身也被移除。
...
```
	...
	* - ``-``
    ...
    enum v4l2_colorspace 值所取代，其中
    ``V4L2_COLORSPACE_SMPTE170M``、``V4L2_COLORSPACE_BT878``、
    ``V4L2_COLORSPACE_470_SYSTEM_M`` 或
    ``V4L2_COLORSPACE_470_SYSTEM_BG`` 之一取代了
    ``V4L2_FMT_CS_601YUV``。
...
12. 在 struct v4l2_requestbuffers 中，`type` 字段被正确定义为
    enum v4l2_buf_type。缓冲区类型如前所述发生了变化。新增了一个
    enum v4l2_memory 类型的 `memory` 字段，用于区分
    使用驱动分配还是应用程序分配的缓冲区的 I/O 方法。详见 io。
...
13. 在 struct v4l2_buffer 中，`type` 字段被正确定义为
    enum v4l2_buf_type。缓冲区类型如前所述发生了变化。新增了一个
    enum v4l2_field 类型的 `field` 字段，用于指示缓冲区包含的是
    顶场（top）还是底场（bottom）。旧的场标志被移除。由于内核中
    并未按原计划加入未校正系统时间时钟，`timestamp` 字段从
    表示采样时间（纳秒）的无符号 64 位整数 stamp_t 类型改回了
    struct timeval。随着第二种内存映射方法的加入，`offset`
    字段移入了联合 `m`，并新增了一个 enum v4l2_memory 类型的
    `memory` 字段用于区分不同的 I/O 方法。详见 io。
    ...
    `V4L2_BUF_REQ_CONTIG` 标志曾由 V4L 兼容层使用，在对该代码
    进行改动后不再需要。而 `V4L2_BUF_ATTR_DEVICEMEM` 标志用于指示
    缓冲区是否确实分配在设备内存而非可 DMA 的系统内存中。它几乎
    没有用处，因此被移除。
...
14. 在 struct v4l2_framebuffer 中，那个预期用于离屏视频内存中
    双缓冲和三缓冲、却未定义同步机制的 `base[^3^]` 数组被单个
    指针取代。`V4L2_FBUF_CAP_SCALEUP` 和 `V4L2_FBUF_CAP_SCALEDOWN`
    标志被移除。应用程序可通过新的裁剪和缩放接口更准确地确定这一
    能力。`V4L2_FBUF_CAP_CLIPPING` 标志被
    `V4L2_FBUF_CAP_LIST_CLIPPING` 和
...
15. 在 struct v4l2_clip 中，`x`、`y`、
    `width` 和 `height` 字段移入了 struct v4l2_rect 类型的
    `c` 子结构。其中 `x` 和 `y`
    字段被重命名为 `left` 和 `top`，即相对于
    上下文相关原点的偏移量。
...
16. 在 struct v4l2_window 中，`x`、`y`、
    `width` 和 `height` 字段如上所述移入了 `w` 子结构。新增了
    一个 enum v4l2_field 类型的 `field` 字段，用于区分
...
17. 数字缩放接口（包括 struct `v4l2_zoomcap`、struct `v4l2_zoom`、
    `V4L2_ZOOM_NONCAP` 和 `V4L2_ZOOM_WHILESTREAMING`）被新的
    裁剪和缩放接口取代。此前未使用的 struct v4l2_cropcap 和
    struct v4l2_crop 为此被重新定义。详见 crop。
...
18. 在 struct v4l2_vbi_format 中，`SAMPLE_FORMAT` 字段现在包含
    一个用于标识视频图像格式的四字符码，且 `V4L2_PIX_FMT_GREY`
    取代了 `V4L2_VBI_SF_UBYTE` 定义。`reserved` 字段被扩展。
...
21. 旧版 V4L2 文档中从 RGB 到 YCbCr 颜色空间的示例变换是不准确的，
    已在 pixfmt 中予以纠正。
...
1. 为无线电（radio）设备新增了能力标志 `V4L2_CAP_RADIO`。在此改动
   之前，无线电设备仅凭其拥有恰好一个 type 字段为 `V4L2_TUNER_RADIO`
   的调谐器来标识。
...
3. 人们发现音频输入和输出接口并不完整。
   ...
   视频输入组合时）并不存在。因此 `VIDIOC_G_AUDIO` 被重命名为
   `VIDIOC_G_AUDIO_OLD`，该 ioctl 在内核 2.6.39 中被移除。新增了
   VIDIOC_ENUMAUDIO ioctl 用于枚举音频输入，
   而 VIDIOC_G_AUDIO <VIDIOC_G_AUDIO> 现在报告当前
   音频输入。
   ...
   对 VIDIOC_G_AUDOUT <VIDIOC_G_AUDOUT> 和
   ...
   在此过渡期间，"videodev" 模块会自动在旧 ioctl 和新 ioctl 之间
...
4. VIDIOC_OVERLAY ioctl 曾被错误地定义为读-写参数。它被改为
   只写（write-only），而读-写版本被重命名为 `VIDIOC_OVERLAY_OLD`。
   旧的 ioctl 在内核 2.6.39 中被移除。在此过渡期间，"videodev"
   内核模块会自动转换为新版，因此驱动必须重新编译，但应用程序
   无需重新编译。
...
6. VIDIOC_S_PARM <VIDIOC_G_PARM> 和
   VIDIOC_S_CTRL <VIDIOC_G_CTRL> ioctls 被定义为只写参数，这与其他
   修改其参数的 ioctl 不一致。它们被改为读-写，而只写版本加上了
   `_OLD` 后缀。旧 ioctl 在内核 2.6.39 中被移除。假定参数为
...
1. 在 pixfmt-rgb 中，以下像素格式错误地照搬自 Bill Dirks 的 V4L2 规范。
   下面的描述指的是内存中的字节，按地址升序排列。


   ``V4L2_PIX_FMT_BGR24`` 的示例始终是正确的。

   在 :ref:`v4l-image-properties` 中，V4L 的
   ``VIDEO_PALETTE_RGB24`` 和 ``VIDEO_PALETTE_RGB32`` 格式到 V4L2
   像素格式的映射也做了相应的纠正。
...
2. 与上述修正无关，驱动可能仍然以不同方式解释某些 V4L2 RGB 像素格式。
   这些问题尚待解决，详见 pixfmt-rgb。
...
1. VIDIOC_CROPCAP ioctl 曾被错误地定义为只读参数。现在它被定义为
   读-写 ioctl，而只读版本被重命名为 `VIDIOC_CROPCAP_OLD`。旧的
   ioctl 在内核 2.6.39 中被移除。
...
1. 在 struct v4l2_buffer 中新增了一个字段 `input`（原
   `reserved[^0^]`）。该字段的目的是在视频采集过程中
   与采集流程同步地切换不同的视频输入（例如摄像头）。这一功能必须
   通过新的 `V4L2_BUF_FLAG_INPUT` 标志启用。`flags` 字段不再是
   只读的。
...
2. 音频输出 ioctl 以 -AUDOUT 结尾，而非 -AUDIOOUT。

3. In the Current Audio Input example the `VIDIOC_G_AUDIO` ioctl took
   the wrong argument.

3. 在"当前音频输入"示例中，`VIDIOC_G_AUDIO` ioctl 使用了错误的
   参数。
...
4. VIDIOC_QBUF 和 VIDIOC_DQBUF <VIDIOC_QBUF> ioctls 的文档
   没有提及 struct v4l2_buffer 的 `memory` 字段。示例中
   也缺失了它。此外，在 `VIDIOC_DQBUF` 页面上 `EIO` 错误码也
   未被文档说明。
...
1. 新增了一个新的分片（sliced）VBI 接口。它在 sliced 中有文档，
   取代了 V4L2 规范 0.8 中最初提出的接口。
...
2. 定义了新的视频标准 `V4L2_STD_NTSC_443`、`V4L2_STD_SECAM_LC`、
   `V4L2_STD_SECAM_DK`（一组 SECAM D、K 和 K1）以及
   `V4L2_STD_ATSC`（一组 `V4L2_STD_ATSC_8_VSB` 和
   `V4L2_STD_ATSC_16_VSB`）。注意 `V4L2_STD_525_60` 集合现在
   包含了 `V4L2_STD_NTSC_443`。另见 v4l2-std-id。
...
3. `VIDIOC_G_COMP` 和 `VIDIOC_S_COMP` ioctl 分别被重命名为
   `VIDIOC_G_MPEGCOMP` 和 `VIDIOC_S_MPEGCOMP`。它们的参数被替换为
   struct `v4l2_mpeg_compression` 指针。（`VIDIOC_G_MPEGCOMP` 和
   `VIDIOC_S_MPEGCOMP` ioctls 在 Linux 2.6.25 中被移除。）
...
capture-example 中的采集示例调用了 VIDIOC_S_CROP <VIDIOC_G_CROP>
...
2. VIDIOC_S_PARM <VIDIOC_G_PARM> 是一个读-写 ioctl，而不是其
   参考页上所说的只写。该 ioctl 在 2003 年已如前所述发生了改变。
...
1. 在 struct v4l2_captureparm 和 struct v4l2_outputparm 中，
...
1. struct v4l2_window 中的 `clips` 字段必须指向
   struct v4l2_clip 数组，而不是链表，因为驱动会忽略
   struct v4l2_clip 的 `next` 指针。
...
1. 新增了视频标准宏：`V4L2_STD_NTSC_M_KR`（NTSC M 韩国版），
   以及集合 `V4L2_STD_MN`、`V4L2_STD_B`、`V4L2_STD_GH` 和
   `V4L2_STD_DK`。`V4L2_STD_NTSC` 和 `V4L2_STD_SECAM` 集合现在
   分别包含了 `V4L2_STD_NTSC_M_KR` 和 `V4L2_STD_SECAM_LC`。
...
1. 在许多地方，分片 VBI 接口的 `V4L2_BUF_TYPE_SLICED_VBI_CAPTURE` 和
...
2. 在 VIDIOC_G_AUDIO <VIDIOC_G_AUDIO> 中澄清了 struct v4l2_audio 的
   `mode` 字段是一个标志位字段。
...
4. 在 VIDIOC_G_FREQUENCY <VIDIOC_G_FREQUENCY> 中澄清了应用程序
   必须在调用 VIDIOC_S_FREQUENCY <VIDIOC_G_FREQUENCY> 之前初始化
   struct v4l2_frequency 的调谐器 `type` 字段。

5. The `reserved` array in struct v4l2_requestbuffers has 2 elements,
   not 32.

5. struct v4l2_requestbuffers 中的 `reserved` 数组有 2 个元素，
   而非 32 个。

6. In output and raw-vbi the device file names
   `/dev/vout` which never caught on were replaced by `/dev/video`.

6. 在 output 和 raw-vbi 中，从未流行的设备文件名
   `/dev/vout` 被 `/dev/video` 取代。
...
7. 从 Linux 2.6.15 起，VBI 设备次设备号的可能范围从 224-239 扩展到
   224-255。相应地，现在可以使用 `/dev/vbi0` 到 `/dev/vbi31` 这样的
   设备文件名。
...
1. 新增了 VIDIOC_G_EXT_CTRLS <VIDIOC_G_EXT_CTRLS>、
   VIDIOC_S_EXT_CTRLS <VIDIOC_G_EXT_CTRLS> 和
   VIDIOC_TRY_EXT_CTRLS <VIDIOC_G_EXT_CTRLS> ioctls，一个配合
   VIDIOC_QUERYCTRL 跳过不受支持控制项的标志，新的控制类型
   `V4L2_CTRL_TYPE_INTEGER64` 和 `V4L2_CTRL_TYPE_CTRL_CLASS`
   （enum v4l2_ctrl_type），以及新的控制标志
   `V4L2_CTRL_FLAG_READ_ONLY`、`V4L2_CTRL_FLAG_UPDATE`、
   `V4L2_CTRL_FLAG_INACTIVE` 和 `V4L2_CTRL_FLAG_SLIDER`
   （control-flags）。详见 extended-controls。
...
1. 在 struct v4l2_sliced_vbi_cap 中，新增了一个缓冲区类型字段以
   取代一个保留字段。注意在枚举类型大小与 int 类型不同的架构上，
   该结构的大小会发生改变。VIDIOC_G_SLICED_VBI_CAP
   <VIDIOC_G_SLICED_VBI_CAP> ioctl 从只读被重新定义为读-写。
   应用程序现在必须初始化 type 字段并清除保留字段。这些变化可能
   **破坏与较旧驱动和应用程序的兼容性**。
...
1. `videodev2.h` 头文件现在采用双重许可：GNU 通用公共许可证（版本二
   或更高）以及 3 条款的 BSD 风格许可证。
...
1. 新增了两个新的场序 `V4L2_FIELD_INTERLACED_TB` 和
   `V4L2_FIELD_INTERLACED_BT`。详见 enum v4l2_field。
...
2. 视频叠加（overlay）接口新增了三种裁剪/混合（clipping/blending）
   方法，分别带有全局、或正向、或取反的局部 alpha 值。详见
   VIDIOC_G_FBUF <VIDIOC_G_FBUF> 和 VIDIOC_S_FBUF
   ...
   在 struct v4l2_window 中新增了 `global_alpha` 字段，扩展了该
   结构。这可能**破坏与直接使用 struct v4l2_window 的应用程序的
   兼容性**。不过，VIDIOC_G/S/TRY_FMT <VIDIOC_G_FMT> ioctls 接受
   一个指向末尾带填充字节的 struct v4l2_format 父结构的指针，
   不会受到影响。
...
3. struct v4l2_window 中 `chromakey` 字段的格式从
   "host order RGB32" 改为与帧缓冲（framebuffer）相同的像素值格式。
   这可能**破坏与现有应用程序的兼容性**。目前不知道有驱动支持
   "host order RGB32" 格式。
...
1. 新增了像素格式 `V4L2_PIX_FMT_PAL8`、`V4L2_PIX_FMT_YUV444`、
   `V4L2_PIX_FMT_YUV555`、`V4L2_PIX_FMT_YUV565` 和
   `V4L2_PIX_FMT_YUV32`。
...
1. 新增了像素格式 V4L2_PIX_FMT_Y16 <V4L2-PIX-FMT-Y16> 和
   V4L2_PIX_FMT_SBGGR16 <V4L2-PIX-FMT-SBGGR16>。
...
2. 新增了控制项 <control> `V4L2_CID_POWER_LINE_FREQUENCY`、
   `V4L2_CID_HUE_AUTO`、`V4L2_CID_WHITE_BALANCE_TEMPERATURE`、
   `V4L2_CID_SHARPNESS` 和 `V4L2_CID_BACKLIGHT_COMPENSATION`。
   控制项 `V4L2_CID_BLACK_LEVEL`、`V4L2_CID_WHITENESS`、
   `V4L2_CID_HCENTER` 和 `V4L2_CID_VCENTER` 被弃用。
...
4. 在 Linux 2.6.18 中已被扩展控制 <extended-controls> 接口取代的
   `VIDIOC_G_MPEGCOMP` 和 `VIDIOC_S_MPEGCOMP` ioctls，最终从
   `videodev2.h` 头文件中移除。
...
1. 新增了像素格式 `V4L2_PIX_FMT_Y16` 和 `V4L2_PIX_FMT_SBGGR16`。

2. Added user controls `V4L2_CID_CHROMA_AGC` and
   `V4L2_CID_COLOR_KILLER`.

2. 新增了用户控制项 `V4L2_CID_CHROMA_AGC` 和 `V4L2_CID_COLOR_KILLER`。
...
1. 新增了 VIDIOC_S_HW_FREQ_SEEK ioctl 和
...
2. 新增了像素格式 `V4L2_PIX_FMT_YVYU`、`V4L2_PIX_FMT_PCA501`、
   `V4L2_PIX_FMT_PCA505`、`V4L2_PIX_FMT_PCA508`、
   `V4L2_PIX_FMT_PCA561`、`V4L2_PIX_FMT_SGBRG8`、
   `V4L2_PIX_FMT_PAC207` 和 `V4L2_PIX_FMT_PJPG`。
...
3. 新增了相机控制项 `V4L2_CID_ZOOM_ABSOLUTE`、
   `V4L2_CID_ZOOM_RELATIVE`、`V4L2_CID_ZOOM_CONTINUOUS` 和
   `V4L2_CID_PRIVACY`。
...
1. 为了便于将 V4L2 API 与内核版本进行比较，现在 V4L2 API 采用
   Linux 内核版本号进行编号。

2. Finalized the RDS capture API. See rds for more information.

2. 完善了 RDS 采集 API。更多信息见 rds。
...
1. 新增了对数字视频（Digital Video）时序的支持，以便支持 HDTV
   接收器和发送器。
...
1. 向相机控制类 <camera-controls> 中新增了
   `V4L2_CID_IRIS_ABSOLUTE` 和 `V4L2_CID_IRIS_RELATIVE` 控制项。
...
1. 移除了 vtx（videotext/teletext）API。该 API 已不再被使用，也没有
...
1. 移除了旧的 VIDIOC_*_OLD 符号以及 V4L1 支持。
...
1. 新增了 V4L2_CTRL_FLAG_VOLATILE，用于向用户态标示易变（volatile）
   的控制项。
...
2. 新增了选择（selection）API，用于扩展对裁剪（cropping）和合成
   （composing）的控制。不影响当前驱动和应用程序的兼容性。详见
...
2. 在 struct v4l2_capabilities 中新增了 device_caps 字段，并新增了
...
1. 新增了 JPEG 压缩控制类 <jpeg-controls>。
...
2. 扩展了 DV Timings API：新增了 VIDIOC_ENUM_DV_TIMINGS、
   VIDIOC_QUERY_DV_TIMINGS 和 VIDIOC_DV_TIMINGS_CAP。
...
2. 为 V4L2 子设备（subdev）接口新增了选择 API：
   VIDIOC_SUBDEV_G_SELECTION 和
   VIDIOC_SUBDEV_S_SELECTION <VIDIOC_SUBDEV_G_SELECTION>。
...
5. 新增了相机控制项 `V4L2_CID_AUTO_EXPOSURE_BIAS`、
   ...
   `V4L2_CID_AUTO_FOCUS_STOP`、`V4L2_CID_AUTO_FOCUS_STATUS` 和
...
1. 将 struct v4l2_buffer 中的 `input` 替换为 `reserved2`，并
   移除了 `V4L2_BUF_FLAG_INPUT`。

2. Added V4L2_CAP_VIDEO_M2M and V4L2_CAP_VIDEO_M2M_MPLANE
   capabilities.

2. 新增了 V4L2_CAP_VIDEO_M2M 和 V4L2_CAP_VIDEO_M2M_MPLANE 能力。
...
1. 在 struct v4l2_buffer 的 `flags` 字段中新增了时间戳类型。
   详见 buffer-flags。

2. Added `V4L2_EVENT_CTRL_CH_RANGE` control event changes flag. See
   ctrl-changes-flags.

2. 新增了 `V4L2_EVENT_CTRL_CH_RANGE` 控制事件变更标志。详见
   ctrl-changes-flags。
...
1. 移除了过时且未使用的 DV_PRESET ioctls：VIDIOC_G_DV_PRESET、
   VIDIOC_S_DV_PRESET、VIDIOC_QUERY_DV_PRESET 和
   VIDIOC_ENUM_DV_PRESET。移除了相关的 v4l2_input/output 能力标志
   V4L2_IN_CAP_PRESETS 和 V4L2_OUT_CAP_PRESETS。

2. Added new debugging ioctl
   VIDIOC_DBG_G_CHIP_INFO.

2. 新增了调试 ioctl VIDIOC_DBG_G_CHIP_INFO。
...
1. 在 struct v4l2_rect 中，`width` 和 `height` 字段的类型从
   _s32 改为 _u32。
...
1. 新增了事件 V4L2_EVENT_SOURCE_CHANGE。
...
1. 新增了相机控制项 `V4L2_CID_PAN_SPEED` 和 `V4L2_CID_TILT_SPEED`。
...
1. 重写了 Colorspace 章节，向 struct v4l2_pix_format、
   struct v4l2_pix_format_mplane 和 struct v4l2_mbus_framefmt 中
   新增了 enum v4l2_ycbcr_encoding 和 enum v4l2_quantization 字段。
...
2. 新增了 `V4L2_CID_RF_TUNER_RF_GAIN` RF 调谐器控制项。
...
X Video 扩展（简称 XVideo 或 Xv）是 X Window 系统的一个扩展，由
XFree86 项目等实现。它的作用范围与 V4L2 类似，都是面向 X 客户端的
视频采集和输出设备 API。Xv 允许应用程序在窗口中显示实时视频、将
窗口内容发送到电视输出，并在 XPixmaps [#f1]_ 中采集或输出静态图像。
...
由于驱动嵌入在 X 服务器中，Xv 相对于 V4L2 视频叠加（overlay）接口
<overlay> 具有若干优势。驱动可以轻松地确定叠加目标，即可见的
...
为了结合 Xv 与 V4L 的优势，XFree86 和 XOrg 中存在一个特殊的 Xv 驱动，
...
截至 XFree86 4.2，该驱动仍然只支持 V4L ioctls，不过通过 V4L2 的
向后兼容层，它应该能与所有 V4L2 设备正常工作。由于 V4L2 允许多次
打开，因此（在 V4L2 驱动支持的情况下）可以在 X 客户端请求视频叠加的
...
与 V4L2 仅有少许关联，XFree86 扩展了 Xv 以支持硬件 YUV 到 RGB 转换和
缩放，从而实现更快的视频播放，并增加了对 MPEG-2 解码硬件的接口。
该 API 可用于显示用 V4L2 设备采集的图像。
...
V4L2 不支持数字地面、有线或卫星广播。存在一个面向数字接收器的独立
项目。你可以在 `https://linuxtv.org <https://linuxtv.org>`__ 找到它的
主页。Linux DVB API 与 V4L2 API 没有关系，只是混合（hybrid）硬件的
驱动可能同时支持两者。
...
- `VIDIOC_G_MPEGCOMP` 和 `VIDIOC_S_MPEGCOMP` ioctls。请使用扩展
...
- VIDIOC_G_DV_PRESET、VIDIOC_S_DV_PRESET、
   VIDIOC_ENUM_DV_PRESETS 和 VIDIOC_QUERY_DV_PRESET ioctls。请使用
...
- `VIDIOC_SUBDEV_G_CROP` 和 `VIDIOC_SUBDEV_S_CROP` ioctls。请使用
   `VIDIOC_SUBDEV_G_SELECTION` 和 `VIDIOC_SUBDEV_S_SELECTION`，
   VIDIOC_SUBDEV_G_SELECTION。
