...
下面描述编解码器控制类中的所有控件。首先是通用控件，然后是特定于某些硬件的控件。
   ...
   这些控件并非仅适用于 MPEG，而是适用于所有编解码器。这些定义以
   V4L2_CID_MPEG/V4L2_MPEG 为前缀，因为控件最初是为 MPEG 编解码器创建的，
   后来被扩展以涵盖所有编码格式。
...
`V4L2_CID_CODEC_CLASS (class)`
    编解码器（Codec）类的描述符。对该控件调用 VIDIOC_QUERYCTRL 将返回
...
enum v4l2_mpeg_stream_type -
    MPEG-1、-2 或 -4 输出流类型。这里不能做任何假设。每种硬件 MPEG 编码器往往支持可用 MPEG 流类型的不同子集。该控件专用于多路复用的 MPEG 流。当前已定义的流类型如下：
...
`V4L2_CID_MPEG_STREAM_PID_PCR (integer)`
    承载 PCR 字段的 MPEG 传输流包 ID（默认 259）
...
enum v4l2_mpeg_stream_vbi_fmt -
    某些卡可以将 VBI 数据（例如字幕（Closed Caption）、图文电视（Teletext））嵌入到 MPEG 流中。该控件选择是否应嵌入 VBI 数据，以及如果嵌入，应采用何种嵌入方式。可能的 VBI 格式列表取决于驱动。当前已定义的 VBI 格式类型如下：
...
enum v4l2_mpeg_audio_encoding -
    ...
    - - `V4L2_MPEG_AUDIO_ENCODING_LAYER_1`
      - MPEG-1/2 第一层（Layer I）编码
    - - `V4L2_MPEG_AUDIO_ENCODING_LAYER_2`
      - MPEG-1/2 第二层（Layer II）编码
    - - `V4L2_MPEG_AUDIO_ENCODING_LAYER_3`
      - MPEG-1/2 第三层（Layer III）编码
...
`V4L2_CID_MPEG_AUDIO_MUTE (boolean)`
    采集时静音音频。这不是通过静音音频硬件来实现的（硬件静音仍可能产生轻微嘶嘶声），而是在编码器内部完成，从而保证固定的、可复现的音频码流。0 = 非静音，1 = 静音。
...
`V4L2_CID_MPEG_VIDEO_CONSTANT_QUALITY (integer)`
    恒定质量等级控制。当 `V4L2_CID_MPEG_VIDEO_BITRATE_MODE` 的值为 `V4L2_MPEG_VIDEO_BITRATE_MODE_CQ` 时适用此控件。有效范围为 1 到 100，其中 1 表示最低质量，100 表示最高质量。编码器将决定适当的量化参数和比特率，以产生所请求的帧质量。
...
enum v4l2_mpeg_video_frame_skip_mode -
    ...
    - - `V4L2_MPEG_VIDEO_FRAME_SKIP_MODE_BUF_LIMIT`
      - 帧跳过模式已启用，缓冲区限制由 VBV（MPEG1/2/4） <v4l2-mpeg-video-vbv-size> 或 CPB（H264）缓冲区大小 <v4l2-mpeg-video-h264-cpb-size> 控件设定。
...
`V4L2_CID_MPEG_VIDEO_MUTE (boolean)`
    采集时将视频“静音”为固定颜色。这对于测试以产生固定的视频码流很有用。0 = 非静音，1 = 静音。

`V4L2_CID_MPEG_VIDEO_MUTE_YUV (integer)`
    设置视频的“静音”颜色。所提供的 32 位整数按如下方式解释（bit 0 = 最低有效位）：
...
`V4L2_CID_MPEG_VIDEO_DEC_PTS (integer64)`
    这个只读控件返回当前显示帧的 33 位视频显示时间戳（Presentation Time Stamp），其定义见 ITU T-REC-H.222.0 和 ISO/IEC 13818-1。它与 VIDIOC_DECODER_CMD 中所用的 PTS 相同。
...
`V4L2_CID_MPEG_VIDEO_DEC_CONCEAL_COLOR (integer64)`
    此控件设置 YUV 色彩空间中的隐藏（conceal）颜色。它描述在参考帧缺失导致出错时，客户端对错误隐藏颜色的偏好。解码器应使用偏好颜色填充参考缓冲区，并将其用于后续解码。该控件每个通道使用 16 位。适用于解码器。
...
`V4L2_CID_MPEG_VIDEO_DEC_DISPLAY_DELAY_ENABLE (boolean)`
    如果启用了显示延迟，则解码器在处理一定数量的 OUTPUT 缓冲区后，被迫返回一个 CAPTURE 缓冲区（已解码帧）。该延迟可通过 `V4L2_CID_MPEG_VIDEO_DEC_DISPLAY_DELAY` 设置。例如，此特性可用于生成视频缩略图。适用于解码器。
...
`V4L2_CID_MPEG_VIDEO_AU_DELIMITER (boolean)`
    如果启用，将生成 AUD（访问单元定界符，Access Unit Delimiter）NALU。这在无需完全解析每个 NALU 即可找到帧起始时很有用。适用于 H264 和 HEVC 编码器。

`V4L2_CID_MPEG_VIDEO_H264_VUI_SAR_ENABLE (boolean)`
    启用在视频可用性信息（Video Usability Information）中写入采样宽高比。适用于 H264 编码器。
...
enum v4l2_mpeg_video_h264_vui_sar_idc -
    用于 H.264 编码的 VUI 采样宽高比指示符。该值在标准表 E-1 中定义。适用于 H264 编码器。
...
`V4L2_CID_MPEG_VIDEO_H264_VUI_EXT_SAR_WIDTH (integer)`
    用于 H.264 VUI 编码的扩展采样宽高比宽度。适用于 H264 编码器。

`V4L2_CID_MPEG_VIDEO_H264_VUI_EXT_SAR_HEIGHT (integer)`
    用于 H.264 VUI 编码的扩展采样宽高比高度。适用于 H264 编码器。
...
enum v4l2_mpeg_video_h264_level -
    H264 视频基本流的等级（level）信息。适用于 H264 编码器。可能的值如下：
...
enum v4l2_mpeg_video_mpeg2_level -
    MPEG2 基本流的等级信息。适用于 MPEG2 编解码器。可能的值如下：
...
enum v4l2_mpeg_video_mpeg4_level -
    MPEG4 基本流的等级信息。适用于 MPEG4 编码器。可能的值如下：
...
enum v4l2_mpeg_video_h264_profile -
    H264 的档次（profile）信息。适用于 H264 编码器。可能的值如下：
...
enum v4l2_mpeg_video_mpeg2_profile -
    MPEG2 的档次信息。适用于 MPEG2 编解码器。可能的值如下：
...
enum v4l2_mpeg_video_mpeg4_profile -
    MPEG4 的档次信息。适用于 MPEG4 编码器。可能的值如下：
...
`V4L2_CID_MPEG_VIDEO_MULTI_SLICE_MAX_MB (integer)`
    一个切片中的最大宏块数。当 `V4L2_CID_MPEG_VIDEO_MULTI_SLICE_MODE` 设置为 `V4L2_MPEG_VIDEO_MULTI_SLICE_MODE_MAX_MB` 时使用。适用于编码器。

`V4L2_CID_MPEG_VIDEO_MULTI_SLICE_MAX_BYTES (integer)`
    一个切片的最大字节数。当 `V4L2_CID_MPEG_VIDEO_MULTI_SLICE_MODE` 设置为 `V4L2_MPEG_VIDEO_MULTI_SLICE_MODE_MAX_BYTES` 时使用。适用于编码器。
...
`V4L2_CID_MPEG_VIDEO_H264_LOOP_FILTER_ALPHA (integer)`
    环内滤波器 alpha 系数，定义于 H264 标准。该值对应于 slice header 字段 slice_alpha_c0_offset_div2，取值范围应为 -6 到 +6（含）。实际的 alpha 偏移 FilterOffsetA 是此值的两倍。适用于 H264 编码器。

`V4L2_CID_MPEG_VIDEO_H264_LOOP_FILTER_BETA (integer)`
    环内滤波器 beta 系数，定义于 H264 标准。该值对应于 slice header 字段 slice_beta_offset_div2，取值范围应为 -6 到 +6（含）。实际的 beta 偏移 FilterOffsetB 是此值的两倍。适用于 H264 编码器。
...
enum v4l2_mpeg_video_h264_entropy_mode -
    H264 的熵编码模式 - CABAC/CAVLC。适用于 H264 编码器。可能的值如下：
...
`V4L2_CID_MPEG_VIDEO_H264_CHROMA_QP_INDEX_OFFSET (integer)`
    指定应加到亮度量化参数上以确定色度量化参数的偏移量。适用于 H264 编码器。

`V4L2_CID_MPEG_VIDEO_CYCLIC_INTRA_REFRESH_MB (integer)`
    循环帧内宏块刷新。这是每帧刷新的连续宏块数量。每一帧依次刷新一组宏块，直到整个循环完成并从帧顶部重新开始。将此控件设为零表示不刷新宏块。注意，当 `V4L2_CID_MPEG_VIDEO_INTRA_REFRESH_PERIOD` 控件被设为非零值时，此控件将不起作用。适用于 H264、H263 和 MPEG4 编码器。

`V4L2_CID_MPEG_VIDEO_INTRA_REFRESH_PERIOD_TYPE (enum)`

enum v4l2_mpeg_video_intra_refresh_period_type -
    设置帧内刷新的类型。刷新整个帧的周期由 V4L2_CID_MPEG_VIDEO_INTRA_REFRESH_PERIOD 指定。注意，如果不存在此控件，则所使用的刷新类型未定义，由驱动决定。适用于 H264 和 HEVC 编码器。可能的值如下：
...
`V4L2_CID_MPEG_VIDEO_INTRA_REFRESH_PERIOD (integer)`
    帧内宏块刷新周期。它设置刷新整个帧的周期。换句话说，它定义了整个帧将被帧内刷新的帧数。例如：将周期设为 1 表示整个帧将被刷新；设为 2 表示一半宏块在 frameX 上进行帧内刷新，另一半宏块在 frameX + 1 上刷新，依此类推。将周期设为零表示未指定周期。注意，如果客户端将此控件设为非零值，则 `V4L2_CID_MPEG_VIDEO_CYCLIC_INTRA_REFRESH_MB` 控件应被忽略。适用于 H264 和 HEVC 编码器。

`V4L2_CID_MPEG_VIDEO_FRAME_RC_ENABLE (boolean)`
    帧级码率控制使能。如果禁用此控件，则每种帧类型的量化参数为常量，并通过相应控件设置（例如 `V4L2_CID_MPEG_VIDEO_H263_I_FRAME_QP`）。如果启用帧码率控制，则量化参数会被调整以满足所选比特率。量化参数的最小值和最大值可通过相应控件设置（例如 `V4L2_CID_MPEG_VIDEO_H263_MIN_QP`）。适用于编码器。

`V4L2_CID_MPEG_VIDEO_MB_RC_ENABLE (boolean)`
    宏块级码率控制使能。适用于 MPEG4 和 H264 编码器。

`V4L2_CID_MPEG_VIDEO_MPEG4_QPEL (boolean)`
    MPEG4 的 1/4 像素运动估计。适用于 MPEG4 编码器。

`V4L2_CID_MPEG_VIDEO_H263_I_FRAME_QP (integer)`
    H263 的 I 帧量化参数。有效范围：1 到 31。
...
`V4L2_CID_MPEG_VIDEO_H264_P_FRAME_QP (integer)`
    H264 的 P 帧量化参数。有效范围：0 到 51。

`V4L2_CID_MPEG_VIDEO_H264_B_FRAME_QP (integer)`
    H264 的 B 帧量化参数。有效范围：0 到 51。

`V4L2_CID_MPEG_VIDEO_H264_I_FRAME_MIN_QP (integer)`
    用于限制 H264 I 帧质量范围的 H264 I 帧最小量化参数。有效范围：0 到 51。如果同时设置了 V4L2_CID_MPEG_VIDEO_H264_MIN_QP，则量化参数应满足两者的要求。

`V4L2_CID_MPEG_VIDEO_H264_I_FRAME_MAX_QP (integer)`
    用于限制 H264 I 帧质量范围的 H264 I 帧最大量化参数。有效范围：0 到 51。如果同时设置了 V4L2_CID_MPEG_VIDEO_H264_MAX_QP，则量化参数应满足两者的要求。

`V4L2_CID_MPEG_VIDEO_H264_P_FRAME_MIN_QP (integer)`
    用于限制 H264 P 帧质量范围的 H264 P 帧最小量化参数。有效范围：0 到 51。如果同时设置了 V4L2_CID_MPEG_VIDEO_H264_MIN_QP，则量化参数应满足两者的要求。

`V4L2_CID_MPEG_VIDEO_H264_P_FRAME_MAX_QP (integer)`
    用于限制 H264 P 帧质量范围的 H264 P 帧最大量化参数。有效范围：0 到 51。如果同时设置了 V4L2_CID_MPEG_VIDEO_H264_MAX_QP，则量化参数应满足两者的要求。

`V4L2_CID_MPEG_VIDEO_H264_B_FRAME_MIN_QP (integer)`
    用于限制 H264 B 帧质量范围的 H264 B 帧最小量化参数。有效范围：0 到 51。如果同时设置了 V4L2_CID_MPEG_VIDEO_H264_MIN_QP，则量化参数应满足两者的要求。

`V4L2_CID_MPEG_VIDEO_H264_B_FRAME_MAX_QP (integer)`
    用于限制 H264 B 帧质量范围的 H264 B 帧最大量化参数。有效范围：0 到 51。如果同时设置了 V4L2_CID_MPEG_VIDEO_H264_MAX_QP，则量化参数应满足两者的要求。

`V4L2_CID_MPEG_VIDEO_MPEG4_I_FRAME_QP (integer)`
    MPEG4 的 I 帧量化参数。有效范围：1 到 31。
...
`V4L2_CID_MPEG_VIDEO_MPEG4_P_FRAME_QP (integer)`
    MPEG4 的 P 帧量化参数。有效范围：1 到 31。

`V4L2_CID_MPEG_VIDEO_MPEG4_B_FRAME_QP (integer)`
    MPEG4 的 B 帧量化参数。有效范围：1 到 31。


`V4L2_CID_MPEG_VIDEO_VBV_SIZE (integer)`
    视频缓冲校验器（Video Buffer Verifier）大小，单位为千字节，用作帧跳过的限制。VBV 在标准中被定义为一种验证所产生码流能否被成功解码的手段。标准将其描述为“一个假设解码器的一部分，在概念上连接到编码器的输出。其目的是对编码器或编辑过程可能产生的数据速率的变化性施加约束。”。适用于 MPEG1、MPEG2、MPEG4 编码器。


`V4L2_CID_MPEG_VIDEO_VBV_DELAY (integer)`
    为 VBV 缓冲区控制设置初始延迟，单位为毫秒。
...
`V4L2_CID_MPEG_VIDEO_H264_CPB_SIZE (integer)`
    编码图像缓冲区（Coded Picture Buffer）大小，单位为千字节，用作帧跳过的限制。CPB 在 H264 标准中被定义为一种验证所产生码流能否被成功解码的手段。适用于 H264 编码器。

`V4L2_CID_MPEG_VIDEO_H264_I_PERIOD (integer)`
    H264 在开放 GOP 中 I 帧之间的周期。对于开放 GOP，这是两个 I 帧之间的周期。IDR（Instantaneous Decoding Refresh，即时解码刷新）帧之间的周期取自 GOP_SIZE 控件。IDR 帧代表即时解码刷新，是一种在其之后不再引用任何先前帧的 I 帧。这意味着可以从 IDR 帧重新开始码流，而无需存储或解码任何先前帧。适用于 H264 编码器。
...
`V4L2_CID_MPEG_VIDEO_REPEAT_SEQ_HEADER (boolean)`
    重复视频序列头。重复这些头部使对视频流的随机访问更容易。适用于 MPEG1、2 和 4 编码器。
...
`V4L2_CID_MPEG_VIDEO_H264_SEI_FRAME_PACKING (boolean)`
    在编码码流中启用生成帧封装补充增强信息（frame packing SEI）。帧封装 SEI 消息包含用于 3D 观看的 L 和 R 平面的排列方式。适用于 H264 编码器。

`V4L2_CID_MPEG_VIDEO_H264_SEI_FP_CURRENT_FRAME_0 (boolean)`
    在帧封装 SEI 中将当前帧设为 frame0。适用于 H264 编码器。
...
enum v4l2_mpeg_video_h264_sei_fp_arrangement_type -
    H264 SEI 的帧封装排列方式类型。适用于 H264 编码器。可能的值如下：
...
`V4L2_CID_MPEG_VIDEO_H264_FMO (boolean)`
    在编码码流中启用灵活宏块排序（FMO）。这是一种用于重组图像中宏块顺序的技术。适用于 H264 编码器。
...
enum v4l2_mpeg_video_h264_fmo_map_type -
    使用 FMO 时，映射类型将图像划分为不同的宏块扫描模式。适用于 H264 编码器。可能的值如下：
...
enum v4l2_mpeg_video_h264_fmo_change_dir -
    指定光栅和擦除映射的切片组变化方向。适用于 H264 编码器。可能的值如下：
...
`V4L2_CID_MPEG_VIDEO_H264_FMO_CHANGE_RATE (integer)`
    指定光栅和擦除映射中第一个切片组的大小。适用于 H264 编码器。

`V4L2_CID_MPEG_VIDEO_H264_FMO_RUN_LENGTH (integer)`
    指定交织映射中连续宏块的数量。适用于 H264 编码器。

`V4L2_CID_MPEG_VIDEO_H264_ASO (boolean)`
    在编码码流中启用任意切片排序（ASO）。适用于 H264 编码器。

`V4L2_CID_MPEG_VIDEO_H264_ASO_SLICE_ORDER (integer)`
    指定 ASO 中的切片顺序。适用于 H264 编码器。所提供的 32 位整数按如下方式解释（bit 0 = 最低有效位）：
...
`V4L2_CID_MPEG_VIDEO_H264_HIERARCHICAL_CODING_LAYER (integer)`
    指定分层编码层的数量。适用于 H264 编码器。

`V4L2_CID_MPEG_VIDEO_H264_HIERARCHICAL_CODING_LAYER_QP (integer)`
    为每一层指定用户自定义的 QP。适用于 H264 编码器。所提供的 32 位整数按如下方式解释（bit 0 = 最低有效位）：
...
`V4L2_CID_FWHT_I_FRAME_QP (integer)`
    FWHT 的 I 帧量化参数。有效范围：1 到 31。

`V4L2_CID_FWHT_P_FRAME_QP (integer)`
    FWHT 的 P 帧量化参数。有效范围：1 到 31。

`V4L2_CID_MPEG_VIDEO_AVERAGE_QP (integer)`
    这个只读控件返回当前已编码帧的平均 QP 值。该值适用于最后一个出队的捕获缓冲区（VIDIOC_DQBUF）。其有效范围取决于编码格式和参数。对于 H264，有效范围为 0 到 51。对于 HEVC，8 位时为 0 到 51，10 位时为 0 到 63。对于 H263 和 MPEG4，有效范围为 1 到 31。对于 VP8，有效范围为 0 到 127。对于 VP9，有效范围为 0 到 255。如果编解码器的 MIN_QP 和 MAX_QP 已设置，则 QP 将满足两者的要求。编解码器需要始终使用指定的范围，而不是硬件自定义范围。适用于编码器
...
以下 MPEG 类控件涉及特定于三星 S5P 系列 SoC 中 Multi Format Codec 5.1 设备的 MPEG 解码与编码设置。
...
`V4L2_CID_MPEG_MFC51_VIDEO_DECODER_H264_DISPLAY_DELAY_ENABLE (boolean)`
    如果启用了显示延迟，则解码器在处理一定数量的 OUTPUT 缓冲区后，被迫返回一个 CAPTURE 缓冲区（已解码帧）。该延迟可通过 `V4L2_CID_MPEG_MFC51_VIDEO_DECODER_H264_DISPLAY_DELAY` 设置。例如，此特性可用于生成视频缩略图。适用于 H264 解码器。
...
`V4L2_CID_MPEG_MFC51_VIDEO_H264_ADAPTIVE_RC_DARK (boolean)`
    针对暗区域的自适应码率控制。仅当启用 H.264 和宏块级码率控制（`V4L2_CID_MPEG_VIDEO_MB_RC_ENABLE`）时有效。适用于 H264 编码器。

`V4L2_CID_MPEG_MFC51_VIDEO_H264_ADAPTIVE_RC_SMOOTH (boolean)`
    针对平滑区域的自适应码率控制。仅当启用 H.264 和宏块级码率控制（`V4L2_CID_MPEG_VIDEO_MB_RC_ENABLE`）时有效。适用于 H264 编码器。

`V4L2_CID_MPEG_MFC51_VIDEO_H264_ADAPTIVE_RC_STATIC (boolean)`
    针对静态区域的自适应码率控制。仅当启用 H.264 和宏块级码率控制（`V4L2_CID_MPEG_VIDEO_MB_RC_ENABLE`）时有效。适用于 H264 编码器。

`V4L2_CID_MPEG_MFC51_VIDEO_H264_ADAPTIVE_RC_ACTIVITY (boolean)`
    针对活动区域的自适应码率控制。仅当启用 H.264 和宏块级码率控制（`V4L2_CID_MPEG_VIDEO_MB_RC_ENABLE`）时有效。适用于 H264 编码器。
...
enum v4l2_mpeg_mfc51_video_frame_skip_mode -
    ...
    - - `V4L2_MPEG_MFC51_VIDEO_FRAME_SKIP_MODE_BUF_LIMIT`
      - 帧跳过模式已启用，缓冲区限制由 VBV（MPEG1/2/4）或 CPB（H264）缓冲区大小控件设定。
...
以下 MPEG 类控件涉及特定于 Conexant CX23415 和 CX23416 MPEG 编码芯片的 MPEG 编码设置。
...
enum v4l2_mpeg_cx2341x_video_chroma_spatial_filter_type -
    选择用于色度空间滤波器的算法（默认 `1D_HOR`）。可能的值如下：
...
`V4L2_CID_MPEG_CX2341X_VIDEO_TEMPORAL_FILTER (integer (0-31))`
    时间滤波器的设置。0 = 关闭，31 = 最大。（全分辨率采集时默认 8，缩放采集时默认 0。）

`V4L2_CID_MPEG_CX2341X_VIDEO_MEDIAN_FILTER_TYPE`
    (enum)

enum v4l2_mpeg_cx2341x_video_median_filter_type -
    中值滤波器类型（默认 `OFF`）。可能的值如下：
...
`V4L2_CID_MPEG_CX2341X_STREAM_INSERT_NAV_PACKETS (boolean)`
    CX2341X MPEG 编码器可以在每四个视频帧之间向码流中插入一个空的 MPEG-2 PES 包。包大小为 2048 字节，包含 packet_start_code_prefix 和 stream_id 字段。stream_id 为 0xBF（私有流 2）。载荷由 0x00 字节组成，由应用程序填充。0 = 不插入，1 = 插入包。
...
`V4L2_CID_MPEG_VIDEO_VPX_GOLDEN_FRAME_REF_PERIOD (integer)`
    设置黄金帧的刷新周期。该周期以帧数定义。对于值 'n'，从第一个关键帧开始，每第 n 帧将被视为黄金帧。例如，对于编码序列 0、1、2、3、4、5、6、7，若黄金帧刷新周期设为 4，则帧 0、4、8 等将被视为黄金帧，因为帧 0 始终是关键帧。
...
enum v4l2_mpeg_video_vp8_profile -
    此控件用于选择 VP8 编码器的档次。它也用于枚举 VP8 编码器或解码器支持的档次。可能的值如下：
...
enum v4l2_mpeg_video_vp9_profile -
    此控件用于选择 VP9 编码器的档次。它也用于枚举 VP9 编码器或解码器支持的档次。可能的值如下：
...
enum v4l2_mpeg_video_vp9_level -
    此控件用于选择 VP9 编码器的等级。它也用于枚举 VP9 编码器或解码器支持的等级。更多信息可参阅 `webmproject <https://www.webmproject.org/vp9/levels/>`__。可能的值如下：
...
`V4L2_CID_MPEG_VIDEO_HEVC_MIN_QP (integer)`
    HEVC 的最小量化参数。有效范围：8 位时为 0 到 51，10 位时为 0 到 63。

`V4L2_CID_MPEG_VIDEO_HEVC_MAX_QP (integer)`
    HEVC 的最大量化参数。有效范围：8 位时为 0 到 51，10 位时为 0 到 63。

`V4L2_CID_MPEG_VIDEO_HEVC_I_FRAME_QP (integer)`
    HEVC 的 I 帧量化参数。有效范围：[V4L2_CID_MPEG_VIDEO_HEVC_MIN_QP, V4L2_CID_MPEG_VIDEO_HEVC_MAX_QP]。

`V4L2_CID_MPEG_VIDEO_HEVC_P_FRAME_QP (integer)`
    HEVC 的 P 帧量化参数。有效范围：[V4L2_CID_MPEG_VIDEO_HEVC_MIN_QP, V4L2_CID_MPEG_VIDEO_HEVC_MAX_QP]。

`V4L2_CID_MPEG_VIDEO_HEVC_B_FRAME_QP (integer)`
    HEVC 的 B 帧量化参数。有效范围：[V4L2_CID_MPEG_VIDEO_HEVC_MIN_QP, V4L2_CID_MPEG_VIDEO_HEVC_MAX_QP]。

`V4L2_CID_MPEG_VIDEO_HEVC_I_FRAME_MIN_QP (integer)`
    用于限制 HEVC I 帧质量范围的 HEVC I 帧最小量化参数。有效范围：8 位时为 0 到 51，10 位时为 0 到 63。如果同时设置了 V4L2_CID_MPEG_VIDEO_HEVC_MIN_QP，则量化参数应满足两者的要求。

`V4L2_CID_MPEG_VIDEO_HEVC_I_FRAME_MAX_QP (integer)`
    用于限制 HEVC I 帧质量范围的 HEVC I 帧最大量化参数。有效范围：8 位时为 0 到 51，10 位时为 0 到 63。如果同时设置了 V4L2_CID_MPEG_VIDEO_HEVC_MAX_QP，则量化参数应满足两者的要求。

`V4L2_CID_MPEG_VIDEO_HEVC_P_FRAME_MIN_QP (integer)`
    用于限制 HEVC P 帧质量范围的 HEVC P 帧最小量化参数。有效范围：8 位时为 0 到 51，10 位时为 0 到 63。如果同时设置了 V4L2_CID_MPEG_VIDEO_HEVC_MIN_QP，则量化参数应满足两者的要求。

`V4L2_CID_MPEG_VIDEO_HEVC_P_FRAME_MAX_QP (integer)`
    用于限制 HEVC P 帧质量范围的 HEVC P 帧最大量化参数。有效范围：8 位时为 0 到 51，10 位时为 0 到 63。如果同时设置了 V4L2_CID_MPEG_VIDEO_HEVC_MAX_QP，则量化参数应满足两者的要求。

`V4L2_CID_MPEG_VIDEO_HEVC_B_FRAME_MIN_QP (integer)`
    用于限制 HEVC B 帧质量范围的 HEVC B 帧最小量化参数。有效范围：8 位时为 0 到 51，10 位时为 0 到 63。如果同时设置了 V4L2_CID_MPEG_VIDEO_HEVC_MIN_QP，则量化参数应满足两者的要求。

`V4L2_CID_MPEG_VIDEO_HEVC_B_FRAME_MAX_QP (integer)`
    用于限制 HEVC B 帧质量范围的 HEVC B 帧最大量化参数。有效范围：8 位时为 0 到 51，10 位时为 0 到 63。如果同时设置了 V4L2_CID_MPEG_VIDEO_HEVC_MAX_QP，则量化参数应满足两者的要求。

`V4L2_CID_MPEG_VIDEO_HEVC_HIER_QP (boolean)`
    HIERARCHICAL_QP 允许主机通过 HIERARCHICAL_QP_LAYER 为每一时间层指定量化参数值。仅当 HIERARCHICAL_CODING_LAYER 大于 1 时有效。将此控件值设为 1 可启用各层的 QP 值设置。
...
`V4L2_CID_MPEG_VIDEO_HEVC_HIER_CODING_LAYER (integer)`
    选择分层编码层。在普通编码（非分层编码）中，应设为零。可能的值为 [0, 6]。0 表示分层编码层 0，1 表示分层编码层 1，依此类推。
...
`V4L2_CID_MPEG_VIDEO_HEVC_FRAME_RATE_RESOLUTION (integer)`
    指示一秒内的均匀间隔子区间（称为 ticks）数量。这是一个 16 位无符号整数，最大值为 0xffff，最小值为 1。
...
enum v4l2_mpeg_video_hevc_tier -
    TIER_FLAG 指定 HEVC 编码图像的层级（tier）信息。层级是为了处理最大比特率不同的应用而设立的。将该标志设为 0 选择 HEVC 的 Main 层级，设为 1 表示 High 层级。High 层级用于需要高比特率的应用。
...
enum v4l2_mpeg_video_hevc_hier_refresh_type -
    选择 HEVC 编码器的刷新类型。主机必须将周期指定到 V4L2_CID_MPEG_VIDEO_HEVC_REFRESH_PERIOD。
...
`V4L2_CID_MPEG_VIDEO_HEVC_REFRESH_PERIOD (integer)`
    选择 HEVC 编码器的刷新周期。它指定两个 CRA/IDR 图像之间的 I 图像数量。仅当 REFRESH_TYPE 不为 0 时有效。

`V4L2_CID_MPEG_VIDEO_HEVC_LOSSLESS_CU (boolean)`
    指示 HEVC 无损编码。设为 0 禁用无损编码，设为 1 启用无损编码。

`V4L2_CID_MPEG_VIDEO_HEVC_CONST_INTRA_PRED (boolean)`
    指示 HEVC 编码器的恒定帧内预测。指定受限帧内预测，其中帧内最大编码单元（LCU）的预测仅使用相邻帧内 LCU 的残差数据和已解码样本来进行。将该值设为 1 启用恒定帧内预测，设为 0 禁用恒定帧内预测。

`V4L2_CID_MPEG_VIDEO_HEVC_WAVEFRONT (boolean)`
    指示 HEVC 编码器的波前并行处理（wavefront parallel processing）。设为 0 禁用该特性，设为 1 启用波前并行处理。

`V4L2_CID_MPEG_VIDEO_HEVC_GENERAL_PB (boolean)`
    将该值设为 1 可为 HEVC 编码器启用 P 帧和 B 帧的组合。

`V4L2_CID_MPEG_VIDEO_HEVC_TEMPORAL_ID (boolean)`
    指示 HEVC 编码器的时间标识符，通过将值设为 1 来启用。

`V4L2_CID_MPEG_VIDEO_HEVC_STRONG_SMOOTHING (boolean)`
    指示当设为 1 时，在 CVS 的帧内预测滤波过程中有条件地使用双线性插值。指示当设为 0 时，在 CVS 中不使用双线性插值。

`V4L2_CID_MPEG_VIDEO_HEVC_MAX_NUM_MERGE_MV_MINUS1 (integer)`
    指示合并候选运动矢量的最大数量。取值范围为 0 到 4。

`V4L2_CID_MPEG_VIDEO_HEVC_TMV_PREDICTION (boolean)`
    指示 HEVC 编码器的时间运动矢量预测。设为 1 启用预测，设为 0 禁用预测。

`V4L2_CID_MPEG_VIDEO_HEVC_WITHOUT_STARTCODE (boolean)`
    指定 HEVC 是否生成以长度字段大小代替起始码模式的码流。长度字段的大小可通过 V4L2_CID_MPEG_VIDEO_HEVC_SIZE_OF_LENGTH_FIELD 控件配置。将该值设为 0 禁用无起始码模式的编码。将该值设为 1 将启用无起始码模式的编码。
...
`V4L2_CID_MPEG_VIDEO_REF_NUMBER_FOR_PFRAMES (integer)`
    选择 HEVC 编码器所需的 P 参考图像数量。P 帧可使用 1 或 2 帧作为参考。

`V4L2_CID_MPEG_VIDEO_PREPEND_SPSPPS_TO_IDR (integer)`
    指示是否在每个 IDR 处生成 SPS 和 PPS。设为 0 禁用在每个 IDR 处生成 SPS 和 PPS。设为 1 启用在每个 IDR 处生成 SPS 和 PPS。
