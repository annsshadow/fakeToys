...
这四种像素格式是每样10 位的打包原始 sRGB / Bayer 格式。每四个连续的样本被打包5 个字节。前 4 个字节中的每一个包含像素的 8 个高位，5 个字节以相同顺序包含每个像素2 个最低有效位
每个 n 像素行包n/2 个绿色样本和 n/2 个蓝色或红色样本，绿-红行与绿-蓝行交替。它们通常被描述为 GRGR... BGBG...、RGRG... GBGB... 等。下面是一个小V4L2_PIX_FMT_SBGGR10P 图像示例
**字节顺序*
每个单元格为一个字节
