...
VIDIOC_SUBSCRIBE_EVENT - VIDIOC_UNSUBSCRIBE_EVENT - 订阅或取消订阅事
## 概要
...
`fd`
    `open()` 返回的文件描述符
`argp`
    指向结构`v4l2_event_subscription` 的指针
## 描述


订阅或取消订V4L2 事件。已订阅的事件通过 VIDIOC_DQEVENT ioctl 出队
    ...
    - - __u32
      ...
      - 事件的类型，参见 event-type```
	   ...
	   用于一次性取消订阅所有事件    * - __u32
      - ``id``
      - 事件源的 ID。如果事件源没有关联ID，则将其设为 0。事件是否需ID
	取决于事件类型    * - __u32
      - ``flags``
      - 事件标志，参:ref:`event-flags`    * - __u32
      - ``reserved``\ [5]
...
```
	...
	（如 `V4L2_EVENT_CTRL`）有意义。其他事件会忽略此标志    - - `V4L2_EVENT_SUB_FL_ALLOW_FEEDBACK`
      - 0x0002
      - 若设置，则直接由 ioctl 引起的事件也会发送给调用ioctl 的文件句柄。例如，
	使用 VIDIOC_S_CTRL <VIDIOC_G_CTRL> 改变一个控件会导致一V4L2_EVENT_CTRL 	发送回同一个文件句柄	通常此类事件会被抑制，以防止反馈环路：一个应用程序将某个控件改为一个值，
