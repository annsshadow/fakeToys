...
`v4l2_meta_format` 接口来获取这些统计信息。缓冲区包含 `mali-c55-config.h` 定义C 结构`mali_c55_stats_buffer` 的单个实例，因此可以通过如下方式缓冲区中获取该结构体
...
配置参数通过 :ref:`mali-c55 3a params <mali-c55-3a-params>` 元数据输出视频节传递，使用 `v4l2_meta_format` 接口。与为每个可配置 ISP 区域包含子结构体的单一
结构体不同，Mali-C55 的参数使v4l2-isp parameters 系统，通过该系统，参数组被
定义为不同的结构体或“块”（blocks），可以被添加到 `v4l2_isp_params_buffer` data 成员中。用户空间负责用需要由驱动配置的块来填data 成员。每个块特有结构体将其第一个成员嵌`v4l2_isp_params_block_header`，并且用户空间必须用
