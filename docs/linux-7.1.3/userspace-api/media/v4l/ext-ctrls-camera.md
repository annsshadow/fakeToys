...
`V4L2_CID_EXPOSURE_ABSOLUTE (integer)`
    决定相机传感器的曝光时间。曝光时间受帧间隔限制。驱动应将数值解释为 100 µs 单位，其中值 1 表示 1/10000 秒，10000 表示 1 秒，100000 表示 10 秒。

`V4L2_CID_EXPOSURE_AUTO_PRIORITY (boolean)`
    当 `V4L2_CID_EXPOSURE_AUTO` 设为 `AUTO` 或
    `APERTURE_PRIORITY` 时，该控制决定设备是否可以动态调整帧率。默认情况下该功能被禁用
    (0)，帧率必须保持恒定。

`V4L2_CID_AUTO_EXPOSURE_BIAS (integer menu)`
    决定自动曝光补偿，仅当 `V4L2_CID_EXPOSURE_AUTO` 控制设为 `AUTO`、
    `SHUTTER_PRIORITY` 或 `APERTURE_PRIORITY` 时才生效。它以
    EV 表示，驱动应将数值解释为 0.001 EV 单位，其中值 1000 表示 +1 EV。

    增大曝光补偿值相当于降低曝光值（EV），并会增加图像传感器处的光量。相机通过调整绝对曝光时间和/或光圈孔径来执行曝光补偿。
...
`V4L2_CID_PAN_ABSOLUTE (integer)`
    ...
    * 3600 到 +180 * 3600（含边界）之间。

`V4L2_CID_TILT_ABSOLUTE (integer)`
    该控制将相机垂直转动到指定位置。正值使相机向上移动，负值向下。驱动应将数值解释
    为角秒，有效值在 -180 ** 3600 到 +180 ** 3600（含边界）之间。
...
`V4L2_CID_AUTO_FOCUS_STOP (button)`
    中止由 `V4L2_CID_AUTO_FOCUS_START` 控制启动的自动对焦。仅当连续自动对焦被禁用（即 `V4L2_CID_FOCUS_AUTO` 控制设为 `FALSE` (0)）时才生效。


`V4L2_CID_AUTO_FOCUS_STATUS (bitmask)`
    自动对焦状态。这是一个只读控制。

    设置 `V4L2_CID_3A_LOCK` 控制的 `V4L2_LOCK_FOCUS` 锁位可能会停止对
...
`V4L2_CID_PRIVACY (boolean)`
    阻止相机获取视频。当该控制设为 `TRUE` (1) 时，相机无法捕获任何图像。强制隐私的常见手段是传感器的机械遮光以及固件图像处理，但设备不限于这些方法。实现 privacy 控制的设备必须支持读访问，并可以支持写访问。
...
enum v4l2_auto_n_preset_white_balance -
    ...
    - - `V4L2_WHITE_BALANCE_FLUORESCENT_H`
      - 使用该设置时，相机将补偿荧光灯 H 照明。
    - - `V4L2_WHITE_BALANCE_HORIZON`
      - 地平线日光的白平衡设置。大约对应于 5000 K 色温。
    - - `V4L2_WHITE_BALANCE_DAYLIGHT`
      - 日光（晴朗天空）的白平衡预设。大约对应于 5000...6500 K 色温。
    - - `V4L2_WHITE_BALANCE_FLASH`
      - 使用该设置时，相机将补偿闪光灯照明。它使颜色略微偏暖，大约对应于
        5000...5500 K 色温。
    - - `V4L2_WHITE_BALANCE_CLOUDY`
      - 中度阴天的白平衡预设。该选项大约对应于 6500...8000 K 色温范围。
...
`V4L2_CID_ISO_SENSITIVITY (integer menu)`
    决定图像传感器的 ISO 等效值，表示传感器对光的灵敏度。这些数字按算术标度表示，遵循 iso12232 标准，其中传感器灵敏度加倍由数值 ISO 值加倍表示。应用程序应将数值解释为标准 ISO 值乘以 1000，例如控制值 800 表示 ISO 0.8。驱动通常只支持标准 ISO 值的一个子集。在
...
enum v4l2_scene_mode -
    ...
    - - `V4L2_SCENE_MODE_CANDLELIGHT`
      - 烛光。相机通常会提高 ISO 灵敏度并降低快门速度。该模式补偿场景中相对靠近的主体。为保留光线氛围，闪光灯被禁用。
    ...
    - - `V4L2_SCENE_MODE_PARTY_INDOOR`
      - 聚会和室内。为捕捉由室内背景照明以及闪光灯共同照明的室内场景而设计。相机通常会提高 ISO 灵敏度，并为低光条件调整曝光。
    - - `V4L2_SCENE_MODE_PORTRAIT`
      - 人像。相机调整光圈以减小景深，有助于将主体从平滑的背景中分离出来。大多数相机会识别场景中人脸并对其对焦。色调被调整以增强肤色。闪光灯强度通常降低。
    - - `V4L2_SCENE_MODE_SPORTS`
      - 运动。显著提高 ISO 并使用快速快门速度以冻结快速移动主体的动作。该模式下可能会看到增加的图像噪点。
...
`V4L2_CID_3A_LOCK (bitmask)`
    ...
    当某个给定算法未启用时，驱动应忽略锁定它的请求，并且不应返回错误。例如，当
...
`V4L2_CID_CAMERA_ORIENTATION (menu)`
    该只读控制通过报告相机所安装设备的安装位置来描述相机朝向。控制值是恒定的，不能被软件修改。该控制对于具有明确定义朝向的设备（例如手机、笔记本电脑和便携设备）特别有意义，因为该控制表示为相对于设备预期使用朝向的位置。例如，安装在手机、平板或笔记本电脑用户侧面的相机被称为具有 `V4L2_CAMERA_ORIENTATION_FRONT` 朝向，而安装在正面相反一侧的相机被称为具有 `V4L2_CAMERA_ORIENTATION_BACK` 朝向。未直接附接到设备、或以允许其自由移动的方式附接的相机传感器（例如网络摄像头和数码相机）被称为具有 `V4L2_CAMERA_ORIENTATION_EXTERNAL` 朝向。
...
`V4L2_CID_CAMERA_SENSOR_ROTATION (integer)`
    ...
    有关传感器安装旋转的精确定义，请参阅设备树绑定文件 'video-interfaces.txt' 中对 'rotation' 属性的详尽描述。
...
`V4L2_CID_HDR_SENSOR_MODE (menu)`
    更改传感器 HDR 模式。HDR 图像是通过使用两个不同的曝光周期合并同一场景的两次捕获获得的。HDR 模式描述了这两个捕获在传感器中合并的方式。
