...
这些驱动通常stateless_decoder 支持，并处理已解析的像素格式，例V4L2_PIX_FMT_H264_SLICE
...
`V4L2_CID_STATELESS_H264_SPS (struct)`
    指定与相H264 切片数据关联的序列参数集（从码流中提取）。其中包含配H264
    无状态硬件解码流水线所需的参数。码流参数依h264 标准7.4.2.1.1 
    ...
    - - __u8
      ...
      - 参见序列参数集约束标<h264_sps_constraints_set_flags>
    - - __u8
      - `level_idc`
      -
    - - __u8
...
`V4L2_CID_STATELESS_H264_PPS (struct)`
    指定与相H264 切片数据关联的图像参数集（从码流中提取）。其中包含配H264
    无状态硬件解码流水线所需的参数。码流参数依h264 标准7.4.2.2 
    “Picture Parameter Set RBSP Semantics”（图像参数RBSP 语义）定义。除非有明确注释
    另有说明，否则进一步的文档请参考上述规范
...
`V4L2_CID_STATELESS_H264_SCALING_MATRIX (struct)`
    指定与相H264 切片数据关联的缩放矩阵（从码流中提取）。码流参数依h264 标准
    7.4.2.1.1.1 “Scaling List Semantics”（缩放列表语义）定义。除非有明确注释
    另有说明，否则进一步的文档请参考上述规范
    ...
    - - __u8
      ...
      - 应用逆扫描过程后的缩放矩阵。期望的列表顺序为：帧内 Y、帧Cb、帧Cr
        帧间 Y、帧Cb、帧Cr。每个缩放列表中的值按光栅扫描顺序排列
    - - __u8
      - `scaling_list_8x8[^6^][^64^]`
      - 应用逆扫描过程后的缩放矩阵。期望的列表顺序为：帧内 Y、帧Y、帧Cb
        帧间 Cb、帧Cr、帧Cr。每个缩放列表中的值按光栅扫描顺序排列

`V4L2_CID_STATELESS_H264_SLICE_PARAMS (struct)`
    指定与相H264 切片数据关联的切片参数（从码流中提取）。其中包含配H264
    无状态硬件解码流水线所需的参数。码流参数依h264 标准7.4.3 
...
`V4L2_CID_STATELESS_H264_PRED_WEIGHTS (struct)`
    依据 h264 标准7.4.3.2 “Prediction Weight Table Semantics”（预测加权表语义）
    定义的预测加权表。预测加权表必须在第 7.3.3 “Slice header syntax”（切片头语法）
    所述条件下由应用程序传入
    ...
    - - struct `v4l2_h264_weight_factors`
      - `weight_factors[^2^]`
      - 索引 0 处的加权因子对应参考列0，索1 处的加权因子对应参考列1
...
`图像参考`
    ...
    - - __u8
      - `fields`
      - 指定该图像如何被引用。参见参考字<h264_ref_fields>
    - - __u8
      - `index`
...
`V4L2_CID_STATELESS_H264_DECODE_PARAMS (struct)`
    指定与相H264 切片数据关联的解码参数（从码流中提取）。其中包含配H264
    无状态硬件解码流水线所需的参数。码流参数依h264 标准定义。除非有明确注释
    另有说明，否则进一步的文档请参考上述规范
...
`解码参数标志`
    ...
    - - __u64
      - `reference_ts`
      - 用作参考的 V4L2 捕获缓冲区的时间戳，B 帧和 P 帧配合使用。该时间戳引
        struct `v4l2_buffer` 中的 `timestamp` 字段。使`v4l2_timeval_to_ns()`
        函数struct `v4l2_buffer` 中的 struct `timeval` 转换__u64
    - - __u32
      - `pic_num`
      - 对于短期参考，本字段必须与推导PicNum (8-28) 一致；对于长期参考，必须与推导
        LongTermPicNum (8-29) 一致。解码帧（而非场）时，pic_num FrameNumWrap 相同
    - - __u16
      - `frame_num`
      - 对于短期参考，本字段必须与切片头语法中frame_num 值一致（驱动会在需要时对该
        进行环绕处理）。对于长期参考，本字段必须设dec_ref_pic_marking() 语法中描述的
        long_term_frame_idx 的值
    - - __u8
      - `fields`
      - 指定DPB 条目如何被引用。参见参考字<h264_ref_fields>
    - - __u8
      - `reserved[^5^]`
      - 应用程序与驱动必须将本字段置为零
    - - __s32
...
`V4L2_CID_STATELESS_H264_DECODE_MODE (enum)`
    ...
    该控件用V4L2_PIX_FMT_H264_SLICE 像素格式的修饰符。支V4L2_PIX_FMT_H264_SLICE
    ...
    - - `V4L2_STATELESS_H264_DECODE_MODE_SLICE_BASED`
      ...
      - 以切片粒度进行解码。OUTPUT 缓冲区必须包含单个切片。选择该模式时
        ...
        需要使`V4L2_BUF_CAP_SUPPORTS_M2M_HOLD_CAPTURE_BUF` 标志
...
`V4L2_CID_STATELESS_H264_START_CODE (enum)`
    指定每个切片所期望H264 切片起始码。该控件用作 V4L2_PIX_FMT_H264_SLICE 像素格式
    的修饰符。支V4L2_PIX_FMT_H264_SLICE 的应用程序必须设置该控件，以指定缓冲区所期望
    ...
    - - `V4L2_STATELESS_H264_START_CODE_NONE`
      - 0
      - 选择该值表H264 切片不带任何起始码地传递给驱动。码流数据应遵循 h264 7.3.1
        NAL 单元语法，因此在需要时会包含仿真预防字节（emulation prevention bytes）
    - - `V4L2_STATELESS_H264_START_CODE_ANNEX_B`
      - 1
      - 选择该值表示期H264 切片Annex B 起始码作为前缀。依h264，有效的起始码可以是
        3 字节0x000001 4 字节0x00000001
...
`V4L2_CID_STATELESS_FWHT_PARAMS (struct)`
    指定与相FWHT 数据关联FWHT（快Walsh-Hadamard 变换）参数（从码流中提取）
    其中包含配置 FWHT 无状态硬件解码流水线所需的参数。该编解码器专用vicodec 测试驱动
    ...
    - - __u64
      - `backward_ref_ts`
      - 用作后向参考的 V4L2 捕获缓冲区的时间戳，P 帧配合使用。该时间戳引
        struct `v4l2_buffer` 中的 `timestamp` 字段。使`v4l2_timeval_to_ns()`
        函数struct `v4l2_buffer` 中的 struct `timeval` 转换__u64
    - - __u32
      - `version`
      - 编解码器版本。设`V4L2_FWHT_VERSION`
    - - __u32
      - `width`
      - 帧的宽度
    - - __u32
    ...
    - - __u32
      - `colorspace`
      - 帧的色彩空间，取自枚`v4l2_colorspace`
    - - __u32
      - `xfer_func`
      - 传输函数，取自枚`v4l2_xfer_func`
    - - __u32
      - `ycbcr_enc`
      - Y'CbCr 编码，取自枚`v4l2_ycbcr_encoding`
    - - __u32
      - `quantization`
      - 量化范围，取自枚`v4l2_quantization`
...
`V4L2_CID_STATELESS_VP8_FRAME (struct)`
    指定与相VP8 已解析帧数据关联的帧参数。其中包含配VP8 无状态硬件解码流水线
    所需的参数。码流参数依vp8 标准定义
    ...
    - - __u64
      - `last_frame_ts`
      - 用作上一参考帧V4L2 捕获缓冲区的时间戳，与帧间编码帧配合使用。该时间戳引
        struct `v4l2_buffer` 中的 `timestamp` 字段。使`v4l2_timeval_to_ns()`
        函数struct `v4l2_buffer` 中的 struct `timeval` 转换__u64
    - - __u64
      - `golden_frame_ts`
    ...
    - - __u64
      - `alt_frame_ts`
      - 用作备用参考帧V4L2 捕获缓冲区的时间戳，与帧间编码帧配合使用。该时间戳引
        struct `v4l2_buffer` 中的 `timestamp` 字段。使`v4l2_timeval_to_ns()`
        函数struct `v4l2_buffer` 中的 struct `timeval` 转换__u64
    - - __u64
      - `flags`
      - 参见帧标<vp8_frame_flags>
...
`分段标志`
    ...
    - - `V4L2_VP8_SEGMENT_FLAG_DELTA_VALUE_MODE`
      - 0x08
      - 若设置，分段特征数据模式delta-value（差值）；若清除，则absolute-value
        （绝对值）
    ...
    - - __u32
      - `flags`
      - 参见环路滤波器标<vp8_loop_filter_flags>
...
`V4L2_CID_STATELESS_MPEG2_SEQUENCE (struct)`
    指定与相MPEG-2 切片数据关联的序列参数（从码流中提取）。其中包含与 mpeg2part2
...
`V4L2_CID_STATELESS_MPEG2_PICTURE (struct)`
    指定与相MPEG-2 切片数据关联的图像参数（从码流中提取）。其中包含与 mpeg2part2
    规范中图像头（picture header）和图像编码扩展（picture coding extension）部分语法元
    对应的字段
    ...
    - - __u64
      - `backward_ref_ts`
      - 用作后向参考的 V4L2 捕获缓冲区的时间戳，B 帧和 P 帧配合使用。该时间戳引
        struct `v4l2_buffer` 中的 `timestamp` 字段。使`v4l2_timeval_to_ns()`
        函数struct `v4l2_buffer` 中的 struct `timeval` 转换__u64
    - - __u64
      - `forward_ref_ts`
      - 用作前向参考的 V4L2 捕获缓冲区的时间戳，B 帧配合使用。该时间戳引
        struct `v4l2_buffer` 中的 `timestamp` 字段。使`v4l2_timeval_to_ns()`
        函数struct `v4l2_buffer` 中的 struct `timeval` 转换__u64
    - - __u32
      - `flags`
    ...
    - - __u8
      ...
      - 当前切片所覆盖帧的图像编码类型（V4L2_MPEG2_PIC_CODING_TYPE_I
        V4L2_MPEG2_PIC_CODING_TYPE_P V4L2_MPEG2_PIC_CODING_TYPE_B）
    - - __u8
      - `picture_structure`
      - 图像结构：隔行顶场，2：隔行底场，3：逐行帧）
    - - __u8
      - `intra_dc_precision`
      - 离散余弦变换（DCT）的精度 位精度，1 位精度，20 位精度，31 位精度）
    - - __u8
      - `reserved[^5^]`
      - 应用程序与驱动必须将本字段置为零
...
`V4L2_CID_STATELESS_MPEG2_QUANTISATION (struct)`
    以之字形扫描顺序指定与相MPEG-2 切片数据关联的量化矩阵。该控件由内核初始化
    矩阵的默认值。若码流传输了用户自定义的量化矩阵加载，则应用程序应使用该控件
    若需要重置量化矩阵（例如在序列头处），应用程序还应设置该控件以加载默认值
    该过程由规范6.3.7 “Quant matrix extension”（量化矩阵扩展）规定
    ...
    - - __u8
      ...
      - 帧内编码帧色度分量的量化矩阵系数，按之字形扫描顺序。仅与非 4:2:0 YUV 格式相关
    - - __u8
      - `chroma_non_intra_quantiser_matrix[^64^]`
      - 非帧内编码帧色度分量的量化矩阵系数，按之字形扫描顺序。仅与非 4:2:0 YUV 格式相关
...
`V4L2_CID_STATELESS_VP9_COMPRESSED_HDR (struct)`
    ...
    与运动矢量相关的更新包含新值或零。所有其他更新包含经 inv_map_table[] 转换后的
    （参vp9 规范 6.3.5 节）
    ...
    - - __u8
      - `tx_mode`
      - 指定 TX 模式。更多细节参TX 模式 <vp9_tx_mode>
    - - __u8
      - `tx8[^2^][^1^]`
      - TX 8x8 概率差值
    - - __u8
...
参见 vp9 规范 .3.1 Tx mode semantics”（Tx 模式语义）一节获取更多细节

`V4L2_CID_STATELESS_VP9_FRAME (struct)`
    指定与相VP9 帧解码请求关联的帧参数。其中包含配VP9 无状态硬件解码流水线
    所需的参数。码流参数依vp9 标准定义
    ...
    - - struct `v4l2_vp9_quantization`
      - `quant`
      - 量化参数。更多细节参`v4l2_vp9_quantization`
    - - struct `v4l2_vp9_segmentation`
      - `seg`
      - 分段参数。更多细节参`v4l2_vp9_segmentation`
    - - __u32
      - `flags`
    ...
    - - __u16
      - `frame_width_minus_1`
      - 1 得到以像素表示的帧宽度。参vp9 规范7.2.3 节
    - - __u16
      - `frame_height_minus_1`
      - 1 得到以像素表示的帧高度。参vp9 规范7.2.3 节
    - - __u16
      - `render_width_minus_1`
      - 1 得到期望的渲染宽度（以像素表示）。该值不用于解码过程，但可能被硬件缩放器
        用于准备可供扫描输出（scanout）的帧。参vp9 规范7.2.4 节
    - - __u16
      - render_height_minus_1
      - 1 得到期望的渲染高度（以像素表示）。该值不用于解码过程，但可能被硬件缩放器
        用于准备可供扫描输出的帧。参vp9 规范7.2.4 节
    - - __u64
      - `last_frame_ts`
      - “last”参考缓冲区的时间戳。该时间戳引struct `v4l2_buffer` 中的 `timestamp`
        字段。使`v4l2_timeval_to_ns()` 函数struct `v4l2_buffer` 中的
        struct `timeval` 转换__u64
    - - __u64
      - `golden_frame_ts`
      - “golden”参考缓冲区的时间戳。该时间戳引struct `v4l2_buffer` 中的 `timestamp`
        字段。使`v4l2_timeval_to_ns()` 函数struct `v4l2_buffer` 中的
        struct `timeval` 转换__u64
    - - __u64
      - `alt_frame_ts`
      - “alt”参考缓冲区的时间戳。该时间戳引struct `v4l2_buffer` 中的 `timestamp`
        字段。使`v4l2_timeval_to_ns()` 函数struct `v4l2_buffer` 中的
        struct `timeval` 转换__u64
    - - __u8
      - `ref_frame_sign_bias`
    ...
    - - __u8
      - `profile`
      - VP9 档次（profile）。可以为 0 3
    - - __u8
      - `bit_depth`
      - 分量位深（比特）。可以为 80 12。注意并非所有档次都支持 10 12 位深
    - - __u8
    ...
    - - __u8
      - `tile_cols_log2`
      - 指定每个 tile 宽度的以 2 为底的对数（宽度8x8 块为单位度量）。必须小于或等于 6
    - - __u8
      - `tile_rows_log2`
      - 指定每个 tile 高度的以 2 为底的对数（高度8x8 块为单位度量）
    - - __u8
      - `reference_mode`
      - 指定要使用的帧间预测类型。更多细节参见参考模<vp9_reference_mode>。注意该
        是作为压缩头部解析过程的一部分推导出来的，因此本应属于 :c:type:
        `v4l2_ctrl_vp9_compressed_hdr` 可选控件。若驱动不需要压缩头部，将本值设
        零是安全的
    - - __u8
      - `reserved[^7^]`
      - 应用程序与驱动必须将本字段置为零
...
更多细节参见 vp9 规范 .2 Uncompressed header semantics”（未压缩头部语义）一节
...
更多细节参见 vp9 规范 .2.7 Interpolation filter semantics”（插值滤波器语义）一节
...
更多细节参见 vp9 规范 .3.6 Frame reference mode semantics”（帧参考模式语义）一节


编码量化参数。更多细节参vp9 规范 .2.10 Segmentation params syntax”（分段参数语法）一节
    ...
    - - __u8
      - `feature_data[^8^][^4^]`
      - 附带在每个特征上的数据。仅当特征被启用时数据条目才有效。该数组应以段编号作
        第一维（0..7）、以 V4L2_VP9_SEG_* 之一作为第二维进行索引。参见分段特ID
        <vp9_segment_feature>銆。
    - - __u8
      - `feature_enabled[^8^]`
      - 位掩码，定义每个段中启用了哪些特征。每个段的值为 V4L2_VP9_SEGMENT_FEATURE_ENABLED(id)
        值的组合，其id V4L2_VP9_SEG_* 之一。参见分段特ID <vp9_segment_feature>
    - - __u8
      - `tree_probs[^7^]`
      - 指定解码 Segment-ID 时要使用的概率值。更多细节参vp9 规范.15 Segmentation map
        （分段映射）一节
    - - __u8
      - `pred_probs[^3^]`
      - 指定解码 Predicted-Segment-ID 时要使用的概率值。更多细节参vp9 规范
        .4.14 Get segment id syntax”（获取ID 语法）一节
    - - __u8
      - `flags`
      - V4L2_VP9_SEGMENTATION_FLAG_* 标志的组合。参见分段标<vp9_segmentation_flags>
    - - __u8
      - `reserved[^5^]`
      - 应用程序与驱动必须将本字段置为零
...
编码量化参数。更多细节参VP9 规范 .2.9 Quantization params syntax”（量化参数语法）一节
    ...
    - - __s8
      - `delta_q_y_dc`
      - 表示相对 base_q_idx Y DC 量化器
    - - __s8
      - `delta_q_uv_dc`
      - 表示相对 base_q_idx UV DC 量化器
    - - __s8
      - `delta_q_uv_ac`
      - 表示相对 base_q_idx UV AC 量化器
    - - __u8
      - `reserved[^4^]`
      - 应用程序与驱动必须将本字段置为零


该结构体包含全部与环路滤波器相关的参数。更多细节参vp9 规范 .2.8 Loop filter semantics
（环路滤波器语义）一节
...
`V4L2_CID_STATELESS_HEVC_SPS (struct)`
    指定与相HEVC 切片数据关联的序列参数集字段（从码流中提取）。这些码流参数依
    hevc 标准定义，并在规范的 .4.3.2 Sequence parameter set RBSP semantics
    （序列参数集 RBSP 语义）一节中描述
    ...
    - - __u8
      ...
      - 指定活动 VPS vps_video_parameter_set_id 值，H.265 规范
        .4.3.2.1 General sequence parameter set RBSP semantics”（通用序列参数RBSP 语义
        一节所述
    - - __u8
      - `seq_parameter_set_id`
      - SPS 提供一个标识符，供其他语法元素引用，如 H.265 规范
        .4.3.2.1 General sequence parameter set RBSP semantics”（通用序列参数RBSP 语义
        一节所述
    - - __u16
    ...
    - - __u8
      ...
      - 用于发信号传MaxLatencyPictures，表示在输出顺序上可以位于任意图像之前、并在解
        顺序上跟随该图像的最大图像数量
    - - __u8
    ...
    - - __u8
      ...
      - 该值加 1 指定用于表示亮度分量的每PCM 样本值的比特数
    - - __u8
      - `pcm_sample_bit_depth_chroma_minus1`
      - 指定用于表示色度分量的每PCM 样本值的比特数
    - - __u8
...
`V4L2_CID_STATELESS_HEVC_PPS (struct)`
    指定与相HEVC 切片数据关联的图像参数集字段（从码流中提取）。这些码流参数依
    hevc 标准定义，并在规范的 .4.3.3 Picture parameter set RBSP semantics
    （图像参数集 RBSP 语义）一节中描述
    ...
    - - __u8
      - `pic_parameter_set_id`
      - PPS 提供一个标识符，供其他语法元素引用
    - - __u8
      - `num_extra_slice_header_bits`
      - 指定引用PPS 的编码图像的切片RBSP 中存在的额外切片头比特数量
    - - __u8
      - `num_ref_idx_l0_default_active_minus1`
    ...
    - - __s8
      - `init_qp_minus26`
      - 该值加 26 指定引用PPS 的每个切片的 SliceQp Y 初始值
    - - __u8
      - `diff_cu_qp_delta_depth`
      - 指定亮度编码树块大小与传cu_qp_delta_abs cu_qp_delta_sign_flag 的编码单元的
        最小亮度编码块大小之间的差值
    - - __s8
      - `pps_cb_qp_offset`
      - 指定对亮度量化参Cb 的偏移
    ...
    - - __u8
      ...
      - 该值加 1 指定将图像划分成tile 列数
    - - __u8
      - `num_tile_rows_minus1`
      - 该值加 1 指定将图像划分成tile 行数
    - - __u8
      - `column_width_minus1[^20^]`
      - 该值加 1 指定i tile 列的宽度，以编码树块为单位
    - - __u8
      - `row_height_minus1[^22^]`
      - 该值加 1 指定i tile 行的高度，以编码树块为单位
    - - __s8
...
`图像参数集标志`
    ...
    - - `V4L2_HEVC_PPS_FLAG_UNIFORM_SPACING`
      - 0x00100000
      - 指定 tile 列边界以tile 行边界在图像上均匀分布


    \normalsize

`V4L2_CID_STATELESS_HEVC_SLICE_PARAMS (struct)`
    指定各种切片特定参数，特别是来自 NAL 单元头、通用切片段头以及码流中加权预测参数部
    的参数。这些码流参数依hevc 标准定义，并在规范的 .4.7 General slice segment header
    semantics”（通用切片段头语义）一节中描述。该控件为动态大小的 1 维数组，使用时必须设
    ...
    - - __u8
      - `slice_type`
      -
	（V4L2_HEVC_SLICE_TYPE_I、V4L2_HEVC_SLICE_TYPE_P 
    ...
    - - __u8
      - `num_ref_idx_l0_active_minus1`
      - 该值加 1 指定可用于解码该切片的参考图像列0 的最大参考索引
    - - __u8
      - `num_ref_idx_l1_active_minus1`
      - 该值加 1 指定可用于解码该切片的参考图像列1 的最大参考索引
    - - __u8
      - `collocated_ref_idx`
    ...
    - - __s8
      - `slice_cb_qp_offset`
      - 指定要加pps_cb_qp_offset 值上的差值
    - - __s8
      - `slice_cr_qp_offset`
      - 指定要加pps_cr_qp_offset 值上的差值
    - - __s8
      - `slice_act_y_qp_offset`
      - 指定8.6.2 节推导出的量化参qP 的亮度偏移
    - - __s8
      - `slice_act_cb_qp_offset`
      - 指定8.6.2 节推导出的量化参qP cb 偏移
    - - __s8
      - `slice_act_cr_qp_offset`
      - 指定8.6.2 节推导出的量化参qP cr 偏移
    - - __s8
      - `slice_beta_offset_div2`
      - 指定 beta 的去块参数偏移除2
    - - __s8
    ...
    - - __u8
      ...
      - L0 参考元素列表，DPB 中的索引表示
    - - __u8
      - `ref_idx_l1[V4L2_HEVC_DPB_ENTRIES_NUM_MAX]`
      - L1 参考元素列表，DPB 中的索引表示
    - - __u16
      - `short_term_ref_pic_set_size`
      - 指定短期参考图像集的大小（比特），在规范中描述st_ref_pic_set()，包含在切片头或
        SPS 中（7.3.6.1 节）
    - - __u16
      - `long_term_ref_pic_set_size`
      - 指定长期参考图像集的大小（比特），包含在切片头SPS 中。即规范7.3.6.1 节中
        条件if(long_term_ref_pics_present_flag) 内的比特数
    - - __u8
      - `padding`
      - 应用程序与驱动必须将本字段置为零
...
`V4L2_CID_STATELESS_HEVC_ENTRY_POINT_OFFSETS (integer)`
    指定入口点偏移（字节）。该控件为动态大小数组，入口点偏移的数量`elems` 字段报告
    该码流参数依hevc 标准定义，并在规范的 .4.7.1 General slice segment header
    semantics”（通用切片段头语义）一节中描述。当一个请求中提交多个切片时，该数组的长度
    必须为请求中所有切片的 num_entry_point_offsets 之和

`V4L2_CID_STATELESS_HEVC_SCALING_MATRIX (struct)`
    指定用于变换系数缩放过程HEVC 缩放矩阵参数。这些矩阵与参数依据 hevc 标准定义，并
    规范.4.5 Scaling list data semantics”（缩放列表数据语义）一节中描述
    ...
    - - __u8
      ...
      - 参考帧的长期标志（V4L2_HEVC_DPB_ENTRY_LONG_TERM_REFERENCE）。该标志的设置如 ITU HEVC
        规范 .3.2 Decoding process for reference picture set”（参考图像集解码过程）一章所述
    - - __u8
      - `field_pic`
      - 该参考是场图像还是帧图像。参HEVC dpb 场图像标<hevc_dpb_field_pic_flags>
    - - __s32
      - `pic_order_cnt_val`
      - 当前图像的图像顺序计数
    - - __u8
...
`V4L2_CID_STATELESS_HEVC_DECODE_MODE (enum)`
    指定要使用的解码模式。目前提供基于切片和基于帧的解码，但后续可能会新增其他模式
    该控件用V4L2_PIX_FMT_HEVC_SLICE 像素格式的修饰符。支V4L2_PIX_FMT_HEVC_SLICE
...
`V4L2_CID_STATELESS_HEVC_START_CODE (enum)`
    指定每个 HEVC 切片所期望的切片起始码。该控件用作 V4L2_PIX_FMT_HEVC_SLICE 像素格式
    修饰符。支V4L2_PIX_FMT_HEVC_SLICE 的应用程序必须设置该控件，以指定缓冲区所期望
    起始码。驱动可能根据其所支持的能力，暴露单个或多个起始码
    ...
    - - `V4L2_STATELESS_HEVC_START_CODE_NONE`
      - 0
      - 选择该值表HEVC 切片不带任何起始码地传递给驱动。码流数据应遵循 hevc 7.3.1.1
        General NAL unit syntax（通用 NAL 单元语法），因此在需要时会包含仿真预防字节
    - - `V4L2_STATELESS_HEVC_START_CODE_ANNEX_B`
      - 1
      - 选择该值表示期HEVC 切片Annex B 起始码作为前缀。依hevc，有效的起始码可以是
        3 字节0x000001 4 字节0x00000001
...
`V4L2_CID_MPEG_VIDEO_BASELAYER_PRIORITY_ID (integer)`
    NAL 单元指定一个优先级标识符，将应用于基础层（base layer）。默认情况下，基础层该
    设为 0，下一层将被分配优先级 ID 1 等等。视频编码器无法决定要应用于某层
    优先ID，因此必须由客户端提供。这适用H264，有效范围为 0 63
    来源：Rec. ITU-T H.264 (06/2019)；G.7.4.1.1、G.8.8.1

`V4L2_CID_MPEG_VIDEO_LTR_COUNT (integer)`
    指定编码器在任何给定时刻可以保留的长期参考（LTR）帧的最大数量。这适用H264 HEVC
    编码器

`V4L2_CID_MPEG_VIDEO_FRAME_LTR_INDEX (integer)`
    设置该控件后，接下来将排队的帧将被标记为长期参考（LTR）帧，并获得LTR 索引，索引范
    0 LTR_COUNT-1。这适用H264 HEVC 编码器。来源：Rec. ITU-T H.264 (06/2019)
    琛?7.9銆。

`V4L2_CID_MPEG_VIDEO_USE_LTR_FRAMES (bitmask)`
    指定用于编码设置该控件后下一个排队帧的长期参考（LTR）帧。这提供一个位掩码，由比特
    [0, LTR_COUNT-1] 组成。这适用H264 HEVC 编码器

`V4L2_CID_STATELESS_HEVC_DECODE_PARAMS (struct)`
    指定各种解码参数，特别是所有列表（短期、长期、之前、当前、之后）的参考图像顺序计
    （POC）以及每个列表的条目数。这些参数依hevc 标准定义，并在规范的 .3 Slice decoding
    ...
    - - __s32
      - `pic_order_cnt_val`
      - PicOrderCntVal，如规范 .3.1 Decoding process for picture order count
        （图像顺序计数解码过程）一节所述
    - - __u16
      - `short_term_ref_pic_set_size`
      - 指定第一个切片的短期参考图像集的大小（比特），该集合在规范中描述为
        st_ref_pic_set()，包含在切片头或 SPS 中（7.3.6.1 节）
    - - __u16
      - `long_term_ref_pic_set_size`
      - 指定第一个切片中包含的长期参考图像集的大小（比特），包含在切片头SPS 中
        即规范第 7.3.6.1 节中条件if(long_term_ref_pics_present_flag) 内的比特数
    - - __u8
    ...
    - - __u8
      ...
      - PocStCurrBefore，如规范 .3.2 Decoding process for reference picture set
        （参考图像集解码过程）一节所述：提供 DPB 数组中当前帧之前的短期参考的索引
    - - __u8
      - `poc_st_curr_after[V4L2_HEVC_DPB_ENTRIES_NUM_MAX]`
      - PocStCurrAfter，如规范 .3.2 Decoding process for reference picture set
        一节所述：提供 DPB 数组中当前帧之后的短期参考的索引
    - - __u8
      - `poc_lt_curr[V4L2_HEVC_DPB_ENTRIES_NUM_MAX]`
      - PocLtCurr，如规范 .3.2 Decoding process for reference picture set
        一节所述：提供 DPB 数组中长期参考的索引
    - - __u8
      - `num_delta_pocs_of_ref_rps_idx`
      - 当切片头short_term_ref_pic_set_sps_flag 等于 0 时，其值与推导
        NumDeltaPocs[RefRpsIdx] 相同。它可用于解析切片头中的 RPS 数据，而非使用
        @short_term_ref_pic_set_size 跳过它。当切片头中 short_term_ref_pic_set_sps_flag
        的值等1 时，num_delta_pocs_of_ref_rps_idx 应设0
...
`V4L2_CID_STATELESS_HEVC_EXT_SPS_LT_RPS (struct)`
    `v4l2_ctrl_hevc_sps` 控件的子集。它以长期参考集参数列表对其进行扩展。这些参数依
    hevc 标准定义，并在规范的 .4.3.2.1 General sequence parameter set RBSP semantics
    （通用序列参数RBSP 语义）一节中描述。该控件为动态大小的 1 维数组。当
    num_long_term_ref_pics_sps 0，或 `v4l2_ctrl_hevc_sps` 中未设置
    ...
    - - __u16
      ...
      - 长期参考图像顺序计数，如规.4.3.2.1 General sequence parameter set RBSP semantics
        （通用序列参数RBSP 语义）一节所述
    - - __u16
      - `flags`
...
`扩展 SPS 长期 RPS 标志`
    ...
    - - `V4L2_HEVC_EXT_SPS_LT_RPS_FLAG_USED_LT`
      ...
      - 指定长期参考图像是否被使用，见规范 7.4.3.2.1 “General sequence parameter set RBSP
        semantics”（通用序列参数RBSP 语义）一节


`V4L2_CID_STATELESS_HEVC_EXT_SPS_ST_RPS (struct)`
    `v4l2_ctrl_hevc_sps` 控件的子集。它以短期参考集参数列表对其进行扩展。这些参数依
    hevc 标准定义，并在规范的 .4.8 Short-term reference picture set semantics
    ...
    - - __u8
      - `delta_idx_minus1`
      - 指定与索引比较的 delta 值。详见规.4.8 Short-term reference picture set semantics
        （短期参考图像集语义）一节
    - - __u8
      - `delta_rps_sign`
      - delta 的符号，如规.4.8 Short-term reference picture set semantics一节所述
    - - __u8
      - `num_negative_pics`
    ...
    - - __u32
      - `used_by_curr_pic`
      - i 位指定短RPS i 是否被当前图像使用
    - - __u32
      - `use_delta_flag`
      - i 位指定短RPS i 是否被包含在短期 RPS 条目中
    - - __u16
      - `abs_delta_rps_minus1`
      - 绝对 delta RPS，如规范 .4.8 Short-term reference picture set semantics一节所述
    - - __u16
      - `delta_poc_s0_minus1[^16^]`
      - 指定短期 RPS 中第 i 个条目的负图像顺序计delta。详见规.4.8 Short-term
        reference picture set semantics一节
    - - __u16
      - `delta_poc_s1_minus1[^16^]`
      - 指定短期 RPS 中第 i 个条目的正图像顺序计delta。详见规.4.8 Short-term
        reference picture set semantics一节
    - - __u16
      - `flags`
...
`扩展 SPS 短期 RPS 标志`
    ...
    - - `V4L2_HEVC_EXT_SPS_ST_RPS_FLAG_INTER_REF_PIC_SET_PRED`
      - 0x00000001
      - 指定短期 RPS 是否从另一个短RPS 预测得到。详见规.4.8 Short-term reference
        picture set semantics一节


`V4L2_CID_STATELESS_AV1_SEQUENCE (struct)`
    表示一AV1 序列 OBU（Open Bitstream Unit，开放码流单元）。更多细节参av1 规范
    5.5 “Sequence header OBU syntax”（序列OBU 语法）
    ...
    - - __u8
      - `order_hint_bits`
      - 指定每帧order_hint 字段所用的比特数
    - - __u8
      - `bit_depth`
      - 用于该序列的位深，更多细节见 av1 规范5.5.2 “Color config syntax
        （颜色配置语法）
    - - __u8
      - `reserved`
      - 应用程序与驱动必须将本字段置为零
...
`AV1 序列标志`
    ...
    - - `V4L2_AV1_SEQUENCE_FLAG_USE_128X128_SUPERBLOCK`
      ...
      - 若设置，表示 superblock 包含 128x128 亮度样本。为 0 时，表示 superblock 包含
        64x64 亮度样本。所包含的色度样本数量取决于 subsampling_x subsampling_y
    ...
    - - `V4L2_AV1_SEQUENCE_FLAG_ENABLE_INTERINTRA_COMPOUND`
      - 0x00000010
      - 若设置，指定帧间块的 mode info 可包interintra 语法元素。若未设置，指定 interintra
        语法元素不会出现
    - - `V4L2_AV1_SEQUENCE_FLAG_ENABLE_MASKED_COMPOUND`
      - 0x00000020
      - 若设置，指定帧间块的 mode info 可包compound_type 语法元素。若未设置，指定
    ...
    - - `V4L2_AV1_SEQUENCE_FLAG_ENABLE_DUAL_FILTER`
      - 0x00000080
      - 若设置，表示帧间预测滤波器类型可在水平和垂直方向上独立指定。若标志0，则只能指定
        一种滤波器类型，并在两个方向均使用
    - - `V4L2_AV1_SEQUENCE_FLAG_ENABLE_ORDER_HINT`
      - 0x00000100
      - 若设置，表示可基order hint 值使用相应工具。若未设置，表示基于 order hint 的工
        被禁用
    ...
    - - `V4L2_AV1_SEQUENCE_FLAG_ENABLE_SUPERRES`
      ...
      - 若设置，指定未压缩头中将出现 use_superres 语法元素。若未设置，指定 use_superres
        语法元素不会出现（而是在未压缩头中直接use_superres 设为 0，无需读取）
    - - `V4L2_AV1_SEQUENCE_FLAG_ENABLE_CDEF`
      - 0x00001000
      - 若设置，指定可启cdef 滤波。若未设置，指定 cdef 滤波被禁用
    - - `V4L2_AV1_SEQUENCE_FLAG_ENABLE_RESTORATION`
      - 0x00002000
      - 若设置，指定可启用环路恢复滤波。若未设置，指定环路恢复滤波被禁用
    - - `V4L2_AV1_SEQUENCE_FLAG_MONO_CHROME`
      - 0x00004000
      - 若设置，表示视频不包U V 颜色平面。若未设置，表示视频包含 Y、U V 颜色平面
    ...
    - - `V4L2_AV1_SEQUENCE_FLAG_SEPARATE_UV_DELTA_Q`
      - 0x00080000
      - 若设置，表示 U V 平面可具有独立的 delta 量化器值。若未设置，表示 U V 平面
        共享相同delta 量化器值


`V4L2_CID_STATELESS_AV1_TILE_GROUP_ENTRY (struct)`
    表示 AV1 Tile Group 内的单个 AV1 tile。注MiRowStart、MiRowEnd、MiColStart MiColEnd
    可通过使用 tile_row tile_col，从 struct v4l2_ctrl_av1_frame 中的 struct
    v4l2_av1_tile_info 获取。更多细节参av1 规范6.10.1 “General tile group OBU
	...
	AV1 扭曲模型（Warp Model），av1 规范3 “Symbols and abbreviated terms
	（符号与缩写术语）所述
...
AV1 参考帧，如 av1 规范6.10.24 “Ref frames semantics”（参考帧语义）所述
...
AV1 全局运动参数，如 av1 规范6.8.17 “Global motion params semantics”（全局运动参数语义）所述
    ...
    - - __u8
      ...
      - 包含每个参考帧标志的位域。更多细节参AV1 全局运动标志 <av1_global_motion_flags>
    - - enum `v4l2_av1_warp_model`
      - `type[V4L2_AV1_TOTAL_REFS_PER_FRAME]`
      - 所使用的全局运动变换类型
    - - __s32
      - `params[V4L2_AV1_TOTAL_REFS_PER_FRAME][^6^]`
      - 本字段与 av1 中的 “gm_params含义相同
    - - __u8
      - `invalid`
      - 位域，指示某个给定参考帧的全局运动参数是否无效。参见第 7.11.3.6 Setup shear 过程
        以及变量 “warpValid”。使V4L2_AV1_GLOBAL_MOTION_IS_INVALID(ref) 来构造合适的掩码
    - - __u8
      - `reserved[^3^]`
      - 应用程序与驱动必须将本字段置为零
...
AV1 环路恢复，如 av1 规范6.10.15 “Loop restoration params semantics”（环路恢复参数语义）所述
...
`AV1 环路恢复标志`
    ...
    - - `V4L2_AV1_LOOP_RESTORATION_FLAG_USES_CHROMA_LR`
      - 0x00000002
      - av1 中的 UsesChromaLr 含义相同


AV1 CDEF 参数语义，如 av1 规范6.10.14 “CDEF params semantics”（CDEF 参数语义）所述
...
AV1 分段特征，如 av1 规范3 “Symbols and abbreviated terms”（符号与缩写术语）所述
...
AV1 分段参数，定义于 av1 规范6.8.13 “Segmentation params semantics”（分段参数语义）
    ...
    - - __u8
      - `last_active_seg_id`
      - 指示具有某个已启用特征的最高编号分id。这在解码分id 时用于仅解码与所用分
        对应的选项
    - - __u8
      - `feature_enabled[V4L2_AV1_MAX_SEGMENTS]`
      - 位掩码，定义每个分段中启用了哪些特征。使V4L2_AV1_SEGMENT_FEATURE_ENABLED 
        构造合适的掩码
    - - __u16
...
AV1 环路滤波器参数，定义av1 规范6.8.10 “Loop filter semantics”（环路滤波器语义）
    ...
    - - __u8
      - `flags`
      - 更多细节参见 AV1 环路滤波器标<av1_loop_filter_flags>
    - - __u8
      - `level[^4^]`
    ...
    - - __u8
      - `sharpness`
      - 表示锐度等级。loop_filter_level loop_filter_sharpness 共同决定何时对块边缘进行
        滤波，以及滤波对样本值的改变量。环路滤波过程在 av1 规范7.14 节中描述
    - - __u8
...
`AV1 环路滤波器标志`
    ...
    - - `V4L2_AV1_LOOP_FILTER_FLAG_DELTA_ENABLED`
      - 0x00000001
      - 若设置，表示滤波器等级取决于用于预测某个块的模式和参考帧。若未设置，表示滤波器等
        不取决于模式和参考帧
    - - `V4L2_AV1_LOOP_FILTER_FLAG_DELTA_UPDATE`
      - 0x00000002
      - 若设置，表示存在额外的语法元素，用于指定哪些模式和参考帧delta 需要更新。若未设置，
        表示这些语法元素不存在
    ...
    - - `V4L2_AV1_LOOP_FILTER_FLAG_DELTA_LF_MULTI`
      - 0x00000008
      - 值为 1 指定为水平亮度边缘、垂直亮度边缘、U 边缘V 边缘分别发送独立的环路滤波
        delta。delta_lf_multi 值为 0 指定所有边缘使用相同的环路滤波delta


AV1 量化参数，定义于 av1 规范6.8.11 “Quantization params semantics”（量化参数语义）
    ...
    - - __u8
      - `flags`
      - 更多细节参见 AV1 环路滤波器标<av1_quantization_flags>
    - - __u8
      - `base_q_idx`
      - 指示基础qindex。用Y AC 系数，并作为其他量化器的基础值
    - - __u8
      - `delta_q_y_dc`
      - 指示相对 base_q_idx Y DC 量化器
    - - __u8
      - `delta_q_u_dc`
      - 指示相对 base_q_idx U DC 量化器
    - - __u8
      - `delta_q_u_ac`
      - 指示相对 base_q_idx U AC 量化器
    - - __u8
      - `delta_q_v_dc`
      - 指示相对 base_q_idx V DC 量化器
    - - __u8
      - `delta_q_v_ac`
      - 指示相对 base_q_idx V AC 量化器
    - - __u8
      - `qm_y`
      - 指定用于亮度平面解码的量化矩阵等级
    - - __u8
...
`AV1 量化标志`
    ...
    - - `V4L2_AV1_QUANTIZATION_FLAG_DIFF_UV_DELTA`
      - 0x00000001
      - 若设置，表示 U V delta 量化器值被分别编码。若未设置，表示 U V delta 量化
        值共享一个公共值
...
AV1 Tile 信息，定义于 ref:`av1` 规范6.8.14 “Tile info semantics”（Tile 信息语义）
    ...
    - - __u32
      ...
      - 一个数组，指定每个 tile 跨图像的起始列（4x4 亮度样本为单位）
    - - __u32
      - `mi_row_starts[V4L2_AV1_MAX_TILE_ROWS + 1]`
      - 一个数组，指定每个 tile 跨图像的起始行（4x4 亮度样本为单位）
    - - __u32
...
`AV1 Tile 信息标志`
    ...
    - - `V4L2_AV1_TILE_INFO_FLAG_UNIFORM_TILE_SPACING`
      - 0x00000001
      - 若设置，表示 tile 在帧上均匀分布（换言之，除右侧和底部边缘可能较小tile 外，所
...
AV1 Tx 模式，如 av1 规范6.8.21 “TX mode semantics”（TX 模式语义）所述
...
`V4L2_CID_STATELESS_AV1_FRAME (struct)`
    表示一个帧OBU。更多细节参av1 规范6.8 “Frame Header OBU semantics
    （帧OBU 语义）
    ...
    - - __u8
      - `skip_mode_frame[^2^]`
      - 指定skip_mode 等于 1 时用于复合预测的帧
    - - __u8
      - `primary_ref_frame`
      - 指定哪个参考帧包含CDF 值以及应在帧开始时加载的其他状态
    ...
    - - __u32
      - `flags`
      - 更多细节参见 AV1 帧标<av1_frame_flags>
    - - enum `v4l2_av1_frame_type`
      - `frame_type`
      - 指定 AV1 帧类型
    - - __u32
      - `order_hint`
      - 指定本帧期望输出顺序OrderHintBits 个最低有效位
    - - __u32
      - `upscaled_width`
      - 上采样宽度
    ...
    - - __u32
      - `current_frame_id`
      - 指定当前帧的id 编号。帧 id 编号是不影响解码过程的附加信息，但为解码器提供了检
        缺失参考帧的方式，以便采取适当措施
    - - __u8
      - `buffer_removal_time[V4L2_AV1_MAX_OPERATING_POINTS]`
      - 指定对于操作opNum，从最后一个随机访问点的移除时间起算、以 DecCT 时钟节拍为单位的
        帧移除时间
    - - __u8
      - `reserved[^4^]`
      - 应用程序与驱动必须将本字段置为零
    - - __u32
      ...
      - 指定每个参考帧的期望输出顺序提示。本字段对应于规范（5.9.2 “Uncompressed header
        syntax”，未压缩头部语法）中的 OrderHints 变量。因此，它仅用于非帧内帧，否则被忽略
        order_hints[^0^] 始终被忽略
    - - __u64
      - `reference_frame_ts[V4L2_AV1_TOTAL_REFS_PER_FRAME]`
      - enum `v4l2_av1_reference_frame` `V4L2_AV1_REF_LAST_FRAME` 开始枚举的每个参考帧
        V4L2 时间戳。这表示规范中描述的参考槽状态，并由用户空间通过7.20 节的
        “Reference frame update process”（参考帧更新过程）更新。该时间戳引struct
        `v4l2_buffer` 中的 `timestamp` 字段。使`v4l2_timeval_to_ns()` 函数struct
        `timeval` struct `v4l2_buffer` 中转换为 __u64
    - - __s8
      - `ref_frame_idx[V4L2_AV1_REFS_PER_FRAME]`
      - 指向 `reference_frame_ts` 的索引，表示帧间帧使用的参考有序列表。与同名的码流语法元
        匹配
    - - __u8
      - `refresh_frame_flags`
...
`AV1 帧标志`
    ...
    - - `V4L2_AV1_FRAME_FLAG_SHOWABLE_FRAME`
      - 0x00000002
      - 若设置，指定本帧可使show_existing_frame 机制输出。若未设置，指定本帧不会使用
    ...
    - - `V4L2_AV1_FRAME_FLAG_ALLOW_INTRABC`
      - 0x00000040
      - 若设置，表示本帧中可使用帧内块复制（intra block copy）。若未设置，表示本帧不允
        帧内块复制
    ...
    - - `V4L2_AV1_FRAME_FLAG_REFERENCE_SELECT`
      - 0x00002000
      - 若设置，指定帧间块的 mode info 包含 comp_mode 语法元素，指示使用单参考还是复合参
    ...
    - - `V4L2_AV1_FRAME_FLAG_FRAME_SIZE_OVERRIDE`
      - 0x00020000
      - 若设置，指定帧大小将指定为某个参考帧的大小，或从 frame_width_minus_1 
...
`V4L2_CID_STATELESS_AV1_FILM_GRAIN (struct)`
    表示可选的胶片颗粒参数。更多细节参av1 规范6.8.20 “Film grain params semantics
    （胶片颗粒参数语义）
    ...
    - - __u8
      ...
      - 表示亮度分量分段线性缩放函数第 i 个点x（亮度值）坐标。这些值在 0..255 的范围上发出
        信号。对10 位视频，这些值对应于除以 4 的亮度值；对于 12 位视频，对应于除16 
        亮度值
    - - __u8
    ...
    - - __u8
      ...
      - 表示 cb 分量分段线性缩放函数第 i 个点x 坐标。这些值在 0..255 的范围上发出信号
    - - __u8
    ...
    - - __u8
      ...
      - 表示 cr 分量分段线性缩放函数第 i 个点x 坐标。这些值在 0..255 的范围上发出信号
    - - __u8
      - `point_cr_scaling[V4L2_AV1_MAX_NUM_CR_POINTS]`
      - 表示 cr 分量分段线性缩放函数第 i 个点的缩放（输出）值
    - - __u8
      - `grain_scaling_minus_8`
      - 表示应用于色度分量值的移位8。grain_scaling_minus_8 可取 0..3 的值，并决定胶片颗
        标准差的范围与量化步长
    - - __u8
      - `ar_coeff_lag`
      - 指定亮度与色度的自回归系数数量
    ...
    - - __u8
      ...
      - 指定自回归系数的范围。0 分别对应自回归系数的范围 [-2, 2)、[-1, 1)
...
`AV1 胶片颗粒标志`
    ...
    - - `V4L2_AV1_FILM_GRAIN_FLAG_CLIP_TO_RESTRICTED_RANGE`
      ...
      - 若设置，表示在对样本值添加胶片颗粒后，应将其裁剪到受限（演播室，即限制）范围
        （参color_range 语义中关studio swing 的解释）。若未设置，表示在对样本值添
        胶片颗粒后，应将其裁剪到全范围
