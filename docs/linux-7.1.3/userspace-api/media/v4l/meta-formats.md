######## 元数据格式

本页是 Video4Linux 元数据（metadata）格式文档的索引，汇总了各类元数据格式的参考条目与各子文档入口，便于读者按格式主题快速定位对应的像素、ISP 与相机元数据接口说明。



这些格式仅用于元数据接口。


- [metafmt-arm-mali-c55](metafmt-arm-mali-c55)
- [metafmt-c3-isp](metafmt-c3-isp)
- [metafmt-d4xx](metafmt-d4xx)
- [metafmt-generic](metafmt-generic)
- [metafmt-intel-ipu3](metafmt-intel-ipu3)
- [metafmt-pisp-be](metafmt-pisp-be)
- [metafmt-pisp-fe](metafmt-pisp-fe)
- [metafmt-rkisp1](metafmt-rkisp1)
- [metafmt-uvc](metafmt-uvc)
- [metafmt-uvc-msxu-1-5](metafmt-uvc-msxu-1-5)
- [metafmt-vivid](metafmt-vivid)
- [metafmt-vsp1-hgo](metafmt-vsp1-hgo)
- [metafmt-vsp1-hgt](metafmt-vsp1-hgt)
- [v4l2-isp](v4l2-isp)
