...
使用该 ioctl，应用程序可以获知此 CEC 设备对应于哪个 HDMI 连接器。调用此 ioctl 时，
应用程序应提供一个指向 cec_connector_info 结构体的指针，内核将用适配器驱动提供的信息
填充该结构体。仅当设置了 `CEC_CAP_CONNECTOR_INFO` 能力时，此 ioctl 才可用。
    ...
    - .. _`CEC-CONNECTOR-TYPE-NO-CONNECTOR`:
      ...
      - 0
      - 没有与该适配器关联的连接器/驱动未提供该信息。
    - .. _`CEC-CONNECTOR-TYPE-DRM`:

      - `CEC_CONNECTOR_TYPE_DRM`
      - 1
      - 表示有一个 DRM 连接器与该适配器关联。有关该连接器的信息可在
    ...
    - .. _`CEC-DRM-CONNECTOR-TYPE-CARD-NO`:
      ...
      - __u32
      - `card_no`
      - DRM 卡编号：来自卡路径的编号，例如 /dev/card0 中的 0。
