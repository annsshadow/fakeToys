...
该驱动用于控制 Nuvoton NPCM SoC 上存在的视频捕获/差分（VCD）引擎与编码压缩（ECE）引擎。VCD 可以从数字视频输入捕获一帧，并在内存中比较两帧；ECE 可以将帧数据压缩为 HEXTILE 格式。
...
应用程序可以使用 `V4L2_CID_NPCM_CAPTURE_MODE` 控制，通过不同的控制值（enum v4l2_npcm_capture_mode）设置 VCD 模式：
...
如果使用 V4L2_PIX_FMT_HEXTILE 格式，VCD 将捕获帧数据，然后 ECE 将数据压缩为 HEXTILE 矩形，并按照远程帧缓冲协议（Remote Framebuffer Protocol）中定义的布局存储到 V4L2 视频缓冲区中：
```
