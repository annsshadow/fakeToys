...
本页Linux 内核用户空间 API 指南的索引，汇总内核代码树中记录的用户空间相关接口文档入口，按系统调用、安全接口、设备与 I/O 等主题分类，便于按主题快速定位



虽然内核的大部分用户空间 API 记录在别处（尤其是在 man-pages_ 项目中），但
内核代码树本身中也能找到一些用户空间相关信息。本手册意在成为汇集这些信息
地方
