## 功耗封顶（Power Capping）框

功耗封顶框架在内核与用户空间之间提供了一致的接口，使得功耗封顶驱动能够以统一的方式将相关设置暴露给用户空间
## 术语


该框架通过 sysfs 以对象树的形式将功耗封顶设备暴露给用户空间。树根层级的对象代表“控制类型（control types）”，对应于不同的功耗封顶方法。例如，intel-rapl 控制类型代表 Intel 的“运行平均功率限制”（Running Average Power Limit，RAPL）技术，idle-injection 控制类型则对应于使用空闲注入（idle injection）来控制功耗
...
上述示例展示了使Intel® IA-64 IA-32 处理器架构中可用Intel RAPL 技术的情况。其中有一个名intel-rapl 的控制类型，它包含两个功耗区 intel-rapl:0 intel-rapl:1，代CPU 封装（package）。每个功耗区又包含两个子intel-rapl:j:0 intel-rapl:j:1（j = 0, 1），分别代表CPU 封装的“核心（core）”与“非核心（uncore）”部分。所有的区与子区都包含能耗监控属性（energy_uj、max_energy_range_uj）以及约束属性（constraint_*），用以施加控制（“封装（package）”功耗区中的约束作用于整CPU 封装，而子区约束只分别作用于该封装各自的部分）。由Intel RAPL 不提供瞬时功率值，因此没有 power_uw 属性
此外，每个功耗区还包含一name 属性，用于标识该区所代表的系统部分
```
...
根据功耗区的不同，Intel RAPL 技术允许对各个功耗区施加一个或多个约束，例如短期、长期以及峰值功率，并带有不同的时间窗口所有的区都包含代表约束名称、功率限制以及时间窗口大小的属性。注意，时间窗口不适用于峰值功率。这里的 constraint_j_* 属性对应于j 个约束（j = 0,1,2）
```
	constraint_0_name
...
energy_uj (rw)
	当前能耗计数器，单位为微焦（micro joules）。写以重置	如果计数器无法重置，则该属性为只读
...
constraint_X_power_limit_uw (rw)
	功率限制，单位为微瓦，应适用于由 “constraint_X_time_window_us指定	时间窗口
...
constraint_X_min_time_window_us (ro)
	允许的最小时间窗口，单位为微秒
power_limit_uw time_window_us 外，其余字段均为可选
### 通用区与控制类型属
...
API 概要
调用 powercap_register_control_type() 注册控制类型对象调用 powercap_register_zone() 注册一个功耗区（在某个给定的控制类型下），
既可以作为顶层功耗区，也可以作为先前注册的另一个功耗区的子区在调powercap_register_zone() 注册某个功耗区之前，必须先定义该区约束的数量以及相应的回调函数
要释放一个功耗区，调powercap_unregister_zone()要释放一个控制类型对象，调用 powercap_unregister_control_type()详细API 可以通过include/linux/powercap.h 使用 kernel-doc 生成