...
导出Linux 内核头文件描述了面向试图使用内核服务的用户空间程序的 API。这些内头文件被系统C 库（例如 glibc uClibc）用来定义可用的系统调用，以及与这些
系统调用一起使用的常量和结构体。C 库的头文件包含来"linux" 子目录的内核头文件系统libc 头文件通常安装在默认位/usr/include，内核头文件则位于其下的子目中（最值得注意的是 /usr/include/linux /usr/include/asm）
...
会在 INSTALL_HDR_PATH 内部自动创建一'include' 目录，头文件被安装在
'INSTALL_HDR_PATH/include' 中
内核头文件导出基础设施David Woodhouse <dwmw2@infradead.org> 维护