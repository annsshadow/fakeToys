...
```
    ...
    viafb_lcd_mode:
	0 : LSB 数据格式输入LCD 面板（默认）
	1 : MSB 数据格式输入LCD 面板

    viafb_lcd_panel_id:
	- 0 : 分辨 640x480, 通道:  抖动: 启用
	- 1 : 分辨 800x600, 通道:  抖动: 启用
	- 2 : 分辨 1024x768, 通道:  抖动: 启用（默认）
	- 3 : 分辨 1280x768, 通道:  抖动: 启用
	- 4 : 分辨 1280x1024, 通道:  抖动: 启用
	- 5 : 分辨 1400x1050, 通道:  抖动: 启用
	- 6 : 分辨 1600x1200, 通道:  抖动: 启用

	- 8 : 分辨 800x480, 通道:  抖动: 启用
	- 9 : 分辨 1024x768, 通道:  抖动: 启用
	- 10: 分辨 1024x768, 通道:  抖动: 禁用
	- 11: 分辨 1024x768, 通道:  抖动: 禁用
	- 12: 分辨 1280x768, 通道:  抖动: 禁用
	- 13: 分辨 1280x1024, 通道:  抖动: 禁用
	- 14: 分辨 1400x1050, 通道:  抖动: 禁用
	- 15: 分辨 1600x1200, 通道:  抖动: 禁用
	- 16: 分辨 1366x768, 通道:  抖动: 禁用
	- 17: 分辨 1024x600, 通道:  抖动: 启用
	- 18: 分辨 1280x768, 通道:  抖动: 启用
	- 19: 分辨 1280x800, 通道:  抖动: 启用

    viafb_accel:
	- 0 : 2D 硬件加	- 1 : 2D 硬件加速（默认
    viafb_SAMM_ON:
	- 0 : viafb_SAMM_ON 禁用（默认）
	- 1 : viafb_SAMM_ON 启用

    viafb_mode1:（副显示设备	- 640x480（默认）
	- 720x480
	- 800x600
	- 1024x768

    viafb_bpp1:（副显示设备	- 8, 16, 32（默32
    viafb_refresh1:（副显示设备	- 60, 75, 85, 100, 120（默60
    viafb_active_dev:
	此选项用于指定活动设备。（CRT、DVI、CRT+LCD……）
	DVI 代表 DVI HDMI，例如，若想启用 HDMI	设置 viafb_active_dev=DVI。在 SAMM 情况下，viafb_active_dev
	之前的是主设备，之后的是副设备
	例如
	要启用一个设备，例如DVI，我们可以使:
	...
	- 2 : 浠?DVI
	- 3 : LCD+DVI（默认）
	- 4 : LCD1+LCD2（内+ 内部	- 16: LCD1+ExternalLCD2（内+ 外部
    viafb_second_size:
	此选项用于设置 SAMM 情况下第二设备的内存大小（MB）	最小大小为 16
    ...
    viafb_bus_width:
	当使24 位总线宽度的数字接口时	应设置此选项
	- 12: 12 LVDS 12 TMDS（默认）
