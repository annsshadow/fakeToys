...
**状态分布：** 已完成（真实化）4 个 · 已完成（基础设施）1 个 · 部分真实化 4 个 · 已接入（桩代码）71 个

| crate 名称 | 对应 Java 模块 | 当前状态 | 接入 main.rs 的版本 | 已接入的端点列表 |
|------------|----------------|----------|----------------------|------------------|
| ai | x_ai_assemble_control | 已接入（桩代码） | — | 桩 |
...
| attendance_assemble_control | x_attendance_assemble_control | 已接入（桩代码） | — | 桩 |
| attendance_core_entity | x_attendance_core_entity | 已接入（桩代码） | — | 桩 |
...
| base | x_base_core_project | 已接入（桩代码） | — | 桩 |
| bbs | x_bbs_assemble_control | 已接入（桩代码） | — | 桩 |
| bbs_assemble_control | x_bbs_assemble_control | 已接入（桩代码） | — | 桩 |
| bbs_core_entity | x_bbs_core_entity | 已接入（桩代码） | — | 桩 |
| calendar | x_calendar_assemble_control | 部分真实化 | — | 部分真实（calendar_list_public、calendar_list_my、calendar_get 等 PostgreSQL 查询已实现） |
| calendar_assemble_control | x_calendar_assemble_control | 已接入（桩代码） | — | 桩 |
| calendar_core_entity | x_calendar_core_entity | 已接入（桩代码） | — | 桩 |
| cms_assemble_control | x_cms_assemble_control | 已接入（桩代码） | — | 桩 |
| cms_control | x_cms_assemble_control | 已接入（桩代码，已挂载） | ac279a0f（U1） | GET /jaxrs/cms/view/list/all（空列表） |
| cms_core_entity | x_cms_core_entity | 已接入（桩代码） | — | 桩 |
| cms_core_express | x_cms_core_express | 已接入（桩代码） | — | 桩 |
| cms_express | x_cms_core_express | 已接入（桩代码） | — | 桩 |
| component | x_component_assemble_control | 已接入（桩代码） | — | 桩 |
| component_assemble_control | x_component_assemble_control | 已接入（桩代码） | — | 桩 |
...
| correlation | x_correlation_core_entity | 已接入（桩代码） | — | 桩 |
| correlation_core_entity | x_correlation_core_entity | 已接入（桩代码） | — | 桩 |
| correlation_core_express | x_correlation_core_express | 已接入（桩代码） | — | 桩 |
| correlation_service_processing | x_correlation_service_processing | 已接入（桩代码） | — | 桩 |
| express | 快递查询应用（Java 侧无独立模块，待确认） | 已接入（桩代码） | — | 桩 |
| file | x_file_assemble_control | 部分真实化 | — | 部分真实（folder_list_top、folder_list_with_folder、complex_top 等 PostgreSQL 查询已实现） |
| file_assemble_control | x_file_assemble_control | 已接入（桩代码） | — | 桩 |
| file_core_entity | x_file_core_entity | 已接入（桩代码） | — | 桩 |
| general | x_general_assemble_control | 已接入（桩代码） | — | 桩 |
| general_assemble_control | x_general_assemble_control | 部分真实化 | — | 部分真实（get_general_control_status、update_general_control_status、get_module_permissions 等 PostgreSQL 查询已实现） |
| general_core_entity | x_general_core_entity | 已接入（桩代码） | — | 桩 |
| hotpic | x_hotpic_assemble_control | 已接入（桩代码） | — | 桩 |
| hotpic_assemble_control | x_hotpic_assemble_control | 已接入（桩代码） | — | 桩 |
| hotpic_core_entity | x_hotpic_core_entity | 已接入（桩代码） | — | 桩 |
| jpush | x_jpush_assemble_control | 已接入（桩代码） | — | 桩 |
| jpush_assemble_control | x_jpush_assemble_control | 已接入（桩代码） | — | 桩 |
| jpush_core_entity | x_jpush_core_entity | 已接入（桩代码） | — | 桩 |
| meeting | x_meeting_assemble_control | 已接入（桩代码） | — | 桩 |
| meeting_assemble_control | x_meeting_assemble_control | 已接入（桩代码） | — | 桩 |
| meeting_core_entity | x_meeting_core_entity | 已接入（桩代码） | — | 桩 |
| message | x_message_assemble_communicate | 已接入（桩代码） | — | 桩 |
| message_assemble_communicate | x_message_assemble_communicate | 已接入（桩代码） | — | 桩 |
| message_core_entity | x_message_core_entity | 已接入（桩代码） | — | 桩 |
| mind | x_mind_assemble_control | 已接入（桩代码） | — | 桩 |
| mind_assemble_control | x_mind_assemble_control | 已接入（桩代码） | — | 桩 |
| mind_core_entity | x_mind_core_entity | 已接入（桩代码） | — | 桩 |
| organization_assemble_express | x_organization_assemble_express | 已接入（桩代码） | — | 桩 |
| organization_core_entity | x_organization_core_entity | 已接入（桩代码） | — | 桩 |
| organization_core_express | x_organization_core_express | 已接入（桩代码） | — | 桩 |
...
| portal | x_portal_assemble_surface | 已接入（桩代码） | — | 桩 |
| portal_assemble_designer | x_portal_assemble_designer | 已接入（桩代码） | — | 桩 |
| portal_assemble_surface | x_portal_assemble_surface | 已接入（桩代码） | — | 桩 |
| portal_core_entity | x_portal_core_entity | 已接入（桩代码） | — | 桩 |
| process_bam | x_processplatform_assemble_bam | 已接入（桩代码） | — | 桩 |
| process_designer | x_processplatform_assemble_designer | 已接入（桩代码） | — | 桩 |
| process_express | x_processplatform_core_express | 已接入（桩代码） | — | 桩 |
| process_surface | x_processplatform_assemble_surface | 已接入（桩代码） | — | 桩 |
| processplatform_assemble_bam | x_processplatform_assemble_bam | 已接入（桩代码） | — | 桩 |
| processplatform_assemble_designer | x_processplatform_assemble_designer | 已接入（桩代码） | — | 桩 |
| processplatform_assemble_surface | x_processplatform_assemble_surface | 已接入（桩代码） | — | 桩 |
| processplatform_core_entity | x_processplatform_core_entity | 已接入（桩代码） | — | 桩 |
| processplatform_core_express | x_processplatform_core_express | 已接入（桩代码） | — | 桩 |
| processplatform_service_processing | x_processplatform_service_processing | 已接入（桩代码） | — | 桩 |
| program_center | x_program_center | 已接入（桩代码） | — | 桩 |
| program_center_core_entity | x_program_center_core_entity | 已接入（桩代码） | — | 桩 |
| program_init | x_program_init | 已接入（桩代码） | — | 桩 |
| query_assemble_designer | x_query_assemble_designer | 已接入（桩代码） | — | 桩 |
| query_assemble_surface | x_query_assemble_surface | 已接入（桩代码） | — | 桩 |
| query_core_entity | x_query_core_entity | 已接入（桩代码） | — | 桩 |
