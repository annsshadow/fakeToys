...
全面推进 oa4rust 后端 Rust 迁移：全部 80 个 crate 接入 `main.rs` 并同步实现 PostgreSQL 真实业务逻辑，清除所有桩代码，使 Rust 后端达到可完全替代 Java 运行的完整度。

---

## Problem Frame

O2OA 后端当前 100% 基于 Java（Maven 55+ 模块），长期技术栈锁定在 Java 生态。oa4rust 项目已将全部 55 个 Java 模块映射为 80 个 Rust crate，编译通过且测试通过。目前全部 80 个 crate 已注册到 workspace 并在 `main.rs` 中挂载路由，但多数 crate 的端点为桩代码（返回 `ActionResult::success(Value::Null)`）。其中 auth、personal、personal_extend、control 等 4 个 crate 已实现真实业务逻辑；attendance、calendar、file、general_assemble_control 等 crate 已包含真实 PostgreSQL 查询代码，仅需补充路由暴露和业务逻辑完善；其余 crate 需从桩代码实现全量真实逻辑。认证模块的验证码和 OAuth 已有真实实现（PNG 图片生成、微信/钉钉 OAuth HTTP 调用），但需确保生产级健壮性。CMS 控制模块仅返回空列表。团队无法在 Rust 后端上推进任何实际工作，迁移停滞的代价是持续维护 Java 运行时和技术栈锁定。本次迁移要求全部 80 个 crate 的每一个端点都必须实现真实业务逻辑，无任何桩代码残留。
...
- 全部 80 个 crate 的路由在 `main.rs` 中可见，前端可通过对应前缀调用任意 API
- 全部 80 个 crate 的每一个端点均已接入 PostgreSQL 真实业务逻辑，无任何永久性桩代码残留，所有 TODO 标记已清除
...
- **全量真实化**：全部 80 个 crate 的每一个端点都必须实现 PostgreSQL 真实业务逻辑，无任何永久性桩代码残留
...
- 76 个未集成 crate 的路由与 `main.rs` 中已有 crate 的路由无冲突
- auth 模块的验证码和 OAuth 集成有可行的第三方服务或自建方案
- 迁移窗口期的数据量在可接受范围内
- 反向代理（nginx）已存在或可部署，作为前缀路由层
- 已实现真实业务逻辑的 4 个 crate（auth、personal、personal_extend、control）与 Java 端行为一致（需前端联调验证）
- 已包含真实 PostgreSQL 查询代码的 crate（attendance、calendar、file、general_assemble_control 等）的真实查询逻辑可被复用，无需完全重写
- 每个 Java JAX-RS 端点的业务逻辑可被独立还原，无需依赖 Java 侧实时运行
- 全部 80 个 crate 的数据库表 schema 可通过现有迁移脚本或增量迁移补齐
- 前端 `o2web` 的 `action.js` 对 `ActionResult<T>` 9 字段 JSON 结构的解析逻辑在 Rust 侧完全兼容
- 团队具备足够的 Rust 开发能力和人员配置，能够在合理时间内完成 80 个 crate 的真实化工作
...
- [Affects R4-R30][Needs research] 每个 Java JAX-RS 端点的业务逻辑细节，对照 Java 源码逐端点还原
- [Affects R49][Technical] 双轨运行期间 Rust 与 Java 模块间的跨服务调用如何处理？
- [Affects R50][Needs research] 模块跟踪清单的格式和更新频率如何定义？
- [Affects R5][Needs research] 各 crate 对应的数据库表 schema 是否完整，缺失表如何补齐？
- [Affects R39-R48][Technical] 统一认证中间件、授权中间件、输入验证中间件、速率限制中间件的实现方案
- [Affects R4][Needs research] 首批优先实现的 20 个 crate 具体是哪些，如何定义"核心用户工作流"和"低复杂度"？
