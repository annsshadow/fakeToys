...
- 终止工作：`GET /api/processplatform/work/terminate/{id}` → `work_terminate` 执行 `UPDATE x_work SET work_status='terminated'`，返回 id/workStatus="terminated"/result="ok"
- 撤回工作：`GET /api/processplatform/work/retract/{id}` → 先查 x_work 原状态，无记录返回 error("work not found")，成功输出 workStatus="retracted" 与 previousStatus
...
- `GET /api/processplatform/task/count/with/person/{id}`
- `GET /api/processplatform/task/processing/{id}`
- `GET /api/processplatform/work/count/with/person/{id}`
- `GET /api/processplatform/work/processing/{id}`
- `GET /api/processplatform/work/retract/{id}`
- `GET /api/processplatform/work/terminate/{id}`
