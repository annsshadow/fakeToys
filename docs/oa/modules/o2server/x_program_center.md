...
- 应用与风格：`GET /api/program/applications`、`appstyle/current/style` 查 `x_applications`（name/app_id/disable），current_style 取前 3 条派生 portalList；`center/regist_applications` 注册应用
...
- `GET /api/program/appstyle/current/style`
- `GET /api/program/datastructure/modules/all`
- `POST /api/program_center/agent/create`
- `POST /api/program_center/agent/save/{id}`
- `POST /api/program_center/application/create`
- `POST /api/program_center/application/save/{id}`
