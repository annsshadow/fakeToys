...
- 组件全量列表：`GET /api/component/core/entity/list/all` → `component_list_all` 原生 SQL 查 `CPT_COMPONENT` 过滤 `deleted_at IS NULL`，order_number 升序；order_number 为 NULL 时输出中省略 orderNumber 键
