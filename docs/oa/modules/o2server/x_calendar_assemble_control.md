...
- 日历详情聚合：`GET /api/calendar/assemble/control/calendar/detail/{id}` 查 `CAL_CALENDAR` 单行并联查 `CAL_EVENT`（status='OPEN'，按 start_time 升序 LIMIT 100）组装 events 与 eventCount
...
- `GET /api/calendar/assemble/control/calendar/detail/{id}`
- `GET /api/calendar_assemble_control/get/control/config`
- `GET /api/calendar_assemble_control/list/control/calendars`
- `GET /api/calendar_assemble_control/update/control/config`
