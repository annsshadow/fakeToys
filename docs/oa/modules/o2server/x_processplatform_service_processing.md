...
- 流程启动/完成：`POST /api/work/{id}/start|complete` → 事务内 `FOR UPDATE` 状态机校验（start 要求 pending、complete 要求 processing），更新 x_work 并插入 "Start Event" 任务或批量完成 x_task 后写入 x_workcompleted
...
- 任务生命周期：`POST /api/task/{id}/claim|complete|reject|transfer/{person}` → FOR UPDATE 校验后更新 task_status/person，写 x_record 审计；complete 自动激活同 activity_token 的下一个 pending 任务，reject 回退前序 completed 任务
- 网关聚合/分叉：`POST /api/gateway/{work_id}/{activity_token}/join|.../gateway/fork/{gateway_instance_id}` → join 校验同 token 全部 completed 才写 gateway_join 记录；fork 按 x_process_transition 为每条转移创建 pending 任务
...
- `POST /api/processplatform/service/processing/cancel/{executionId}`
- `POST /api/processplatform/service/processing/create`
- `POST /api/processplatform/service/processing/execute/{id}`
- `GET /api/processplatform/service/processing/get/{id}`
- `GET /api/processplatform/service/processing/instance/{executionId}`
- `GET /api/processplatform/service/processing/list/{category}`
- `GET /api/processplatform/service/processing/process/{id}/complex`
- `GET /api/processplatform/service/processing/work/list`
- `PUT /api/work/{id}/processing`
- `POST /api/work/{id}/retract`
- `POST /api/work/{id}/terminate`
