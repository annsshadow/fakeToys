...
- 健康检查：`GET /api/base/echo/get` → `echo_get` 无需数据库，固定返回 `{"type":"echo","message":"pong"}`（带 utoipa OpenAPI 注解）
- 缓存监控：`GET /api/base/cache/detail` → `cache_detail` 从 deadpool 池取连接，执行 `SELECT count(*) FROM pg_class WHERE relname LIKE 'cache_%'` 统计 `cache_` 前缀表数量，返回 `{status:"running", cacheCount}`
- OpenAPI 入口：`GET /api/base/openapi/info` → `openapi_info` 返回 307 Redirect 到 `/openapi.json`
