...
- 上传建档：`POST /api/file/assemble/control/file/upload|create` → 从 Session 取 person_unique 为 creator → INSERT INTO `x_file`（name/path/size/folder_id）→ 返回文件元数据
- 下载分发：`GET /api/file/{id}/download/stream`、`/api/attachment/download/{attid}/stream` 及 anonymous 变体 → 查 `FILE_FILE` 内容并 base64 解码 → 按原 MIME 附件流式响应
...
- `GET /api/anonymous/file/{id}/download/stream`
- `GET /api/attachment/download/{attid}/stream`
- `GET /api/file/assemble/control/attachment2/{id}/office/preview/type/{type}`
- `POST /api/file/assemble/control/file/create`
- `POST /api/file/assemble/control/file/delete/{id}`
- `GET /api/file/assemble/control/file/list/{folderId}`
- `POST /api/file/assemble/control/file/upload`
- `GET /api/file/assemble/control/file/{id}`
- `POST /api/file/core/entity/file/create`
- `POST /api/file/core/entity/file/delete/{id}`
- `POST /api/file/core/entity/file/update/{id}`
- `GET /api/file/{id}/download/stream`
