...
- 视图列表/详情：`GET /api/query/view/list|view/{id}` → deadpool 查 QUERY_VIEW 且 deleted_at IS NULL（list 按 create_time 倒序 limit 20），description/querySql 可选字段按存在输出；详情未命中返回 error("view not found")
- 视图创建：`POST /api/query/view/create` → `view_create` uuid v4 生成 id，status 缺省 active，INSERT 后回显 id/name/creatorId
- 查询项列表：`GET /api/query/item/list` → `item_list` 查 QUERY_ITEM 过滤软删 limit 20，输出 viewId/fieldName/dataType
- 导入记录列表：`GET /api/query/import/list` → `import_list` 查 QUERY_IMPORT，importTime 可空
