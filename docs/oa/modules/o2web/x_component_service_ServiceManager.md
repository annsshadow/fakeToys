# x_component_service_ServiceManager

## Responsibility

服务组件，负责服务管理的界面展示与交互操作。

## Entry Point

- `oa\o2web\source\x_component_service_ServiceManager\Main.js`
...
- 跨组件跳转：通过 `openApplication` 打开其它应用（FindDesigner、service）。
- 核心交互：服务组件，负责服务管理的界面展示与交互操作。
- 核心交互：服务组件，负责服务管理的界面展示与交互操作。
