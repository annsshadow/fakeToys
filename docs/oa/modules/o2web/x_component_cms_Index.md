...
CMS（内容管理）前端组件，负责内容管理首页的界面展示与交互操作，通过 REST API 与后端模块通信。

## Entry Point

- `oa\o2web\source\x_component_cms_Index\Main.js`
...
- 与后端 `AppInfoAction` 交互：在组件中调用 `AppInfoAction.listHasDocument`、`listHasDocumentAppType`、`listHasDocument_WithAppType` 等方法完成 列出/列出/列出 等操作。
