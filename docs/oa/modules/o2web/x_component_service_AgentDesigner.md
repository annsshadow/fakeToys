# x_component_service_AgentDesigner
...
- `oa\o2web\source\x_component_service_AgentDesigner\Main.js`
...
- 与后端 `AgentItemAction` 交互：在组件中调用 `AgentItemAction.addEvent` 等方法完成 添加 等操作。
- 与后端 `AgentAction` 交互：在组件中调用 `AgentAction.execute` 等方法完成 执行 等操作。
