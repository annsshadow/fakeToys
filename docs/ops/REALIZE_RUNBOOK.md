...
## 4.2 S4 真实流量 gate 已废止——无旧系统、改好即上线（2026-09-15 口径）

旧口径（S4 金丝雀/真实 pilot 观察窗口门禁）已废止：o2server/o2web **从未上线**，不存在旧系统、
不存在生产镜像流量，因此影子比对、真实试点流量、观察窗口 5xx 预算签核都没有前提，
也无需「先小范围 pilot 再放大」。现口径：**全部静态门禁 + 仓内 live E2E 通过即上线，无旧系统可回退**。

### 上线判据（替代原 S4 gate）

1. 静态门禁全绿：workspace `cargo test`、前端 vitest/typecheck/build、契约守卫
   （desktop/mobile endpoints、backend fixture、W13 manifest）、behavior_compare 基线无回归。
2. 仓内 live E2E：双栈/单栈起服务后 Playwright 实跑（见 §S3 相关节）关键旅程无 404/500。
3. 上线后监控：nginx access log 5xx 告警 + `trace_middleware` 5xx 日志（上线后常态观测，
   非上线前置条件）。

### 已移除的 S4 机制（2026-09-15）

- `.github/workflows/oa4rust-pilot-gate.yml`（真实流量 pilot gate workflow）——已删除；
- `deploy/shadow-traffic.sh`、`deploy/nginx-auth-routes-java-drill.conf`、`deploy/rollback-playbook.md`
  （Java 影子流量 / 回切 Java / 切回 Java 回滚手册——旧系统不存在，机制作废）——已删除；
- `docs/ops/s4-pilot-canary-runbook.md`（S4 金丝雀交接 runbook）——已删除。

`scripts/pilot_gate.py` + `tests/test_pilot_gate.py` + `tests/fixtures/pilot_access.fixture`
保留为**可选离线观测工具**（上线后可对任意 access log 窗口离线核算 5xx 预算，非门禁），
本地回归命令不变：
...
保留件：`deploy/auth-rollback-drill.sh`（Rust 侧 auth cookie 功能矩阵演练，非旧系统机制）、
`deploy/nginx-auth-routes-rust-drill.conf`、`deploy/nginx.conf`（服务配置）、`docs/ops/rollback-plan.md`
（Rust 新版本→旧版本部署回滚，与旧系统无关）。
