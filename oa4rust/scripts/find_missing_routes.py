...
print(f'Total Rust routes: {len(rust_routes)}')

# 2. 提取所有 Java @Path，生成缺失路由列表
missing_routes = []
for java_mod, rust_crate in module_crate_map.items():
    prefix = None
    for jm, p in module_crate_map.items():
        if jm == java_mod:
            prefix = p
            prefix = prefix.replace('_', '/')
            if not prefix.startswith('/jaxrs/'):
                prefix = '/jaxrs/' + prefix
            break
    ...
    for path in sorted(java_paths):
        if path not in rust_routes:
            missing_routes.append((rust_crate, path))

print(f'Total missing routes: {len(missing_routes)}')
for crate, path in missing_routes[:20]:
    print(f'  {crate}: {path}')
