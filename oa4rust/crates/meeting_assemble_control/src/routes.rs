...
pub fn meeting_assemble_control_routes(pool: Pool) -> Router {
    Router::new()
        ...
        // ════════════ plan002 U2：端点全量闭合（Java jaxrs 76 端点对齐） ════════════
        // 既有错位路由保留（回归保护），缺口一律按 Java 真实 verb+path 注册。
        // ── Attachment 族（11）──
        .route("/jaxrs/meeting/assemble/control/attachment/list/meeting/{meetingId}", get(crate::u2_attachment_list_with_meeting))
        .route("/jaxrs/meeting/assemble/control/attachment/{id}", get(crate::u2_attachment_get).delete(crate::u2_attachment_delete))
        .route("/jaxrs/meeting/assemble/control/attachment/{id}/download/{stream}", get(crate::u2_attachment_download))
        .route("/jaxrs/meeting/assemble/control/attachment/{id}/update", put(crate::u2_attachment_update))
        .route("/jaxrs/meeting/assemble/control/attachment/{id}/update/callback/{ballback}", post(crate::u2_attachment_update_callback))
        .route("/jaxrs/meeting/assemble/control/attachment/create/from/processplatform", post(crate::u2_attachment_create_from_processplatform))
        .route("/jaxrs/meeting/assemble/control/attachment/list/{id}/next/{count}", get(crate::u2_attachment_list_next))
        .route("/jaxrs/meeting/assemble/control/attachment/list/{id}/prev/{count}", get(crate::u2_attachment_list_prev))
        .route("/jaxrs/meeting/assemble/control/attachment/meeting/{meetingId}/upload/{summary}", post(crate::u2_attachment_upload))
        .route("/jaxrs/meeting/assemble/control/attachment/meeting/{meetingId}/upload/{summary}/callback/{callback}", post(crate::u2_attachment_upload_callback))
        // ── Building 族补齐（create / edit / delete + start/completed 正确段序 GET）──
        .route("/jaxrs/meeting/assemble/control/building", post(crate::u2_building_create))
        .route("/jaxrs/meeting/assemble/control/building/{id}", put(crate::u2_building_edit).delete(crate::u2_building_delete))
        .route("/jaxrs/meeting/assemble/control/building/list/start/{start}/completed/{completed}", get(crate::building_list_start_start_completed_completed))
        .route("/jaxrs/meeting/assemble/control/building/list/start/{start}/completed/{completed}/allmeeting", get(crate::building_list_start_start_completed_completed_allmeeting))
        .route("/jaxrs/meeting/assemble/control/building/list/start/{start}/completed/{completed}/room/{room}/meeting/{meeting}", get(crate::building_list_start_start_completed_completed_room_room_meeting_meeting))
        // ── Config 族补齐（POST save + GET manage）──
        .route("/jaxrs/meeting/assemble/control/config", post(crate::u2_config_save))
        .route("/jaxrs/meeting/assemble/control/config/system/config/manage", get(crate::u2_config_manage_get))
        // ── Meeting 族补齐（动词修正 + IDOR 写操作 + 分页/manage 正确段序）──
        .route("/jaxrs/meeting/assemble/control/meeting", post(create_meeting))
        .route("/jaxrs/meeting/assemble/control/meeting/{id}", delete(crate::u2_meeting_delete_owned).post(crate::u2_meeting_modify).put(crate::u2_meeting_put_save))
        .route("/jaxrs/meeting/assemble/control/meeting/{id}/accept", get(meeting_id_accept))
        .route("/jaxrs/meeting/assemble/control/meeting/{id}/add/invite", put(meeting_id_add_invite))
        .route("/jaxrs/meeting/assemble/control/meeting/{id}/checkin", get(crate::u2_meeting_checkin_get))
        .route("/jaxrs/meeting/assemble/control/meeting/{id}/checkin/code", get(crate::meeting_id_checkin_code))
        .route("/jaxrs/meeting/assemble/control/meeting/{id}/confirm/allow", get(meeting_id_confirm_allow))
        .route("/jaxrs/meeting/assemble/control/meeting/{id}/confirm/deny", get(meeting_id_confirm_deny))
        .route("/jaxrs/meeting/assemble/control/meeting/{id}/delete/invite", put(meeting_id_delete_invite))
        .route("/jaxrs/meeting/assemble/control/meeting/{id}/manual/completed", get(meeting_id_manual_completed))
        .route("/jaxrs/meeting/assemble/control/meeting/{id}/modify/completedtime", put(meeting_id_modify_completedtime))
        .route("/jaxrs/meeting/assemble/control/meeting/{id}/modify/starttime", put(meeting_id_modify_starttime))
        .route("/jaxrs/meeting/assemble/control/meeting/{id}/reject", get(meeting_id_reject))
        .route("/jaxrs/meeting/assemble/control/meeting/list/{id}/next/{count}", get(crate::meeting_list_id_next_count))
        .route("/jaxrs/meeting/assemble/control/meeting/list/{id}/prev/{count}", get(crate::meeting_list_id_prev_count))
        .route("/jaxrs/meeting/assemble/control/meeting/list/{page}/size/{size}", post(crate::meeting_list_page_size_size))
        .route("/jaxrs/meeting/assemble/control/meeting/list/{page}/size/{size}/manage", post(crate::meeting_list_page_size_size_manage))
        .route("/jaxrs/meeting/assemble/control/meeting/list/forward/monthcount/{monthCount}/all", get(crate::meeting_list_forward_monthcount_monthCount_all))
        .route("/jaxrs/meeting/assemble/control/meeting/list/invite/{page}/size/{size}", post(crate::meeting_list_invite_page_size_size))
        .route("/jaxrs/meeting/assemble/control/meeting/list/year/{year}/month/{month}/day/{day}/{roomId}", get(crate::meeting_list_year_year_month_month_day_day_roomId))
        .route("/jaxrs/meeting/assemble/control/meeting/list/coming/month/{count}", get(crate::meeting_list_coming_month_count))
        .route("/jaxrs/meeting/assemble/control/meeting/list/apply/{page}/size/{size}", post(crate::meeting_list_apply_page_size_size))
        // ── OpenMeeting 根端点（开放会议服务器配置读视图）──
        .route("/jaxrs/meeting/assemble/control/openmeeting", get(crate::u2_openmeeting_get))
        // ── Room 族补齐（create / edit / delete / setPhoto）──
        .route("/jaxrs/meeting/assemble/control/room", post(crate::u2_room_create))
        .route("/jaxrs/meeting/assemble/control/room/{id}", put(crate::u2_room_edit).delete(crate::u2_room_delete))
        .route("/jaxrs/meeting/assemble/control/room/{id}/photo", post(crate::u2_room_set_photo))
