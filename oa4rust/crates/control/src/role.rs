...
/// 获取角色列表（下一批）：GET /jaxrs/role/list/{flag}/next/{count}
...
/// 获取角色列表（上一批）：GET /jaxrs/role/list/{flag}/prev/{count}
