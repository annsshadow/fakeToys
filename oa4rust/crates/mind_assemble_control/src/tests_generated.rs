...
mod tests {
    ...
    async fn test_list_folders() {
        ...
        assert_ne!(
            response.status(),
            StatusCode::NOT_FOUND,
            "list_folders route should be registered"
        );
    }
    ...
    async fn test_get_folder() {
        ...
        assert_ne!(
            response.status(),
            StatusCode::NOT_FOUND,
            "get_folder route should be registered"
        );
    }

    #[tokio::test]
    async fn test_save_folder() {
        let pool = shared::testing::test_pool();
        let app = crate::router(pool);
        let response = app
            .oneshot(
        ...
        assert_ne!(
            response.status(),
            StatusCode::NOT_FOUND,
            "save_folder route should be registered"
        );
    }
    ...
    #[tokio::test]
    async fn test_folder_move_folderId() {
        let pool = shared::testing::test_pool();
        let app = crate::router(pool);
        let response = app
            .oneshot(
        ...
        assert_ne!(
            response.status(),
            StatusCode::NOT_FOUND,
            "folder_move_folderId route should be registered"
        );
    }

    // SKIPPED: folder_id not accessible
    #[tokio::test]
    async fn test_folder_id_force() {
        let pool = shared::testing::test_pool();
        let app = crate::router(pool);
        let response = app
            .oneshot(
        ...
        assert_ne!(
            response.status(),
            StatusCode::NOT_FOUND,
            "folder_id_force route should be registered"
        );
    }
}
