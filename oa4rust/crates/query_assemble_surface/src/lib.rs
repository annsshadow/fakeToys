    ...
    Ok(Json(ActionResult::success(Value::Object(
        serde_json::Map::from_iter([
            ...
            (
                ...
                Value::String(format!("/preview/query/{}", id)),
            ),
            (
                "html".to_string(),
                Value::String("<div>Query Preview</div>".to_string()),
            ),
        ]),
    ))))
}
    ...
    let data: Vec<Value> = rows
        ...
        .map(|row| {
            Value::Object(serde_json::Map::from_iter([
                ...
                (
                    "workId".to_string(),
                    Value::String(row.get::<_, Option<String>>("work_id").unwrap_or_default()),
                ),
                (
                    "result".to_string(),
        ...
        .map(|row| {
            Value::Object(serde_json::Map::from_iter([
                ...
                (
                    "excelData".to_string(),
                    Value::String(
                        row.get::<_, Option<String>>("excel_data")
                            .unwrap_or_default(),
                    ),
                ),
                (
        ...
        Some(row) => Ok(Json(ActionResult::success(Value::Object(
            serde_json::Map::from_iter([
                ...
                (
                    "bundle".to_string(),
                    Value::String(
                        row.get::<_, Option<String>>("bundle_data_v2")
                            .unwrap_or_default(),
                    ),
                ),
            ]),
