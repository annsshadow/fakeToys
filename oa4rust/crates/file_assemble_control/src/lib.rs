...
pub async fn get_control_config(
    ...
    let data = match row {
        Ok(r) => serde_json::Map::from_iter([
            ...
            (
                "defaultStorage".to_string(),
                Value::String(r.get_str("default_storage").to_string()),
            ),
            (
                "maxUploadSize".to_string(),
                Value::Number(serde_json::Number::from(r.get_i64("max_upload_size"))),
            ),
        ]),
        Err(_) => serde_json::Map::from_iter([
            ...
            (
                "maxUploadSize".to_string(),
                Value::Number(serde_json::Number::from(104857600i64)),
            ),
        ]),
    };
        ...
        Ok(r) => r
            ...
            .map(|row| {
                Value::Object(serde_json::Map::from_iter([
                    (
                        "id".to_string(),
                        Value::String(row.get_str("id").to_string()),
                    ),
                    (
                        "name".to_string(),
                        Value::String(row.get_str("name").to_string()),
                    ),
...
pub async fn update_control_config(
    ...
    Ok(Json(ActionResult::success(Value::Object(
        serde_json::Map::from_iter([
            ...
            (
                "maxUploadSize".to_string(),
                Value::Number(serde_json::Number::from(max_upload_size)),
            ),
        ]),
    ))))
}
            ...
            .map(|row| {
                Value::Object(serde_json::Map::from_iter([
                    ...
                    (
                        ...
                        Value::String(row.get_str("description").to_string()),
                    ),
                ]))
            })
            .collect(),
...
pub async fn update_file_entity(
    ...
    let row = client
        .query_opt(
            "SELECT creator FROM x_file WHERE id = $1 AND deleted_at IS NULL",
            &[&id],
        )
        .await
        .map_err(|_| AppError::Internal)?;
...
pub async fn delete_file_entity(
    ...
    let result = client
        .execute("DELETE FROM x_file WHERE id = $1", &[&id])
        .await
        .map_err(|_| AppError::Internal)?;
...
pub async fn attachment_upload_folder_folderId(
    ...
    let data = match file_data {
        Some(d) => d,
        None => return Ok(Json(ActionResult::error("no file provided"))),
    };
    let client = pool.get().await.map_err(|_| AppError::Internal)?;
    let id = uuid::Uuid::new_v4().to_string();
    ...
    let ext = match mime {
        ...
        _ => {
            if let Some(ref fname) = filename {
                fname.split('.').next_back().unwrap_or("bin")
            } else {
                "bin"
            }
        }
...
pub async fn attachment_upload_folder_folderId_callback_callback(
    ...
    attachment_upload_folder_folderId(
        pool,
        Extension(session),
        axum::extract::Path(folder_id),
        form,
    )
    .await
}
...
pub async fn attachment_id_binary_base64(
    ...
    let row = client
        .query_opt(
            "SELECT content FROM FILE_FILE WHERE id = $1 AND deleted_at IS NULL",
            &[&id],
        )
        .await
        .map_err(|_| AppError::Internal)?;
...
pub async fn attachment_id_update(
    ...
    let row = client
        .query_opt(
            "SELECT person FROM FILE_FILE WHERE id = $1 AND deleted_at IS NULL",
            &[&id],
        )
        .await
        .map_err(|_| AppError::Internal)?;
...
pub async fn attachment2_exist_file_fileMd5(
    ...
    let count: i64 = client
        .query_one(
            "SELECT COUNT(*) FROM FILE_FILE WHERE md5 = $1 AND deleted_at IS NULL",
            &[&md5],
        )
        .await
        .map_err(|_| AppError::Internal)?
...
pub async fn attachment2_user_capacity(
    ...
    Ok(Json(ActionResult::success(Value::Object(
        serde_json::Map::from_iter([
            ...
            (
                "limit".to_string(),
                Value::Number(serde_json::Number::from(1073741824i64)),
            ),
...
fn docx_to_html(bytes: &[u8]) -> Option<String> {
    ...
    while let Some(start) = rest.find("<w:p") {
        let Some(close_rel) = rest[start..].find("</w:p>") else {
            break;
        };
...
fn xlsx_to_html(bytes: &[u8]) -> Option<String> {
    ...
    let shared: Vec<String> = {
        ...
        while let Some(start) = rest.find("<si>") {
            let Some(end_rel) = rest[start..].find("</si>") else {
                break;
            };
            ...
            while let Some(t_start) = cur.find("<t") {
                let Some(gt_rel) = cur[t_start..].find('>') else {
                    break;
                };
                let cs = t_start + gt_rel + 1;
                let Some(e_rel) = cur[cs..].find("</t>") else {
                    break;
                };
    ...
    while let Some(start) = rest.find("<row") {
        let Some(row_end_rel) = rest[start..].find("</row>") else {
            break;
        };
        ...
        while let Some(c_start) = cur.find("<c ") {
            let Some(c_end_rel) = cur[c_start..].find("</c>") else {
                break;
            };
    ...
    for (_, sname) in &slide_names {
        ...
        let texts = {
            ...
            while let Some(start) = rest.find("<a:t>") {
                let cs = start + 5;
                let Some(e_rel) = rest[cs..].find("</a:t>") else {
                    break;
                };
...
#[axum::debug_handler]
#[allow(non_snake_case)]
pub async fn editor_list(pool: Extension<Pool>) -> Result<Json<ActionResult<Value>>, AppError> {
    let client = pool.get().await.map_err(|_| AppError::Internal)?;
    let rows = client
...
pub async fn file_clean_unused_referencetype_cmsdocument_manage(
    ...
    let result = client
        .execute(
            "DELETE FROM FILE_FILE WHERE reference_type = $1 AND deleted_at IS NULL",
            &[&String::from("cmsdocument_manage")],
        )
        .await
        .map_err(|_| AppError::Internal)?;
    Ok(Json(ActionResult::success(Value::Object(
...
pub async fn file_copy_attachment_attachmentId_referencetype_referenceType_reference_reference_scale_scale(
    pool: Extension<Pool>,
    axum::extract::Path((attachment_id, reference_type, reference, _scale)): axum::extract::Path<(
        String,
        String,
        String,
        String,
...
pub async fn file_list_id_next_count_referencetype_referenceType(
    pool: Extension<Pool>,
    axum::extract::Path((id, _count, reference_type)): axum::extract::Path<(
        String,
        String,
        String,
    )>,
...
pub async fn file_referencetype_referenceType_reference_reference(
    ...
    file_list_referencetype_referenceType_reference_reference(
        pool,
        axum::extract::Path((reference_type.clone(), reference.clone())),
    )
    .await
}
...
pub async fn file_upload_referencetype_referenceType_reference_reference_scale_scale(
    pool: Extension<Pool>,
    Extension(session): Extension<shared::session::Session>,
    axum::extract::Path((reference_type, reference, _scale)): axum::extract::Path<(
        String,
        String,
        String,
    )>,
...
pub async fn file_upload_referencetype_referenceType_reference_reference_scale_scale_callback_callback(
    pool: Extension<Pool>,
    Extension(session): Extension<shared::session::Session>,
    axum::extract::Path((reference_type, reference, _scale, _callback)): axum::extract::Path<(
        String,
        String,
        String,
        String,
    ...
    file_upload_referencetype_referenceType_reference_reference_scale_scale(
        pool,
        Extension(session),
        axum::extract::Path((reference_type, reference, _scale)),
        form,
    )
    .await
}
...
pub async fn recycle_empty(
    ...
    let result = client
        .execute(
            "DELETE FROM FILE_FILE WHERE deleted_at IS NOT NULL AND person = $1",
            &[&session.person_unique],
        )
        .await
        .map_err(|_| AppError::Internal)?;
    ...
    let data: Vec<Value> = rows
        ...
        .map(|row| {
            Value::Object(serde_json::Map::from_iter([
                ...
                (
                    "deletedAt".to_string(),
                    Value::String(row.get("deleted_at")),
                ),
            ]))
        })
        .collect();
...
pub async fn recycle_id_delete(
    ...
    let row = client
        .query_opt(
            "SELECT person FROM FILE_FILE WHERE id = $1 AND deleted_at IS NOT NULL",
            &[&id],
        )
        .await
        .map_err(|_| AppError::Internal)?;
    let Some(row) = row else {
        return Ok(Json(ActionResult::error("file not in recycle bin")));
    };
    let creator: String = row.get::<_, Option<String>>("person").unwrap_or_default();
    shared::middleware::require_owner(&pool, &session, &creator).await?;
    let result = client
        .execute(
            "DELETE FROM FILE_FILE WHERE id = $1 AND deleted_at IS NOT NULL",
            &[&id],
        )
        .await
        .map_err(|_| AppError::Internal)?;
...
pub async fn recycle_id_resume(
    ...
    client
        .execute(
            "UPDATE FILE_FILE SET deleted_at = NULL, update_time = NOW() WHERE id = $1",
            &[&id],
        )
        .await
        .map_err(|_| AppError::Internal)?;
...
pub async fn share_list_folder_share_shareId_folder_folderId(
    ...
    share_list_att_share_shareId_folder_folderId(pool, axum::extract::Path((share_id, folder_id)))
        .await
}
...
pub async fn share_share_shareId_file_fileId_folder_folderId(
    pool: Extension<Pool>,
    Extension(session): Extension<shared::session::Session>,
    axum::extract::Path((share_id, file_id, folder_id)): axum::extract::Path<(
        String,
        String,
        String,
    )>,
    ...
    u2_share_save_to_folder(
        pool,
        Extension(session),
        axum::extract::Path((share_id, file_id, folder_id)),
    )
    .await
}
    ...
    let data: Vec<Value> = rows
        ...
        .map(|row| {
            Value::Object(serde_json::Map::from_iter([
                (
                    "name".to_string(),
                    Value::String(row.get::<_, String>("rtype")),
                ),
                (
                    "value".to_string(),
                    Value::String(row.get::<_, String>("rtype")),
...
pub async fn u2_folder_rename(
    ...
    let row = client
        .query_opt(
            "SELECT person FROM FILE_FOLDER WHERE id = $1 AND deleted_at IS NULL",
            &[&id],
        )
        .await
        .map_err(|_| AppError::Internal)?;
...
fn u2_share_row_to_json(row: &deadpool_postgres::tokio_postgres::Row) -> Value {
    let mut map = serde_json::Map::from_iter([
        ...
        (
            "shareType".to_string(),
            Value::String(
                row.get::<_, Option<String>>("share_type")
                    .unwrap_or_default(),
            ),
        ),
    ]);
...
const U2_SHARE_SELECT: &str =
    "SELECT id, person, name, file_id, file_type, extension, length, share_type FROM FILE_SHARE";
...
pub async fn u2_share_create(
    ...
    if share_type == "password" && password.trim().is_empty() {
        return Err(AppError::BadRequest(
            "password is required for password share".to_string(),
        ));
    }
    ...
    let (extension, length): (Option<String>, i64) = match &meta {
        Some(m) => (
            m.get("extension"),
            m.get::<_, Option<i64>>("length").unwrap_or(0),
        ),
        None => (None, 0),
    };
...
pub async fn u2_share_get_with_password(
    ...
    match row {
        ...
        None => Ok(Json(ActionResult::error(
            "share not found or password mismatch",
        ))),
    }
}
...
pub async fn u2_attachment2_list_type_page_size_size(
    ...
    attachment2_list_type_page_size_size(
        pool,
        axum::extract::Path((String::from("all"), page, size)),
    )
    .await
}
