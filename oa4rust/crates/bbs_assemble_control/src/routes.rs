...
pub fn router(pool: Pool) -> Router {
    Router::new()
        ...
        .route(
            ...
            get(u2::u2_attachment_list_by_subject),
        )
        .route(
        ...
        .route(
            &fmt("permission/replyPublishable/{subjectId}"),
            get(crate::permission_replyPublishable_subjectId),
        )
        .route(
            &fmt("permission/section/{sectionId}"),
            get(permission_section_sectionId),
        )
        .route(
            &fmt("permission/subject/{subjectId}"),
            get(permission_subject_subjectId),
        )
        .route(
        ...
        .route(
            &fmt("reply/filter/list/page/{page}/count/{count}"),
            put(u2::u2_reply_filter_list),
        )
        ...
        .route(
            &fmt("section/viewsub/{sectionId}"),
            get(u2::u2_section_viewsub),
        )
        ...
        .route(
            &fmt("shutup/list/paging/{page}/size/{size}"),
            post(shutup_list),
        )
        ...
        .route(
            &fmt("subject/recommended/index/{count}"),
            get(topic_recommended_index),
        )
        .route(
            &fmt("subject/statgrade/sectionName/{sectionName}/subjectType/{subjectType}"),
            get(u2::u2_statgrade),
        )
        .route(&fmt("subject/top/{sectionId}"), get(subject_top_sectionId))
        .route(&fmt("subject/view/{id}"), get(subject_view_id))
        .route(
            &fmt("subject/filter/listsubjectinfo/page/{page}/count/{count}"),
            post(u2::u2_subject_listsubjectinfo_page),
        )
        .route(
            &fmt("subject/creamed/list/page/{page}/count/{count}"),
            put(subject_creamed_list),
        )
        .route(
            &fmt("subject/filter/list/page/{page}/count/{count}"),
            put(subject_filter_list),
        )
        .route(
            &fmt("subject/index/list/page/{page}/count/{count}"),
            put(subject_index_list),
        )
        .route(
            &fmt("subject/recommended/list/page/{page}/count/{count}"),
            put(crate::topic_recommended_list),
        )
        .route(
            &fmt("subject/search/list/page/{page}/count/{count}"),
            put(u2::u2_subject_search_page),
        )
        // ── subjectattach（SubjectAttachmentAction，4 条）──
        .route(&fmt("subjectattach/{id}"), get(u2::u2_subjectattach_get))
        .route(
            &fmt("subjectattach/{id}"),
            delete(u2::u2_subjectattach_delete),
        )
        .route(
            &fmt("subjectattach/{id}/binary/base64/{size}"),
            get(u2::u2_subjectattach_base64),
        )
        .route(
            &fmt("subjectattach/list/subject/{id}"),
            get(u2::u2_subjectattach_list),
        )
        ...
        .route(
            &fmt("user/permission/forum/{forumId}"),
            get(u2::u2_permission_admin_forum),
        )
        .route(
            &fmt("user/permission/role/{roleCode}"),
            get(u2::u2_permission_admin_role),
        )
        .route(
            &fmt("user/permission/section/{sectionId}"),
            get(u2::u2_permission_admin_section),
        )
        ...
        .route(
            &fmt("user/reply/my/list/page/{page}/count/{count}"),
            put(u2::u2_my_reply_list),
        )
        ...
        .route(
            &fmt("user/role/forum/{forumId}"),
            put(u2::u2_role_list_by_forum),
        )
        .route(
            &fmt("user/role/rolecode/selected"),
            put(u2::u2_role_selected_by_code),
        )
        .route(
            &fmt("user/role/section/{sectionId}"),
            put(u2::u2_role_list_by_section),
        )
        .route(&fmt("user/role/unit/selected"), put(u2::u2_role_by_unit))
        .route(&fmt("user/role/user/selected"), put(u2::u2_role_by_user))
        .route(&fmt("user/role/{id}"), get(u2::u2_role_get))
        .route(&fmt("user/role/{id}"), delete(u2::u2_role_delete))
        ...
        .route(
            &fmt("user/section/force/{id}"),
            delete(u2::u2_user_section_delete_force),
        )
        .route(
            &fmt("user/section/forum/{forumId}"),
            get(u2::u2_user_section_forum),
        )
        .route(
            &fmt("user/section/sub/{sectionId}"),
            get(u2::u2_user_section_sub),
        )
        .route(
            &fmt("user/section/{id}"),
            delete(u2::u2_user_section_delete),
        )
        ...
        .route(
            &fmt("user/subject/change/section"),
            put(u2::u2_subject_change_section),
        )
        .route(
            &fmt("user/subject/my/list/page/{page}/count/{count}"),
            put(u2::u2_my_subject_list),
        )
        .route(&fmt("user/subject/vote/submit"), put(u2::u2_vote_submit))
        .route(
            &fmt("user/subject/voterecord/list/page/{page}/count/{count}"),
            put(u2::u2_voterecord_list),
        )
        .route(
            &fmt("user/subject/acceptreply/{id}/{replyId}"),
            get(u2::u2_subject_accept_reply),
        )
        .route(
            &fmt("user/subject/unacceptreply/{id}"),
            get(u2::u2_subject_unaccept_reply),
        )
        .route(
        ...
        .route(
            &fmt("user/subject/nonCream/{id}"),
            get(u2::u2_subject_non_cream),
        )
        .route(
            &fmt("user/subject/setOriginal/{id}"),
            get(u2::u2_subject_set_original),
        )
        .route(
            &fmt("user/subject/nonOriginal/{id}"),
            get(u2::u2_subject_non_original),
        )
        .route(
            &fmt("user/subject/setRecommendToBBSIndex/{id}"),
            get(u2::u2_subject_set_recommend_index),
        )
        .route(
            &fmt("user/subject/nonRecommendToBBSIndex/{id}"),
            get(u2::u2_subject_non_recommend_index),
        )
        .route(
            &fmt("user/subject/topToBBS/{id}"),
            get(u2::u2_subject_top_to_bbs),
        )
        .route(
            &fmt("user/subject/nonTopToBBS/{id}"),
            get(u2::u2_subject_non_top_to_bbs),
        )
        .route(
            &fmt("user/subject/topToForum/{id}"),
            get(u2::u2_subject_top_to_forum),
        )
        .route(
            &fmt("user/subject/nonTopToForum/{id}"),
            get(u2::u2_subject_non_top_to_forum),
        )
        .route(
            &fmt("user/subject/topToMainSection/{id}"),
            get(u2::u2_subject_top_to_main_section),
        )
        .route(
            &fmt("user/subject/nonTopToMainSection/{id}"),
            get(u2::u2_subject_non_top_to_main_section),
        )
        .route(
            &fmt("user/subject/topToSection/{id}"),
            get(u2::u2_subject_top_to_section),
        )
        .route(
            &fmt("user/subject/nonTopToSection/{id}"),
            get(u2::u2_subject_non_top_to_section),
        )
        .route(&fmt("user/subject/lock/{id}"), get(u2::u2_subject_lock))
        .route(&fmt("user/subject/unlock/{id}"), get(u2::u2_subject_unlock))
        .route(
            &fmt("user/subject/complete/{id}"),
            get(u2::u2_subject_complete),
        )
        .route(
            &fmt("user/subject/uncomplete/{id}"),
            get(u2::u2_subject_uncomplete),
        )
        .route(&fmt("user/subject"), post(u2::u2_subject_save))
        .route(&fmt("user/subject/{id}"), get(u2::u2_subject_get))
        .route(
            &fmt("user/subject/{id}"),
            delete(u2::u2_subject_soft_delete),
        )
        // ── userinfo（UserInfoAction，2 条）──
        .route(
            &fmt("userinfo/update/nick/name/{person}"),
            get(u2::u2_userinfo_update_nick),
        )
