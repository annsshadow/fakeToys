...
// RES的KEY 为商户MD5密钥的前16位
    ...
    private static SecretKeySpec aesKey(String key) {
        System.out.println("aesKey origin:" + key);
        byte[] bs = key.getBytes();
        if (bs.length != 16) {
            bs = Arrays.copyOf(bs, 16);// 处理数组长度为16
            try {
                System.out.print("aesKey copyOf bs:");
                System.out.write(bs);
                System.out.println();
                System.out.println("aesKey copyOf toString:" + bs.toString());
            }catch (IOException e){
                e.printStackTrace();
            }
    ...
    public static void main(String[] args) {
        System.out.println("OaMd5:" + oaMd5());
        System.out.println("----------------------------------------------------------------------------------------");
        //1.用户UserId免登Token生成
