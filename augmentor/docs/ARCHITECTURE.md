...
- 真实语料 6,902 条按 `instruction` 分层（6,531 组：371 组 2 条 + 6,160 组 1 条）：HEAD 交出
...
- **代价记 A53，未还**：摊派是每组一次分配器调用（约 2.95 µs / 组），全量 `instruction`
  端到端 5.80 → **26.07 ms（×4.49）**，`output`（916 组）2.90 → 6.12 ms（×2.11），比值对
  n/2n/3n 稳定 ⇒ 与组数线性；不分层路径 0 代价（同批实测 0.848 ms）。这条路径无 CLI/API 入口
  （`DataSplitter` 是 `__all__` 导出的公开 SDK 类），故按 L36→L37 的先例：本轮修对答案，
  代价下一轮还。
