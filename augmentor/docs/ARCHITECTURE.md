...
四处「按占比切条数」统一走 `augmentor/allocation.py:largest_remainder()`：
...
差 < 1 条；装不下的段把名额让给还有空间的段；`minimum_each` 只在名额覆盖得住时生效。

代价实测：典型 3–4 段单次 2.7–3.4 µs（被替掉的逐段下取整是 0.2 µs），可整除路径新旧同答。
**但份数才是成本变量**：分配器约 **0.4 µs/段**，而 `_stratified_sample` 的段数 = 分组键基数，
`SampleConfig.stratify_key` 默认 `"instruction"` ⇒ 真实数据集上每组只有 1 条、段数 == 行数。
按 L35 的「同值桩」口径 A/B（桩返回与真函数逐个相同的配额表）实测：`_stratified_sample`
1000 条 / 40 类 → 500 条 中位 0.195 → **0.218 ms（+11.8%）**；6902 组（每键唯一）→ 500 条
中位 4.81 → **7.83 ms（+62.7%，分配器本体 2.90 ms）**。`aggregate_weighted(5 源 × 2000 → 5000)`
同值桩 A/B 那一档：`aggregate_weighted(5 源 × 2000 → 5000)` 中位 4.33 → 4.36 ms（**+0.62%**，
分配器本体 3.94 µs；min 那一档出现 4.02 < 4.17 的**倒挂** ⇒ 只能说这一档「未测出代价」，
不能说「已证可忽略」）。**这条 +62.7% 是本轮回置换来的
真实代价，未被抵掉**，修法与代价一起记在 `OPTIMIZATION_LOOP.md` 的 A48。
**分配轮数只与份数同阶**（一段被取满即冻结），所以 `target_size` 是 1e9 还是 1e6 都在微秒级
返回（1e9 名额实测 4.27 µs）——不要写成逐条挪名额，实测那种写法在 1e6 名额 / 偏斜 `caps`
上是 67 ms，1e5 名额 / 100 段上是 327 ms，而 `target_size` 只有下界没有上界，是调用方
可达的路径。

两处刻意保留的偏心，写下来免得被当成 bug：

1. **平局按输入顺序补**，因此等权时最后一段永远拿得最少（`largest_remainder(7, [.25]*4)`
   → `[2, 2, 2, 1]`）。选「可复现」而不是「轮流公平」，因为随机平局会让同一份输入两次
   跑出不同产物。
2. **`DataSplitter._stratified_split` 不在这四处之内**：它是逐组再切三段，同一修正在组内
   套用反而更糟（实测 6 组 × 2 条：HEAD 为 `(6, 0, 6)`，test 占 50%；组内跑最大余数法变成
   `(12, 0, 0)`，val 与 test 双双为空）。正确解法要跨组对三段做全局对账，属另一口径，
   记在 `OPTIMIZATION_LOOP.md` 的 A47。
