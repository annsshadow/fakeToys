...
（**此条有已知缺陷 A55**：准入判据实读成 `total >= count` 而不是 `total >= count *
minimum_each`，故 `minimum_each >= 2` 且名额盖得住份数但盖不住保底之和时会超发 —— 实测
`lr(5, [1, 1, 1], minimum_each=2)` 交 `[2, 2, 2]`，和 6 > 5；全仓唯一调用方传 1，所以只经
SDK 直调分配器可达。见账本 Backlog）。
