...
`create_model_backend(config)` 按 `config.type` 在模块级表 `MODEL_BACKENDS` 上查表分发，
未知类型给出**列出全部合法取值**的错误信息，避免排查时反复试错。
合法取值是封闭清单 `config.MODEL_TYPES`（清单的唯一住处，工厂与静态规格表都引它，
两集相等由 `tests/unit/test_model_type_choices_l74.py` 钉住）：类型名写错在**构造期**
就抛 `DataValidationError`，因此 `load_config` 阶段即出声，不再退化成
「模型后端不可用」那条 WARNING（A115 / L74）。要记住两处同名不同物：
`ERNIEBackend` 的 `type` 取值不叫类名，出厂模板里那条条目名也不等于 `type`。

基类 `ModelBackend.generate()` 的磁盘响应缓存以 `_cache_key` 为键，键里覆盖
`type` / `model` / `temperature` / `top_p` / `max_output_tokens` 与 prompt
（少一个就会静默命中上一个配置的响应）。比例类两键走 `_ratio_token` 归一：
YAML 写 `1` 与 `1.0` 是同一个语义，归一后同键，命中率不再随字面量劈叉；
float 写法的键与归一前逐字节相同 ⇒ 收紧不作废任何已落盘的缓存（A116 / L74）。
...
> **（L71 校正）** 这一段留下的两个悬念都已在 L71 / A74 关掉，读到这里时它不再描述代码事实：
