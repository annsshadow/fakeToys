...
- **L35** （哈希由下一轮提交补） `fix(validation)` 同族第三轮 —— 把 A45 里 A44 之后剩下的 6 处「步长 / 配额 / 检索条数」旋钮接上同一处判据，并新增两条可证边界（**判参先于花钱的副作用**、**跨后端同一个旋钮必须同答**）。
