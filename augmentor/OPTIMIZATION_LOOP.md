...
- **L36** （哈希由下一轮提交补） `fix(allocation)` 新口径第一轮 —— 把「比例 → 条数」的**余数归属**从「逐份 `int(total * 比例)`」收进一处最大余数法，接上 4 个站点，并新增第 6 条主张：**让渡一次算清，代价不随 `total` 放大**。
