...
| A41 | `augmentor/quality.py:69,79`（`diversity_sample_size` 无校验） | **L32 实测（同轮扫出，但那是另一类口径，不顺手改）**：构造参数完全不校验。`=0` 时参照切片为空 → 多样性**恒返回 1.0**（实测候选与参照逐字相同仍得 1.0，oracle 是 0.0），总分按权重 0.3 虚高 **0.3** → 劣质样本被 `passed` 采纳；`-1` 走 `existing[:-1]` 尾部截断，实测在 2 条语料上恰好与 oracle 相同 —— 症状随数据变，更隐蔽。**A40 同族：静默抬高分数**。仓库 8 个构造点都不传该参数 → SDK-only。修它要一并拍「只许正整数」还是「0 = 显式关闭多样性」，属质量口径决策 | S | **L33 更新**：判据函数已就位（`validation.require_count`），本条剩下的只是把它接到 `quality.py` 的构造参数上并拍口径 —— 选 `minimum=1` 是一行代码；若选「0 = 显式关闭多样性」，则必须一并写明关闭时那一维给几分（现在给的是 **1.0**，即满分，这才是真正的坑），属质量口径决策，不能顺手。**行号漂移当场核对**：本行 position 原写 `quality.py:69,79`，当前工作树是 **68 / 78**。
...
- **L33** （哈希由下一轮提交补） `fix(validation)` 同构扫描第二次落地 —— 计数/分页旋钮的越界值从「切片换语义」改成一处判据。
