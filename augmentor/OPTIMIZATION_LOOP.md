...
- **L34** （哈希由下一轮提交补） `fix(validation)` 同族第二轮 —— `require_count` 接完剩下的 10 处窗口/条数/步长/保留数旋钮，并新增两条边界（判参先于副作用、0 在不可逆语义下不放行）。
