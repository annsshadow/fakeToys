...
| A51 | `augmentor/allocation.py:_unit_cap_quota:126`（`if left:`） | **L37 记账时由覆盖率报告抓出（本轮不改，属下一轮触碰 `allocation.py` 时顺手收）**：全量报告里 `allocation.py` 是 **72 语句 0 missed**、分支 39/40，唯一 partial 就是这一行的假分支，而且它**不可达** —— `cut = sorted(exacts, reverse=True)[total - 1]` 取的就是第 `total` 大的值，故「严格大于 `cut`」的段数 `m <= total - 1`，于是 `left = total - m >= 1` 恒成立（顶格平局那一段本身就至少贡献一个待补名额）。写法上有两种收法：直接删掉守卫（循环内的 `if left == 0: break` 已经管住退出），或把不变量写成断言。**删守卫是行为不变的**（与本轮同类），所以它必须进下一轮的注入清单而不是被顺手做掉 | S |
| A53 | `augmentor/data_splitter.py:_stratified_split`（每组一次 `largest_remainder` 摊派 + 一次全局目标计算） | **L38 新记（本轮修法自带的代价，按 L36→L37 的先例「本轮修对答案、代价下一轮还」）**：摊派是**每组一次**分配器调用，HEAD 是每组两次 `int()` 乘法，所以这是本轮引入的净新增成本。真实语料 `train_data.json`（6902 条）back-to-back、HEAD 作桩（桩实测复现 `(371, 0, 6531)` 与 `(4925, 142, 1835)` 两个缺陷读数 ⇒ 同值桩可信）：`instruction`（6531 组）端到端 5.803 → **26.072 ms（×4.49）**，`output`（916 组）2.900 → **6.120 ms（×2.11）**，比值随组数线性，摊派本体约 **2.95 µs/组**；不分层路径 0 代价（同批 0.848 ms 对照）。两条还法都可行而未选：①在 `data_splitter` 里给「同尺寸组批量摊派」写闭式解（一次算完同 n 的组，段数从组数降到尺寸基数），②给 `largest_remainder` 加「三段 + caps 全等」快路（与 A51 同一处代码，可与它同轮做）。**未做的另一原因**：本轮是行为变更轮，掺入代价优化会破坏「一轮只改一类口径」 **待办** | M |
...
- **L38** （哈希由下一轮提交补） `fix(data-splitter)` 行为变更轮 —— 把 A47 从「量过之后刻意不接」变成「接掉」：`_stratified_split` 改为跨组对三段做全局对账。
  ...
  - **缺陷实测（真实 `train_data.json`，6902 条，不是构造例）**：`instruction`（6531 组 = 371×2 + 6,160×1）HEAD `(371, 0, 6531)`，与目标 `(5522, 690, 690)` 差 `(-5151, -690, +5841)`，即名义 10% 的 test 实拿 94.6%、val 恒 0 条；`output`（916 组）HEAD `(4925, 142, 1835)`、差 `(-597, -548, +1145)`，val 只覆盖 82/916 个类别。根因与 §3.13 另外四处同源但更狠：`int(n * 0.1)` 在 `n ≤ 9` 恒为 0，而这份语料 94% 的组 `n ≤ 2`。
