...
| A51 | `augmentor/allocation.py:_unit_cap_quota:126`（`if left:`） | **L39 起该形状有两条**（`allocation.py` 分支 54/56：`125->130` 与 `168->176`，都是「补名额的 `for` 只按 `left` 个名额自减到 0 就自然走完」⇒ 循环内的 `if left == 0: break` 永不触发），收法是把循环写成 `for _, pos in keys[:left]:` 并删掉内层守卫，可证同答。**L37 记账时由覆盖率报告抓出（本轮不改，属下一轮触碰 `allocation.py` 时顺手收）**：全量报告里 `allocation.py` 是 **72 语句 0 missed**、分支 39/40，唯一 partial 就是这一行的假分支，而且它**不可达** —— `cut = sorted(exacts, reverse=True)[total - 1]` 取的就是第 `total` 大的值，故「严格大于 `cut`」的段数 `m <= total - 1`，于是 `left = total - m >= 1` 恒成立（顶格平局那一段本身就至少贡献一个待补名额）。写法上有两种收法：直接删掉守卫（循环内的 `if left == 0: break` 已经管住退出），或把不变量写成断言。**删守卫是行为不变的**（与本轮同类），所以它必须进下一轮的注入清单而不是被顺手做掉 | S |
...
- **L39** （哈希由下一轮提交补） `perf(allocation,data-splitter)` 代价轮 —— 把 A53 从「量过之后记在账上」变成「还掉七成」，行为一字不许变。
