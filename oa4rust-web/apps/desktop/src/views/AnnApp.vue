<template>
  <div class="crud-view">
    ...
    <div class="content-panel glass-card">
      <div class="toolbar">
        <input v-model="search" placeholder="搜索标题 / 分类..." class="search-input" />
        <button class="btn-refresh" @click="loadData">🔄 刷新</button>
      </div>
      <div v-if="loading" class="loading-state"><div class="skel" v-for="i in 5" :key="i"></div></div>
      <div v-else-if="items.length===0" class="empty-state"><div class="empty-icon">🧠</div><p>暂无AI公告配置</p></div>
    ...
    <div v-if="showCreate||showEdit" class="modal-overlay" @click.self="closeModal">
      <div class="modal glass-card">
        <h3>{{ showEdit?'编辑':'新建' }}AI公告配置</h3>
        <div class="form-group"><label>标题</label><input v-model="form.title" placeholder="标题" class="form-input" /></div>
        <div class="form-group"><label>分类</label><input v-model="form.category" placeholder="如 model / notice" class="form-input mono" /></div>
        <div class="form-group"><label>状态</label><input v-model="form.status" placeholder="active / disabled" class="form-input" /></div>
        <div class="form-group"><label>内容</label><textarea v-model="form.content" rows="4" placeholder="公告内容" class="form-textarea"></textarea></div>
...
function openCreate() {
  form.value = { title: '', category: '', status: 'active', content: '' }
...
function deleteItem(item: Item) {
  if (confirmMsg('确定删除该配置？')) delM.mutate(item.id)
}
function loadData() {
  qc.invalidateQueries({ queryKey: qk })
}
