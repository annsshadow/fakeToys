<template>
  <div class="crud-view">
    <div class="view-header glass-card">
      <div>
        <h1>CMS视图设计器</h1>
        <p class="subtitle">/jaxrs/cms/assemble/control/view/*（x_cms_view）</p>
      </div>
    ...
    <div class="content-panel glass-card">
      ...
      <div v-else-if="items.length===0" class="empty-state"><div class="empty-icon">🖼️</div><p>暂无视图定义</p></div>
      <table v-else class="data-table">
        <thead><tr><th>名称</th><th>应用</th><th>分类</th><th>配置</th><th>操作</th></tr></thead>
        <tbody>
          <tr v-for="item in filtered" :key="item.id">
            <td>{{ item.name||'—' }}</td>
            <td class="mono">{{ item.app_id||'—' }}</td>
            <td class="mono">{{ item.category_id||'—' }}</td>
            <td class="val-cell mono">{{ item.view_config||'—' }}</td>
            <td>
    ...
    <div v-if="showCreate||showEdit" class="modal-overlay" @click.self="closeModal">
      <div class="modal glass-card">
        <h3>{{ showEdit?'编辑':'新建' }}视图定义</h3>
        <div class="form-group"><label>名称</label><input v-model="form.name" placeholder="视图名称" class="form-input" /></div>
        <div class="form-group"><label>应用 ID</label><input v-model="form.app_id" placeholder="app_id" class="form-input mono" /></div>
        <div class="form-group"><label>分类 ID</label><input v-model="form.category_id" placeholder="category_id" class="form-input mono" /></div>
        <div class="form-group"><label>配置（JSON）</label><textarea v-model="form.view_config" rows="6" placeholder='{"columns":[...]}' class="form-textarea mono"></textarea></div>
...
const listEp = '/jaxrs/cms/assemble/control/view/list'
const createEp = '/jaxrs/cms/assemble/control/view/create'
const qk = ['CmsViewDesigner', 'list']
...
function openCreate() {
  form.value = { name: '', app_id: '', category_id: '', view_config: '' }
...
const saveM = useMutation({
  mutationFn: async (payload: Item) => {
    saving.value = true
    try {
      if (editingId.value) return api.post(`/jaxrs/cms/assemble/control/view/save/${editingId.value}`, payload)
...
const delM = useMutation({
  mutationFn: async (id: string) => api.post(`/jaxrs/cms/assemble/control/view/delete/${id}`),
  onSuccess: () => {
    qc.invalidateQueries({ queryKey: qk })
  },
})
async function deleteItem(item: Item) {
  if (await confirmMsg('确定删除该视图？')) delM.mutate(item.id)
}
function loadData() {
  qc.invalidateQueries({ queryKey: qk })
}
