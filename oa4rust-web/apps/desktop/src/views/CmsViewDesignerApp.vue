<template>
  <div class="crud-view">
    <div class="view-header glass-card">
      <div>
        <h1>CMS视图设计器</h1>
        <p class="subtitle">/api/cms/assemble/control/view/*（x_cms_view）</p>
      </div>
...
const listEp = '/api/cms/assemble/control/view/list'
const createEp = '/api/cms/assemble/control/view/create'
...
const saveM = useMutation({
  mutationFn: async (payload: Item) => {
    saving.value = true
    try {
      if (editingId.value) return api.post(`/api/cms/assemble/control/view/save/${editingId.value}`, payload)
...
const delM = useMutation({
  mutationFn: async (id: string) => api.post(`/api/cms/assemble/control/view/delete/${id}`),
  onSuccess: () => {
    qc.invalidateQueries({ queryKey: qk })
  },
})
