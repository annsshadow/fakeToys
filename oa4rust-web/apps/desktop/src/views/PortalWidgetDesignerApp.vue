<template>
  <div class="crud-view">
    <div class="view-header glass-card">
      <div>
        <h1>门户组件设计器</h1>
        <p class="subtitle">/jaxrs/portal/assemble/designer/widget/*（x_portal_widget）</p>
      </div>
      <button class="btn-primary" @click="openCreate">+ 新建</button>
    </div>
    <div class="content-panel glass-card">
      <div class="toolbar">
        <input v-model="search" placeholder="搜索名称 / 门户 / 分类..." class="search-input" />
        <button class="btn-refresh" @click="loadData">🔄 刷新</button>
      </div>
      <div v-if="loading" class="loading-state"><div class="skel" v-for="i in 5" :key="i"></div></div>
      <div v-else-if="items.length===0" class="empty-state"><div class="empty-icon">🧱</div><p>暂无门户组件</p></div>
      <table v-else class="data-table">
        <thead><tr><th>名称</th><th>门户</th><th>分类</th><th>配置摘要</th><th>操作</th></tr></thead>
        <tbody>
          <tr v-for="item in filtered" :key="item.id">
            <td>{{ item.name||'—' }}</td>
            <td class="mono">{{ item.portalId||'—' }}</td>
            <td>{{ item.category||'—' }}</td>
            <td class="val-cell mono">{{ (item.config||'').slice(0, 60)||'—' }}</td>
            <td>
    ...
    <div v-if="showCreate||showEdit" class="modal-overlay" @click.self="closeModal">
      <div class="modal glass-card">
        <h3>{{ showEdit?'编辑':'新建' }}门户组件</h3>
        <div class="form-group"><label>名称</label><input v-model="form.name" placeholder="组件名称" class="form-input" /></div>
        <div class="form-group"><label>门户 ID</label><input v-model="form.portalId" placeholder="portalId" class="form-input mono" /></div>
        <div class="form-group"><label>分类</label><input v-model="form.category" placeholder="category" class="form-input mono" /></div>
        <div class="form-group"><label>配置（JSON）</label><textarea v-model="form.config" rows="5" placeholder='{"props":{...}}' class="form-textarea mono"></textarea></div>
...
const listEp = '/jaxrs/portal/assemble/designer/widget/list'
const createEp = '/jaxrs/portal/assemble/designer/widget/create'
const qk = ['PortalWidgetDesigner', 'list']
  ...
  search.value
    ? items.value.filter(
        (i) =>
          ...
          (i.portalId || '').toLowerCase().includes(search.value.toLowerCase()) ||
...
function openCreate() {
  form.value = { name: '', portalId: '', category: '', config: '' }
...
const saveM = useMutation({
  mutationFn: async (payload: Item) => {
    saving.value = true
    try {
      if (editingId.value) return api.post(`/jaxrs/portal/assemble/designer/widget/save/${editingId.value}`, payload)
...
const delM = useMutation({
  mutationFn: async (id: string) => api.post(`/jaxrs/portal/assemble/designer/widget/delete/${id}`),
  onSuccess: () => {
    qc.invalidateQueries({ queryKey: qk })
  },
})
async function deleteItem(item: Item) {
  if (await confirmMsg('确定删除该组件？')) delM.mutate(item.id)
}
function loadData() {
  qc.invalidateQueries({ queryKey: qk })
}
