...
const filteredForms = computed(() =>
  listFilter.value
    ? forms.value.filter(
        (f) =>
          (f.name || '').toLowerCase().includes(listFilter.value.toLowerCase()) ||
          (f.flag || '').toLowerCase().includes(listFilter.value.toLowerCase()),
      )
    : forms.value,
)
...
function parseOptions(s?: string) {
  ...
  return s
    ...
    .filter(Boolean)
    .map((l) => {
      const [v, lb = v] = l.split('|')
      return { value: v.trim(), label: lb.trim() }
    })
}
...
const fieldTemplates = [
  {
    ...
    icon: '👤',
    fields: [
      { type: 'text', label: '姓名', key: 'name', required: true },
      { type: 'email', label: '邮箱', key: 'email' },
      { type: 'phone', label: '手机', key: 'phone' },
    ],
  },
  {
    name: '订单信息',
    icon: '📦',
    fields: [
      { type: 'text', label: '订单号', key: 'order_id' },
      { type: 'date', label: '日期', key: 'order_date' },
      { type: 'number', label: '金额', key: 'amount', min: 0 },
      { type: 'select', label: '状态', key: 'status', optionsStr: '待支付|待支付\n已支付|已支付\n已完成|已完成' },
    ],
  },
  {
    name: '员工信息',
    ...
    fields: [
      { type: 'text', label: '姓名', key: 'name', required: true },
      { type: 'select', label: '部门', key: 'dept', optionsStr: '技术部|技术部\n销售部|销售部\n人事部|人事部' },
      { type: 'date', label: '入职日期', key: 'hire_date' },
      { type: 'number', label: '薪资', key: 'salary', min: 0 },
    ],
  },
  {
    name: '反馈表单',
    icon: '💬',
    fields: [
      { type: 'select', label: '类型', key: 'type', optionsStr: '问题|问题反馈\n建议|建议\n其他|其他' },
      { type: 'textarea', label: '内容', key: 'content', rows: 5, required: true },
      { type: 'email', label: '联系方式', key: 'contact' },
    ],
  },
  {
    name: '调查问卷',
    ...
    fields: [
      ...
      { type: 'select', label: '年龄', key: 'age', optionsStr: '18-25|18-25岁\n26-35|26-35岁\n36-50|36-50岁' },
      { type: 'rating', label: '评分', key: 'rating', min: 1, max: 5 },
      { type: 'textarea', label: '意见', key: 'comments', rows: 3 },
    ],
  },
  {
    name: '预约表单',
    ...
    fields: [
      { type: 'text', label: '姓名', key: 'name', required: true },
      { type: 'phone', label: '手机', key: 'phone', required: true },
      { type: 'date', label: '日期', key: 'date', required: true },
      { type: 'textarea', label: '备注', key: 'notes', rows: 2 },
    ],
  },
  {
    name: '地址信息',
    icon: '📍',
    fields: [
      { type: 'text', label: '省份', key: 'province' },
      { type: 'text', label: '城市', key: 'city' },
      { type: 'textarea', label: '详细地址', key: 'address', rows: 3 },
      { type: 'text', label: '邮编', key: 'zip' },
    ],
  },
  {
    name: '登录表单',
    ...
    fields: [
      ...
      { type: 'text', label: '密码', key: 'password' },
      { type: 'checkbox', label: '记住我', key: 'remember' },
    ],
  },
  {
    name: '注册表单',
    ...
    fields: [
      ...
      { type: 'email', label: '邮箱', key: 'email', required: true },
      { type: 'text', label: '密码', key: 'password', required: true },
      { type: 'checkbox', label: '同意条款', key: 'agree', required: true },
    ],
  },
  {
    name: '联系人',
    ...
    fields: [
      ...
      { type: 'phone', label: '手机', key: 'phone' },
      { type: 'email', label: '邮箱', key: 'email' },
      { type: 'text', label: '公司', key: 'company' },
      { type: 'textarea', label: '备注', key: 'note', rows: 2 },
    ],
  },
]
...
// --- Column Layout ---
function setColumnCount(n: 1 | 2 | 3) {
  columnCount.value = n
}
...
function removeValidationRule(idx: number) {
  validationRules.value.splice(idx, 1)
}
...
const newAccessRule = ref<{ fieldKey: string; role: string; action: string }>({
  fieldKey: '',
  role: 'admin',
  action: 'show',
})
...
const layoutConfig = ref<FormLayoutConfig>({
  columns: 1,
  gutter: 16,
  align: 'left',
  labelWidth: 100,
...
const validationSummary = ref<{ valid: boolean; errors: number; warnings: number }>({
  valid: true,
  errors: 0,
  warnings: 0,
})
...
}>({ total: 0, byType: {}, withValidation: 0, withCondition: 0, requiredCount: 0 })
const auditLogs = ref<FormAuditLog[]>([])
const showAuditPanel = ref(false)
const fieldOptionsCache = ref<Record<string, FieldOption[]>>({})
// ── Advanced Functions ─────────────────────────────────────────────
function toggleFieldProps(fieldKey: string) {
  fieldPropsExpanded.value[fieldKey] = !fieldPropsExpanded.value[fieldKey]
}
function getFieldTypeStats(): Record<string, number> {
...
function updateFieldStats() {
  ...
  fields.forEach((f) => {
    ...
    if (f.validation?.required) requiredCount++
    if (f.validation) withValidation++
    if (f.condition) withCondition++
  })
  formFieldStats.value = { total: fields.length, byType, withValidation, withCondition, requiredCount }
}
function searchFields(query: string) {
  searchFieldQuery.value = query
  if (!query.trim()) return []
  return (currentForm.value?.fields || []).filter(
    (f) =>
...
function bulkApplyAction(action: string, value: string) {
  if (!currentForm.value) return
  const fields = currentForm.value.fields
  if (action === 'setRequired') {
    fields.forEach((f) => {
      if (f.validation) f.validation.required = value === 'true'
    })
  ...
  pushFormHistory()
  showToast('批量操作已应用', 'success')
}
function generateConditionTree(): ConditionTree {
  return {
    id: genId(),
    type: 'group',
    logic: 'AND',
    children: (currentForm.value?.fields || [])
      ...
      .map((f) => ({
        ...
        logic: 'AND',
        conditions: [
          { field: f.condition?.field || '', op: f.condition?.operator || '=', value: f.condition?.value || '' },
        ],
      })),
  }
}
function evaluateConditionTree(tree: ConditionTree | null, data: Record<string, any>): boolean {
  if (!tree) return true
  if (tree.type === 'condition' && tree.conditions) {
    return tree.conditions.every((c) => {
      const val = data[c.field]
      if (c.op === '=') return String(val) === c.value
      if (c.op === '!=') return String(val) !== c.value
  ...
  if (tree.type === 'group') {
    const results = tree.children?.map((c) => evaluateConditionTree(c, data)) || []
    return tree.logic === 'AND' ? results.every((r) => r) : results.some((r) => r)
  }
  return true
}
function addAccessRule(fieldKey: string, role: string, action: string) {
  accessRules.value.push({ fieldKey, role, action: action as 'show' | 'hide' | 'readonly' })
}
function removeAccessRule(idx: number) {
  accessRules.value.splice(idx, 1)
}
function getAccessAction(fieldKey: string, role: string): 'show' | 'hide' | 'readonly' | null {
  const rule = accessRules.value.find((r) => r.fieldKey === fieldKey && r.role === role)
  return rule?.action || null
}
function addDataSource(fieldKey: string, url: string, method: string, keyField: string, labelField: string) {
  dataSources.value.push({ fieldKey, url, method, keyField, labelField })
}
function removeDataSource(idx: number) {
  dataSources.value.splice(idx, 1)
}
function openConditionTree() {
  showConditionTree.value = true
  conditionTreeRoot.value = generateConditionTree()
}
function addConditionNode(parentId: string) {
  if (!conditionTreeRoot.value) return
  const addChildren = (node: ConditionTree): void => {
  ...
  addChildren(conditionTreeRoot.value)
}
function addGroupNode(parentId: string) {
  if (!conditionTreeRoot.value) return
  const addChildren = (node: ConditionTree): void => {
    if (node.id === parentId && node.children) {
      node.children.push({ id: genId(), type: 'group', logic: 'AND', children: [] })
    } else if (node.children) {
      node.children.forEach(addChildren)
    }
  }
...
function removeConditionNode(nodeId: string) {
  if (!conditionTreeRoot.value) return
  const remove = (node: ConditionTree): boolean => {
    if (node.children) {
  ...
  remove(conditionTreeRoot.value)
}
...
function validateAllFields(): { valid: boolean; errors: Record<string, string> } {
  const errors: Record<string, string> = {}
  let valid = true
  currentForm.value?.fields.forEach((f) => {
    const fieldErrors = validateField(f)
    if (fieldErrors.length > 0) {
      errors[f.key || ''] = fieldErrors.join('; ')
      valid = false
    }
  })
  return { valid, errors }
...
function simulateSubmit() {
  ...
  if (!result.valid) {
    ...
    showToast('表单验证失败', 'error')
    return
  }
  submissionResult.value = { status: 'success', data: previewData.value, errors: {} }
  showToast('提交成功!', 'success')
}
function exportForm(format: 'json' | 'yaml' | 'html') {
  if (!currentForm.value) return
  let output = ''
  if (format === 'json') {
    output = JSON.stringify(currentForm.value, null, 2)
  } else if (format === 'yaml') {
    output = JSON.stringify(currentForm.value, null, 2)
  } else {
    output = generateHTMLForm()
  }
  exportResult.value = output
  showExportModal.value = true
  exportFormat.value = format
}
function generateHTMLForm(): string {
  if (!currentForm.value) return ''
...
function exportFormData() {
  const data =
    currentForm.value?.fields.reduce(
      (acc, f) => {
...
function deleteField(idx: number) {
  if (!currentForm.value || idx < 0) return
  currentForm.value.fields.splice(idx, 1)
  if (selectedField.value && selectedField.value.key === currentForm.value?.fields[idx]?.key) selectedField.value = null
  pushFormHistory()
  showToast('字段已删除', 'warning')
}
function toggleSectionField(sectionId: string, fieldKey: string) {
  const section = sections.value.find((s) => s.id === sectionId)
  if (!section) return
  const idx = section.fields.indexOf(fieldKey)
  if (idx >= 0) section.fields.splice(idx, 1)
  else section.fields.push(fieldKey)
}
function toggleTabField(tabId: string, fieldKey: string) {
  const tab = tabs.value.find((t) => t.id === tabId)
  if (!tab) return
  const idx = tab.fields.indexOf(fieldKey)
  if (idx >= 0) tab.fields.splice(idx, 1)
  else tab.fields.push(fieldKey)
}
function getFieldInSection(sectionId: string, fieldKey: string): boolean {
  const section = sections.value.find((s) => s.id === sectionId)
  return section?.fields.includes(fieldKey) || false
}
function getFieldInTab(tabId: string, fieldKey: string): boolean {
  const tab = tabs.value.find((t) => t.id === tabId)
  return tab?.fields.includes(fieldKey) || false
}
function getFormDataPreview(): Record<string, any> {
...
function resetFieldDefaults() {
  ...
  previewData.value = getFormDataPreview()
}
...
function filterFieldPicker(query: string) {
  fieldPickerSearch.value = query
}
function onFieldDragEnd() {
  fieldDragSource.value = null
}
function getFormFieldStats(): {
  total: number
  byType: Record<string, number>
  required: number
  withValidation: number
  ...
  updateFieldStats()
  return {
    total: formFieldStats.value.total,
    byType: formFieldStats.value.byType,
    required: formFieldStats.value.requiredCount,
    withValidation: formFieldStats.value.withValidation,
    withCondition: formFieldStats.value.withCondition,
  }
}
...
function exportDocumentation() {
  const doc = generateFormDocumentation()
  exportResult.value = doc
  showExportModal.value = true
  exportFormat.value = 'json'
}
...
function logAuditEvent(action: string, fieldKey?: string, oldValue?: string, newValue?: string) {
  auditLogs.value.push({ id: genId(), timestamp: Date.now(), user: 'current', action, fieldKey, oldValue, newValue })
}
function addFieldToSection(sectionId: string, fieldKey: string) {
  const section = sections.value.find((s) => s.id === sectionId)
  if (section && !section.fields.includes(fieldKey)) section.fields.push(fieldKey)
}
function addFieldToTab(tabId: string, fieldKey: string) {
  const tab = tabs.value.find((t) => t.id === tabId)
  if (tab && !tab.fields.includes(fieldKey)) tab.fields.push(fieldKey)
}
function getSectionFieldCount(sectionId: string): number {
  const section = sections.value.find((s) => s.id === sectionId)
  return section?.fields.length || 0
}
function getTabFieldCount(tabId: string): number {
  const tab = tabs.value.find((t) => t.id === tabId)
  return tab?.fields.length || 0
}
function buildFieldOptions(field: FormField): FieldOption[] {
  if (field.options) {
    try {
  ...
  if (field.dataSource) {
    return fieldOptionsCache.value[field.dataSource] || []
  }
  return []
}
...
const perfStats = ref<{ renderTime: number; fieldCount: number; validationTime: number; errorRate: number }>({
  renderTime: 0,
  fieldCount: 0,
  validationTime: 0,
  errorRate: 0,
...
function measurePerformance(): void {
  ...
  perfStats.value = {
    ...
    fieldCount: currentForm.value.fields.length,
...
function exportSchemaAsJSON(): void {
  ...
  currentForm.value.fields.forEach((f) => {
    ...
    if (f.validation?.required) {
      if (!props[f.key]) props[f.key] = {}
      props[f.key].required = true
    }
...
interface DepEdge {
  from: string
  to: string
  type: 'calc' | 'visible' | 'disable' | 'required'
  label: string
}
...
const currentIntlField = ref(''),
  intlMode = ref<'zh' | 'en' | 'ja' | 'ko'>('en'),
  intlText = ref('')
...
// -- Advanced Functions
function openCrossFieldPanel(): void {
  showCrossFieldPanel.value = true
}
function addCrossFieldCalc(): void {
  ...
  newCalcSource.value = ''
  newCalcFormula.value = ''
  newCalcTarget.value = ''
}
function removeCrossFieldCalc(id: string): void {
  crossFieldCalcs.value = crossFieldCalcs.value.filter((c) => c.id !== id)
}
function toggleCrossFieldCalc(id: string): void {
  const c = crossFieldCalcs.value.find((x) => x.id === id)
  if (c) c.enabled = !c.enabled
}
function openDataSourcePanel(): void {
...
function openA11yPanel(): void {
  ...
  if (currentForm) {
    currentForm.fields.forEach((field) => {
      if (!field.label)
        issues.push({ fieldId: field.id, issue: '缺少字段标签', severity: 'high', suggestion: '为字段添加描述性标签' })
      if (field.type === 'select' && !field.options)
        issues.push({ fieldId: field.id, issue: '下拉字段无选项定义', severity: 'medium', suggestion: '添加选项配置' })
    })
    if (issues.length === 0)
      issues.push({ fieldId: 'form', issue: '所有字段符合无障碍标准', severity: 'low', suggestion: '' })
  }
  a11yIssues.value = issues
}
function openDepGraphPanel(): void {
  showDepGraphPanel.value = true
  buildDepEdges()
}
...
function openJsonSchemaPanel(): void {
  showJsonSchemaPanel.value = true
  exportJsonSchema()
}
...
function openFieldDocsPanel(): void {
  showFieldDocsPanel.value = true
  generateFieldDocs()
}
...
const validationHistory = ref<
  Array<{ id: string; timestamp: number; valid: boolean; errors: number; warnings: number }>
>([])
const showComparePanel = ref(false)
const compareFormA = ref(''),
  compareFormB = ref(''),
  compareResult = ref({ added: 0, removed: 0, modified: 0 })
...
const publishVersions = ref<
  Array<{ id: string; version: string; publishedAt: number; publishedBy: string; status: string }>
>([])
...
// -- Extended Functions
function openBulkEditPanel(): void {
  showBulkEditPanel.value = true
}
function applyBulkEdit(): void {
  ...
  f.fields.forEach((field) => {
    if (bulkEditField.value === '' || field.key === bulkEditField.value) {
      if (bulkEditAction.value === 'required')
        field.validation = { ...field.validation, required: bulkEditValue.value === 'true' }
...
function doSearch(): void {
  if (!f || !searchQuery.value.trim()) {
    searchResults.value = []
    return
  }
  searchResults.value = f.fields.filter(
    (x) =>
...
function runCompare(): void {
  const f1 = forms.value.find((x) => x.id === compareFormA.value),
    f2 = forms.value.find((x) => x.id === compareFormB.value)
  if (!f1 || !f2) return
  const s1 = new Set(f1.fields.map((f) => f.key)),
    s2 = new Set(f2.fields.map((f) => f.key))
  compareResult.value = {
    added: [...s2].filter((k) => !s1.has(k)).length,
    removed: [...s1].filter((k) => !s2.has(k)).length,
    modified: f1.fields.filter((f) => f2.fields.some((g) => g.key === f.key && f.label !== g.label)).length,
  }
}
function openRestorePanel(): void {
...
function lockField(fid: string, fname: string): void {
  lockedFields.value.push({ fieldId: fid, fieldName: fname, lockedBy: '当前用户', lockedAt: Date.now() })
}
function unlockField(fid: string): void {
  lockedFields.value = lockedFields.value.filter((f) => f.fieldId !== fid)
}
...
function generatePreview(): void {
  ...
  previewData.value = {}
  f.fields.forEach((fd) => {
    previewData.value[fd.key || ''] = fd.defaultValue || ''
  })
}
function openValidatePanel(): void {
  showValidatePanel.value = true
  runValidate()
}
function runValidate(): void {
  ...
  f.fields.forEach((fd) => {
    if (fd.validation?.required && !fd.defaultValue)
      errs.push({ field: fd.key || '', error: '必填字段为空', severity: 'error' })
    if (fd.validation?.minLength && fd.defaultValue && fd.defaultValue.length < fd.validation.minLength)
      errs.push({ field: fd.key || '', error: '长度不足', severity: 'warning' })
  })
  validateErrors.value = errs
  recordValidation(
    errs.filter((e) => e.severity === 'error').length === 0,
    errs.filter((e) => e.severity === 'error').length,
    errs.filter((e) => e.severity === 'warning').length,
  )
}
</script>
<style scoped>
