<template>
  <div class="crud-view">
    <div class="view-header glass-card">
      <div>
        <h1>流程 XForm</h1>
        <p class="subtitle">/jaxrs/processplatform/assemble/designer/xform/*（x_pp_xform，迁移 097）</p>
      </div>
      <button class="btn-primary" @click="openCreate">+ 新建</button>
    </div>
    <div class="content-panel glass-card">
      <div class="toolbar">
        <input v-model="search" placeholder="搜索名称 / 状态..." class="search-input" />
        <button class="btn-refresh" @click="loadData">🔄 刷新</button>
      </div>
      <div v-if="loading" class="loading-state"><div class="skel" v-for="i in 5" :key="i"></div></div>
      <div v-else-if="items.length===0" class="empty-state"><div class="empty-icon">🧮</div><p>暂无流程 XForm</p></div>
      <table v-else class="data-table">
        <thead><tr><th>名称</th><th>状态</th><th>定义摘要</th><th>更新时间</th><th>操作</th></tr></thead>
        <tbody>
          <tr v-for="item in filtered" :key="item.id">
            <td>{{ item.name||'—' }}</td>
            <td>{{ item.status||'draft' }}</td>
            <td class="val-cell mono">{{ (item.definition||'').slice(0, 60)||'—' }}</td>
    ...
    <div v-if="showCreate||showEdit" class="modal-overlay" @click.self="closeModal">
      <div class="modal glass-card">
        <h3>{{ showEdit?'编辑':'新建' }}流程 XForm</h3>
        <div class="form-group"><label>名称</label><input v-model="form.name" placeholder="XForm 名称" class="form-input" /></div>
        <div class="form-group"><label>状态</label><input v-model="form.status" placeholder="draft / published" class="form-input mono" /></div>
        <div class="form-group"><label>定义（JSON）</label><textarea v-model="form.definition" rows="6" placeholder='{"moduleList":{...},"layouts":{...}}' class="form-textarea mono"></textarea></div>
...
const listEp = '/jaxrs/processplatform/assemble/designer/xform/list'
const createEp = '/jaxrs/processplatform/assemble/designer/xform/create'
const qk = ['ProcessXform', 'list']
  ...
  search.value
    ? items.value.filter(
        (i) =>
          ...
          (i.status || '').toLowerCase().includes(search.value.toLowerCase()),
      )
    : items.value,
)

function openCreate() {
  form.value = { name: '', status: 'draft', definition: '' }
  editingId.value = null
  showCreate.value = true
}
function editItem(item: Item) {
  form.value = { name: item.name, status: item.status, definition: item.definition }
...
const saveM = useMutation({
  mutationFn: async (payload: Item) => {
    ...
    try {
      if (editingId.value)
        return api.post(`/jaxrs/processplatform/assemble/designer/xform/save/${editingId.value}`, payload)
...
const delM = useMutation({
  mutationFn: async (id: string) => api.post(`/jaxrs/processplatform/assemble/designer/xform/delete/${id}`),
  onSuccess: () => {
    qc.invalidateQueries({ queryKey: qk })
  },
})
async function deleteItem(item: Item) {
  if (await confirmMsg('确定删除该 XForm？')) delM.mutate(item.id)
}
function loadData() {
  qc.invalidateQueries({ queryKey: qk })
}
