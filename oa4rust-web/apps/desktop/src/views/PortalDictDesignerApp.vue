<template>
  <div class="crud-view">
    ...
    <div class="content-panel glass-card">
      <div class="toolbar">
        <input v-model="search" placeholder="搜索名称 / 应用..." class="search-input" />
        <button class="btn-refresh" @click="loadData">🔄 刷新</button>
      </div>
      <div v-if="loading" class="loading-state"><div class="skel" v-for="i in 5" :key="i"></div></div>
      <div v-else-if="items.length===0" class="empty-state"><div class="empty-icon">📗</div><p>暂无门户字典</p></div>
      <table v-else class="data-table">
        <thead><tr><th>名称</th><th>应用</th><th>数据</th><th>操作</th></tr></thead>
        <tbody>
          <tr v-for="item in filtered" :key="item.id">
            <td>{{ item.name||'—' }}</td>
            <td class="mono">{{ item.appName||'—' }}</td>
            <td class="val-cell mono">{{ item.appData||'—' }}</td>
            <td>
    ...
    <div v-if="showCreate||showEdit" class="modal-overlay" @click.self="closeModal">
      <div class="modal glass-card">
        <h3>{{ showEdit?'编辑':'新建' }}门户字典</h3>
        <div class="form-group"><label>名称</label><input v-model="form.name" placeholder="字典名称" class="form-input" /></div>
        <div class="form-group"><label>应用</label><input v-model="form.appName" placeholder="appName" class="form-input mono" /></div>
        <div class="form-group"><label>数据（JSON）</label><textarea v-model="form.appData" rows="5" placeholder='{"options":[...]}' class="form-textarea mono"></textarea></div>
...
function openCreate() {
  form.value = { name: '', appName: '', appData: '' }
