<template>
  <div class="crud-view">
    <div class="view-header glass-card">
      <div>
        <h1>笔记管理</h1>
        <p class="subtitle">/jaxrs/cms/core/entity/note/*</p>
      </div>
      <button class="btn-primary" @click="openCreate">+ 新建</button>
    </div>
    <div class="content-panel glass-card">
      <div class="toolbar">
        <input v-model="search" placeholder="搜索标题 / 内容..." class="search-input" />
        <button class="btn-refresh" @click="loadData">🔄 刷新</button>
      </div>
      <div v-if="loading" class="loading-state"><div class="skel" v-for="i in 5" :key="i"></div></div>
      <div v-else-if="items.length===0" class="empty-state"><div class="empty-icon">📓</div><p>暂无笔记</p></div>
      <table v-else class="data-table">
        <thead><tr><th>标题</th><th>内容摘要</th><th>成员</th><th>更新时间</th><th>操作</th></tr></thead>
        <tbody>
          <tr v-for="item in filtered" :key="item.id">
            <td>{{ item.title||'—' }}</td>
            <td class="desc-cell">{{ item.content||'—' }}</td>
    ...
    <div v-if="showCreate||showEdit" class="modal-overlay" @click.self="closeModal">
      <div class="modal glass-card">
        <h3>{{ showEdit?'编辑':'新建' }}笔记</h3>
        <div class="form-group"><label>标题</label><input v-model="form.title" placeholder="标题" class="form-input" /></div>
        <div class="form-group"><label>内容</label><textarea v-model="form.content" rows="4" placeholder="笔记内容" class="form-textarea"></textarea></div>
...
const listEp = '/jaxrs/cms/core/entity/note/list'
const createEp = '/jaxrs/cms/core/entity/note/create'
const qk = ['Note', 'list']
  ...
  search.value
    ? items.value.filter(
        (i) =>
          ...
          (i.content || '').toLowerCase().includes(search.value.toLowerCase()),
      )
    : items.value,
)

function openCreate() {
  form.value = { title: '', content: '', personId: '' }
...
const saveM = useMutation({
  mutationFn: async (payload: Item) => {
    saving.value = true
    try {
      if (editingId.value) return api.put(`/jaxrs/cms/core/entity/note/save/${editingId.value}`, payload)
...
const delM = useMutation({
  mutationFn: async (id: string) => api.post(`/jaxrs/cms/core/entity/note/delete/${id}`),
  onSuccess: () => {
    qc.invalidateQueries({ queryKey: qk })
  },
})
async function deleteItem(item: Item) {
  if (await confirmMsg('确定删除该笔记？')) delM.mutate(item.id)
}
function loadData() {
  qc.invalidateQueries({ queryKey: qk })
}
