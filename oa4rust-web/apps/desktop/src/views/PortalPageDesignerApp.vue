  ...
  <div class="crud-view">
    ...
    <div class="content-panel glass-card">
      ...
      <table v-else class="data-table">
        <thead><tr><th>名称</th><th>分类</th><th>内容摘要</th><th>操作</th></tr></thead>
        <tbody>
          <tr v-for="item in filtered" :key="item.id">
            <td>{{ item.name||'—' }}</td>
            <td class="mono">{{ item.category||'—' }}</td>
            <td class="val-cell mono">{{ contentPreview(item.content) }}</td>
            <td>
    ...
    <div v-if="showCreate||showEdit" class="modal-overlay" @click.self="closeModal">
      <div class="modal glass-card">
        ...
        <p v-if="contentError" class="form-error">JSON 解析失败：{{ contentError }}</p>
        <div class="modal-actions">
          <button class="btn-cancel" @click="closeModal">取消</button>
          <button class="btn-save" :disabled="!form.name?.trim()||!!contentError||saving" @click="saveItem">{{ saving?'保存中…':'保存' }}</button>
        </div>
      </div>
    </div>
  </div>
...
async function loadPortalMeta2() {
  try {
    ...
    const [cat, out, portalCat] = await Promise.all([
      api.get('/api/portal/assemble/designer/templatepage/list/category'),
      api.get('/api/portal/assemble/designer/output/list'),
      api.get('/api/portal/assemble/designer/portalcategory/list'),
    ])
    const n = (r: any) => (Array.isArray(r?.data) ? r.data.length : 0)
...
const saveM = useMutation({
  mutationFn: async () => {
    saving.value = true
    try {
      // U2 类型化 handler：create 收 {name, category, content:JSON}；save 收 {content:JSON}。
...
async function deleteItem(item: Item) {
  if (await confirmMsg('确定删除该页面？')) delM.mutate(item.id)
}
function loadData() {
  qc.invalidateQueries({ queryKey: qk })
}
...
.form-error{font-size:12px;color:var(--color-danger);margin:0}
