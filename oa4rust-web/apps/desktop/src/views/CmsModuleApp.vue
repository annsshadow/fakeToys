<template>
  <div class="crud-view">
    <div class="view-header glass-card">
      <div>
        <h1>CMS模块管理</h1>
        <p class="subtitle">/jaxrs/cms/core/entity/module/*</p>
      </div>
      <button class="btn-primary" @click="openCreate">+ 新建</button>
    </div>
    <div class="content-panel glass-card">
      <div class="toolbar">
        <input v-model="search" placeholder="搜索模块 / 应用..." class="search-input" />
        <button class="btn-refresh" @click="loadData">🔄 刷新</button>
      </div>
      <div v-if="loading" class="loading-state"><div class="skel" v-for="i in 5" :key="i"></div></div>
      <div v-else-if="items.length===0" class="empty-state"><div class="empty-icon">📦</div><p>暂无模块</p></div>
      <table v-else class="data-table">
        <thead><tr><th>名称</th><th>应用</th><th>类型</th><th>状态</th><th>更新时间</th><th>操作</th></tr></thead>
        <tbody>
    ...
    <div v-if="showCreate||showEdit" class="modal-overlay" @click.self="closeModal">
      <div class="modal glass-card">
        ...
        <div class="form-group"><label>模块类型</label><input v-model="form.moduleType" placeholder="如 content / view" class="form-input" /></div>
...
const listEp = '/jaxrs/cms/core/entity/module/list'
const createEp = '/jaxrs/cms/core/entity/module/create'
...
function openCreate() {
  form.value = { name: '', appId: '', moduleType: '', description: '' }
...
const saveM = useMutation({
  mutationFn: async (payload: Item) => {
    saving.value = true
    try {
      if (editingId.value) return api.put(`/jaxrs/cms/core/entity/module/save/${editingId.value}`, payload)
...
const delM = useMutation({
  mutationFn: async (id: string) => api.post(`/jaxrs/cms/core/entity/module/delete/${id}`),
  onSuccess: () => {
    qc.invalidateQueries({ queryKey: qk })
  },
})
async function deleteItem(item: Item) {
  if (await confirmMsg('确定删除该模块？')) delM.mutate(item.id)
}
function loadData() {
  qc.invalidateQueries({ queryKey: qk })
}
