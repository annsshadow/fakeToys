<template>
  <div class="crud-view">
    ...
    <div class="content-panel glass-card">
      <div class="toolbar">
        <input v-model="search" placeholder="搜索名称 / 模型标识..." class="search-input" />
        <button class="btn-refresh" @click="loadData">🔄 刷新</button>
      </div>
      <div v-if="loading" class="loading-state"><div class="skel" v-for="i in 5" :key="i"></div></div>
      <div v-else-if="items.length===0" class="empty-state"><div class="empty-icon">📥</div><p>暂无导入模型</p></div>
      <table v-else class="data-table">
        <thead><tr><th>名称</th><th>模型标识</th><th>查询标识</th><th>创建时间</th><th>操作</th></tr></thead>
        <tbody>
          <tr v-for="item in filtered" :key="item.id">
            <td>{{ item.name||'—' }}</td>
            <td class="mono">{{ item.modelFlag||'—' }}</td>
            <td class="mono">{{ item.queryFlag||'—' }}</td>
    ...
    <div v-if="showCreate||showEdit" class="modal-overlay" @click.self="closeModal">
      <div class="modal glass-card">
        <h3>{{ showEdit?'编辑':'新建' }}导入模型</h3>
        <div class="form-group"><label>名称</label><input v-model="form.name" placeholder="模型名称" class="form-input" /></div>
        <div class="form-group"><label>模型标识</label><input v-model="form.modelFlag" placeholder="model_flag" class="form-input mono" /></div>
        <div class="form-group"><label>查询标识</label><input v-model="form.queryFlag" placeholder="query_flag" class="form-input mono" /></div>
        <div class="form-group"><label>内容</label><textarea v-model="form.content" rows="4" placeholder="导入内容 / 映射定义" class="form-textarea mono"></textarea></div>
  ...
  search.value
    ? items.value.filter(
        (i) =>
          ...
          (i.modelFlag || '').toLowerCase().includes(search.value.toLowerCase()),
      )
    : items.value,
)

function openCreate() {
  form.value = { name: '', modelFlag: '', queryFlag: '', content: '' }
...
async function deleteItem(item: Item) {
  if (await confirmMsg('确定删除该导入模型？')) delM.mutate(item.id)
}
function loadData() {
  qc.invalidateQueries({ queryKey: qk })
}
