<template>
  <div class="crud-view">
    <div class="view-header glass-card">
      <div>
        <h1>流程字典设计器</h1>
        <p class="subtitle">/jaxrs/processplatform/assemble/designer/dict/*（pp_e_applicationdict）</p>
      </div>
      <button class="btn-primary" @click="openCreate">+ 新建</button>
    </div>
    <div class="content-panel glass-card">
      <div class="toolbar">
        <input v-model="search" placeholder="搜索字典 / 应用..." class="search-input" />
        <button class="btn-refresh" @click="loadData">🔄 刷新</button>
      </div>
      <div v-if="loading" class="loading-state"><div class="skel" v-for="i in 5" :key="i"></div></div>
      <div v-else-if="items.length===0" class="empty-state"><div class="empty-icon">📖</div><p>暂无流程字典</p></div>
      <table v-else class="data-table">
        <thead><tr><th>名称</th><th>应用</th><th>更新时间</th><th>操作</th></tr></thead>
        <tbody>
          <tr v-for="item in filtered" :key="item.id">
            <td>{{ item.name||'—' }}</td>
            <td class="mono">{{ item.application||'—' }}</td>
    ...
    <div v-if="showCreate||showEdit" class="modal-overlay" @click.self="closeModal">
      <div class="modal glass-card">
        <h3>{{ showEdit?'编辑':'新建' }}流程字典</h3>
        <div class="form-group"><label>名称</label><input v-model="form.name" placeholder="字典名称" class="form-input" /></div>
        <div class="form-group"><label>应用标识</label><input v-model="form.application" placeholder="application flag" class="form-input mono" /></div>
...
const listEp = '/jaxrs/processplatform/assemble/designer/dict/list'
const createEp = '/jaxrs/processplatform/assemble/designer/dict/create'
const qk = ['ProcessDictDesigner', 'list']
  ...
  search.value
    ? items.value.filter(
        (i) =>
          ...
          (i.application || '').toLowerCase().includes(search.value.toLowerCase()),
      )
    : items.value,
)

function openCreate() {
  form.value = { name: '', application: '' }
  editingId.value = null
  showCreate.value = true
}
function editItem(item: Item) {
  form.value = { name: item.name, application: item.application }
...
const saveM = useMutation({
  mutationFn: async (payload: Item) => {
    ...
    try {
      if (editingId.value)
        return api.post(`/jaxrs/processplatform/assemble/designer/dict/save/${editingId.value}`, payload)
...
const delM = useMutation({
  mutationFn: async (id: string) => api.post(`/jaxrs/processplatform/assemble/designer/dict/delete/${id}`),
  onSuccess: () => {
    qc.invalidateQueries({ queryKey: qk })
  },
})
async function deleteItem(item: Item) {
  if (await confirmMsg('确定删除该字典？')) delM.mutate(item.id)
}
function loadData() {
  qc.invalidateQueries({ queryKey: qk })
}
