<template>
  <div class="crud-view">
    <div class="view-header glass-card">
      <div>
        <h1>CMS 栏目管理</h1>
        <p class="subtitle">/jaxrs/cms/core/entity/column/*</p>
      </div>
      <button class="btn-primary" @click="openCreate">+ 新建栏目</button>
    </div>
    <div class="content-panel glass-card">
      <div class="toolbar">
        <input v-model="search" placeholder="搜索栏目名称..." class="search-input" />
        <button class="btn-refresh" @click="loadData">🔄 刷新</button>
      </div>
      <div v-if="loading" class="loading-state"><div class="skel" v-for="i in 5" :key="i"></div></div>
      <div v-else-if="items.length === 0" class="empty-state"><div class="empty-icon">📏</div><p>暂无栏目，点击"新建栏目"创建</p></div>
      <table v-else class="data-table">
        <thead><tr><th>名称</th><th>上级</th><th>排序</th><th>状态</th><th>更新时间</th><th>操作</th></tr></thead>
        <tbody>
          <tr v-for="item in filtered" :key="item.id">
            <td>{{ item.name || '—' }}</td>
            <td class="mono">{{ item.parentId || '（顶级）' }}</td>
    ...
    <div v-if="showCreate || showEdit" class="modal-overlay" @click.self="closeModal">
      <div class="modal glass-card">
        <h3>{{ showEdit ? '编辑栏目' : '新建栏目' }}</h3>
        <div class="form-group"><label>名称</label><input v-model="form.name" placeholder="栏目名称" class="form-input" /></div>
        <div class="form-group"><label>上级栏目 ID（可选）</label><input v-model="form.parentId" placeholder="parent_id" class="form-input mono" /></div>
        <div class="form-group"><label>排序</label><input v-model="form.sortOrder" type="number" placeholder="sort_order" class="form-input" /></div>
...
const listEp = '/jaxrs/cms/core/entity/column/list'
const createEp = '/jaxrs/cms/core/entity/column/create'
const qk = ['cms_Column', 'list']
...
const { data } = useQuery({
  ...
  queryFn: async () => {
    ...
    try {
      ...
      const rows = Array.isArray(r?.data) ? (r.data as Item[]) : []
      return rows
    } finally {
      loading.value = false
    }
...
function openCreate() {
  form.value = { name: '', parentId: '', sortOrder: 0, status: 'active', description: '' }
...
const saveM = useMutation({
  mutationFn: async (payload: Item) => {
    saving.value = true
    try {
      if (editingId.value) return api.put(`/jaxrs/cms/core/entity/column/save/${editingId.value}`, payload)
...
const delM = useMutation({
  mutationFn: async (id: string) => api.post(`/jaxrs/cms/core/entity/column/delete/${id}`),
  onSuccess: () => {
    qc.invalidateQueries({ queryKey: qk })
  },
})
async function deleteItem(item: Item) {
  if (await confirmMsg('确定删除该栏目？')) delM.mutate(item.id)
}
function loadData() {
  qc.invalidateQueries({ queryKey: qk })
}
