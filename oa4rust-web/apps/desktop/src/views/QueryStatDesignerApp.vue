...
<template>
  <div class="crud-view">
    ...
    <div class="content-panel glass-card">
      <div class="toolbar">
        <input v-model="search" placeholder="搜索名称 / 查询标识 / 统计类型..." class="search-input" />
        <button class="btn-refresh" @click="loadData">🔄 刷新</button>
      </div>
      <div v-if="loading" class="loading-state"><div class="skel" v-for="i in 5" :key="i"></div></div>
      <div v-else-if="items.length===0" class="empty-state"><div class="empty-icon">📊</div><p>暂无统计定义</p></div>
      <table v-else class="data-table">
        <thead><tr><th>名称</th><th>查询标识</th><th>统计类型</th><th>创建时间</th><th>操作</th></tr></thead>
        <tbody>
    ...
    <div v-if="showCreate||showEdit" class="modal-overlay" @click.self="closeModal">
      <div class="modal glass-card">
        <h3>{{ showEdit?'编辑':'新建' }}统计定义</h3>
        <div class="form-group"><label>名称</label><input v-model="form.name" placeholder="统计名称" class="form-input" /></div>
        <div class="form-group"><label>查询标识</label><input v-model="form.queryFlag" placeholder="query_flag" class="form-input mono" /></div>
        <div class="form-group"><label>统计类型</label><input v-model="form.statType" placeholder="如 count / sum / avg" class="form-input mono" /></div>
  ...
  search.value
    ? items.value.filter(
        (i) =>
          ...
          (i.queryFlag || '').toLowerCase().includes(search.value.toLowerCase()) ||
          (i.statType || '').toLowerCase().includes(search.value.toLowerCase()),
      )
    : items.value,
)

function openCreate() {
  form.value = { name: '', queryFlag: '', statType: '' }
...
async function deleteItem(item: Item) {
  if (await confirmMsg('确定删除该统计定义？')) delM.mutate(item.id)
}
function loadData() {
  qc.invalidateQueries({ queryKey: qk })
}
