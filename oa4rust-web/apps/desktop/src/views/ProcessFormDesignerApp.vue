  ...
  <div class="crud-view">
    ...
    <div v-if="showCreate||showEdit" class="modal-overlay" @click.self="closeModal">
      <div class="modal glass-card">
        <h3>{{ showEdit?'编辑':'新建' }}流程表单</h3>
        <div class="form-group"><label>名称</label><input v-model="form.name" placeholder="表单名称" class="form-input" /></div>
...
async function deleteItem(item: Item) {
  if (await confirmMsg('确定删除该表单？')) delM.mutate(item.id)
}
function loadData() {
  qc.invalidateQueries({ queryKey: qk })
}
